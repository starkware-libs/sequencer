...
class ResyncExecutor:
    ...
    async def execute(self, trigger: ResyncTriggerPayload) -> int:
        is_repeated_trigger = shared.record_resync_cause(
            trigger["tx_hash"],
            trigger["source_block_number"],
            trigger["rollback_block_number"],
            trigger["reason"],
        )
        if is_repeated_trigger:
            next_start_block = trigger["source_block_number"] + 1
        else:
            next_start_block = trigger["rollback_block_number"]

        self._get_sequencer_manager().scale_to_zero()
        reports.write_pre_resync_reports(
            trigger_tx_hash=trigger["tx_hash"],
            trigger_block=trigger["rollback_block_number"],
