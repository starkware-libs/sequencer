...
class _ResyncTracker:
    ...
    def record_cause(
        ...
        def _selected_start_block() -> int:
            return failing_block_number + 1 if is_repeated_trigger else rollback_block_number

        entry = self.certain_failures.get(tx_hash)
        is_repeated_trigger = entry is not None
        if entry:
            rollback_block_number = _selected_start_block()
            entry["count"] += 1
            entry["failing_block_number"] = failing_block_number
            entry["rollback_block_number"] = rollback_block_number
            entry["reason"] = reason
            return True, rollback_block_number
        ...
        self.resync_causes[tx_hash] = {
            "tx_hash": tx_hash,
            "failing_block_number": failing_block_number,
            "rollback_block_number": rollback_block_number,
            "reason": reason,
            "count": 1,
        }
        return False, rollback_block_number
    ...
    def record_resync_cause(
        ...
        with self._lock:
            return self._resync.record_cause(
                tx_hash, failing_block_number, rollback_block_number, reason
            )
