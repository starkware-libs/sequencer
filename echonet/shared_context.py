...
def _find_archived_block_path(*, block_number: int, field: str) -> Optional[Path]:
    ...
    candidates = [p for p in matches if p.is_file() and p.parent.name.startswith("blocks_")]
    if not candidates:
        return None
...
class _ReportL2GasMismatchTracker:
    ...
    l2_gas_mismatches: Dict[str, JsonObject]
    ...
    def record_l2_gas_mismatch(
        ...
        *,
        ...
        self.l2_gas_mismatches[tx_hash] = {
            "tx_hash": tx_hash,
...
class _BlockStore:
    ...
    def _enforce_blocks_archives_size_cap(cls) -> None:
        ...
        archives = sorted(
            (
                Path(p)
                for p in glob.glob(str(root_dir / "blocks_*"))
                if Path(p).is_dir() and Path(p).name.startswith("blocks_")
            ),
    ...
    def get_report_snapshot(self) -> SnapshotModel:
        with self._lock:
            ...
            return SnapshotModel(
                ...
                l2_gas_mismatches=dict(self._l2_gas_mismatches.l2_gas_mismatches),
            )
