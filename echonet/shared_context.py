...
class SharedContext:
    def __init__(self) -> None:
        ...
        # Base values used for deterministic hash/root generation in echo_center.
        # These are updated whenever echo_center refreshes its base (e.g., after resync).
        self.base_block_number: Optional[int] = None
        self.base_block_hash_hex: Optional[str] = None
    ...
    # --- Base block helpers (echo_center) ---
    def set_base_block_info(self, base_block_number: int, base_block_hash_hex: str) -> None:
        with self.lock:
            self.base_block_number = int(base_block_number)
            self.base_block_hash_hex = str(base_block_hash_hex)

    def get_base_block_info(self) -> tuple[int, int]:
        """
        Returns (base_block_number, base_block_hash_int).
        If not initialized yet, falls back to (consts.START_BLOCK_DEFAULT, 0).
        """
        with self.lock:
            bn = int(self.base_block_number) if self.base_block_number is not None else int(consts.START_BLOCK_DEFAULT)
            h_hex = self.base_block_hash_hex
            h_int = int(h_hex, 16) if h_hex else 0
            return bn - 1, h_int

    def get_last_proved_block_callback(self) -> tuple[int, int]:
        # Echo-net's L1Manager uses this callback for stateBlockNumber/stateBlockHash.
