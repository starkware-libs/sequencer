...
from report_models import SnapshotModel
...
class _ResyncTracking:
    ...
    resync_causes: Dict[str, JsonObject]
    certain_failures: Dict[str, JsonObject]
    ...
    def record_cause(self, tx_hash: str, block_number: int, reason: str) -> bool:
        if tx_hash in self.certain_failures:
            self.certain_failures[tx_hash]["count"] = (
                self.certain_failures[tx_hash].get("count", 1) + 1
            )
...
class _BlockStore:
    ...
    # --- Block store API ---
    def store_block(
        self, block_number: int, *, blob: JsonObject, block: JsonObject, state_update: JsonObject
    ) -> None:
...
class SharedContext:
    ...
    def get_report_snapshot(self) -> SnapshotModel:
        with self._lock:
            configured_start_block = consts.CONFIG.blocks.start_block
