...
class _TxTracking:
    ...
    pending_live: Dict[str, int]  # tx_hash -> source block number
    ever_pending: Set[str]  # cumulative set of tx hashes ever observed pending
    committed: Dict[str, int]  # cumulative map: tx_hash -> commit block number
    pending_not_committed_count: int  # incremental: ever_pending - committed
    ...
    def record_sent(self, tx_hash: str, source_block_number: int) -> None:
        self.pending_live[tx_hash] = source_block_number
        if tx_hash not in self.ever_pending and tx_hash not in self.committed:
            self.ever_pending.add(tx_hash)
            self.pending_not_committed_count += 1

    def mark_committed(self, tx_hash: str, block_number: int) -> None:
        already_committed = tx_hash in self.committed
        self.committed[tx_hash] = block_number
        if (
            (not already_committed)
            and (tx_hash in self.ever_pending)
            and self.pending_not_committed_count > 0
        ):
            self.pending_not_committed_count -= 1
        self.pending_live.pop(tx_hash, None)

    def forwarded_block_once(self, block_number: int, tx_count: int) -> None:
...
@dataclass(slots=True)
class _ResyncTracking:
    """Tracks resync triggers and promotes repeated errors to 'certain failures'."""

    resync_causes: Dict[str, JsonDict]
    certain_failures: Dict[str, JsonDict]

    @classmethod
    def empty(cls) -> "_ResyncTracking":
    ...
    def record_cause(self, tx_hash: str, block_number: int, reason: str) -> bool:
        if tx_hash in self.certain_failures:
            self.certain_failures[tx_hash]["count"] = (
                int(self.certain_failures[tx_hash].get("count", 1)) + 1
            )
...
class _BlockStore:
    ...
    blocks: Dict[int, JsonDict]  # block_number -> {blob, block, state_update}
    fgw_blocks: Dict[int, Any]  # feeder-gateway block_number -> raw block object
    ...
    def snapshot_items(self) -> List[tuple[int, JsonDict]]:
        if not self.blocks:
            return []
        return sorted(
            ((int(bn), dict(entry)) for bn, entry in self.blocks.items()), key=lambda p: p[0]
        )

    # --- Block store API ---
    def store_block(
        self, block_number: int, *, blob: JsonDict, block: JsonDict, state_update: JsonDict
    ) -> None:
    ...
    @staticmethod
    def write_snapshot_items_to_disk(snapshot_items: List[tuple[int, JsonDict]]) -> None:
        if not snapshot_items:
            return
        ts_suffix = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
        try:
            base_dir = consts.CONFIG.paths.log_dir / f"blocks_{ts_suffix}"
...
class _Progress:
    ...
    base_block_number: Optional[int]
    base_block_hash_hex: Optional[str]

    @classmethod
    def empty(cls) -> "_Progress":
        return cls(
    ...
    def set_initial_start_block_if_absent(self, block_number: int) -> None:
        ...
        if self.initial_start_block is None:
            self.initial_start_block = bn
        if self.current_start_block is None:
            self.current_start_block = bn
    ...
    def set_base_block_info(self, base_block_number: int, base_block_hash_hex: str) -> None:
        self.base_block_number = base_block_number
    ...
    def get_base_block_info(self, *, default_start_block: int) -> tuple[int, int]:
        ...
        echo_center sets base_block_number to the configured "start block", but L1Manager wants
        the previous block number (start_block - 1).
        """
        bn = self.base_block_number if self.base_block_number is not None else default_start_block
...
class SharedContext:
    ...
    def __init__(self) -> None:
        ...
        self._tx = _TxTracking.empty()
        self._errors = _ErrorTracking.empty()
        self._resync = _ResyncTracking.empty()
        self._blocks = _BlockStore.empty()
        self._progress = _Progress.empty()
    ...
    def record_forwarded_block(self, block_number: int, tx_count: int) -> None:
        with self._lock:
            self._tx.forwarded_block_once(block_number, tx_count)

    def mark_committed_tx(self, tx_hash: str, block_number: int) -> None:
        with self._lock:
            self._tx.mark_committed(tx_hash, block_number)

    def get_sent_block_number(self, tx_hash: str) -> int:
        with self._lock:
            return self._tx.pending_live[tx_hash]

    def get_resync_evaluation_inputs(self) -> tuple[Dict[str, JsonDict], Dict[str, int]]:
        """
        ...
        - pending_live (tx_hash -> source block number)
        """
        with self._lock:
            return dict(self._errors.gateway_errors_live), dict(self._tx.pending_live)
    ...
    def add_mainnet_revert_error(self, tx_hash: str, error: str) -> None:
        with self._lock:
            self._errors.add_mainnet_revert_error(tx_hash, error)

    def add_mainnet_revert_errors(self, errors: Mapping[str, str]) -> None:
        with self._lock:
            for tx_hash, err in errors.items():
                self._errors.add_mainnet_revert_error(tx_hash, err)

    def add_echonet_revert_error(self, tx_hash: str, error: str) -> None:
        with self._lock:
            self._errors.add_echonet_revert_error(tx_hash, error)
    ...
    def clear_for_resync(self) -> None:
        """Clear transient state for a new run while preserving cumulative stats."""
        with self._lock:
            snapshot_items = self._blocks.snapshot_items()
            self._tx.pending_live.clear()
            self._errors.clear_transient()
            self._blocks.clear_transient()
    ...
    def store_block(
        ...
        with self._lock:
            self._blocks.store_block(
                block_number, blob=blob, block=block, state_update=state_update
            )
    ...
    # --- Reporting ---
    def get_report_snapshot(self) -> JsonDict:
        with self._lock:
            configured_start_block = int(consts.CONFIG.blocks.start_block)
            current_block = self._progress.sender_current_block
            initial_start_block = self._progress.get_initial_start_block(
                default_start_block=configured_start_block
            )
            current_start_block = self._progress.get_current_start_block(
                default_start_block=configured_start_block
            )
            blocks_sent_count = (
                max(0, int(current_block) - int(initial_start_block))
            ...
            return {
                "sent_tx_hashes": dict(self._tx.pending_live),
                ...
                "total_sent_tx_count": int(self._tx.total_forwarded_tx_count),
                "pending_total_count": len(self._tx.ever_pending),
                "pending_not_committed_count": int(self._tx.pending_not_committed_count),
    ...
    def set_base_block_info(self, base_block_number: int, base_block_hash_hex: str) -> None:
        with self._lock:
            self._progress.set_base_block_info(base_block_number, base_block_hash_hex)

    def get_base_block_info(self) -> tuple[int, int]:
        with self._lock:
            return self._progress.get_base_block_info(
                default_start_block=int(consts.CONFIG.blocks.start_block)
            )

    def get_current_start_block(self, *, default_start_block: int) -> int:
        with self._lock:
            return self._progress.get_current_start_block(
                default_start_block=int(default_start_block)
            )
...
_l1_client = L1Client(api_key=consts.CONFIG.l1.alchemy_api_key)
