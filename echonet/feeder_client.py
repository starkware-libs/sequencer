...
class FeederClient:
    """Synchronous client for the Starknet Feeder Gateway backed by requests."""
    ...
    def proxy_raw(self, endpoint: str, params: Dict[str, Any]) -> Tuple[bytes, int, Dict[str, str]]:
        """Perform a raw GET to the feeder and return (content, status, headers_dict)."""
        resp = self.session.get(
        ...
        # Filter to simple str->str headers to avoid issues with Flask Response
        headers_out: Dict[str, str] = {k: v for k, v in resp.headers.items()}
        return resp.content, resp.status_code, headers_out
