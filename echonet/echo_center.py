...
_BASE_BLOCK_NUMBER = consts.START_BLOCK_DEFAULT
...
def _init_base_values() -> None:
    """Initialize base block_hash and state_root from feeder for (consts.START_BLOCK_DEFAULT - 1)."""
...
def _refresh_base_if_needed() -> None:
    """If consts.START_BLOCK_DEFAULT changed (after resync), refresh base number and hashes."""
    global _BASE_BLOCK_NUMBER
    if _BASE_BLOCK_NUMBER != consts.START_BLOCK_DEFAULT:
        _BASE_BLOCK_NUMBER = consts.START_BLOCK_DEFAULT
        _init_base_values()
...
def _transform_blob_for_storage(blob: dict):
    ...
    bn_for_meta = consts.START_BLOCK_DEFAULT
...
def write_blob():
    ...
    _refresh_base_if_needed()
    block_number = int(blob["block_number"])
    shared.set_last_block(block_number)
...
@app.route("/eth_getBlockByNumber", methods=["GET"])
def eth_get_block_by_number():
    params = request.get_json(silent=True) or request.args.to_dict(flat=True)
    print(f"[FLASK] eth_getBlockByNumber params: {params}")
    payload = l1_manager.get_block_by_number(params)
    return _json_response(payload, 200)


@app.route("/eth_getLogs", methods=["GET"])
def eth_get_logs():
    params = request.get_json(silent=True) or request.args.to_dict(flat=True)
    print(f"[FLASK] eth_getLogs params: {params}")
    payload = l1_manager.get_logs(params if isinstance(params, dict) else {})
    return _json_response(payload, 200)


@app.route("/eth_blockNumber", methods=["GET"])
def eth_block_number():
    params = request.get_json(silent=True) or request.args.to_dict(flat=True)
    print(f"[FLASK] eth_blockNumber params: {params}")
    payload = l1_manager.get_block_number()
    return _json_response(payload, 200)
