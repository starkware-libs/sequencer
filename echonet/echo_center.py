...
# They are taken from a random block from Mainnet.
...
# This is taken from a random block from Mainnet.
...
class BlobTransformer:
    ...
    def _halve_gas_prices(v: str) -> str:
        """By shifting the hex string right by 1, the gas prices are halved."""
...
class EchoCenterService:
    ...
    @staticmethod
    def _empty_response(status: int = requests.codes.ok) -> Response:
    ...
    def handle_write_blob(self) -> Response:
        """
    ...
    def handle_write_pre_confirmed_block(self) -> Response:
        self.flask_logger.debug("Received pre-confirmed block")
        return self._empty_response(requests.codes.ok)

    def handle_report_snapshot(self) -> Response:
    ...
    def handle_block_dump(self) -> Response:
    ...
    def handle_get_block(self) -> Response:
        """
    ...
    def handle_get_state_update(self) -> Response:
        args = request.args.to_dict(flat=True)
    ...
    def handle_get_signature(self) -> Response:
        args = request.args.to_dict(flat=True)
        bn_raw = args.get("blockNumber")
        bn_parsed = self._parse_block_number(bn_raw)
    ...
    def handle_get_class_by_hash(self) -> Response:
        args = request.args.to_dict(flat=True)
        bn_raw = args.get("blockNumber")
    ...
    def handle_get_compiled_class_by_class_hash(self) -> Response:
        args = request.args.to_dict(flat=True)
        bn_raw = args.get("blockNumber")
        class_hash = args["classHash"]
