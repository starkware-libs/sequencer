...
class EchoCenterService:
    ...
    def _json_response(payload: Any, status: int = consts.HTTP_OK) -> Response:
        # Important: avoid double-encoding JSON-RPC payloads.
        # Some internal helpers return already-serialized JSON strings/bytes.
    ...
    def _init_base_values(self) -> None:
        ...
        self.shared.set_base_block_info(self._base_block_number, self._base_block_hash_hex)
    ...
    def _transform_blob_for_storage(self, blob: dict):
        ...
        for entry in tx_entries:
            ...
            elif tx_type == "DECLARE":
                declare_pass_through = [
                    "class_hash",
                    "compiled_class_hash",
                ]
                tx_obj.update({k: tx[k] for k in declare_pass_through})
            else:
    ...
    def handle_l1(self):
        ...
        if method == "eth_call":
            payload = self.l1_manager.get_call(params if isinstance(params, dict) else {})
