...
class L1Manager:
    ...
    def get_block_number(self) -> dict:
        """Returns a block number for the Rust scraper, or None if nothing is known yet."""
        self._promote_ready_gated_txs()
        if self.blocks:
            latest_block_number = max(self.blocks.keys())
            finalized_block_number = (
                latest_block_number + L1Manager.L1_SCRAPER_FINALITY_CONFIG_VALUE
            )
            self.logger.debug(
                f"get_block_number: returning {finalized_block_number}"
                f" (latest={latest_block_number} +"
                f" finality={L1Manager.L1_SCRAPER_FINALITY_CONFIG_VALUE})"
            )
            return rpc_response(hex(finalized_block_number))

        if self._gated_txs:
            fence = min(tx.block_number for tx in self._gated_txs) - 1
            self.logger.debug(
                f"get_block_number: no exposed blocks, returning fence={fence}"
                f" (min_gated={fence + 1})"
            )
            return rpc_response(hex(fence))

        self.logger.debug("get_block_number: no blocks known, returning None")
