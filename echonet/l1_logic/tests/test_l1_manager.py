...
class TestL1Manager(unittest.TestCase):
    ...
    def test_get_block_number_gated_until_threshold(self):
        """get_block_number returns gated_block - 1 while gated, then exposed + finality once exposed."""
        ...
        with patch.object(L1Blocks, "find_l1_block_for_tx") as mock_find:
            ...
            fence = hex(l1_block_number - 1)
            exposed = hex(l1_block_number + L1Manager.L1_SCRAPER_FINALITY_CONFIG_VALUE)

            # Echonet below threshold: block is gated, get_block_number returns fence (gated - 1).
            ...
            self.assertEqual(manager_below.get_block_number()["result"], fence)
            self.assertEqual(manager_below.get_block_number()["result"], fence)
            self.assertEqual(manager_below.get_block_number()["result"], fence)

            # Echonet at threshold: block exposed, get_block_number returns exposed + finality.
            ...
            self.assertEqual(manager_at.get_block_number()["result"], exposed)
            self.assertEqual(manager_at.get_block_number()["result"], exposed)
            self.assertEqual(manager_at.get_block_number()["result"], exposed)
