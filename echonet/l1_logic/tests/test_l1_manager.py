...
class TestL1Manager(unittest.TestCase):
    ...
    def test_cleanup_old_blocks(self):
        ...
        # When requesting b30, keeps blocks >= b30 - 20 = b10.
        self.manager.get_block_by_number(hex(b30))

        self.assertIn(b10, self.manager.blocks, "Block b10 should be kept (within cleanup buffer)")
        self.assertIn(b20, self.manager.blocks, "Block b20 should be kept")
        self.assertIn(b30, self.manager.blocks, "Block b30 should be kept")

        # Requesting b51 cleans up b10 and b20 (b51 - 20 = b31, so blocks < b31 are removed).
        b51 = DEFAULT_L1_BLOCK_NUMBER + 51
        self.manager.get_block_by_number(hex(b51))
        result = self.manager.get_block_by_number(hex(b10))
        self.assertEqual(result["result"], self.manager.default_l1_block(hex(b10)))
        result = self.manager.get_block_by_number(hex(b20))
        self.assertEqual(result["result"], self.manager.default_l1_block(hex(b20)))
