...
class L1Blocks:
    ...
    def _find_block_near_timestamp(
        ...
        estimated_block_number = reference_block.number - (
            time_diff // ETHEREUM_AVERAGE_SECONDS_PER_BLOCK
        )
        ...
        for iteration in range(L1Blocks._MAX_BLOCK_SEARCH_ITERATIONS):
            ...
            if abs(timestamp_diff) <= L1Blocks._MAX_ALLOWED_TIMESTAMP_DIFF_SECONDS:
                logger.debug(
                    f"Found block {estimated_block_number} (diff: {timestamp_diff}s) after {iteration + 1} iterations"
                )
            ...
            estimated_block_number -= block_adjustment
            estimated_block_number = max(0, estimated_block_number)

            logger.debug(
                f"Iteration {iteration + 1}: diff={timestamp_diff}s, moved to block {estimated_block_number}"
            )
