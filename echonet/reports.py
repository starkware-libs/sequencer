...
def _fmt_utc_ts(epoch_seconds: int | None) -> str:
...
def _render_kv(lines: list[str], items: Sequence[tuple[str, str]], *, pad: int = 26) -> None:
    for k, v in items:
...
class PreResyncContext:
    """Metadata for a pre-resync report bundle (who/what/when triggered it)."""
...
class SnapshotTextReport:
    ...
    def render(self) -> str:
        ...
        _render_kv(
            lines,
            [
                ("Start block (configured)", str(s.start_block)),
                (
                    "Initial start block",
                ...
                ("First block timestamp", _fmt_utc_ts(s.first_block_timestamp)),
                ("Latest block timestamp", _fmt_utc_ts(s.latest_block_timestamp)),
                (
                ...
                (
                    "Forward rate",
                    _fmt_rate(s.total_sent_tx_count, s.timestamp_diff_seconds, unit="tx/s"),
                ),
            ],
        )
        ...
        _render_kv(
            ...
            [
                (
                    "Committed tx hashes",
                    f"{s.committed_count} ({_pct(s.committed_count, s.pending_total_count)})",
                ),
                (
                    "Pending tx hashes (not committed yet)",
                    f"{s.pending_not_committed_count} ({_pct(s.pending_not_committed_count, s.pending_total_count)})",
                ),
                (
                    "Gateway errors (live)",
                    f"{len(s.gateway_errors)} ({_pct(len(s.gateway_errors), s.total_sent_tx_count)})",
                ),
                (
                    "Reverted only on Mainnet",
                    f"{len(s.revert_errors_mainnet)} ({_pct(len(s.revert_errors_mainnet), s.total_sent_tx_count)})",
                ),
                (
                    "Reverted only on Echonet",
                    f"{len(s.revert_errors_echonet)} ({_pct(len(s.revert_errors_echonet), s.total_sent_tx_count)})",
                ),
        ...
        if not s.sent_tx_hashes:
            ...
            for tx_hash, src_bn in sorted(s.sent_tx_hashes.items(), key=lambda kv: kv[0]):
        ...
        if not s.resync_causes:
            ...
            for tx_hash, meta in sorted(s.resync_causes.items(), key=lambda kv: kv[0]):
                lines.append(
                    f"{tx_hash}: bn={meta.get('block_number')} reason={meta.get('reason')} count={meta.get('count')}"
                )
        ...
        if not s.certain_failures:
            ...
            for tx_hash, meta in sorted(s.certain_failures.items(), key=lambda kv: kv[0]):
                lines.append(
                    f"{tx_hash}: bn={meta.get('block_number')} reason={meta.get('reason')} count={meta.get('count')}"
                )
...
class RevertComparisonTextReport:
    ...
    def render(
        self, *, mainnet_reverts: Mapping[str, str], echonet_reverts: Mapping[str, str]
    ) -> str:
    ...
    def _render_grouped(title: str, grouped: Mapping[str, list[tuple[str, str]]]) -> list[str]:
        ...
        for error_type, txs in sorted(grouped.items(), key=lambda kv: (-len(kv[1]), kv[0])):
            lines.append("")
            lines.append(f"-- {error_type} ({len(txs)} txs, {_pct(len(txs), total)}) --")
...
def write_pre_resync_reports(
    *,
    ...
    PreResyncReportWriter(log_dir=consts.CONFIG.paths.log_dir).write(
