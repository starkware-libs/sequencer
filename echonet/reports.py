...
class SnapshotTextReport:
    ...
    def render(self) -> str:
        ...
        if os_stats:
            completed = os_stats.get("completed", 0)
            failed = os_stats.get("failed", 0)
            dropped = os_stats.get("dropped", 0)
            deferred_events = os_stats.get("deferred_events", 0)
            abandoned = os_stats.get("abandoned", 0)
            skipped = os_stats.get("skipped", 0)
            ...
            recent_failures = os_stats.get("recent_failures", [])
            if recent_failures:
                lines.append("")
...
class _SnapshotReportRollup:
    ...
    def from_snapshot(cls, s: SnapshotModel) -> "_SnapshotReportRollup":
        ...
        os_completed_count = os_stats_for_sev.get("completed", 0)
        os_failed_count = os_stats_for_sev.get("failed", 0)
        os_dropped_count = os_stats_for_sev.get("dropped", 0)
        os_abandoned_count = os_stats_for_sev.get("abandoned", 0)
        os_skipped_count = os_stats_for_sev.get("skipped", 0)
        ...
        os_failures_rows = [dict(entry) for entry in os_stats_for_sev.get("recent_failures", [])]
