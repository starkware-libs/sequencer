...
def _install_block_hash_cli_into_pod(
    ...
    while time.time() <= deadline:
        try:
            ...
            _run([*pod_exec, "mkdir", "-p", str(Path(remote_binary).parent)])
            _run([*kube, "cp", str(local_binary), f"{pod_name}:{remote_binary}", "-c", "echonet"])
            _run([*pod_exec, "chmod", "+x", remote_binary])
            logger.info(
                f"Installed block-hash CLI into pod: {local_binary} -> {pod_name}:{remote_binary}"
            )
            return
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.block_hash_cli_binary == "skip":
        ...
        if local_block_hash_cli is None:
            logger.warning(
    ...
    if local_block_hash_cli is not None:
