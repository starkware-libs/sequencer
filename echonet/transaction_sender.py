        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if valid_txs:
                    ...
                    if other_txs:
                        for tx in other_txs:
                            await _forward_one(tx, src_bn=block_number, src_ts=ts_val)
                    shared.increment_total_sent(len(valid_txs))
            ...
            if gw:
                txh, info = next(iter(gw.items()))
                bn = int(info.get("block_number"))
                resp = info.get("response")
                threshold = int(block_number) - 10
                if bn <= threshold:
                    trigger_tx_hash = str(txh)
                    trigger_block = bn
                    trigger_reason = f"Gateway error: {resp}"

            if trigger_tx_hash is None:
                sent_map: Dict[str, int] = snapshot.get("sent_tx_hashes") or {}
                if sent_map:
                    threshold = int(block_number) - 10
                    # candidates with age >= 10 blocks
                    aged = [(txh, bn) for txh, bn in sent_map.items() if int(bn) <= threshold]
                    if aged:
                        # Choose the oldest one
                        txh_oldest, bn_oldest = min(aged, key=lambda x: int(x[1]))
                        trigger_tx_hash = str(txh_oldest)
                        trigger_block = int(bn_oldest)
                        trigger_reason = "Still pending after >=10 blocks"

            if trigger_tx_hash is not None and trigger_block is not None:
                logging.warning(
                    f"Resync triggered by tx {trigger_tx_hash} at block {trigger_block}: {trigger_reason}"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        payload = {
                            **base_snapshot,
                            "sent_empty": len(base_snapshot.get("sent_tx_hashes") or {}) == 0,
                        ...
                        with contextlib.redirect_stdout(buf_snapshot):
                            print(header, end="")
                            reports._print_snapshot_from_data(payload)  # type: ignore[attr-defined]
                        # Reverts comparison (include details in file)
                        with contextlib.redirect_stdout(buf_reverts):
                            print(header, end="")
                            reports._compare_reverts_from_data(payload)  # type: ignore[attr-defined]
                        os.makedirs(str(consts.LOG_DIR), exist_ok=True)
                        ts_suffix = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
                        out_snapshot = os.path.join(
                            str(consts.LOG_DIR), f"report_snapshot_{ts_suffix}.log"
                        )
                        out_reverts = os.path.join(
                            str(consts.LOG_DIR), f"report_reverts_{ts_suffix}.log"
                        )
                        ...
                        logging.warning(f"Failed to write reports before resync: {rep_err}")
                ...
                try:
                    ...
                    logging.error(f"Failed to resync sequencer: {err}")
                try:
            ...
            if effective_sleep > 0:
                try:
                    await asyncio.wait_for(stop_event.wait(), timeout=effective_sleep)
        ...
        *,
        ...
        def _runner() -> None:
            try:
                ...
                # Perform initial revert->scale dance to align sequencer with starting block
