...
from echonet.manage_sequencer import SequencerManager
...
class ResyncPolicy:
    ...
    def evaluate(
        ...
        return ResyncTrigger(
            ...
            reason=f"Resync after >= {CONFIG.resync.error_threshold} errors: {reason_first}",
        )
...
class ResyncExecutor:
    ...
    async def execute(self, trigger: ResyncTrigger) -> int:
        ...
        _get_sequencer_manager().scale_to_zero()
        ...
        await loop.run_in_executor(
            None, lambda: _get_sequencer_manager().resync(block_number=next_start_block)
        )
