...
class TxSelector:
    ...
    @staticmethod
    def filter_blocked(
        transactions: Sequence[JsonDict], blocked_senders: Set[str]
    ) -> list[JsonDict]:
        return [tx for tx in transactions if tx.get("sender_address") not in blocked_senders]

    @staticmethod
    def deploy_account_first(transactions: Sequence[JsonDict]) -> list[JsonDict]:
        deploy_txs: list[JsonDict] = []
        other_txs: list[JsonDict] = []
...
class TxTransformer:
    ...
    def prepare_for_forwarding(self, env: TxEnvelope) -> Optional[JsonDict]:
        tx = env.tx
        tx_type = tx.get("type")
...
class HttpForwarder:
    ...
    async def forward(self, tx: JsonDict, *, source_block_number: int) -> None:
...
class ResyncPolicy:
    ...
    def evaluate(
        self,
        *,
        gateway_errors: Dict[str, JsonDict],
...
class TransactionSenderService:
    ...
    async def run(self, config: SenderConfig) -> None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async def producer() -> None:
                ...
                current_start_block = int(config.start_block)
                while not self._stop_event.is_set():
                    ...
                    if trigger:
                        ...
                        current_start_block = int(block_number)
                        continue
    ...
    async def _sleep_between_blocks(
        ...
        *,
        ...
        if (int(current_block) - int(start_block)) < int(
            consts.CONFIG.sleep.initial_slow_blocks_count
        ):
            effective_sleep = (
...
class TransactionSenderRunner:
    ...
    def start(self, *, config: SenderConfig) -> bool:
        ...
        def _runner() -> None:
            try:
                initial_revert_then_restore(int(config.start_block))

                shared.set_initial_start_block_if_absent(int(config.start_block))
                shared.set_current_start_block(int(config.start_block))
