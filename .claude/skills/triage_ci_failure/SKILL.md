...
**Then verify the run isn't stale — do this before trusting any pasted link.** This repo uses Graphite stacks; branches get force-pushed constantly, so a pasted run URL is often already obsolete. Triaging a stale run yields a confidently *wrong* answer ("just merge-gatekeeper noise, harmless") while the live head is genuinely red. Get the run's `head_sha`, find its PR, and compare to the PR's current `head.sha` (`GET /pulls/{pr}`). If they differ, pivot to the check-runs at the current head and say so. Only triage the pasted run when the SHAs match (or there's no PR, e.g. a push to `main`).
...
If it's still failing on the latest attempt, continue below.
