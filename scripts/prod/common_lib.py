...
def run_in_parallel(
    ...
    thread is swallowed by the executor and would not stop the process). Exceptions are collected
    per item; if any item failed, a summary is printed and the process exits with code 1 once all
    items have settled.
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_index = {
            executor.submit(run_one, index, item): index for index, item in enumerate(items)
        }
