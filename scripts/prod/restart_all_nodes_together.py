...
def get_logs_explorer_url(
    ...
    return (
        ...
        f"resource.labels.namespace_name:%22{namespace}%22%0A"
        f"textPayload%20%3D~%20%22DECISION_REACHED.*proposer%200x0*{validator_id}%22??project={project_name}"
    )
...
def main():
    usage_example = """
Examples:
  # Restart all nodes to at the next block after current feeder block (default: All_At_Once strategy)
  ...
  %(prog)s --namespace-list apollo-sepolia-integration-0 apollo-sepolia-integration-1 --cluster-list cluster1 cluster2 -f feeder.integration-sepolia.starknet.io
    ...
    logs_explorer_urls = None
    if args.restart_strategy == RestartStrategy.ONE_BY_ONE:
        logs_explorer_urls = [
            get_logs_explorer_url(
                namespace, get_validator_id(namespace, context), args.project_name
            )
