...
HEIGHT_BLOCK_RE = re.compile(r"(?:block[\s_]?number|block|height)", re.IGNORECASE)
...
def height_match(entry: Dict[str, Any], height_str: str) -> bool:
    ...
    height_block_match = HEIGHT_BLOCK_RE.search(blob)
    if height_block_match:
        rest_of_blob = blob[height_block_match.end() :]
        if re.search(rf"\b{re.escape(height_str)}\b", rest_of_blob):
            return True
