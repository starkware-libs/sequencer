...
def height_match(entry: Dict[str, Any], height_str: str) -> bool:
    ...
    return height_str in (jp.get("message") or "")
...
def get_round(entry: Dict[str, Any]) -> Optional[int]:
    ...
    match_alt = re.search(r"\bround:\s*(\d+)\b", msg)
    if match_alt:
...
def proposer_of(entries: List[Dict[str, Any]], round_num: int) -> Optional[str]:
    for e in entries:
        if get_round(e) == round_num:
            msg = (e.get("jsonPayload") or {}).get("message") or ""
...
def proposer_ns_of(
    ...
    for e in entries:
        if get_round(e) == round_num:
            msg = (e.get("jsonPayload") or {}).get("message") or ""
            if "Starting round" in msg and "as Proposer" in msg:
                return get_namespace(e)
    pid = proposer_of(entries, round_num)
...
def ntxs_for_round(
    ...
    pns = proposer_ns_of(entries, ns_by_id, round_num)
    if not pns:
        return ""
...
def collect_round_metadata(round_number: int, consensus_data: ConsensusData) -> RoundMetadata:
    ...
    proposer = proposer_of(consensus_data.all_log_entries, round_number)
