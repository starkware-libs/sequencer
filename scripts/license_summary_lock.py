...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description="Get the license information from crates.io for ALL Cargo dependencies (including transitive) from Cargo.lock",
    )
    parser.add_argument(
        "--lock_file",
        type=str,
...
def get_all_external_dependencies(lock_file: str) -> List[str]:
    """Extract all external (non-internal) dependencies from Cargo.lock.

    This includes both direct dependencies and all transitive dependencies.
    """
    ...
    # Extract packages from crates.io registry (external dependencies)
    external_deps = set()
    for package in packages:
        ...
        if "registry+" in source:
            # Exclude internal workspace crates
            if not name.startswith("apollo_") and not name.startswith("starknet_"):
...
def main():
    ...
    try:
        dependencies = get_all_external_dependencies(args.lock_file)
        ...
        print(
            f"Found {len(dependencies)} external dependencies (including transitive).\n"
        )
        ...
        report_lines.append("LICENSE DETAILS (ALL DEPENDENCIES INCLUDING TRANSITIVE)")
        report_lines.append(separator)
        ...
        print(f"Error: File '{args.lock_file}' not found.", file=sys.stderr)
        sys.exit(1)
    except KeyError as e:
        print(f"Error: Missing expected key in Cargo.lock file: {e}", file=sys.stderr)
        sys.exit(1)
    except IOError as e:
