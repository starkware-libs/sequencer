    ...
    for service in services:
        ...
        try:
            if controller_lower == "statefulset":
                obj = apps_v1.read_namespaced_stateful_set(resource_name, namespace=namespace)
            elif controller_lower == "deployment":
                obj = apps_v1.read_namespaced_deployment(resource_name, namespace=namespace)
            else:
        ...
        while elapsed < timeout_seconds:
            ...
            print(f"⚠️ Timeout waiting for {controller} {resource_name} to become ready.")
            sys.exit(1)
