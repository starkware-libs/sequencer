...
def convert_to_legacy_format(services: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Convert sequencer service configs to legacy JSON format.
...
def check_manifest_files(deployment_config_path: str, workspace: str, namespace: str) -> None:
    ...
    For sequencer, the path structure is:
    deployments/sequencer/dist/sequencer-{service_name}/{Controller}.sequencer-{service_name}-{controller}.k8s.yaml
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check manifest files and wait for K8s services to be ready (sequencer)."
    )
    parser.add_argument(
        "--layout",
        type=str,
    ...
    # Load sequencer configs and convert to legacy format
