...
def verify_data_directory(pod_name: str) -> None:
    print(f"📂 Verifying /data contents in pod {pod_name}...")
    try:
        ...
        if result.stdout.strip():
            print(f"✅ /data in pod {pod_name} contains files:\n{result.stdout}")
        else:
            print(f"⚠️ /data in pod {pod_name} is empty.")
    except subprocess.CalledProcessError as e:
        print(f"❌ Failed to inspect /data in pod {pod_name}: {e}")

    try:
        result = subprocess.run(
            ["kubectl", "exec", pod_name, "--", "ls", "-la", "/data/batcher"],
            check=True,
            capture_output=True,
            text=True,
        )
        if result.stdout.strip():
            print(
                f"✅ /data/batcher in pod {pod_name} contains files:\n{result.stdout}"
            )
        else:
            print(f"⚠️ /data in pod {pod_name} is empty.")
    except subprocess.CalledProcessError as e:
