...
def verify_data_directory(pod_name: str) -> None:
    print(f"📂 Verifying contents of /data in pod {pod_name} recursively...")

    try:
        # Run `find` with `stat` to print each file with its size
        ...
        if not lines:
            print(f"⚠️ /data in pod {pod_name} is empty.")
            return
        ...
        print(f"\n📦 Total files: {len(lines)}, Total size: {total_size} bytes")
    except subprocess.CalledProcessError as e:
        print(f"❌ Failed to recursively list /data in pod {pod_name}: {e}")
...
def main(
    ...
    verify_data_directory(
        get_pod_name(get_service_label(node_type, state_sync_service))
    )
    ...
    try:
        config_output = subprocess.check_output(
            ["kubectl", "get", "configmap", "sequencer-node-config", "-o", "yaml"],
            text=True,
        )
        print("🧾 ConfigMap sequencer-node-config:")
        print(config_output)
    except subprocess.CalledProcessError as e:
        print("⚠️ Failed to fetch ConfigMap:", e)
    if exit_code != 0:
        print("❌ Sequencer simulator failed!")

        print("📡 Fetching /monitoring/metrics before exiting")

        try:
            metrics_output = subprocess.check_output(
                ["curl", "-s", "http://127.0.0.1:8082/monitoring/metrics"],
                text=True,
            )
            print("📊 /monitoring/metrics output:")
            print(metrics_output)
        except subprocess.CalledProcessError as e:
            print("⚠️ Failed to fetch metrics:", e)
