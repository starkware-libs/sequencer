...
def copy_state(pod_name: str, namespace: str, data_dir: str, verbose: bool = False) -> None:
    ...
    try:
        exec_cmd = ["kubectl", "exec", pod_name, "-n", namespace, "--", "sh", "-c", "rm -rf /data/* 2>/dev/null || true"]
        if verbose:
...
def wait_for_resource(controller: str, name: str, namespace: str, timeout: int = 180, verbose: bool = False) -> None:
...
def main(layout: str, namespace: str, data_dir: str, overlay: Optional[str] = None, verbose: bool = False) -> None:
    ...
    for controller, resource_name in resources_to_wait_for:
        wait_for_resource(controller=controller, name=resource_name, namespace=namespace, verbose=verbose)
...
if __name__ == "__main__":
    ...
    main(layout=args.layout, namespace=args.namespace, data_dir=args.data_dir, overlay=args.overlay, verbose=args.verbose)
