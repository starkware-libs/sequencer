...
def bfs_layers(start: str, internal_map: Dict[str, Path], include_dev: bool) -> List[List[str]]:
    ...
    while q:
        ...
        for crate in current_crates_layer:
            ...
            deps = parse_crate_deps(
                internal_map[crate], include_dev=include_dev, internal_names=internal_names
            )
            for dep in deps:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Repo root (default: project root)",
    )
    parser.add_argument(
        "--include-dev", action="store_true", help="Include dev-dependencies in traversal"
    )
    parser.add_argument(
        "--publish-order",
        action="store_true",
        help="Output publish order (reverse BFS) instead of BFS layers",
    )
