    ...
    def cmds(self, crates: Set[str], is_nightly: bool) -> List[List[str]]:
        ...
        if self == BaseCommand.TEST:
            ...
            for p in nightly_crates:
                # Omit --all-features: these packages have features that require a nightly
                # toolchain, so enabling them on stable would fail. A dedicated nightly CI
                # workflow checks these packages with their full feature set.
