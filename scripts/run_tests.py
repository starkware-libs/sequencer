...
class BaseCommand(Enum):
    ...
    def cmds(self, crates: Set[str]) -> List[List[str]]:
        ...
        if self == BaseCommand.TEST:
            ...
            build_cmds_no_feat = build_cmds(with_feature=False)

            # Only run cairo_native feature if the blockifier crate is modified.
            if CAIRO_NATIVE_CRATE_TRIGGERS.isdisjoint(crates):
                return (build_cmds_no_feat + port_cmds + run_cmds)

            print(f"Composing sequencer integration test commands with cairo_native feature.")
            build_cmds_with_feat = build_cmds(with_feature=True)
            return (build_cmds_no_feat + port_cmds + run_cmds + build_cmds_with_feat + run_cmds)
