...
def compare_revert_files(app_file: Path, send_file: Path) -> None:
    ...
    and vice versa. Print the full JSONL line (including the revert error) for each.
    """
    ...
    def _hash_to_line_map(file_path: Path) -> dict[str, str]:
    ...
    app_line_map = _hash_to_line_map(app_file)
    send_line_map = _hash_to_line_map(send_file)

    print("Reverted only in Echonet:")
    for h in only_in_app:
        print(app_line_map.get(h.lower(), h))
    print("Reverted only in Mainnet:")
    for h in only_in_send:
        print(send_line_map.get(h.lower(), h))
