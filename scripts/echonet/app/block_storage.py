...
class BlockStorage:
    ...
    def read_state_update(self, block_number: int) -> Optional[Dict[str, Any]]:
        path = self.root / f"{block_number}_state_update.json"
        if not path.is_file():
            return None
        try:
            with path.open("r", encoding="utf-8") as f:
    ...
    def write_block_and_state_update(
        ...
        su_path = self.root / f"{block_number}_state_update.json"
        self._write_json_atomic(su_path, state_update)
        # Update in-memory index
        self._insert_number(block_number)
        return block_path, su_path

    def write_blob(self, block_number: int, blob: Dict[str, Any]) -> pathlib.Path:
        path = self.root / f"{block_number}_blob.json"
    ...
    def purge(self) -> int:
        ...
        for p in self.root.glob("*.json"):
            try:
                ...
                pass
        for p in self.root.glob("*_state_update.json"):
            try:
                p.unlink()
            except Exception:
                pass
