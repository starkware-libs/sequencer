...
def _transform_blob_for_storage(blob: dict):
    ...
    if l1_gas_price is not None:
        ...
        l1_out["price_in_wei"] = _hex_div(l1_out.get("price_in_wei"), 1)
        l1_out["price_in_fri"] = _hex_div(l1_out.get("price_in_fri"), 1)
...
def get_block():
    ...
    if bn_parsed == "latest":
        ...
        if highest is None:
            return ("", 404)
        if header_only:
            # For headerOnly, do not fail if the full block file is missing or unreadable.
            # Prefer stored hash if present, otherwise compute a deterministic one.
            obj = _storage.read_block(highest)
            stored_bh = obj.get("block_hash") if isinstance(obj, dict) else None
