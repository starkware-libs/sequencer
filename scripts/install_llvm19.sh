...
# Bump together with the upstream script. Verification is enforced; the
# upstream script runs as root, so an unauthenticated curl|bash here would
# let any apt.llvm.org compromise execute arbitrary code in CI/Docker builds.
