...
class ExternalCommandCheck(Check):
    ...
    def run_check(self):
        for cmd in self.commands:
            run_command(" ".join(cmd), allow_error=False, print_output_on_error=True)
...
class TaploCheck(ExternalCommandCheck):
    def __init__(self):
        super().__init__([["bash", path.join(SCRIPTS_LOCATION, "taplo.sh")]])
...
def parse_args(all_checks: list[Check]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Presubmit script - fast parts.")

    parser.add_argument(
        "--stage",
        choices=[check.__name__ for check in all_checks].append("all"),
        default="all",
...
def get_checks_to_run(args: argparse.Namespace, all_checks: list[Check]) -> list[Check]:
    if args.stage == "all":
...
def main():
    ...
    args = parse_args(all_checks)

    checks = get_checks_to_run(args, all_checks)
