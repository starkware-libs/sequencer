#!/bin/bash
#
# Builds the starknet_tx_prover Docker image.
#
# Usage:
#   ./scripts/build_starknet_tx_prover.sh [OPTIONS]
#
# Options:
#   --image-tag TAG               Docker image tag (default: tx_prover:latest)
...
#   ./scripts/build_starknet_tx_prover.sh
#
#   # Build debug mode
#   ./scripts/build_starknet_tx_prover.sh --build-mode debug
...
# Default values.
DOCKERFILE_PATH="${REPO_ROOT}/crates/starknet_tx_prover/Dockerfile"
IMAGE_TAG="us-central1-docker.pkg.dev/starkware-dev/sequencer/tx-prover:latest"
BUILD_MODE="release"
RUSTFLAGS=""
DOCKER_BUILD_ARGS=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Builds the starknet_tx_prover Docker image."
            echo ""
            echo "Options:"
            echo "  --image-tag TAG               Docker image tag (default: tx_prover:latest)"
...
main() {
    echo ""
    info "Building starknet_tx_prover Docker image"
    echo ""
