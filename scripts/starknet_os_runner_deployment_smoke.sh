...
SPEC_VERSION_EXPECTED=$(grep -oP 'const SPEC_VERSION: &str = "\K[^"]+' \
    "$REPO_ROOT/crates/starknet_os_runner/src/server/rpc_impl.rs" 2>/dev/null \
    || echo "0.10.0")
...
build_valid_prove_request() {
    ...
    jq -c --argjson base "$BASE_BLOCK" --slurpfile tx "$TMP_DIR/prove_tx.json" \
...
main() {
    ...
    echo "Running Starknet OS Runner deployment smoke tests"
