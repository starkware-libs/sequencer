...
def create_benchmark_plot(csv_file: str, output_path: str = None, output_dir: str = None):
    ...
    def duration_per_fact(duration_millis: List[float], n_facts: List[int]) -> List[float]:
        return [ms * 1000 / n if n > 0 else 0 for ms, n in zip(duration_millis, n_facts)]

    read_duration_per_read_fact = duration_per_fact(read_duration_millis, n_read_facts)
    read_duration_per_new_fact = duration_per_fact(read_duration_millis, n_new_facts)
    compute_duration_per_new_fact = duration_per_fact(compute_duration_millis, n_new_facts)
    write_duration_per_new_fact = duration_per_fact(write_duration_millis, n_new_facts)
    total_duration_per_new_fact = duration_per_fact(block_duration_millis, n_new_facts)
