...
# Script to analyze LOCAL mainnet timing logs (not the remote JSON files)
# Usage: ./scripts/analyze_local_mainnet.sh

LOG_FILE="mainnet_timing_logs.txt"

if [ ! -f "$LOG_FILE" ]; then
    echo "No local mainnet logs found: $LOG_FILE"
    echo "Run: ./scripts/run_mainnet_node.sh 2>&1 | tee mainnet_timing_logs.txt"
    exit 1
fi

echo "========================================="
echo "LOCAL MAINNET TIMING ANALYSIS"
echo "========================================="
echo ""
...
# Use the correct log pattern from mainnet logs
...
try:
    with open('mainnet_timing_logs.txt', 'r') as f:
        lines = f.readlines()
...
if [ "$BLOCK_COUNT" -eq 0 ]; then
    echo "No blocks processed - mainnet sync may be very slow"
