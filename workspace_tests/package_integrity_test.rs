...
static CRATES_ALLOWED_TO_USE_TEST_CODE: [&'static str; 6] = [
...
fn test_no_testing_feature_in_business_logic() {
    let mut testing_feature_deps: Vec<_> = MEMBER_TOMLS
        ...
        .filter_map(|(package_name, toml)| {
            if let Some(ref dependencies) = toml.dependencies {
                Some((package_name, dependencies))
            } else {
                None
            }
        })
    ...
    assert!(
        ...
        "The following crates have (non-testing) dependencies with the 'testing' feature \
         activated: {testing_feature_deps:#?}. If the crate is a test crate, add it to {}.",
