...
# deployments/images/sequencer/dummy_exchange_rate_oracle.Dockerfile
...
RUN cargo chef cook --recipe-path recipe.json --bin dummy_exchange_rate_oracle
COPY . .
RUN cargo build --bin dummy_exchange_rate_oracle

FROM ubuntu:24.04 AS final_stage

ENV ID=1001
WORKDIR /app
COPY --from=builder /app/target/debug/dummy_exchange_rate_oracle ./target/debug/dummy_exchange_rate_oracle
...
ENTRYPOINT ["tini", "--", "/app/target/debug/dummy_exchange_rate_oracle"]
