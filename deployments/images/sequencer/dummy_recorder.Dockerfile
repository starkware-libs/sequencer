...
RUN cargo chef cook --recipe-path recipe.json --bin dummy_recorder -p apollo_integration_tests
COPY . .
RUN cargo build --bin dummy_recorder -p apollo_integration_tests
