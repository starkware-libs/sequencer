...
RUN cargo chef cook --recipe-path recipe.json --bin dummy_eth_to_strk_oracle -p apollo_integration_tests
COPY . .
RUN cargo build --bin dummy_eth_to_strk_oracle -p apollo_integration_tests
