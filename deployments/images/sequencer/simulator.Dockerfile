...
RUN cargo chef cook --recipe-path recipe.json --bin sequencer_simulator -p apollo_integration_tests
COPY . .
RUN cargo build --bin sequencer_simulator -p apollo_integration_tests
