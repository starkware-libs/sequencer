...
# deployments/images/sequencer/dummy_gcs.Dockerfile
...
COPY .cargo .cargo
RUN cargo chef cook --recipe-path recipe.json --bin dummy_gcs
COPY . .
RUN cargo build --bin dummy_gcs

FROM ubuntu:24.04 AS final_stage

ENV ID=1001
WORKDIR /app
COPY --from=builder /app/target/debug/dummy_gcs ./target/debug/dummy_gcs
...
ENTRYPOINT ["tini", "--", "/app/target/debug/dummy_gcs"]
