...
sys.path.insert(0, str(DEPLOYMENTS_SEQUENCER))
from src.config.loaders import DeploymentConfigLoader  # noqa: E402
...
def test_within_service_collision_from_include_is_detected(tmp_path: Path) -> None:
    ...
    common.write_text(f"config:\n  sequencerConfig:\n    {common_key}: {collision_port}\n")
    service = tmp_path / "core.yaml"
    service.write_text(
        f"include:\n  - {common}\n"
        "name: core\n"
        f"config:\n  sequencerConfig:\n    {service_key}: {collision_port}\n"
    )
