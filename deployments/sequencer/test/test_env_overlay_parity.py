...
# chain_params gives every value a flat, dot-free name; the YAML `config.sequencerConfig` still uses
# the full nested config paths. This maps each flat name back to the config path it feeds so the
# parity comparison aligns. Names absent here (chain_id, starknet_url, recorder_url,
# native_classes_whitelist, eth/strk_fee_token_address, versioned_constants_overrides, validation_only)
# already match their YAML key and pass through unchanged. Scaffolding — dropped when this test is
# deleted downstack.
