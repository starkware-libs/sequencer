...
// the prepare-production-overlays CI job. Composes the env's real chain_params + topology with the shared
// dummy node-level stand-ins. The real per-node config comes from the devops repo's node.jsonnet at deploy.
