...
# Fix home dir ownership (Docker may have pre-created it as root for volume mounts like ~/.kube)
# Only chown the directory itself, not -R: .kube is a read-only mount from the host
USER_HOME=$(getent passwd "$USER_NAME" | cut -d: -f6)
if [[ -n "$USER_HOME" ]] && [[ -d "$USER_HOME" ]]; then
