...
info "  Mounting: ${KUBE_CONFIG_DIR} -> /home/${USER_NAME}/.kube (kubectl config)"
info "  Working directory: /workspace/deployments/sequencer"

# Run the container
# Pass user info to entrypoint (no HOME mount)
# Mount repo root to /workspace, and host ~/.kube so kubectl works inside the container
docker run -it --rm \
    -v "${REPO_ROOT}:/workspace" \
    -v "${KUBE_CONFIG_DIR}:/home/${USER_NAME}/.kube:ro" \
