...
def _get_str(d: dict, key: str) -> Optional[str]:
    """Return d[key] if it is a str, else None."""
    v = d.get(key)
...
def _get_schema_model_for_path(path: str) -> Optional[type[Any]]:
    ...
    # Traverse the path to find the nested model (resolve each segment by field name or alias)
    for part in parts:
        field_name = (
            _resolve_field_name(current_model, part)
...
def _key_valid_for_model(model: type[Any], key: str) -> bool:
    ...
    return _resolve_field_name(model, key) is not None
...
def merge_service_ports(
    ...
    names = [name for p in result if (name := _get_str(p, "name"))]
...
def apply_services_overlay_strict(
    ...
    for svc_name in overlay_map:
        if svc_name not in layout_map:
            raise ValueError(f"❌ Overlay tried to introduce new service '{svc_name}' not in layout")
