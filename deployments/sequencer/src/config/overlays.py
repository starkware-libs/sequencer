...
Both the YAML loader (`app._get_config_paths`) and the native `node.jsonnet` locator
(`native._node_jsonnet_path`) walk the same overlay dirs in the same order, so the walk +
