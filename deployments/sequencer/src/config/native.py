...
Pipeline:
  1. For each override bucket (`chain_params`, `node_params`, `replacers`), locate its per-layer
     ...
     layer < each `-o` overlay dir, in order), including the cross-repo devops overlay dirs. A layer
     may also drop `<bucket>.<tag>.jsonnet` override fragments beside the base file; these are
     deep-merged after it, letting the devops overlay extend the sequencer repo's `<bucket>.jsonnet`
     without renaming or replacing it (see `_dir_bucket_files`).
...
def _dir_bucket_files(layer_dir: Path, filename: str) -> List[Path]:
    """Returns the bucket files contributed by `layer_dir` for `filename` (e.g. `chain_params.jsonnet`).

    The base `<layer_dir>/<filename>` (if present) followed by any `<bucket>.<tag>.jsonnet` override
    fragments in the same dir, sorted, each deep-merged AFTER the base. This lets a later overlay
    (notably the cross-repo devops overlay) EXTEND an earlier layer's bucket by dropping a
    differently-named fragment beside it — no need to rename or replace the base `<bucket>.jsonnet`
    (which the deploy-time overlay copy would otherwise clobber).
    """
    stem = filename[: -len(".jsonnet")] if filename.endswith(".jsonnet") else filename
    files: List[Path] = []
    base = layer_dir / filename
    if base.is_file():
        files.append(base)
    files.extend(sorted(path for path in layer_dir.glob(f"{stem}.*.jsonnet") if path.is_file()))
    return files
...
def resolve_bucket_files(
    ...
    Each layer contributes its base `<filename>` plus any `<bucket>.<tag>.jsonnet` override fragments
    (see `_dir_bucket_files`), so a later overlay can extend an earlier layer's bucket in place.
    """
    ...
    files.extend(_dir_bucket_files(base / "configs" / "overlays" / layout / "common", filename))
    ...
    for overlay_dir in overlay_dirs(base, layout, _expand_overlay_ancestors(overlays)):
        files.extend(_dir_bucket_files(overlay_dir, filename))
