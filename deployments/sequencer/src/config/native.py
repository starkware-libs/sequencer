...
def resolve_layer_files(
    ...
    Mirrors the YAML overlay chain resolution in `app.py._get_config_paths`:
      ...
      - Each `-o` overlay (dot notation, must start with the layout name) contributes every
        `<overlay dir>/*sequencer_config.jsonnet`, in the order the overlays were given.
    ...
    deltas). Returned in deep-merge order: base first, then each overlay (last wins).
    """
    ...
    # Each `-o` overlay dir, in the given order (last wins). `overlay_dirs` does the dotted-path
    # walk + layout-name validation shared with `app._get_config_paths`.
    for overlay_dir in overlay_dirs(base, layout, overlays):
