...
class DeploymentConfigLoader(Config):
    ...
    def _collect_include_order(
        ...
        """Recursively traverse includes from path; return ordered list of unique paths (dependencies first). Each file appears at most once. Raises on cycle."""
    ...
    def _load_one_file_with_includes(
        ...
        """Obtain entry file, recursively traverse included files (DAG; each file at most once), then merge in dependency order. Strips 'include' from result. Raises on cycle. If validate_final_as_service is True, the last file is merged with ServiceConfig validation (for service YAMLs that include common files)."""
