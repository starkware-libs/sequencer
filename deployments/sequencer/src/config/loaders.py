...
class DeploymentConfigLoader(Config):
    ...
    def _load_one_file_with_includes(self, path: Path) -> dict:
        """Obtain entry file, recursively traverse included files (DAG; each file at most once), then merge in dependency order. Strips 'include' from result. Raises on cycle."""
