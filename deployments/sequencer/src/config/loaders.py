...
class DeploymentConfigLoader(Config):
    ...
    def _load_one_file_with_includes(self, path: Path, _seen: Optional[set[str]] = None) -> dict:
        """Load one YAML file, resolving optional 'include: path' recursively. Path is relative to config_base_dir (or current file if unset). Strips 'include' from result. Raises on cycle."""
        ...
        if include_val is not None and isinstance(include_val, str) and include_val.strip():
            included_path = self._resolve_include_path(include_val.strip(), path)
            ...
            merged = merge_common_with_overlay_strict(included_dict, current, source=str(path))
            return merged or {}
        out = dict(raw)
