...
class GrafanaAlertRuleGroupConstruct(GrafanaBaseConstruct):
    ...
    def __init__(
        ...
        # TODO: the hash is redundant — namespace already uniquely identifies the environment
        # within a cluster both in K8s (namespace-scoped CRD) and in Grafana (folder-scoped).
        # Remove hash_value and simplify custom_name to f"{self.namespace}-arg-{k8s_group_name}".
        self.hash_value = generate_random_hash(from_string=f"{self.cluster}-{self.namespace}")
    ...
    def _create_alert_rule_group_crds(self):
        ...
        for group_file in sorted(self.grafana_alert_group.get_alert_files()):
            ...
            custom_name = f"{self.namespace}-arg-{self.hash_value}-{k8s_group_name}"

            rules = [
                self._get_shared_grafana_alert_rule_group_rules(rule)
