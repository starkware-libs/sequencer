...
class ConfigMapConstruct(BaseConstruct):
    ...
    def _build_native_node_config(self) -> dict:
        ...
        The per-bucket `chain_params.jsonnet` / `node_params.jsonnet` / `replacers.jsonnet` override
        files (base < env < devops-env < per-node) carry all applicative config, so the flat-preset
        placeholder fill and the YAML `sequencerConfig` deltas are NOT applied here.
        """
