...
If you need to build the Docker image locally instead of pulling from a registry:

```bash
# From the repository root
docker build \
  -f deployments/images/sequencer/Dockerfile \
  -t apollo_node:latest \
  .
```
...
And in `docker-compose.yml`, the image reference resolves to just `apollo_node:latest` when `IMAGE_REGISTRY` is empty.
