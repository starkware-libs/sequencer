...
def load_config_file(
    alert_rules_overrides_config_path: Optional[str], logger_instance=None
) -> dict:
    """
    ...
    """
    if not alert_rules_overrides_config_path or not os.path.isfile(
        alert_rules_overrides_config_path
    ):
        return {}

    log = logger_instance
    if log:
        log.debug(f"Loading config file: {alert_rules_overrides_config_path}")
    with open(alert_rules_overrides_config_path, "r") as f:
