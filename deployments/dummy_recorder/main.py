...
def argument_parser():
    ...
    parser.add_argument(
        "--cluster",
        type=str,
        help="Kubernetes cluster name. Required if --ingress is enabled.",
    )
    parser.add_argument(
        "--ingress-domain",
        type=str,
        help="Ingress domain. Required if --ingress is enabled.",
    )
...
class DummyRecorder(Chart):
    def __init__(
        ...
        if self.create_ingress:
            assert self.cluster, "--cluster is required if --ingress is enabled."
            assert self.domain, "--ingress-domain is required if --ingress is enabled."
            self._get_ingress()
