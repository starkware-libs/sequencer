...
Single-file modules use `foo.rs`. Modules with submodules use `foo/mod.rs`.
...
Write `expect` messages for code readers, not for the program runner. Explain _why_ the error should never happen, not _what_ is the error.
...
std::env::var("IMPORTANT_PATH").expect("set by wrapper_script.sh at startup");
```
