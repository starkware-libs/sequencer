...
fn find_deleted_nodes_for_tree<'a>(
    ...
    original_tree: &impl OriginalSkeletonTree<'a>,
    ...
    for (node_index, node) in original_tree.get_nodes() {
...
pub(crate) fn find_deleted_nodes(
    ...
    for (address, deleted_leaves) in storage_tries_deleted_leaves {
        ...
        let deleted_nodes = match updated_forest.storage_tries.get(&address) {
            Some(updated_storage_trie) => find_deleted_nodes_for_tree(
                deleted_leaves,
                original_storage_trie,
                updated_storage_trie,
            ),
            None => original_storage_trie.get_nodes().keys().cloned().collect(),
        };
