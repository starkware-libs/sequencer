...
fn find_deleted_nodes_for_tree<'a>(
    ...
    original_skeleton_tree
        .get_nodes()
        .iter()
        .filter_map(|(node_index, node)| {
            // Filter out the unmodified subtrees.
            if !matches!(node, OriginalSkeletonNode::UnmodifiedSubTree(_))
                // Filter out the nodes that are not in the updated tree.
                && updated_skeleton_tree.get_node(*node_index).is_err()
            {
                Some(*node_index)
            } else {
                None
...
pub(crate) fn find_deleted_nodes(
    ...
    // Note: Contract updates are not explicitly part of the state diff but result from storage
    // changes and contract state updates.
    let contracts_trie_deleted_leaves = original_contracts_trie_leaves
        .iter()
        .filter_map(|(index, state)| {
            // If the contract state is not empty in the original tree and it doesn't exist in the
            // updated tree, then delete it.
    ...
    for (address, updates) in actual_storage_updates {
        let deleted_leaves_indices: HashSet<NodeIndex> = updates
            .iter()
            .filter_map(|(index, leaf)| if leaf.is_empty() { Some(*index) } else { None })
            .collect();
        if deleted_leaves_indices.is_empty() {
            continue;
        }
