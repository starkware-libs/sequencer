...
pub(crate) trait TrieReader {
    fn create_contracts_trie<'a>(
        ...
        Output = ForestResult<(OriginalSkeletonTreeImpl<'a>, HashMap<NodeIndex, ContractState>)>,
    > + Send;
    ...
    ) -> impl Future<Output = ForestResult<HashMap<ContractAddress, OriginalSkeletonTreeImpl<'a>>>> + Send;
    ...
    ) -> impl Future<Output = ForestResult<OriginalSkeletonTreeImpl<'a>>> + Send;
}

impl<'a, DB: TrieReader + Send> ForestReader<'a> for DB {
    async fn read(
        &mut self,
