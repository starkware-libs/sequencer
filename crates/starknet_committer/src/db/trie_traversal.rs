        ...
        for (filled_root, subtree) in filled_roots.into_iter().zip(current_subtrees.iter()) {
            match filled_root.data {
                ...
                NodeData::Edge(EdgeData { bottom_data, path_to_bottom }) => {
                    ...
                    if subtree.is_unmodified() {
                        if !Layout::SubTree::should_traverse_unmodified_children() {
                            skeleton_tree.nodes.insert(
                                path_to_bottom.bottom_index(subtree.get_root_index()),
                                OriginalSkeletonNode::UnmodifiedSubTree(
                                    Layout::SubTree::unmodified_child_hash(bottom_data).unwrap(),
                                ),
                            );
                        } else {
    ...
    for (address, updates) in actual_storage_updates {
        ...
        let original_skeleton = create_original_skeleton_tree::<L, Layout>(
            ...
            *sorted_leaf_indices,
            ...
            &Layout::generate_key_context(TrieType::StorageTrie(*address.0)),
        )
        .await?;
