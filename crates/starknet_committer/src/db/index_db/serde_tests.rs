...
static CONTRACT_STATE_LEAF: LazyLock<IndexFilledNode<IndexLayoutContractState>> = LazyLock::new(|| {
...
static COMPILED_CLASS_HASH_LEAF: LazyLock<IndexFilledNode<IndexLayoutCompiledClassHash>> = LazyLock::new(|| {
    let data = NodeData::Leaf(IndexLayoutCompiledClassHash(CompiledClassHash(Felt::from(1))));
    let hash = TreeHashFunctionImpl::compute_node_hash(&data);
    IndexFilledNode(FilledNode { hash, data: index_leaf_data_from_hash_data(data) })
});

static STARKNET_STORAGE_VALUE_LEAF: LazyLock<IndexFilledNode<IndexLayoutStarknetStorageValue>> = LazyLock::new(|| {
