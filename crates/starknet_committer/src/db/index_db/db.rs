...
///
/// Set to FIRST_AVAILABLE_PREFIX_FELT + 2
static COMMITMENT_OFFSET_METADATA_PREFIX: LazyLock<[u8; 32]> =
    LazyLock::new(|| (*FIRST_AVAILABLE_PREFIX_FELT + Felt::TWO).to_bytes_be());

/// The db key prefix of the block number to state diff hash mapping.
///
/// Set to FIRST_AVAILABLE_PREFIX_FELT + 3
static STATE_DIFF_HASH_METADATA_PREFIX: LazyLock<[u8; 32]> =
    LazyLock::new(|| (*FIRST_AVAILABLE_PREFIX_FELT + Felt::THREE).to_bytes_be());

/// The db key prefix of the block number to state root mapping.
///
/// Set to FIRST_AVAILABLE_PREFIX_FELT + 4
static STATE_ROOT_METADATA_PREFIX: LazyLock<[u8; 32]> =
    LazyLock::new(|| (*FIRST_AVAILABLE_PREFIX_FELT + Felt::from_hex_unchecked("4")).to_bytes_be());
