...
pub async fn convert_facts_forest_db_to_index_db(
    ...
    convert_forest(storage, roots, SkipMissingNodes::No).await
}
...
pub async fn convert_facts_filled_forest_to_index(
    ...
    convert_forest(storage, roots, SkipMissingNodes::Yes).await
}
...
where
    ...
    KeyContext: Sync,
{
    convert_single_trie(storage, root_hash, key_context, current_leaves, SkipMissingNodes::No).await
}
...
async fn traverse_and_convert<FactsLeaf, IndexLeaf, KeyContext>(
    ...
    KeyContext: Sync,
{
    println!("index: {:?}, hash: {:?}", current_index, current_hash);
    ...
    let facts_db_key = create_db_key(db_prefix, &current_hash.0.to_bytes_be());
    ...
    let Some(filled_node_raw) = filled_node_raw else {
        match skip_missing {
            SkipMissingNodes::Yes => {
                // Node not found in delta - this is an unmodified branch, skip it.
                return;
            }
            SkipMissingNodes::No => {
                panic!(
                    "Node not found in storage: index={:?}, hash={:?}. If converting a filled \
                     forest delta, use convert_facts_filled_forest_to_index.",
    ...
    let index_db_key =
        create_db_key(IndexLeaf::get_static_prefix(key_context), &current_index.0.to_be_bytes());
