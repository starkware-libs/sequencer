...
fn fetch_nodes<'a, L: Leaf + IndexLayoutLeaf, Hasher: TreeHashFunction<L>>(
    ...
    while !current_subtrees.is_empty() {
        ...
        let filled_roots =
            get_roots_from_storage::<L, Hasher>(&current_subtrees, storage, key_context)?;
...
fn get_roots_from_storage<'a, L: Leaf + IndexLayoutLeaf, Hasher: TreeHashFunction<L>>(
    subtrees: &Vec<SubTree<'a>>,
    storage: &mut impl Storage,
    key_context: &KeyContext,
...
pub fn create_original_skeleton_tree<'a, L: Leaf + IndexLayoutLeaf, Hasher: TreeHashFunction<L>>(
    storage: &mut impl Storage,
