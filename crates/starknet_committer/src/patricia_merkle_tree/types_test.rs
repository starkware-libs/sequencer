...
/// Payloads persisted before compression became one-shot have no size in their frame header, and
/// are read back by the same decoder.
#[cfg(feature = "os_input")]
#[test]
fn test_compressed_state_commitment_infos_decompresses_sizeless_frame() {
    let commitment_infos = StateCommitmentInfos::default();
    let bincode_payload = bincode::serialize(&commitment_infos).unwrap();
    let sizeless_frame =
        zstd::encode_all(bincode_payload.as_slice(), zstd::DEFAULT_COMPRESSION_LEVEL).unwrap();
    assert_eq!(zstd::zstd_safe::get_frame_content_size(&sizeless_frame).unwrap(), None);

    let decompressed = CompressedStateCommitmentInfos(sizeless_frame).decompress().unwrap();

    assert_eq!(decompressed, commitment_infos);
}
