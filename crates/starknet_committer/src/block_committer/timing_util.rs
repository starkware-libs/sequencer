...
pub struct BenchmarkTimeMeasurement {
    ...
    pub accumulated_written_facts: Vec<usize>, /* Number of facts in the DB prior to the current
                                                * block. */
...
impl TimeMeasurementTrait for BenchmarkTimeMeasurement {
    ...
    fn stop_measurement(&mut self, action: Action<usize>) {
        ...
        self.current_block_measurement.update_after_action(&action, millis);

        if let Action::EndToEnd = action {
            self.total_time += millis;
            self.accumulated_written_facts.push(self.total_facts);
...
impl BenchmarkTimeMeasurement {
    ...
    fn average_window_time(&self, window_size: usize) -> Vec<f64> {
        ...
        for i in 0..n_windows {
            ...
            let total_duration: u64 = self.block_measurements
    ...
    pub fn to_csv(
        ...
        for i in 0..n_results {
            ...
            let mut record = vec![
                ...
                self.accumulated_written_facts[i].to_string(),
