...
impl TimeMeasurementTrait for BenchmarkTimeMeasurement {
    ...
    fn stop_measurement(&mut self, action: Action, entries_count: usize) -> u128 {
        let duration_in_millis =
            self.current_block_measurement.stop_measurement(action, entries_count);
        info!(
        ...
        match action {
            Action::Write => {
                ...
                self.total_db_entry_count +=
                    self.current_block_measurement.block_measurement.n_writes;
            }
            Action::EndToEnd => {
                self.total_time += duration_in_millis;
                self.block_number += 1;
                self.block_measurements
                    .push(take(&mut self.current_block_measurement.block_measurement));
            }
            _ => {}
        }
        duration_in_millis
