    ...
    M: MeasurementsTrait + Send,
{
    ...
    // The Compute action is recorded by `commit_block_compute_phase` itself; the fetch branch
    // does not touch `measurements`, so the &mut borrow stays exclusive to the compute branch.
    measurements.start_measurement(Action::FetchWitnessesFirstPass);
    let (fetch_result, compute_result) = tokio::join!(
        forest_storage.fetch_patricia_witnesses(
