...
impl BlockifierStateReader for ExecutionStateReader {
    ...
    fn get_compiled_class(&self, class_hash: ClassHash) -> StateResult<RunnableCompiledClass> {
        if let Some(api_contract_class) = self
            .maybe_pending_data
            .as_ref()
            .and_then(|pending_data| pending_data.classes.get_class(class_hash))
        {
            match api_contract_class {
                ApiContractClass::ContractClass(sierra) => {
                    if let Some(pending_casm) =
                        self.maybe_pending_data.as_ref().and_then(|pending_data| {
                            pending_data.classes.get_compiled_class(class_hash)
                        })
                    {
                        let sierra_version =
