...
// Represent the string `PROOF0` as a Felt.
...
impl TryFrom<&ProofFacts> for ProofFactsVariant {
    ...
    fn try_from(proof_facts: &ProofFacts) -> Result<Self, Self::Error> {
        // ProofFacts structure: [PROOF_VERSION, variant_marker, ...fields].
        let Some((proof_version_marker, rest)) = proof_facts.0.as_slice().split_first() else {
            return Ok(ProofFactsVariant::Empty);
        };

        // Validate that the first element is PROOF_VERSION.
        if *proof_version_marker != Felt::from(PROOF_VERSION) {
            return Err(StarknetApiError::InvalidProofFacts(format!(
                ...
                proof_version_marker
            )));
        }

        let Some((program_variant_marker, snos_fields)) = rest.split_first() else {
            return Err(StarknetApiError::InvalidProofFacts(
                "Proof facts missing variant marker after PROOF_VERSION".to_string(),
            ));
        };

        if *program_variant_marker != Felt::from(VIRTUAL_SNOS) {
            return Err(StarknetApiError::InvalidProofFacts(format!(
                "Non-SNOS proofs are not currently supported. Expected second field to be {} \
                 (VIRTUAL_SNOS), but got {}",
                Felt::from(VIRTUAL_SNOS),
                program_variant_marker
            )));
        }
        ...
        let [program_hash, version, block_number_felt, block_hash, config_hash, ..] = snos_fields
            ...
            return Err(StarknetApiError::InvalidProofFacts(format!(
                "SNOS proof facts must have at least 7 fields (PROOF_VERSION + VIRTUAL_SNOS + 5 \
                 data fields), got {}",
                proof_facts.0.len()
            )));
        };
...
impl ProgramOutput {
    ...
    /// - Prepending `PROOF_VERSION` as the first element.
    /// - Replacing the first element (number of tasks) with the given `program_variant` marker.
    ///
    /// The `program_variant` parameter specifies the variant marker to use (e.g., `VIRTUAL_SNOS`
    /// for proofs generated by the `starknet_os_runner` crate).
    ///
    /// Returns an error if the program output is empty or if the first element (number of tasks)
    /// is not 1.
    pub fn to_proof_facts(&self, program_variant: Felt) -> StarknetApiResult<ProofFacts> {
        let num_tasks = self.0.first().ok_or_else(|| {
            StarknetApiError::InvalidProgramOutput("Program output is empty".to_string())
        })?;
        if *num_tasks != Felt::ONE {
            return Err(StarknetApiError::InvalidProgramOutput(format!(
                "Expected num_tasks to be 1, got {num_tasks}"
            )));
        }
        let mut facts = vec![Felt::from(PROOF_VERSION)];
        // Add the variant marker in place of num_tasks.
