...
impl ProgramOutput {
    ...
    /// - Replacing the first element (number of tasks) with the given `output_variant` marker.
    ///
    /// The `output_variant` parameter specifies the variant marker to use (e.g., `VIRTUAL_SNOS`
    /// for proofs generated by the `starknet_os_runner` crate).
    pub fn to_proof_facts(&self, output_variant: Felt) -> ProofFacts {
        let mut facts = vec![Felt::from(PROOF_VERSION)];
        if !self.0.is_empty() {
            // Add the variant marker in place of num_tasks.
            facts.push(output_variant);
