...
pub trait HashableCompiledClass<EH: EntryPointHashable>: Sized {
    ...
    /// If `hash_version` is `None`, the default hash version is used.
    fn hash(&self, hash_version: Option<&HashVersion>) -> CompiledClassHash {
        match hash_version.unwrap_or(&HashVersion::default()) {
