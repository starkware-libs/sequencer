...
impl<T> TryFrom<&Vec<T>> for SierraVersion
where
    T: TryInto<u64> + Display + Clone,
    <T as TryInto<u64>>::Error: std::fmt::Display,
{
    type Error = StarknetApiError;

    fn try_from(sierra_program: &Vec<T>) -> Result<Self, Self::Error> {
        ...
        let map_to_api_error = |felt: T, index: usize| {
            move |err| {
        ...
        let major = sierra_program[0]
            .clone()
            .try_into()
            .map_err(map_to_api_error(sierra_program[0].clone(), 0))?;
        let minor = sierra_program[1]
            .clone()
            .try_into()
            .map_err(map_to_api_error(sierra_program[1].clone(), 1))?;
        let patch = sierra_program[2]
            .clone()
            .try_into()
            .map_err(map_to_api_error(sierra_program[2].clone(), 2))?;
