...
impl ContractAddress {
    ...
    /// The lower bound is above the special saved addresses and the upper bound is ongruent with
    /// the storage var address upper bound.
    pub fn strict_validate_contract_address(&self) -> Result<(), StarknetApiError> {
        let value = self.0.0;
        let l2_address_upper_bound = Felt::from(*L2_ADDRESS_UPPER_BOUND);
        if (BLOCK_HASH_TABLE_ADDRESS.0.0 < value) && (value < l2_address_upper_bound) {
            return Ok(());
        }
