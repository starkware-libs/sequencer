...
pub fn calculate_block_hash(
    ...
    let block_hash_version: BlockHashVersion = partial_block_hash.starknet_version.try_into()?;
    let block_commitments = &partial_block_hash.header_commitments;
    Ok(BlockHash(
        HashChain::new()
            .chain(&block_hash_version.clone().into())
            .chain(&partial_block_hash.block_number.0.into())
            .chain(&state_root.0)
            .chain(&partial_block_hash.sequencer.0)
            .chain(&partial_block_hash.timestamp.0.into())
            ...
            .chain_iter(
                gas_prices_to_hash(
                    &partial_block_hash.l1_gas_price,
                    &partial_block_hash.l1_data_gas_price,
                    &partial_block_hash.l2_gas_price,
                    &block_hash_version,
                )
                .iter(),
            )
            .chain(
                &ascii_as_felt(&partial_block_hash.starknet_version.to_string())
