    ...
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([
            ...
            ser_param(
                "should_revert",
                &self.should_revert,
                "Whether the component should revert block and do nothing else.",
                ParamPrivacyInput::Public,
            ),
        ])
    }
    ...
    Fut: Future<Output = ()>,
{
    if current_block_number <= revert_up_to_and_including {
        panic!(
            "{component_name}'s current block {current_block_number} is not larger than the \
             target block {revert_up_to_and_including}. No reverts are needed."
        );
    }
    ...
    while current_block_number > revert_up_to_and_including {
        ...
        info!("Successfully reverted {component_name} block number {current_block_number}.");
    }
