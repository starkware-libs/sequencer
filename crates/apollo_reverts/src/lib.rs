...
pub fn revert_block(storage_writer: &mut StorageWriter, target_block_number: BlockNumber) {
    storage_writer
        .begin_rw_txn()
        .unwrap()
        .revert_header(target_block_number)
        .unwrap()
        .0
        .revert_body(target_block_number)
        .unwrap()
        .0
        .revert_state_diff(target_block_number)
        .unwrap()
        .0
        .try_revert_class_manager_marker(target_block_number)
        .unwrap()
        .try_revert_base_layer_marker(target_block_number)
        .unwrap()
        .commit()
        .unwrap();
}
