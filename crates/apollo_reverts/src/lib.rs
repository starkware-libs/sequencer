    ...
    Fut: Future<Output = ()>,
{
    ...
    let latest_block_info_msg = match storage_height_marker.prev() {
        Some(latest_block_in_storage) => {
            format!(
                "The latest block saved in {component_name}'s storage is \
                 {latest_block_in_storage}!"
            )
        }
        None => format!("There aren't any blocks saved in {component_name}'s storage!"),
    };

    info!(
        "Done reverting {component_name}'s storage up to height {target_height_marker}!
        {latest_block_info_msg}
