...
use crate::test_utils::{get_test_casm, get_test_deprecated_contract_class, prepare_storage};
...
fn set_execution_state_reader(
    ...
    ExecutionStateReader {
        storage_reader,
        state_number: StateNumber::unchecked_right_after_block(BlockNumber(state_block_number)),
        maybe_pending_data: None,
        missing_compiled_class: Cell::new(None),
        class_manager_client: Some((class_manager_client, run_time_handle)),
    }
}
...
fn get_compiled_class() {
    ...
    // Set execution state reader with state block 0.
    let shared_mock_class_manager_client = Arc::new(mock_class_manager_client);
    let exec_state_reader = set_execution_state_reader(shared_mock_class_manager_client.clone(), 0);

    // Class with with hash 0x2 is already declared.
    ...
    // Class with with hash 0x3 is not yet declared.
    let class_hash = class_hash!("0x3");
    ...
    // Set execution state reader with state block 2.
    let exec_state_reader = set_execution_state_reader(shared_mock_class_manager_client.clone(), 2);

    // All classes are declared in the state.
    let class_hash = class_hash!("0x2");
    let result = exec_state_reader.get_compiled_class(class_hash).unwrap();
    assert_eq!(
        result,
...
fn get_non_existing_compiled_class() {
    ...
    let exec_state_reader = set_execution_state_reader(Arc::new(mock_class_manager_client), 0);
