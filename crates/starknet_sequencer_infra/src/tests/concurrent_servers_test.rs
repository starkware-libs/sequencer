...
impl ConcurrentComponent {
    ...
    pub async fn get_value(&self, name: String, value: TestValue) -> TestValue {
        ...
        let res = if name == "a" {
            while self.b.load(Ordering::Relaxed) != 0 {
                task::yield_now().await;
            }
            self.b.store(value, Ordering::Relaxed);
            while self.a.load(Ordering::Relaxed) == 0 {
                task::yield_now().await;
            }
            let a = self.a.load(Ordering::Relaxed);
            self.a.store(0, std::sync::atomic::Ordering::Relaxed);
            a
        } else {
            while self.a.load(Ordering::Relaxed) != 0 {
                task::yield_now().await;
            ...
            self.b.store(0, std::sync::atomic::Ordering::Relaxed);
            b
        };
...
#[async_trait]
impl ConcurrentComponentClientTrait for LocalComponentAClient {
    async fn get_value(&self, name: String, value: TestValue) -> TestResult {
...
#[async_trait]
impl ConcurrentComponentClientTrait for RemoteComponentAClient {
    async fn get_value(&self, name: String, value: TestValue) -> TestResult {
        match self.send(ConcurrentComponentRequest::GetValue(name, value)).await? {
            ConcurrentComponentResponse::GetValue(value) => Ok(value),
        }
...
async fn setup_concurrent_test(remote: bool) -> Box<dyn ConcurrentComponentClientTrait> {
    ...
    let local_client = LocalComponentAClient::new(tx_a);
    ...
    if remote {
        return Box::new(RemoteComponentAClient::new(config, socket));
    }
