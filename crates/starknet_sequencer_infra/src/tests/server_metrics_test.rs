...
async fn setup_concurrent_local_test() -> (Arc<Semaphore>, LocalTestComponentClient) {
...
fn assert_server_metrics(
    ...
    expected_received_msgs: u64,
    expected_processed_msgs: u64,
...
async fn only_metrics_counters_for_local_server() {
    ...
    let (test_sem, client) = setup_concurrent_local_test().await;
