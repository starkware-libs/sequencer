    ...
    task::spawn(async move {
            ...
            T: Serialize + DeserializeOwned + Debug + Send + Sync + Clone,
        {
            Ok(Response::builder()
                ...
                .body(Body::from(SerdeWrapper::new(body).serialize().unwrap()))
                .unwrap())
        }
    ...
    impl ComponentAClientTrait for FaultyAClient {
        async fn a_get_value(&self) -> ResultA {
            ...
            let http_request = Request::post(uri)
                ...
                .body(Body::from(SerdeWrapper::new(component_request).serialize().unwrap()))
                .unwrap();
            ...
            let response = SerdeWrapper::<ServerError>::deserialize(&body_bytes).unwrap();
