...
pub async fn configure_tracing() {
    TRACING_INITIALIZED
        .get_or_init(|| async {
            configure_tracing_internal();
        })
        .await;
}

pub fn configure_tracing_sync() {
    TRACING_INITIALIZED.set(()).expect("Tracing has already been initialized.");
    configure_tracing_internal();
}

fn configure_tracing_internal() {
