    ...
    match component_config.execution_mode.clone() {
        ...
        ComponentExecutionMode::LocalExecution { enable_remote_connection: true } => {
                ...
                && remote_server_config.is_some()
            {
                ...
                error.message = Some(
                    "Both local and remote server configurations must be provided in local \
                     execution mode with remote connection enabled."
                        .into(),
                );
                Err(error)
            }
        ...
        ComponentExecutionMode::LocalExecution { enable_remote_connection: false } => {
                ...
                && remote_server_config.is_none()
            {
                ...
                error.message = Some(
                    "Cannot provide remote configuration in local execution mode with remote \
                     connection disabled."
                        .into(),
                );
                Err(error)
