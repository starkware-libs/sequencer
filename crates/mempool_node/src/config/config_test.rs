...
fn test_valid_component_execution_config(#[case] execution_mode: ComponentExecutionMode) {
    ...
    let (remote_client_config, remote_server_config) =
        if execution_mode == ComponentExecutionMode::Remote {
            (Some(RemoteClientConfig::default()), Some(RemoteServerConfig::default()))
        } else {
            (None, None)
        };
    let component_exe_config = ComponentExecutionConfig {
