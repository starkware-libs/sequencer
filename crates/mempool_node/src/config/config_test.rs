...
#[case(LocationType::Local, false, true, "Local communication config is missing.")]
#[case(LocationType::Local, false, false, "Local communication config is missing.")]
#[case(
    LocationType::Remote,
    true,
    true,
...
#[case(LocationType::Remote, true, false, "Remote communication config is missing.")]
#[case(LocationType::Remote, false, false, "Remote communication config is missing.")]
fn test_invalid_component_execution_config(
    #[case] location: LocationType,
    #[case] local_config_flag: bool,
    #[case] remote_config_flag: bool,
    #[case] expected_error_message: &str,
) {
    // Initialize an invalid config and check that the validator finds an error.
    let local_config =
        if local_config_flag { Some(LocalComponentCommunicationConfig::default()) } else { None };
    let remote_config =
        if remote_config_flag { Some(RemoteComponentCommunicationConfig::default()) } else { None };
