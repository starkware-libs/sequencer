...
///
/// WARNING: It is `DANGEROUS` to use this macro on types that implement Deref, for two reasons (the
/// first leading to under-counting and the second to over-counting):
/// 1. If a type P<T> does not explicitly implement SizeOf, calling P<T>.size_bytes() will
///    implicitly call T.size_bytes(), which will neglect counting the stack size taken by P<T>
///    itself. Currently, this macro is implemented for the following Deref types: `Box<T>, Rc<T>,
///    Arc<T>, RefCell<T>, Mutex<T>, RwLock<T>`.
/// 2. Even if P<T> implements SizeOf, for pointer types such as `Rc<T>` or `Arc<T>`, it is possible
///    to count the same memory location twice if multiple pointers point to the same data.
///
///
///
