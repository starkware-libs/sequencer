...
async fn consecutive_heights_success() {
    ...
    // Prepate the propose_block requests for the first and the second heights.
    ...
    // Start the second height, and make sure the previouse height proposal is cleared, by trying to
...
async fn send_content_to_unknown_proposal(#[case] content: SendProposalContent) {
    ...
    let send_proposal_content_intput =
        SendProposalContentInput { proposal_id: PROPOSAL_ID, content };
    let result = batcher.send_proposal_content(send_proposal_content_intput).await;
...
async fn send_content_to_an_invalid_proposal(
    ...
    let send_proposal_content_intput =
        SendProposalContentInput { proposal_id: PROPOSAL_ID, content };
    let result = batcher.send_proposal_content(send_proposal_content_intput).await.unwrap();
...
async fn consecutive_proposal_generation_success() {
    ...
    // Make sure we can generate 4 consequtive proposals.
    for i in 0..2 {
