...
impl Default for MockDependencies {
    fn default() -> Self {
        ...
        let expected_gas_price = propose_block_input(ProposalId::default())
            .block_info
            .gas_prices
            .strk_gas_prices
            .l2_gas_price;
        mempool_client.expect_update_gas_price().returning(move |gas_price| {
...
async fn mempool_not_ready() {
    ...
    batcher.start_height(StartHeightInput { height: INITIAL_HEIGHT }).await.unwrap();
    let result = batcher.propose_block(propose_block_input(PROPOSAL_ID)).await;
    assert_eq!(result, Err(BatcherError::MempoolNotReady));
}
