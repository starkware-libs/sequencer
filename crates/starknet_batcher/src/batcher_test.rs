...
fn mock_active_proposal(proposal_manager: &mut MockProposalManagerTraitWrapper) {
    proposal_manager
        .expect_wrap_get_active_proposal()
        .returning(|| async move { Some(PROPOSAL_ID) }.boxed());
}

fn mock_non_active_proposal(proposal_manager: &mut MockProposalManagerTraitWrapper) {
    proposal_manager.expect_wrap_get_active_proposal().returning(|| async move { None }.boxed());
}

async fn create_batcher_and_validate_block(mock_dependencies: MockDependencies) -> Batcher {
...
async fn batcher_with_invalid_proposal() -> Batcher {
    ...
    mock_non_active_proposal(&mut proposal_manager);
    ...
    create_batcher_and_validate_block(MockDependencies {
        proposal_manager,
        block_builder_factory,
        ..Default::default()
    })
...
async fn batcher_with_completed_validated_proposal() -> Batcher {
    let block_builder_factory = mock_create_builder_for_validate_block();
    let mut proposal_manager = MockProposalManagerTraitWrapper::new();
    mock_validate_block(&mut proposal_manager);
    mock_non_active_proposal(&mut proposal_manager);
...
fn mock_proposal_manager_validate_flow() -> MockProposalManagerTraitWrapper {
    let mut proposal_manager = MockProposalManagerTraitWrapper::new();
    mock_validate_block(&mut proposal_manager);
    mock_active_proposal(&mut proposal_manager);
    proposal_manager
...
async fn send_content_after_proposal_already_finished() {
    let mut batcher = batcher_with_completed_validated_proposal().await;
...
async fn send_txs_to_an_invalid_proposal() {
    let mut batcher = batcher_with_invalid_proposal().await;
...
async fn decision_reached_no_executed_proposal() {
    ...
    proposal_manager
        ...
        .times(1)
        .with(eq(PROPOSAL_ID))
        .return_once(move |_| async move { None }.boxed());
