...
use crate::proposal_manager::{
    ...
    ProposalOutput,
    ProposalResult,
    ProposalStatus as InternalProposalStatus,
    StartHeightError,
};
...
fn non_default_proposal_commitment() -> ProposalCommitment {
    ProposalCommitment {
...
fn mock_proposal_manager_validate_flow() -> MockProposalManagerTraitWrapper {
    ...
    proposal_manager
        ...
        .return_once(|_, _, _, tx_provider| {
            {
                async move {
                    // Spawn a task to keep tx_provider alive until the deadline is reached.
                    // Without this, the provider would be dropped, causing the batcher to fail
                    // when sending transactions to it during the test.
                    tokio::spawn(async move {
                        tokio::time::sleep(BLOCK_GENERATION_TIMEOUT).await;
                        drop(tx_provider);
                    });
                    Ok(())
                }
    ...
    proposal_manager
        ...
        .times(1)
        .with(eq(PROPOSAL_ID))
        .return_once(move |_| { async move { Ok(non_default_proposal_commitment()) } }.boxed());
    proposal_manager
}
...
async fn validate_proposal_full_flow() {
    ...
    // TODO(Yael 14/11/2024): The test will pass without calling start height. Leaving this here
    // for future compatibility with the upcoming batcher-proposal_manager unification.
    ...
    assert_eq!(
        send_finish_result,
        SendProposalContentResponse {
            response: ProposalStatus::Finished(non_default_proposal_commitment())
        }
    );
}
...
async fn send_finish_to_an_invalid_proposal() {
    let proposal_error = GetProposalResultError::BlockBuilderError(Arc::new(
        BlockBuilderError::ExecutorError(TransactionExecutorError::BlockFull),
    ));
