...
impl Batcher {
    ...
    async fn handle_send_txs_request(
        ...
        match proposal_result {
            ...
            Err(err) => {
                Ok(SendProposalContentResponse { response: self.proposal_status_from(err)? })
            }
        }
    }
    ...
    async fn handle_finish_proposal_request(
        ...
        let proposal_status = match proposal_result {
            ...
            Err(err) => self.proposal_status_from(err)?,
        };
    ...
    fn proposal_status_from(&self, proposal_error: ProposalError) -> BatcherResult<ProposalStatus> {
