...
impl Batcher {
    ...
    async fn commit_proposal_and_block(
        ...
        match l1_provider_result {
            Err(L1ProviderClientError::L1ProviderError(L1ProviderError::UnexpectedHeight {
                expected,
                got,
            })) => {
                error!(
            ...
            Err(err) => panic!("Unexpected error while committing block in L1 provider: {err}"),
            Ok(()) => (),
        }
