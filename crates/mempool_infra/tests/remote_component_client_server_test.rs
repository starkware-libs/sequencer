...
async fn test_faulty_client_setup() {
    ...
    setup_for_tests(123, port_a, port_b).await;
    ...
    let faulty_a_client = FaultyAClient { port: port_a };
...
#[case::response_deserialization_failure(
    create_client_and_faulty_server(get_available_port().unwrap(),ARBITRARY_DATA).await,
