...
use starknet_mempool_infra::serde_utils::SerdeWrapper;
    ...
    task::spawn(async move {
            ...
            T: Serialize + DeserializeOwned + Debug + Send + Sync + Clone,
        {
            Ok(Response::builder()
                ...
                .body(Body::from(SerdeWrapper::new(body).to_bincode().unwrap()))
                .unwrap())
        }
    ...
    impl ComponentAClientTrait for FaultyAClient {
        async fn a_get_value(&self) -> ResultA {
            ...
            let http_request = Request::post(uri)
                ...
                .body(Body::from(SerdeWrapper::new(component_request).to_bincode().unwrap()))
                .unwrap();
            ...
            let response = SerdeWrapper::<ServerError>::from_bincode(&body_bytes).unwrap();
