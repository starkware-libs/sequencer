    ...
    T: Serialize + Send + Sync + 'static + Clone,
{
    ...
    ComponentAClient::new(LOCAL_IP, port, RETRIES)
}

async fn setup_for_tests(setup_value: ValueB, a_port: u16, b_port: u16) {
    let a_client = ComponentAClient::new(LOCAL_IP, a_port, RETRIES);
    let b_client = ComponentBClient::new(LOCAL_IP, b_port, RETRIES);
...
async fn test_proper_setup() {
    ...
    let a_client = ComponentAClient::new(LOCAL_IP, A_PORT_TEST_SETUP, RETRIES);
    let b_client = ComponentBClient::new(LOCAL_IP, B_PORT_TEST_SETUP, RETRIES);
...
async fn test_unconnected_server() {
    let client = ComponentAClient::new(LOCAL_IP, UNCONNECTED_SERVER_PORT, RETRIES);
