    ...
    T: Serialize + DeserializeOwned + Debug + Send + Sync + 'static + Clone,
{
    ...
    let config = RemoteConnectionConfig { socket, ..Default::default() };
    ComponentAClient::new(config)
}

async fn setup_for_tests(setup_value: ValueB, a_socket: SocketAddr, b_socket: SocketAddr) {
    let a_config = RemoteConnectionConfig { socket: a_socket, ..Default::default() };
    let b_config = RemoteConnectionConfig { socket: b_socket, ..Default::default() };
...
async fn test_retry_request() {
    ...
    let retry_config = RemoteConnectionConfig {
        socket,
        retries: 1,
    ...
    let no_retry_config = RemoteConnectionConfig {
        socket,
        retries: 0,
        idle_connections: MAX_IDLE_CONNECTION,
        idle_timeout: IDLE_TIMEOUT,
