    ...
    Response: for<'a> Deserialize<'a>,
{
    ...
    pub async fn send(&self, component_request: Request) -> ClientResult<Response> {
        // We construct 'http_request' andsssssssssssstry sending request 'max_retries' of times,
        ...
        // If not successful, do it one last time outside of the loop and returning the result.
        // 'http_request is not clonable so we construct it here again.
        let http_request = self.construct_http_request(&component_request);
