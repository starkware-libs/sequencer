    ...
    Response: Send + Sync,
{
    ...
    pub async fn send(&self, request: Request) -> Response {
        let request = RequestAndMonitor::Original(request);
        let res = self.internal_send(request).await;
        match res {
            ResponseAndMonitor::Original(response) => response,
            _ => panic!("Unexpected response variant."),
        }
    }

    pub async fn send_alive(&self) -> bool {
        let res = self.internal_send(RequestAndMonitor::<Request>::IsAlive).await;
        match res {
            ResponseAndMonitor::IsAlive(is_alive) => is_alive,
            _ => panic!("Unexpected response variant."),
        }
    }
}
