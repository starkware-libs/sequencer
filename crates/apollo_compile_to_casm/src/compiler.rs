...
impl SierraToCasmCompiler {
    pub fn new(config: SierraCompilationConfig) -> Self {
        ...
        let libfunc_args = libfunc_args(config.allowed_libfuncs_list);
        Self { config, path_to_binary, libfunc_args }
    }
...
fn libfunc_args(allowed_libfuncs_list: AllowedLibfuncsList) -> [String; 2] {
    let (flag, value) = match allowed_libfuncs_list {
