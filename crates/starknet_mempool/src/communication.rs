...
impl MempoolCommunicationWrapper {
    ...
    pub(crate) fn has_tx_from_address(
        ...
        Ok(self.mempool.has_tx_from_address(contract_address))
    }
}
...
impl ComponentRequestHandler<MempoolRequest, MempoolResponse> for MempoolCommunicationWrapper {
    async fn handle_request(&mut self, request: MempoolRequest) -> MempoolResponse {
        match request {
            ...
            MempoolRequest::DeployAccountExists(contract_address) => {
                MempoolResponse::DeployAccountExists(self.has_tx_from_address(contract_address))
            }
        }
    }
}
