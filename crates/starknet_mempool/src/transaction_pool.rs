...
impl PoolCapacity {
    fn add(&mut self, tx_size_bytes: u64) {
        self.n_txs += 1;
        self.size_bytes = self
            .size_bytes
            .checked_add(tx_size_bytes)
            .expect("Overflow when adding to PoolCapacity size_bytes.");
    }

    fn remove(&mut self, tx_size_bytes: u64) {
        self.n_txs =
            self.n_txs.checked_sub(1).expect("Underflow: Cannot subtract from an empty pool.");
        self.size_bytes = self
            .size_bytes
            .checked_sub(tx_size_bytes)
            .expect("Underflow when subtracting from PoolCapacity size_bytes.");
    }
