...
fn committed_account_nonce_cleanup() {
    let mut mempool = Mempool::new(
        MempoolConfig { committed_nonce_retention_block_count: 1, ..Default::default() },
    ...
    // Commit another empty block. this should remove the previously committed nonce.
    commit_block(&mut mempool, [], []);

    // Add the transaction again. This time it should be accepted.
