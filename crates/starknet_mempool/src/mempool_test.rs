...
#[test]
fn test_add_tx_accepts_nonce_1_with_deploy_account_in_staging() {
    // Setup.
    let mut mempool = MempoolContentBuilder::new().build_into_mempool();
    let input = add_tx_input!(tx_hash: 0, address: "0x0", tx_nonce: 0, account_nonce: 0);
    add_tx(&mut mempool, &input);
    // Take the transaction out of the pool and update the mempool state with the nonce.
