...
/// Represents the state tracked by the mempool tracks.
...
impl MempoolState {
    ...
    fn validate_incoming_tx(
        ...
        if self.get(address).is_some_and(|existing_nonce| {
            if enable_fee_escalation {
                tx_nonce < existing_nonce
            } else {
                tx_nonce <= existing_nonce
            }
        }) {
...
impl Mempool {
    ...
    fn validate_incoming_tx(&self, tx_reference: TransactionReference) -> MempoolResult<()> {
        self.state.validate_incoming_tx(self.config.enable_fee_escalation, tx_reference)
    }
