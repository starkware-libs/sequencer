...
pub struct MempoolConfig {
    ...
    declare_delay: std::time::Duration,
}
...
impl Default for MempoolConfig {
    fn default() -> Self {
        MempoolConfig {
            ...
            declare_delay: std::time::Duration::from_millis(1000),
        }
    }
}
...
impl SerializeConfig for MempoolConfig {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from([
            ...
            ser_param(
                "delay_duration",
                &self.declare_delay.as_millis(),
                "The duration in milliseconds that declare transactions should be delayed before \
                 being proposed (so validators have enough time to compile before they need to \
...
impl Mempool {
    pub fn new(config: MempoolConfig) -> Self {
        let declare_delay = config.declare_delay;
        Mempool {
            config,
            tx_pool: TransactionPool::default(),
            tx_queue: TransactionQueue::new(declare_delay),
