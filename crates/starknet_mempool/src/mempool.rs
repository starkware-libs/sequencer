...
impl Mempool {
    ...
    fn validate_incoming_tx(
        ...
        // If the transaction is the first one for an account that was not deployed yet, it means
        // that the gateway has skipped validations. In this case, we need to verify that a
        // deploy_account transaction exists for this account. It is sufficient to check if
        // the account exists in the mempool since it means that either it has a
