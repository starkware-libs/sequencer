...
impl NetworkBehaviour for Behaviour {
    ...
    ) -> Poll<ToSwarm<Self::ToSwarm, <Self::ConnectionHandler as ConnectionHandler>::FromBehaviour>>
    {
        ...
        let time_for_next_bootstrap_dial = self.time_for_next_bootstrap_dial.clone();
        let time_for_next_kad_query = self.time_for_next_kad_query.clone();
        let is_dialing_to_bootstrap_peer = self.is_dialing_to_bootstrap_peer;
        let heartbeat_interval = self.config.heartbeat_interval.clone();
        let mut poll_future = self.poll_future.take().unwrap_or(
            async move {
                ...
                {
                    tokio::time::sleep_until(time_for_next_bootstrap_dial.clone().into()).await;
                    PollFutureOutput {
                        event: ToSwarm::Dial {
