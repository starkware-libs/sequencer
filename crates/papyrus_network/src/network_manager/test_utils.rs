...
) -> (SqmrClientSender, TestReceiver<MockClientResponsesManager<Query, Response>>)
where
...
impl<Query: TryFrom<Bytes>, Response: TryFrom<Bytes>> MockClientResponsesManager<Query, Response> {
    ...
    pub async fn assert_reported(self, timeout: Duration) -> Result<(), oneshot::Canceled> {
        tokio::time::timeout(timeout, self.report_receiver).await.unwrap()
    }
