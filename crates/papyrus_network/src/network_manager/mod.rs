...
impl<SwarmT: SwarmTrait> GenericNetworkManager<SwarmT> {
        ...
        Bytes: From<T>,
    {
        ...
        let reported_messages_sender =
            self.reported_peers_sender.clone().with(|metadata: BroadcastedMessageMetadata| {
                ready(Ok(metadata.originator_id.private_get_peer_id()))
            });
...
pub struct BroadcastTopicChannels<T: TryFrom<Bytes>> {
    ...
    pub reported_messages_sender: GenericSender<BroadcastedMessageMetadata>,
    pub continue_propagation_sender: GenericSender<BroadcastedMessageMetadata>,
}
