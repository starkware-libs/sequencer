...
impl TransactionConverterTrait for TransactionConverter {
    ...
    async fn convert_rpc_tx_to_internal_rpc_tx(
        ...
        let (tx_without_hash, verification_handle) = match tx {
            RpcTransaction::Invoke(RpcInvokeTransaction::V3(tx)) => {
                ...
                let verification_handle = self.spawn_proof_verification(
                    &tx.proof_facts,
                    &tx.proof,
                    tx.nonce,
                    tx.sender_address,
    ...
    async fn store_proof_in_proof_manager(
        ...
        proof_manager_client.set_proof(proof_facts, nonce, sender_address, proof).await?;
        Ok(start.elapsed())
    }
}
...
impl TransactionConverter {
    fn spawn_verify_and_store_task(&self, handle: VerificationHandle) -> VerifyAndStoreProofTask {
        ...
        let sender_address = handle.sender_address;
    ...
    fn spawn_proof_verification(
        ...
        let verification_task = tokio::spawn(async move {
            let contains_proof = proof_manager_client
                .contains_proof(task_proof_facts.clone(), nonce, sender_address)
                .await?;
