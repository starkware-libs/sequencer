...
pub fn get_proof_facts_for_testing() -> ProofFacts {
    ...
    let content = std::fs::read_to_string(&proof_facts_path)
        .expect("Failed to read example_proof_facts.json from resources directory");
    let hex_values: Vec<String> =
        serde_json::from_str(&content).expect("Failed to parse example_proof_facts.json");
    let felts: Vec<Felt> = hex_values
        .into_iter()
        .map(|hex| Felt::from_hex(&hex).expect("Failed to parse hex value as Felt"))
        .collect();
    ProofFacts::from(felts)
}
...
async fn test_proof_verification_called_for_invoke_v3_with_proof_facts() {
    ...
    // Expect contains proof to be called and return true (proof already exists).
    // This skips actual proof verification which requires a valid proof.
...
async fn test_consensus_tx_to_internal_with_proof_facts_verifies_and_sets_proof() {
    ...
    // Return true to indicate proof already exists and skip actual verification.
    mock_proof_manager_client
        .expect_contains_proof()
        .once()
        .with(eq(proof_facts.clone()))
