...
#[validate(schema(function = "validate_static_config"))]
...
fn validate_static_config(config: &StateSyncStaticConfig) -> result::Result<(), ValidationError> {
...
fn validate_config(config: &StateSyncConfig) -> result::Result<(), ValidationError> {
    validate_static_config(&config.static_config)
}
