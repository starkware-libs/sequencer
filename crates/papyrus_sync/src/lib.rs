    ...
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([
            ...
            ser_param(
                ...
                "Whether to store sierras and casms to the storage.",
                ParamPrivacyInput::Public,
            ),
        ])
    }
...
> {
    ...
    // Should only be used inside perform_storage_writes.
...
> GenericStateSync<TCentralSource, TPendingSource, TBaseLayerSource>
{
    ...
    >(
        ...
        let mut writer =
            self.writer.take().expect("Someone took self.writer outside of perform_storage_writes");
        let writer = spawn_blocking(move || match f(&mut writer) {
            Ok(()) => Ok(writer),
            Err(err) => Err((err, writer)),
        })
        .await
        // We can't recover here because we lost the writer so we panic.
        .expect("JoinError occured while performing storage writes")
        .map_err(|(err, writer)| {
