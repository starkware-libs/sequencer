...
const INITIAL_FIRST_BLOCK_TO_COMPILE_FROM: u64 = u64::MAX;

const FIRST_BLOCK_TO_COMPILE_FROM_ORDERING: Ordering = Ordering::Relaxed;
...
> {
    ...
    first_block_to_compile_from: Arc<AtomicU64>,
}
...
> GenericStateSync<TCentralSource, TPendingSource, TBaseLayerSource>
{
    ...
    fn store_block(
        ...
        if self.first_block_to_compile_from.load(FIRST_BLOCK_TO_COMPILE_FROM_ORDERING)
            == INITIAL_FIRST_BLOCK_TO_COMPILE_FROM
            && block.header.block_header_without_hash.starknet_version
                >= STARKNET_VERSION_TO_COMPILE_FROM
        {
            self.first_block_to_compile_from
                .store(block_number.0, FIRST_BLOCK_TO_COMPILE_FROM_ORDERING);
        }
        Ok(())
    }
    ...
    async fn store_state_diff(
        ...
        if let Some(class_manager_client) = &self.class_manager_client {
            ...
            if block_number.0
                >= self.first_block_to_compile_from.load(FIRST_BLOCK_TO_COMPILE_FROM_ORDERING)
            {
...
impl StateSync {
    ...
    pub fn new(
        ...
        Self {
            ...
            class_manager_client,
            first_block_to_compile_from: Arc::new(AtomicU64::new(
                INITIAL_FIRST_BLOCK_TO_COMPILE_FROM,
            )),
        }
    }
...
fn stream_new_compiled_classes<TCentralSource: CentralSourceTrait + Sync + Send>(
    ...
    try_stream! {
        loop {
            ...
            let first_block_to_compile_from_value =
                BlockNumber(first_block_to_compile_from.load(FIRST_BLOCK_TO_COMPILE_FROM_ORDERING));
            let up_to = min(
                min(state_marker, BlockNumber(from.0 + u64::from(max_stream_size))),
                first_block_to_compile_from_value,
            );
            if from >= first_block_to_compile_from_value {
                info!(
