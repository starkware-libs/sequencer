...
> {
    ...
    // We use std mutex and not tokio because the mutex is used in a synchronous context.
    writer: Arc<std::sync::Mutex<StorageWriter>>,
...
pub enum StateSyncError {
    ...
    // We can't have the actual error here since it is not Send.
    #[error("Writer mutex was poisoned.")]
    PoisonError,
}

impl<T> From<PoisonError<T>> for StateSyncError {
...
> GenericStateSync<TCentralSource, TPendingSource, TBaseLayerSource>
{
    pub async fn run(mut self) -> StateSyncResult {
        ...
        fn is_recoverable(err: &StateSyncError) -> bool {
            ...
            match err {
                ...
                // TODO(shahak): Consider trying to un-poison the mutex.
                StateSyncError::SequencerPubKeyChanged { .. } | StateSyncError::PoisonError => {
                    false
                }
            }
        }
    ...
    >(
        ...
        spawn_blocking(move || f(&mut *(writer.lock()?))).await?
    }
}
