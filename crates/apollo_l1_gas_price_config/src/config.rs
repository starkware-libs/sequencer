...
pub struct ChainlinkOracleConfig {
    ...
    #[validate(range(min = 1))]
    pub lag_interval_seconds: u64,
    /// At least 2: `fetch_rate` caches this bucket's failure and then falls back to the previous
    /// bucket's rate, so a cache holding one entry would evict the rate it is about to read.
