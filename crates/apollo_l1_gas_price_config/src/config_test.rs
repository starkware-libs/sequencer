...
// One entry cannot hold this bucket's cached failure and the previous bucket's rate at once, so
// caching the failure would evict the rate the fallback then reads.
#[case::single_entry_max_cache_size(ChainlinkOracleConfig {
    max_cache_size: 1,
    ..Default::default()
})]
