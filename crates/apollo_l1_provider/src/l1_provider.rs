...
impl L1Provider {
    ...
    pub fn commit_block(
        ...
        match self.check_height_with_error(height) {
            ...
            Err(e) => {
                // We are returning an error -> not accepting the block with this height. So we must
                // catch up to it, and finish catching up when the provider has synced this height.
                if self.state.is_uninitialized() {
                    warn!(
