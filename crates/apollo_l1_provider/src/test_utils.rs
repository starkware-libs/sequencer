...
use apollo_time::time_keeper::{Clock, DefaultClock};
...
impl L1ProviderContentBuilder {
    ...
    pub fn with_clock(mut self, clock: Arc<dyn apollo_time::time_keeper::Clock>) -> Self {
    ...
    pub fn with_timed_txs(
        mut self,
        txs: impl IntoIterator<Item = (u64, L1HandlerTransaction)>,
    ) -> Self {
    ...
    pub fn with_timed_committed(
        mut self,
        committed: impl IntoIterator<Item = (u64, L1HandlerTransaction)>,
    ) -> Self {
    ...
    fn from(mut content: TransactionManagerContent) -> TransactionManager {
        ...
        for rejected_tx in rejected {
            ...
            let mut record = TransactionRecord::new(TransactionPayload::Full {
                tx: rejected_tx,
                created_at_block: 0.into(), // timestamps are irrelevant for txs once rejected.
            });
...
impl TransactionManagerContentBuilder {
    fn with_txs(mut self, txs: impl IntoIterator<Item = L1HandlerTransaction>) -> Self {
        ...
        self.uncommitted = Some(txs.into_iter().map(|tx| (dummy_timestamp, tx).into()).collect());
        self
    }
    ...
    fn with_committed(mut self, committed: impl IntoIterator<Item = L1HandlerTransaction>) -> Self {
        self.committed.get_or_insert_default().extend(
            committed
                ...
                .map(|tx| (tx.tx_hash, TransactionPayload::Full { tx, created_at_block: 0.into() })),
        );
        self
    }
    ...
    fn with_timed_committed(
        ...
        self.committed.get_or_insert_default().extend(committed.into_iter().map(|timed_tx| {
            (
                timed_tx.tx.tx_hash,
                TransactionPayload::Full { tx: timed_tx.tx, created_at_block: timed_tx.timestamp },
            )
        }));
        self
    }
...
impl From<(u64, L1HandlerTransaction)> for TimedL1HandlerTransaction {
    fn from((timestamp, tx): (u64, L1HandlerTransaction)) -> Self {
...
impl From<TimedL1HandlerTransaction> for TransactionRecord {
    fn from(timed_tx: TimedL1HandlerTransaction) -> Self {
        TransactionRecord::new(TransactionPayload::Full {
            tx: timed_tx.tx,
            created_at_block: timed_tx.timestamp,
        })
    }
}
