...
impl Default for L1ProviderConfig {
    fn default() -> Self {
        Self {
            ...
            new_l1_handler_cooldown_seconds: Duration::from_secs(4 * 60),
        }
    }
}
...
pub fn validate_cooldown(
    ...
    let message_to_scrape_time_diff_lowerbound = l1_scraper_config.polling_interval_seconds
        + Duration::from_secs(L1_BLOCK_TIME * l1_scraper_config.finality);
    if message_to_scrape_time_diff_lowerbound <= l1_provider_config.new_l1_handler_cooldown_seconds
    {
        Ok(())
    } else {
