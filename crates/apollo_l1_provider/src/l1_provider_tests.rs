...
#[rstest]
#[case(tx_hash!(1), ValidationStatus::Validated, "validate rejected transaction")]
#[case(tx_hash!(2), ValidationStatus::Invalid(InvalidValidationStatus::AlreadyIncludedOnL2), "validate non-rejected transaction")]
#[case(tx_hash!(3), ValidationStatus::Validated, "validate uncommitted transaction")]
fn validate_rejected_transactions(
    ...
    assert_eq!(
        l1_provider.validate(tx, BlockNumber(1)).unwrap(),
        expected_status,
        "Failed case: {}",
        description
    );
...
#[test]
fn staged_rejected_transaction() {
    let tx1 = tx_hash!(1);
    let mut l1_provider = setup_rejected_transactions();
    ...
    assert_eq!(
        ...
        ValidationStatus::Invalid(InvalidValidationStatus::AlreadyIncludedInProposedBlock),
        "Failed case: {}",
        "validate staged rejected transaction"
    );
}

#[test]
fn rollback_rejected_transaction() {
    let tx1 = tx_hash!(1);
    let mut l1_provider = setup_rejected_transactions();
    ...
    assert_eq!(
        ...
        ValidationStatus::Validated,
        "Failed case: {}",
        "validate rollback rejected transaction"
    );
}
