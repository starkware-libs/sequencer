...
async fn catching_up_commit_block_received_twice_no_error() {
    ...
    call_commit_block_to_start_catching_up_and_expect_error(&mut l1_provider, BlockNumber(2));
...
async fn commit_block_historical_height_short_circuits_catching_up() {
    ...
    l1_provider.initialize(STARTUP_HEIGHT, vec![]).await.expect("l1 provider initialize failed");
    call_commit_block_to_start_catching_up_and_expect_error(&mut l1_provider, TARGET_HEIGHT);
...
async fn catching_up_e2e() {
    ...
    call_commit_block_to_start_catching_up_and_expect_error(&mut l1_provider, CATCH_UP_HEIGHT);
...
async fn tx_cancelled_during_initialization_never_reaches_proposable_index() {
    ...
    let mut l1_provider = L1ProviderContentBuilder::new()
        ...
        .with_clock(clock.clone())
        .build_into_l1_provider();
    l1_provider.initialize(STARTUP_HEIGHT, vec![tx, cancel_event]).await.unwrap();

    let snapshot = l1_provider.tx_manager.snapshot();
    assert!(snapshot.proposable_index.is_empty());
    assert!(snapshot.cancellation_started_on_l2.contains(&tx_hash));
    let cancelled_tx = l1_provider.tx_manager.records.get(&tx_hash).unwrap();
