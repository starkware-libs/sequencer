...
fn process_events_committed_txs() {
    ...
    let timestamp = 1;
    let mut l1_provider = L1ProviderContentBuilder::new()
        .with_timed_txs([(timestamp, l1_handler(1))])
        .with_timed_committed([(timestamp, l1_handler(2))])
...
fn add_tx_identical_timestamp_both_stored() {
    ...
    // The proposable_index should be ordered by (timestamp, tx_hash)
    let expected = L1ProviderContentBuilder::new()
        .with_timed_txs([(timestamp_1, tx_1), (timestamp_2, tx_2), (timestamp_3, tx_3)])
        .build();
...
fn get_txs_identical_timestamps() {
    ...
    let l1_provider = L1ProviderContentBuilder::new()
        .with_timed_txs([
            (timestamp_1, tx_1.clone()),
            (timestamp_2, tx_2.clone()),
            (timestamp_3, tx_3.clone()),
        ])
        .with_state(ProviderState::Propose);
...
fn get_txs_timestamp_cutoff_none_eligible() {
    ...
    let mut l1_provider = L1ProviderContentBuilder::new()
        .with_config(config)
        .with_clock(Arc::new(mock_clock))
        .with_timed_txs([(timestamp_1, tx_1.clone()), (timestamp_2, tx_2.clone())])
        .with_state(ProviderState::Propose)
        .build_into_l1_provider();
