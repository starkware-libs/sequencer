...
async fn l1_handler_tx_consumed_txs() {
    ...
    let current_l1_block_number = Arc::new(AtomicU64::new(1));
    let current_l1_block_number_clone = current_l1_block_number.clone();
    base_layer
        .expect_latest_l1_block_number()
        .returning(move || Ok(current_l1_block_number_clone.load(Ordering::Relaxed)));
    base_layer
        .expect_l1_block_at()
    ...
    current_l1_block_number.store(2, Ordering::Relaxed);
    ...
    tokio::time::advance(TIMELOCK_DURATION + ONE_SEC).await;
