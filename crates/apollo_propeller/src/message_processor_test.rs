...
use crate::message_processor::{EventStateManagerToEngine, MessageProcessor};
...
async fn test_reconstructed_unit_signature_matches_original() {
    ...
    let all_units = create_units_to_publish(
        message.clone(),
        channel,
        publisher_keypair,
        num_data_shards,
    ...
    let my_shard_index = tree_manager
        .get_my_shard_index_given_publisher(&publisher)
    ...
    for (i, unit) in all_units.iter().enumerate() {
        let shard_index = ShardIndex(u32::try_from(i).unwrap());
    ...
    let reconstructed_unit = loop {
        tokio::select! {
            Some(msg) = engine_rx.recv() => {
                match msg {
                    EventStateManagerToEngine::SendUnitToPeers { unit, .. } => {
