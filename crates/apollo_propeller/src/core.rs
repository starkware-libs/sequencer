...
impl Core {
    ...
    pub(crate) async fn broadcast(
        ...
        let root = spawn_monitored("prepare_units_task", async move {
            ...
            // Use try_send to detect backpressure immediately
            if !try_send_or_exit(&tx, r, "Broadcaster->Core") {
                // Core dropped, this shouldn't happen during normal operation
                tracing::error!("Core dropped broadcaster results channel unexpectedly");
            }
            task_result
        })
        .await
    ...
    pub(crate) fn handle_unit(&mut self, sender: PeerId, unit: PropellerUnit) {
        ...
        // Use try_send to detect backpressure immediately
        if !try_send_or_exit(&handle.validator_tx, unit_to_validate, "Core->Validator") {
            // Validator task closed (likely due to timeout or completion)
            tracing::trace!(
                "Validator task closed for channel={} publisher={:?} root={:?}",
                channel,
                publisher,
                root
            );
    ...
    async fn send_unit_to_peer(&mut self, unit: PropellerUnit, peer: PeerId) {
        ...
        if let Some(message) = message {
            self.emit_handler_event(peer, HandlerIn::SendUnit(message)).await;
        }
    }
    ...
    async fn drain_pending_outputs(&mut self, output_tx: &mpsc::Sender<CoreOutput>) {
        while let Some(output) = self.pending_outputs.pop_front() {
            ...
            // Use try_send to detect backpressure immediately
            if !try_send_or_exit(output_tx, output, "Core->Behaviour") {
                // Behaviour dropped, stop draining
                tracing::trace!("Behaviour dropped output channel, stopping drain");
                return;
            }
        }
    }
