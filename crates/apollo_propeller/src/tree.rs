...
use crate::types::{CommitteeSetupError, ShardIndex, TreeGenerationError};
...
impl PropellerScheduleManager {
    ...
    pub fn get_peer_for_shard_index(
        ...
        let publisher_index = self
            ...
            .map_err(|_| TreeGenerationError::PublisherNotInCommittee { publisher: *publisher })?;
        let index =
            if shard_index < publisher_index { shard_index } else { shard_index.saturating_add(1) };
        self.committee_nodes.get(index).map(|(peer, _)| *peer).ok_or({
