...
impl PropellerTreeManager {
    ...
    /// Get the peers without the local peer
    pub(crate) fn make_first_broadcast_list(&self) -> Vec<(PeerId, ShardIndex)> {
        let publisher = self.get_local_peer_id();
        let mut shard_counter = 0u32;
        let r: Vec<(PeerId, ShardIndex)> = self
            .nodes
            .iter()
            .filter_map(|(peer_id, _)| {
                if *peer_id != publisher {
                    let shard_index = ShardIndex(shard_counter);
                    shard_counter += 1;
                    Some((*peer_id, shard_index))
                } else {
                    None
                }
            })
            .collect();
        debug_assert!(r.iter().all(|(peer_id, shard_index)| {
            self.get_peer_for_shard_id(&publisher, *shard_index).unwrap() == *peer_id
        }));
        r
    }
