...
pub fn sign_message_id(
    ...
    let msg = build_signed_payload(message_id, channel, nonce);
...
fn build_signed_payload(message_id: &MessageRoot, channel: Channel, nonce: u64) -> Vec<u8> {
    [SIGNING_PREFIX, &message_id.0, &channel.0.to_be_bytes(), &nonce.to_be_bytes(), SIGNING_POSTFIX]
        .concat()
}
