...
// the encoded output) to avoid re-encoding when all data shards are already present. (I suspect
// this is already accounted for, verify) TODO(AndrewL): Consider rebuilding all shards in a single
// reed-solomon call instead of reconstructing data shards and then regenerating coding shards
// separately. <github.com/AndersTrier/reed-solomon-simd/issues/65>
...
pub fn create_units_to_publish(
    ...
    for (index, shard) in all_shards.into_iter().enumerate() {
        ...
        let message = PropellerUnit::new(
            ...
            signature.clone(),
            ShardIndex(
                index.try_into().expect("failed converting usize shard index to u32 ShardIndex"),
            ),
            shard,
            proof,
        );
...
// TODO(AndrewL): Validate that data_shards is non-empty to avoid a panic when building the
// Merkle tree (empty tree has no root). Currently callers always provide non-empty data.
