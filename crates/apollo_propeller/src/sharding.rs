...
use crate::types::{Committee, ReconstructionError, ShardIndex, ShardPublishError};
use crate::unit::{PropellerUnit, Shard, Shards};
...
pub fn reconstruct_message_from_shards(
    ...
    data_count: usize,
    coding_count: usize,
) -> Result<(Vec<u8>, Shards, MerkleProof), ReconstructionError> {
    ...
    let my_shards = Shards(vec![Shard(std::mem::take(&mut all_shards[my_shard_index]))]);
    Ok((
        un_padded_message,
        my_shards,
...
///
/// Each shard is wrapped in a `Shards` proto message and the Merkle leaf is the
...
fn create_coding_shards_and_merkle(
    ...
    let leaf_data: Vec<Vec<u8>> = all_shards
        .iter()
        .map(|shard| Shards(vec![Shard(shard.clone())]).encode_to_proto_bytes())
        .collect();
