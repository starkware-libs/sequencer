...
use crate::channel_utils::{try_send_critical, try_send_or_exit};
                ...
                match msg {
                    ValidatorToStateManager::ValidatedUnit { sender, unit } => {
                        ...
                        if unit_index == my_shard_index && !my_shard_broadcasted {
                            ...
                            broadcast_shard(&core_tx, &unit, &tree_manager, publisher, local_peer_id);
                        }
                        ...
                        if reconstructed_message.is_none() {
                            ...
                            if tree_manager.should_receive(access_count) {
                                ...
                                try_send_critical(&core_tx, StateManagerToCore::Event(event), "StateManager->Core");

                                send_finalized(&core_tx, &validator_tx, channel, publisher, message_root);
                                break;
                            }
                        }
                    }
                ...
                match result.result {
                    ...
                    Ok(success) => {
                        ...
                        if !my_shard_broadcasted {
                            ...
                            broadcast_shard(&core_tx, &reconstructed_unit, &tree_manager, publisher, local_peer_id);
                        }
...
fn send_finalized(
    ...
    try_send_critical(core_tx, msg, "StateManager->Core");
    try_send_or_exit(validator_tx, StateManagerToValidator::Shutdown, "StateManager->Validator");
}
