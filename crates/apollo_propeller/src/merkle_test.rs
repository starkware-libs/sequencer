...
fn test_merkle_proof_validity(#[case] n: u8) {
    ...
    for proof_index in 0..n.into() {
        let proof = tree.prove(proof_index).unwrap();
        let leaf_hash = MerkleTree::hash_leaf(&data[proof_index]);
        assert!(tree.verify(&leaf_hash, &proof, proof_index));
    }
}
...
fn test_merkle_tampered_data_proof_invalidity(#[case] n: u8) {
    ...
    for proof_index in 0..n.into() {
        let proof = tree.prove(proof_index).unwrap();
        let mut tampered_data = data[proof_index].clone();
...
fn test_merkle_regression() {
    ...
    insta::assert_snapshot!("merkle_root", hash_to_hex(&tree.root()));

    // Snapshot all leaf hashes
    let leaves_snapshot = hashes_to_hex(tree.leaves()).join("\n");
