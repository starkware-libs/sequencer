...
use crate::types::{Committee, Event, MessageRoot, ReconstructionError, ShardValidationError};
...
pub enum EventStateManagerToEngine {
    ...
    Finalized { committee: Committee, publisher: PeerId, message_root: MessageRoot },
...
impl MessageProcessor {
    pub async fn run(mut self) {
        debug!(
            "[MSG_PROC] Started for committee={:?} publisher={:?} root={:?}",
            self.committee, self.publisher, self.message_root
        );
        ...
        debug!(
            "[MSG_PROC] Stopped for committee={:?} publisher={:?} root={:?}",
            self.committee, self.publisher, self.message_root
        );
    }
    ...
    async fn process_units(&mut self) {
        ...
        trace!(
            "[MSG_PROC] All senders closed for committee={:?} publisher={:?} root={:?}",
            self.committee,
            self.publisher,
            self.message_root
        );
    ...
    fn emit_timeout_and_finalize(&self) {
        trace!(
            "[MSG_PROC] Timeout reached for committee={:?} publisher={:?} root={:?}",
            self.committee,
            self.publisher,
            self.message_root
        );
