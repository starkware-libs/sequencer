...
pub enum EventStateManagerToEngine {
    ...
    Finalized { channel: Channel, publisher: PeerId, nonce: u64, message_root: MessageRoot },
...
enum ReconstructionState {
    PreConstruction {
        ...
        common_fields: Option<CommonFieldsAcrossShards>,
    },
...
impl ReconstructionState {
    ...
    fn add_unit(
        ...
        match self {
            Self::PreConstruction { received_shards, did_broadcast_my_shard, common_fields } => {
                if is_my_shard {
                    *did_broadcast_my_shard = true;
                }
                if common_fields.is_none() {
                    *common_fields = Some(CommonFieldsAcrossShards {
    ...
    fn handle_reconstruction_output(
        ...
        if should_broadcast {
            let (signature, nonce) = match state {
                ReconstructionState::PreConstruction { common_fields, .. } => {
                    let parts = common_fields.as_ref().expect("Common fields must exist");
