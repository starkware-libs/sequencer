...
impl ReconstructionState {
    ...
    fn maybe_emit(&mut self, tree_manager: &PropellerScheduleManager) -> AddUnitAction {
        match self {
            Self::PostConstruction { num_received_shards, reconstructed_message } => {
                if tree_manager.should_receive(*num_received_shards) {
...
impl MessageProcessor {
    ...
    async fn reconstruct_blocking(&self, shards: Vec<PropellerUnit>) -> ReconstructionResult {
        ...
        let my_index: usize = self.my_shard_index.0.try_into().expect("Shard index could not be converted to usize");
