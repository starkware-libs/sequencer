...
pub fn split_data_into_shards(
    message: Vec<u8>,
    number_of_data_shards: usize,
) -> Option<Vec<Vec<u8>>> {
    if !message.len().is_multiple_of(number_of_data_shards) {
        return None;
    }
    let shard_size = message.len() / number_of_data_shards;
...
pub fn generate_coding_shards(
    data_shards: &[Vec<u8>],
    number_of_coding_shards: usize,
) -> Result<Vec<Vec<u8>>, String> {
    if number_of_coding_shards == 0 {
        return Ok(Vec::new());
    }

    let number_of_data_shards = data_shards.len();
    ...
    let mut encoder =
        ReedSolomonEncoder::new(number_of_data_shards, number_of_coding_shards, shard_size)
            .map_err(|e| format!("Failed to create Reed-Solomon encoder: {}", e))?;

    for shard in data_shards.iter().take(number_of_data_shards) {
        encoder
...
pub fn reconstruct_message_from_shards(
    ...
    let mut data_shards = Vec::with_capacity(number_of_data_shards);
    for index in 0..number_of_data_shards {
