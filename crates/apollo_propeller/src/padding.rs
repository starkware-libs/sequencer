...
/// * `divisor` - The number the output length must be divisible by
pub fn pad_message(message: Vec<u8>, divisor: NonZeroUsize) -> Vec<u8> {
...
pub fn unpad_message(message: Vec<u8>) -> Result<Vec<u8>, UnpaddingError> {
    ...
    let (original_message_length, varint_prefix_len) =
        match unsigned_varint::decode::usize(&message) {
            ...
            Err(e) => {
                return Err(UnpaddingError::InvalidVarintLengthPrefix(
                    e.to_string(),
                    message.len(),
                ));
            }
