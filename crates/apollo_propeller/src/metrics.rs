...
pub struct PropellerMetrics {
    ...
    // Queue Size Metrics
    /// Current number of pending events in the events queue
    pub pending_events_queue_size: MetricGauge,

    /// Current number of messages being tracked in the message states cache
    pub message_states_cache_size: MetricGauge,
}
...
impl PropellerMetrics {
    ...
    pub fn register(&self) {
        ...
        // Queue size metrics
        self.pending_events_queue_size.register();
        self.pending_events_queue_size.set(0f64);
        self.message_states_cache_size.register();
        self.message_states_cache_size.set(0f64);
    }
    ...
    /// Update queue size gauges based on current queue state
    pub fn update_queue_sizes(&self, pending_events: usize, message_states: usize) {
        self.pending_events_queue_size.set(pending_events as f64);
        self.message_states_cache_size.set(message_states as f64);
    }
}
