...
fn test_get_peer_for_shard_id(
    ...
    let publisher = schedule_manager.get_nodes()[1].0; // Use the second peer as publisher
    let result = schedule_manager.get_peer_for_shard_index(&publisher, shard_index);
    match expected_result {
        Ok(peer_index) => assert_eq!(result.unwrap(), schedule_manager.get_nodes()[peer_index].0),
