...
impl Engine {
    ...
    pub fn unregister_committee(&mut self, committee_id: CommitteeId) -> bool {
        let removed = self.committees.remove(&committee_id).is_some();
        if removed {
    ...
    fn send_unit_to_peer(&mut self, unit: PropellerUnit, peer: PeerId) {
        let shard_len = u64::try_from(unit.shard().len()).unwrap_or(u64::MAX);
        // Only count as sent if the peer is connected; emit_handler_event will record the failure
        // via emit_event -> track_event if the peer is disconnected.
