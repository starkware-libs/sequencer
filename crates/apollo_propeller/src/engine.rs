...
impl Engine {
    ...
    pub(crate) async fn broadcast(
        ...
        let r = tokio::task::spawn_blocking(move || {
            create_units_to_publish(message, channel, keypair, num_data_shards, num_coding_shards)
        })
        .await
        .expect("Failed to join create_units_to_publish task");
    ...
    pub(crate) fn handle_send_error(&mut self, peer_id: PeerId, error: String) {
    ...
    fn handle_broadcaster_result(&mut self, result: Result<Vec<PropellerUnit>, ShardPublishError>) {
        match result {
            Ok(units) => {
    ...
    fn broadcast_prepared_units(
        ...
        tracing::trace!(
            "[BROADCAST] Publisher {:?} broadcasting {} units",
            self.local_peer_id,
            units.len()
        );
        ...
        loop {
            tokio::select! {
                ...
                Some(result) = self.broadcaster_results_rx.recv() => {
                    self.handle_broadcaster_result(result);
                }
