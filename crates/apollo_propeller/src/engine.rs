...
impl Engine {
    ...
    fn prepare_units(
        ...
        let tx = self.prepared_units_tx.clone();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let _ = tx.send((result, response_tx));
        });
    }
    ...
    fn handle_unit(&mut self, sender: PeerId, unit: PropellerUnit) {
        ...
        let _ = unit_tx.send((sender, unit));
    }
    ...
    fn emit_event(&mut self, event: Event) {
        self.to_behaviour_tx.send(EngineOutput::GenerateEvent(event)).expect("Behaviour has exited");
    }
            ...
            tokio::select! {
                Some(cmd) = commands_rx.recv() => match cmd {
                    ...
                    EngineCommand::Broadcast { channel, message, response_tx } => {
                        self.prepare_units(channel, message, response);
                    }
