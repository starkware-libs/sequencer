...
impl Handler {
    ...
    fn on_fully_negotiated_outbound(
        ...
        let index = self.outbound_substream.iter().position(|s| s.is_none()).expect(
            "Slot must be available because we only request substreams when a slot is free",
        );
    ...
    > {
            ...
            self.outbound_substream.iter_mut().zip(self.establishing_outbound_substream.iter_mut())
        {
            ...
            // the streams are long-lived (We only open a sub-stream once per connection assuming no
            // errors occur). If we don't have an outbound substream, request one
            if outbound_substream.is_none() {
                if !*is_establishing {
                    // Mark that we're establishing a substream to prevent duplicate requests.
                    // We return Poll::Ready here to emit the substream request event.
                    // The actual substream will be provided later via on_fully_negotiated_outbound.
