...
fn make_unit(publisher: PeerId, timestamp_ns: u64, root: MessageRoot) -> PropellerUnit {
    PropellerUnit::new(
        TEST_CHANNEL,
        publisher,
        root,
...
fn message_key(publisher: PeerId, timestamp_ns: u64, root: MessageRoot) -> MessageKey {
    MessageKey { channel: TEST_CHANNEL, publisher, timestamp_ns, root }
}

fn finalize_message(engine: &mut Engine, publisher: PeerId, timestamp_ns: u64, root: MessageRoot) {
...
async fn nonce_rejects_unit_with_old_timestamp() {
    let mut s = setup();
    s.engine.peer_nonce_timestamp.put(s.publisher, BASE_TIMESTAMP_NS);
...
async fn nonce_updated_on_cache_expiry() {
    ...
    assert!(s.engine.peer_nonce_timestamp.peek(&s.publisher).is_none());
    ...
    assert_eq!(
        s.engine.peer_nonce_timestamp.peek(&s.publisher),
