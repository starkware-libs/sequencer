...
fn make_unit(publisher: PeerId, timestamp: u64) -> PropellerUnit {
...
async fn test_fresh_unit_is_not_dropped() {
    ...
    let fresh_timestamp = now_secs();
    let unit = make_unit(publisher, fresh_timestamp);
...
fn test_stale_unit_is_dropped() {
    ...
    let stale_timestamp = now_secs() - engine.config.stale_message_timeout.as_secs() - 1;
    let unit = make_unit(publisher, stale_timestamp);
