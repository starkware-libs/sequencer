...
fn random_proto_unit(rng: &mut Rng) -> ProtoUnit {
    ...
    ProtoUnit {
        shard: rng.next_bytes(shard_len),
        index: rng.next_u64(),
...
async fn fuzz_mixed_valid_and_random_units() {
    ...
    // Drain all events and count how many valid units we got
            ...
            match rng.next_usize(6) {
                ...
                3 => {
                    ...
                    let error_type = match rng.next_usize(3) {
                        ...
                        _ => StreamUpgradeError::Io(std::io::Error::new(
...
async fn fuzz_alternating_inbound_errors_and_recovery() {
    ...
    for i in 0..30 {
        ...
        if rng.next_bool() {
            ...
            // Send valid data
            let unit = make_test_unit_with_shard(vec![i as u8; 10]);
