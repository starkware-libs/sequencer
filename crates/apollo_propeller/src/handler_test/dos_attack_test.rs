...
async fn events_to_emit_no_amplification_under_flood() {
    ...
    let num_units = 1000;
    let units: Vec<_> =
        (0..num_units).map(|i| make_test_unit_with_shard(vec![(i % 256) as u8; 20])).collect();
    ...
    // Now drain all events and count
    let mut received_count = 0;
    loop {
        match handler.next().now_or_never() {
            Some(Some(ConnectionHandlerEvent::NotifyBehaviour(HandlerOut::Unit(_)))) => {
                received_count += 1;
            }
...
async fn send_queue_mass_drain_on_error() {
    ...
    for i in 0..queue_size {
        simulate_send_unit(&mut handler, make_test_unit_with_shard(vec![(i % 256) as u8; 10]));
    }
...
async fn large_shard_units_no_crash() {
    ...
    let max_size = 4096;
    let mut handler = make_handler_with_max_size(max_size);
    let (inbound, remote, _h) = get_connected_streams().await;
...
async fn rapid_valid_invalid_oscillation() {
    ...
    validate_received_unit(&mut handler, &final_unit).await;
}
...
async fn inbound_flood_during_outbound_stall() {
    ...
    // Drain and verify all inbound units arrive despite outbound being stalled
    let mut received = Vec::new();
    for _ in 0..200 {
        match handler.next().now_or_never() {
            Some(Some(ConnectionHandlerEvent::NotifyBehaviour(HandlerOut::Unit(u)))) => {
                received.push(u);
            }
            Some(Some(_)) => continue,
            _ => break,
...
async fn dial_upgrade_error_rapid_cycling_no_leak() {
    ...
    for i in 0..500u16 {
        simulate_send_unit(&mut handler, make_test_unit_with_shard(vec![(i % 256) as u8; 20]));
        validate_outbound_substream_request(&mut handler).await;
