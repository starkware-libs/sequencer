...
pub fn make_handler() -> Handler {
    Handler::new(TEST_PROTOCOL, MAX_WIRE_MESSAGE_SIZE)
}

pub fn make_handler_with_max_size(max_size: usize) -> Handler {
    Handler::new(TEST_PROTOCOL, max_size)
}
...
pub async fn validate_received_unit(handler: &mut Handler, expected: &PropellerUnit) {
    let event = handler.next().await.unwrap();
    assert_matches!(
        event,
        ConnectionHandlerEvent::NotifyBehaviour(HandlerOut::Unit(ref received))
