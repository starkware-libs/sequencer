...
use crate::types::{Channel, ShardSignatureVerificationError, SignatureParts};
use crate::{
    signature,
    MessageRoot,
...
pub struct UnitValidator {
    ...
    verified_signature_parts: Option<SignatureParts>,
...
impl UnitValidator {
    ...
    fn verify_signature(
        ...
        if let Some(signature_parts) = &self.verified_signature_parts {
            let SignatureParts { signature, timestamp_ns } = signature_parts;
            return if signature == unit.signature() && *timestamp_ns == unit.timestamp_ns() {
                Ok(())
            } else {
        ...
        if let Ok(()) = &result {
            self.verified_signature_parts = Some(SignatureParts {
