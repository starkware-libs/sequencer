...
fn generate_random_unit<R: Rng>(rng: &mut R, max_shard_size: usize) -> PropellerUnit {
    ...
    PropellerUnit::new(channel, publisher, message_id, signature, index, shard, proof)
}
...
impl HandlerTestBehaviour {
    ...
    pub fn new(max_wire_message_size: usize) -> Self {
        Self::with_config(StreamProtocol::new("/propeller/1.0.0"), max_wire_message_size)
    }

    /// Create a new HandlerTestBehaviour with custom configuration
    pub fn with_config(protocol_id: StreamProtocol, max_wire_message_size: usize) -> Self {
        Self { protocol_id, max_wire_message_size, events: VecDeque::new() }
    }
...
impl NetworkBehaviour for HandlerTestBehaviour {
    ...
    fn handle_established_inbound_connection(
        ...
        Ok(Handler::new(self.protocol_id.clone(), self.max_wire_message_size))
    }
...
async fn e2e(seed: u64, transport_type: TransportType) {
    ...
    let max_shard_size = 1 << rng.gen_range(1..=18);
    ...
    let original_unit = generate_random_unit(&mut rng, max_shard_size);
    let mut sent = [0; 2];
