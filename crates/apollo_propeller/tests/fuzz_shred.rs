...
// fn corrupt_byte(bytes: &mut Vec<u8>, seed: u64) -> (usize, u8, u8) {
...
//             let byte_pos = rng.gen_range(0..bytes.len());
//             bytes.remove(byte_pos);
//             (byte_pos, 0, 0)
//         }
//         2 => {
//             // add a random byte
//             let byte_pos = rng.gen_range(0..=bytes.len());
//             bytes.insert(byte_pos, rng.gen_range(0..=255u8));
//             (byte_pos, bytes[byte_pos], bytes[byte_pos])
//         }
//         _ => unreachable!(),
//     }
// }
...
//     let messages = setup.sender.prepare_units(setup.valid_data).unwrap();
...
//             // Corrupt publisher to trigger ReceivedPublishedShard or UnexpectedSender
//             PropellerUnit::new(
//                 message.root(),
...
//             )
//         } else if seed % 73 == 0 {
//             // Create message with out-of-bounds shard index to trigger TreeError
//             // With 2 peers, only shard index 0 is valid, so use 1 or higher
//             PropellerUnit::new(
//                 message.root(),
//                 message.publisher(),
//                 message.signature().to_vec(),
...
//                 apollo_propeller::PropellerUnit::decode(&mut corrupted_bytes_mut, 1 << 20)
//             else {
//                 return;
//             };
//             corrupted_message
...
//                 "CRITICAL: Corrupted message passed validation! Seed: {}",
//                 seed,
//             ),
//             Err(error) => {
...
// pub(crate) fn generate_random_message(seed: u64) -> apollo_propeller::PropellerUnit {
//     let mut rng = ChaChaRng::seed_from_u64(seed);
//     apollo_propeller::PropellerUnit::random(&mut rng, 1 << 12)
// }
...
//             apollo_propeller::PropellerUnit::decode(&mut decode_buffer, 1 << 20)
//         else {
//             panic!("Encode then decode failed! Seed: {}, Original: {:?}", seed,
// original_message);         };
...
// \              every byte in the encoding matters, if this is not the case, why the useless
// bytes?",             seed, byte_pos, original_byte, new_byte
//         );
//     });
// }
...
//                 continue;
//             };
//             // Skip if the decoded message happens to be the same as the original.
//             // This can occur due to protobuf varint encoding allowing multiple representations.
