// //! Unit tests demonstrating ShardSendFailed events for disconnected peers.

// use std::time::Duration;

// use apollo_propeller::{Behaviour, Config, Event, MessageAuthenticity};
...
//     Config::builder().fanout(2).fec_data_shards(2).fec_coding_shards(2).build()
// }
...
//             match event {
//                 Event::ShardSendFailed { sent_to, error, .. } => {
...
//     let peers =
//         vec![(leader_peer_id, 3000), (connected_peer_id, 2000), (disconnected_peer_id, 1000)];

//     leader_swarm.behaviour_mut().set_peers(peers.clone()).unwrap();
//     connected_swarm.behaviour_mut().set_peers(peers).unwrap();

//     tokio::time::sleep(Duration::from_millis(100)).await;

//     let test_data = vec![42u8; 64];
//     leader_swarm.behaviour_mut().broadcast(test_data, 123).unwrap();
...
//     if send_failed_events.is_empty() {
//         panic!("No ShardSendFailed events - protocol should inform about disconnected peers");
//     } else {
//         // With 2 data shards + 2 coding shards, we expect 4 send failures per disconnected peer
//         // The tree topology determines how many peers get the message, so we check for at least
// 4         // failures
//         assert!(
...
//     let test_data = vec![42u8; 64];
//     leader_swarm.behaviour_mut().broadcast(test_data, 789).unwrap();
...
//     } else {
//         // With 2 data shards + 2 coding shards, we expect 4 send failures when connection drops
//         assert!(
//             send_failed_events.len() >= 4,
//             "Expected at least 4 failures, got {}",
//             send_failed_events.len()
...
//     let config = Config::builder().fanout(1).fec_data_shards(1).fec_coding_shards(1).build();
...
//     swarm.behaviour_mut().set_peers(vec![(peer_id, 2000), (target_peer, 1000)]).unwrap();
...
//     } else {
//         // With 1 data shard + 1 coding shard, we expect 2 send failures
