...
fn add_all_peers(swarms: &mut [Swarm<Behaviour>], peer_ids: &[PeerId]) {
    ...
    for swarm in swarms.iter_mut() {
        let _ = swarm.behaviour_mut().register_channel_peers(CHANNEL, peer_weights.clone());
    }
}
...
async fn broadcast_message(
    ...
    let join_handle =
        swarms[publisher_idx].behaviour_mut().broadcast(CHANNEL, test_message.to_vec()).unwrap();
    let message_root = join_handle.await.unwrap().unwrap();
    message_root
}
