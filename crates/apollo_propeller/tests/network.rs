...
async fn test_turbine_message_propagation_demo() {
    ...
    let config = Config::builder().fanout(6).fec_data_shards(4).fec_coding_shards(4).build();
    ...
    let _ = node1.behaviour_mut().set_peers(vec![(peer1, 1500), (peer2, 1000)]);
    let _ = node2.behaviour_mut().set_peers(vec![(peer1, 1500), (peer2, 1000)]);

    // Give connections time to stabilize
    tokio::time::sleep(Duration::from_millis(200)).await;

    // Leader broadcasts - must be divisible by num_data_shards
    let data_shards = node1.behaviour().config().fec_data_shards();
    let expected_size = data_shards * 64; // 64 bytes per shard
    let mut test_data = b"Hello Turbine Network!".to_vec();
    test_data.resize(expected_size, 0); // Pad to exact size
    node1.behaviour_mut().broadcast(test_data.clone(), 0).unwrap();
    ...
    for round in 0..20 {
        ...
        if let Some(event) = node2.next_behaviour_event().now_or_never() {
            match event {
                Event::ShardReceived { sender, shard } => {
                    tracing::info!(
                        "🎉 SUCCESS: Node2 received shard from {}: message_id={}, index={}",
                        sender,
                        shard.id.message_id,
                        shard.id.index
                    );
...
fn test_turbine_api_with_100_peers_fanout_6() {
    ...
    tracing::info!("🚀 Testing Turbine API with {} peers and fanout {}", NUM_PEERS, FANOUT);

    // Create configuration with fanout 6
    let config = Config::builder().fanout(FANOUT).fec_data_shards(16).fec_coding_shards(16).build();
    ...
    let peers: Vec<(PeerId, u64)> =
        (0..NUM_PEERS).map(|_| (PeerId::random(), rng.gen_range(100..10000))).collect();
    ...
    let peers_to_add: Vec<(PeerId, u64)> =
        peers.iter().filter(|(peer_id, _)| *peer_id != local_peer_id).cloned().collect();
    ...
    for test_round in 0..5 {
        ...
        let data_shards = turbine.config().fec_data_shards();
        let expected_size = data_shards * 64; // 64 bytes per shard
