...
// use apollo_propeller::{Behaviour, Config, MessageAuthenticity};
...
//     tracing::info!("Testing Propeller API with {} peers and fanout {}", NUM_PEERS, FANOUT);
...
//             let weight = rng.gen_range(100..10000);
//             (peer_id, weight)
//         })
//         .collect();
...
//     // Create behaviour
//     let mut propeller = Behaviour::new(MessageAuthenticity::Author(local_peer_id),
// config.clone());     // Local peer ID is now set automatically in constructor
...
//     // Data must be divisible by num_data_shards
//     let data_shards = config.fec_data_shards();
//     let test_data = vec![42u8; data_shards * 64]; // 64 bytes per shard
...
//         let data_shards = config.fec_data_shards();
//         let test_data = vec![message_idx as u8; data_shards * 64]; // 64 bytes per shard
...
//     for (fanout, data_shards, coding_shards) in configs {
//         let config = Config::builder()
//             .fanout(fanout)
//             .fec_data_shards(data_shards)
//             .fec_coding_shards(coding_shards)
//             .build();

//         let _propeller =
//             Behaviour::new(MessageAuthenticity::Author(PeerId::random()), config.clone());

//         assert_eq!(config.fanout(), fanout);
//         assert_eq!(config.fec_data_shards(), data_shards);
//         assert_eq!(config.fec_coding_shards(), coding_shards);
//     }
