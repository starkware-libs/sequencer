...
use crate::staking_manager::{Epoch, StakingManager, StakingManagerConfig};
...
fn set_stakers(contract: &mut MockStakingContract, epoch: Epoch, stakers: Vec<Staker>) {
    contract
        .expect_get_stakers()
        .with(eq(epoch.epoch_id), always())
        .returning(move |_, _| Ok(stakers.clone()));
}

fn set_current_epoch(contract: &mut MockStakingContract, epoch: Epoch) {
    contract.expect_get_current_epoch().times(1).returning(move |_| Ok(epoch.clone()));
}
...
fn set_current_epoch_with_times(
    ...
    contract.expect_get_current_epoch().times(times).returning(move |_| Ok(epoch.clone()));
}
...
fn get_committee_success(
    ...
    let committee = committee_manager.get_committee(BlockNumber(1), block_context).unwrap();
...
fn get_committee_cache(
    ...
    let committee = committee_manager.get_committee(BlockNumber(1), block_context.clone()).unwrap();
    ...
    let committee =
        committee_manager.get_committee(BlockNumber(101), block_context.clone()).unwrap();
    ...
    let err = committee_manager.get_committee(BlockNumber(1), block_context).unwrap_err();
...
fn get_committee_for_next_epoch(
    ...
    let err = committee_manager.get_committee(BlockNumber(150), block_context).unwrap_err();
    assert_matches!(err, CommitteeProviderError::InvalidHeight { .. });
}
...
async fn get_proposer_success(
    ...
    let proposer = committee_manager.get_proposer(BlockNumber(1), 0, block_context).await.unwrap();
...
async fn get_proposer_empty_committee(
    ...
    let err = committee_manager.get_proposer(BlockNumber(1), 0, block_context).await.unwrap_err();
...
async fn get_proposer_random_value_exceeds_total_weight(
    ...
    let _ = committee_manager.get_proposer(BlockNumber(1), 0, block_context).await;
}
