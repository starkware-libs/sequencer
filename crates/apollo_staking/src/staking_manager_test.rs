...
async fn get_committee_applies_dynamic_config_changes(
    ...
    // Create config for epoch 1 with committee size 2.
    let epoch1_config = test_config_with_committee_size(2);

    // Create config for epoch 2 with committee size 3.
    let epoch2_config = test_config_with_committee_size(3);
    ...
    // Test epoch 1: committee size should be 2.
    let committee = committee_manager.get_committee(E1_H1).await.unwrap();
    assert_eq!(committee.members().len(), 2);

    // Test epoch 2: committee size should be 3.
    let committee = committee_manager.get_committee(E2_H1).await.unwrap();
