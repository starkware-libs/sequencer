...
fn get_committee_success(
    ...
    let committee = committee_manager.get_committee(BlockNumber(1)).unwrap();

    assert_eq!(*committee, expected_committee);
}

#[rstest]
fn get_committee_cache(default_config: StakingManagerConfig, mut contract: MockStakingContract) {
    ...
    let committee = committee_manager.get_committee(BlockNumber(101)).unwrap();
    ...
    let err = committee_manager.get_committee(BlockNumber(1)).unwrap_err();
...
fn get_committee_for_next_epoch(
    ...
    let committee = (*committee_manager.get_committee(BlockNumber(101)).unwrap()).clone();
    ...
    let err = committee_manager.get_committee(BlockNumber(150)).unwrap_err();
    assert_matches!(err, CommitteeProviderError::InvalidHeight { .. });
}
