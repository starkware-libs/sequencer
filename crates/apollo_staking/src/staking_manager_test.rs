...
fn set_stakers(state: &mut State, block_context: &Context, stakers: &Vec<ContractStaker>) {
...
fn get_committee_cache(
    ...
    set_stakers(&mut state, &block_context, &vec![ContractStaker::from(&STAKER_1)]);
    let context = ExecutionContext {
        state_reader: state.clone(),
        block_context: block_context.clone(),
        state_sync_client: Arc::new(MockStateSyncClient::new()),
    ...
    // change.
    set_stakers(&mut state, &block_context, &vec![ContractStaker::from(&STAKER_2)]);
    let context = ExecutionContext {
        state_reader: state.clone(),
        block_context: block_context.clone(),
        state_sync_client: Arc::new(MockStateSyncClient::new()),
...
fn get_committee_filters_out_stakers_without_public_key(
    ...
    let mut contract_stakers: Vec<ContractStaker> =
        vec![STAKER_1, STAKER_2, STAKER_3].iter().map(&ContractStaker::from).collect();
    ...
    let committee = (*committee_manager.get_committee(1, context).unwrap()).clone();
...
async fn get_proposer_success(
    ...
    let contract_stakers: Vec<ContractStaker> =
        vec![STAKER_1, STAKER_2, STAKER_3, STAKER_4].iter().map(&ContractStaker::from).collect();
    set_stakers(&mut state, &block_context, &contract_stakers);
