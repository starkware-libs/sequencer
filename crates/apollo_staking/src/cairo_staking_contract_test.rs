...
fn get_stakers_success(state: Arc<Mutex<State>>) {
    ...
    assert_eq!(contract.get_stakers(0).unwrap(), vec![Staker::from(&STAKER_2)]);
}

#[rstest]
fn get_stakers_filters_missing_public_key(state: Arc<Mutex<State>>) {
    let contract = create_contract(state.clone());
...
#[rstest]
fn get_current_epoch_success(state: Arc<Mutex<State>>) {
    let contract = create_contract(state.clone());

    set_current_epoch(&mut state.lock().unwrap(), EPOCH_1);
    assert_eq!(contract.get_current_epoch().unwrap(), EPOCH_1);

    // Change the state and verify that the contract is updated.
    set_current_epoch(&mut state.lock().unwrap(), EPOCH_2);
    assert_eq!(contract.get_current_epoch().unwrap(), EPOCH_2);
}
