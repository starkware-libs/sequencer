...
fn test_deterministic_same_inputs() {
    let generator = BlockPseudorandomGenerator {
        randomness_block_hash: Some(BlockHash(StarkHash::from(123u64))),
    };
    let height = BlockNumber(100);
    let round = 5;
    let range = 10000;

    let result1 = generator.generate(height, round, range);
    let result2 = generator.generate(height, round, range);
...
fn test_deterministic_none_block_hash() {
    let generator = BlockPseudorandomGenerator { randomness_block_hash: None };
    let height = BlockNumber(100);
    let round = 5;
    let range = 10000;
...
fn test_range_zero() {
    ...
    let result = generator.generate(height, round, range);
