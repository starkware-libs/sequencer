...
use starknet_l1_gas_price::eth_to_fri_oracle::EthToFriOracleClient;
...
impl ConsensusManager {
    ...
    pub async fn run(&self) -> Result<(), ConsensusError> {
        ...
        let context = SequencerConsensusContext::new(
            ...
            Some(Arc::new(EthToFriOracleClient::new(
                self.config.eth_to_fri_oracle_config.base_url.clone(),
                self.config.eth_to_fri_oracle_config.headers.clone(),
            ))),
        );
