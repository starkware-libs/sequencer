...
impl ConsensusManager {
    ...
    pub async fn run(&self) -> Result<(), ConsensusError> {
        self.revert_batcher_blocks_and_halt_if_needed().await;
    ...
    async fn revert_batcher_blocks_and_halt_if_needed(&self) {
        let Some(revert_up_to_and_including) = self.config.revert_up_to_and_including else {
            return;
        };
