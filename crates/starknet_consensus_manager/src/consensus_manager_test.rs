...
async fn revert_batcher_blocks() {
    ...
    // TODO(dvir): consider calling here `consensus_manager.run()`, this will probably require to
    // add timeout.
    consensus_manager.revert_batcher_blocks(REVERT_UP_TO_AND_INCLUDING_HEIGHT).await;
}
