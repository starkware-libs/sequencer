...
#[rstest]
#[should_panic(expected = "Batcher's current block 10 is not larger than the target block 10. No \
                           reverts are needed.")]
#[case::equal_block(BATCHER_HEIGHT)]
#[should_panic(expected = "Batcher's current block 10 is not larger than the target block 11. No \
                           reverts are needed.")]
