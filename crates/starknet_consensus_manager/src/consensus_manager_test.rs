...
async fn revert_batcher_blocks_if_needed() {
    ...
    let manager_config = ConsensusManagerConfig {
        first_block_to_revert: Some(REVERT_FROM_HEIGHT),
        ..Default::default()
    };
...
async fn try_revert_from_equal_block() {
    ...
    let manager_config = ConsensusManagerConfig {
        first_block_to_revert: Some(BATCHER_HEIGHT),
        ..Default::default()
    };
...
async fn try_revert_from_larger_block() {
    ...
    let manager_config = ConsensusManagerConfig {
        first_block_to_revert: BATCHER_HEIGHT.next(),
        ..Default::default()
    };
...
async fn no_reverts_without_config() {
    let manager_config =
        ConsensusManagerConfig { first_block_to_revert: None, ..Default::default() };
