...
/// On a sampling interval boundary, so that an offset below the interval stays in this bucket.
const TIMESTAMP: u64 = 1_700_000_000 / SAMPLING_INTERVAL_SECONDS * SAMPLING_INTERVAL_SECONDS;
...
/// The sampling bucket a block timestamp falls into.
fn bucket_timestamp(block_timestamp: u64) -> u64 {
    block_timestamp / SAMPLING_INTERVAL_SECONDS * SAMPLING_INTERVAL_SECONDS
}

/// Every freshness bound is measured against the block timestamp, so the readings below are
/// dated relative to `TIMESTAMP` rather than to the bucket it falls into.
...
fn bucket_result(
    ...
    if bucket.bucket_timestamp != bucket_timestamp(timestamp) {
        return None;
    }
    bucket.result.clone()
}
...
/// Waits for the current bucket's spawned query to finish without calling `fetch_rate`, which
/// would harvest it. This is the state a bucket is left in when its query resolves after the last
/// call for it.
async fn wait_for_query_to_finish(client: &ChainlinkOracleClient) {
    for _ in 0..MAX_POLL_ATTEMPTS {
        let is_finished = {
            let state = client.state.lock().unwrap();
            state.current_bucket.as_ref().is_some_and(|bucket| {
                bucket.query.as_ref().is_some_and(|query| query.is_finished())
            })
        };
        if is_finished {
            return;
...
async fn first_call_spawns_a_query_and_later_calls_hit_the_cache() {
    ...
    assert_eq!(bucket_timestamp(same_bucket_timestamp), bucket_timestamp(TIMESTAMP));
...
async fn unresolved_bucket_falls_back_to_the_last_valid_rate() {
    ...
    let next_bucket_timestamp = TIMESTAMP + SAMPLING_INTERVAL_SECONDS;
...
/// A bucket is the block timestamp floored to the sampling interval, with no lag: a timestamp on
/// an interval boundary is its own bucket, and the whole interval after it maps there too.
#[rstest]
#[case::on_the_boundary(0)]
#[case::inside_the_interval(SAMPLING_INTERVAL_SECONDS - 1)]
#[tokio::test]
async fn bucketing_applies_no_lag(#[case] offset_into_the_interval: u64) {
    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, fresh_updated_at())),
    );
    let _ = client.fetch_rate(TIMESTAMP + offset_into_the_interval).await;

    let state = client.state.lock().unwrap();
    assert_eq!(
        state.current_bucket.as_ref().expect("A call must set the bucket").bucket_timestamp,
        TIMESTAMP
    );
}

/// The freshness guards are measured against the block timestamp, not against the bucket it falls
/// into. A round written in the block being priced leads the bucket by up to a whole sampling
/// interval, which is far past `max_future_updated_at_seconds`, yet leads the block by nothing.
#[tokio::test]
async fn freshness_is_measured_against_the_block_timestamp() {
    ...
    assert_eq!(bucket_timestamp(block_timestamp), TIMESTAMP);

    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, block_timestamp)),
    );
    assert_eq!(resolve_rate(&client, block_timestamp).await.unwrap(), STRK_TO_USD_RATE);
}

/// A failed read is retried within its own bucket, so a transient failure costs one retry interval
/// rather than the rest of the sampling interval.
#[tokio::test]
async fn failed_read_is_retried_after_the_retry_interval() {
    ...
    // One second short of the retry interval, the failure still stands.
    let before_retry_timestamp = TIMESTAMP + failure_retry_interval_seconds - 1;
    assert_matches!(
        client.fetch_rate(before_retry_timestamp).await,
        Err(ExchangeRateOracleClientError::ContractCallError(_))
    );
    assert_eq!(num_batcher_calls.load(Ordering::SeqCst), num_calls_after_first_attempt);

    // At the retry interval the failure is dropped and the same bucket is queried again.
    let retry_timestamp = TIMESTAMP + failure_retry_interval_seconds;
    assert_eq!(bucket_timestamp(retry_timestamp), TIMESTAMP);
    assert_matches!(
        client.fetch_rate(retry_timestamp).await,
        Err(ExchangeRateOracleClientError::QueryNotReadyError(_))
    );
    wait_for_bucket_result(&client, retry_timestamp).await;
    assert!(
...
/// The retry interval governs failed reads only. A bucket that read successfully keeps serving
/// that rate for the rest of its sampling interval, however many retry intervals it spans.
#[tokio::test]
async fn successful_read_is_not_requeried_within_its_bucket() {
    let failure_retry_interval_seconds = test_config().failure_retry_interval_seconds;
    let (batcher_client, num_batcher_calls) = counting_batcher_client(strk_usd_responses(
        FeedFixture::new(STRK_USD_ANSWER, fresh_updated_at()),
    ));
    ...
    let rate = resolve_rate(&client, TIMESTAMP).await.unwrap();
    let num_calls_after_success = num_batcher_calls.load(Ordering::SeqCst);

    let num_retry_intervals_in_the_bucket =
        SAMPLING_INTERVAL_SECONDS / failure_retry_interval_seconds;
    assert!(num_retry_intervals_in_the_bucket > 1);
    for num_elapsed_intervals in 1..num_retry_intervals_in_the_bucket {
        let later_timestamp = TIMESTAMP + num_elapsed_intervals * failure_retry_interval_seconds;
        assert_eq!(bucket_timestamp(later_timestamp), TIMESTAMP);
        assert_eq!(client.fetch_rate(later_timestamp).await.unwrap(), rate);
        tokio::task::yield_now().await;
    }
    assert_eq!(num_batcher_calls.load(Ordering::SeqCst), num_calls_after_success);
}

/// A query can resolve after the last call for its bucket. That success must be recorded before a
/// later bucket supersedes it, or the round trip is wasted and the fallback ages from an older
/// read than the client actually obtained.
#[tokio::test]
async fn a_success_resolved_after_the_last_call_for_its_bucket_is_not_lost() {
    let client = ChainlinkOracleClient::new(
        ChainlinkRateKind::StrkToUsd,
        test_config(),
        sampling_interval(),
    ...
    // The only call for this bucket, so nothing harvests the query it spawns.
    assert_matches!(
        client.fetch_rate(TIMESTAMP).await,
        Err(ExchangeRateOracleClientError::QueryNotReadyError(_))
    );
    ...
    // The next bucket's own query fails, so it can only answer from the harvested rate.
    let next_bucket_timestamp = TIMESTAMP + SAMPLING_INTERVAL_SECONDS;
    assert_eq!(client.fetch_rate(next_bucket_timestamp).await.unwrap(), STRK_TO_USD_RATE);
}

/// The last valid rate is served while it is within `MAX_FALLBACK_SAMPLING_INTERVALS` of the bucket
...
#[case::many_buckets_later(60, false)]
#[tokio::test]
async fn last_valid_rate_is_served_only_within_the_allowance(
    ...
    let later_timestamp = TIMESTAMP + num_intervals_ahead * SAMPLING_INTERVAL_SECONDS;
    wait_for_bucket_result(&client, later_timestamp).await;
...
/// A re-proposal asks for a bucket the client has already advanced past. That bucket spawns no
/// query of its own, so it is served the read the client holds, which leads it.
#[tokio::test]
async fn earlier_bucket_is_served_a_held_rate_within_the_allowance() {
    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, fresh_updated_at())),
    );
...
/// The allowance bounds the distance in both directions, so a bucket further back than it is not
/// priced from a read taken that far ahead of it.
#[tokio::test]
async fn earlier_bucket_past_the_allowance_is_not_served_a_held_rate() {
    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, fresh_updated_at())),
    );
