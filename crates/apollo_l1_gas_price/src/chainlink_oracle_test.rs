...
/// The cache key of the bucket a timestamp falls into.
fn bucket_of(timestamp: u64) -> u64 {
    quantized_query_timestamp(timestamp) / test_config().lag_interval_seconds
}
...
fn round_retdata(answer: u128, updated_at: u64) -> Vec<Felt> {
    // A realistic phase-encoded `round_id`: `(phase_id << 128) | aggregator_round_id`. It exceeds
    // u64, so a client that decoded it as one would fail these tests.
...
fn cached_result(
    ...
    client.cached_results.lock().unwrap().peek(&bucket_of(timestamp)).cloned()
}

/// Drives `fetch_rate` until the bucket's own result reaches the cache. Unlike `resolve_rate` this
/// does not stop at the first `Ok`, which for a failing bucket is the previous bucket's rate.
async fn wait_for_cached_result(client: &ChainlinkOracleClient, timestamp: u64) {
    for _ in 0..MAX_POLL_ATTEMPTS {
        if cached_result(client, timestamp).is_some() {
            return;
        }
...
/// A future `updated_at` saturates the staleness subtraction to zero, so without its own bound a
/// feed dated ahead of the query would be accepted as fresh forever.
#[rstest]
...
/// The saturating staleness subtraction makes the maximal `updated_at` look perfectly fresh, so a
/// feed poisoned once with it would otherwise never expire.
#[tokio::test]
...
async fn strk_usd_minimum_bound_is_inclusive(#[case] answer_offset: i8, #[case] is_accepted: bool) {
    let config = test_config();
    let boundary_answer = micro_units_to_answer(config.min_strk_usd_price_micro_usd, FEED_DECIMALS);
    ...
    let result = resolve_rate(&client, TIMESTAMP).await;
    if is_accepted {
        assert_eq!(result.unwrap(), micro_units_to_rate(config.min_strk_usd_price_micro_usd));
    } else {
        assert_matches!(
            result,
...
async fn strk_usd_maximum_bound_is_inclusive(#[case] answer_offset: i8, #[case] is_accepted: bool) {
    let config = test_config();
    let boundary_answer = micro_units_to_answer(config.max_strk_usd_price_micro_usd, FEED_DECIMALS);
    let answer = boundary_answer.wrapping_add_signed(answer_offset.into());
    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(answer, fresh_updated_at())),
    ...
    let result = resolve_rate(&client, TIMESTAMP).await;
    if is_accepted {
        assert_eq!(result.unwrap(), micro_units_to_rate(config.max_strk_usd_price_micro_usd));
    } else {
        assert_matches!(
            result,
            Err(ExchangeRateOracleClientError::RateOutOfBoundsError { pair_name, .. })
...
async fn eth_usd_minimum_bound_is_inclusive(#[case] answer_offset: i8, #[case] is_accepted: bool) {
    let config = test_config();
    let boundary_answer = micro_units_to_answer(config.min_eth_usd_price_micro_usd, FEED_DECIMALS);
...
async fn eth_usd_maximum_bound_is_inclusive(#[case] answer_offset: i8, #[case] is_accepted: bool) {
    let config = test_config();
    let boundary_answer = micro_units_to_answer(config.max_eth_usd_price_micro_usd, FEED_DECIMALS);
    let eth_usd_answer = boundary_answer.wrapping_add_signed(answer_offset.into());
...
async fn eth_to_fri_bounds_are_inclusive(
    ...
    let boundary_rate_micro_strk = match bound {
        DerivedRateBound::Minimum => config.min_eth_to_fri_rate_micro_strk,
        DerivedRateBound::Maximum => config.max_eth_to_fri_rate_micro_strk,
    };
...
async fn first_call_spawns_a_query_and_later_calls_hit_the_cache() {
    ...
    assert_eq!(bucket_of(same_bucket_timestamp), bucket_of(TIMESTAMP));
...
/// While the next bucket's query is still in flight, the previous bucket's rate is served rather
/// than blocking the proposal path.
#[tokio::test]
async fn unresolved_bucket_falls_back_to_the_previous_bucket() {
    let client = make_client(
        ChainlinkRateKind::StrkToUsd,
        strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, fresh_updated_at())),
    );
...
/// A cached failure keeps the batcher from being queried again this bucket, but it must not deny
/// the proposal path a rate the client already holds from the previous bucket.
#[tokio::test]
async fn cached_failure_still_falls_back_to_the_previous_bucket() {
    ...
    let next_bucket_timestamp = TIMESTAMP + test_config().lag_interval_seconds;
    wait_for_cached_result(&client, next_bucket_timestamp).await;
    assert_matches!(cached_result(&client, next_bucket_timestamp), Some(Err(_)));
...
async fn each_bucket_queries_the_feed_once() {
    ...
    // Two buckets ahead, so the previous-bucket fallback does not short-circuit the query.
    let later_timestamp = TIMESTAMP + 2 * test_config().lag_interval_seconds;
    resolve_rate(&client, later_timestamp).await.unwrap();
...
/// `fetch_rate` runs on every proposal build and validate, so a failure that is not cached would
/// hammer the batcher at block rate instead of once per lag interval. With no previous bucket to
/// fall back on, the cached failure is what the caller sees.
#[tokio::test]
async fn failed_query_is_cached_for_the_rest_of_the_bucket() {
    ...
    assert_eq!(cached_result(&client, TIMESTAMP - test_config().lag_interval_seconds), None);
...
/// The guard counters record why a query was rejected, so each guard must increment its own.
#[rstest]
#[case::stale_feed(
    strk_usd_responses(FeedFixture::new(STRK_USD_ANSWER, stale_updated_at())),
...
#[tokio::test]
async fn guard_counters_record_the_rejection_reason(
    #[case] responses: FeedResponses,
    #[case] guard_counter: &'static MetricCounter,
) {
    ...
    assert_eq!(
        guard_counter.parse_numeric_metric::<u64>(&recorder.handle().render()),
        Some(1),
        "{} did not record the rejection",
