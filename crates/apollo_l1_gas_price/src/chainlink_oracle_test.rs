...
/// The fallback must hold for the one call that observes a failing query complete and caches the
/// failure, not just the calls before and after it.
#[tokio::test]
async fn no_call_is_denied_a_rate_the_previous_bucket_holds() {
    let client = ChainlinkOracleClient::new(
        ChainlinkRateKind::StrkToUsd,
        test_config(),
        batcher_client_failing_after(
    ...
    let next_bucket_timestamp = TIMESTAMP + test_config().lag_interval_seconds;
    for _ in 0..MAX_POLL_ATTEMPTS {
        if cached_result(&client, next_bucket_timestamp).is_some() {
            break;
        }
        assert_eq!(
            client.fetch_rate(next_bucket_timestamp).await.expect("Previous rate should be served"),
            rate
        );
        tokio::task::yield_now().await;
    }
...
/// With no previous bucket to fall back on, the caller sees the cached failure.
#[tokio::test]
