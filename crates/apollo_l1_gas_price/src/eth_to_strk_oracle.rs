...
impl SerializeConfig for EthToStrkOracleConfig {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([
            ser_param(
                "url_header_list",
                &serialize_optional_list_with_nested_object(&self.url_header_list),
    ...
    fn default() -> Self {
        Self {
            url_header_list: Some(vec![UrlAndHeaders {
                base_url: Url::parse("https://api.example.com/api").expect("Invalid URL"),
...
impl EthToStrkOracleClient {
    ...
    fn spawn_query(
        ...
        let UrlAndHeaders { base_url, headers } = self
            .config
            .url_header_list
            .as_ref()
            .expect("url_header_list must be provided in EthToStrkOracleConfig")
            .get(self.index)
...
impl EthToStrkOracleClientTrait for EthToStrkOracleClient {
    ...
    async fn eth_to_fri_rate(&self, timestamp: u64) -> Result<u128, EthToStrkOracleClientError> {
        ...
        // First check if rate is already cached.
        ...
        let mut queries = self.queries.lock().expect("Lock poisoned");
        let handle = queries
