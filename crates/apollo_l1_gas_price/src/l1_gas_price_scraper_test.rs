...
fn make_fake_block(block_number: u64) -> L1BlockHeader {
    L1BlockHeader {
        number: block_number,
...
fn check_gas_prices_make_sense(data: &GasPriceData) -> bool {
...
fn setup_scraper(
    ...
    mock_contract.expect_get_block_header().returning(move |block_number| {
        if block_number >= end_block { Ok(None) } else { Ok(Some(make_fake_block(block_number))) }
    });

    let mut mock_provider = MockL1GasPriceProviderClient::new();
    mock_provider
        .expect_add_price_info()
        .withf(check_gas_prices_make_sense)
    ...
    mock_contract
        ...
        .returning(move |block_number| {
            if block_number >= END_BLOCK1 {
                ...
                Ok(Some(make_fake_block(block_number)))
            }
        });
    mock_contract
        .expect_get_block_header()
...
async fn l1_reorg_gas_price_scraper_error() {
        ...
        .returning(move |block_number| {
            if block_number >= END_BLOCK2 {
                ...
                Ok(Some(L1BlockHeader {
                    ...
                    base_fee_per_gas: u128::from(block_number) * GAS_PRICE,
                    blob_fee: u128::from(block_number) * DATA_PRICE,
                    hash: u64_to_block_hash(block_number - 1), // Different hash for reorg
                    parent_hash: u64_to_block_hash(block_number.saturating_sub(1)),
                }))
            }
        });
    ...
    assert!(matches!(error, L1GasPriceScraperError::L1ReorgDetected { .. }));
}
