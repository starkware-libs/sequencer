...
fn setup_scraper(
    ...
    mock_contract.expect_latest_l1_block_number().returning(move || Ok(end_block));
...
async fn run_l1_gas_price_scraper_two_blocks() {
    ...
    mock_contract.expect_latest_l1_block_number().returning(move || Ok(END_BLOCK2));
    mock_contract
...
async fn get_first_block_fails_then_succeeds() {
    ...
    let first_block_number = scraper.get_first_block_number().await.unwrap();
