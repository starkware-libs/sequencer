...
impl L1GasPriceProvider {
    ...
    pub fn get_price_info(&self, timestamp: BlockTimestamp) -> L1GasPriceProviderResult<PriceInfo> {
        ...
        // - not enough history: use blocks from start of cache up to `last_index`. `first_index` is
        //   0.
        // - enough history: take exactly `num_blocks`, `first_index` is `num_blocks - last_index`.
        let first_index = usize::min(num_blocks - last_index, 0);
