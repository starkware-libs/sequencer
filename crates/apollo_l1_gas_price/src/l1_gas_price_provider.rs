...
/// the HTTP config, the sampling interval, the metrics bundle, and the config key named on
/// failure) is derived here from `rate_kind` alone, so one feed's settings can only ever reach its
/// own client.
///
