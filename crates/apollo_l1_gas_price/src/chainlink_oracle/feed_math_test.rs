...
fn round_data_decodes_the_answer_and_the_update_time() {
    let round: ChainlinkRoundData =
        decode_retdata(round_retdata(STRK_USD_ANSWER, UPDATED_AT), CurrencyPair::StrkUsd).unwrap();
...
fn malformed_retdata_rejected(#[case] malformed_round_retdata: Vec<Felt>) {
    assert_matches!(
        decode_retdata::<ChainlinkRoundData>(malformed_round_retdata, CurrencyPair::StrkUsd),
