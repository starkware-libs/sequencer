...
impl ChainlinkRate for EthToFri {
    ...
    async fn query_rate(
        ...
        // Sequential, not `try_join`, for the reason `read_feed` reads its own two calls
        // sequentially: a failing leg would drop the other mid-flight, and the batcher's local
        // component server panics when a dropped request's response channel closes.
        let eth_to_usd_rate =
