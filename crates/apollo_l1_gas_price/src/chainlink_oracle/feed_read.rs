...
pub async fn read_feed(
    ...
    let decimals_retdata =
        call_view(batcher_client, feed_address, DECIMALS_ENTRY_POINT, pair).await?;
    let round_retdata =
        call_view(batcher_client, feed_address, LATEST_ROUND_DATA_ENTRY_POINT, pair).await?;
