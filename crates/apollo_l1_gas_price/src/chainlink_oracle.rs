...
impl ExchangeRateOracleClientTrait for ChainlinkOracleClient {
    ...
    async fn fetch_rate(&self, timestamp: u64) -> Result<u128, ExchangeRateOracleClientError> {
        ...
        if let Some(cached_result) = cache.get(&quantized_timestamp).cloned() {
            ...
            // Caching the failure bounds the batcher call volume, but must not deny the proposal
            // path a rate this client already holds.
        ...
        // Same fallback as the cached-failure path above, applied to the one call that observes
        // the failure first, so it is not the only call in the bucket denied the previous rate.
        if let Some(Ok(rate)) = cache.get(&previous_timestamp) {
            debug!(
                "Query for timestamp {timestamp} failed, using previous rate {rate} from \
                 quantized timestamp={previous_timestamp}"
            );
            return Ok(*rate);
        }
...
async fn read_feed(
    ...
    // `decimals()` is re-read every round rather than cached: the absolute bounds cannot catch a
    // misread value, since the STRK/USD band spans five decades on purpose (8 decimals misread as
    // 6 turns $0.03 into $3.00 and passes).
...
async fn call_view(
    ...
    // feed contract can stall its request loop. This client only caps the number of calls it makes.
