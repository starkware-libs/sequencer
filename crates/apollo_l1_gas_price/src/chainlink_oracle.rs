...
/// How many sampling intervals past its own bucket the last valid rate may still be served, when
/// the bucket being queried has no rate of its own. Three intervals covers three consecutive
/// sampling intervals of failed reads, and stays far inside `max_staleness_seconds`, which bounds
/// how far the price itself may have moved.
...
impl ExchangeRateOracleClientTrait for ChainlinkOracleClient {
    ...
    async fn fetch_rate(
        ...
        if let Some(valid_read) = last_valid_read.filter(|valid_read| {
            bucket_timestamp.saturating_sub(valid_read.bucket_timestamp) <= max_fallback_age_seconds
        }) {
            debug!(
