...
/// A reading's currency pair, used in guard errors and as the guard counters' label.
...
/// How many sampling intervals may separate the last valid rate's bucket from the bucket being
/// queried, when that bucket has no rate of its own. The bound applies in both directions: a
/// re-proposal for an earlier bucket is served a rate read ahead of it under the same allowance.
/// Three intervals covers three consecutive sampling intervals of failed reads, and stays far
...
/// makes the rate's distance from another bucket checkable, in either direction, when that bucket
...
/// The newest sampling bucket `fetch_rate` has been called for.
struct CurrentBucket {
    bucket_timestamp: u64,
    /// The bucket's own result, once its query resolves. Holding a success keeps a healthy feed to
    /// one query per bucket rather than one per proposal; a held failure is cleared by a retry.
    ...
    /// Block timestamp the bucket's latest query was issued for, which
    /// `failure_retry_interval_seconds` is measured from.
    last_attempt_block_timestamp: u64,
}
...
/// falls back to the last valid read, while that read is within `MAX_FALLBACK_SAMPLING_INTERVALS`
/// of the bucket being queried. A bucket is one `sampling_interval_seconds` window of block
/// timestamps, identified by the timestamp it starts at.
///
...
pub struct ChainlinkOracleClient {
    ...
    /// Width of the sampling bucket a successful read is held for, so that a healthy feed is read
    /// once per interval however many proposals it spans.
...
impl ChainlinkOracleClient {
    ...
    /// `block_timestamp` is the caller's own timestamp, which every freshness guard is measured
    /// against. `bucket_timestamp` only identifies the sampling bucket the result is held for.
    fn spawn_query(&self, block_timestamp: u64, bucket_timestamp: u64) -> RateQuery {
        let batcher_client = self.batcher_client.clone();
        let config = self.config.clone();
        let rate_kind = self.rate_kind;
        let metrics = self.metrics;
        AbortOnDropHandle::new(tokio::spawn(async move {
            let result = query_rate(&batcher_client, &config, rate_kind, block_timestamp).await;
            match &result {
                Ok(rate) => {
                    metrics.success_count.increment(1);
                    set_unix_now_seconds(metrics.last_success_timestamp);
                    metrics.rate.set_lossy(*rate);
                    debug!("Resolved {rate_kind:?} query for bucket {bucket_timestamp} to {rate}");
                }
                Err(error) => {
                    metrics.error_count.increment(1);
                    warn!("Failed {rate_kind:?} query for bucket {bucket_timestamp}: {error:?}");
                }
            }
            result
        }))
    ...
    fn store_resolved_result(&self, bucket: &mut CurrentBucket) {
        ...
        let bucket_timestamp = bucket.bucket_timestamp;
        let joined = bucket
            .query
            .take()
        ...
        let result = joined.unwrap_or_else(|error| {
            warn!("Query failed to join handle for bucket {bucket_timestamp}: {error:?}");
            self.metrics.error_count.increment(1);
            Err(ExchangeRateOracleClientError::JoinError(error.to_string()))
        });
        debug!("Storing result for bucket {bucket_timestamp}: {result:?}");
        bucket.result = Some(result);
    }

    /// Stores a resolved query's result on `bucket`, and records a success as the last valid read.
    /// Recording happens here rather than where the rate is returned, so that a query which
    /// resolved after the last call for its bucket is still harvested when that bucket is
    /// superseded, instead of being dropped together with the round trip that produced it.
    fn harvest_resolved_result(
        &self,
        bucket: &mut CurrentBucket,
        last_valid_read: &mut Option<ValidRead>,
    ) {
        self.store_resolved_result(bucket);
        if let Some(Ok(rate)) = &bucket.result {
            *last_valid_read =
                Some(ValidRead { rate: *rate, bucket_timestamp: bucket.bucket_timestamp });
        }
    }

    /// Replaces a held failure with a fresh query once `failure_retry_interval_seconds` has passed
    /// since the bucket's last attempt, so a transient failure does not freeze the price until the
    /// next sampling bucket. A query still in flight is left alone, keeping one query per client.
    fn retry_failed_bucket(&self, bucket: &mut CurrentBucket, block_timestamp: u64) {
        let seconds_since_attempt =
            block_timestamp.saturating_sub(bucket.last_attempt_block_timestamp);
        if !matches!(bucket.result, Some(Err(_)))
            || seconds_since_attempt < self.config.failure_retry_interval_seconds
        {
            return;
        }
        debug!(
            "Retrying failed bucket {} at block timestamp {block_timestamp}",
            bucket.bucket_timestamp
        );
        bucket.result = None;
        bucket.query = Some(self.spawn_query(block_timestamp, bucket.bucket_timestamp));
        bucket.last_attempt_block_timestamp = block_timestamp;
    }
}
...
impl ExchangeRateOracleClientTrait for ChainlinkOracleClient {
    ...
    async fn fetch_rate(
        ...
        let sampling_interval_seconds = self.sampling_interval_seconds.get();
        // The bucket is only a cache key. Every freshness guard is measured against
        // `block_timestamp` itself, inside the query.
        let bucket_timestamp =
            block_timestamp / sampling_interval_seconds * sampling_interval_seconds;
        ...
        if current_bucket.as_ref().is_none_or(|bucket| bucket.bucket_timestamp < bucket_timestamp) {
            if let Some(outgoing_bucket) = current_bucket {
                self.harvest_resolved_result(outgoing_bucket, last_valid_read);
            }
            *current_bucket = Some(CurrentBucket {
                bucket_timestamp,
                result: None,
                query: Some(self.spawn_query(block_timestamp, bucket_timestamp)),
                last_attempt_block_timestamp: block_timestamp,
            });
        }
        let bucket = current_bucket.as_mut().expect("Bucket is set above when empty");

        let bucket_result = if bucket.bucket_timestamp == bucket_timestamp {
            self.harvest_resolved_result(bucket, last_valid_read);
            self.retry_failed_bucket(bucket, block_timestamp);
            bucket.result.clone()
        } else {
            None
        };
        ...
        // A bucket with no rate of its own serves the last valid read while it is close enough.
        // The distance is measured between bucket timestamps rather than against wall clock, so
        // the decision is the same on every node for a given block timestamp. It is bounded in
        // both directions: the read an earlier bucket is served from is a read this client has
        // since advanced past, so it leads that bucket by as much as it may lag it.
        let max_fallback_distance_seconds =
            MAX_FALLBACK_SAMPLING_INTERVALS.saturating_mul(sampling_interval_seconds);
        if let Some(valid_read) = last_valid_read.filter(|valid_read| {
            bucket_timestamp.abs_diff(valid_read.bucket_timestamp) <= max_fallback_distance_seconds
        }) {
            debug!(
                "No rate for block timestamp {block_timestamp}, using rate {} from bucket {}",
                valid_read.rate, valid_read.bucket_timestamp
            );
            return Ok(valid_read.rate);
        }
        match bucket_result {
            Some(Err(error)) => Err(error),
            _ => Err(ExchangeRateOracleClientError::QueryNotReadyError(block_timestamp)),
        }
    }
}
...
async fn read_feed(
    ...
    // wide to catch that: STRK/USD accepts $0.0001 to $10 on purpose, so reading an 8-decimal
    ...
    if block_timestamp.saturating_sub(round.updated_at) > config.max_staleness_seconds {
    ...
    if round.updated_at.saturating_sub(block_timestamp) > config.max_future_updated_at_seconds {
