...
impl ExchangeRateOracleMetrics {
    ...
    pub fn record_success(&self, rate: ExchangeRate) {
        self.success_count.increment(1, &self.pair.labels());
        set_unix_now_seconds_with_labels(self.last_success_timestamp, &self.pair.labels());
        self.rate.set_lossy(rate, &self.pair.labels());
    }
