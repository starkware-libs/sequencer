...
// Make a provider with five block price. Timestamps are 2 seconds apart, starting from 0.
// To get the prices for the middle three blocks use the timestamp for block[3].
// Returns the provider, a vector of block prices to compare with, and timestamp of block[3].
fn make_provider() -> (L1GasPriceProvider, Vec<PriceInfo>, u64) {
    ...
    for i in 0..5 {
        ...
        provider.add_price_info(block_num, BlockTimestamp(time), price_info).unwrap();
    }
...
fn gas_price_provider_mean_prices() {
    ...
    // range. Timestamp3 (for sample[3]) is used to define the interval of samples 1 to 3.
    ...
    let gas_price_calculation = (block_prices[1].base_fee_per_gas.0
        + block_prices[2].base_fee_per_gas.0
        + block_prices[3].base_fee_per_gas.0)
        / num_samples;
    let data_price_calculation =
        (block_prices[1].blob_fee.0 + block_prices[2].blob_fee.0 + block_prices[3].blob_fee.0)
            / num_samples;
...
fn gas_price_provider_adding_samples() {
    ...
    provider.add_price_info(5, timestamp, price_info).unwrap();
    ...
    provider.add_price_info(6, timestamp, price_info).unwrap();
...
fn gas_price_provider_can_start_at_nonzero_height() {
    ...
    provider.add_price_info(42, timestamp, price_info).unwrap();
}
