...
async fn make_server(
    ...
    server
        ...
        .with_body(json!({key1: val1, key2: val2}).to_string())
        .create()
}
...
async fn eth_to_fri_rate_two_urls() {
    ...
    let _m1 = make_server(&mut server1, "foo", "0x0", "bar", 18).await;
    // For server2 we get the expected response.
    let _m2 = make_server(&mut server2, "price", &expected_rate_hex, "decimals", 18).await;
    ...
    let _m3 = make_server(&mut server2, "price", &expected_rate_hex, "bar", 18).await;
    // First request should fail because the cache is empty.
