...
async fn eth_to_fri_rate_uses_cache_on_quantized_hit() {
    ...
    let rate1 = client.eth_to_fri_rate(timestamp1).await;
    // Because caching is used, the second request reuses the first response and doesn't trigger
    // another server call.
    // Sleep to ensure the first request is completed before the second.
    tokio::time::sleep(Duration::from_millis(1)).await;
    let rate2 = client.eth_to_fri_rate(timestamp2).await;

    assert!(rate1.is_err());
    assert_eq!(rate2.unwrap(), expected_rate);
}
