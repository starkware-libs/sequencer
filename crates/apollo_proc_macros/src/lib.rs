...
fn async_timing_check(
    ...
    let time_limit_duration = quote! {
        std::time::Duration::from_millis(#time_limit_ms)
    };

    quote! {
        let start = std::time::Instant::now();
        let result = tokio::time::timeout(#time_limit_duration, async {
            #fn_block
        }).await;
...
/// ```
///
/// The default time limit is DEFAULT_TIME_LIMIT_MS. If the test exceeds this limit, it will panic
/// with a message indicating how long it took.
#[proc_macro_attribute]
...
pub fn timed_rstest(attr: TokenStream, input: TokenStream) -> TokenStream {
    generate_timed_test(attr, input, quote! { #[rstest] }, None, false)
}
...
pub fn timed_rstest_tokio(attr: TokenStream, input: TokenStream) -> TokenStream {
    generate_timed_test(attr, input, quote! { #[rstest] }, Some(quote! { #[tokio::test] }), true)
}
