...
/// Dynamic config provider for class manager's storage reader server.
...
impl DynamicConfigProvider for ClassManagerDynamicConfigProvider {
    async fn get_storage_reader_dynamic_config(
        ...
        let dynamic_config: ClassManagerDynamicConfig =
            self.config_manager_client.get_class_manager_dynamic_config().await.map_err(|e| {
                DynamicConfigError(format!("Failed to fetch class manager dynamic config: {}", e))
            })?;
