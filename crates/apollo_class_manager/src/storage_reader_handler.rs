...
/// Handler for storage reader requests in ClassManager.
pub struct ClassManagerStorageReaderHandler {
    storage_reader: StorageReader,
}

impl ClassManagerStorageReaderHandler {
    ...
    pub fn handle_request(&self, request: StorageReaderRequest) -> StorageReaderResponse {
        debug!("ClassManager handling storage reader request: {:?}", request.description());
        ...
        let txn = match txn_result {
            Ok(txn) => txn,
            Err(e) => {
                error!("ClassManager failed to open read transaction: {:?}", e);
...
mod tests {
    ...
    fn test_handle_marker_request() {
        ...
        let handler = ClassManagerStorageReaderHandler::new(reader);

        let request = StorageReaderRequest::GetMarker(apollo_storage::MarkerKind::Class);
