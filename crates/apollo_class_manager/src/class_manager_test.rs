...
#[tokio::test]
async fn class_manager_class_bytecode_length_validation() {
    // Setup.
    ...
    let class = RawClass::try_from(SierraContractClass::default()).unwrap();
    let (expected_executable_class, _) = mock_compile_expectations(&mut compiler, class.clone());
    let executable_class_bytecode_length =
        ContractClass::try_from(expected_executable_class.clone()).unwrap().bytecode_length();

    // Prepare class manager.
    let mut class_manager = ClassManager::new_for_testing(
        compiler,
        ClassManagerConfig {
            max_compiled_contract_bytecode_size: executable_class_bytecode_length - 1,
            ..Default::default()
        },
    );
    ...
    assert_matches!(
        class_manager.add_class(class).await,
        Err(ClassManagerError::ContractBytecodeSizeTooLarge { .. })
    );
}
