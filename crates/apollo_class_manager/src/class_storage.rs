...
use apollo_storage::metrics::STORAGE_OPEN_CLASS_MANAGER_READ_TRANSACTIONS;
...
impl ClassHashStorage {
    pub fn new(config: ClassHashStorageConfig) -> ClassHashStorageResult<Self> {
        ...
        let (reader, writer) = apollo_storage::open_storage(
            storage_config,
            Some(&STORAGE_OPEN_CLASS_MANAGER_READ_TRANSACTIONS),
        )?;
