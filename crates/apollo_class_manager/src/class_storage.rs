...
pub struct ClassHashStorage {
    ...
    // Kept alive to maintain the server running. Wrapped in Arc so that cloning
    // ClassHashStorage shares the same task; we only abort when the last clone is dropped.
    #[allow(dead_code)]
