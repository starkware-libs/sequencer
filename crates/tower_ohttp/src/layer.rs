        ...
        Box::pin(async move {
            ...
            let bhttp_message = match bhttp::Message::read_bhttp(&mut Cursor::new(&bhttp_bytes)) {
                ...
                Err(error) => {
                    ...
                    return Ok(OhttpError::InvalidFormat("Invalid Binary HTTP message")
                        .into_response()
                        .map(build_body));
                }
            };
            ...
            let inner_request = match rebuild_request(&bhttp_message) {
                Ok(request) => request,
                Err(err) => return Ok(err.into_response().map(build_body)),
            };
            let inner_request = inner_request.map(build_body);

            let inner_response = inner.oneshot(inner_request).await?;

            match encapsulate_response(inner_response, server_response).await {
                Ok(response) => Ok(response.map(build_body)),
                Err(err) => Ok(err.into_response().map(build_body)),
            }
        })
    }
}
