...
async fn decapsulation_strips_client_supplied_request_id() {
    ...
    let response =
        harness.ohttp_round_trip("POST", "/", b"", &[("x-request-id", b"client-inner-id")]).await;

    assert_eq!(response.status, 200);
    assert_eq!(response.bhttp_message.header().get(b"x-echo-saw-request-id").unwrap(), b"false");
}
