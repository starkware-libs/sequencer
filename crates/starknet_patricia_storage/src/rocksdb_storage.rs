...
impl Default for RocksDbOptions {
    fn default() -> Self {
        ...
        // TODO(Nimrod): Make statistics collection configurable.
...
impl RocksDbOptions {
    pub fn from_config(config: &RocksDbStorageConfig) -> Self {
        ...
        block.set_bloom_filter(config.bloom_filter_bits, false);
...
pub struct RocksDbStorageConfig {
    ...
    pub path: PathBuf,
    /// Number of threads for parallelism (used for `increase_parallelism`).
    pub num_threads: i32,
...
impl RocksDbStorageConfig {
    pub fn new(path: &Path) -> Self {
        Self { path: path.to_path_buf(), ..Default::default() }
    }
}
...
impl RocksDbStorage {
    ...
    pub fn new(config: RocksDbStorageConfig) -> PatriciaStorageResult<Self> {
