...
fn collect_db_stats(db: &DB, cache: Option<&Cache>) -> CfStats {
    ...
    let cache_usage_mb =
        if let Some(cache) = cache { (cache.get_usage() as u64) / (1024 * 1024) } else { 0 };
    let level_stats =
...
impl RocksdbStorageStats {
    ...
    pub fn collect(db: &RocksDbStorage) -> Self {
        let latest_stats = collect_db_stats(&db.latest_db, Some(&db.latest_cf_cache_handle));
        let history_stats = collect_db_stats(&db.recent_history_db, None);
