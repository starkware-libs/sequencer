...
impl MdbxStorage {
    pub fn open(path: &Path, table: String) -> PatriciaStorageResult<Self> {
        let db = MdbxDb::<WriteMap>::new().open(path)?;
        let txn = db.begin_rw_txn()?;
        txn.create_table(Some(&table), TableFlags::empty())?;
        txn.commit()?;
        Ok(Self { db, table })
    }
}
...
impl Storage for MdbxStorage {
    fn get(&self, key: &DbKey) -> PatriciaStorageResult<Option<DbValue>> {
        ...
        let table = txn.open_table(Some(&self.table.clone()))?;
