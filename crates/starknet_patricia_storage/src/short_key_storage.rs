...
//                 &mut self,
//                 keys: &[&DbKey],
//             ) -> PatriciaStorageResult<Vec<Option<DbValue>>> { let small_keys = keys .iter()
//               .map(|key| Self::small_key(key)) .collect::<Vec<_>>(); self.storage
//               .mget_mut(small_keys.iter().collect::<Vec<&DbKey>>().as_slice()) .await
//             }
