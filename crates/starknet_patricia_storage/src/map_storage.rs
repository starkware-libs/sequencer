...
impl<S: Storage> CachedStorage<S> {
    ...
    pub fn total_writes(&self) -> u128 {
        u128::from(self.writes.load(Ordering::Acquire))
    }
}
...
impl<S: Storage> Storage for CachedStorage<S> {
    ...
    async fn mset(&mut self, key_to_value: DbHashMap) -> PatriciaStorageResult<()> {
        self.writes.fetch_add(
            u64::try_from(key_to_value.len()).expect("usize should fit in u64"),
            Ordering::Relaxed,
        );
    ...
    fn get_stats(&self) -> PatriciaStorageResult<Self::Stats> {
        ...
        let writes = u128::from(self.writes.load(Ordering::Acquire));
