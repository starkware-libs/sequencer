...
impl TransactionPool {
    ...
    pub fn remove_up_to_nonce_when_committed(
        ...
        for tx_ref in &removed_txs {
            ...
            let time_spent = (now - submission_time).to_std().unwrap().as_secs_f64();
            TRANSACTION_TIME_SPENT_UNTIL_COMMITTED.record(time_spent);

            if let Some(batching_time) =
                self.txs_by_submission_time.take_batching_time(tx_ref.tx_hash)
            {
                let time_until_batched = (batching_time - submission_time)
                    .to_std()
                    .expect("batching_time must be later than submission_time")
                    .as_secs_f64();
    ...
    pub fn mark_taken_for_batching(&mut self, tx_hash: TransactionHash) {
        self.txs_by_submission_time.mark_taken_for_batching(tx_hash);
    }
    ...
    pub fn clear_taken_for_batching_for_address(&mut self, address: ContractAddress) {
        ...
        for tx_hash in tx_hashes {
            self.txs_by_submission_time.clear_taken_for_batching(tx_hash);
        }
    }
...
impl TimedTransactionMap {
    ...
    /// Marks the transaction as "taken for batching" (returned by `get_txs`).
    fn mark_taken_for_batching(&mut self, tx_hash: TransactionHash) {
        let now = self.clock.now();
        if let Some(submission_id) = self.hash_to_submission_id.get_mut(&tx_hash) {
            if submission_id.batching_time.is_none() {
                submission_id.batching_time = Some(now);
            }
        }
    }

    fn take_batching_time(&mut self, tx_hash: TransactionHash) -> Option<DateTime> {
        self.hash_to_submission_id.get_mut(&tx_hash)?.batching_time.take()
    }

    fn clear_taken_for_batching(&mut self, tx_hash: TransactionHash) {
        if let Some(submission_id) = self.hash_to_submission_id.get_mut(&tx_hash) {
            submission_id.batching_time = None;
        }
    }
}
