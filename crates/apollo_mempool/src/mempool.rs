...
pub struct Mempool {
    ...
    delayed_queues: Vec<AddTransactionQueue>,
...
impl Mempool {
    ...
    fn drain_ready_delayed_txs(&mut self) {
        let now = self.clock.now();
        let ready_txs: Vec<_> =
            self.delayed_queues.iter_mut().flat_map(|q| q.drain_ready(now)).collect();
    ...
    fn add_or_delay_tx(&mut self, mut args: AddTransactionArgs) {
        let now = self.clock.now();
        for queue in &mut self.delayed_queues {
    ...
    /// Validates that no delayed transaction exists with the same (address, nonce), preventing
    /// front-running of delayed transactions via fee escalation.
    fn validate_no_delayed_tx_front_run(
        &self,
        tx_reference: TransactionReference,
    ) -> MempoolResult<()> {
        if self.delayed_queues.iter().any(|q| q.contains(tx_reference.address, tx_reference.nonce))
        {
    ...
    fn size_in_bytes(&self) -> u64 {
        self.tx_pool.size_in_bytes()
            + self.delayed_queues.iter().map(|q| q.size_in_bytes()).sum::<u64>()
    }
    ...
    fn update_accounts_with_gap(&mut self, address_to_nonce: AddressToNonce) {
        for (address, account_nonce) in address_to_nonce {
            ...
            // exists.
            if self.delayed_queues.iter().any(|q| q.contains(address, account_nonce)) {
