...
impl Mempool {
    ...
    pub fn commit_block(&mut self, args: CommitBlockArgs) {
        ...
        for address in addresses_to_rewind {
            // These txs were proposed (staged) but did not make it to commit. Clear any
            // "taken for batching" marks so that a future successful take measures the correct
            // batching time.
            self.tx_pool.clear_taken_for_batching_for_address(address);
