...
impl Mempool {
    pub fn new(config: MempoolConfig, clock: Arc<dyn Clock>) -> Self {
        Mempool {
            ...
            tx_controller: TransactionController::new(clock.clone()),
    ...
    pub fn commit_block(&mut self, args: CommitBlockArgs) {
        ...
        for (&address, &next_nonce) in &address_to_nonce {
            ...
            if self.tx_queue.get_nonce(address).is_none() {
                if let Some(tx_reference) =
                    self.tx_controller.tx_pool.get_by_address_and_nonce(address, next_nonce)
                {
                    self.insert_to_tx_queue(tx_reference);
                }
            }
        ...
        for address in addresses_to_rewind {
            ...
            let tx_reference = self
                ...
                .next()
                .unwrap_or_else(|| panic!("Address {address} should appear in transaction pool."));
            self.tx_queue.remove(address);
            self.insert_to_tx_queue(*tx_reference);
        }
        ...
        for tx_hash in rejected_tx_hashes {
            if let Ok(tx) = self.tx_controller.remove(tx_hash) {
    ...
    pub fn account_tx_in_pool_or_recent_block(&self, account_address: ContractAddress) -> bool {
        self.state.contains_account(account_address)
            || self.tx_controller.tx_pool.contains_account(account_address)
    }
    ...
    fn validate_incoming_tx(
        ...
        if self.tx_controller.tx_pool.get_by_tx_hash(tx_reference.tx_hash).is_ok() {
    ...
    fn enqueue_next_eligible_txs(&mut self, txs: &[TransactionReference]) -> MempoolResult<()> {
        for tx in txs {
            ...
            if let Some(next_tx_reference) =
                self.tx_controller.tx_pool.get_next_eligible_tx(current_account_state)?
            {
    ...
    fn handle_fee_escalation(&mut self, incoming_tx: &InternalRpcTransaction) -> MempoolResult<()> {
        ...
        if !self.config.enable_fee_escalation {
            if self.tx_controller.tx_pool.get_by_address_and_nonce(address, nonce).is_some() {
        ...
        let Some(existing_tx_reference) =
            self.tx_controller.tx_pool.get_by_address_and_nonce(address, nonce)
        else {
    ...
    pub fn mempool_snapshot(&self) -> MempoolResult<MempoolSnapshot> {
        Ok(MempoolSnapshot {
            transactions: self.tx_controller.tx_pool.chronological_txs_hashes(),
    ...
    fn size_in_bytes(&self) -> u64 {
        self.tx_controller.tx_pool.size_in_bytes() + self.delayed_declares.size_in_bytes()
    }
    ...
    fn content(&self) -> MempoolContent {
        MempoolContent {
            tx_pool: self.tx_controller.tx_pool.tx_pool(),
    ...
    fn update_state_metrics(&self) {
        MEMPOOL_POOL_SIZE.set_lossy(self.tx_controller.tx_pool.len());
