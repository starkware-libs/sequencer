...
impl TransactionQueueTrait for FifoTransactionQueue {
    ...
    fn remove_by_address(&mut self, address: ContractAddress) -> bool {
        let hashes_to_remove: Vec<TransactionHash> = self
            .queue
            .iter()
            .filter_map(|&tx_hash| {
                self.hash_to_tx
                    .get(&tx_hash)
                    .filter(|tx_ref| tx_ref.address == address)
                    .map(|_| tx_hash)
            })
            .collect();
        ...
        self.queue.retain(|tx_hash| !hashes_to_remove.contains(tx_hash));

        !hashes_to_remove.is_empty()
    }
