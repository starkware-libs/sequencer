...
impl FifoTransactionQueue {
    ...
    fn enqueue_tx(&mut self, tx: FifoTransaction, insert_to_front: bool) {
        let source = if insert_to_front { "rewind" } else { "insert" };
        debug!(
            "FIFO {}: tx_hash={}, timestamp={}, queue_before={:?}",
            source, tx.tx_reference.tx_hash, tx.timestamp, self.queue
        );
...
impl TransactionQueueTrait for FifoTransactionQueue {
    fn insert(&mut self, tx_reference: TransactionReference, _validate_resource_bounds: bool) {
        ...
        self.enqueue_tx(FifoTransaction { tx_reference, timestamp }, false);
    }
    ...
    fn remove_txs(&mut self, txs: &[TransactionReference]) -> Vec<TransactionReference> {
        ...
        self.queue.retain(|tx| {
            if tx_hashes.contains(&tx.tx_reference.tx_hash) {
                removed_hashes.insert(tx.tx_reference.tx_hash);
                false
            } else {
                true
            }
    ...
    fn rewind_txs(&mut self, rewind_data: RewindData<'_>) -> IndexSet<TransactionHash> {
        ...
        let rewound_hashes: IndexSet<TransactionHash> = txs_to_rewind
            .into_iter()
            .map(|tx| {
                // Add to FRONT of queue so rewound transactions are processed before new ones.
                self.enqueue_tx(tx, true);
