...
impl MempoolCommunicationWrapper {
    ...
    async fn try_fetch_tx_block_metadata(
        ...
        let Some(&timestamp) = metadata.get("timestamp") else {
            return Err("invalid response: missing 'timestamp'".to_string());
        };
        let Some(&raw_block_number) = metadata.get("block_number") else {
            return Err("invalid response: missing 'block_number'".to_string());
        };
        Ok(TxBlockMetadataResponse { timestamp, block_number: BlockNumber(raw_block_number) })
    }
}
