...
impl BootstrapClient {
    ...
    async fn get_bootstrap_state(&self) -> Result<BootstrapState, String> {
    ...
    async fn get_bootstrap_transactions(&self) -> Result<Vec<RpcTransaction>, String> {
        let request =
    ...
    /// Checks if the given internal transaction matches one of the expected bootstrap
    /// transactions. Comparison is done by (contract_address, nonce) since
    /// `InternalRpcTransaction` and `RpcTransaction` are different types.
    ///
...
pub enum BootstrapValidation {
    ...
    NotBootstrapping,
    /// The transaction is a valid bootstrap transaction; skip resource bounds validation.
    ValidBootstrapTx,
}
