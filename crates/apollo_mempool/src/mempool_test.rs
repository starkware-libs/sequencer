...
/// Shows a bug in the mempool where a bootstrap declare transaction is added to the mempool, but
/// then it ends up in the pending queue.
...
fn bootstrap_declare_in_pending_after_add() {
    ...
    let mut mempool = Mempool::new(
        MempoolConfig {
            static_config: MempoolStaticConfig {
                validate_resource_bounds: true, /* This causes transactions with low gas price to
                                                 * go to pending_queue */
                ..Default::default()
            },
            ..Default::default()
    ...
    // Create and add a bootstrap declare transaction (zero resource bounds, bootstrap address,
    // nonce 0).
    use starknet_api::executable_transaction::DeclareTransaction;
