...
#[rstest]
#[case::gas_resolved_after_commit(
    vec![(1, true)], // Add nonce 1, a gap created.
    2, false // Commit nonce 2, resolving the gap.
)]
#[case::gap_created_after_commit(
    vec![(0, false), (2, false)], // Add nonces 0 and 2, no gap.
    1, true // Commit nonce 1, creating a gap.
)]
fn gap_after_commit(
    mut mempool: Mempool,
    #[case] nonce_gap_expected_sequence: Vec<(u64, bool)>,
    #[case] committed_nonce: u8,
    #[case] gap_expected: bool,
) {
    let addr = "0x0";
    for (nonce, expected_gap) in nonce_gap_expected_sequence {
...
#[rstest]
#[case::tx_follows_gap("0x0")] // Adding tx that creates a gap.
#[case::first_tx_creates_gap("0x1")] // Adding unrelated tx to fill capacity.
fn rejects_tx_that_creates_or_follows_gap_when_mempool_is_full(#[case] addr: &str) {
    let tx1 = add_tx_input!(tx_hash: 1, address: addr, tx_nonce: 1, account_nonce: 0);
    let mut mempool = Mempool::new(
    ...
    let tx2 = add_tx_input!(tx_hash: 2, address: "0x0", tx_nonce: 2, account_nonce: 0);
