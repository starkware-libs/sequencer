...
fn metrics_correctness() {
    ...
    // Advance the clock so that declare_1 will move to the pool, and add another declare tx.
    fake_clock.advance(mempool.config.declare_delay + Duration::from_secs(1));
    add_tx(&mut mempool, &declare_2);

    // Request 1 transaction from the mempool, so that we have a staged transaction.
