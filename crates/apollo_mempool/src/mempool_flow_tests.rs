...
fn test_eviction_tracker(mut mempool: Mempool) {
    // Account 0: Naive ready transaction, no gap.
    let tx_ready_address_0 =
        add_tx_input!(tx_hash: 1, address: "0x0", tx_nonce: 0, account_nonce: 0);

    // Account 1: Starts with a gap (nonce 1 present, nonce 0 missing), gap will be filled later by
    // adding nonce 0.
    let tx_with_gap_address_1 =
        add_tx_input!(tx_hash: 2, address: "0x1", tx_nonce: 1, account_nonce: 0);

    // Account 2: Same as above, but gap will be filled later via commit.
    let tx_with_gap_address_2 =
        add_tx_input!(tx_hash: 3, address: "0x2", tx_nonce: 1, account_nonce: 0);

    // Account 3: No gap, has both nonce 0 and 1.
    ...
    // Account 4: Has nonces 0 and 1, but nonce 0 will be rejected → gap created.
    ...
    // Account 5: Has nonces 0 and 2; when nonce 0 accepted → gap created (missing 1).
    let tx_ready_address_5 =
        add_tx_input!(tx_hash: 8, address: "0x5", tx_nonce: 0, account_nonce: 0);
    let tx_with_gap_address_5 =
        add_tx_input!(tx_hash: 9, address: "0x5", tx_nonce: 2, account_nonce: 0);
    ...
    // Fill the gap for account 1 by adding nonce 0.
    ...
    // Resolves gap for account 2.
    // Commits nonce 0 of account 5, leaves gap.
    // Rejects nonce 0 of account 4, creates gap.
