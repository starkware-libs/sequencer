...
#[rstest]
fn test_resolve_block_metadata_persists_after_queue_emptied(mut mempool: Mempool) {
    let input1 = add_tx_input!(tx_hash: 1, address: "0x1", tx_nonce: 0, account_nonce: 0);
    let input2 = add_tx_input!(tx_hash: 2, address: "0x1", tx_nonce: 1, account_nonce: 0);
    ...
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 1000);
...
fn test_get_txs_does_not_return_txs_with_different_timestamp(mut mempool: Mempool) {
    ...
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 1001);
...
fn test_get_txs_pauses_once_on_block_number_gap(mut mempool: Mempool) {
    ...
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 100);
    assert_eq!(mempool.get_txs(10).unwrap(), vec![input1.tx, input2.tx]);

    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 200);
    assert_eq!(mempool.get_txs(10).unwrap(), vec![input3.tx]);

    // Block 4 is missing.
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 300);
...
fn test_rewind_partial_block_then_continue_to_next_block(mut mempool: Mempool) {
    ...
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 1000);
    get_txs_and_assert_expected(&mut mempool, 10, &[input3.tx, input4.tx]);
    assert_eq!(mempool.resolve_block_metadata().unwrap().timestamp, 2000);
...
#[rstest]
fn test_resolve_block_metadata_returns_none_when_never_had_transactions(mut mempool: Mempool) {
    assert!(mempool.resolve_block_metadata().is_none());
}
