...
use crate::communication::{MempoolCommunicationWrapper, TxBlockMetadataResponse};
use crate::mempool::Mempool;

// Starts a mock HTTP server that simulates the recorder's get_tx_block_metadata endpoint.
// Returns the base URL (e.g., "http://127.0.0.1:12345").
async fn start_mock_recorder(response: Result<TxBlockMetadataResponse, StatusCode>) -> String {
...
async fn test_fetch_tx_block_metadata_success() {
    let recorder_url = start_mock_recorder(Ok(TxBlockMetadataResponse {
        timestamp: 1000u64,
