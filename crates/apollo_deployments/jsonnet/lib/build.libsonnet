...
// override object the applicative config reads from (see applicative_config/init.libsonnet):
...

{
  build(layoutName, params)::
    ...
    // Effective replacers: the committed defaults, overridden by any user-supplied replacers. The
    // applicative config reads each key directly (no inline fallback), so every key must be present.
    local replacers = defaultReplacers + std.get(params, 'replacers', {});
