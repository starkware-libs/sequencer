...
//   - overrides.<key>  — a value the environment must supply per-deployment via the `overrides` arg.
// When a new Rust config field is added, add the key here with its intended deployment default, or
// reference `overrides.<key>` if it must be set per-environment.

// `overrides` is the single object carrying every value the environment must supply:
//   - cross-cutting values reused across multiple config sites are bound to the locals below, so
//     those sites are the *same expression* — equal by construction, impossible to set divergently;
//   - per-component required values are referenced inline at their exact schema path, e.g.
//     `overrides.batcher_config.dynamic_config.n_concurrent_txs`, so `overrides` is shaped like the
//     config it patches and the environment sets each value at its natural location.
function(overrides)
  ...
  local validateResourceBounds = overrides.validate_resource_bounds;
  local maxCpuTime = overrides.max_cpu_time;
  ...
  local revertConfig = overrides.revert_config;
  local behaviorMode = overrides.behavior_mode;
  local validatorId = overrides.validator_id;
  local validationOnly = overrides.validation_only;
  {
    base_layer: {
      base_layer_config: {
    ...
    state_sync: {
      state_sync_config: {
        ...
        static_config: {
          central_sync_client_config: overrides.state_sync_config.static_config.central_sync_client_config,
          network_config: overrides.state_sync_config.static_config.network_config,
          p2p_sync_client_config: overrides.state_sync_config.static_config.p2p_sync_client_config,
          revert_config: revertConfig,
          rpc_config: {
