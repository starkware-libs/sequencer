...
function(overrides)
  ...
  local validationOnly = false;
  // `state_sync` optional sub-configs. The env supplies each one as `null` (the `#is_none:true`
  // option ⇒ None) or as an object (active, possibly carrying only the sparse sub-field overrides);
  // an active option is `default` deep-merged with those overrides. Secret/pointer fields use the
  // same placeholders as the other network configs (`secret_key`/`http_headers` → '' filled by
  // secrets.json; `chain_id`/`starknet_url` → the pointer locals).
