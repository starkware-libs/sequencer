...
// dotted-key map. The applicative components read `replacers['<key>']` directly (no inline fallback);
// `build()` supplies the effective map as `default_replacers + <user replacers>` (user values win),
// which is logically identical to the former per-key `std.get(replacers, '<key>', <default>)`.
