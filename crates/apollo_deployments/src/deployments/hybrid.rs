...
fn create_hybrid_instance_config_override(
    ...
    // TODO(Tsabary): this can be moved to the deployment override module, which removed the need to
    // re-compute this per node. Not urgent.
    // List all nodes as respective bootstrap peers.
    let mut consensus_bootstrap_peer_multiaddrs = vec![];
    let mut mempool_bootstrap_peer_multiaddrs = vec![];

    for (node_id_iter, _) in node_and_validator_ids.iter() {
        let consensus_bootstrap_peer_multiaddr = build_peer_address(
            HybridNodeServiceName::Core,
            CORE_SERVICE_PORT,
            *node_id_iter,
            &get_peer_id(*node_id_iter),
        );
        consensus_bootstrap_peer_multiaddrs.push(consensus_bootstrap_peer_multiaddr);
        ...
        mempool_bootstrap_peer_multiaddrs.push(mempool_bootstrap_peer_multiaddr);
    }
    ...
    InstanceConfigOverride::new(
        NetworkConfigOverride::new(
            Some(consensus_bootstrap_peer_multiaddrs),
            consensus_advertised_multiaddr,
        ),
        NetworkConfigOverride::new(
            Some(mempool_bootstrap_peer_multiaddrs),
