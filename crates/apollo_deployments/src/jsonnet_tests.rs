...
/// Evaluates `build({ chain_params, node_params })` and returns its JSON: a map from
...
fn eval_build(layout: &str) -> Value {
    ...
    // match the `lib/layouts/<name>.libsonnet` filename, folded into `chain_params.topology`.
    eval_jsonnet(
        "build",
        format!(
            "(import 'lib/build.libsonnet').build({{ chain_params: (import \
             '{TESTING_CHAIN_PARAMS_PATH}') + {{ mandatory+: {{ topology: import \
             'lib/layouts/{layout}.libsonnet' }} }}, node_params: import \
