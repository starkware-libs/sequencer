...
pub(crate) enum DeploymentType {
    New,
    // TODO(Tsabary): enable the following variant representing an existing node being upgraded,
    // and remove the dead code annotation. Also distinguish the resultant configs based on the
    // variant.
    #[allow(dead_code)]
    Upgrade,
}
...
impl DeploymentType {
    fn validator_id_offset(&self) -> usize {
        match self {
            ...
            DeploymentType::Upgrade => DEFAULT_VALIDATOR_ID.try_into().unwrap(),
        }
    }
}
...
fn get_validator_id(id: usize, deployment_type: DeploymentType) -> String {
    ...
    format!("0x{}", id + deployment_type.validator_id_offset())
}
