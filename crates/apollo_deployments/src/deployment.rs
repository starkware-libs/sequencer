...
impl Deployment {
    ...
    pub fn new(
        ...
        let additional_config_filenames: Vec<String> =
            std::iter::once(base_app_config_file_path.to_string_lossy().to_string())
                .chain(config_override.config_files(&config_override_dir, false))
                .collect();
    ...
    pub fn dump_config_override_files(&self) {
        self.config_override.config_files(&self.config_override_dir, true);
    }

    #[cfg(test)]
    pub fn test_dump_application_config_files(&self) {
        let app_configs = self.application_config_values();
        for (service_name, value) in app_configs.into_iter() {
            let config_path = self.deployment_name.get_service_file_path(&service_name);

            serialize_to_file_test(value, &config_path, FIX_BINARY_NAME);
        }
    }
}
