...
/// Evaluates `build(<params>)` with the `<layout>` layout folded into `chain_params.topology`, and
/// returns the per-service config map: service name → that service's fully-assembled
/// `SequencerNodeConfig` as JSON. `layout` is the layout name, used to import the layout object.
/// `params` is a jsonnet expression yielding the `{ chain_params, node_params }` object — e.g. an
/// inline object literal or `"import 'testing/integration_node.jsonnet'"`. Paths in `params`
/// resolve relative to the jsonnet dir (the evaluator's import root).
pub fn build_service_configs(layout: &str, params: &str) -> serde_json::Map<String, Value> {
    ...
    let built = eval_jsonnet(
        "build",
        format!(
            "(import 'lib/build.libsonnet').build({params} + {{ chain_params+: {{ topology: \
             import 'lib/layouts/{layout}.libsonnet' }} }})"
        ),
    );
    match built {
