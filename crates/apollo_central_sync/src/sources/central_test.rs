...
#[tokio::test]
async fn stream_block_state_updates() {
    ...
    let stream = central_source
        .stream_new_block_state_updates(expected_block_num, BlockNumber(END_BLOCK_NUMBER));
    pin_mut!(stream);
    while let Some(Ok(central_block_state_update)) = stream.next().await {
        assert_eq!(expected_block_num, central_block_state_update.block_number);
...
#[tokio::test]
async fn stream_block_state_updates_some_are_missing() {
    const START_BLOCK_NUMBER: u64 = 5;
    ...
    let stream = central_source
        .stream_new_block_state_updates(expected_block_num, BlockNumber(END_BLOCK_NUMBER));
    pin_mut!(stream);
    while let Some(block_state_update_result) = stream.next().await {
        if expected_block_num == BlockNumber(MISSING_BLOCK_NUMBER) {
            assert_matches!(
                block_state_update_result,
                Err(CentralError::BlockNotFound { block_number })
                if block_number == expected_block_num
            );
            break;
        }
        let central_block_state_update = block_state_update_result.unwrap();
        assert_eq!(expected_block_num, central_block_state_update.block_number);
        expected_block_num = expected_block_num.unchecked_next();
    }
...
#[tokio::test]
async fn stream_block_state_updates_error() {
    const START_BLOCK_NUMBER: u64 = 5;
    const END_BLOCK_NUMBER: u64 = 13;
    ...
    assert_eq!(expected_block_num, BlockNumber(ERROR_BLOCK_NUMBER));
}

#[tokio::test]
async fn stream_new_block_state_updates() {
    const START_BLOCK_NUMBER: u64 = 5;
    ...
    let stream = central_source
        .stream_new_block_state_updates(initial_block_num, BlockNumber(END_BLOCK_NUMBER));
    pin_mut!(stream);

    let Some(Ok(central_block_state_update)) = stream.next().await else {
        panic!("Match of streamed block state update failed!");
    };
    let current_block_num = central_block_state_update.block_number;
    let current_block_hash = central_block_state_update.block_hash;
    let state_diff = central_block_state_update.state_diff;
    let deployed_contract_class_definitions =
        central_block_state_update.deployed_contract_class_definitions;
