...
impl<TStarknetClient: StarknetReader + Send + Sync + 'static> StateUpdateStream<TStarknetClient> {
    ...
    fn next_output(&mut self) -> Option<CentralResult<CentralStateUpdate>> {
        ...
        // #region agent log
        let queue_blocks: Vec<u64> =
            self.downloaded_state_updates.iter().map(|(bn, _, _)| bn.0).collect();
        debug_log_stream(
            "state_update_stream.rs:next_output",
            "Outputting state update",
            &format!(
                r#"{{"output_block":{},"remaining_in_queue":{:?}}}"#,
                block_number.0, queue_blocks
            ),
            "D",
        );
        // #endregion
        ...
        *should_poll_again = true;
        match maybe_state_update {
            ...
            Ok(Some(state_update)) => {
                ...
                // #region agent log
                let queue_blocks_before: Vec<u64> =
                    self.downloaded_state_updates.iter().map(|(bn, _, _)| bn.0).collect();
                debug_log_stream(
                    "state_update_stream.rs:handle_downloaded",
                    "Adding downloaded state update to queue",
                    &format!(
                        r#"{{"downloaded_block":{},"queue_before":{:?}}}"#,
                        block_number.0, queue_blocks_before
                    ),
                    "D",
                );
                // #endregion
