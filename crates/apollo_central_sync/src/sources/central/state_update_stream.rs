...
impl<TStarknetClient: StarknetReader + Send + Sync + 'static> StateUpdateStream<TStarknetClient> {
    ...
    fn next_output(&mut self) -> Option<CentralResult<CentralStateUpdate>> {
        ...
        let classes_and_casms = self.downloaded_classes.drain(..n_classes);
        let classes_and_casms = class_hashes.into_iter().zip(classes_and_casms).collect();
    ...
    fn handle_downloaded_classes(
        ...
        *should_poll_again = true;
        match maybe_class {
            ...
            Ok((Some(class), casm)) => {
                self.downloaded_classes.push_back((class, casm));
                Ok(())
            }
            // Class was not found.
