...
fn state_update_with_classes(class_hashes: Vec<ClassHash>) -> apollo_starknet_client::reader::StateUpdate {
...
fn class_backlog_backpressure_blocks_and_recovers() {
    ...
    let class_cache =
        std::sync::Arc::new(std::sync::Mutex::new(lru::LruCache::new(NonZeroUsize::new(2).unwrap())));
...
async fn class_backlog_backpressure_does_not_drop_or_deadlock() {
    ...
    for expected_block_number in 0..3u64 {
        let central_state_update = stream
            ...
            .unwrap_or_else(|err| panic!("unexpected error at block {expected_block_number}: {err:?}"));
