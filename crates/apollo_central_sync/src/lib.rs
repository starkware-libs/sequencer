...
> GenericStateSync<TCentralSource, TPendingSource, TBaseLayerSource>
{
    ...
    async fn store_state_diff(
        ...
        // Collect CASMs for storage when we're in the non-backward-compatible path (same block
        // where we fetch from central). Only used when store_sierras_and_casms is true.
        let store_sierras_and_casms = self.config.store_sierras_and_casms;
        let mut casms_to_append: Vec<(ClassHash, CasmContractClass)> = Vec::new();
            ...
            if compiler_backward_compatibility_marker <= block_number {
                ...
                for (expected_class_hash, class) in &classes {
                    ...
                    if store_sierras_and_casms {
                        casms_to_append.push((*expected_class_hash, casm.clone()));
                    }
                    ...
                    let contract_class = ContractClass::V1((casm, sierra_version));
        ...
        self.perform_storage_writes(move |writer| {
            ...
            if store_sierras_and_casms {
                ...
                for (class_hash, casm) in &casms_to_append {
                    debug_assert!(
                        classes.contains_key(class_hash),
                        "casms_to_append should only contain class hashes from classes"
                    );
                    debug!("Appending CASM for class hash {class_hash}.");
