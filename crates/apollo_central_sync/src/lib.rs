...
// Reads the header, state, compiled-class, and compiler-backward-compatibility markers from the
// writer's RW transaction, so they reflect batched (uncommitted) writes. Kept as a separate async
// fn (rather than inlined into the stream) so the non-`Send` transaction is fully contained here
// and the progress stream only holds the marker values across awaits.
async fn read_progress_markers(
    writer: &Arc<Mutex<StorageWriter>>,
) -> Result<(BlockNumber, BlockNumber, BlockNumber, BlockNumber), StorageError> {
...
fn check_sync_progress(
    ...
    try_stream! {
        ...
        let (mut header_marker, mut state_marker, mut casm_marker, _) =
            read_progress_markers(&writer).await?;
