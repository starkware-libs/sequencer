...
> GenericStateSync<TCentralSource, TPendingSource, TBaseLayerSource>
{
    ...
    async fn store_state_diff(
        ...
        if let Some(class_manager_client) = &self.class_manager_client {
            ...
            if !non_backward_compatible_casms.is_empty() {
                for (expected_class_hash, casm) in &non_backward_compatible_casms {
...
fn stream_new_state_diffs<TCentralSource: CentralSourceTrait + Sync + Send>(
    ...
    try_stream! {
        loop {
            ...
            let mut backward_compatiblity_reached = false;
            ...
            while let Some(maybe_state_diff) = state_diff_stream.next().await {
                ...
                // TODO(noamsp): decide if this flag is okay (case that the flag was turned to true but the store for a non-backward block failed)
                if !backward_compatiblity_reached {
                    let compiler_backward_compatibility_marker = reader.begin_ro_txn()?.get_compiler_backward_compatibility_marker()?;
                    if block_number >= compiler_backward_compatibility_marker {
                        backward_compatiblity_reached = true;
                    } else {
