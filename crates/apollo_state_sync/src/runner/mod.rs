...
/// Dynamic config provider for state sync's storage reader server.
...
impl DynamicConfigProvider for StateSyncDynamicConfigProvider {
    async fn get_storage_reader_dynamic_config(
        ...
        let dynamic_config: StateSyncDynamicConfig =
            self.config_manager_client.get_state_sync_dynamic_config().await.map_err(|e| {
                DynamicConfigError(format!("Failed to fetch state sync dynamic config: {}", e))
            })?;
...
impl StateSyncResources {
    pub fn new(
        ...
        let dynamic_config_provider =
            Arc::new(StateSyncDynamicConfigProvider { config_manager_client });
