...
pub struct StateSync {
    ...
    config_manager_client: SharedConfigManagerClient,
    dynamic_config: StateSyncDynamicConfig,
}
...
impl StateSync {
    ...
    async fn update_dynamic_config(&mut self) {
        let state_sync_dynamic_config = self
            .config_manager_client
            .get_state_sync_dynamic_config()
            .await
            .expect("Should be able to get state sync dynamic config");
        self.dynamic_config = state_sync_dynamic_config;
    }
}
