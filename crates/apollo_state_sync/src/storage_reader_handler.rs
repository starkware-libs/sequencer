...
/// Handler for storage reader requests in StateSync.
pub struct StateSyncStorageReaderHandler {
    storage_reader: StorageReader,
}

impl StateSyncStorageReaderHandler {
    ...
    pub fn handle_request(&self, request: StorageReaderRequest) -> StorageReaderResponse {
        debug!("StateSync handling storage reader request: {:?}", request.description());
        ...
        let txn = match txn_result {
            Ok(txn) => txn,
            Err(e) => {
                error!("StateSync failed to open read transaction: {:?}", e);
...
mod tests {
    ...
    fn test_handle_marker_request() {
        ...
        let handler = StateSyncStorageReaderHandler::new(reader);
