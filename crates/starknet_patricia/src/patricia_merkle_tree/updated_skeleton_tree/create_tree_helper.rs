...
use crate::patricia_merkle_tree::original_skeleton_tree::tree::{
    OriginalSkeletonNodeMapWrapper,
...
impl UpdatedSkeletonTreeImpl {
    ...
    pub(crate) fn finalize_middle_layers<'a>(
        ...
        if original_skeleton.get_nodes().is_empty() {
            ...
            let mut wrapper = OriginalSkeletonNodeMapWrapper::new(original_skeleton);
            self.update_node_in_nonempty_tree(&NodeIndex::ROOT, &mut wrapper, &sorted_leaf_indices)
        }
    }
    ...
    pub(crate) fn update_node_in_nonempty_tree<'a>(
        &mut self,
        root_index: &NodeIndex,
        original_skeleton: &mut OriginalSkeletonNodeMapWrapper<'a>,
        leaf_indices: &SortedLeafIndices<'_>,
    ) -> TempSkeletonNode {
