...
impl From<&TempSkeletonNode> for UpdatedSkeletonNode {
    fn from(temp_node: &TempSkeletonNode) -> Self {
        match temp_node {
            ...
            TempSkeletonNode::OriginalEdge { path, n_new_hashes } => {
                UpdatedSkeletonNode::Edge { path_to_bottom: *path, n_new_hashes: *n_new_hashes }
            }
            TempSkeletonNode::OriginalUnmodified { hash } => {
                UpdatedSkeletonNode::UnmodifiedSubTree(*hash)
            }
            TempSkeletonNode::Leaf => UpdatedSkeletonNode::Leaf,
            TempSkeletonNode::Empty => {
                unreachable!("An empty node has no corresponding updated skeleton node.")
            }
        }
    }
}
    ...
    fn finalize_binary_child(&mut self, index: NodeIndex, child: &TempSkeletonNode) {
        match child {
            TempSkeletonNode::OriginalBinary { .. } | TempSkeletonNode::OriginalEdge { .. } => {
                self.skeleton_tree.insert(index, child.into());
            }
