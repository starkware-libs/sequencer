...
use starknet_patricia_storage::storage_trait::{create_db_key, DbStorageKey, DbStorageValue};
...
fn create_patricia_key(val: u128) -> DbStorageKey {
    create_db_key(PatriciaPrefix::InnerNode.into(), &U256::from(val).to_be_bytes())
}

fn create_binary_val(left: u128, right: u128) -> DbStorageValue {
    DbStorageValue(
        (create_32_bytes_entry(left).into_iter().chain(create_32_bytes_entry(right))).collect(),
    )
}

fn create_edge_val(hash: u128, path: u128, length: u8) -> DbStorageValue {
    DbStorageValue(
...
pub fn create_binary_entry(left: u128, right: u128) -> (DbStorageKey, DbStorageValue) {
    (create_patricia_key(left + right), create_binary_val(left, right))
}

pub fn create_edge_entry(hash: u128, path: u128, length: u8) -> (DbStorageKey, DbStorageValue) {
