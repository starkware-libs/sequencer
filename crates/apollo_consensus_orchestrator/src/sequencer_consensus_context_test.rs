...
#[tokio::test]
async fn build_proposal_with_skip_write_prev_height_blob() {
    ...
    // Report that the previous block was already written.
    deps.state_sync_client
        .expect_get_latest_block_number()
        .times(1)
        .return_const(Ok(Some(HEIGHT.prev().unwrap())));

    // Clear the "default" expectations on the cende ambassador. If we can skip writing the blob, it
    // should not be called at all.
    deps.cende_ambassador = MockCendeContext::new();

    let mut context = deps.build_context();
    let _fin_receiver = context
        .build_proposal(ProposalInit { height: HEIGHT, ..Default::default() }, TIMEOUT, true)
        .await;
}
