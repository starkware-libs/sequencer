...
fn make_config_manager_client(
    ...
    LocalConfigManagerReaderClient::new(config_rx)
}
...
async fn test_dynamic_config_updates_min_gas_price() {
    ...
    // Create a watch channel seeded with the first config. We'll update it to the second config
    // between the two set_height_and_round() calls so the second read picks up the new values.
    let first_context_config = ContextDynamicConfig {
    ...
    let second_context_config = ContextDynamicConfig {
    ...
    let (config_tx, config_rx) = tokio::sync::watch::channel(NodeDynamicConfig {
        context_dynamic_config: Some(first_context_config),
        ..Default::default()
    });
    ...
    context_deps.config_manager_client = Some(config_manager_client);
    ...
    // Update the watch channel to the second config before the next height
    config_tx
        .send(NodeDynamicConfig {
            context_dynamic_config: Some(second_context_config),
            ..Default::default()
        })
        .unwrap();
