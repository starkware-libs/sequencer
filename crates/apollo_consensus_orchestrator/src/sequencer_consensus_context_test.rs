...
async fn validate_then_repropose(#[case] execute_all_txs: bool) {
    ...
    context.decision_reached(HEIGHT_0, ROUND_1, TEST_PROPOSAL_COMMITMENT, None).await.unwrap();
}
...
async fn validate_then_build_then_decision_reached_round_0_uses_round_0_init() {
    ...
    context.decision_reached(HEIGHT_0, ROUND_0, TEST_PROPOSAL_COMMITMENT, None).await.unwrap();
}
...
async fn oracle_fails_on_second_block(#[case] l1_oracle_failure: bool) {
    ...
    context.decision_reached(HEIGHT_0, ROUND_0, proposal_commitment, None).await.unwrap();
...
async fn change_gas_price_overrides() {
    ...
    context.decision_reached(HEIGHT_1, ROUND_1, proposal_commitment, None).await.unwrap();
