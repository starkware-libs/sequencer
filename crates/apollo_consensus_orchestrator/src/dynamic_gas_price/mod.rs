...
//!   price ceiling (so a broken or malicious STRK/USD oracle cannot walk the fee up without limit).
//!
...
///
/// Either way the result is clamped into the geometric bounds returned by `fee_proposal_bounds`,
/// since the frozen `fee_actual` can itself sit above `max_gas_price`.
///
/// `max_gas_price` is the L2 gas price ceiling in force for this block, or `None` before
/// [`L2_GAS_PRICE_CAP_VERSION`](crate::fee_market::L2_GAS_PRICE_CAP_VERSION).
...
///
/// with `margin = margin_ppt / PPT_DENOMINATOR`, both then capped at `max_gas_price` when a
/// ceiling is in force (`None` before
/// [`L2_GAS_PRICE_CAP_VERSION`](crate::fee_market::L2_GAS_PRICE_CAP_VERSION), no ceiling).
///
...
///
/// The ceiling caps *both* bounds, not just `upper`: capping only `upper` could leave `lower`
/// above the ceiling, rejecting an honest proposer pinned there. Capping both keeps
/// `lower <= upper`, so an honest proposal is always in range.
///
...
pub(crate) fn fee_proposal_bounds(
    ...
    match max_gas_price {
        Some(max_gas_price) => (lower.min(max_gas_price.0), upper.min(max_gas_price.0)),
        None => (lower, upper),
    }
}
