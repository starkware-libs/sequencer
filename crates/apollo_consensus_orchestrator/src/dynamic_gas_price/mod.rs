...
/// Denominator for parts-per-thousand config and versioned-constants values, e.g. `2` = 0.2%.
...
/// `fee_actual` can itself sit above `max_l2_gas_price`, the ceiling from
...
pub fn compute_fee_proposal(
    ...
    let (lower, upper) = fee_proposal_bounds(fee_actual, margin_ppt, max_l2_gas_price);
...
///
/// with `margin = margin_ppt / PPT_DENOMINATOR`, both then capped at `max_l2_gas_price`, from
/// [`l2_gas_price_cap`](crate::fee_market::l2_gas_price_cap).
///
...
pub(crate) fn fee_proposal_bounds(
    ...
    (lower.min(max_l2_gas_price.0), upper.min(max_l2_gas_price.0))
}
