...
use apollo_l1_gas_price_types::{ExchangeRateOracleClientTrait, MockExchangeRateOracleClientTrait};
...
impl Node {
    ...
    async fn propose(&self) -> GasPrice {
        ...
        let fee_target = match &self.oracle {
            Some(oracle) => match oracle.fetch_rate(0).await {
...
fn mock_oracle_failing() -> Arc<dyn ExchangeRateOracleClientTrait> {
    ...
    mock.expect_fetch_rate().returning(|_| {
        Err(ExchangeRateOracleClientError::RequestError("simulated failure".to_string()))
    });
    Arc::new(mock)
}
...
mod consensus_flow {
    ...
    fn scripted_oracle(rates: Vec<Option<u128>>) -> Arc<dyn ExchangeRateOracleClientTrait> {
        ...
        let mut mock = MockExchangeRateOracleClientTrait::new();
        mock.expect_fetch_rate().returning(move |_| {
            ...
            *last.lock().unwrap() = next;
            match next {
                Some(rate) => Ok(rate),
                None => {
                    Err(ExchangeRateOracleClientError::RequestError("scripted outage".to_string()))
                }
            }
        });
        Arc::new(mock)
    ...
    async fn run_proposer_chain(
        ...
        deps.strk_to_usd_oracle = Some(oracle);
        ...
        for i in 0..n_blocks {
            ...
            observed.push(init.fee_proposal_fri.expect("V0_14_3+ proposer must emit Some"));

            // The orchestrator chains `init.fee_proposal_fri` into the proposal commitment
            // (see `proposal_commitment_from`); pass the matching value here so the
            // stored commitment lookup in `BuiltProposals::get_proposal` succeeds.
            context
                .decision_reached(
                    height,
                    0,
                    crate::snip35::proposal_commitment_from(
                        PARTIAL_BLOCK_HASH,
