...
fn test_gas_price_with_extreme_values() {
    ...
    assert_eq!(calculate_next_base_gas_price(price, gas_used, gas_target, min_gas_price), min_gas_price);

    let price = min_gas_price;
    let gas_target = max_block_size / 2;
    let gas_used = max_block_size;
    assert!(calculate_next_base_gas_price(price, gas_used, gas_target, min_gas_price) > min_gas_price);
}
...
fn test_get_min_gas_price_for_height_exact_match() {
    ...
    let fallback = GasPrice(5_000_000_000);

    // Exact match
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(500), &price_per_height, fallback),
...
fn test_get_min_gas_price_for_height_between_entries() {
    ...
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(300), &price_per_height, fallback),
        GasPrice(10_000_000_000)
    );

    // Between 500 and 1000, should use 500's price
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(750), &price_per_height, fallback),
        GasPrice(20_000_000_000)
    );
}
...
fn test_get_min_gas_price_for_height_before_first_entry() {
    ...
    // Before first entry, should use fallback
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(50), &price_per_height, fallback),
        fallback
    );
}
...
fn test_get_min_gas_price_for_height_after_last_entry() {
    ...
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(2000), &price_per_height, fallback),
...
fn test_get_min_gas_price_for_height_empty_list() {
    ...
    // Empty list, should use fallback
    assert_eq!(
        get_min_gas_price_for_height(BlockNumber(100), &price_per_height, fallback),
        fallback
    );
}
