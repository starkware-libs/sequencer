...
fn test_every_version_with_versioned_constants_carries_the_latest_multiplier() {
    // The ceiling is ungated, so it also binds blocks whose `starknet_version` predates it. Every
    // version the crate can construct constants for therefore carries the same multiplier, and no
    // block version escapes the ceiling.
    ...
    for version in StarknetVersion::iter() {
        let Ok(constants) = VersionedConstants::get(&version) else {
            // Versions below the first one with versioned constants have no JSON to check. Anything
            // above it must have one, so a missing entry is a hole in the ceiling, not a skip.
...
// A price below the ceiling-clamped SNIP-35 floor: the floor binds the price from below while the
// ceiling binds the floor itself, so one block counts both bounds.
...
fn test_sustained_congestion_stops_at_the_ceiling() {
    ...
    // Exact equality, not `price <= cap`: the latter would also pass for a price that never rose,
    // which is the opposite of what 100 full blocks must produce.
    assert_eq!(price, TEST_MAX_L2_GAS_PRICE);
}

#[test]
fn test_fee_actual_above_the_ceiling_publishes_the_ceiling() {
    // A malicious STRK/USD oracle can only raise `fee_actual`, which enters here as a floor. That
    // floor is itself clamped, so the published price stays at the ceiling instead of being
    // dragged above it.
