...
pub fn l2_gas_price_cap(min_gas_price: GasPrice) -> GasPrice {
    ...
    // A multiplier of 0 or 1 pins the L2 gas price at 0 or at the minimum. Backstop for
    // `max_gas_price_multiplier_is_above_one_in_every_version`, which gates every shipped resource:
    // the value comes from a compiled-in resource or the field default, so a binary that passed
    // CI cannot trip this.
    assert!(
        max_gas_price_multiplier > 1,
        "max_gas_price_multiplier is {max_gas_price_multiplier}, must be greater than one."
    );
