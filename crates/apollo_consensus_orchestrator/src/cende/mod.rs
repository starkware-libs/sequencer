...
///
/// `transactions` and `execution_infos` are aligned by index and ordered as in the block.
#[cfg(feature = "os_input")]
...
/// Sends a GET request to a recorder endpoint that responds with a nullable block number. `path` is
/// the endpoint path, used only for diagnostics. Returns `Ok(None)` when the recorder responds with
/// a null block number, and `Err` when the request fails or the response cannot be parsed.
