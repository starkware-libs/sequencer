...
#[cfg(feature = "os_input")]
use super::RECORDER_GET_ACCESSED_KEYS_INPUT_PATH;
...
#[case::parses_some(200, false, false)]
// Recorder responds with `null` (it has no data stored) → `Ok(None)`.
#[case::none_when_recorder_has_none(200, true, false)]
// Non-transient failure status → `Err`.
#[case::errors_on_failure_status(400, false, true)]
#[tokio::test]
async fn get_accessed_keys_input(
    #[case] mock_status_code: usize,
    #[case] recorder_has_no_block: bool,
    ...
    let response_body = if recorder_has_no_block { "null".to_string() } else { block_data_body };
