            ...
            proposal_part = args.content_receiver.next() => {
                match handle_proposal_part(
                    ...
                    HandledProposalPart::Finished(built_block, received_fin) => {
                        // Await all verification handles under the deadline before
                        // finalizing. This ensures that if a verification task hangs,
                        // the deadline fires and consensus doesn't get stuck.
                        tokio::select! {
                            _ = args.cancel_token.cancelled() => {
                                abort_verification_handles(&mut verification_handles);
                                batcher_abort_proposal(args.deps.batcher.as_ref(), args.proposal_id).await.ok();
                                return Err(ValidateProposalError::ProposalInterrupted(
                                    "awaiting verification handles".to_string(),
                                ));
                            }
                            _ = args.deps.clock.sleep_until(deadline) => {
                                abort_verification_handles(&mut verification_handles);
                            ...
                            result = await_all_verification_handles(
                                ...
                                if let Err(e) = result {
                                    ...
                                    return Err(ValidateProposalError::ProposalPartFailed(e, None));
                                }
                            }
                        }
...
/// verification to avoid leaking background tasks.
