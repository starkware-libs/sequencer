...
async fn handle_proposal_part(
    ...
    match proposal_part {
        ...
        Some(ProposalPart::Fin(fin)) => {
            ...
            let Ok(executed_txs_count_usize) = fin.executed_transaction_count.try_into() else {
            ...
            *content = truncate_to_executed_txs(content, executed_txs_count_usize);
            ...
            let input = SendProposalContentInput {
                proposal_id,
                content: SendProposalContent::Finish(executed_txs_count_usize),
            };
            ...
            info!(
                ...
                ?batcher_block_id,
                executed_txs_count_usize,
                "Finished validating proposal."
            );
            if executed_txs_count_usize == 0 {
