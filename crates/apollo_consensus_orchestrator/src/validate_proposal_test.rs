...
async fn validate_proposal_success() {
    ...
    content_sender
        ...
        .expect("Failed to send proposal part");
    content_sender
        .send(ProposalPart::ExecutedTransactionCount(1))
        .await
        .expect("Failed to send proposal part");
    content_sender
        .send(ProposalPart::Fin(ProposalFin { proposal_commitment: BlockHash::default() }))
        .await
        .expect("Failed to send proposal part");
    // Batcher returns a valid proposal status.
    proposal_args.deps.batcher.expect_send_proposal_content().times(1).returning(
    ...
    proposal_args.deps.batcher.expect_send_proposal_content().times(1).returning(
        move |input: SendProposalContentInput| {
            assert_eq!(input.proposal_id, proposal_args.proposal_id);
            assert_eq!(input.content, SendProposalContent::Finish(1));
...
async fn invalid_second_proposal_part() {
    ...
    let res = validate_proposal(proposal_args.into()).await;
    assert_matches!(res, Err(ValidateProposalError::InvalidProposalPart(msg,_))
        if msg.contains("Invalid second proposal part"));
}
...
async fn batcher_returns_invalid_proposal() {
    ...
    let res = validate_proposal(proposal_args.into()).await;
    assert_matches!(res, Err(ValidateProposalError::InvalidProposalPart(msg,_))
        if msg.contains("Batcher returned Invalid status"));
}
