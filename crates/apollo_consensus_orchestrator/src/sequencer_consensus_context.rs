...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn validators(&self, height: BlockNumber) -> Result<Vec<ValidatorId>, ConsensusError> {
        let committee = self.deps.committee_provider.get_committee(height).await.map_err(|e| {
    ...
    async fn proposer(
        ...
        self.deps.committee_provider.get_actual_proposer(height, round).await.map_err(|e| {
            ConsensusError::InternalNetworkError(format!("Committee provider error: {e}"))
        })
    }
    ...
    async fn virtual_proposer(
        ...
        if self.config.dynamic_config.use_different_virtual_proposer {
            return self.deps.committee_provider.get_proposer(height, round).await.map_err(|e| {
                ConsensusError::InternalNetworkError(format!("Committee provider error: {e}"))
            });
        }
