...
async fn initiate_build(args: &ProposalBuildArguments) -> BuildProposalResult<ConsensusBlockInfo> {
    ...
    if l1_prices.base_fee_per_gas < args.min_l1_gas_price_wei {
        l1_prices.base_fee_per_gas = args.min_l1_gas_price_wei;
    }
    if l1_prices.base_fee_per_gas > args.max_l1_gas_price_wei {
        l1_prices.base_fee_per_gas = args.max_l1_gas_price_wei;
    }
    if l1_prices.blob_fee < args.min_l1_data_gas_price_wei {
        l1_prices.blob_fee = args.min_l1_data_gas_price_wei;
    }
    if l1_prices.blob_fee > args.max_l1_data_gas_price_wei {
        l1_prices.blob_fee = args.max_l1_data_gas_price_wei;
    }
...
async fn is_block_info_valid(
    ...
    let mut gas_prices = match gas_prices {
        ...
        Err(e) => {
            warn!("Failed to get L1 gas prices, replacing with minimum values!: {e:?}");
            PriceInfo {
                base_fee_per_gas: min_l1_gas_price_wei,
                blob_fee: min_l1_data_gas_price_wei,
            }
        }
    };
    ...
    if gas_prices.blob_fee < min_l1_data_gas_price_wei {
        gas_prices.blob_fee = min_l1_data_gas_price_wei;
    }
    if gas_prices.blob_fee > max_l1_data_gas_price_wei {
        gas_prices.blob_fee = max_l1_data_gas_price_wei;
    }
