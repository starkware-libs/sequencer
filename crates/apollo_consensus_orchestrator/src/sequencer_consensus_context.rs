...
use starknet_api::versioned_constants_logic::{
    effective_starknet_version,
    VersionedConstantsTrait,
};
...
impl SequencerConsensusContext {
    ...
    /// as a parameter (rather than reading `effective_starknet_version()` here) keeps the two sides
    ...
    async fn compute_proposer_fee_proposal(
        ...
        if let (Some(fee_target), Some(max_gas_price)) = (fee_target, max_gas_price) {
            if fee_target > max_gas_price {
                warn!(
                    "Oracle-derived fee_target {} exceeds the L2 gas price maximum {}, pinning \
