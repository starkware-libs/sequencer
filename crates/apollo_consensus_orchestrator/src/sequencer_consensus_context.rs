...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn build_proposal(
        ...
        let args = ProposalBuildArguments {
            context_deps: self.context_deps.clone(),
            gas_price_params,
...
impl SequencerConsensusContext {
    async fn validate_current_round_proposal(
        ...
        let proposal_id = ProposalId(self.proposal_id);
        self.proposal_id += 1;
        let context_deps = self.context_deps.clone();
        let gas_price_params = GasPriceParams {
            min_l1_gas_price_wei: GasPrice(self.config.min_l1_gas_price_wei),
            max_l1_gas_price_wei: GasPrice(self.config.max_l1_gas_price_wei),
            min_l1_data_gas_price_wei: GasPrice(self.config.min_l1_data_gas_price_wei),
...
async fn build_proposal(mut args: ProposalBuildArguments) {
    ...
    let Some((proposal_commitment, content)) = get_proposal_content(
        args.proposal_id,
        args.context_deps.batcher.as_ref(),
        args.proposal_sender,
        args.cende_write_success,
        args.context_deps.transaction_converter,
        args.cancel_token,
    )
    .await
    else {
...
async fn initiate_build(args: &ProposalBuildArguments) -> ProposalResult<ConsensusBlockInfo> {
    ...
    let timestamp = args.context_deps.clock.now_as_timestamp();
    let (eth_to_fri_rate, mut l1_prices) = get_oracle_rate_and_prices(
        args.context_deps.eth_to_strk_oracle_client.clone(),
        args.context_deps.l1_gas_price_provider.clone(),
        timestamp,
    ...
    let retrospective_block_hash =
        retrospective_block_hash(args.context_deps.state_sync_client.clone(), &block_info).await?;
    let build_proposal_input = ProposeBlockInput {
        proposal_id: args.proposal_id,
        deadline: args.context_deps.clock.now() + batcher_timeout,
        retrospective_block_hash,
        block_info: convert_to_sn_api_block_info(&block_info),
    };
    debug!("Initiating build proposal: {build_proposal_input:?}");
    args.context_deps.batcher.propose_block(build_proposal_input).await?;
    Ok(block_info)
}
...
async fn validate_proposal(mut args: ProposalValidateArguments) {
    let mut content = Vec::new();
    let now = args.context_deps.clock.now_at_tokio_instant();
    ...
    if !is_block_info_valid(
        ...
        block_info.clone(),
        args.context_deps.eth_to_strk_oracle_client,
        args.context_deps.clock.as_ref(),
        args.context_deps.l1_gas_price_provider,
    ...
    if let Err(e) = initiate_validation(
        args.context_deps.batcher.as_ref(),
        args.context_deps.state_sync_client,
        block_info.clone(),
        args.proposal_id,
    ...
    let (built_block, received_fin) = loop {
        tokio::select! {
            _ = args.cancel_token.cancelled() => {
                ...
                batcher_abort_proposal(args.context_deps.batcher.as_ref(), args.proposal_id).await;
                return;
            }
            ...
            proposal_part = args.content_receiver.next() => {
                match handle_proposal_part(
                    ...
                    proposal_part,
                    &mut content,
                    args.context_deps.transaction_converter.clone(),
                ).await {
