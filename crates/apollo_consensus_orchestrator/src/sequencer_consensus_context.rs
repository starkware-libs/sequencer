...
impl BuiltProposals {
    ...
    pub(crate) fn insert_proposal(
        ...
        let previous =
            by_round.insert(round, (*proposal_commitment, (init, transactions, *proposal_id)));
        assert!(
            previous.is_none(),
    ...
    fn update_for_reproposal(
        ...
        self.insert_proposal(proposal_commitment, init.clone(), transactions.clone(), &proposal_id);
        (init, transactions)
    }
}
...
impl SequencerConsensusContext {
    ...
    async fn prev_blob_write_decision_for_height(
        &self,
        height: BlockNumber,
    ) -> PrevBlobWriteDecision {
        if height == BlockNumber(0) {
            return PrevBlobWriteDecision::SkipCendeWrite;
        }
        match self.deps.cende_ambassador.get_latest_received_block().await {
            ...
            None => PrevBlobWriteDecision::WritePrevBlob,
        }
    }
...
async fn send_reproposal(
    ...
    let fin =
        ProposalFin { proposal_commitment, executed_transaction_count, commitment_parts: None };
