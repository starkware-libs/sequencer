...
impl BuiltProposals {
    ...
    /// Gets the proposal, updates init with build_param and timestamp for reproposal, stores the
    /// updated init (so decision_reached retrieves the correct init for finalize_decision), and
    /// returns (init, transactions) for sending the reproposal.
    fn get_and_update_for_reproposal(
        &mut self,
        height: &BlockNumber,
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn repropose(&mut self, id: ProposalCommitment, build_param: BuildParam) {
        ...
        let (init, txs) = self
            ...
            .get_and_update_for_reproposal(&height, &id, &build_param, timestamp);
