...
impl SequencerConsensusContext {
    ...
    /// called before `run_consensus` so the window is populated before voting begins. Reads are
    /// expected to succeed under the documented apollo_storage invariants
    /// (`State <= Header`, `Body <= Header`); a failure indicates a violated invariant.
    pub async fn initialize_fee_proposals_window(
        ...
        let window_size =
            u64::try_from(FEE_PROPOSAL_WINDOW_SIZE).expect("FEE_PROPOSAL_WINDOW_SIZE fits in u64");
        let start = start_height.0.saturating_sub(window_size);
        for block_number in (start..start_height.0).map(BlockNumber) {
            let block = self.deps.state_sync_client.get_block(block_number).await?;
    ...
    async fn compute_snip35_fee_proposal(&self, height: BlockNumber, timestamp: u64) -> GasPrice {
        // Without a real fee_actual we have no valid clamping center; freeze the fee_proposal
        // at l2_gas_price. The validator computes fee_actual the same way, so it also freezes.
        let Some(fee_actual) = compute_fee_actual(&self.fee_proposals_window, height) else {
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn build_proposal(
        ...
        let fee_proposal =
            self.compute_snip35_fee_proposal(build_param.height, self.deps.clock.unix_now()).await;
        ...
        let args = ProposalBuildArguments {
            ...
            fee_proposal,
            fee_actual: compute_fee_actual(&self.fee_proposals_window, build_param.height),
        };
