...
struct ProposalBuildArguments {
    ...
    part_per_thousand_l1_data_gas_price_multiplier: u128,
    valid_proposals: Arc<Mutex<HeightToIdToContent>>,
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn build_proposal(
        ...
        let args = ProposalBuildArguments {
            ...
            part_per_thousand_l1_data_gas_price_multiplier: self
                .config
                .part_per_thousand_l1_data_gas_price_multiplier,
...
impl SequencerConsensusContext {
    async fn validate_current_round_proposal(
        ...
        let part_per_thousand_l1_data_gas_price_multiplier =
            self.config.part_per_thousand_l1_data_gas_price_multiplier;
        let transaction_converter = self.transaction_converter.clone();
...
async fn initiate_build(args: &ProposalBuildArguments) -> ProposalResult<ConsensusBlockInfo> {
    ...
    let data_gas_price_multiplier =
        Ratio::new(args.part_per_thousand_l1_data_gas_price_multiplier, 1000);
    l1_prices.blob_fee = (data_gas_price_multiplier * l1_prices.blob_fee)
        .to_integer()
...
async fn validate_proposal(mut args: ProposalValidateArguments) {
    ...
    let min_max_prices = GasPriceParams {
        ...
        part_per_thousand_l1_data_gas_price_multiplier: args
            .part_per_thousand_l1_data_gas_price_multiplier,
    };
...
async fn is_block_info_valid(
    ...
    let data_gas_price_multiplier =
        Ratio::new(gas_price_params.part_per_thousand_l1_data_gas_price_multiplier, 1000);
    gas_prices.blob_fee = (data_gas_price_multiplier * gas_prices.blob_fee).to_integer().clamp(
        gas_price_params.min_l1_data_gas_price_wei,
        gas_price_params.max_l1_data_gas_price_wei,
    );
