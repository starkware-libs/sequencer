...
define_metrics!(
    ConsensusOrchestrator => {
        ...
        // Counter, not gauge: a gauge would read 0 on restart before the first block, false-
        // triggering a "below minimum" alert. The counter's initial 0 is a true "no drift" reading.
        LabeledMetricCounter { CONSENSUS_L2_GAS_PRICE_CLAMPED, "consensus_l2_gas_price_clamped", "Number of times the computed next L2 gas price ran into a bound and was clamped to it. `minimum`: below the floor in force, which is the configured minimum or the SNIP-35 fee_actual floor when that is higher, so it also counts blocks ramping up towards a risen floor. `maximum`: above max_gas_price_multiplier x the configured minimum", init = 0, labels = L2_GAS_PRICE_CLAMP_BOUND },
        ...
        MetricCounter { SNIP35_FEE_TARGET_ABOVE_MAXIMUM, "snip35_fee_target_above_maximum", "Number of blocks whose oracle-derived fee_target exceeded the L2 gas price maximum, so the fee_proposal band was capped at that maximum. Counts oracle targets only: a fee_target taken from the override_l2_gas_price_fri operator pin is excluded", init = 0 },
    }
);

pub const LABEL_L2_GAS_PRICE_CLAMP_BOUND: &str = "l2_gas_price_clamp_bound";

/// Which bound the computed price ran into: the floor in force (the configured minimum, or the
/// SNIP-35 `fee_actual` floor when that is higher), or `max_gas_price_multiplier` times the
/// configured minimum.
