...
#[tokio::test]
async fn retrospective_block_hash_error() {
    let (mut proposal_args, _proposal_receiver) = create_proposal_build_arguments();
    // Make sure clients are being called, by setting height to >= STORED_BLOCK_HASH_BUFFER.
    proposal_args.proposal_init.height = BlockNumber(STORED_BLOCK_HASH_BUFFER);
    // Setup batcher client to return an error.
    ...
    assert!(matches!(
        res,
        Err(BuildProposalError::RetrospectiveBlockHashError(
...
#[tokio::test]
async fn retrospective_block_hash_not_ready(
    ...
    assert!(matches!(
        res,
        Err(BuildProposalError::RetrospectiveBlockHashError(
            RetrospectiveBlockHashError::NotReady(_)
        ))
    ));
}
...
async fn state_sync_ready_after_a_while() {
    ...
    proposal_args.deps.batcher.expect_get_stored_block_hash().returning(|block_number| {
...
async fn batcher_ready_after_a_while() {
    ...
    proposal_args.deps.batcher.expect_get_stored_block_hash().times(1).returning(|block_number| {
        Err(BatcherClientError::BatcherError(BatcherError::BlockHashNotFound(block_number)))
    });
