...
fn test_compute_fee_actual_random_window() {
    // 12 values with window_size=10: takes the last 10, excluding the first two [1, 17].
    // Those excluded values would have affected the median if included.
    // Sorted window: [42, 87, 100, 271, 314, 999, 1024, 1729, 6000, 9999].
    // Even-length median = sorted[4] + (sorted[5] - sorted[4]) / 2 = 314 + 342 = 656.
    let proposals = vec![
        GasPrice(314),
        GasPrice(1729),
        GasPrice(42),
    ...
    assert_eq!(compute_fee_actual(&proposals, 10), Some(GasPrice(656)));
}
