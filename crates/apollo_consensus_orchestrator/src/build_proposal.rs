...
async fn initiate_build(args: &mut ProposalBuildArguments) -> BuildProposalResult<ProposalInit> {
    ...
    let init = ProposalInit {
        ...
        // Until a real commitment is designed this stays at the sentinel. The value comes from
        // `expected_version_constant_commitment`, the single source of truth also used by the
        // validator (`is_proposal_init_valid`), which rejects any proposal whose
        // `version_constant_commitment` differs from it. Emitting a real value therefore means
        // changing that one function, which wires proposer and validator together in the same
        // change.
