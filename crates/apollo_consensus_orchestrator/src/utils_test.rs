...
#[rstest]
#[case::not_ready(StateSyncClientError::StateSyncError(StateSyncError::BlockNotFound(
    RETRO_BLOCK_NUMBER
)))]
#[case::error(StateSyncClientError::ClientError(ClientError::CommunicationFailure("".to_string())))]
#[tokio::test]
async fn retrospective_block_hash_state_sync_error(#[case] state_sync_error: StateSyncClientError) {
    let (mut test_proposal_args, _proposal_receiver) = create_proposal_build_arguments();
    test_proposal_args.build_param.height = CURRENT_BLOCK_NUMBER;
    // Clone the error to use it in the match statement.
    ...
    match &state_sync_error_cloned {
        StateSyncClientError::StateSyncError(StateSyncError::BlockNotFound(_)) => {
            assert!(matches!(res, RetrospectiveBlockHashError::StateSyncNotReady(_)));
        }
        _ => {
            assert!(matches!(res, RetrospectiveBlockHashError::StateSyncError(_)));
        }
    }
}

#[rstest]
#[case::not_ready(BatcherClientError::BatcherError(BatcherError::BlockHashNotFound(
    RETRO_BLOCK_NUMBER
)))]
#[case::error(BatcherClientError::BatcherError(BatcherError::InternalError))]
#[tokio::test]
async fn retrospective_block_hash_batcher_error(#[case] batcher_error: BatcherClientError) {
    let (mut test_proposal_args, _proposal_receiver) = create_proposal_build_arguments();
    test_proposal_args.build_param.height = CURRENT_BLOCK_NUMBER;
    // Clone the error to use it in the match statement.
    ...
    match &batcher_error_cloned {
        BatcherClientError::BatcherError(BatcherError::BlockHashNotFound(_)) => {
            assert!(matches!(res, RetrospectiveBlockHashError::BatcherNotReady(_)));
        }
        _ => {
            assert!(matches!(res, RetrospectiveBlockHashError::BatcherError(_)));
        }
    }
}
