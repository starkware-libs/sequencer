...
pub(crate) async fn wait_for_retrospective_block_hash(
    ...
    let mut result = retrospective_block_hash(state_sync_client.clone(), block_info).await;

    // Try to get the block hash until the deadline is reached or the block is found.
    while let Err(StateSyncClientError::StateSyncError(StateSyncError::BlockNotFound(_))) = result {
        if clock.now() >= deadline {
            break;
        }
        tokio::time::sleep(retry_interval).await;
        result = retrospective_block_hash(state_sync_client.clone(), block_info).await;
    }
