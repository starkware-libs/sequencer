...
impl RetrospectiveBlockHashError {
    ...
    pub(crate) fn block_number(&self) -> Option<BlockNumber> {
        match self {
            ...
            Self::NotReady { batcher_error, state_sync_error } => {
                error!(
                    "Batcher and/or state sync are not ready, and both errors weren't caused from \
                     simply not being ready. Batcher error: {batcher_error:?}, State sync error: \
                     {state_sync_error:?}."
                );
                None
            }
...
pub(crate) async fn retrospective_block_hash(
    ...
    if retrospective_block_number.is_none() {
        info!(
    ...
    let block_number = BlockNumber(
        retrospective_block_number.expect("Retrospective block number should be present"),
    );
