...
impl RetrospectiveBlockHashError {
    pub(crate) fn from_errors(
        ...
        } else if batcher_not_ready && !state_sync_not_ready {
    ...
    pub(crate) fn block_number(&self) -> Option<BlockNumber> {
        match self {
            Self::NotReady { batcher_error, state_sync_error } => match batcher_error {
                BatcherClientError::BatcherError(BatcherError::BlockHashNotFound(block_number)) => {
                    Some(*block_number)
                }
                _ => match state_sync_error {
                    StateSyncClientError::StateSyncError(StateSyncError::BlockNotFound(
                        block_number,
                    )) => Some(*block_number),
                    _ => {
                        error!(
                            "Batcher and/or state sync are not ready, and both errors weren't \
                             caused from simply not being ready. Batcher error: \
            ...
            Self::FailedRetrievingHash { block_number, .. } => Some(*block_number),
        }
    }
}
#[derive(Debug)]
                ...
                Err(batcher_error) => {
                    ...
                    warn!(
                        "Successfully retrieved retrospective block hash from state sync after \
                         failing to get it from the Batcher, at retrospective block number \
                         {block_number}."
                    );
                    block_hash
                }
        ...
        match &result {
            Err(RetrospectiveBlockHashError::NotReady { batcher_error, state_sync_error }) => {
                ...
                if effective_retry_interval == Duration::ZERO {
                    let block_number_str = match result.as_ref().unwrap_err().block_number() {
