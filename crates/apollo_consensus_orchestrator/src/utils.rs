...
/// reading, and forces an attacker to hold the feed across many blocks to move the rate far.
///
/// A node that holds no previous block does not clamp, so it can disagree with caught-up peers on
/// the rate for one height. See the `None` branch below.
fn bound_eth_to_fri_rate_change(
    ...
    // TODO(Asaf): seed `previous_proposal_init` from state_sync at startup, the way
    // `initialize_fee_proposals_window` seeds the fee window, so a node restarted at tip clamps on
    // its first height instead of proposing an unclamped rate its peers would reject. The same
    // anchor already gates the price fallback path, so the gap predates this bound.
