...
const EPOCH_LENGTH: u64 = 100; // Number of blocks in an epoch.
...
pub struct CommitteeManagerConfig {
    ...
    // Defines how many blocks in advance the proposer can be predicted.
    ...
    pub proposer_prediction_window: u64,
}
...
pub enum CommitteeManagerError {
    ...
    #[error("Failed to get block {block_number}")]
...
impl CommitteeManager {
    ...
    pub async fn get_proposer<S: StateReader>(
        ...
        // Generate a deterministic random number based on the height, round, and block hash.
        let random = generate_deterministic_random(height, round, block_hash);
        ...
        let proposer = self.select_proposer(&committee, random);
        Ok(proposer.address)
    }
    ...
    fn select_proposer(&self, _committee: &Committee, _random: u64) -> &Staker {
        todo!()
    }
}

fn generate_deterministic_random(
    _height: BlockNumber,
    _round: Round,
