...
fn convert_proposal_to_vec_u8_and_back() {
    ...
    match transaction {
        Transaction::Declare(DeclareTransaction::V3(data)) => {
            data.resource_bounds = ValidResourceBounds::AllResources(Default::default());
        }
        Transaction::Invoke(InvokeTransaction::V3(data)) => {
            data.resource_bounds = ValidResourceBounds::AllResources(Default::default());
        }
        Transaction::DeployAccount(DeployAccountTransaction::V3(data)) => {
            data.resource_bounds = ValidResourceBounds::AllResources(Default::default());
        }
        _ => {}
    }
    // if let Transaction::Declare(DeclareTransaction::V3(ref mut data)) = transaction {
    //     if let ValidResourceBounds::AllResources(ref mut bounds) = data.resource_bounds {
    //         bounds.l2_gas.max_amount = 1;
    //     }
    // }
    // if let Transaction::Invoke(InvokeTransaction::V3(ref mut data)) = transaction {
    //     if let ValidResourceBounds::AllResources(ref mut bounds) = data.resource_bounds {
    //         bounds.l2_gas.max_amount = 1;
    //     }
    // }
    // if let Transaction::DeployAccount(DeployAccountTransaction::V3(ref mut data)) = transaction {
    //     if let ValidResourceBounds::AllResources(ref mut bounds) = data.resource_bounds {
    //         bounds.l2_gas.max_amount = 1;
    //     }
    // }
