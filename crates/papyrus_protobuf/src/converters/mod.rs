...
use papyrus_common::compression_utils::CompressionUtilsError;
...
pub enum ProtobufConversionError {
    ...
    // The original error does not derive ParitalEq and Clone Traits
    #[error("Unexpected compression utils error: {0}")]
    CompressionUtilsError(String),
    // The original error does not derive ParitalEq and Clone Traits
    #[error("Unexpected serde_json error: {0}")]
    SerdeJsonError(String),
}

impl From<CompressionUtilsError> for ProtobufConversionError {
    fn from(error: CompressionUtilsError) -> Self {
        ProtobufConversionError::CompressionUtilsError(error.to_string())
    }
}
