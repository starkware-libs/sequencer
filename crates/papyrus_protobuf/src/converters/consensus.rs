...
impl<T: Protoable> TryFrom<protobuf::StreamMessage> for StreamMessage<T> {
    type Error = ProtobufConversionError;
...
impl<T: Protoable> From<StreamMessage<T>> for protobuf::StreamMessage {
...
impl<T: Protoable> From<StreamMessage<T>> for Vec<u8> {
    fn from(value: StreamMessage<T>) -> Self {
...
impl<T: Protoable> TryFrom<Vec<u8>> for StreamMessage<T> {
    type Error = ProtobufConversionError;
