...
impl GatewayCompiler {
    ...
    pub fn process_declare_tx(
        ...
        executable_declare_tx.validate_compiled_class_hash().map_err(|err| match err {
            StarknetApiError::CompiledClassHashMismatch { .. } => {
                debug!("Compiled class hash mismatch: {:?}", err);
                GatewaySpecError::CompiledClassHashMismatch
            }
            _ => {
                debug!("Failed to validate compiled class hash. Error: {:?}", err);
                GatewaySpecError::UnexpectedError { data: "Internal server error.".to_owned() }
            }
        })?;
