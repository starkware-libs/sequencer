...
use blockifier::test_utils::CHAIN_ID_NAME;
...
fn test_compile_contract_class_compiled_class_hash_mismatch(
    ...
    let err = gateway_compiler
        .process_declare_tx(declare_tx, &ChainId::Other(CHAIN_ID_NAME.to_string()))
        .unwrap_err();
    assert_eq!(err, GatewaySpecError::CompiledClassHashMismatch);
    assert!(logs_contain(
        format!(
            "Failed to create executable declare transaction CompiledClassHashMismatch {{ \
