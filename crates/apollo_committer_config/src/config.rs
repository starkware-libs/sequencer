...
pub struct CommitmentInfosPruningConfig {
    /// Commitment infos of heights below `offset - retention_blocks` are pruned from storage when
    /// committing a block with witnesses.
    ...
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([
            ser_param(
                ...
                "Commitment infos of heights below the committer offset minus this value are \
                 pruned from storage when committing a block with witnesses.",
