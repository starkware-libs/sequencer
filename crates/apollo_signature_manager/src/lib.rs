...
/// Type-erased signature manager that supports multiple key store backends.
///
/// Adding a new backend: implement `KeyStore` for the new store type, add a
/// variant here, and handle it in `sign_identification`, `sign_precommit_vote`,
/// and `create_signature_manager`.
#[derive(Clone, Debug)]
...
/// # Panics
///
/// Panics if `config.key_source` is `GoogleSecretManager` (requires the `gsm`
/// feature flag, added in a follow-up).
