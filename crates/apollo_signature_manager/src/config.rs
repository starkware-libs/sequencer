...
pub enum KeySourceConfig {
    /// Use a hard-coded test key. Suitable only for development and testing.
    #[default]
    Testing,
...
impl SerializeConfig for SignatureManagerConfig {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([ser_param(
            "key_source",
            &self.key_source,
            "Key source configuration for the signature manager. Use 'Testing' for development or \
             'GoogleSecretManager' with a secret_name for production.",
