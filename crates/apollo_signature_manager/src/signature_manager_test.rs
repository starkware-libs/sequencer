...
// Expected signature for PeerIdentity (peer_id + challenge 0xdead as Felt).
// The digest uses challenge.to_bytes_be() (32 bytes) since Challenge wraps Felt.
const ALICE_IDENTITY_SIGNATURE: Signature = Signature {
    r: Felt::from_hex_unchecked(
        "0x7c897ed52bec182b3d7d672da49e2db55abbb6cb36e2cff74f7d1eec91efc64",
    ),
    s: Felt::from_hex_unchecked(
        "0x7f66c9304351c967ce14da9f031144f0940238c5b95d039e42f093cc9096569",
    ),
};
...
impl PeerIdentity {
    pub fn new() -> Self {
        ...
        Self {
            peer_id: PeerId::from_bytes(&peer_id).unwrap(),
            challenge: Challenge(Felt::from_hex_unchecked("0xdead")),
        }
    }
}
...
async fn test_identify() {
    ...
    let PeerIdentity { peer_id, challenge } = PeerIdentity::new();
    let signature = signature_manager.identify(peer_id, challenge).await;
