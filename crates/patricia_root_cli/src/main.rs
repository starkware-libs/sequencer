...
async fn main() {
    ...
    let raw_entries: HashMap<String, String> =
        serde_json::from_str(&contents).expect("Failed to parse JSON as a string->string map");
    ...
    let leaf_modifications: LeafModifications<StarknetStorageValue> = raw_entries
        .into_iter()
        .map(|(key, value)| {
            (NodeIndex::from_leaf_felt(&parse_felt(&key)), StarknetStorageValue(parse_felt(&value)))
        })
        .collect();
...
fn parse_felt(value: &str) -> Felt {
    Felt::from_hex(value).unwrap_or_else(|_| panic!("Failed to parse felt: {value}"))
}
