...
- All fee-related fields must be zero: resource bounds (`l1_gas`, `l2_gas`, `l1_data_gas`) must
  have `max_amount` and `max_price_per_unit` set to `"0x0"`, and `tip` must be `"0x0"`. Proving
  is client-side; no fees are charged. Disable this check with `SKIP_FEE_FIELD_VALIDATION=true`.
...
| `1000` | Invalid transaction input | Invalid request field: non-zero resource bounds or tip, or other malformed input. See `data`. |
...
|----------|----------|---------|-------------|
| `RPC_URL` | `--rpc-url` | *(required)* | Starknet RPC node URL (v0.10 compatible). |
...
| `SKIP_FEE_FIELD_VALIDATION` | `--skip-fee-field-validation` | `false` | Allow non-zero fee-related fields (resource bounds, tip) in requests. |
| `STRK_FEE_TOKEN_ADDRESS` | `--strk-fee-token-address` | *(auto per chain)* | Override STRK fee token address (hex). Useful for custom environments that share a chain ID. |
