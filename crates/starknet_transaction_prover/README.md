...
shutdown semantics. The `/health` and `/metrics` endpoints are unauthenticated by design — intended
for load balancers, scrapers, and orchestrators — and expose no service state, transaction data, or
credentialed URLs.
...
(Service busy). A successful request clears the saturation window immediately, so 503 → 200
...
contents) has no shared key to join them, preserving OHTTP unlinkability.
...
host (see [Logging](#logging)), and feature flags. No fee token address, TLS path, or
