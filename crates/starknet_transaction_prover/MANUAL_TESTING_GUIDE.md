...
find_tx_hash() {
  ...
  printf 'no %s %s found in the last %d blocks\n' "$tx_type" "$tx_version" "$lookback" >&2
  return 1
}
```
...
Covered by unit tests (`src/server/cors_test.rs`, `src/server/config_test.rs`); no manual
step needed.
...
Install from the [releases page](https://github.com/tsenart/vegeta/releases),
...
Install from the [releases page](https://github.com/rakyll/hey/releases).
...
Covered by unit tests (`src/server/config_test.rs`, `tls_config_validation`); no manual
step needed.
...
- The startup log line (see section 14.1) reports the values the env vars dictate.
...
Copy the block below and fill in the details. Record PASS / FAIL / SKIP per
check, with the actual response wherever it deviates from expected.
