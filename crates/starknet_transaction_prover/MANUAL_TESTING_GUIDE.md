...
- `scheme`, `ip`, `port`, `max_concurrent_requests`, `max_connections`,
...
`ip`, `port`, `max_concurrent_requests`, `max_connections`, `cors_mode`, and
`ohttp_enabled`. All values must match the resolved config.
...
- Startup log line includes `scheme`, `ip`, `port`, concurrency caps, `cors_mode`, and `ohttp_enabled`
