...
| 10 | TLS / HTTPS | 10-15 min |
| 11 | OHTTP envelope encryption | 10-15 min |
| 12 | Request body size limit | 5-10 min |
| 13 | Environment variable configuration | 5-10 min |
| 14 | Observability | 5-10 min |
...
# Derived from PROVER_URL; used by section 10 (TLS) tests.
export PROVER_HOST="${PROVER_URL#*://}"
export PROVER_HOST="${PROVER_HOST%%[:/]*}"
export PROVER_PORT="${PROVER_URL##*:}"
export PROVER_PORT="${PROVER_PORT%%/*}"
# URLs without an explicit port leave PROVER_PORT empty — default to 443 (section 10 uses HTTPS).
[[ -n "$PROVER_PORT" ]] || export PROVER_PORT=443
```
...
Confirm the service is reachable before exercising the JSON-RPC methods:
...
- `result` matches the `SPEC_VERSION` constant in
  `crates/starknet_transaction_prover/src/server/rpc_impl.rs`. Read the
  current value from the checkout rather than hardcoding it here, since it
  changes across spec revisions:

  ```bash
  grep 'const SPEC_VERSION' crates/starknet_transaction_prover/src/server/rpc_impl.rs
  ```
...
Not covered here: the blocking-check flow (error code `10000`, "Transaction
blocked", configured via `blocking_check_url` / `blocking_check_timeout_millis`
/ `blocking_check_fail_open`) requires wiring up an external screening service
...
Find a `DECLARE` `0x3` tx, drop `transaction_hash`, and inject a dummy
`contract_class` (the RPC response omits it because the full Sierra class is
too large to return).
...
if [ -z "$DECLARE_HASH" ]; then
  ...
  # Inject a dummy contract_class so the DECLARE tx deserializes correctly.
  DUMMY_CLASS='{"sierra_program":["0x0","0x1","0x2"],"contract_class_version":"0.1.0","entry_points_by_type":{"CONSTRUCTOR":[{"function_idx":1,"selector":"0x2"}],"EXTERNAL":[{"function_idx":3,"selector":"0x4"}],"L1_HANDLER":[{"function_idx":5,"selector":"0x6"}]},"abi":"dummy abi"}'
  jq --argjson cls "$DUMMY_CLASS" '.contract_class = $cls' /tmp/declare_tx.json > /tmp/declare_tx_patched.json

  jq -nc --argjson base "$BASE_BLOCK" --slurpfile tx /tmp/declare_tx_patched.json \
...
- `error.code = 1001`
- `error.message = "the transaction type is not supported"`
...
> **Note:** This test requires `validate_zero_fee_fields=true` (the default —
> do not pass `--skip-fee-field-validation`). If the service is running with
> section 4.2's recommended `validate_zero_fee_fields=false` setup, restart it
> without that flag (see section 2) before running this test. It exercises a
> separate service configuration from the section 4.2 happy path and does not
> need to run back-to-back with it.
...
Restart with `cors_allow_origin=["http://localhost:5173"]` (see section 2).
...
Restart with `cors_allow_origin=[]` (the default; see section 2) — for local
testing, omit `--cors-allow-origin` entirely.
...
Verify that invalid origin values are rejected at startup. Restart with
`cors_allow_origin=["ftp://example.com"]` (see section 2) and check the
startup output.
...
The server applies `CompressionLayer` (from `tower-http`) to eligible HTTP
responses (above tower-http's default size threshold; tiny responses and
excluded content types pass through uncompressed), supporting gzip, brotli, and
zstd. These tests verify compression negotiation works correctly.
...
Restart with `max_concurrent_requests=1` (see section 2) so only one proving
...
Restart with `max_concurrent_requests=2` (see section 2).
...
These tests apply only when the service is started with `--tls-cert-file` and
`--tls-key-file` (or the equivalent config file / env vars). For a pure-HTTP
deployment, skip this section.

### 10.1 HTTPS request succeeds

With the service listening on `https://$PROVER_HOST:$PROVER_PORT`:

```bash
curl -sS --cacert /path/to/ca-or-selfsigned.pem "https://$PROVER_HOST:$PROVER_PORT" \
  -H 'content-type: application/json' \
  -d '{"jsonrpc":"2.0","id":1,"method":"starknet_specVersion","params":[]}' | jq .
```

Expect a `result` field containing the spec version.

### 10.2 HTTP rejected on an HTTPS endpoint

```bash
curl -sSv --max-time 5 "http://$PROVER_HOST:$PROVER_PORT" \
  -H 'content-type: application/json' \
  -d '{"jsonrpc":"2.0","id":1,"method":"starknet_specVersion","params":[]}'
```

Expect the client to fail the TLS handshake (error `1` / `Empty reply from
server`); no JSON-RPC response.

### 10.3 Certificate validity

```bash
echo | openssl s_client -servername "$PROVER_HOST" \
  -connect "$PROVER_HOST:$PROVER_PORT" 2>/dev/null \
  | openssl x509 -noout -dates -subject -issuer
```

Pass criteria:

- `notAfter=` is comfortably in the future (operator-defined threshold, e.g. > 30 days).
- `subject=` and `issuer=` match what operations expects for this deployment.

### 10.4 Incomplete TLS config fails at startup

Start the service with only `--tls-cert-file` (no `--tls-key-file`); the process
must exit with a clear `IncompleteTlsConfig` error and not bind a port.

## 11. OHTTP Envelope Encryption

These tests apply only when the service is started with `--ohttp-enabled`.
They verify the privacy layer is wired up and the key endpoint is reachable.
Skip for deployments that do not front requests with OHTTP.

`--ohttp-enabled` requires the `OHTTP_KEY` environment variable: a hex-encoded
32-byte X25519 private key (64 hex characters). Without it the process exits
at startup before binding a port. For local testing:

```bash
export OHTTP_KEY="$(openssl rand -hex 32)"
...
### 11.1 `GET /ohttp-keys` returns key material

```bash
curl -sSi "$PROVER_URL/ohttp-keys"
```
...
- A `cache-control` header with a non-zero `max-age` (keys are cacheable).
- Body is non-empty binary — length matches the key material shipped by the
  service's OHTTP configuration.

### 11.2 Repeat request hits the cache

```bash
for _ in 1 2 3; do curl -sS -o /dev/null -w "%{time_total}\n" "$PROVER_URL/ohttp-keys"; done
```

Successive responses should be ~constant-time and not elevate upstream load.

### 11.3 Non-OHTTP JSON-RPC still works

OHTTP is additive; plain JSON-RPC clients must continue to work:
...
Expect the usual spec-version response.

## 12. Request Body Size Limit

The service enforces `max_request_body_size` (default 5 MiB) as a DoS guard.

### 12.1 Oversized body is rejected

Send a body strictly larger than the configured limit:

```bash
LIMIT_BYTES=$(printf '5242880')  # 5 MiB default
OVERSIZED=$((LIMIT_BYTES + 1024))
python3 -c "import json,sys; print(json.dumps({'jsonrpc':'2.0','id':1,'method':'starknet_specVersion','params':[],'pad':'x'*$OVERSIZED}))" \
  | curl -sSi "$PROVER_URL" -H 'content-type: application/json' --data-binary @-
```

Pass criteria:

- HTTP 413 (Payload Too Large) **or** a JSON-RPC error with a clear message
  indicating the body exceeds the limit.
- The service process does not crash and continues to serve smaller requests.

### 12.2 Just-under-limit body is accepted

Send a body of ~`max_request_body_size - 1 KiB`. It should be accepted (the
request may produce a functional error depending on the payload — accept any
non-413, non-oversize response).

## 13. Environment Variable Configuration

Most CLI flags have env-var equivalents; this matters for Docker / Kubernetes
deployments. Verify a handful by starting the service with env vars only:

```bash
RPC_URL="http://rpc.example:9545/rpc/v0_10" \
CHAIN_ID=SN_SEPOLIA \
PROVER_IP=127.0.0.1 \
PROVER_PORT=8080 \
...
cargo run -p starknet_transaction_prover --release
```

Pass criteria (read from the startup log line — it echoes the effective config):

- `scheme`, `local_address`, `max_concurrent_requests`, `max_connections`,
  `cors_mode`, and `ohttp_enabled` match what the env vars dictate.
- CLI flags override env vars when both are set; the log reflects the CLI
  value in that case.

## 14. Observability

Confirm the service emits what operators need to triage issues.

### 14.1 Startup log line

On startup, the service logs one INFO line containing (at minimum): `scheme`,
`local_address` (the bound `ip:port`), `max_concurrent_requests`,
`max_connections`, `cors_mode`, and `ohttp_enabled`. All values must match the resolved config.

### 14.2 Structured error logs

Trigger each negative flow from Section 5 and confirm the service logs a
structured error with enough context (request id, error code, tx hash if
applicable) to correlate against the client-visible JSON-RPC error.

## 15. Documenting Test Results
...
- **Per-section results** — for each check in the results table below, record
...
| 5.2 | DECLARE rejected (1001) | PASS / FAIL / SKIP | |
| 5.3 | DEPLOY_ACCOUNT rejected (1001) | PASS / FAIL / SKIP | |
...
| 10.1 | TLS HTTPS happy path | PASS / FAIL / SKIP | |
| 10.2 | HTTP rejected on HTTPS port | PASS / FAIL / SKIP | |
| 10.3 | Certificate validity | PASS / FAIL / SKIP | notAfter: ___ |
| 10.4 | Incomplete TLS config fails at startup | PASS / FAIL / SKIP | |
| 11.1 | OHTTP key endpoint returns 200 | PASS / FAIL / SKIP | |
| 11.2 | OHTTP key endpoint cached | PASS / FAIL / SKIP | |
| 11.3 | Non-OHTTP JSON-RPC still works | PASS / FAIL / SKIP | |
| 12.1 | Oversized body rejected | PASS / FAIL | |
| 12.2 | Just-under-limit body accepted | PASS / FAIL | |
| 13   | Env-var-only config | PASS / FAIL | |
| 14.1 | Startup log line | PASS / FAIL | |
| 14.2 | Structured error logs | PASS / FAIL | |
