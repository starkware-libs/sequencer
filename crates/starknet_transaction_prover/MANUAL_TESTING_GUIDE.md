...
- `result` matches the `SPEC_VERSION` constant in
  `crates/starknet_transaction_prover/src/server/rpc_impl.rs` (currently `"0.10.2"`)
...
- HTTP 200.
- `content-type: application/ohttp-keys` (or vendor-specific equivalent).
...
- TLS: HTTPS request succeeds with the configured certificate (skip if HTTP-only)
- TLS: plaintext HTTP request to HTTPS port fails the handshake
- TLS: certificate `notAfter` date is far enough in the future
- TLS: starting with cert but no key (or vice versa) fails fast with `IncompleteTlsConfig`
- OHTTP: `GET /ohttp-keys` returns 200 with non-empty key material and a cache-control header (skip if disabled)
- OHTTP: non-OHTTP JSON-RPC still returns a valid response
- Body-size limit: oversized body rejected (HTTP 413 or clear JSON-RPC error); service survives
- Body-size limit: just-under-limit body is accepted
- Env vars: service starts with env-only config; startup log reflects the env-var values
- Env vars: CLI flags take precedence when both are set
- Startup log line includes `scheme`, `local_address`, concurrency caps, `cors_mode`, and `ohttp_enabled`
- Structured error logs accompany every negative-flow response

## 16. Documenting Test Results
...
- **Per-section results** — for each item in the checklist (section 15), record
