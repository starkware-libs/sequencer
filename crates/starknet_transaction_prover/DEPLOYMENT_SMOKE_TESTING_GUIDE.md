...
- `OHTTP_SMOKE=true` -- run the OHTTP key-endpoint check (Section 3.6). The
...
### 3.5 TLS certificate validity (skip if HTTP-only)

For HTTPS deployments, verify the certificate is currently valid and has enough
runway left that it will not expire before the next planned deployment.

```bash
PROVER_HOST_PORT="${PROVER_URL#https://}"; PROVER_HOST_PORT="${PROVER_HOST_PORT%%/*}"
PROVER_HOST="${PROVER_HOST_PORT%%:*}"
PROVER_PORT="${PROVER_HOST_PORT##*:}"
[ "$PROVER_HOST" = "$PROVER_PORT" ] && PROVER_PORT=443
...
- `notBefore=` is in the past and `notAfter=` is far enough in the future
  (operator-defined threshold, e.g. ≥ 30 days).
- HTTPS requests from Section 3.1–3.2 succeeded against this endpoint.

### 3.6 OHTTP key endpoint (skip if OHTTP disabled)

For deployments with `--ohttp-enabled`, confirm the public key endpoint is
reachable and serves cacheable, non-empty key material. The smoke script runs
this check only when `OHTTP_SMOKE=true` is exported -- it cannot detect the
server-side flag on its own.
...
- HTTP 200.
- Body is non-empty.
- A `cache-control` header with a non-zero `max-age` is present.

### 3.7 Request body size limit enforced

Send a body strictly larger than `max_request_body_size` (default 5 MiB) and
confirm it is rejected without crashing the service.

```bash
LIMIT_BYTES=5242880
OVERSIZED=$((LIMIT_BYTES + 1024))
python3 -c "import json; print(json.dumps({'jsonrpc':'2.0','id':1,'method':'starknet_specVersion','params':[],'pad':'x'*$OVERSIZED}))" \
  | curl -sSi "$PROVER_URL" -H 'content-type: application/json' --data-binary @-
```

Pass:

- Response is HTTP 413 or a JSON-RPC error that clearly indicates the body is
  too large.
- Immediately after, Section 3.1 still succeeds (service is alive).

### 3.8 Concurrency/recovery quick check
