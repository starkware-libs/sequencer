...
Runtime is dominated by the one real proving call (3.9); every other check is a
...
(`rpc_call_chain`, `rpc_call_prover`, `find_tx_block_and_hash`) rather than a copy here.
...
- `MAX_REQUEST_BODY_SIZE` -- body-size limit in bytes assumed by the
  oversized-body check. Defaults to the server default,
  `DEFAULT_MAX_REQUEST_BODY_SIZE` in `src/server/config.rs`.
- `PROVE_MAX_TIME` -- per-request timeout, in seconds, for
  ...
  fixed shorter timeout regardless of this setting.
...
| 3.7 | Malformed params | A malformed `starknet_proveTransaction` call is rejected with error code `-32602` (`Invalid params`). | `check_malformed_params` | §5.6 |
| 3.8 | Pending block rejected | Proving against `block_id: "pending"` returns error code `24` (`Block not found`). | `check_pending_block_rejected` | §5.1 |
| 3.9 | Happy path | A real `INVOKE` `0x3` transaction, sent unmodified, proves successfully (`result.proof` and `result.proof_facts` present). | `build_valid_prove_request`, `check_prove_happy_path` | §4.2 |
| 3.10 | Concurrency and recovery | Several concurrent prove requests all complete with no transport or `-32603` errors, and the service still answers `starknet_specVersion` correctly afterward. This does **not** prove the concurrency limit rejects excess load -- see Section 1. | `check_concurrency_and_recovery` | §8.1-8.2 |

## 4. Pass/Fail Gate

Deployment is smoke-validated only if all checks in Section 3 pass.

The summary line reports `PASS`, `FAIL` and `SKIP` counts. A skipped check has
not been validated: `SKIP` is not a pass. Checks skip when their preconditions
are absent -- a non-HTTPS `PROVER_URL` or missing `openssl` (3.4), or
`OHTTP_SMOKE` unset (3.5).
