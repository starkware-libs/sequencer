...
pub(crate) fn make_query(
    n_class_hashes: usize,
    n_contract_addresses: usize,
    storage_keys_per_contract: &[usize],
) -> RpcStorageProofsQuery {
    let class_hashes: Vec<Felt> = (0..n_class_hashes).map(|i| Felt::from(i as u64)).collect();
    let contract_addresses: Vec<ContractAddress> = (0..n_contract_addresses)
        .map(|i| ContractAddress::try_from(Felt::from(1000 + i as u64)).unwrap())
        .collect();
    let contract_storage_keys: Vec<ContractStorageKeys> = storage_keys_per_contract
        .iter()
        .enumerate()
        .map(|(i, &n_keys)| ContractStorageKeys {
            contract_address: Felt::from(2000 + i as u64),
            storage_keys: (0..n_keys).map(|k| Felt::from(k as u64)).collect(),
        })
        .collect();
