...
#[test]
fn test_convert_state_maps_preserves_class_hashes() {
    let contract = address(1);
    let class_hash = ClassHash(Felt::from(0x42u64));
    let mut state_maps = StateMaps::default();
    state_maps.class_hashes.insert(contract, class_hash);

    let state_diff = state_maps_to_committer_state_diff(state_maps);

    assert_eq!(state_diff.address_to_class_hash, [(contract, class_hash)].into());
}

#[test]
fn test_convert_state_maps_declared_contracts_dropped() {
    let class_hash = ClassHash(Felt::from(0x42u64));
    let mut state_maps = StateMaps::default();
    state_maps.declared_contracts.insert(class_hash, true);
