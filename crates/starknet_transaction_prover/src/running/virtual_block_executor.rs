...
impl VirtualBlockExecutor for RpcVirtualBlockExecutor {
    fn base_block_info(
        ...
        // Virtual OS: nothing is sent to L1 from this executor; `proof_fact` on L2 carries the
        // output. Bouncer `l1_gas` / `message_segment_length` are therefore not L1 DA limits
        // here—only client-side caps (see `RpcVirtualBlockExecutorConfig::bouncer_config`).
