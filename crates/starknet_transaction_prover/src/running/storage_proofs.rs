...
fn missing_collapse_sibling_key(
    ...
    while !current_index.is_leaf() {
        // Reachable when the proof doesn't reach `key`'s leaf: an incomplete proof, or a zero-write
        // to a key absent from the trie (a no-op delete). The `current_index != leaf_index` guard
        // after the loop then makes us fetch nothing.
    ...
    // The walk only proves the collapse structure if it reached the deleted leaf. If it stopped
    // short — an inner node on the path is missing from the proof, or the key simply isn't in the
    // trie (a no-op delete) — `collapse_sibling` reflects an unverified path, so fetch nothing.
