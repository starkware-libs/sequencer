//! Integration tests for the parallel blocking check in `prove_transaction`.
//!
//! Uses a mock `VirtualSnosRunner` combined with a `mockito` server for the
//! external check endpoint. The mock runner always returns an error after an
//! optional delay — this avoids needing the `stwo_proving` feature while still
//! exercising the blocking check integration points.
//!
...
/// A mock runner that returns an error after an optional delay.
///
/// Always returns `RunnerError::InputGenerationError` to avoid needing the
/// `stwo_proving` feature. Tests distinguish blocking check outcomes by
/// checking for `TransactionBlocked` vs `RunnerError`.
#[derive(Clone)]
struct MockRunner {
    delay: Duration,
}
...
async fn test_no_blocking_check_configured_proceeds_to_proving() {
    let prover = build_prover(MockRunner::immediate(), None);
    let result = prove(&prover).await;
    assert_runner_error(&result);
}

#[tokio::test]
async fn test_check_blocked_before_proof_returns_blocked() {
    ...
    // Slow runner so the check completes first.
    let prover = build_prover(MockRunner::new(Duration::from_secs(5)), Some(client));
...
#[tokio::test]
async fn test_check_allowed_before_proof_proceeds_to_proving() {
    let mut server = Server::new_async().await;
    let _mock = server
        .mock("POST", "/")
        .with_status(200)
    ...
    let url = Url::parse(&server.url()).unwrap();
    let client = BlockingCheckClient::new(url, 0, true);
    // Slow runner so check completes first, then proof continues.
    let prover = build_prover(MockRunner::new(Duration::from_millis(200)), Some(client));
...
#[tokio::test]
async fn test_proof_ready_first_timeout_zero_fail_open_proceeds_to_proving() {
    // Use a URL that will hang (no response) to ensure check doesn't complete before the proof.
    let url = Url::parse("http://10.255.255.1:1").unwrap();
    let client = BlockingCheckClient::new(url, 0, true);
    let prover = build_prover(MockRunner::immediate(), Some(client));
...
#[tokio::test]
async fn test_proof_ready_first_timeout_zero_fail_close_returns_blocked() {
    // Use a URL that will hang (no response) to ensure check doesn't complete before the proof.
    let url = Url::parse("http://10.255.255.1:1").unwrap();
    let client = BlockingCheckClient::new(url, 0, false);
    let prover = build_prover(MockRunner::immediate(), Some(client));
