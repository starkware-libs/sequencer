...
/// A runner that panics if called. Used in tests where validation should reject the transaction
/// before the runner is invoked — reaching this runner means the validation under test failed.
#[derive(Clone)]
...
/// A runner that always returns an error. Used in tests that need to verify the runner is reached
/// (i.e., validation passed) without requiring a real node.
#[derive(Clone)]
...
#[rstest]
#[case::non_empty_proof("non-empty proof field", {
    let mut tx = build_valid_invoke();
    extract_invoke_v3_mut(&mut tx).proof = Proof(Arc::new(vec![0u8]));
    tx
})]
#[case::non_empty_proof_facts("non-empty proof_facts field", {
    let mut tx = build_valid_invoke();
...
#[rstest]
#[case::non_zero_l1_gas("non-zero l1_gas resource bounds", {
    let mut tx = build_valid_invoke();
...
#[case::non_zero_tip("non-zero tip", {
    let mut tx = build_valid_invoke();
...
/// Happy-path validation test: a default-valid invoke (empty proof fields, zero fees, Latest block)
/// must pass every validation gate and be handed to the runner. Uses `FailingRunner` so the test
/// can assert the runner was reached without needing a real node; if validation had rejected, the
/// error would be `InvalidTransactionInput`/`ValidationError`/`InvalidTransactionType`, not
/// `RunnerError`.
#[tokio::test]
async fn test_valid_invoke_reaches_runner() {
    let prover = VirtualSnosProver::from_runner(FailingRunner);
    let result = prover.prove_transaction(BlockId::Latest, build_valid_invoke()).await;
    assert_matches!(
        result,
...
// --- Integration tests: full prover pipeline ---
