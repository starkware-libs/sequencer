...
fn valid_invoke() -> RpcTransaction {
...
async fn test_pending_block_rejected() {
    ...
    let result = prover.prove_transaction(BlockId::Pending, valid_invoke()).await;
...
#[rstest]
#[case::non_zero_l1_gas({
    let mut tx = valid_invoke();
    invoke_v3_mut(&mut tx).resource_bounds = AllResourceBounds {
...
#[tokio::test]
async fn test_fee_fields_rejected_when_validation_enabled(#[case] tx: RpcTransaction) {
...
async fn test_valid_invoke_reaches_runner() {
    ...
    let result = prover.prove_transaction(BlockId::Latest, valid_invoke()).await;
    assert_matches!(
        result,
