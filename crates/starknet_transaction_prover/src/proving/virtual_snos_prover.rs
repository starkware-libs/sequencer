...
impl<R: VirtualSnosRunner + 'static> VirtualSnosProver<R> {
    ...
    pub async fn prove_transaction(
        ...
        // The histogram spans the whole call: input validation, the optional
        // blocking check, the virtual OS run and proving. It records failures
        // too, whose latency differs from a success. The `outcome` label lets a
        // query separate the two.
        metrics::histogram!(
