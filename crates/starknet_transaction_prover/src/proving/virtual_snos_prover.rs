...
impl<R: VirtualSnosRunner + 'static> VirtualSnosProver<R> {
    ...
    async fn prove_transaction_inner(
        ...
        let invoke_v3 = extract_rpc_invoke_tx(transaction.clone()).inspect_err(|_err| {
            // No `error`: the variant carries a message payload, so it defaults to sensitive
            // under `may_embed_transaction_data` — the reason code carries all a reader needs.
        ...
        validate_transaction_input(&invoke_v3, self.validate_zero_fee_fields).inspect_err(
            |_err| {
                // No `error`: the invalid-input message quotes the client's fee inputs, and
                // transaction data is private — the reason code carries all a reader needs.
    ...
    async fn prove_with_blocking_check(
        ...
        // Arcs or small configs. Instrumenting the task carries the ambient tracing span
        // (request id and tx fields) into the spawned task, which would otherwise drop it.
