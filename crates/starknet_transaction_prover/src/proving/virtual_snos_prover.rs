...
impl<R: VirtualSnosRunner + 'static> VirtualSnosProver<R> {
    ...
    async fn prove_transaction_inner(
        ...
        // Origin logs carry only bounded event and reason codes, never the error text. See
        // `VirtualSnosProverError` for why.
    ...
    async fn run_and_prove(
        ...
        let runner_output = self
            ...
            .inspect_err(|_err| warn!(event = "os_run_error"))?;
        ...
        let result = self
            .prove_virtual_snos_run(runner_output)
            .await
            .inspect_err(|_err| warn!(event = "proving_error"))?;
