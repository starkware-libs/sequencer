...
pub(crate) async fn prove(mut cairo_pie: CairoPie) -> Result<ProverOutput, ProvingError> {
    // The virtual OS runs with `all_cairo` layout which includes ECDSA, but the privacy prover
    // uses `all_cairo_stwo` which does not. Remove the empty ECDSA additional data so the
    // bootloader doesn't try to relocate signature data into a VM without the ECDSA builtin.
    cairo_pie.additional_data.0.remove(&BuiltinName::ecdsa);
