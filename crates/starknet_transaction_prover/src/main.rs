...
async fn main() -> anyhow::Result<()> {
    ...
    use starknet_transaction_prover::server::config::{CliArgs, ServiceConfig};
    ...
    // Initialize tracing with RUST_LOG (default: info,starknet_transaction_prover=debug).
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,starknet_transaction_prover=debug"));
