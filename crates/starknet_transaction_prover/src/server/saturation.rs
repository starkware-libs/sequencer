...
//!
//! `ProvingRpcServerImpl` records rejects, worker-slot acquisitions and releases here, which lets
//! the monitor answer, via `saturated_for_at_least`, how long the service has been continuously
//! rejecting requests -- distinguishing sustained overload from an isolated reject.
...
impl SaturationMonitor {
    ...
    /// Record forward progress -- a request acquired a worker slot, or a slot was
    /// released (proving finished, failed, or the client disconnected). Either way
    /// the service is not stuck rejecting, so the saturation window clears.
    ///
