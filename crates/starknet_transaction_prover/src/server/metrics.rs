...
/// Bucket bounds, in seconds, for the queue-wait histogram, which records
/// successful worker-slot acquisitions only. A request either finds a free
/// worker slot at once or waits behind proofs, so the buckets are densest near
/// zero. The last boundary is the default queue-wait timeout. Waits that hit
/// the timeout are counted by the `rejected_wait_timeout` outcome, not here.
