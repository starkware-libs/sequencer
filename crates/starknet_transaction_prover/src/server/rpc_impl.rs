...
impl ProvingRpcServer for ProvingRpcServerImpl {
    ...
    async fn prove_transaction(
        ...
        // Binding order is load-bearing: locals drop in reverse binding order, so `_permit`
        // (bound last) releases the worker slot before the guard clears the saturation window.
        // The reverse order would let a rejection interleave between the clear and the release
        // and re-latch with no future clear event.
        let (_saturation_clear_guard, _permit) = {
            ...
            match timeout(self.queue_wait_timeout, self.concurrency_semaphore.acquire()).await {
                Ok(Ok(permit)) => {
                    ...
                    let saturation_clear_guard = SaturationClearGuard {
                        saturation_monitor: self.saturation_monitor.clone(),
                    };
                    (saturation_clear_guard, permit)
                }
                Ok(Err(_)) => {
...
/// Clears the saturation window when a worker slot is released. Freed capacity is direct
/// evidence the service can accept work again, covering the case where the load balancer
/// stopped sending traffic (so no future acquire will ever run `mark_accepted`). Drop-based
/// so the proving-success, proving-error, and client-disconnect exits all clear it.
...
impl Drop for SaturationClearGuard {
    fn drop(&mut self) {
        self.saturation_monitor.mark_slot_released();
    }
}
