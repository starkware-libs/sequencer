...
//! (which would 405 a GET). Returns 503 once its `SaturationMonitor` reports a
//! busy-reject window that has stayed open, with no worker-slot progress, for the
//! configured threshold. The response body is opaque: no timestamps, counters, or
//! upstream URLs.
...
impl HealthLayer {
    pub fn new(saturation_monitor: SaturationMonitor, threshold: Duration) -> Self {
        Self { saturation_monitor, saturation_threshold: threshold }
    }
}
...
impl<S> HealthService<S> {
    fn health_response(&self) -> Response<HttpBody> {
        let (status, body) =
            if self.saturation_monitor.saturated_for_at_least(self.saturation_threshold) {
