...
///
/// When `saturation` is `Some`, responses flip to `503` if the monitor has
/// been continuously saturated for at least `saturation_threshold`. When
/// `saturation` is `None`, `/health` always returns `200 OK`.
...
impl HealthLayer {
    /// Constructs a `HealthLayer` that never reports saturation. Behaviour
    /// matches the original always-200 implementation.
    pub fn new() -> Self {
        Self::default()
    }

    /// Constructs a `HealthLayer` that consults `monitor` and reports
    /// saturated (`503`) once the saturation window meets `threshold`.
