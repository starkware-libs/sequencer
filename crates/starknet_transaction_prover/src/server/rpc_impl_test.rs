...
async fn full_queue_rejects_with_service_busy_and_counts_queue_full() {
    ...
    // max_concurrent + max_queued = 0 → admission capacity 0 → every request is rejected at
    // admission.
...
async fn wait_timeout_rejects_with_service_busy_and_counts_wait_timeout() {
    ...
    // waits for a worker that never frees, and is rejected on the timeout.
    let saturation_monitor = SaturationMonitor::default();
    let rpc_impl = ProvingRpcServerImpl::new(
        dummy_prover(),
        0,
...
/// when the slot is later released. This is the only test that fails if the eager `mark_progress`
/// call on acquisition is removed and clearing is left solely to `SaturationClearGuard`'s drop:
/// the guard and the permit are still held at the assertion, so a cleared window can only be the
/// work of the acquisition itself.
#[tokio::test]
async fn accepted_request_clears_saturation_window_while_in_flight() {
    ...
    // Latch the window, as a burst of busy-rejects would.
    saturation_monitor.mark_rejected();
    assert!(saturation_monitor.saturated_for_at_least(Duration::ZERO));
