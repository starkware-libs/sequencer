...
/// Regression test: saturation must clear when an in-flight job releases its worker slot, even
/// if no further request ever arrives. A load balancer reacting to the 503 stops sending
/// traffic, so a clear path that depends on new requests would leave the pod 503 forever.
/// `SaturationClearGuard` is held alongside the worker permit in `prove_transaction`; dropping
/// it here is what a finishing job does. The latch itself is deliberately set directly rather
/// than through a `prove_transaction` reject: the reject paths are covered by the two tests
/// above, and going through them here would emit a second `rejected_*` outcome that races their
/// exact-delta assertions on the shared metrics recorder.
#[test]
