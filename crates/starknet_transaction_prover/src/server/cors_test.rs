...
#[test]
fn test_normalize_deduplicates_equivalent_origins() {
    // Two strings that normalize to the same origin must collapse into one entry.
    let result = normalize_cors_allow_origins(vec![
...
#[test]
fn test_normalize_wildcard_collapses_other_origins() {
    // A wildcard among a list of origins must collapse the whole list to a single "*".
    let result = normalize_cors_allow_origins(vec![
        "http://example.com".to_string(),
        "*".to_string(),
...
#[test]
fn test_normalize_rejects_origin_with_path() {
    let result = normalize_cors_allow_origins(vec!["http://example.com/path".to_string()]);
    assert_matches!(result, Err(ConfigError::InvalidArgument(_)));
}

#[test]
fn test_normalize_rejects_origin_with_query() {
    let result = normalize_cors_allow_origins(vec!["http://example.com?q=1".to_string()]);
    assert_matches!(result, Err(ConfigError::InvalidArgument(_)));
}
