...
/// `Some(bytes)` writes a tempfile and uses its path; `None` uses a nonexistent path.
/// The tempfiles are bound to local variables so they outlive the `load_tls_acceptor` call.
#[rstest]
#[case::missing_cert(None, Some(&b"dummy key"[..]))]
#[case::missing_key(Some(&b"dummy cert"[..]), None)]
#[case::invalid_pem(Some(&b"not a valid PEM cert"[..]), Some(&b"not a valid PEM key"[..]))]
fn test_load_tls_acceptor_failure(
    #[case] cert_bytes: Option<&[u8]>,
    #[case] key_bytes: Option<&[u8]>,
) {
    let cert_file = cert_bytes.map(write_pem_to_tempfile);
    let key_file = key_bytes.map(write_pem_to_tempfile);
    let cert_path: PathBuf =
        cert_file.as_ref().map_or_else(|| "/nonexistent/cert.pem".into(), |f| f.path().into());
    let key_path: PathBuf =
        key_file.as_ref().map_or_else(|| "/nonexistent/key.pem".into(), |f| f.path().into());
