...
impl KnownRpcError {
    ...
    fn sample_error_object(self) -> ErrorObjectOwned {
        match self {
            ...
            Self::UnsupportedTxVersion => errors::unsupported_tx_version("v99".to_string()),
            Self::InvalidTransactionInput => {
...
fn test_error_responses_match_spec() {
    ...
    for variant in KnownRpcError::ALL {
        let spec_entry = &SPEC["components"]["errors"][variant.spec_key()];
        assert!(!spec_entry.is_null(), "spec missing entry for KnownRpcError::{variant:?}");
    ...
    let spec_keys: HashSet<&str> =
        SPEC["components"]["errors"].as_object().unwrap().keys().map(String::as_str).collect();
