...
fn test_error_responses_match_spec() {
    ...
    let enum_keys: HashSet<&str> =
        errors::ProverRpcError::iter().map(|v| v.spec_key()).collect();
    for variant in errors::ProverRpcError::iter() {
        let spec_key = variant.spec_key();
        let spec_entry = &spec_errors[spec_key];
        assert!(
            !spec_entry.is_null(),
            "ProverRpcError variant {spec_key} is missing from the OpenRPC spec. \
             Add it to resources/proving_api_openrpc.json under components/errors."
        );
        let sample_data =
            if variant.has_data() { Some("test".to_string()) } else { None };
