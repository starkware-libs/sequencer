...
/// Default saturation window before `/health` returns 503.
...
pub struct ServiceConfig {
    ...
    /// Saturation window (milliseconds) before `/health` flips to 503: how long the service must
    /// have been continuously rejecting proving requests due to the concurrency limit.
...
pub struct CliArgs {
    ...
    /// Saturation window (milliseconds) before `/health` returns 503 (default: 10000): how long
    /// the service must have been continuously rejecting proving requests before it flips.
