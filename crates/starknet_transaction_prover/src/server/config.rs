...
struct RawServiceConfig {
    ...
    blocking_check_timeout_secs: u64,
...
impl ServiceConfig {
    ...
    pub fn from_args(args: CliArgs) -> Result<Self, ConfigError> {
        ...
        if let Some(timeout) = args.blocking_check_timeout_secs {
            if timeout != config.blocking_check_timeout_secs {
                info!(
                    "CLI override: blocking_check_timeout_secs: {} -> {}",
                    config.blocking_check_timeout_secs, timeout
                );
                config.blocking_check_timeout_secs = timeout;
            }
        }
        ...
        let prover_config = ProverConfig {
            ...
            blocking_check_timeout_secs: config.blocking_check_timeout_secs,
...
pub struct CliArgs {
    ...
    /// Seconds to wait for the blocking check after the proof is ready (0 = no extra wait).
    #[arg(long, value_name = "SECS", env = "BLOCKING_CHECK_TIMEOUT_SECS")]
    pub blocking_check_timeout_secs: Option<u64>,
