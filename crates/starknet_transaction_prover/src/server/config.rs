...
pub struct ServiceConfig {
    ...
    /// How long a busy-reject window may stay open, in milliseconds, before `/health` flips to
    /// 503. A reject opens the window and any worker-slot acquisition or release closes it.
...
pub struct CliArgs {
    ...
    /// How long a busy-reject window may stay open without worker-slot progress before
    /// `/health` returns 503 (default: 10000 ms).
