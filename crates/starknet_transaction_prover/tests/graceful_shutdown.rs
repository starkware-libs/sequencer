...
//!
//! Lives in its own integration-test binary on purpose: the test sends a real
//! `SIGTERM` to its own process, which is process-wide state. Sharing a process
//! with the unit tests would let the signal reach another test's server, and
//! would race the handler registration that makes sending it safe at all.
...
/// The first `SIGTERM` must call `stop()` on the server, which is the whole
/// promise of the signal bridge. Only the inner race helper was unit-tested;
/// this covers the path from a real signal through to a stopped server.
#[tokio::test]
