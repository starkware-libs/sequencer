...
# Real SNOS proving (happy-path and concurrency checks) can take minutes on a production
# deployment; lightweight calls (specVersion, health, malformed params, ...) use a fixed 30s cap
# instead — see rpc_call_prover.
...
# (e.g. https://host/v3/<api-key>, https://user:token@host), so logging must never echo a URL
# verbatim.
url_scheme_host() {
    local url="$1"
    local scheme="${url%%://*}"
...
rpc_call_prover() {
    local payload="$1"
    # Lightweight calls (specVersion, malformed params, ...) default to a 30s cap. Callers that
    # invoke starknet_proveTransaction for real pass $PROVE_MAX_TIME explicitly since a full SNOS
    # proof can take minutes on a production deployment.
    local max_time="${2:-30}"
...
find_tx_hash() {
    ...
    if [[ -z "$latest_block" || "$latest_block" == "null" ]]; then
        ...
        echo "  ERROR: chain RPC unreachable/invalid response for starknet_blockNumber" >&2
        return 2
    fi
...
check_health() {
    ...
    local http_status
    http_status=$(curl -sS -o "$TMP_DIR/health.json" -w '%{http_code}' --max-time 10 \
...
build_valid_prove_request() {
    ...
    if [[ -n "${TX_HASH:-}" ]]; then
        ...
        local find_result
        local find_status=0
        find_result=$(find_tx_hash "INVOKE" "0x3" "$LOOKBACK_BLOCKS") || find_status=$?
...
check_pending_block_rejected() {
    ...
    # Reuse the request built by build_valid_prove_request, swapping only the block_id param.
    # This is rejected during up-front validation before the OS runs, so it stays cheap even
    # though it depends on a request that itself requires a real chain lookup.
    local request
    request=$(jq -c '.params[0] = "pending"' "$TMP_DIR/prove_request_valid.json")

    local resp
    resp=$(rpc_call_prover "$request")
    echo "$resp" > "$TMP_DIR/prove_pending.json"

    if jq -e '.error.code == 24' "$TMP_DIR/prove_pending.json" >/dev/null; then
        pass_step "starknet_proveTransaction rejected pending block_id with error code 24 (Block not found)"
    else
        fail_step "starknet_proveTransaction did not reject pending block_id with error code 24 (Block not found)"
    fi
}
...
check_prove_happy_path() {
    ...
    local resp
    resp=$(rpc_call_prover "$(cat "$TMP_DIR/prove_request_valid.json")" "$PROVE_MAX_TIME")
...
# Sends 3 concurrent prove requests and checks the service stays up and recovers afterwards.
# `busy_count` (responses with error.code -32005) is informational only: the default
# `max_concurrent_requests` comfortably serves 3 requests, so a 0 busy_count here does not
# indicate a problem. Actually forcing a -32005 rejection requires deploying with
...
check_concurrency_and_recovery() {
    ...
    if [[ "$success_count" -ge 1 && "$transport_failures" -eq 0 && "$internal_error_count" -eq 0 ]]; then
        ...
        fail_step "Service did not handle concurrent load cleanly (success=$success_count busy=$busy_count [informational] transport_failures=$transport_failures internal_errors=$internal_error_count)"
    fi
...
check_ohttp_keys() {
    ...
    if ! grep -qi 'max-age=[1-9]' "$TMP_DIR/ohttp_keys.headers"; then
...
main() {
    ...
    # Print only scheme+host: provider RPC URLs routinely embed API keys in the path or query
    # string and must never be echoed verbatim.
