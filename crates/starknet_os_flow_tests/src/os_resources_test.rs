...
fn sha_overhead_from_measurement(
    mut measured_base: ExecutionResources,
    linear: ExecutionResources,
    block_to_round: usize,
) -> ExecutionResources {
    ...
    measured_base.n_steps += linear_steps / block_to_round;
    measured_base.n_memory_holes += linear_memory_holes / block_to_round;
    for (builtin, count) in linear_builtin_instance_counter.iter() {
        *measured_base.builtin_instance_counter.entry(*builtin).or_insert(0) +=
            BUILTIN_INSTANCE_SIZES.get(builtin).unwrap() * count / block_to_round;
    }
    measured_base
}
...
fn update_resources_for_virtual_builtin_syscall(
    ...
    match selector {
        Selector::Sha256ProcessBlock => sha_overhead_from_measurement(
            measured_base,
            SHA256_BATCH_RESOURCES_LINEAR.clone(),
            SHA256_BLOCK_TO_ROUND,
        ),
        Selector::Sha512ProcessBlock => sha_overhead_from_measurement(
            measured_base,
            SHA512_BATCH_RESOURCES_LINEAR.clone(),
            SHA512_BLOCK_TO_ROUND,
        ),
