...
use blockifier::execution::syscalls::vm_syscall_utils::SyscallUsage;
...
const SYSCALLS_WITH_INNER_CALLS: [Selector; 4] =
    [Selector::CallContract, Selector::DelegateCall, Selector::Deploy, Selector::LibraryCall];
...
async fn test_os_resources_regression() {
    ...
    // Run test.
    // Need to run with a custom block context, to override the sierra gas mode (to make sure we
    // run in cairo-steps mode, to get VM step measurements).
    // Also, grab the execution info of the tx for later.
    ...
    let tx_trace = test_output.runner_output.os_logger.get_txs().last().unwrap();
    let syscall_traces = tx_trace.get_syscalls();
    ...
    let measurements: IndexMap<Selector, ExecutionResources> = syscall_traces
        ...
        .filter_map(|syscall_trace| {
            ...
            // iterator.
            let inner_overhead = if SYSCALLS_WITH_INNER_CALLS.contains(&selector) {
    ...
    for (syscall, resources) in measurements {
        let expected_resources = vc.get_additional_os_syscall_resources(&HashMap::from([(
            syscall,
