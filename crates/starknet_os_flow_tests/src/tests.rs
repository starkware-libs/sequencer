...
async fn declare_deploy_scenario(#[values(1, 2)] n_blocks: usize) {
    ...
    let ContractClass::V1((test_contract_casm, _)) = test_contract.get_class() else {
        panic!("Expected a V1 contract class.");
    };
    // Set non-trivial resource bounds.
    ...
    let sierra_version =
        SierraVersion::extract_from_program(&test_contract_sierra.sierra_program).unwrap();
    let contract_class = ContractClass::V1((test_contract_casm, sierra_version.clone()));
    ...
    // TODO(Nimrod): Add validations on the output.
}
