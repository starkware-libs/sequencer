...
async fn declare_deploy_scenario(#[values(1, 2)] n_blocks: usize) {
    ...
    use starknet_os::io::os_output::OsStateDiff;
    ...
    let OsStateDiff::Full(full_os_state_diff) = os_runner_output.os_output.state_diff else {
        panic!("A full OS state diff is expected because of the OS run config.");
    };
    // Verify that the OS output contains a new entry in the classes trie.
    assert!(
        full_os_state_diff
            .classes
            .iter()
            .find(|entry| entry.class_hash == class_hash)
            .is_some_and(|entry| entry.next_compiled_class_hash == compiled_class_hash
                && entry.prev_compiled_class_hash == CompiledClassHash::default())
    );
    // Verify that there is an address with the deployed class hash.
    assert!(
        full_os_state_diff
            .contracts
            .iter()
            .find(|entry| entry.new_class_hash == class_hash)
            .is_some_and(|entry| entry.prev_class_hash == ClassHash::default())
    );
}
