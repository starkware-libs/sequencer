...
async fn test_os_logic(
    ...
    test_manager.add_funded_account_invoke(invoke_tx_args! {
        calldata,
    ...
    // Call set_value(address=81, value=0) on the first contract without proof_facts (no client-side
    // proving). Used to test redundant value update (0 -> 0) and make sure it is not written to
    // on-chain data.
    let calldata = create_calldata(
