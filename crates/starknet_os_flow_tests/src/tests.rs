...
async fn trivial_diff_scenario(
    ...
    let (mut test_manager, [test_contract_address]) = TestBuilder::create_standard_with_config(
...
async fn test_os_logic(
    ...
    update_expected_storage(
        &mut expected_storage_updates,
        delegate_proxy_address,
        **get_storage_var_address("two_counters", &[l1_handler_calldata.0[0]]),
...
async fn test_v1_bound_accounts_cairo0() {
    ...
    let (mut test_manager, _) = TestBuilder::create_standard([]).await;
    let chain_id = &test_manager.chain_id();
...
async fn test_new_class_execution_info(#[values(true, false)] use_kzg_da: bool) {
    ...
    let (mut test_manager, [main_contract_address]) = TestBuilder::create_standard_with_config(
        [(test_contract, calldata![Felt::ZERO, Felt::ZERO])],
...
async fn test_experimental_libfuncs_contract(#[values(true, false)] use_kzg_da: bool) {
    let (mut test_manager, []) = TestBuilder::create_standard_with_config(
        [],
        TestBuilderConfig { use_kzg_da, ..Default::default() },
    )
    .await;
...
async fn test_deprecated_tx_info() {
    ...
    let (mut test_manager, _) = TestBuilder::create_standard([(tx_info_writer, calldata![])]).await;
    let chain_id = &test_manager.chain_id();
...
async fn test_deprecated_send_to_l1() {
    let (mut test_manager, [test_contract_address]) = TestBuilder::create_standard([(
        FeatureContract::TestContract(CairoVersion::Cairo0),
        calldata![Felt::ZERO, Felt::ZERO],
    )])
    .await;
...
async fn test_deploy_syscall() {
    ...
    let (mut test_manager, _) = TestBuilder::create_standard([
        (test_contract, calldata![Felt::ZERO, Felt::ZERO]),
        (empty_contract, calldata![]),
    ])
    .await;
...
async fn test_initial_sierra_gas() {
    ...
    let (mut test_manager, [account_address]) = TestBuilder::create_standard_with_config(
...
async fn test_single_empty_block() {
    let (test_manager, _) = TestBuilder::create_standard([]).await;
    let test_output = test_manager.build().await.run();
    test_output.perform_default_validations();
}
