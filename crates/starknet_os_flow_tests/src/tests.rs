...
async fn test_new_class_flow(#[case] use_kzg_da: bool, #[case] n_blocks_in_multi_block: usize) {
    ...
    let (mut test_manager, mut nonce_manager, [main_contract_address]) =
        TestManager::<DictStateReader>::new_with_default_initial_state([(
            test_contract,
    ...
    let expected_execution_info = ExpectedExecutionInfo::new(
        ...
        *NON_TRIVIAL_RESOURCE_BOUNDS,
        nonce_manager.get(*FUNDED_ACCOUNT_ADDRESS),
    )
    .to_syscall_result();
    let invoke_tx_args = invoke_tx_args! {
        sender_address: *FUNDED_ACCOUNT_ADDRESS,
    ...
    let deploy_account_tx =
        deploy_account_tx(deploy_tx_args, nonce_manager.next(faulty_account_address));
...
async fn test_reverted_call() {
    ...
    let (
        mut test_manager,
        mut nonce_manager,
        [main_contract_address, test_contract2_address, empty_contract_address],
    ) = TestManager::<DictStateReader>::new_with_default_initial_state([
        // The `my_storage_var` cell is initialized as the sum of the ctor args in the constructor.
        // This cell is also used in revert tests, so it must be initialized to zero.
        (test_contract, calldata![Felt::ZERO, Felt::ZERO]),
...
async fn test_resources_type() {
    let test_contract = FeatureContract::TestContract(CairoVersion::Cairo1(RunnableCairo1::Casm));
    let (mut test_manager, mut nonce_manager, [sierra_gas_contract_address]) =
        TestManager::<DictStateReader>::new_with_default_initial_state([(
            test_contract,
            calldata![Felt::ZERO, Felt::ZERO],
        )])
...
async fn test_meta_tx() {
    ...
    let (mut test_manager, mut nonce_manager, [meta_tx_contract_address, tx_info_contract_address]) =
        TestManager::<DictStateReader>::new_with_default_initial_state([
