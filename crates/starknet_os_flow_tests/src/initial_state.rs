...
pub(crate) static DEPLOY_ACCOUNT_ARGS: LazyLock<DeployAccountTxArgs> = LazyLock::new(|| {
...
pub(crate) const CONTRACT_ADDRESS_SALT: ContractAddressSalt = ContractAddressSalt(Felt::ONE);
pub(crate) static ERC_20_CONSTRUCTOR_CALLDATA: LazyLock<[Felt; 9]> = LazyLock::new(|| {
    [
...
-> (Vec<Transaction>, OsExecutionContracts, NonceManager) {
    ...
    // Deploy an account.
    let deploy_account_tx_args = DEPLOY_ACCOUNT_ARGS.clone();
    ...
    let calldata: Vec<_> = [erc20_class_hash.0, CONTRACT_ADDRESS_SALT.0]
        .into_iter()
        .chain([ERC_20_CONSTRUCTOR_CALLDATA.len().into()])
        .chain(ERC_20_CONSTRUCTOR_CALLDATA.iter().cloned())
        .collect();
    ...
    (txs, os_execution_contracts, nonce_manager)
}
