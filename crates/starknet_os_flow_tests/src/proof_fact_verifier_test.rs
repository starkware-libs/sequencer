...
/// fixture's leaves carry the proving side's test-task hash, and the transaction proofs
/// the OS does accept are verified against the privacy recursion circuits or the
/// production (`large_proofs`) registry - not the `canonical_small` registry the fold's
/// circuit hashes are pinned to. Until the fold pins the production registry and a
/// recursive-tree fixture over OS-valid proof facts exists, the matching path is covered
/// by `test_cairo_fold_matches_circuit_verifier_output`, and the OS output wiring by the
/// fold assertions of every flow test.
#[tokio::test]
