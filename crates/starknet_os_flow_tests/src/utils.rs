...
pub(crate) fn create_cached_state_input_and_commitment_infos(
    ...
    let mut contract_storage_keys = extended_state_diff.storage.keys().fold(
    ...
    address_to_previous_storage_root_hash.keys().for_each(|address| {
        contract_storage_keys.entry(*address).or_default();
    });
    ...
    let storage_tries_commitment_infos = address_to_previous_storage_root_hash
        ...
        .map(|(address, previous_root_hash)| {
            let storage_proof = flatten_preimages(
                storage_proofs.contracts_trie_storage_proofs.get(address).unwrap(),
            );
            (
                *address,
                CommitmentInfo {
