...
fn fetch_storage_proofs_from_state_maps(
    ...
    let class_hashes: Vec<ClassHash> = {
        let mut class_hashes: HashSet<ClassHash> =
            state_maps.compiled_class_hashes.keys().cloned().collect();
        class_hashes.extend(state_maps.class_hashes.values().cloned());
        class_hashes.into_iter().collect()
    };
