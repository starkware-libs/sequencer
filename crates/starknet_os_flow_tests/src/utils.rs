...
/// Builds the OS initial reads for the given state.
///
/// With the `os_input` feature, delegates to `CachedState::get_os_initial_reads`, building the
/// reads exactly as production does. Without it, extends the raw initial reads with the class hash
/// and nonce of each accessed contract and the compiled class hash of each accessed class.
#[cfg(feature = "os_input")]
pub(crate) fn build_os_initial_reads<S: StateReader>(state: &mut CachedState<S>) -> StateMaps {
    state.get_os_initial_reads().unwrap()
}
