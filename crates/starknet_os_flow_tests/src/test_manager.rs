...
use starknet_os::io::os_output::{MessageToL2, OsOutput, OsStateDiff, StarknetOsRunnerOutput};
...
impl OsTestOutput {
    ...
    fn perform_global_validations(&self) {
        ...
        let raw_os_output = self.runner_output.raw_os_output.clone();
        let os_output = OsOutput::try_from_output_iter(&mut raw_os_output.into_iter())
            .expect("Creating OsOutput from raw OS output should not fail.");
...
impl<S: FlowTestState> TestManager<S> {
    ...
    fn get_decompressed_state_diff(
        ...
        let raw_os_output = runner_output.raw_os_output.clone();
        let os_output = OsOutput::try_from_output_iter(&mut raw_os_output.into_iter())
            .expect("Creating OsOutput from raw OS output should not fail.");
