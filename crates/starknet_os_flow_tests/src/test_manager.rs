...
impl<S: FlowTestState> TestManager<S> {
    ...
    pub(crate) fn new_with_initial_state_data(initial_state_data: InitialStateData<S>) -> Self {
        Self {
            ...
            config: TestManagerConfig::default(),
    ...
    /// Returns the next block number to be used for execution.
    pub(crate) fn next_block_number(&self) -> BlockNumber {
    ...
    pub(crate) fn divide_transactions_into_n_blocks(&mut self, n_blocks: usize) {
        ...
        self.per_block_txs =
            divide_vec_into_n_parts(self.per_block_txs.pop().unwrap(), n_blocks);
    }
    ...
    pub(crate) async fn execute_flow_test(self) -> OsTestOutput<S> {
        ...
        {
            let block_context =
                block_context_from_base_context(&base_block_context, block_index, use_kzg_da);
        ...
        let previous_block_number = PreviousBlockNumber(os_block_inputs.first().unwrap().block_info.block_number.prev());
        ...
        let blockifier_chain_info = base_block_context.chain_info();
        let chain_info = OsChainInfo {
            chain_id: blockifier_chain_info.chain_id.clone(),
            strk_fee_token_address: blockifier_chain_info
                .fee_token_addresses
                .strk_fee_token_address,
        };
...
fn block_context_from_base_context(
    ...
    BlockContext::new(
        BlockInfo {
            block_number: BlockNumber(
                base_context.block_info().block_number.0
                    + u64::try_from(block_index + 1).unwrap(),
            ),
            use_kzg_da,
            ..base_context.block_info().clone()
        },
        base_context.chain_info().clone(),
        base_context.versioned_constants().clone(),
        BouncerConfig::max(),
    )
}
