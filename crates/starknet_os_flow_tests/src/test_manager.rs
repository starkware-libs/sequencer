...
impl<S: FlowTestState> TestManager<S> {
    ...
    pub(crate) async fn execute_test_with_block_contexts(
        ...
        Self::execute_flow_test(initial_state_data, self.per_block_transactions, block_contexts)
            .await
    }
    ...
    // Private static method which executes the flow test.
    async fn execute_flow_test(
        mut initial_state_data: InitialStateData<S>,
        per_block_txs: Vec<Vec<BlockifierTransaction>>,
        block_contexts: Vec<BlockContext>,
    ) -> StarknetOsRunnerOutput {
