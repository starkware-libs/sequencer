...
fn expected_proof_facts_fold(os_block_inputs: &[OsBlockInput]) -> (Felt, Felt, usize) {
    ...
    for block_input in os_block_inputs {
        ...
        // The OS bakes leaf circuit index 0 for every transaction until the production
        // registry (with several leaf circuits) is adopted; mirror that here.
        let transaction_proof_facts: Vec<TransactionProofFacts<'_>> = block_proof_facts
            .iter()
            .map(|proof_facts| TransactionProofFacts {
                proof_facts: &proof_facts.0,
                leaf_circuit_index: 0,
            })
            .collect();
        block_root_entries.push(fold_block_proof_facts(&transaction_proof_facts));
    }
