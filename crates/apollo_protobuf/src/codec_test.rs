...
#[case(100)]
#[case(1_000)]
fn test_encode_decode_multiple_messages_all_at_once(#[case] num_messages: usize) {
    let mut test_codec = codec(DEFAULT_MAX_MESSAGE_LEN_BYTES);
    let mut buf = BytesMut::new();
    let mut messages = Vec::new();

    // Create varying messages with different lengths and content
    for i in 0..num_messages {
        let message_len = i % 100;
        let message = TestMessage { data: vec![(i % 256) as u8; message_len] };
        messages.push(message.clone());
        test_codec.encode(message, &mut buf).unwrap();
    }

    for i in 0..num_messages {
        let decoded = test_codec.decode(&mut buf).unwrap();
        assert_eq!(decoded, Some(messages[i].clone()));
    }
    assert!(buf.is_empty());
}
...
fn test_multiple_messages_byte_by_byte() {
    ...
    for i in 0..total_len {
        ...
        let result = test_codec.decode(&mut partial_buf).unwrap();
        if partial_buf.is_empty() {
            let message = result.unwrap();
            assert_eq!(message, sent_messages[received_messages]);
            // Verify that we just finished reading at the expected message boundary
            assert_eq!(i, expected_message_end_positions[received_messages]);
            received_messages += 1;
        } else {
...
fn test_incomplete_message_returns_none(#[case] bytes_to_remove: usize) {
    ...
    assert_eq!(result.unwrap(), None);
    // Testing that we didn't touch partial_buf
