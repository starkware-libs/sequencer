...
//!
//! This module provides a [`ProtoCodec`] implementation that encodes and decodes
...
impl<T: Message + Default> Decoder for ProtoCodec<T> {
    type Item = T;
...
impl<T: Message> Encoder for ProtoCodec<T> {
    type Item<'a> = T;
    type Error = std::io::Error;
