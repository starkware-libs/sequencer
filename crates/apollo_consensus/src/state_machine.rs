...
/// The function signature used for real and virtual leader selection.
pub(crate) type LeaderFn<'a> = dyn Fn(Round) -> ValidatorId + 'a;
...
impl StateMachine {
    ...
    pub(crate) fn start(
        ...
        self.advance_to_round(0, leader_fn, virtual_leader_fn)
    }
    ...
    pub(crate) fn handle_event(
        ...
        self.handle_enqueued_events(leader_fn, virtual_leader_fn)
    }
    ...
    fn handle_enqueued_events(
        ...
        while let Some(event) = self.events_queue.pop_front() {
            let mut resultant_requests =
                self.handle_event_internal(event, leader_fn, virtual_leader_fn);
    ...
    fn handle_event_internal(
        ...
        match event {
            StateMachineEvent::FinishedBuilding(proposal_id, round) => {
                self.handle_finished_building(proposal_id, round, leader_fn, virtual_leader_fn)
            }
            ...
            StateMachineEvent::Prevote(vote) => {
                self.handle_prevote(vote, leader_fn, virtual_leader_fn)
            }
            StateMachineEvent::Precommit(vote) => {
                self.handle_precommit(vote, leader_fn, virtual_leader_fn)
            }
            StateMachineEvent::TimeoutPropose(round) => {
                self.handle_timeout_propose(round, virtual_leader_fn)
            }
            StateMachineEvent::TimeoutPrevote(round) => {
                self.handle_timeout_prevote(round, virtual_leader_fn)
            }
            StateMachineEvent::TimeoutPrecommit(round) => {
                self.handle_timeout_precommit(round, leader_fn, virtual_leader_fn)
            }
    ...
    fn handle_finished_building(
        ...
        self.map_round_to_upons(round, leader_fn, virtual_leader_fn)
    }
    ...
    fn handle_timeout_propose(
        ...
        output.append(&mut self.advance_to_step(Step::Prevote, virtual_leader_fn));
        output
    }
    ...
    fn handle_timeout_prevote(
        ...
        output.append(&mut self.advance_to_step(Step::Precommit, virtual_leader_fn));
        output
    }
    ...
    fn handle_timeout_precommit(
        ...
        self.advance_to_round(round + 1, leader_fn, virtual_leader_fn)
    }
    ...
    fn advance_to_round(
        ...
        output.append(&mut self.current_round_upons(virtual_leader_fn));
        output
    }
    ...
    fn advance_to_step(
        ...
        self.step = step;
        self.current_round_upons(virtual_leader_fn)
    }

    fn map_round_to_upons(
        &mut self,
        round: u32,
        leader_fn: &LeaderFn<'_>,
        virtual_leader_fn: &LeaderFn<'_>,
        ...
        match round.cmp(&self.round) {
            std::cmp::Ordering::Less => self.past_round_upons(round, virtual_leader_fn),
            std::cmp::Ordering::Equal => self.current_round_upons(virtual_leader_fn),
            std::cmp::Ordering::Greater => {
                self.maybe_advance_to_round(round, leader_fn, virtual_leader_fn)
            }
        }
    }

    fn current_round_upons(&mut self, virtual_leader_fn: &LeaderFn<'_>) -> VecDeque<SMRequest> {
        let mut output = VecDeque::new();
        output.append(&mut self.upon_new_proposal(virtual_leader_fn));
        output.append(&mut self.upon_reproposal(virtual_leader_fn));
        output.append(&mut self.maybe_initiate_timeout_prevote());
        output.append(&mut self.upon_prevote_quorum(virtual_leader_fn));
        output.append(&mut self.upon_nil_prevote_quorum(virtual_leader_fn));
        output.append(&mut self.maybe_initiate_timeout_precommit());
        output.append(&mut self.upon_decision(self.round, virtual_leader_fn));
        output
    }
    ...
    fn past_round_upons(
        ...
        let mut output = VecDeque::new();
        output.append(&mut self.upon_reproposal(virtual_leader_fn));
        output.append(&mut self.upon_decision(round, virtual_leader_fn));
        output
    }

    // LOC 22 in the paper.
    fn upon_new_proposal(&mut self, virtual_leader_fn: &LeaderFn<'_>) -> VecDeque<SMRequest> {
    ...
    // LOC 28 in the paper.
    fn upon_reproposal(&mut self, virtual_leader_fn: &LeaderFn<'_>) -> VecDeque<SMRequest> {
        if self.step != Step::Propose {
            return VecDeque::new();
        }
        let Some((proposal_id, valid_round)) = self.proposals.get(&self.round) else {
    ...
    // LOC 36 in the paper.
    fn upon_prevote_quorum(&mut self, virtual_leader_fn: &LeaderFn<'_>) -> VecDeque<SMRequest> {
        if self.step == Step::Propose {
            return VecDeque::new();
        }
    ...
    // LOC 44 in the paper.
    fn upon_nil_prevote_quorum(&mut self, virtual_leader_fn: &LeaderFn<'_>) -> VecDeque<SMRequest> {
        if self.step != Step::Prevote {
            return VecDeque::new();
        }
    ...
    fn maybe_advance_to_round(
        ...
        {
            self.advance_to_round(round, leader_fn, virtual_leader_fn)
        } else {
            VecDeque::new()
        }
    }
    ...
    fn virtual_leader_in_favor(
        ...
        let virtual_leader = virtual_leader_fn(round);
