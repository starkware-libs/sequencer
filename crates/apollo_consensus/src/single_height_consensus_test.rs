...
fn proposer() {
    ...
    assert!(matches!(reqs.pop_front(), Some(SMRequest::StartBuildProposal { round: 0 })));
...
fn rebroadcast_votes() {
    ...
    // Rebroadcast with current round (round 1) - should broadcast and schedule another rebroadcast.
    let rebroadcast_vote = precommit(Some(BLOCK.id.0), 0, 1, *PROPOSER_ID);
    let ret =
        shc.handle_event(&leader_fn, StateMachineEvent::RebroadcastVote(rebroadcast_vote.clone()));
    let reqs = match ret.unwrap() {
        ShcReturn::Requests(r) => r,
        _ => panic!("expected requests"),
    };
    // Expect both BroadcastVote and ScheduleTimeoutRebroadcast for the same vote.
    assert!(reqs.iter().any(|req| matches!(
        req,
        SMRequest::BroadcastVote(v) if *v == rebroadcast_vote
    )));
    assert!(reqs.iter().any(|req| matches!(
        req,
        SMRequest::ScheduleTimeoutRebroadcast(v) if *v == rebroadcast_vote
    )));
}
...
fn repropose() {
    ...
    match ret {
        ShcReturn::Requests(reqs) => {
            assert!(reqs.iter().any(
                |req| matches!(req, SMRequest::ScheduleTimeoutPrevote { round } if *round == 0)
            ));
            assert!(reqs.iter().any(|req| matches!(
                req,
                SMRequest::BroadcastVote(v) if v.vote_type == VoteType::Precommit && v.round == 0
    ...
    match ret {
        ShcReturn::Requests(reqs) => {
            assert!(reqs.iter().any(|req| matches!(
                req,
                SMRequest::ScheduleTimeoutPrecommit { round } if *round == 0
            )));
        }
        _ => panic!("expected requests"),
    }
        ...
        ShcReturn::Requests(reqs) => {
            assert!(reqs.iter().any(|req| matches!(
                ...
                SMRequest::Repropose {
                    proposal_id,
                    init
                } if *proposal_id == BLOCK.id && init.round == 1 && init.valid_round == Some(0)
            )));
            assert!(reqs.iter().any(|req| matches!(
                req,
                SMRequest::BroadcastVote(v) if v.vote_type == VoteType::Prevote && v.round == 1
