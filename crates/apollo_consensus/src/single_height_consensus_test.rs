...
fn repropose_flow() {
    ...
    let ret = shc
        .handle_event(&leader_fn, StateMachineEvent::FinishedBuilding(Some(BLOCK.id), 0))
        .unwrap();
    // Step 1: Expect a BroadcastVote(Prevote) request for round 0.
    match ret {
    ...
    // Step 2: No new requests are expected at this point.
    match ret {
    ...
    let ret =
        shc.handle_vote(&leader_fn, prevote(Some(BLOCK.id.0), 0, 0, *VALIDATOR_ID_2)).unwrap();
    // Step 3: Expect ScheduleTimeoutPrevote{round:0} and BroadcastVote(Precommit).
    match ret {
        ShcReturn::Requests(reqs) => {
    ...
    // Step 4: Expect Repropose with init.round == 1, init.valid_round == Some(0), and a
