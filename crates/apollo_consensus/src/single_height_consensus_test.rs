...
fn rebroadcast_votes() {
    ...
    assert_matches!(ret, ShcReturn::Requests(mut reqs) => {
        assert_matches!(reqs.pop_front(), Some(SMRequest::Repropose(proposal_id, init)) if proposal_id == BLOCK.id && init.round == 1 && init.valid_round == Some(0));
        assert_matches!(reqs.pop_front(), Some(SMRequest::BroadcastVote(v)) if v.vote_type == VoteType::Prevote && v.round == 1);
        assert!(reqs.is_empty());
    });
