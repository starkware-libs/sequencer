...
fn proposer() {
    ...
    let virtual_leader_fn = |_round| -> ValidatorId { *PROPOSER_ID };
    // Start should request to build proposal.
    let start_ret = shc.start(&leader_fn, &virtual_leader_fn);
...
fn validator(repeat_proposal: bool) {
    ...
    let ret = shc.handle_proposal(&virtual_leader_fn, block_info.clone());
    assert_matches!(ret, mut reqs => {
    ...
    let ret = shc.handle_vote(
        &leader_fn,
        &virtual_leader_fn,
        // Virtual leader (PROPOSER_ID) must be in favor for the quorum to be accepted.
        prevote(Some(BLOCK.id.0), HEIGHT_0, ROUND_0, *PROPOSER_ID),
    );
    // Expect a precommit broadcast request present.
    assert_matches!(ret, mut reqs => {
...
fn vote_twice(same_vote: bool) {
    ...
    let _ = shc.handle_vote(&leader_fn, &virtual_leader_fn, first_vote.clone());
    ...
    // decision.
    let res = shc.handle_vote(&leader_fn, &virtual_leader_fn, second_vote.clone());
...
fn rebroadcast_votes() {
    ...
    let virtual_leader_fn = |_round| -> ValidatorId { *PROPOSER_ID };
    // Start and build.
    let _ = shc.start(&leader_fn, &virtual_leader_fn);
    ...
    let _ = shc.handle_vote(
        &leader_fn,
        &virtual_leader_fn,
        precommit(None, HEIGHT_0, ROUND_0, *VALIDATOR_ID_1),
    );
    let _ = shc.handle_vote(
        &leader_fn,
        &virtual_leader_fn,
        precommit(None, HEIGHT_0, ROUND_0, *VALIDATOR_ID_2),
    );
    ...
    let ret = shc.handle_event(
        &leader_fn,
        &virtual_leader_fn,
        StateMachineEvent::TimeoutPrecommit(ROUND_0),
    );
    assert_matches!(ret, mut reqs => {
...
async fn duplicate_votes_during_awaiting_finished_building_are_ignored() {
    ...
    let leader_fn = |_round| -> ValidatorId { *PROPOSER_ID };
    let virtual_leader_fn = |_round| -> ValidatorId { *PROPOSER_ID };
    let ret = shc.start(&leader_fn, &virtual_leader_fn);
    assert_matches!(ret, mut reqs => {
        assert_matches!(reqs.pop_front(), Some(SMRequest::StartBuildProposal(ROUND_0)));
        assert!(reqs.is_empty());
    });
    ...
    assert_matches!(
        shc.handle_vote(&leader_fn, &virtual_leader_fn, duplicate_vote.clone()),
...
fn broadcast_vote_before_decision_on_validation_finish() {
    ...
    let round = block_info.round;
    let ret = shc.handle_proposal(&virtual_leader_fn, block_info);
    assert_matches!(ret, mut reqs => {
    ...
    let ret = shc.handle_vote(
        &leader_fn,
        &virtual_leader_fn,
        precommit(None, HEIGHT_0, ROUND_0, *VALIDATOR_ID_3),
    );
