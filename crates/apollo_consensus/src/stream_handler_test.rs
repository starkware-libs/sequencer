...
fn setup() -> (
    ...
    let (inbound_internal_sender, network_to_client_receiver) = mpsc::channel(CHANNEL_SIZE);
...
async fn inbound_in_order() {
    ...
    let (
        ...
        mut network_to_client_receiver,
    ...
    let mut receiver = network_to_client_receiver.next().now_or_never().unwrap().unwrap();
    for i in 0..num_messages {
...
async fn inbound_delayed_first() {
    ...
    assert!(network_to_client_receiver.try_next().is_err());
