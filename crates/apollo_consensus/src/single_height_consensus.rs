...
pub enum ShcTask {
    ...
    ValidateProposal(ProposalInit, oneshot::Receiver<Option<ProposalCommitment>>),
}
    ...
    pub async fn run(self) -> ShcEvent {
        ...
        match self {
            ...
            ShcTask::ValidateProposal(init, block_receiver) => {
                ...
                // The output is an option of the proposal id.
                // - If the proposed id and the calculated id match, set it as Some.
                ...
                // failures.
                let proposal_id = block_receiver.await.unwrap_or_default();
