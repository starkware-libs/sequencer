...
// No async timers in SHC; manager handles timing.
...
impl SingleHeightConsensus {
    ...
    fn handle_state_machine_requests(
        ...
        if let Some(pos) =
            requests.iter().position(|r| matches!(r, SMRequest::DecisionReached(_, _)))
        {
            if let Some(SMRequest::DecisionReached(proposal_id, round)) = requests.remove(pos) {
