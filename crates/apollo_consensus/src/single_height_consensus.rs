...
impl SingleHeightConsensus {
    pub(crate) fn new(
        ...
        let state_machine =
            StateMachine::new(height, id, n_validators, is_observer, quorum_type, proposer_cache.clone());
        Self {
            validators,
            timeouts,
            state_machine,
    ...
    #[instrument(skip_all)]
    pub(crate) fn handle_proposal(&mut self, block_info: ConsensusBlockInfo) -> Requests {
        ...
        let Some(proposer_id) = self.proposer_cache.virtual_proposer(block_info.round).ok() else {
            return VecDeque::new();
        };
        ...
        if self.state_machine.has_proposal_for_round(block_info.round)
            || self.pending_validation_rounds.contains(&block_info.round)
        {
            warn!("Round {} already handled a proposal, ignoring", block_info.round);
            return VecDeque::new();
        }
        let timeout = self.timeouts.get_proposal_timeout(block_info.round);
        info!(
            "Accepting {block_info:?}. node_round: {}, timeout: {timeout:?}",
            self.state_machine.round()
        );
        ...
        self.pending_validation_rounds.insert(block_info.round);
        // Ask the manager to start validation.
        VecDeque::from([SMRequest::StartValidateProposal(block_info)])
    }
    ...
    fn handle_finished_validation(
        ...
        let Some(proposer) = self.proposer_cache.virtual_proposer(round).ok() else {
            return VecDeque::new();
        };
        debug!(
            proposer = %proposer,
        ...
        self.state_machine
            .handle_event(StateMachineEvent::FinishedValidation(proposal_id, round, None))
    }
    ...
    fn handle_finished_building(
        ...
        self.state_machine
            .handle_event(StateMachineEvent::FinishedBuilding(proposal_id, round))
    }
