...
fn make_consensus_config() -> ConsensusConfig {
...
async fn run_consensus_dynamic_client_updates_validator_between_heights(
    ...
    let first = consensus_config.dynamic_config.clone();
    let second = ConsensusDynamicConfig { validator_id: *PROPOSER_ID, ..first.clone() };
    mock_client.expect_get_consensus_dynamic_config().times(1).return_const(Ok(first));
    mock_client.expect_get_consensus_dynamic_config().times(1).return_const(Ok(second));
