...
async fn manager_multiple_heights_unordered(consensus_config: ConsensusConfig) {
    ...
    context
        ...
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::ONE) && *r == ROUND_0)
        .return_once(move |_, _, _, _| Ok(()));
    context
        .expect_decision_reached()
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::TWO) && *r == ROUND_0)
        .return_once(move |_, _, _, _| Ok(()));
...
async fn test_timeouts(consensus_config: ConsensusConfig) {
    ...
    context
        .expect_decision_reached()
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::ONE) && *r == ROUND_1)
        .return_once(move |_, _, _, _| Ok(()));
...
async fn future_height_limit_caching_and_dropping(mut consensus_config: ConsensusConfig) {
    ...
    context
        .expect_decision_reached()
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::ZERO) && *r == ROUND_0)
        .return_once(move |_, _, _, _| Ok(()));
    context
        .expect_decision_reached()
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::ONE) && *r == ROUND_0)
...
async fn current_height_round_limit_caching_and_dropping(mut consensus_config: ConsensusConfig) {
    ...
    context
        .expect_decision_reached()
        .withf(move |_, r, c, _| *c == ProposalCommitment(Felt::ONE) && *r == ROUND_2)
        .return_once(move |_, _, _, _| Ok(()));
    let committee_provider =
...
async fn manager_runs_normally_when_height_is_greater_than_last_voted_height(
    ...
    context
        .expect_decision_reached()
        .withf(move |_, _, c, _| *c == ProposalCommitment(Felt::ONE))
        .return_once(move |_, _, _, _| Ok(()));
    let committee_provider =
        mock_committee_provider_with_members(vec![*PROPOSER_ID, *VALIDATOR_ID]);
    let mut manager = MultiHeightManager::new(
...
/// Before any height has been run the committee size is unknown (`0`), which disables the cap so
/// no votes are dropped.
#[test]
fn future_votes_uncapped_when_committee_size_unknown() {
    let mut cache = ConsensusCache::<MockTestContext>::new(FutureMsgLimitsConfig::default());
    let num_votes = 50;
