...
#[case::is_true(true)]
#[tokio::test]
async fn manager_sends_write_prev_height_to_single_height_consensus(
    consensus_config: ConsensusConfig,
    #[case] skip_write_prev_height_blob: bool,
) {
    ...
    let mut context = MockTestContext::new();
    context.expect_try_sync().with(eq(CURRENT_HEIGHT)).times(1).returning(|_| false);

    // We verify that the single height consensus instance is created with skipping writing the
    // previous height blob.
    ...
    mock_single_height_consensus_factory
        .expect_new_instance()
        .withf(move |args: &SingleHeightConsensusArgs| {
            args.skip_write_prev_height_blob == skip_write_prev_height_blob
        })
        .returning(move |args: SingleHeightConsensusArgs| SingleHeightConsensus::new(args));

    let mut manager = MultiHeightManager::new_with_single_height_consensus_factory(
        consensus_config,
        QuorumType::Byzantine,
        Arc::new(Mutex::new(NoOpHeightVotedStorage)),
        Box::new(mock_single_height_consensus_factory),
    );
    let mut subscriber_channels = subscriber_channels.into();
    manager
        .run_height(
...
async fn manager_tells_single_height_consensus_to_skip_writing_previous_height_blob(
    ...
    mock_single_height_consensus_factory
        .expect_new_instance()
        .withf(move |args: &SingleHeightConsensusArgs| args.skip_write_prev_height_blob)
        .returning(move |args: SingleHeightConsensusArgs| SingleHeightConsensus::new(args));
