...
async fn run_consensus_dynamic_client_updates_validator_between_heights() {
    // Setup votes channel and proposals stream.
    let TestSubscriberChannels { mock_network, subscriber_channels } =
        mock_register_broadcast_topic().unwrap();
    ...
    // Context with expectations: H1 via sync; at H2 we are the proposer.
    let mut context = MockTestContext::new();
    context.expect_set_height_and_round().returning(move |_, _| ());
    ...
    // Expect build_proposal at height 2 with proposer == PROPOSER_ID.
    context
    ...
    context
        .expect_decision_reached()
        .withf(move |_block, votes| votes.first().map(|v| v.height) == Some(2))
        .return_once(move |_, _| {
