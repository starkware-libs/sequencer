        ...
        loop {
            ...
            let shc_return = tokio::select! {
                message = broadcast_channels.broadcasted_messages_receiver.next() => {
                    let message = message.ok_or(ConsensusError::InternalNetworkError("Votes channel should never be closed".to_string()))?;
                    self.handle_vote(context, height, Some(shc), message, broadcast_channels).await?
                },
                content_receiver = proposals_receiver.next() => {
                    let content_receiver = content_receiver.ok_or(ConsensusError::InternalNetworkError("Proposals channel should never be closed".to_string()))?;
    ...
    async fn handle_vote(
        ...
        let message = match vote {
            ...
            (Err(e), metadata) => {
                ...
                warn!("Failed to parse incoming consensus vote, dropping vote: {e}");
                return Ok(ShcReturn::Requests(VecDeque::new()));
            }
        };
