...
use crate::storage::{get_voted_height_storage, HeightVotedStorageTrait};
...
impl<ContextT: ConsensusContext> MultiHeightManager<ContextT> {
    ...
    pub(crate) fn new(consensus_config: ConsensusConfig, quorum_type: QuorumType) -> Self {
        let voted_height_storage =
            get_voted_height_storage(consensus_config.static_config.storage_config.clone());
    ...
    async fn run_height_inner(
        ...
        // consensus.
        if self.last_voted_height_at_initialization.is_some()
            && self.last_voted_height_at_initialization.unwrap() <= height
        {
            info!(
