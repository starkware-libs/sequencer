...
impl<ContextT: ConsensusContext> MultiHeightManager<ContextT> {
    ...
    async fn process_start_height(
        ...
        if let Some(decision) = self
            .execute_requests(context, height, pending_requests, shc_events, broadcast_channels)
            .await?
        {
    ...
    async fn run_request(
        ...
        match request {
            ...
            SMRequest::DecisionReached(_) => {
                // DecisionReached requests are handled in execute_requests after processing
                // all other requests. This should never be reached here.
                Err(ConsensusError::InternalInconsistency(
                    "DecisionReached request should be handled in execute_requests, not \
                     run_request"
                        .to_string(),
                ))
            }
