...
const PROPOSAL_ARRIVAL_DELAY: (u64, u64) = (2, 20); // Delay for proposal to arrive after round start
const PREVOTE_ARRIVAL_DELAY: (u64, u64) = (20, 50); // Delay for prevotes to arrive after round start
const PRECOMMIT_AFTER_PREVOTE_DELAY: (u64, u64) = (5, 20); // Delay for precommits after prevotes

// Processing delays
const VALIDATION_DELAY: (u64, u64) = (15, 30); // Time to validate a proposal
const BUILD_PROPOSAL_DELAY: (u64, u64) = (15, 30); // Time to build a proposal

// Timeout delays
const TIMEOUT_PROPOSE_DELAY: (u64, u64) = (15, 30); // Timeout for propose step
const TIMEOUT_PREVOTE_DELAY: (u64, u64) = (5, 10); // Timeout for prevote step
const TIMEOUT_PRECOMMIT_DELAY: (u64, u64) = (5, 10); // Timeout for precommit step
...
impl DiscreteEventSimulation {
    ...
    fn pre_generate_all_rounds(&mut self) {
        for round_idx in 0..self.num_rounds {
            ...
            // 1. Proposal from leader
            let proposal_tick = round_start_tick
                + self.rng.gen_range(PROPOSAL_ARRIVAL_DELAY.0..PROPOSAL_ARRIVAL_DELAY.1);
    ...
    /// Schedules honest peer votes for a round. Votes are scheduled within the round time range.
    fn schedule_peer_votes(&mut self, round: Round, round_start_tick: u64) {
        ...
        for i in 1..self.validators.len() {
            ...
            let prevote_tick = round_start_tick
                + self
                    .rng
                    .gen_range(PREVOTE_ARRIVAL_DELAY.0..PREVOTE_ARRIVAL_DELAY.1)
                    .min(round_end_tick - 1);
            let precommit_tick = prevote_tick
                + self
                    .rng
                    .gen_range(PRECOMMIT_AFTER_PREVOTE_DELAY.0..PRECOMMIT_AFTER_PREVOTE_DELAY.1)
                    .min(round_end_tick);
            ...
            match req {
                ...
                SMRequest::ScheduleTimeout(step, round) => {
                    let (delay, event) = match step {
                        Step::Propose => (
                            self.rng.gen_range(TIMEOUT_PROPOSE_DELAY.0..TIMEOUT_PROPOSE_DELAY.1),
                            StateMachineEvent::TimeoutPropose(round),
                        ),
                        Step::Prevote => (
                            self.rng.gen_range(TIMEOUT_PREVOTE_DELAY.0..TIMEOUT_PREVOTE_DELAY.1),
                            StateMachineEvent::TimeoutPrevote(round),
                        ),
                        Step::Precommit => (
                            self.rng
                                .gen_range(TIMEOUT_PRECOMMIT_DELAY.0..TIMEOUT_PRECOMMIT_DELAY.1),
