...
lazy_static! {
    static ref VALIDATOR_ID: ValidatorId =
        ValidatorId::from(u64::try_from(NODE_UNDER_TEST).unwrap());
}
...
impl DiscreteEventSimulation {
    ...
    /// If `equivocate` is true, sends two conflicting votes:
    /// - If `first_is_fake` is false: first with the given commitment (should_count), second with
    ///   fake (should_count=false)
    /// - If `first_is_fake` is true: first with fake commitment (should_count), second with the
    ///   given commitment (should_count=false)
    fn schedule_prevote_and_precommit(
        ...
        // Determine first and second commitments when equivocating
        let (first_commitment, second_commitment) = if equivocate && first_is_fake {
            (fake_commitment, commitment)
        } else {
            (commitment, fake_commitment) // second_commitment won't be used when not equivocating
        };
        ...
        self.schedule(
            ...
            InputEvent::Vote(
                Vote {
                    ...
                    height: HEIGHT_0,
                    round,
                    proposal_commitment: first_commitment,
                    voter,
                },
                should_count,
            ),
        ...
        // If equivocating, send second set with conflicting commitment
        if equivocate {
            self.schedule(
                prevote_delay + 1,
                InputEvent::Vote(
                    Vote {
                        ...
                        height: HEIGHT_0,
                        round,
                        proposal_commitment: second_commitment,
                        voter,
                    },
    ...
    fn generate_faulty_traffic(&mut self, node_idx: usize, round: Round) {
        ...
        match self.get_fault_type(node_idx, round) {
            ...
            FaultType::NilVoter => {
                ...
                self.schedule_prevote_and_precommit(node_id, round, None, false, false, false);
            }
            FaultType::EquivocatorRealFirst => {
                ...
                self.schedule_prevote_and_precommit(
                    ...
                    true,
                    true,
                    false, // Real commitment first
                );
            }
            FaultType::EquivocatorFakeFirst => {
                // Send two conflicting votes (different commitments) for the same round
                // First vote with fake commitment should count, second should not count.
                self.schedule_prevote_and_precommit(
                    node_id,
                    round,
                    proposal_commitment,
...
fn verify_result(sim: &DiscreteEventSimulation, result: Option<&Decision>) {
                ...
                if v.vote_type == VoteType::Precommit {
                    if let Some(proposal_commitment) = v.proposal_commitment {
                        let entry = stats.entry(v.round).or_insert_with(|| RoundStats {
                            expected_commitment: proposal_commitment_for_round(v.round, false),
                            ..Default::default()
                        });
            ...
            | InputEvent::Internal(StateMachineEvent::FinishedBuilding(c, r)) => {
                if let Some(proposal_commitment) = *c {
                    let entry = stats.entry(*r).or_insert_with(|| RoundStats {
                        expected_commitment: proposal_commitment_for_round(*r, false),
                        ..Default::default()
                    });
                    if proposal_commitment == entry.expected_commitment {
    ...
    for r in 0..=sim.current_max_round {
        if let Some(s) = stats.get(&r) {
            ...
            let expected_commitment = proposal_commitment_for_round(r, false);
