...
fn number_of_required_votes(is_honest: bool) {
    let id = *VALIDATOR_ID;
    let num_validators = if is_honest {
        3 // Honest quorum requires 2 votes out of 3 validators.
    } else {
        4 // Byzantine quorum requires 3 votes out of 4 validators.
    };
    let mut wrapper =
        TestWrapper::new(id, num_validators, |_: Round| *PROPOSER_ID, false, is_honest);
    ...
    // In case is_byzantine is false (honest quorum), the second vote is enough for a quorum.
