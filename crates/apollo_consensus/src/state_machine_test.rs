...
impl TestWrapper {
    ...
    pub fn start(&mut self) {
        self.requests
            .append(&mut self.state_machine.start(&self.leader_fn, &self.virtual_leader_fn))
    }
    ...
    pub fn send_finished_validation_with_valid_round(
        &mut self,
        proposal_id: Option<ProposalCommitment>,
        round: Round,
        valid_round: Round,
    ) {
        self.send_event(StateMachineEvent::FinishedValidation(
            proposal_id,
            round,
            Some(valid_round),
        ))
    ...
    fn send_event(&mut self, event: StateMachineEvent) {
        self.requests.append(&mut self.state_machine.handle_event(
            event,
            &self.leader_fn,
            &self.virtual_leader_fn,
        ));
    }
}
...
fn quorums_require_virtual_leader_in_favor_for_value() {
    // Virtual leader is fixed to VALIDATOR_ID_3 and must be one of the voters in favor for:
    // - LOC 35 (prevote quorum for a value -> precommit)
    // - LOC 49 (precommit quorum for a value -> decision)
    let mut wrapper = TestWrapper::new(
        *VALIDATOR_ID,
        4,
        |_: Round| *PROPOSER_ID,
    ...
    wrapper.send_prevote(PROPOSAL_ID, ROUND); // peer 1
    assert!(wrapper.next_request().is_none());
    wrapper.send_prevote(PROPOSAL_ID, ROUND); // peer 2

    // Mixed prevote quorum still schedules timeout, but we must NOT precommit yet.
    assert_eq!(wrapper.next_request().unwrap(), SMRequest::ScheduleTimeout(Step::Prevote, ROUND));
    assert!(wrapper.next_request().is_none());

    // Now the virtual leader prevotes for the value -> we can precommit.
    wrapper.send_prevote(PROPOSAL_ID, ROUND); // peer 3 (virtual leader)
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::BroadcastVote(mk_vote(VoteType::Precommit, ROUND, PROPOSAL_ID, *VALIDATOR_ID))
    );
    ...
    wrapper.send_precommit(PROPOSAL_ID, ROUND); // peer 1
    assert!(wrapper.next_request().is_none());
    wrapper.send_precommit(PROPOSAL_ID, ROUND); // peer 2
    ...
    wrapper.send_precommit(PROPOSAL_ID, ROUND); // peer 3 (virtual leader)
    assert_decision_reached(&mut wrapper, PROPOSAL_ID);
}

#[test]
fn reproposal_requires_virtual_leader_prevote_in_valid_round() {
    let mut wrapper = TestWrapper::new(
        *VALIDATOR_ID,
        4,
        |_: Round| *PROPOSER_ID,
        |_: Round| *VALIDATOR_ID_3, // virtual leader is VALIDATOR_ID_3
        false,
        QuorumType::Byzantine,
    );
    ...
    // Round 0: receive proposal and self-prevote for it.
    wrapper.send_finished_validation(PROPOSAL_ID, ROUND);
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::BroadcastVote(mk_vote(VoteType::Prevote, ROUND, PROPOSAL_ID, *VALIDATOR_ID))
    ...
    // Reach prevote quorum in round 0 WITHOUT the virtual leader's prevote (self + 2 peers).
    ...
    // Advance to round 1 via timeouts (no need to reach any decision in round 0).
    wrapper.send_timeout_prevote(ROUND);
    // We broadcast a nil precommit due to timeout.
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::BroadcastVote(mk_vote(VoteType::Precommit, ROUND, None, *VALIDATOR_ID))
    );
    ...
    // Round 1: receive a reproposal referring to valid_round=0.
    wrapper.send_finished_validation_with_valid_round(PROPOSAL_ID, ROUND + 1, ROUND);
    // Should NOT prevote yet: virtual leader hasn't prevoted for the value in valid_round.
    assert!(wrapper.next_request().is_none());

    // Now inject the missing virtual leader prevote in round 0 for the value.
    wrapper.send_prevote_from(PROPOSAL_ID, ROUND, *VALIDATOR_ID_3);
    // Now the reproposal condition can be accepted -> we prevote in round 1.
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::BroadcastVote(mk_vote(VoteType::Prevote, ROUND + 1, PROPOSAL_ID, *VALIDATOR_ID))
    );
