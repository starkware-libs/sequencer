...
fn assert_broadcast_prevote(
    ...
    let expected = mk_vote(VoteType::Prevote, round, proposal_id, voter);
    match wrapper.next_request() {
        Some(SMRequest::BroadcastVote(v)) if v == expected => {}
        other => panic!("expected BroadcastVote(Prevote {:?}), got {:?}", expected, other),
    }
}
...
fn assert_broadcast_precommit(
    ...
    let expected = mk_vote(VoteType::Precommit, round, proposal_id, voter);
    match wrapper.next_request() {
        Some(SMRequest::BroadcastVote(v)) if v == expected => {}
        other => panic!("expected BroadcastVote(Precommit {:?}), got {:?}", expected, other),
    }
}
