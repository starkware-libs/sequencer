...
fn advance_to_round_when_proposer_function_fails() {
    ...
    // we return empty VecDeque but still update the round, and can still
    // progress via receiving 2/3 precommits.
    ...
    // When advancing to round 1, proposer lookup fails, so we return empty VecDeque
    // but round should still be updated
    assert!(wrapper.next_request().is_none());
