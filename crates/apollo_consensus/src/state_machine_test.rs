...
fn events_arrive_in_ideal_order(is_proposer: bool) {
    ...
    wrapper.start();
    if is_proposer {
        assert_eq!(wrapper.next_request().unwrap(), SMRequest::StartBuildProposal { round: ROUND });
        wrapper.send_finished_building(PROPOSAL_ID, ROUND);
    } else {
        // Waiting for the proposal.
        assert_eq!(
            wrapper.next_request().unwrap(),
            SMRequest::ScheduleTimeoutPropose { round: ROUND }
        );
    ...
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::DecisionReached { proposal_id: PROPOSAL_ID.unwrap(), round: ROUND }
    );
    assert!(wrapper.next_request().is_none());
}
...
fn buffer_events_during_get_proposal(vote: Option<ProposalCommitment>) {
    ...
    wrapper.start();
    assert_eq!(wrapper.next_request().unwrap(), SMRequest::StartBuildProposal { round: 0 });
    assert!(wrapper.next_request().is_none());
...
fn advance_to_the_next_round() {
    ...
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::ScheduleTimeoutPropose { round: ROUND + 1 }
    );
    assert_eq!(
        wrapper.next_request().unwrap(),
...
fn dont_handle_enqueued_while_awaiting_get_proposal() {
    ...
    wrapper.send_timeout_precommit(ROUND);
    assert_eq!(wrapper.next_request().unwrap(), SMRequest::StartBuildProposal { round: ROUND + 1 });
    assert!(wrapper.next_request().is_none());
    ...
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::ScheduleTimeoutPrevote { round: ROUND + 1 }
    );
    // Nil Prevote quorum, so we broadcast a nil Precommit.
    assert_eq!(
        wrapper.next_request().unwrap(),
    ...
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::ScheduleTimeoutPrecommit { round: ROUND + 1 }
    );
}
...
fn return_proposal_if_locked_value_is_set() {
    ...
    assert_eq!(
        ...
        SMRequest::Repropose {
            ...
            init: apollo_protobuf::consensus::ProposalInit {
                height: HEIGHT,
                round: ROUND + 1,
