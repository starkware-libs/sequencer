...
fn quorums_require_virtual_leader_in_favor_for_value() {
    // Virtual leader (VALIDATOR_ID_3) must be one of the voters in favor for:
    // - prevote quorum for a value -> precommit
    // - precommit quorum for a value -> decision
    let mut wrapper = TestWrapper::new(
        *VALIDATOR_ID,
        4,
        |_: Round| Ok(*PROPOSER_ID),
    ...
    wrapper.send_prevote_from(PROPOSAL_ID, ROUND, *VALIDATOR_ID_3); // peer 3 (virtual leader)
    assert_eq!(
        wrapper.next_request().unwrap(),
        SMRequest::BroadcastVote(mk_vote(VoteType::Precommit, ROUND, PROPOSAL_ID, *VALIDATOR_ID))
    );
...
fn reproposal_requires_virtual_leader_prevote_in_valid_round() {
    let mut wrapper = TestWrapper::new(
        *VALIDATOR_ID,
        4,
        |_: Round| Ok(*PROPOSER_ID),
        |_: Round| Ok(*VALIDATOR_ID_3), // virtual leader is VALIDATOR_ID_3
        false,
        QuorumType::Byzantine,
    );
