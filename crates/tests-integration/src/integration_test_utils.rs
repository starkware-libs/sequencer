    ...
    Fut: Future<Output = TransactionHash> + 'a,
{
    ...
    let tx_hashes = [
        send_rpc_tx(account0_invoke_nonce1, send_rpc_tx_fn).await,
        send_rpc_tx(account0_invoke_nonce2, send_rpc_tx_fn).await,
        send_rpc_tx(account1_invoke_nonce1, send_rpc_tx_fn).await,
    ];
