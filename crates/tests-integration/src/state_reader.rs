...
use mempool_test_utils::starknet_api_test_utils::{deploy_account_tx, FeatureAccount};
...
pub async fn spawn_test_rpc_state_reader(accounts: Vec<FeatureAccount>) -> SocketAddr {
    ...
    .into_iter()
    .map(|contract| {
        FeatureAccount::new_with_custom_address(contract, contract.get_instance_address(0))
    })
    .chain(accounts)
    .collect();
    ...
    let storage_reader =
        initialize_papyrus_test_state(block_context.chain_info(), BALANCE, accounts, fund_accounts);
...
fn initialize_papyrus_test_state(
    ...
    contract_instances: Vec<FeatureAccount>,
    fund_additional_accounts: Vec<ContractAddress>,
) -> StorageReader {
    let state_diff = prepare_state_diff(
        chain_info,
        contract_instances.clone(),
        initial_balances,
...
fn prepare_state_diff(
    ...
    for contract in contract_instances {
        ...
        match contract.contract.cairo_version() {
            CairoVersion::Cairo0 => {
        ...
        fund_feature_account_contract(&mut storage_diffs, &contract, initial_balances, chain_info);
    }
        ...
        match contract.contract.cairo_version() {
            CairoVersion::Cairo0 => {
                cairo0_contract_classes.push((
                    contract.class_hash,
                    serde_json::from_str(&contract.contract.get_raw_class()).unwrap(),
                ));
            }
            CairoVersion::Cairo1 => {
...
fn fund_feature_account_contract(
    ...
    match contract.contract {
        ...
        | FeatureContract::FaultyAccount(_) => {
            fund_account(storage_diffs, &contract.sender_address(), initial_balances, chain_info);
        }
        _ => (),
    }
}
