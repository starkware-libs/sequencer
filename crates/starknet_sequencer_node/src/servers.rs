...
async fn await_first_and_stop_others(
    futures_with_strings: Vec<(
        impl Future<Output = Result<(), ComponentServerError>> + Send + 'static,
        String,
    )>,
) -> anyhow::Result<()> {
    let mut tasks: FuturesUnordered<JoinHandle<(Result<(), ComponentServerError>, String)>> =
        futures_with_strings
            .into_iter()
            .map(|(future, label)| {
    ...
    if let Some(result) = tasks.next().await {
        ...
        match result {
            Ok((res, label)) => {
                error!("First completed: {}", label);

                // Panic after stopping other futures
                error!("Exiting after stopping all other tasks!");
                Ok(res?)
            }
            Err(e) => {
...
impl LocalServers {
    async fn run(self) -> anyhow::Result<()> {
        let result = await_first_and_stop_others(vec![
