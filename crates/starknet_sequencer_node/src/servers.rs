...
/// * `$max_concurency` - the maximum number of concurrent connections the server will handle.
///
/// # Returns
///
...
/// * $max_concurency - the maximum number of concurrent requests the server will handle.
...
macro_rules! create_local_server {
    ($execution_mode:expr, $component:expr, $receiver:expr, $max_concurency:expr, $server_type:tt) => {
        match *$execution_mode {
            ReactiveComponentExecutionMode::LocalExecutionWithRemoteDisabled
            | ReactiveComponentExecutionMode::LocalExecutionWithRemoteEnabled => {
