...
pub async fn run_component_servers(servers: SequencerNodeServers) -> anyhow::Result<()> {
    let local_servers_future = servers.local_servers.run().await;
    let remote_servers_future = servers.remote_servers.run().await;
    let wrapper_servers_future = servers.wrapper_servers.run().await;

    let mut all_servers = FuturesUnordered::new();
    all_servers.extend(local_servers_future);
    all_servers.extend(remote_servers_future);
    all_servers.extend(wrapper_servers_future);

    if let Some((_, servers_type)) = all_servers.next().await {
        panic!("{} Servers ended unexpectedly.", servers_type);
    } else {
        panic!("Error while waiting for the first task");
    }
}
