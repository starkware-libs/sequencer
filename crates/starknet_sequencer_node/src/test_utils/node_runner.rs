...
pub fn spawn_run_node(node_config_path: PathBuf, node_runner: NodeRunner) -> AbortOnDropHandle<()> {
    AbortOnDropHandle::new(task::spawn(async move {
        ...
        let (mut node_handle, _annotator_handle, _copy_task) =
...
async fn spawn_node_child_process(
    ...
    info!("Running the node output annotator.");
    ...
    let copy_task = AbortOnDropHandle::new(tokio::spawn(async move {
    ...
    (node_process, annotator_process, copy_task)
}
