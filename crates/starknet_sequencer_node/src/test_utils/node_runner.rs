...
impl NodeRunner {
    pub fn new(node_index: usize, executable_index: usize, logs_file: PathBuf) -> Self {
        Self {
...
async fn spawn_node_child_process(
    ...
    tokio::spawn(async move {
        ...
        info!("Writing node logs to file: {:?}", node_runner.logs_file);
        let mut file = tokio::fs::File::create(node_runner.logs_file)
            .await
