...
impl NodeRunner {
    pub fn new(node_index: usize, executable_index: usize, logs_dir: PathBuf) -> Self {
        create_dir_all(&logs_dir).unwrap();
        Self { node_index, executable_index, logs_dir }
    }
    ...
    pub fn logs_file_path(&self) -> PathBuf {
        self.logs_dir.join(format!("node_{}_part_{}.log", self.node_index, self.executable_index))
    }
}
