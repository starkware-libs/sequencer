...
define_hint_enum!(
    ...
    vm_enter_scope(dict(
        ...
        aliases_pointer=aliases_pointer,
    ...
    (GuessAliasPointer, guess_alias_ptr, indoc! {r#"ids.aliases_entry = aliases_pointer"#}),
    (
        UpdateAliasPointer,
        update_alias_ptr,
        indoc! {r#"aliases_pointer = ids.squashed_aliases_storage_end"#}
    ),
    (
...
segments.write_arg(ids.sha256_ptr_end, padding)"#}
    ...
    (
        ...
        enter_execute_block_scope,
        indoc! {r#"# A map from contract address to a pointer for the contract state changes.
        # This is used to make sure that the same contract state changes are written on
        # the same memory segments through the multi-block.
        inner_state_to_pointer = dict()
        # A pointer to the sate tree to make sure that all state changes are written on the same
        # memory segments through the multi-block.
        state_pointer = segments.add()
        # A pointer to the classes tree to make sure that all class changes are written on the same
        # memory segments through the multi-block.
        classes_pointer = segments.add()
        # A pointer to the start of the state aliases to make sure that all aliases are written on the
        # same memory segments through the multi-block.
        aliases_pointer = segments.add()
        vm_enter_scope(dict(
            state_pointer=state_pointer,
            inner_state_to_pointer=inner_state_to_pointer,
            classes_pointer=classes_pointer,
...
for i, task in enumerate(tasks):
    ...
    (
        ...
        enter_combined_block_scope_aggregator,
        r#"# A map from contract address to a pointer for the contract state changes.
# This is used to make sure that the same contract state changes are written on
# the same memory segments through the multi-block.
inner_state_to_pointer = dict()
# A pointer to the sate tree to make sure that all state changes are written on the same
