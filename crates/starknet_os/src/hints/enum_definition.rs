...
define_hint_enum!(
    ...
    (
        ...
        write_use_kzg_da_to_memory,
        indoc! {r#"memory[fp + 19] = to_felt_or_relocatable(# syscall_handler is a global variable we define at execute blocks.
# This means that when we use it here we are using the last syscall handler.
...
segments.write_arg(ids.sha256_ptr_end, padding)"#}
	...
	preimage = {
	    int(root): children
	    for root, children in block_input.contract_state_commitment_info.commitment_facts.items()
	}
	...
	preimage = {
	    int(root): children
	    for root, children in block_input.contract_class_commitment_info.commitment_facts.items()
	}
