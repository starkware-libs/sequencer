...
impl Operation {
    fn encode(&self) -> [MaybeRelocatable; 2] {
        match self {
            Self::ChangeClass { class_hash } => {
                [Felt::from(*CHANGE_CLASS_ENTRY).into(), class_hash.into()]
            }
...
fn test_revert(#[case] test_vector: Vec<Operation>) {
        ...
        match operation {
            ...
            Operation::StorageWrite { address, value } => {
                expected_storages
                    ...
                    .or_insert_with(|| HashMap::from([(*address, *value)]));
            }
        }
    }
    ...
    let explicit_args = vec![
        ...
        EndpointArg::Value(ValueArg::Single(revert_log_end.into())),
    ];
    let implicit_args = vec![ImplicitArg::NonBuiltin(EndpointArg::Value(ValueArg::Single(
        contract_state_changes.clone().into(),
    )))];
