...
pub(crate) fn prepare_state_entry_for_revert(mut ctx: HintContext<'_>) -> OsHintResult {
    ...
    ctx.insert_into_scope(Scope::ContractAddressForRevert, contract_address);
    Ok(())
}
...
pub(crate) fn read_storage_key_for_revert<S: StateReader>(
    ...
    let storage_key: StorageKey = ctx.get_integer(Ids::StorageKey)?.try_into()?;
    ...
    ctx.insert_value(Ids::PrevValue, storage_value)?;
    Ok(())
}
...
pub(crate) fn generate_dummy_os_output_segment(mut ctx: HintContext<'_>) -> OsHintResult {
    ...
    ctx.insert_value(Ids::Outputs, base)?;
    Ok(())
}
