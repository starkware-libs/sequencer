...
impl PartialEq for BytecodeSegmentNode {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (BytecodeSegmentNode::Leaf(a), BytecodeSegmentNode::Leaf(b)) => a.data == b.data,
...
/// Creates the bytecode segment structure from the given bytecode, bytecode segment lengths and the
/// visited pcs. If the visited pcs are not given, assume the entire bytecode is visited.
