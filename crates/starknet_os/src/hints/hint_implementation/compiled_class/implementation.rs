...
pub(crate) fn validate_compiled_class_facts_post_execution<S: StateReader>(
    ...
    // No need for is_segment_used callback: the VM's `MemoryCell::is_accessed`.
    // TODO(Dori): See if we can avoid the clone here.
    exec_scopes.enter_scope(HashMap::from([(
        Scope::BytecodeSegmentStructures.into(),
        any_box!(bytecode_segment_structures.clone()),
    )]));
    Ok(())
}
