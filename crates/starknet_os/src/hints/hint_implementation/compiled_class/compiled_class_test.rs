...
impl TestableHashVersion for HashVersion {
    fn compiled_class_hash_entrypoint_name(&self) -> &'static str {
        match self {
            HashVersion::V1 => {
...
fn run_compiled_class_hash_entry_point(
    ...
    let program_bytes = hash_version.program_bytes();
...
fn test_compiled_class_hash_resources_estimation() {
    ...
    let (mut actual_execution_resources, _hash_computed_by_the_os) =
