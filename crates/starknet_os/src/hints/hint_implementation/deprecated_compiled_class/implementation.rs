...
pub(crate) fn load_deprecated_class_inner<S: StateReader>(
    ...
    ctx.insert_scope(Scope::ClassHash, class_hash);
    ctx.insert_scope(Scope::CompiledClass, compiled_class_v0);
...
pub(crate) fn load_deprecated_class<S: StateReader>(
    ...
    let expected_hash: ClassHash = ctx.get_scope(Scope::ClassHash)?;
