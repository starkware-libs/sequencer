...
impl<T: StarkHash> HashState<T> {
    fn new() -> Self {
        Self { current_hash: Felt::ZERO, n_words: 0, finalized: false, _marker: PhantomData }
    }

    fn update_single(&mut self, item: &Felt) {
        assert!(!self.finalized, "HashState is finalized");
    ...
    fn update_with_hashchain(&mut self, data: &[Felt]) {
        assert!(!self.finalized, "HashState is finalized");
        let n = data.len();
        let mut data_hash = Felt::ZERO;
        for x in data.iter() {
            data_hash = T::hash(&data_hash, x);
        }
        self.update_single(&T::hash(&data_hash, &Felt::from(n)));
    }

    fn finalize(&mut self) -> Felt {
        assert!(!self.finalized, "HashState is finalized");
        self.finalized = true;
