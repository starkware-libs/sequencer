...
impl CommitmentType {
    pub(crate) fn hash_builtin_struct(&self) -> CairoStruct {
        match self {
            CommitmentType::State => CairoStruct::HashBuiltin,
...
fn set_preimage_for_commitments<S: StateReader>(
    ...
    let CommitmentInfo { previous_root, updated_root, commitment_facts, tree_height } =
        match hint_processor.get_commitment_type() {
...
pub(crate) fn set_preimage_for_class_commitments<S: StateReader>(
    ...
    // Set the commitment type to class.
    hint_args.hint_processor.set_commitment_type_to_class();
    set_preimage_for_commitments(hint_args)?;
    Ok(())
}
...
pub(crate) fn load_edge<S: StateReader>(
    ...
    if preimage_len != 3 {
        return Err(OsHintError::AssertionFailed {
            message: format!(
                "The length of an edge preimage node is expected to be 3, found {preimage_len}"
            ),
        });
    }
    let (length, path, bottom) = (preimage[0], preimage[1], preimage[2]);
    ...
    insert_value_to_nested_field(
        hash_ptr,
        hint_processor.get_commitment_type().hash_builtin_struct(),
        vm,
        &["result"],
        &hint_processor.os_program,
        node.0 - length,
