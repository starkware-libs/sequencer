...
fn set_preimage_for_commitments<S: StateReader>(
    ...
    ctx.insert_value(Ids::InitialRoot.into(), previous_root.0)?;
    ctx.insert_value(Ids::FinalRoot.into(), updated_root.0)?;
...
pub(crate) fn set_preimage_for_current_commitment_info<S: StateReader>(
    ...
    ctx.insert_value(Ids::InitialContractStateRoot.into(), commitment_info.previous_root.0)?;
    ctx.insert_value(Ids::FinalContractStateRoot.into(), commitment_info.updated_root.0)?;
...
pub(crate) fn should_use_read_optimized_patricia_update<S: StateReader>(
    ...
    // hash.
    Ok(ctx.insert_value(Ids::ShouldUseReadOptimized.into(), Felt::ONE)?)
}
...
pub(crate) fn guess_state_ptr<S: StateReader>(
    ...
    Ok(ctx.insert_value(Ids::FinalSquashedContractStateChangesStart.into(), state_changes_start)?)
}
...
pub(crate) fn update_state_ptr<S: StateReader>(
    ...
    if let Some(state_update_pointers) = &mut hint_processor.state_update_pointers {
        let contract_state_changes_end =
            ctx.get_ptr(Ids::FinalSquashedContractStateChangesEnd.into())?;
...
pub(crate) fn guess_classes_ptr<'program, CHP: CommonHintProcessor<'program>>(
    ...
    Ok(ctx.insert_value(Ids::SquashedDict.into(), class_changes_start)?)
}
...
pub(crate) fn update_classes_ptr<'program, CHP: CommonHintProcessor<'program>>(
    ...
    if let Some(state_update_pointers) = &mut hint_processor.get_mut_state_update_pointers() {
        let classes_changes_end = ctx.get_ptr(Ids::SquashedDictEnd.into())?;
