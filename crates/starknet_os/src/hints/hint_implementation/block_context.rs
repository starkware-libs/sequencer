...
pub(crate) fn guess_block_info<S: StateReader>(
    ...
    ctx.insert_value(Ids::BlockInfo.into(), block_info_ptr)?;
...
pub(crate) fn chain_id_and_fee_token_address<S: StateReader>(
    ...
    ctx.insert_value(Ids::ChainId.into(), Felt::try_from(&chain_info.chain_id)?)?;
    ctx.insert_value(Ids::FeeTokenAddress.into(), **chain_info.strk_fee_token_address)?;
    Ok(())
}
