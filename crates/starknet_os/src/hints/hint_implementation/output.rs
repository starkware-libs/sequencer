...
pub(crate) fn calculate_keys_using_sha256_hash(
    ...
    // Mutable counter for nonce values, incremented for each key. Each key uses the same random
    // seed but with a different nonce to ensure cryptographic uniqueness.
    let mut nonce_counter = 0u8;
    let mut generate_key = || -> Felt {
        ...
        if nonce == 0 {
            ...
            // group order (~ 2^251). This is required for the Diffie-Hellman elliptic curve
            // encryption.
    ...
    let private_keys: Vec<MaybeRelocatable> =
        (0..num_private_keys).map(|_| generate_key().into()).collect();
