...
pub(crate) fn sha256_hash_compressed_data_with_random(
    ...
    let mut private_key_1_input = hash_result.to_vec();
    private_key_1_input.push(1);
    let private_key_1_hash = Sha256::digest(&private_key_1_input);
    // Use only first 31 bytes (248 bits) to ensure result is < 2^248 < EC group order.
    let mut private_key_1_bytes = [0u8; 32];
    private_key_1_bytes[1..].copy_from_slice(&private_key_1_hash[..31]);
    let private_key_1 = Felt::from_bytes_be(&private_key_1_bytes);
    insert_value_from_var_name(Ids::PrivateKey1.into(), private_key_1, vm, ids_data, ap_tracking)?;
