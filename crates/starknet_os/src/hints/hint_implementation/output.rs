...
pub(crate) fn set_proof_fact_topology<'program, CHP: CommonHintProcessor<'program>>(
    ...
    let onchain_data_start = ctx.get_ptr(Ids::DaStart.into())?;
    let output_ptr = ctx.get_ptr(Ids::OutputPtr.into())?;
...
pub(crate) fn set_state_updates_start(mut ctx: HintArgs<'_>) -> OsHintResult {
    let use_kzg_da_felt = ctx.get_integer(Ids::UseKzgDa.into())?;
    ...
    let compress_state_updates = ctx.get_integer(Ids::CompressStateUpdates.into())?;
    ...
    ctx.insert_value(Ids::StateUpdatesStart.into(), segment)?;

    Ok(())
}

pub(crate) fn set_compressed_start(mut ctx: HintArgs<'_>) -> OsHintResult {
    let n_keys = ctx.get_integer(Ids::NKeys.into())?;
    ...
    ctx.insert_value(Ids::CompressedStart.into(), segment)?;
...
pub(crate) fn set_encrypted_start(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    ctx.insert_value(Ids::EncryptedStart.into(), segment)?;

    Ok(())
}

pub(crate) fn set_n_updates_small(mut ctx: HintArgs<'_>) -> OsHintResult {
    let n_updates = ctx.get_integer(Ids::NUpdates.into())?;
...
pub(crate) fn calculate_keys_using_sha256_hash(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    // randomness source.
    let compressed_start = ctx.get_ptr(Ids::CompressedStart.into())?;
    let compressed_end = ctx.get_ptr(Ids::CompressedEnd.into())?;
    ...
    ctx.insert_value(Ids::SymmetricKey.into(), symmetric_key)?;
    ...
    ctx.insert_value(Ids::SnPrivateKeys.into(), private_keys_start)?;
