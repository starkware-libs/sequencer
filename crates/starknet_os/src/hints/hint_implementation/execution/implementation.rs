...
pub(crate) fn tx_proof_facts_len<S: StateReader>(
    hint_processor: &mut SnosHintProcessor<'_, S>,
    HintArgs { vm, .. }: HintArgs<'_>,
) -> OsHintResult {
    let proof_facts = get_proof_facts(hint_processor.get_current_execution_helper()?)?;
    insert_nondet_hint_value(vm, AllHints::OsHint(OsHint::TxProofFactsLen), proof_facts.0.len())?;
    Ok(())
}
...
pub(crate) fn tx_proof_facts<S: StateReader>(
    ...
    insert_value_into_ap(vm, proof_facts_base)?;
    Ok(())
}
