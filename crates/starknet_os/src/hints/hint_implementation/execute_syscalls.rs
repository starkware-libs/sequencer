...
pub(crate) fn relocate_sha256_segment<S: StateReader>(
    ...
    let actual_out_state_ptr =
        get_ptr_from_var_name(Ids::ActualOutState.into(), vm, ids_data, ap_tracking)?;
    ...
    vm.load_data(actual_out_state_ptr, &data)?;

    // Relocate segment.
    vm.add_relocation_rule(state_ptr, actual_out_state_ptr.into())?;
    Ok(())
}
