...
pub(crate) fn is_block_number_in_block_hash_buffer(mut ctx: HintArgs<'_>) -> OsHintResult {
    let request_block_number = ctx.get_integer(Ids::RequestBlockNumber.into())?;
    let current_block_number = ctx.get_integer(Ids::CurrentBlockNumber.into())?;
...
pub(crate) fn relocate_sha256_segment<S: StateReader>(
    ...
    let actual_out_state_ptr = ctx.get_ptr(Ids::ActualOutState.into())?;
