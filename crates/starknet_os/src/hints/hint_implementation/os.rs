...
pub(crate) fn write_use_kzg_da_and_full_output_to_memory<S: StateReader>(
    ...
    ctx.insert_value(Ids::UseKzgDa.into(), use_kzg_da)?;
    ctx.insert_value(Ids::FullOutput.into(), full_output)?;
    Ok(())
}
...
pub(crate) fn check_block_hash_consistency<S: StateReader>(
    ...
    let calculated_block_hash = ctx.get_integer("block_hash")?;
...
pub(crate) fn get_n_blocks<S: StateReader>(
    ...
    Ok(ctx.insert_value(Ids::NBlocks.into(), n_blocks)?)
}
...
pub(crate) fn get_n_class_hashes_to_migrate<S: StateReader>(
    ...
    ctx.insert_value(Ids::NClassesToMigrate.into(), n_classes)?;
    Ok(())
}
pub(crate) fn log_remaining_blocks<S: StateReader>(
    ...
    let n_blocks = ctx.get_integer(Ids::NBlocks.into())?;
...
pub(crate) fn get_block_hashes<S: StateReader>(
    ...
    ctx.insert_value("parent_hash", os_input.prev_block_hash.0)?;
    ...
    ctx.insert_value("header_commitments", header_commitments_ptr)?;

    let starknet_version_felt = Felt::try_from(&block_info.starknet_version)?;
    ctx.insert_value("starknet_version", starknet_version_felt)?;
    ...
    ctx.insert_value("gas_prices_hash", gas_prices_hash)?;
