...
pub(crate) fn get_block_hashes<S: StateReader>(
    ...
    // may be newer than the latest known `StarknetVersion` variant; the override carries it then,
    // while `block_info.starknet_version` falls back to the latest known version.
