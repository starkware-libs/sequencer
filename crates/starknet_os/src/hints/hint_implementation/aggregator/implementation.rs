...
pub(crate) fn allocate_segments_for_messages(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    ctx.insert_value(Ids::InitialCarriedOutputs.into(), initial_carried_outputs)?;
    Ok(())
}
...
pub(crate) fn get_os_output_for_inner_blocks(
    ...
    ctx.insert_value(Ids::OsProgramHash.into(), program_hash)?;
    ctx.insert_value(Ids::NTasks.into(), n_outputs)?;

    let os_output_ptr = ctx.get_ptr(Ids::OsOutputs.into())?;
...
pub(crate) fn get_chain_id_and_fee_token_address_from_input(
    ...
    ctx.insert_value(Ids::ChainId.into(), chain_id)?;
    ctx.insert_value(Ids::FeeTokenAddress.into(), fee_token_address)?;
    Ok(())
}
