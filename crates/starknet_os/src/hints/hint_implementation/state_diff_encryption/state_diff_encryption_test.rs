...
/// ## Helper Functions
///
/// Generate committee private keys using a seeded random number generator.
fn generate_committee_private_keys(seed: u64, num_keys: usize) -> Vec<Felt> {
...
fn test_state_diff_encryption_function(
    ...
    let private_keys = generate_committee_private_keys(seed, num_committee_members);
...
fn test_symmetric_key_encryption_function(#[case] private_keys: &[Felt]) {
    ...
    let encrypted_symmetric_keys =
        runner.vm.get_integer_range(encrypted_dst, encrypted_dst_length).unwrap();
    let encrypted_symmetric_keys: Vec<Felt> =
        encrypted_symmetric_keys.into_iter().map(|felt| *felt).collect();
