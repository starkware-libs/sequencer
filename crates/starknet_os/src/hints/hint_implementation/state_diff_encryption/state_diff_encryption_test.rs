...
fn test_compute_public_keys_function(#[case] private_keys: &[Felt]) {
    ...
    runner.vm.load_data(
            sn_private_keys,
            &sn_private_keys_vector.clone().into_iter().map(Into::into).collect::<Vec<MaybeRelocatable>>(),
        )
        .unwrap();
    ...
    let sn_private_keys_from_memory = runner.vm.get_integer_range(encrypted_dst, encrypted_dst_length).unwrap();
    let sn_private_keys_from_memory: Vec<Felt> = sn_private_keys_from_memory.into_iter().map(|felt| *felt).collect();
