...
fn test_state_diff_encryption_decryption(#[case] private_keys: &[Felt]) {
    ...
    let sn_public_keys =  runner.vm.add_memory_segment();
    let encrypted_symmetric_keys = runner.vm.add_memory_segment();
    ...
    let expected_encrypted_dst_length = data.len();
    assert_eq!(encrypted_dst_length, expected_encrypted_dst_length);
    ...
    let sn_public_keys_vec = runner.vm.get_integer_range(sn_public_keys, n_keys).unwrap();
    let sn_public_keys: Vec<Felt> = sn_public_keys_vec.into_iter().map(|felt| *felt).collect();
    let encrypted_symmetric_keys_vec = runner.vm.get_integer_range(encrypted_symmetric_keys, n_keys).unwrap();
    let encrypted_symmetric_keys: Vec<Felt> = encrypted_symmetric_keys_vec.into_iter().map(|felt| *felt).collect();
