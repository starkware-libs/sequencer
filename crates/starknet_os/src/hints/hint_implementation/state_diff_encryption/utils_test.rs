...
fn test_decrypt_state_diff_from_blobs() {
    ...
    // Random state diff.
    let state_diff_length: usize = rng.gen_range(0..=FIELD_ELEMENTS_PER_BLOB * 3);
    let mut state_diff: Vec<Felt> = Vec::with_capacity(state_diff_length);
    for _ in 0..state_diff_length {
        state_diff.push(Felt::from_bytes_be(&rng.gen::<[u8; 32]>()))
    }

    // Encrypt state diff and convert to blobs.
    let encrypted_state_diff = encrypt_state_diff(symmetric_key, &state_diff.clone());
    let encrypted_symmetric_key =
        encrypt_symmetric_key(&sn_private_keys, &public_keys, symmetric_key);
    let sn_public_keys = compute_starknet_public_keys(&sn_private_keys);
    let da_segment: Vec<Felt> = [Felt::from(n_keys)]
        .into_iter()
    ...
    let da_segment_fr: Vec<Fr> = da_segment
        .into_iter()
        .map(|felt| {
        ...
        .collect();
    let chunked_da_segment_fr = da_segment_fr.chunks(FIELD_ELEMENTS_PER_BLOB);

    let blobs = chunked_da_segment_fr
        .map(|chunk| polynomial_coefficients_to_blob(chunk.to_vec()).unwrap())
        .collect();
    let decrypted = decrypt_state_diff_from_blobs(blobs, private_keys[0], 0).unwrap();
    assert_eq!(decrypted, state_diff);
}
