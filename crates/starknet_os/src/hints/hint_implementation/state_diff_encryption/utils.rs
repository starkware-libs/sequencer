...
pub fn encrypt_symmetric_key(
    ...
    sn_private_keys
        ...
        .map(|(&sn_private_key, &public_key)| {
            let public_key_point = AffinePoint::new_from_x(&public_key, true)
                .expect("{public_key} does not represent the x coordinate of a point on the curve.");
...
pub fn encrypt_state_diff(symmetric_key: Felt, state_diff: &[Felt]) -> Vec<Felt> {
    ...
    let encrypted_state_diff = state_diff
        ...
        .enumerate()
        .map(|(i, felt)| {
            felt + calc_blake_hash(&[symmetric_key, Felt::from(i)])
        })
        .collect();
...
#[allow(dead_code)]
pub fn compute_starknet_public_keys(
    sn_private_keys: &[Felt],
) -> Vec<Felt> {
    sn_private_keys
        .iter()
