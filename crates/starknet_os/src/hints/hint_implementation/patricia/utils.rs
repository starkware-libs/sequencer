...
// TODO(Rotem): Maybe we can avoid using None. Update when implementing the other functions.
#[derive(Clone, Debug, PartialEq)]
pub enum UpdateTree {
    InnerNode(InnerNode),
    Leaf(HashOutput),
...
impl UpdateTree {
    // TODO(Rotem): make sure the hints using this function check that the node is not None or Leaf.
    pub fn get_children(&self) -> Result<(&UpdateTree, &UpdateTree), PatriciaError> {
        match self {
            UpdateTree::InnerNode(inner_node) => Ok(inner_node.get_children()),
            UpdateTree::Leaf(_) => Err(PatriciaError::ExpectedInnerNode("Leaf".to_string())),
            UpdateTree::None => Err(PatriciaError::ExpectedInnerNode("None".to_string())),
        }
    }
}

/// Given an update tree generated by build_update_tree(), returns which update case it belongs
/// to. This is a utility to make cairo hints shorter.
impl TryFrom<&UpdateTree> for DecodeNodeCase {
    type Error = PatriciaError;

    fn try_from(update_tree: &UpdateTree) -> Result<Self, Self::Error> {
        match update_tree {
            UpdateTree::InnerNode(inner_node) => Ok(DecodeNodeCase::from(inner_node)),
            UpdateTree::Leaf(_) => Err(PatriciaError::ExpectedInnerNode("Leaf".to_string())),
            UpdateTree::None => Err(PatriciaError::ExpectedInnerNode("None".to_string())),
        }
    }
