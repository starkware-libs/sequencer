...
impl InnerNode {
    ...
    pub(crate) fn decode_node(&self) -> (Children<'_>, DecodeNodeCase) {
        match self {
            InnerNode::Left(left) => {
                (Children { left_child: left.as_ref(), right_child: &None }, DecodeNodeCase::Left)
            }
            InnerNode::Right(right) => {
                (Children { left_child: &None, right_child: right.as_ref() }, DecodeNodeCase::Right)
            }
            InnerNode::Both(left, right) => (
                Children { left_child: left.as_ref(), right_child: right.as_ref() },
                DecodeNodeCase::Both,
            ),
        }
    }
