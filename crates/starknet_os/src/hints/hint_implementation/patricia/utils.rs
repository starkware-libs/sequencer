...
fn get_descents(
    ...
    let (lefts, rights) = loop {
        ...
        {
            path = path.turn_right()?;
            height = SubTreeHeight(height.0 - 1);
            if height.0 == 0 {
                break (
    ...
    if length > 1 {
        descent_map.insert(
            DescentStart { height: orig_height, path_to_upper_node: orig_path },
            path.remove_first_edges(EdgePathLength::new(u8::from(path.0.length) - length)?)?,
        );
    }

    if height.0 > 0 {
        let next_height = SubTreeHeight::new(height.0 - 1);
