...
pub(crate) fn is_case_right(ctx: HintContext<'_>) -> OsHintResult {
    let case: DecodeNodeCase = ctx.get_scope(Scope::Case)?;
...
pub(crate) fn set_ap_to_descend(mut ctx: HintContext<'_>) -> OsHintResult {
    ...
    let case_descent = match descent_map.get(&descent_start) {
        None => Felt::ZERO,
        Some(path) => {
            ctx.insert_scope(Scope::Descend, path.clone());
            Felt::ONE
        }
    };
...
pub(crate) fn prepare_preimage_validation_non_deterministic_hashes<S: StateReader>(
    ...
    let node: UpdateTree = ctx.get_scope(Scope::Node)?;
    ...
    ctx.insert_scope(Scope::LeftChild, left_child.clone());
    ctx.insert_scope(Scope::RightChild, right_child.clone());
    ctx.insert_scope(Scope::Case, case.clone());
...
pub(crate) fn build_descent_map<S: StateReader>(
    ...
    ctx.insert_scope(Scope::Node, node);
    ctx.insert_scope(Scope::DescentMap, descent_map);
...
pub(crate) fn enter_scope_new_node(mut ctx: HintContext<'_>) -> OsHintResult {
    ...
    let (new_node, case_not_left) = match case {
        DecodeNodeCase::Left => (ctx.get_scope(Scope::LeftChild)?, Felt::ZERO),
        DecodeNodeCase::Right => (ctx.get_scope(Scope::RightChild)?, Felt::ONE),
        DecodeNodeCase::Both => {
...
fn enter_scope_next_node_bit(is_left: bool, ctx: HintContext<'_>) -> OsHintResult {
    ...
    let new_node = match ids_bit {
        x if x == left_bit => ctx.get_scope(Scope::LeftChild)?,
        x if x == Felt::ONE - left_bit => ctx.get_scope(Scope::RightChild)?,
        _ => {
...
pub(crate) fn enter_scope_descend_edge(ctx: HintContext<'_>) -> OsHintResult {
    let mut new_node: UpdateTree = ctx.get_scope(Scope::Node)?;
