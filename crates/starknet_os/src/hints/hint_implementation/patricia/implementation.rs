...
fn enter_scope_next_node_bit(
    ...
    let new_node = match ids_bit {
        ...
        _ => {
            return Err(OsHintError::ExpectedBit { id: Ids::Bit, felt: left_bit });
        }
    };
    enter_scope_specific_node(new_node, exec_scopes)
}
...
pub(crate) fn enter_scope_next_node_bit_0<S: StateReader>(
    ...
    enter_scope_next_node_bit(Felt::ZERO, vm, exec_scopes, ids_data, ap_tracking)
}

pub(crate) fn enter_scope_next_node_bit_1<S: StateReader>(
    HintArgs { vm, exec_scopes, ids_data, ap_tracking, .. }: HintArgs<'_, '_, S>,
) -> OsHintResult {
    enter_scope_next_node_bit(Felt::ONE, vm, exec_scopes, ids_data, ap_tracking)
}
...
pub(crate) fn enter_scope_descend_edge<S: StateReader>(
    ...
    let length = {
        let ids_length = get_integer_from_var_name(Ids::Length.into(), vm, ids_data, ap_tracking)?;
        u8::try_from(ids_length).map_err(|error| OsHintError::IdsConversion {
            variant: Ids::Length,
            felt: ids_length,
            ty: "u8".to_string(),
            reason: error.to_string(),
    ...
    for _ in (0..length).rev() {
        ...
        new_node = match inner_node {
            ...
            InnerNode::Both(_, _) => return Err(OsHintError::ExpectedSingleChild),
        }
    }
