...
pub(crate) fn set_siblings(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    let siblings = ctx.get_ptr(Ids::Siblings.into())?;

    ctx.vm.insert_value(siblings, Felt::from(length))?;
    ctx.insert_value(Ids::Word.into(), Felt::from(&path))?;
...
pub(crate) fn is_case_right(ctx: HintArgs<'_>) -> OsHintResult {
    ...
    let bit = ctx.get_integer(Ids::Bit.into())?;
...
pub(crate) fn set_bit<S: StateReader>(
    ...
    ctx.insert_value(Ids::Bit.into(), bit_felt)?;
...
pub(crate) fn split_descend(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    ctx.insert_value(Ids::Length.into(), Felt::from(length))?;
    ctx.insert_value(Ids::Word.into(), Felt::from(&path))?;
...
pub(crate) fn height_is_zero_or_len_node_preimage_is_two<S: StateReader>(
    ...
    let height = ctx.get_integer(Ids::Height.into())?;

    let answer = if height == Felt::ZERO {
        Felt::ONE
    } else {
        let node = HashOutput(ctx.get_integer(Ids::Node.into())?);
...
pub(crate) fn prepare_preimage_validation_non_deterministic_hashes<S: StateReader>(
    ...
    let ids_node = HashOutput(ctx.get_integer(Ids::Node.into())?);
    ...
    let current_hash_address = ctx.get_ptr(Ids::CurrentHash.into())?;
...
pub(crate) fn build_descent_map<S: StateReader>(
    ...
    let update_ptr_address = ctx.get_ptr(Ids::UpdatePtr.into())?;
    ...
    let prev_root = HashOutput(ctx.get_integer(Ids::PrevRoot.into())?);
    let new_root = HashOutput(ctx.get_integer(Ids::NewRoot.into())?);
...
pub(crate) fn load_edge<S: StateReader>(
    ...
    ctx.insert_value(Ids::Edge.into(), edge_ptr)?;
    ...
    let hash_ptr = ctx.get_ptr(Ids::HashPtr.into())?;
    insert_value_to_nested_field(
        hash_ptr,
        hint_processor.commitment_type.hash_builtin_struct(),
        ctx.vm,
...
pub(crate) fn load_bottom<S: StateReader>(
    ...
    let hash_ptr_address = ctx.get_ptr(Ids::HashPtr.into())?;
    let nested_fields_and_values =
        [("x", binary_data.left_data.0.into()), ("y", binary_data.right_data.0.into())];
    insert_values_to_fields(
        hash_ptr_address,
