...
fn test_get_children() {
    ...
    assert_eq!(children_1, CanonicChildren::Both(node_2, node_3));
    assert_eq!(children_3, CanonicChildren::Left(node_6));

    // Empty node.
    let node_empty = CanonicNode::BinaryOrLeaf(HashOutput(Felt::ZERO));
    let children_empty = get_children(&node_empty, &preimage_map).unwrap();
    assert_eq!(children_empty, CanonicChildren::Both(node_empty.clone(), node_empty));
