...
pub(crate) fn remaining_gas_gt_max(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    let max_gas = ctx.get_integer(Ids::MaxGas.into())?;
    let remaining_gas_gt_max: Felt = (remaining_gas > max_gas).into();
    Ok(ctx.insert_value(Ids::RemainingGasGtMax.into(), remaining_gas_gt_max)?)
}
...
pub(crate) fn debug_expected_initial_gas<S: StateReader>(
    ...
    if current_execution_helper.os_logger.debug {
        ...
        let call_initial_gas = ctx.get_integer(Ids::InnerRemainingGas.into())?;
