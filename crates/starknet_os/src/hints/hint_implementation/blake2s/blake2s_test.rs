...
/// Return the Blockifier's estimated execution resources for Blake2s hashing,
/// used by the bouncer to decide when to close a block due to resource limits.
fn estimated_blake_hash_execution_resources(data: &[Felt]) -> ExecutionResources {
    let (n_small_felts, n_big_felts) = data_to_felt_count(data);
    let mut estimated = blake_hash_execution_resources(n_big_felts, n_small_felts);
...
fn test_cairo_vs_rust_blake2s_implementation(#[case] test_data: Vec<Felt>) {
    ...
    match result {
        Ok((_, explicit_return_values, cairo_runner)) => {
            ...
            let resources_after = cairo_runner.get_execution_resources().unwrap();
            let estimated_resources = estimated_blake_hash_execution_resources(&test_data);
            // Asserts that actual Cairo execution resources match the Blockifier’s estimate,
            // used by the bouncer to decide when to close a block due to resource limits.
            assert_eq!(resources_after, estimated_resources);
        }
        Err(e) => {
