...
/// Asserts the Cairo OS `get_contract_address_blake_escaped` matches the Rust Blake2
/// contract-address derivation (the Blake hash plus the Pedersen-image escape). This is the
/// parity guarantee for the v4 / deploy_v2 address derivation. The escape-step counts of the
/// frozen-vector cases were verified against an independent python implementation.
#[rstest]
