...
fn test_write_contract_changes() {
    ...
    for (i, changes) in first_changes.iter().chain(second_changes.iter()).enumerate() {
        ...
        let prev_state_addr = &state_dict[state_dict_index + 1];
        // A state is a triplet of the form (class_hash, storage_dict_ptr, nonce).
        let prev_state = vm
            .get_continuous_range(prev_state_addr.try_into().unwrap(), StateEntry::size())
            .unwrap();
        ...
        let new_state_addr = &state_dict[state_dict_index + 2];
        let new_state = vm
            .get_continuous_range(new_state_addr.try_into().unwrap(), StateEntry::size())
            .unwrap();
        ...
        let storage_changes =
            vm.get_continuous_range(prev_storage_addr, n_storage_mem_cells).unwrap();
        assert_eq!(
            storage_changes,
            changes
                .storage_changes
                .iter()
                .flat_map(|changes| changes.to_maybe_relocatables())
                .collect::<Vec<_>>()
        );
    }
}
