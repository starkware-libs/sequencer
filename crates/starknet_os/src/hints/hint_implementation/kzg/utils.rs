...
pub(crate) fn split_commitment(num: BigUint) -> (BigUint, BigUint) {
    // Ensure the input is 384 bits (48 bytes).
    let num = num & &((BigUint::one() << COMMITMENT_BITS) - BigUint::one());

    // Split the number.
    let mask = (BigUint::one() << COMMITMENT_BITS_MIDPOINT) - BigUint::one();
    let low = &num & &mask;
...
pub(crate) fn polynomial_coefficients_to_kzg_commitment(
...
) -> Result<(BigUint, BigUint), FftError> {
    ...
    let kzg_bigint = BigUint::from_str_radix(&commitment_bytes.as_hex_string(), 16)?;
