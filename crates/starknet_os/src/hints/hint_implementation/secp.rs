...
pub(crate) fn is_on_curve(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    ctx.insert_value(Ids::IsOnCurve.into(), Felt::from(is_on_curve))?;
...
pub(crate) fn read_ec_point_from_address<S: StateReader>(
    ...
    let not_on_curve = ctx.get_integer(Ids::NotOnCurve.into())?;
    ...
    ctx.insert_value(Ids::ResultPtr.into(), result)?;
    Ok(())
}
