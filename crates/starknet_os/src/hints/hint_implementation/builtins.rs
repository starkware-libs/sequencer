...
pub(crate) fn selected_builtins(ctx: HintArgs<'_>) -> OsHintResult {
    let n_selected_builtins = ctx.get_integer(Ids::NSelectedBuiltins.into())?;
    let new_scope =
...
pub(crate) fn select_builtin(mut ctx: HintArgs<'_>) -> OsHintResult {
    ...
    let selected_encodings_ptr = ctx.get_ptr(Ids::SelectedEncodings.into())?;
    let all_encodings_ptr = ctx.get_ptr(Ids::AllEncodings.into())?;
    ...
    ctx.insert_value(Ids::SelectBuiltin.into(), Felt::from(select_builtin))?;
...
pub(crate) fn update_builtin_ptrs<S: StateReader>(
    ...
    let n_builtins = ctx.get_integer(Ids::NBuiltins.into())?;
    ...
    let selected_encodings = ctx.get_ptr(Ids::SelectedEncodings.into())?;

    let builtin_ptrs = ctx.get_ptr(Ids::BuiltinPtrs.into())?;

    let orig_builtin_ptrs = builtin_ptrs;

    let selected_ptrs = ctx.get_ptr(Ids::SelectedPtrs.into())?;
    ...
    ctx.insert_value(Ids::ReturnBuiltinPtrs.into(), return_builtin_ptrs_base)?;
    Ok(())
}
