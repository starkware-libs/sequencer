...
fn os_intersection_aggregator_program_hints_excluding_vm_hints() -> HashSet<String> {
    let os_program_hints = program_hints(&OS_PROGRAM);
    let aggregator_program_hints = program_hints(&AGGREGATOR_PROGRAM);
    os_program_hints
...
/// C = CommonHint, S = StatelessHint, then we verify that:
/// (OP ∩ AP) \ VM ⊆ S ∪ C
fn test_shared_program_hints_in_common_or_stateless(
    common_hints: HashSet<String>,
    stateless_hints: HashSet<String>,
) {
    let shared_program_hints: HashSet<String> =
        os_intersection_aggregator_program_hints_excluding_vm_hints();

    let not_in_common_or_stateless: HashSet<_> = shared_program_hints
        .difference(&common_hints.union(&stateless_hints).cloned().collect())
        .cloned()
        .collect();

    assert!(
        not_in_common_or_stateless.is_empty(),
        "The following hints are in both OS & Aggregator programs, but not present in \
         `CommonHint` or `StatelessHint`: {not_in_common_or_stateless:#?}."
    );
}

#[rstest]
/// If OP = OS program hints, AP = aggregator program hints, VM = VM hints,
/// C = CommonHint, then we verify that:
/// C ⊆ (OP ∩ AP) \ VM
fn test_common_hints_in_os_and_aggregator_programs(common_hints: HashSet<String>) {
    let shared_program_hints: HashSet<String> =
        os_intersection_aggregator_program_hints_excluding_vm_hints();
    let difference =
        common_hints.difference(&shared_program_hints).cloned().collect::<HashSet<_>>();

    assert!(
        difference.is_empty(),
        "The following common hints are not present in the both the OS and Aggregator programs: \
         {difference:#?}."
    );
}
...
fn test_aggregator_hints_are_unique_aggregator_program_hints(
    ...
    let union_os_vm: HashSet<String> = os_program_hints.union(&vm_hints).cloned().collect();
    let unique_aggregator_program_hints: HashSet<String> =
        aggregator_program_hints.difference(&union_os_vm).cloned().collect();
