    ...
    flat_outer_dict
        ...
        .map(|chunk| {
            let (address, prev_state_entry_ptr, new_state_entry_ptr) = (
                ContractAddress(
                    PatriciaKey::try_from(chunk.first().unwrap().get_int().unwrap()).unwrap(),
                ),
