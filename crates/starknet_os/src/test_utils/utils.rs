...
pub(crate) fn parse_dict_entries(
    n_changes: usize,
    storage_changes_iter: &mut IntoIter<Felt>,
) -> Vec<DictEntry> {
    (0..n_changes)
        .map(|_| {
            let key = storage_changes_iter.next().unwrap();
            let prev_value = storage_changes_iter.next().unwrap();
            let value = storage_changes_iter.next().unwrap();
...
impl FullOutputOsStateDiff {
    pub(crate) fn new_from_felts(os_output_iter: &mut IntoIter<Felt>) -> Self {
