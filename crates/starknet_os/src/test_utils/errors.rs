...
pub enum ImplicitArgError {
    #[error(
        "If `validate_builtins_offset` is true, builtins must be at the start of implicit args; \
         found Builtin at index {builtin_index} after NonBuiltin at index {non_builtin_index}"
    )]
