...
impl ImplicitArg {
    ...
    fn memory_length(&self) -> usize {
        match self {
            ImplicitArg::Builtin(_) => 1,
            ImplicitArg::NonBuiltin(endpoint_arg) => endpoint_arg.memory_length(),
        }
    }
}
    ...
    match value_structure {
        EndpointArg::Value(value_arg) => match value_arg {
            ...
            ValueArg::Array(array) => {
                ...
                Ok((EndpointArg::Value(ValueArg::Array(felt_array)), (address + array.len())?))
            }
            ValueArg::Composed(endpoint_args) => {
                let mut current_address = address;
                let mut endpoint_arg_array = vec![];
                for endpoint_arg in endpoint_args.iter() {
        ...
        EndpointArg::Pointer(pointer_arg) => match pointer_arg {
            PointerArg::Array(array) => {
                ...
                let mut felt_array = vec![];
                for i in 0..array.len() {
                    felt_array.push(vm.get_integer((array_pointer + i)?)?.into_owned());
                }
                Ok((EndpointArg::Pointer(PointerArg::Array(felt_array)), (address + 1)?))
            }
            PointerArg::Composed(endpoint_args) => {
                let mut current_address = vm.get_relocatable(address)?;
                let mut endpoint_arg_array = vec![];
                for endpoint_arg in endpoint_args.iter() {
                    let (value, next_address) =
                        load_endpoint_arg_from_end_address(endpoint_arg, current_address, vm)?;
                ...
                Ok((EndpointArg::Pointer(PointerArg::Composed(endpoint_arg_array)), (address + 1)?))
            }
        },
    }
}
...
/// Note - the values inside the implicit & explicit return values' structures are ignored.
fn get_return_values(
    ...
    for implicit_arg in implicit_return_values_structures.iter() {
        match implicit_arg {
            ...
            ImplicitArg::NonBuiltin(non_builtin_return_value) => {
                let (value, next_arg_address) = load_endpoint_arg_from_end_address(
    ...
    for expected_return_value in explicit_return_values_structures.iter() {
        let (value, next_arg_address) =
            load_endpoint_arg_from_end_address(expected_return_value, current_address, vm)?;
