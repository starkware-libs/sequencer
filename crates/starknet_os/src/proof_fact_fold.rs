...
/// The allowed leaf cairo-verifier circuits' hashes, from the proving side's circuit
/// registry, in registry order: each is
/// blake2s(log_blowup_factor || component_log_sizes || preprocessed_root). A leaf circuit
...
/// [`LEAF_VERIFIER_CIRCUIT_HASHES`]). One value covers all fold nodes: internal and root
...
/// One transaction's fold input: its proof facts, and the index into
/// [`LEAF_VERIFIER_CIRCUIT_HASHES`] of the leaf cairo-verifier circuit that proved it.
...
/// list), any transaction's proof facts are shorter than 3 felts, or any
/// `leaf_circuit_index` is outside the [`LEAF_VERIFIER_CIRCUIT_HASHES`] table.
pub fn fold_block_proof_facts(
    ...
    let leaf_entries: Vec<FoldEntry> = per_transaction_proof_facts
        .iter()
        .map(|transaction_proof_facts| FoldEntry {
