...
pub(crate) fn get_address_of_nested_fields_from_base_address<IG: IdentifierGetter>(
    ...
    let (actual_type, actual_base_address) =
        strip_suffix_for_cairo_type(var_type.into(), base_address, vm)?;
...
/// Returns the actual type and the actual address of variable or a field depends on wether the type
/// is a pointer.
fn strip_suffix_for_cairo_type<'a>(
    cairo_type: &'a str,
    base_address: Relocatable,
    vm: &VirtualMachine,
...
fn fetch_nested_fields_address<IG: IdentifierGetter>(
    ...
    let (cairo_type, new_base_address) =
        strip_suffix_for_cairo_type(&field_member.cairo_type, new_base_address, vm)?;
