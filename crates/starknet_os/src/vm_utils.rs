...
pub(crate) trait LoadCairoObjectStateless {
    /// Inserts the cairo 0 representation of `self` into the VM at the given address.
    /// Returns the next address after the inserted object.
...
impl<T: LoadCairoObjectStateless> LoadCairoObjectStateless for Vec<T> {
    fn load_into_stateless(
        &self,
        vm: &mut VirtualMachine,
        address: Relocatable,
        ...
        let mut next_address = address;
        for t in self.iter() {
            next_address = t.load_into_stateless(vm, next_address)?;
        }
        Ok(next_address)
    }
}
