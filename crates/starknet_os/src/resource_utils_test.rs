    ...
    Felt: From<T>,
{
    ...
    for _ in 0..number_of_blocks {
        let random_felts: Vec<T> = (0..24).map(|_| rng.gen::<T>()).collect();
        input.extend_from_slice(&random_felts);
        let len = input.len();
        let message: &[T; 16] = input[len - 24..len - 8].try_into().unwrap();
        let state: &[T; 8] = input[len - 8..].try_into().unwrap();
...
fn test_finalize_sha512() {
    ...
    let number_of_rounds_1 = (number_of_blocks_1 - 1) / SHA512_BLOCK_TO_ROUND + 1;
    let number_of_blocks_2 = number_of_blocks_1 + SHA512_BLOCK_TO_ROUND;
    let number_of_rounds_2 = (number_of_blocks_2 - 1) / SHA512_BLOCK_TO_ROUND + 1;
    let resources_1 =
