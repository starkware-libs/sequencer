...
pub fn run_os<S: StateReader>(
    ...
    let use_kzg = os_hints_config.full_output && os_hints_config.use_kzg_da;
    ...
    // KZG commitment.
    let output_attributes = match runner_output
        .cairo_pie
        .additional_data
        .0
    ...
    {
        BuiltinAdditionalData::Output(OutputBuiltinAdditionalData { attributes, .. }) => {
            assert!(attributes.is_empty(), "No attributes should be added in KZG mode.");
            attributes
        }
        _ => {
    ...
    if use_kzg {
        ...
        assert!(
            ...
            "'{:?}' is missing",
            OUTPUT_ATTRIBUTE_FACT_TOPOLOGY
        );
    }
