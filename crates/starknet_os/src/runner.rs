...
pub fn run_os<S: StateReader>(
    ...
    let txs_trace: Option<Vec<OsTransactionTrace>> = if cfg!(any(test, feature = "testing")) {
        let execution_helper = snos_hint_processor.get_current_execution_helper().unwrap();
        Some(execution_helper.os_logger.get_txs().clone())
    } else {
        None
    };
