...
/// Starkware doesn't use compact formatting for JSON but default python formatting. This is
/// required to hash to the same value after sorted serialization.
...
impl serde_json::ser::Formatter for PythonDefaultFormatter {
        ...
        W: ?Sized + std::io::Write,
    {
        ...
        for c in fragment.chars() {
            if c.is_ascii() {
                writer.write_all(&[u8::try_from(c).expect("Converting ascii never fails")])?;
            } else {
