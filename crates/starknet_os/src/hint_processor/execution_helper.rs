...
pub struct OsExecutionHelper<S: StateReader> {
    ...
    // TODO(Meshi): change it to pub(crate) when it is used in future hint. 
    _os_program: Program,
}

impl<S: StateReader> OsExecutionHelper<S> {
    pub fn new(os_input: StarknetOsInput, os_program: Program) -> Self {
        Self { cached_state: Self::initialize_cached_state(&os_input), os_input, _os_program: os_program }
    }
