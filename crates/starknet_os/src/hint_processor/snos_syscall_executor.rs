...
impl<S: StateReader> SyscallExecutor for SnosHintProcessor<'_, S> {
    ...
    fn get_sha256_segment_end_ptr(
        ...
        Ok((segment_start + total_offset)?)
    }

    fn set_sha256_segment_end_ptr(&mut self, _sha256_segment_end_ptr: Relocatable) {
        // Don't change the ptr, increment the block count to calculate the offset.
