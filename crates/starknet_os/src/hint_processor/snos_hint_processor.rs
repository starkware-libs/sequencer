...
impl<'a, S: StateReader> SnosHintProcessor<'a, S> {
    ...
    /// Returns the address of the given nested fields of the current execution info.
    pub fn get_execution_info_nested_field_address(
        &self,
        nested_fields: &[&str],
        vm: &VirtualMachine,
    ) -> Result<Relocatable, ExecutionHelperError> {
        Ok(get_address_of_nested_fields_from_base_address(
