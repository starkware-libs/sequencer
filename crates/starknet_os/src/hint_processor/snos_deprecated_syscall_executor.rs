...
impl<'a, S: StateReader> SnosHintProcessor<'a, S> {
    ...
    #[allow(clippy::result_large_err)]
    fn get_mut_call_info_tracker(&mut self) -> &mut CallInfoTracker<'a> {
        self.get_mut_current_execution_helper()
    ...
    #[allow(clippy::result_large_err)]
    fn get_call_entry_point(&mut self) -> &CallEntryPoint {
        &self.get_mut_call_info_tracker().call_info.call
    }
}
...
impl<S: StateReader> DeprecatedSyscallExecutor for SnosHintProcessor<'_, S> {
    ...
    fn storage_read(
        ...
        let value =
            *syscall_handler.get_mut_call_info_tracker().execute_code_read_iterator.next().unwrap();
