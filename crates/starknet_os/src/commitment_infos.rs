...
// storage. Kept here (as a free function rather than an inherent method) because the struct now
// lives in `starknet_committer` and the orphan rule forbids an inherent impl on a foreign type.
