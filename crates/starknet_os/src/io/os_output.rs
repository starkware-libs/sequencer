...
pub struct StarknetOsRunnerOutput {
    pub os_output: Vec<Felt252>,
    pub cairo_pie: CairoPie,
}

// Retrieve the output ptr data of a finalized run as a vec of felts.
pub fn get_run_output(vm: &VirtualMachine) -> Result<Vec<Felt252>, StarknetOsError> {
...
fn get_raw_output(
    ...
    output_base: usize,
    output_size: usize,
) -> Result<Vec<Felt252>, StarknetOsError> {
    ...
    let raw_output: Result<Vec<Felt252>, _> = range_of_output
        .iter()
        .map(|x| match x {
            Some(cow) => match **cow {
