...
pub struct OsBlockInput {
    ...
    // latest known `StarknetVersion` variant (`block_info.starknet_version` is then a fallback to
    // the latest known version, whose ASCII encoding would yield the wrong hash).
