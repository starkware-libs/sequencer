...
pub trait BatcherClient: Send + Sync {
    ...
    /// Sends the content to a validation proposal to the batcher (only relevant when validating a
    /// proposal).
    /// Note: the batcher acks when the content is received, not waiting for it to finish
    /// processing. The next send might receive an `InvalidProposal` response for the previous send.
