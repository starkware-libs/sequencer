...
impl PapyrusReader {
    ...
    fn get_compiled_class_non_native_flow(
        ...
        match versioned_contract_class {
            Some(contract_class) => Ok(contract_class.get_casm()),
            None => {
                let contract_class_from_db = self.get_compiled_class_inner(class_hash)?.get_casm();
                ...
                CachedCasm::WithSierra(runnable_casm, sierra) => {
                    if let RunnableCompiledClass::V1(casm_v1) = runnable_casm.clone() {
                        ...
                        if self.contract_class_manager.wait_on_native_compilation() {
                            return Ok(self.handle_sync_native_compilation(class_hash, cached_casm));
                        }
                    }
                    Ok(runnable_casm)
                }
