...
impl PapyrusReader {
    ...
    fn get_compiled_class_non_native_flow(
        ...
        match versioned_contract_class {
            ...
            None => {
                let contract_class_from_db = match self.get_compiled_class_inner(class_hash)? {
                    CachedCasm::WithSierra(casm, _) | CachedCasm::WithoutSierra(casm) => casm,
                };
    ...
    fn handle_sync_compilation(
        ...
        match cached_native {
            CachedCairoNative::Compiled(compiled_native) => {
                self.contract_class_manager.set_casm(class_hash, cached_casm);
                ...
                CachedCasm::WithSierra(runnable_casm, sierra) => {
                    if let RunnableCompiledClass::V1(casm_v1) = runnable_casm.clone() {
                        ...
                        if self.contract_class_manager.wait_on_native_compilation() {
                            return Ok(self.handle_sync_compilation(
                                class_hash,
                                runnable_casm,
                                cached_casm,
                            ));
