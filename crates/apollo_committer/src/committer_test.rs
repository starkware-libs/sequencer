...
async fn revert_happy_flow() {
    ...
    committer
        .commit_block(CommitBlockRequest {
            state_diff: ThinStateDiff {
                class_hash_to_compiled_class_hash: indexmap! {
                    ClassHash(1_u64.into()) => CompiledClassHash(2_u64.into()),
                },
                ..Default::default()
            },
            state_diff_commitment: Some(StateDiffCommitment(PoseidonHash(2_u64.into()))),
    ...
    let response = committer.revert_block(revert_block_request(1, height_1)).await.unwrap();
