    ...
    assert_eq!(
        ...
        Err(CommitterError::InvalidStateDiffCommitment {
            input_commitment: StateDiffCommitment(PoseidonHash(
                another_state_diff_commitment.into()
            )),
            stored_commitment: StateDiffCommitment(PoseidonHash(state_diff_commitment.into())),
...
async fn commit_different_state_diff() {
    ...
    assert_matches!(
        response,
        Err(CommitterError::InvalidStateDiffCommitment { height: BlockNumber(_block_number), .. })
    );
}
