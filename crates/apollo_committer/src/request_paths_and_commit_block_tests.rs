...
/// Reverts a block committed by [`commit_block_with_empty_witnesses`] by emptying its class leaf.
async fn revert_block_with_empty_witnesses(committer: &mut ApolloTestCommitter, height: u64) {
...
async fn revert_below_commitment_infos_lower_bound() {
    ...
    for height in (1..4).rev() {
        revert_block_with_empty_witnesses(&mut committer, height).await;
    }
