...
/// Leaf payloads for accessed class hashes. Required to build facts-layout storage from the
/// returned [`PreimageMap`], which contains inner nodes only and not leaf rows.
...
/// Leaf payloads for accessed storage keys, per contract. Required to build facts-layout
/// storage from the returned [`PreimageMap`], which contains inner nodes only and not leaf
/// rows.
...
fn block_0_state_diff() -> ThinStateDiff {
    ...
    ThinStateDiff {
        ...
        storage_diffs: indexmap! {
            ...
            contract_2 => indexmap! {
                accessed_storage_key_2().0 => 200_u128.into(),
            },
        },
...
/// Re-fetches Patricia paths from facts-layout storage and verifies that every witness node
/// lies on a valid path to the requested leaves.
async fn verify_preimage_map_paths_exist<L>(
    witness_nodes: &PreimageMap,
    root_hashes: &[HashOutput],
...
{
    let mut all_fetched = PreimageMap::new();
...
async fn verify_witness_patricia_paths(
    ...
    for contract_address in &accessed_keys.contract_addresses {
        ...
        let storage_root_hashes =
            [HashOutput::ROOT_OF_EMPTY_TREE, contract_state.storage_root_hash];
...
async fn assert_witnesses_and_digest_present(
    ...
    assert!(committer.forest_storage.read_witnesses(height).await.unwrap().is_some());
}
...
async fn revert_removes_witnesses_and_digest() {
    ...
    committer
        ...
        .await
        .unwrap();
    assert_witnesses_and_digest_present(&mut committer, BlockNumber(height_1)).await;
