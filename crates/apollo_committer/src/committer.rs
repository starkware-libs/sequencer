...
/// Compresses the state commitment infos into a `base64(zstd(serde_json(..)))` string.
///
/// The witness is compressed once here, at the source, so it stays compact across the
/// committer->batcher RPC (which has an 8 MB response cap), batcher storage, and the cende blob.
#[cfg(feature = "os_input")]
...
/// zstd compression level for the state commitment infos witness.
#[cfg(feature = "os_input")]
