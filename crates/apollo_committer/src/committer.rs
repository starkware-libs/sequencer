...
impl<S: Storage> Committer<S> {
    ...
    async fn load_global_root(&mut self, block_number: BlockNumber) -> CommitterResult<GlobalRoot> {
        ...
        Ok(GlobalRoot(self.deserialize_felt(db_value)?))
    }
    ...
    fn deserialize_felt(&self, db_value: DbValue) -> CommitterResult<Felt> {
        Ok(StarknetStorageValue::deserialize(&db_value, &EmptyDeserializationContext)
            .map_err(|err| self.map_internal_error(err))?
            .0)
    }
