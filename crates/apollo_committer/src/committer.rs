...
fn update_metrics(
    ...
    let n_modifications = modifications_counts.storage_tries
...
fn log_block_measurements(
    ...
    info!(
        "Block {height} stats: durations in ms (total/read/compute/write): \
         {:.0}/{:.0}/{:.0}/{:.0}, total block duration per modification: {:.2}s, rates \
        ...
        total_block_duration_per_modification.map_or(String::new(), |d| format!("{d:.2}")),
