...
pub fn init_apollo_committer_storage() -> PatriciaStorageResult<ApolloStorage> {
    ...
    // TODO(Ariel): take path from the committer config.
    let rocksdb_storage =
        RocksDbStorage::open(Path::new("apollo_committer_storage"), RocksDbOptions::default())?;
    ...
    Committer<ForestDB, BlockCommitter>
{
    pub async fn new(
        config: CommitterConfig,
        init_storage: impl FnOnce() -> PatriciaStorageResult<ForestDB::Storage>,
