...
/// The negative tests below all start from a valid proof / proof-facts pair and apply one small
/// mutation, asserting that verification then fails. They share the newest pinned fixture via
/// [`FIXTURE_PROOF_VERSION`].
///
/// An empty proof payload is rejected before the verifier runs.
#[test]
fn verify_proof_rejects_empty_proof() {
    let (proof_facts, _compressed_proof) = load_proof_fixture(FIXTURE_PROOF_VERSION);
...
fn verify_proof_rejects_proof_facts_too_short() {
    let (_proof_facts, compressed_proof) = load_proof_fixture(FIXTURE_PROOF_VERSION);
...
/// A version marker that is neither V0 nor V1 is rejected.
#[test]
fn verify_proof_rejects_unsupported_proof_version() {
    let (proof_facts, compressed_proof) = load_proof_fixture(FIXTURE_PROOF_VERSION);
    let unsupported_version = Felt::from(0xDEAD_u64);
    let mut tampered_facts = (*proof_facts.0).clone();
    tampered_facts[0] = unsupported_version;
    let result = verify_proof(ProofFacts(Arc::new(tampered_facts)), Proof::from(compressed_proof));
    assert_eq!(result, Err(VerifyProofError::InvalidProofVersion { actual: unsupported_version }));
}

/// V0 proofs are no longer verifiable (the v0 circuit was removed), so a V0 version marker must be
/// rejected up front rather than dispatched to a verifier.
#[test]
fn verify_proof_rejects_v0_proof_version() {
    let (proof_facts, compressed_proof) = load_proof_fixture(FIXTURE_PROOF_VERSION);
    let mut tampered_facts = (*proof_facts.0).clone();
    tampered_facts[0] = PROOF_VERSION_V0;
    let result = verify_proof(ProofFacts(Arc::new(tampered_facts)), Proof::from(compressed_proof));
    assert_eq!(result, Err(VerifyProofError::InvalidProofVersion { actual: PROOF_VERSION_V0 }));
}
...
fn verify_proof_rejects_corrupted_compressed_proof() {
    let (proof_facts, mut compressed_proof) = load_proof_fixture(FIXTURE_PROOF_VERSION);
