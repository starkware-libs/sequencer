...
/// Splits a wire-format proof into its header prefix (proof version and circuit preprocessed root)
/// and the zstd-compressed payload, then decompresses the payload. Mirrors the verifier's own
/// framing, so the negative tests below mutate the proof it actually deserializes.
