...
fn test_flow_commit_block_rewinds_queued_nonce(mut mempool: Mempool) {
    ...
    let nonces = [("0x0", 2)];
    let tx_hashes = [1];
    // tx_nonce_2 was accepted, but 3 and 4 were not, so are rewound.
    commit_block(&mut mempool, nonces, tx_hashes);

    // nonce_3 and nonce_4 were re-enqueued correctly.
    get_txs_and_assert_expected(&mut mempool, 1, &[tx_nonce_3.tx]);
