...
#[rstest]
fn test_flow_update_gas_price_threshold(mut mempool: Mempool) {
    // Setup.
    let input_tip_100_gas_price_20 =
        add_tx_input!(tx_hash: 1, address: "0x0", tip: 100, max_l2_gas_price: 20);
    let input_tip_50_gas_price_30 =
        add_tx_input!(tx_hash: 2, address: "0x1", tip: 50, max_l2_gas_price: 30);

    for input in [&input_tip_100_gas_price_20, &input_tip_50_gas_price_30] {
        add_tx(&mut mempool, input);
    }
    ...
    get_txs_and_assert_expected(&mut mempool, 2, &[input_tip_100_gas_price_20.tx]);
}
