...
fn test_flow_commit_block_from_different_leader(mut mempool: Mempool) {
    ...
    let nonces = [("0x0", 3)];
    let tx_hashes = [
        1,  // Known hash accepted for nonce 2.
        99, // Unknown hash accepted for nonce 3.
    ];
    commit_block(&mut mempool, nonces, tx_hashes);

    // Assert: two stale txs were removed, one was added to a block by a different leader and
    // the other "lost" to a different tx with the same nonce that was added by the different
    // leader.
