...
fn test_update_gas_price_threshold_increases_threshold() {
    // Setup.
    let low_gas_tx =
        TransactionReference::new(&tx!(tx_hash: 0, address: "0x0", max_l2_gas_price: 100));
    let high_gas_tx =
        TransactionReference::new(&tx!(tx_hash: 1, address: "0x1", max_l2_gas_price: 150));
...
fn test_update_gas_price_threshold_decreases_threshold() {
    // Setup.
    let low_gas_tx =
        TransactionReference::new(&tx!(tx_hash: 0, address: "0x0", max_l2_gas_price: 88));
    let high_gas_tx =
        TransactionReference::new(&tx!(tx_hash: 1, address: "0x1", max_l2_gas_price: 99));
    let mut mempool: Mempool = MempoolContentBuilder::new()
