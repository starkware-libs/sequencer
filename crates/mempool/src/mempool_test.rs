...
use mempool_test_utils::starknet_api_test_utils::{
    create_resource_bounds_with_l2_max_price_per_unit,
...
#[rstest]
fn test_add_tx_multi_nonce_success() {
    // Setup.
    let input_priority_tx_address_0_nonce_0 =
        add_tx_input!(tx_hash: 1, sender_address: "0x0", tx_nonce: 0, account_nonce: 0);
    let input_priority_tx_address_0_nonce_1 =
        add_tx_input!(tx_hash: 3, sender_address: "0x0", tx_nonce: 1, account_nonce: 0);
    let input_pending_tx_address_1_nonce_0 = add_tx_input!(tx_hash: 2, sender_address: "0x1", tx_nonce: 0,account_nonce: 0, resource_bounds: create_resource_bounds_with_l2_max_price_per_unit(VALID_L2_GAS_MAX_PRICE_PER_UNIT - 1));
    ...
    // Test.
    for input in [
        &input_priority_tx_address_0_nonce_0,
        &input_pending_tx_address_1_nonce_0,
        &input_priority_tx_address_0_nonce_1,
    ] {
        add_tx(&mut mempool, input);
    }
    ...
    let expected_priority_queue_txs =
        [TransactionReference::new(&input_priority_tx_address_0_nonce_0.tx)];
    let expected_pending_queue_txs =
        [TransactionReference::new(&input_pending_tx_address_1_nonce_0.tx)];
    let expected_pool_txs = [
        input_priority_tx_address_0_nonce_0.tx,
        input_pending_tx_address_1_nonce_0.tx,
        input_priority_tx_address_0_nonce_1.tx,
    ];
    let expected_mempool_content = MempoolContentBuilder::new()
        .with_pool(expected_pool_txs)
