...
impl MempoolContent {
    ...
    fn _with_pool_and_account_nonces<P, A>(pool_txs: P, account_nonce_pairs: A) -> Self
    where
        P: IntoIterator<Item = Transaction>,
...
#[rstest]
fn test_get_txs_with_different_l2_gas_prices() {
    // Setup.
    ...
    let priority_input_tx_nonce_0 =
        add_tx_input!(tx_hash: 0_u8, tx_nonce: 0_u8, account_nonce: 0_u8);
    let priority_input_tx_nonce_1 =
        add_tx_input!(tx_hash: 1_u8, tx_nonce: 1_u8, account_nonce: 0_u8);
    let pending_input_tx_nonce_2 = add_tx_input!(tx_hash: 2_u8, tx_nonce: 2_u8, account_nonce: 0_u8, resource_bounds: ValidResourceBounds::AllResources(resource_bounds_below_threshold));

    let queue_txs = [TransactionReference::new(&priority_input_tx_nonce_0.tx)];
    let pool_txs = [
        priority_input_tx_nonce_0.tx.clone(),
        priority_input_tx_nonce_1.tx.clone(),
        pending_input_tx_nonce_2.tx,
    ];
    let mut mempool: Mempool = MempoolContent::with_pool_and_queue(pool_txs, queue_txs).into();
    mempool._update_gas_price_threshold(VALID_L2_GAS_MAX_PRICE_PER_UNIT);

    // Test.
    // Ensures that `get_txs` returns only the priority transactions.
    // Verifies that the recursive `get_txs` inserts transactions to the correct queue.
    let txs = mempool.get_txs(3).unwrap();

    // Assert.
    assert_eq!(txs, &[priority_input_tx_nonce_0.tx, priority_input_tx_nonce_1.tx]);
    // TODO(Mohammad): test the two queues status using mempool content.
}
