...
impl MempoolContent {
    ...
    fn _assert_eq_transaction_queues_content(&self, mempool: &Mempool) {
        self.tx_queue.as_ref().unwrap()._assert_eq_priority_and_pending_queues(&mempool.tx_queue);
    }
...
impl MempoolContentBuilder {
    ...
    {
        self.tx_queue =
            Some(TransactionQueueContentBuilder::new().with_priority(queue_txs).build());
        self
    }
    ...
    fn _gas_price_threshold(mut self, gas_price_threshold: u128) -> Self {
        self.tx_queue = Some(
            TransactionQueueContentBuilder::new()._gas_price_threshold(gas_price_threshold).build(),
        );
        self
    }
...
macro_rules! add_tx_input {
    ...
    () => {
        add_tx_input!(tip: 1, tx_hash: 0_u8, sender_address: "0x0", tx_nonce: 0_u8, account_nonce: 0_u8)
    };
}
