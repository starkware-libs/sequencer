...
fn test_fee_escalation() {
    ...
    // Setup.
    let input = add_tx_input!(tx_hash: 1, tip: 1, max_l2_gas_price: 1);
    let valid_replacement_input = add_tx_input!(tx_hash: 2, tip: 10, max_l2_gas_price: 10);
    ...
    // Setup.
    let input_not_enough_tip = add_tx_input!(tx_hash: 3, tip: 11, max_l2_gas_price: 100);
    let input_not_enough_gas_price = add_tx_input!(tx_hash: 4, tip: 100, max_l2_gas_price: 11);
    let input_not_enough_both = add_tx_input!(tx_hash: 5, tip: 11, max_l2_gas_price: 11);
    ...
    // Assert: transaction indeed was not replaced.
    let expected_mempool_content =
