...
impl MempoolContent {
    fn with_pool_and_queues<P, Q, R>(pool_txs: P, priority_txs: Q, pending_txs: R) -> Self
    where
        ...
        R: IntoIterator<Item = TransactionReference>,
    {
        Self {
            tx_pool: Some(pool_txs.into_iter().collect()),
            tx_queue: Some(
                TransactionQueueContentBuilder::new()
                    .with_priority(priority_txs)
                    .with_pending(pending_txs)
                    .build(),
            ),
            ..Self::default()
        }
    ...
    fn with_pool_and_ready_queue<P, Q, R>(pool_txs: P, priority_txs: Q) -> Self
    where
        P: IntoIterator<Item = Transaction>,
        // TODO(Ayelet): Consider using `&ThinTransaction` instead of `TransactionReference`.
        Q: IntoIterator<Item = TransactionReference>,
    {
        Self {
            tx_pool: Some(pool_txs.into_iter().collect()),
            tx_queue: Some(
                TransactionQueueContentBuilder::new().with_priority(priority_txs).build(),
            ),
            ..Self::default()
        }
    }

    fn with_pool_and_account_nonces<P, A>(pool_txs: P, account_nonce_pairs: A) -> Self
    where
        P: IntoIterator<Item = Transaction>,
    ...
    fn with_queue<Q, P>(priority_txs: Q, pending_txs: P) -> Self
    where
        Q: IntoIterator<Item = TransactionReference>,
    ...
    fn with_account_nonces<A>(account_nonce_pairs: A) -> Self
    where
        A: IntoIterator<Item = (ContractAddress, Nonce)>,
    {
    ...
    fn assert_eq_transaction_queue_content(&self, mempool: &Mempool) {
        self.tx_queue.as_ref().unwrap().assert_eq_priority_and_pending_content(&mempool.tx_queue);
    }
...
fn test_get_txs_returns_by_priority_order(#[case] requested_txs: usize) {
    ...
    let mut mempool: Mempool =
        MempoolContent::with_pool_and_ready_queue(txs_iterator, tx_references_iterator).into();
    ...
    let mempool_content =
        MempoolContent::with_pool_and_ready_queue(remaining_txs.to_vec(), remaining_tx_references);
...
fn test_get_txs_multi_nonce() {
    ...
    let mut mempool: Mempool =
        MempoolContent::with_pool_and_ready_queue(pool_txs.clone(), queue_txs).into();
    ...
    let expected_mempool_content = MempoolContent::with_pool_and_ready_queue([], []);
...
fn test_get_txs_replenishes_queue_only_between_chunks() {
    ...
    let mut mempool: Mempool =
        MempoolContent::with_pool_and_ready_queue(pool_txs, queue_txs).into();
...
fn test_get_txs_replenishes_queue_multi_account_between_chunks() {
    ...
    let expected_mempool_content =
        MempoolContent::with_pool_and_ready_queue(expected_pool_txs, expected_queue_txs);
    expected_mempool_content.assert_eq_pool_and_queue_content(&mempool);
}
...
fn test_get_txs_with_holes_single_account() {
    ...
    let expected_mempool_content = MempoolContent::with_pool_and_ready_queue(pool_txs, queue_txs);
    expected_mempool_content.assert_eq_pool_and_queue_content(&mempool);
}
...
fn test_commit_block_rewinds_nonce() {
    ...
    let mut mempool: Mempool =
        MempoolContent::with_pool_and_ready_queue(pool_txs, queued_txs).into();
...
fn test_account_nonce_does_not_decrease_in_add_tx() {
    ...
    let mut mempool: Mempool = MempoolContent::with_account_nonces(account_nonces).into();
    ...
    let expected_mempool_content = MempoolContent::with_account_nonces(account_nonces);
    expected_mempool_content.assert_eq_account_nonces(&mempool);
}
...
fn test_account_nonces_update_in_commit_block() {
    ...
    let mut mempool: Mempool =
        MempoolContent::with_pool_and_account_nonces(pool_txs, account_nonces).into();
...
fn test_flow_partial_commit_block() {
    ...
    let expected_mempool_content = MempoolContent::with_pool_and_ready_queue(expected_pool_txs, []);
    expected_mempool_content.assert_eq_pool_and_queue_content(&mempool);
}
