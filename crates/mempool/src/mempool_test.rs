...
#[rstest]
fn test_account_nonces_flow(mut mempool: Mempool) {
    // Setup.
    let input =
        add_tx_input!(tx_hash: 1, sender_address: "0x0", tx_nonce: 3_u8, account_nonce: 0_u8);
...
fn test_account_nonce_does_not_decrease() {
    // Setup.
    let input_account_nonce_3 =
        add_tx_input!(tx_hash: 1, sender_address: "0x0", tx_nonce: 4_u8, account_nonce: 3_u8);
    let input_account_nonce_2 =
        add_tx_input!(tx_hash: 2, sender_address: "0x0", tx_nonce: 3_u8, account_nonce: 2_u8);
    let address = input_account_nonce_3.account.sender_address;

    let account_nonces = vec![(address, input_account_nonce_3.account.state.nonce)];
...
fn test_account_nonces_removal_in_get_txs(mut mempool: Mempool) {
    ...
    // Test.
    // TODO(Ayelet): Consider adding full content instead of using add tx.
    add_tx(&mut mempool, &input);
    mempool.get_txs(1).unwrap();
