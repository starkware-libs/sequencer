...
fn test_tx_pool_capacity(mut mempool: Mempool) {
    ...
    assert_matches!(mempool.add_tx(input_1), Err(MempoolError::DuplicateTransaction { .. }));
    assert_eq!(mempool._tx_pool().n_txs(), 2);
...
#[rstest]
fn test_tx_pool_capacity_in_commit_block() {
    // Setup.
    let input =
        add_tx_input!(tx_hash: 0, sender_address: 0_u8, tx_nonce: 0_u8, account_nonce: 0_u8);
    let pool_txs = [input.tx];
    let mut mempool: Mempool = MempoolContent::with_pool(pool_txs).into();
