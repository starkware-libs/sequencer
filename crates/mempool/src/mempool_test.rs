...
impl MempoolContent {
    fn with_pool_and_queue<P, Q, R>(pool_txs: P, priority_txs: Q, pending_txs: R) -> Self
    where
        ...
        R: IntoIterator<Item = TransactionReference>,
    {
        Self {
            ...
            tx_queue: Some(TransactionQueueContent::_with_priority_and_pending(
                priority_txs,
                pending_txs,
            )),
            ..Self::default()
    ...
    fn assert_eq_transaction_queue_content(&self, mempool: &Mempool) {
        self.tx_queue.as_ref().unwrap()._assert_eq_priority_and_pending_content(&mempool.tx_queue);
    }
