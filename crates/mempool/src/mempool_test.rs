...
impl MempoolContent {
    ...
    fn _with_two_queues<Q>(priority_queue_txs: Q, pending_queue_txs: Q) -> Self
    where
        Q: IntoIterator<Item = TransactionReference>,
    {
        Self {
            tx_queue: Some(TransactionQueue::_new_with_transactions(
                priority_queue_txs.into_iter().collect(),
                pending_queue_txs.into_iter().collect(),
            )),
            ..Self::default()
        }
    }
