...
fn test_add_same_nonce_tx_after_previous_not_included_in_block(mut mempool: Mempool) {
    ...
    let tx_input_nonce_5 =
        add_tx_input!(tx_hash: 3, sender_address: "0x0", tx_nonce: 5, account_nonce: 3);

    for input in [&tx_input_nonce_3, &tx_input_nonce_4, &tx_input_nonce_5] {
        add_tx(&mut mempool, input);
    }

    // Test.
    let expected_txs = [&tx_input_nonce_3, &tx_input_nonce_4].map(|input| input.tx.clone());
    get_txs_and_assert_expected(&mut mempool, 2, &expected_txs);
    ...
    get_txs_and_assert_expected(&mut mempool, 2, &[tx_input_nonce_4.tx, tx_input_nonce_5.tx]);
}
