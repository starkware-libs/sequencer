...
fn test_get_txs_returns_by_priority_order(#[case] n_requested_txs: usize) {
    // Setup.
    let tx_tip_20_hash_1 = tx!(tx_hash: 1, address: "0x0", tip: 20);
    let tx_tip_30_hash_2 = tx!(tx_hash: 2, address: "0x1", tip: 30);
    let tx_tip_10_hash_3 = tx!(tx_hash: 3, address: "0x2", tip: 10);

    let queue_txs =
        [&tx_tip_20_hash_1, &tx_tip_30_hash_2, &tx_tip_10_hash_3].map(TransactionReference::new);
    let pool_txs = [&tx_tip_20_hash_1, &tx_tip_30_hash_2, &tx_tip_10_hash_3].map(|tx| tx.clone());
    ...
    let sorted_txs = [tx_tip_30_hash_2, tx_tip_20_hash_1, tx_tip_10_hash_3];
