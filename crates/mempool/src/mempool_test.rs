...
impl MempoolContent {
    ...
    fn assert_eq_transaction_queue_content(&self, mempool: &Mempool) {
        assert_eq!(
            self.tx_queue.as_ref().unwrap()._get_address_to_tx_mapping(),
            mempool.tx_queue._get_address_to_tx_mapping()
        );
        assert_eq!(
            self.tx_queue.as_ref().unwrap()._get_priority_txs(),
            mempool.tx_queue._get_priority_txs()
        );
        assert_eq!(
            self.tx_queue.as_ref().unwrap()._get_pending_txs(),
            mempool.tx_queue._get_pending_txs()
        );
    }
