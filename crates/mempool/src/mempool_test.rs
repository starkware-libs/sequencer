...
fn test_account_nonces(mut mempool: Mempool) {
    ...
    let input =
        add_tx_input!(tx_hash: 1, sender_address: "0x0", tx_nonce: 0_u8, account_nonce: 0_u8);
    let address = input.tx.sender_address;
    ...
    // Assert the account nonce is updated after adding a transaction.
    expected_mempool.assert_eq_account_nonces(&mempool);

    // Test.
    let nonce_committed = Nonce(felt!(0_u16));
    let state_changes = HashMap::from([(address, AccountState { nonce: nonce_committed })]);
    assert!(mempool.commit_block(state_changes).is_ok());

    // Assert the account nonce is updated after committing a block.
    let nonce_after_commit = nonce_committed.try_increment().unwrap();
    let expected_mempool = MempoolContent::with_account_nonces(vec![(address, nonce_after_commit)]);
    expected_mempool.assert_eq_account_nonces(&mempool);
}
