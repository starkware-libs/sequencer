...
fn commit_block(
    ...
    let state_changes = HashMap::from_iter(
        state_changes
            .into_iter()
            .map(|(address, nonce)| (contract_address!(address), Nonce(felt!(nonce)))),
    );
...
fn test_account_nonces_update_in_add_tx(mut mempool: Mempool) {
    ...
    // Assert.
    let expected_mempool_content = MempoolContentBuilder::new()
        .with_account_nonces([(input.account.sender_address, input.account.nonce)])
        .build();
...
fn test_account_nonce_does_not_decrease_in_commit_block() {
    ...
    let state_changes = HashMap::from([(sender_address, Nonce(felt!(0_u8)))]);
    assert_eq!(mempool.commit_block(state_changes), Ok(()));
