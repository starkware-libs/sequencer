...
impl MempoolContentBuilder {
    ...
    pub fn with_pool<P>(mut self, pool_txs: P) -> Self
    where
    ...
    pub fn with_queue<Q>(mut self, queue_txs: Q) -> Self
    where
        // TODO(Ayelet): Consider using `&Transaction` instead of `TransactionReference`.
    ...
    pub fn with_account_nonces<A>(mut self, account_nonce_pairs: A) -> Self
    where
    ...
    pub fn build(self) -> MempoolContent {
        MempoolContent {
...
fn test_add_tx(mut mempool: Mempool) {
    ...
    let mut add_tx_inputs = [
        add_tx_input!(tip: 50, tx_hash: 1, sender_address: "0x0", tx_nonce: 0_u8, account_nonce: 0_u8),
        add_tx_input!(tip: 100, tx_hash: 2, sender_address: "0x1", tx_nonce: 0_u8, account_nonce: 0_u8),
        add_tx_input!(tip: 80, tx_hash: 3, sender_address: "0x2", tx_nonce: 0_u8, account_nonce: 0_u8),
    ];
    ...
    let account_nonce_pairs: Vec<(ContractAddress, Nonce)> = add_tx_inputs
        .iter()
        .map(|input| (input.account.sender_address, input.account.state.nonce))
        .collect();
...
fn test_add_tx_lower_than_queued_nonce() {
    ...
    let expected_mempool_content =
        MempoolContentBuilder::new().with_queue(queue_txs.clone()).build();
