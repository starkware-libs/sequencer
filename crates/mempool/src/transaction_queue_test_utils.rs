...
impl TransactionQueueContent {
    pub fn assert_eq(&self, tx_queue: &TransactionQueue) {
        if let Some(priority_queue) = &self.priority_queue {
            let tx_priority_queue: Vec<_> = tx_queue.priority_queue.iter().rev().copied().collect();
            assert!(priority_queue.iter().copied().eq(tx_priority_queue));
        }

        if let Some(pending_queue) = &self.pending_queue {
            let tx_pending_queue: Vec<_> = tx_queue.pending_queue.iter().rev().copied().collect();
            assert!(pending_queue.iter().copied().eq(tx_pending_queue));
        }
    }
