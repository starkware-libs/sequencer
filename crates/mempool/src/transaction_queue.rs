...
impl TransactionQueue {
    ...
    #[cfg(test)]
    pub fn _get_priority_txs(&self) -> Vec<&TransactionReference> {
        self.priority_queue.iter().map(|tx| &tx.0).collect()
    }

    #[cfg(test)]
    pub fn _get_pending_txs(&self) -> Vec<&TransactionReference> {
        self.pending_queue.iter().map(|tx| &tx.0).collect()
    }

    #[cfg(test)]
    pub fn _get_address_to_tx_mapping(&self) -> &HashMap<ContractAddress, TransactionReference> {
        &self.address_to_tx
    }
}
