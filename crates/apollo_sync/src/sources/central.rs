...
use apollo_client::reader::{
    BlockSignatureData,
    ReaderClientError,
    StarknetFeederGatewayClient,
    StarknetReader,
};
use apollo_client::{ClientCreationError, RetryConfig};
...
pub struct GenericCentralSource<TStarknetClient: StarknetReader + Send + Sync> {
    pub concurrent_requests: usize,
    pub apollo_client: Arc<TStarknetClient>,
    ...
    for GenericCentralSource<TStarknetClient>
{
    ...
    async fn get_latest_block(&self) -> Result<Option<BlockHashAndNumber>, CentralError> {
        self.apollo_client.latest_block().await.map_err(Arc::new)?.map_or(Ok(None), |block| {
                ...
                futures_util::stream::iter(initial_block_number.iter_up_to(up_to_block_number))
                    .map(|bn| async move {
                        let block_and_signature = futures_util::try_join!(
                            self.apollo_client.block(bn),
                            self.apollo_client.block_signature(bn)
                        );
    ...
    async fn get_class(&self, class_hash: ClassHash) -> Result<ApiContractClass, CentralError> {
        ...
        let client_class = self.apollo_client.class_by_hash(class_hash).await.map_err(Arc::new)?;
        match client_class {
    ...
    async fn get_compiled_class(
        ...
        match self.apollo_client.compiled_class_by_hash(class_hash).await {
    ...
    async fn get_sequencer_pub_key(&self) -> Result<SequencerPublicKey, CentralError> {
        Ok(self.apollo_client.sequencer_pub_key().await.map_err(Arc::new)?)
    }
}
...
fn client_to_central_block(
    ...
    maybe_client_block: Result<
        (Option<apollo_client::reader::Block>, Option<apollo_client::reader::BlockSignatureData>),
        ReaderClientError,
    >,
...
impl CentralSource {
    pub fn new(
        ...
        Ok(CentralSource {
            ...
            apollo_client: Arc::new(apollo_client),
            storage_reader,
