...
async fn test_update_timestamps_success() {
    ...
    let _http_test_client =
        http_client_server_setup(mock_gateway_client, http_server_config.clone()).await;
...
async fn test_update_timestamps_gateway_error() {
    ...
    mock_gateway_client.expect_update_timestamps().times(1).returning(|_| {
