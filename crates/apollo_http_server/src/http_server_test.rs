...
async fn record_region_test(#[case] index: u16, #[case] tx: impl GatewayTransaction + Clone) {
...
#[case::add_rpc_tx(1, rpc_tx())]
#[tokio::test]
async fn test_response(#[case] index: u16, #[case] tx: impl GatewayTransaction + Clone) {
    let mut mock_gateway_client = MockGatewayClient::new();
    ...
    let panicking_task = AssertUnwindSafe(add_tx_http_client.assert_add_tx_success(tx));
