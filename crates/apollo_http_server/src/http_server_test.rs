...
async fn test_tx_version(
    ...
    let mut tx_json =
        TransactionSerialization(serde_json::to_value(deprecated_gateway_tx()).unwrap());
    ...
    match expected {
        Ok(expected_tx_hash) => {
            let tx_hash = http_client.assert_add_tx_success(tx_json).await;
            assert_eq!(expected_tx_hash, tx_hash);
        }
        Err(expected_err) => {
            let serialized_err =
                http_client.assert_add_tx_error(tx_json, StatusCode::BAD_REQUEST).await;
