...
async fn add_tx_inner(
    ...
    // Bound the number of concurrent in-flight requests. Acquire the permit before spawning so
    // that, under flood, excess requests are rejected with 503 rather than piling up as detached
    // tasks that each retain a multi-MB payload while blocked on the gateway channel.
