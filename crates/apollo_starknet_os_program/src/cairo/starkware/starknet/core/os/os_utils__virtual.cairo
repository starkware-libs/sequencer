...
// Recursively hashes each L2-to-L1 message separately and outputs them.
func hash_messages_to_l1_recursive{poseidon_ptr: PoseidonBuiltin*}(
    messages_ptr_start: MessageToL1Header*,
    messages_ptr_end: MessageToL1Header*,
    messages_to_l1_hashes_ptr: felt*,
) -> (messages_to_l1_hashes_ptr: felt*) {
    alloc_locals;

    // Base case: if we've processed all messages, return.
    if (messages_ptr_start == messages_ptr_end) {
        return (messages_to_l1_hashes_ptr=messages_to_l1_hashes_ptr);
    }
    ...
    // Store the hash.
    assert [messages_to_l1_hashes_ptr] = message_hash;
    let messages_to_l1_hashes_ptr = messages_to_l1_hashes_ptr + 1;
    ...
    return hash_messages_to_l1_recursive(
        messages_ptr_start=next_message_ptr,
        messages_ptr_end=messages_ptr_end,
        messages_to_l1_hashes_ptr=messages_to_l1_hashes_ptr,
    );
}
...
func process_os_output{
    ...
    let messages_to_l1_hashes_ptr_start: felt* = &output_header_placeholder[
        VirtualOsOutputHeader.SIZE
    ];

    let (messages_to_l1_hashes_ptr_end) = hash_messages_to_l1_recursive(
        ...
        messages_to_l1_hashes_ptr=messages_to_l1_hashes_ptr_start,
    );

    // Calculate the number of messages from the pointer difference.
    let n_messages_to_l1 = messages_to_l1_hashes_ptr_end - messages_to_l1_hashes_ptr_start;
    ...
    // Update the output pointer to point after the header and all message hashes.
    let output_ptr = messages_to_l1_hashes_ptr_end;
