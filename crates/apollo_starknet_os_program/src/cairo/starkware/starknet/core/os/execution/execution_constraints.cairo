...
func check_is_reverted(is_reverted: felt) { return (); }
// Validates that the proof facts of an invoke transaction represent valid server side virtual OS proof facts.
func check_proof_facts{range_check_ptr, contract_state_changes: DictAccess*}(
    ...
    // validate that the current block number is not too recent.
    ...
    // validate that the proof facts block hash is connected to the proof facts block number.
