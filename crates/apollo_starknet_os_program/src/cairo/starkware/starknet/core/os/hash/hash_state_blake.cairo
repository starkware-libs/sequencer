...
func blake_hash_update_with_nested_hash{hash_state: HashState}(data_ptr: felt*, data_length: felt) {
    let (hash) = encode_felt252_data_and_calc_blake_hash(data_len=data_length, data=data_ptr);
    hash_update_single(item=hash);
    return ();
}

func blake_hash_finalize{hash_state: HashState}(hash_state: HashState) -> felt {
    let (hash) = encode_felt252_data_and_calc_blake_hash(data_len=data_length, data=data_ptr);
    return (hash);
}