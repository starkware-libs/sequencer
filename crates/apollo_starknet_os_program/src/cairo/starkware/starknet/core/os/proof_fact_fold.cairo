...
from starkware.cairo.common.math import assert_nn_le, assert_not_zero, unsigned_div_rem
...
struct ProofFactsReference {
    ...
    proof_facts: felt*,
    // The index of the leaf cairo-verifier circuit that proved this transaction, in the
    // allowed-circuits table (see `get_leaf_verifier_circuit_hash`).
    leaf_circuit_index: felt,
}

// Records a reference to an executed transaction's proof facts, to be folded at the end
// of the block; no-op for a transaction with empty proof facts. `leaf_circuit_index` is
// the transaction's index into the allowed leaf cairo-verifier circuits table (see
// `get_leaf_verifier_circuit_hash`, which range-checks it at fold time).
...
func build_leaf_entries{range_check_ptr}(
    ...
    let (leaf_verifier_circuit_hash) = get_leaf_verifier_circuit_hash(
        leaf_circuit_index=proof_facts_references.leaf_circuit_index
    );
