...
func execute_blocks{
    ...
    // TODO(Meshi): Remove the python implementation once the OS test no longer support the Python VM.
    local n_classes = nondet %{ len(block_input.class_hashes_to_migrate) %};
    // TODO(Meshi): Note the rust implementation and the python one are different.
    // Remove the python implementation once the OS test no longer support the Python VM.
    %{
        vm_enter_scope(dict(
            class_hashes_to_migrate=iter(block_input.class_hashes_to_migrate.keys()),
...
// Migrates contract classes from V1 (Poseidon) Casm hash to V2 (Blake).
// The classes are guessed, and should at least cover the classes that will be executed by the block.
func migrate_classes_to_v2_casm_hash{poseidon_ptr: PoseidonBuiltin*, range_check_ptr}(
