...
func bytecode_hash_internal_node{range_check_ptr, hash_state: HashState}(
    ...
    // is_segment_used is not zero if (full_contract || is used) is true.
    tempvar is_segment_used = full_contract + is_segment_used;
    // is_used_leaf is not zero if (is_leaf && is_segment_used) is true.
