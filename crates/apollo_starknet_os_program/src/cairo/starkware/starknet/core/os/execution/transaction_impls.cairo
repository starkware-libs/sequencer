...
func execute_invoke_function_transaction{
    ...
    // The leaf circuit index is 0 while the allowed-circuits table has a single (test
    // registry) entry; with the production registry it becomes per-transaction OS input,
    // copied from the transaction proof's header.
    record_proof_facts_reference(
        proof_facts_size=proof_facts_size, proof_facts=proof_facts, leaf_circuit_index=0
    );
