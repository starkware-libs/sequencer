...
func encrypt_symmetric_key{range_check_ptr, ec_op_ptr: EcOpBuiltin*}(
    ...
    symmetric_key: felt,
    symmetric_key_encryptions: felt*,
) {
    if (n_keys == 0) {
        return ();
    }
    ...
    assert symmetric_key_encryptions[0] = symmetric_key + encode_felt252_data_and_calc_blake_hash(
        data_len=1, data=&shared_secret.x
    );
    return encrypt_symmetric_key(
        ...
        symmetric_key_encryptions=&symmetric_key_encryptions[1],
    );
}
