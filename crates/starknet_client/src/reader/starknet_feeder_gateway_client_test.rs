...
fn get_latest_block_url() -> &'static str {
...
async fn get_block_number() {
    ...
    let mock_block = mock("GET", get_latest_block_url())
        .with_status(200)
    ...
    let mock_no_block =
        mock("GET", get_latest_block_url()).with_status(400).with_body(body).create();
    let latest_block = starknet_client.latest_block().await.unwrap();
...
async fn latest_block_unserializable() {
    test_unserializable(get_latest_block_url(), |starknet_client| async move {
