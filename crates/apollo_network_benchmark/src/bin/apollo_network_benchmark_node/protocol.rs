...
lazy_static::lazy_static! {
    pub static ref STRESS_TEST_TOPIC: Sha256Topic = Topic::new("stress_test_topic".to_string());
}
...
/// Per-iteration timeout used to drain pending ReversedSqmr queries without blocking the
/// broadcaster. Short enough to behave like a non-blocking check yet long enough to absorb
/// scheduler jitter on a busy runtime.
const REVERSED_SQMR_QUERY_DRAIN_TIMEOUT: Duration = Duration::from_millis(1);
    ...
    match protocol {
        NetworkProtocol::Gossipsub => {
            let channels = network_manager
                .register_broadcast_topic::<Vec<u8>>(STRESS_TEST_TOPIC.clone(), buffer_size)
                .expect(
...
impl ReversedSqmrSender {
    ...
    async fn collect_new_queries(&mut self) {
        // Drain any pending queries so the broadcaster always operates on the most recent
        // one. The per-iteration timeout is short enough to behave like a non-blocking
        // check; see `REVERSED_SQMR_QUERY_DRAIN_TIMEOUT`.
        while let Ok(query) =
            tokio::time::timeout(REVERSED_SQMR_QUERY_DRAIN_TIMEOUT, self.server.next()).await
        {
...
impl MessageReceiver {
    /// Consumes this receiver and runs `handle_received_message` once per inbound payload.
    /// Loops forever under normal operation and only returns when the underlying network
    /// stream ends or the surrounding task is aborted at test teardown.
    pub async fn for_each<F>(self, mut handle_received_message: F)
    where
            ...
            MessageReceiver::Gossipsub(receiver) => {
                receiver
                    .for_each(|message| async move {
                        ...
                        handle_received_message(payload, Some(peer_id));
                    })
                    .await
            }
                ...
                receiver
                    .for_each(|server_query| async move {
                        match server_query.query() {
                            Ok(payload) => handle_received_message(payload.to_vec(), None),
                            Err(parse_error) => {
                    ...
                    Ok(mut response_manager) => loop {
                        match response_manager.next().await {
                            Some(Ok(response_data)) => {
                                handle_received_message(response_data, None);
                            }
