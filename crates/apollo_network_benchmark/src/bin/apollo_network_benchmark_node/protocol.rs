...
/// Short enough that draining pending queries doesn't block the broadcaster, but long
/// enough to absorb scheduler jitter on a busy runtime.
...
/// The inner `Vec<u8>` is the wire payload — a serialized
/// [`crate::message::StressTestMessage`].
...
/// Reversed-SQMR sender role: receivers initiate the query; this side keeps the most
/// recent incoming query and pushes broadcasts to it as responses.
...
impl ReversedSqmrSender {
    ...
    async fn collect_new_queries(&mut self) {
        // Drain pending queries so the broadcaster always operates on the most recent one.
        while let Ok(query) =
...
impl MessageReceiver {
    /// Loops forever under normal operation; only returns when the underlying stream ends
    /// or the surrounding task is aborted at test teardown.
