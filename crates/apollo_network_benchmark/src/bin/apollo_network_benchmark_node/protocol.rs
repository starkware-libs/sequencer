...
const SQMR_PROTOCOL_NAME: &str = "/stress-test/1.0.0";
    ...
    async fn collect_new_queries(&mut self) {
            ...
            tokio::time::timeout(Duration::from_millis(1), self.server.next()).await
        {
            if let Some(query) = query {
                info!("ReversedSqmr: Received new query, replacing previous query");
        ...
        match self {
            MessageSender::Gossipsub(client) => {
                if let Err(broadcast_error) = client.broadcast_message(message).await {
                    error!("Gossipsub broadcast failed: {:?}", broadcast_error);
                }
            }
            MessageSender::Sqmr(client) => match client.send_new_query(message).await {
                ...
                Err(query_error) => {
                    error!("Failed to send SQMR query: {:?}", query_error);
                }
            },
                ...
                receiver
                    .for_each(|server_query| async move {
                        match server_query.query() {
                            ...
                            Err(parse_error) => {
                                error!("SQMR: failed to parse query: {:?}", parse_error);
                            }
                        }
                    })
                    .await
                    ...
                    Ok(mut response_manager) => loop {
                        match response_manager.next().await {
                            ...
                            Some(Err(parse_error)) => {
                                error!("ReversedSqmr: failed to parse response: {:?}", parse_error);
                                break;
                            }
                            None => {
                                error!("ReversedSqmr: response stream ended");
                                break;
                            }
                        }
                    },
