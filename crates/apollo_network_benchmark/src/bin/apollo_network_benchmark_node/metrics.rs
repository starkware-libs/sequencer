...
use crate::protocol::STRESS_TEST_TOPIC;

// Network metrics (NETWORK_*, PING_LATENCY_SECONDS) are registered by `NetworkManager::run`,
// so they don't appear here.
pub(crate) fn register_metrics() {
    BROADCAST_MESSAGE_THEORETICAL_HEARTBEAT_MILLIS.register();
    BROADCAST_MESSAGE_THEORETICAL_THROUGHPUT.register();
...
pub fn get_throughput(message_size_bytes: usize, heartbeat_interval: Duration) -> f64 {
    let tps = Duration::from_secs(1).as_secs_f64() / heartbeat_interval.as_secs_f64();
...
pub fn seconds_since_epoch() -> u64 {
    ...
    now.duration_since(UNIX_EPOCH)
        .expect("system time is after UNIX_EPOCH on any reasonable host")
        .as_secs()
}

/// Wires up the apollo_network metric hooks so the library reports connection, session, and
/// event counters through this crate's metric registry.
pub fn create_network_metrics() -> apollo_network::metrics::NetworkMetrics {
    ...
    broadcast_metrics_by_topic.insert(STRESS_TEST_TOPIC.hash(), stress_test_broadcast_metrics);
