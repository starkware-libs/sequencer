...
pub fn receive_stress_test_message(
    ...
    // memory without bound. The pending-count metric will lag rather than OOM.
    if let Err(send_error) = indexed_message_sender.try_send(indexed_message) {
        warn!("Dropping indexed-message sample: {send_error}");
    }
}

pub async fn record_indexed_message(mut indexed_message_receiver: Receiver<IndexedMessage>) {
