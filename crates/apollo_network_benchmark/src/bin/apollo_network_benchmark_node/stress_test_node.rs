...
/// Effectively disables Kademlia bootstrapping: the heartbeat is set so far in the future
/// that the protocol never refreshes its routing table during a benchmark run. We rely on
/// the explicit bootstrap multiaddr list instead.
const KADEMLIA_DISCOVERY_DISABLED_INTERVAL: Duration = Duration::from_secs(u64::MAX);
...
impl StressTestNode {
    fn create_network_config(args: &NodeArgs) -> Result<NetworkConfig, Box<dyn std::error::Error>> {
        ...
        network_config.discovery_config.heartbeat_interval = KADEMLIA_DISCOVERY_DISABLED_INTERVAL;
        ...
        if !args.runner.bootstrap.is_empty() {
            let bootstrap_peers: Vec<Multiaddr> = args
                ...
                .iter()
                .map(|bootstrap_addr| {
                    Multiaddr::from_str(bootstrap_addr.trim()).map_err(|parse_error| {
                        format!(
                            "invalid multiaddr in --bootstrap entry {bootstrap_addr:?}: \
                             {parse_error}"
                        )
                    })
                })
    ...
    pub fn new(args: NodeArgs) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    fn network_manager_task(&mut self) -> BoxFuture<'static, ()> {
        let network_manager = self
            .network_manager
            .take()
            .expect("network_manager is set in new() and taken at most once");
        // Ignore the run() result: race_and_kill_tasks aborts siblings when this task
        // ends, so surfacing it here would be redundant noise.
        async move {
    ...
    fn message_sender_task(&mut self) -> Option<BoxFuture<'static, ()>> {
        ...
        Some(async move { send_stress_test_messages(message_sender, &args).await }.boxed())
    }

    fn message_receiver_tasks(&mut self) -> Vec<BoxFuture<'static, ()>> {
        ...
        vec![
            async move { record_indexed_messages(indexed_message_receiver).await }.boxed(),
            async move {
    ...
    pub async fn run(mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut tasks = vec![self.network_manager_task()];
        tasks.extend(self.message_receiver_tasks());
        tasks.extend(self.message_sender_task());

        let spawned_tasks: Vec<_> = tasks.into_iter().map(tokio::spawn).collect();
