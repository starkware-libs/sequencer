...
///
/// Uses targeted refresh calls instead of `refresh_all()` to avoid refreshing unnecessary
/// information (disks, networks, components, all processes) on every collection cycle.
...
pub async fn monitor_process_metrics(interval_seconds: u64) {
    ...
    let mut state =
        Some(State { system: System::new(), networks: Networks::new_with_refreshed_list() });

    loop {
        interval.tick().await;

        let mut passed_state = state.take().unwrap();
        // The metrics update needs to be done in a blocking context to avoid slowing down tokio
        // threads.
