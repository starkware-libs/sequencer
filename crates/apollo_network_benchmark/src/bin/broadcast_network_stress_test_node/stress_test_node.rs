...
impl BroadcastNetworkStressTestNode {
    ...
    pub async fn start_message_sender(&mut self) -> Option<BoxFuture<'static, ()>> {
        ...
        Some(
            async move {
                send_stress_test_messages(message_sender, &args_clone, peers, &explore_config)
                    .await;
            }
            .boxed(),
        )
    ...
    pub fn make_message_receiver_task(&mut self) -> Vec<BoxFuture<'static, ()>> {
    ...
    async fn get_tasks(&mut self) -> Vec<BoxFuture<'static, ()>> {
        ...
        tasks.extend(self.make_message_receiver_task());
