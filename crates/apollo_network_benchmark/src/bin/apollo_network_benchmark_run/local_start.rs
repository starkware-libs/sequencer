...
fn generate_docker_compose_json(args: &LocalStartArgs, image_tag: &str) -> anyhow::Result<String> {
    ...
    let bootstrap_nodes: Vec<String> = (0..args.shared.num_nodes)
        .map(|node_index| {
            let peer_id = peer_id_from_node_id(u64::from(node_index))?;
            let port = port_with_offset(P2P_PORT_BASE, node_index)?;
            Ok(make_multi_address(
                &format!("/dns4/{STRESS_TEST_NAME}-node-{node_index}"),
                port,
                &peer_id,
                args.shared.quic,
            ))
    ...
    for node_index in 0..args.shared.num_nodes {
        let metric_port = port_with_offset(METRIC_PORT_BASE, node_index)?;
        let p2p_port = port_with_offset(P2P_PORT_BASE, node_index)?;
        ...
        let env: serde_json::Map<String, serde_json::Value> =
            env_pairs.into_iter().map(|(name, value)| (name, json!(value))).collect();

        services.insert(
            format!("node-{}", node_index),
            json!({
                "image": image_tag,
                "container_name": format!("{STRESS_TEST_NAME}-node-{node_index}"),
                "cap_add": ["NET_ADMIN"],
...
fn write_config_files(working_dir: &std::path::Path, num_nodes: u32) -> anyhow::Result<()> {
    let metric_urls: Vec<String> = (0..num_nodes)
        .map(|node_index| {
            let port = port_with_offset(METRIC_PORT_BASE, node_index)?;
            Ok(format!("{STRESS_TEST_NAME}-node-{node_index}:{port}"))
        })
        .collect::<anyhow::Result<Vec<_>>>()?;
...
fn run_experiment(args: LocalStartArgs) -> anyhow::Result<()> {
    ...
    let image_tag = if let Some(ref existing_tag) = args.image {
        existing_tag.clone()
    } else {
        pr!("Building image");
...
pub fn run(args: LocalStartArgs) -> anyhow::Result<()> {
    ...
    run_with_deployment_guard(
        ...
        || {
            ...
            let network_controls = &args.network_controls;
            if network_controls.latency.is_some() || network_controls.throughput.is_some() {
