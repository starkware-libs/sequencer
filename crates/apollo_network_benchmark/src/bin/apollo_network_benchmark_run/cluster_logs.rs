...
// kubectl logs can hang indefinitely on terminated pods. Cap each call so the loop makes
// progress even if some pods are unreachable.
const KUBECTL_LOGS_TIMEOUT_SECONDS: u32 = 5;
...
pub fn run() -> anyhow::Result<()> {
    ...
    for node_index in 0..num_nodes {
        run_cmd(
            &format!(
                "timeout {KUBECTL_LOGS_TIMEOUT_SECONDS} kubectl logs -n {namespace_name} -l \
