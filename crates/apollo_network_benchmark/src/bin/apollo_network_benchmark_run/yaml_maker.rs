...
fn get_prometheus_config_data(
    ...
    for (node_index, url) in metric_urls.iter().enumerate() {
        scrape_configs.push(json!({
            "job_name": format!("broadcast_network_stress_test_{}", node_index),
            "static_configs": [{
                "targets": [url],
                "labels": {
...
pub fn get_prometheus_json_file(num_nodes: u32) -> anyhow::Result<String> {
    let urls: Vec<String> = (0..num_nodes)
        .map(|node_index| {
            format!(
                "{STRESS_TEST_NAME}-{node_index}.{STRESS_TEST_NAME}-headless:{METRIC_PORT_BASE}"
            )
        })
        .collect();
...
pub fn get_network_stress_test_deployment_json_file(
    ...
    let bootstrap_nodes: Vec<String> = (0..args.num_nodes)
        .map(|node_index| {
            ...
            Ok(make_multi_address(
                &format!("/dns/{STRESS_TEST_NAME}-{node_index}.{STRESS_TEST_NAME}-headless"),
                P2P_PORT_BASE,
                &peer_id,
                args.quic,
            ))
    ...
    let toleration_key_value = if let Some(toleration_str) = node_toleration {
        // Split into exactly 2 parts so values containing '=' (legitimate in toleration
        // values) are preserved on the right-hand side.
        let parts: Vec<&str> = toleration_str.splitn(2, '=').collect();
        anyhow::ensure!(
            parts.len() == 2,
