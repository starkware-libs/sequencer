...
fn port_forward(service_name: &str, local_port: u16, remote_port: u16, namespace_name: &str) {
    ...
    match status {
        Ok(exit_status) if !exit_status.success() => {
            eprintln!("kubectl port-forward for {service_name} exited with {exit_status}");
        }
        Err(spawn_error) => {
            eprintln!(
                "Failed to spawn kubectl port-forward for {service_name}: {spawn_error}. Is \
...
pub fn run() -> anyhow::Result<()> {
    ...
    let grafana_namespace = namespace_name.clone();
    let prometheus_namespace = namespace_name;

    let grafana_thread = thread::spawn(move || {
        port_forward("grafana-service", 3000, 3000, &grafana_namespace);
    });

    let prometheus_thread = thread::spawn(move || {
        port_forward("prometheus-service", 9090, 9090, &prometheus_namespace);
    });
