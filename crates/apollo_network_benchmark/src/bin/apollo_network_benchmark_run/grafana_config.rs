...
fn quantile(metric: &dyn MetricDetails, quantile_value: &str) -> String {
    format!("{}{{quantile=\"{}\"}}", metric.get_name(), quantile_value)
}
    ...
    vec![
        ...
        (
            ...
            vec![
                (metric_name(&BROADCAST_MESSAGE_THEORETICAL_THROUGHPUT), "binBps"),
                (rate(&RECEIVE_MESSAGE_BYTES_SUM, "20s"), "binBps"),
                (metric_name(&BROADCAST_MESSAGE_THEORETICAL_HEARTBEAT_MILLIS), "ms"),
