...
/// Time to wait after `kubectl apply --wait` before polling pod status, so newly-applied
/// pods have time to begin pulling images instead of showing as `Pending`.
const KUBECTL_APPLY_SETTLE_DELAY: Duration = Duration::from_secs(10);
...
fn run_experiment(args: ClusterStartArgs) -> anyhow::Result<()> {
    ...
    let mut deployment_data = serde_json::json!({
        "cluster_working_directory": working_dir.display().to_string(),
        "timestamp": timestamp,
    ...
    let delay_seconds = args.shared.user.timeout_seconds;
    ...
    // `kubectl apply --wait` returns when objects are created, but pods may still be
    // pulling images. Sleep so the get-pods status print below shows useful state.
    thread::sleep(KUBECTL_APPLY_SETTLE_DELAY);
    ...
    pr!("WARNING: Please don't forget to delete the namespace manually after the experiment is \
         complete !!!");
    pr!("");
    pr!("To set up port forwarding, run: cargo run --release --bin apollo_network_benchmark_run \
