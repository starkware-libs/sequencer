...
fn write_json_files(image_tag: &str, args: &ClusterStartArgs) -> anyhow::Result<Vec<String>> {
    let files = vec![
        (
        ...
        (
            ...
            STRESS_TEST_HEADLESS_SERVICE_JSON.to_string(),
        ),
...
pub fn run(args: ClusterStartArgs) -> anyhow::Result<()> {
    use crate::mod_utils::run_with_deployment_guard;
