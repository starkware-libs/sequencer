...
fn deployment_working_directory(prefix: &str) -> anyhow::Result<PathBuf> {
    let home = std::env::var("HOME").context("HOME environment variable is not set")?;
    Ok(PathBuf::from(home).join(format!("apollo_network_benchmark_{}", prefix)))
}

fn deployment_file_path(prefix: &str) -> anyhow::Result<PathBuf> {
    Ok(deployment_working_directory(prefix)?.join(DEPLOYMENT_FILE_NAME))
}

pub fn local_deployment_working_directory() -> anyhow::Result<PathBuf> {
    deployment_working_directory("local")
}

pub fn cluster_deployment_working_directory() -> anyhow::Result<PathBuf> {
    deployment_working_directory("cluster")
}

pub fn cluster_deployment_file_path() -> anyhow::Result<PathBuf> {
...
pub fn get_deployment_num_nodes(data: &serde_json::Value) -> anyhow::Result<u64> {
    data.get("args")
        .and_then(|args_value| args_value.get("shared"))
        .and_then(|shared_value| shared_value.get("num_nodes"))
        .and_then(|num_nodes_value| num_nodes_value.as_u64())
