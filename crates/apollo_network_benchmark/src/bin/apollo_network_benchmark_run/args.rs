...
pub fn get_env_var_pairs(
    ...
    let broadcaster = match args.user.broadcaster {
        Some(b) => u32::try_from(b).context("broadcaster value too large for u32")?,
        None => args.num_nodes.checked_sub(1).context("num_nodes must be at least 1")?,
    };
