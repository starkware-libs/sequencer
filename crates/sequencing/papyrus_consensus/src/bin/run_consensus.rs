...
const MONITORING_PERIOD: u64 = 10;
...
struct Node {
    ...
    height_and_timestamp: (Option<u64>, Option<u64>), //(height, timestamp)
...
impl Node {
    ...
    async fn get_metric(&self, metric: &str) -> Option<u64> {
        let port = self.monitoring_gateway_server_port;
        let command = format!(
    ...
    async fn check_node(&mut self) -> (Option<u64>, Option<u64>) {
        ...
        if self.height_and_timestamp.0 != height {
            ...
            self.height_and_timestamp = (height, Some(Instant::now().elapsed().as_secs()));
        }
...
async fn monitor_simulation(
    ...
    let curr_time = Instant::now().elapsed().as_secs();
    if let Some(duration) = duration {
        if duration < (curr_time - start_time.elapsed().as_secs()) {
            return true;
        }
    }
    ...
    for node in nodes {
        ...
        if let (Some(_), Some(last_update)) = (height, last_update) {
            if (curr_time - last_update) > stagnation_timeout {
...
async fn run_simulation(mut nodes: Vec<Node>, duration: Option<u64>, stagnation_timeout: u64) {
    ...
    loop {
        tokio::select! {
            should_break = async {
                tokio::time::sleep(Duration::from_secs(MONITORING_PERIOD)).await;
                let elapsed = start_time.elapsed().as_secs();
                println!("\nTime elapsed: {}s", elapsed);

                monitor_simulation(&mut nodes, start_time, duration, stagnation_timeout).await
            } => {
                if should_break {
                    break;
                }
...
async fn build_node(data_dir: &str, logs_dir: &str, i: usize, papyrus_args: &PapyrusArgs) -> Node {
    ...
    for (key, value) in conditional_params_f.iter() {
        if let Some(v) = value {
