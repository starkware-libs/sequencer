    ...
    pub async fn run(self) -> ShcEvent {
        match self {
            ...
            ShcTask::ValidateProposal(
                ...
                let proposal_id = match built_from_content_receiver.await {
...
impl SingleHeightConsensus {
    ...
    /// The SHC validates `Init`, then starts block validation within the context.
    ...
    pub(crate) async fn handle_proposal<ContextT: ConsensusContext>(
        ...
        // Receiving and validating proposals is non-blocking, which means a malicious node
    ...
    pub async fn handle_event<ContextT: ConsensusContext>(
        ...
        match event {
            ...
            ShcEvent::ValidateProposal(
                ...
                // If there was a network issue and id is None, we should still be able to receive
