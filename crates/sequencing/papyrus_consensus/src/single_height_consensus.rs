...
impl<BlockT: ConsensusBlock> SingleHeightConsensus<BlockT> {
    ...
    pub(crate) async fn handle_proposal<ContextT: ConsensusContext<Block = BlockT>>(
        ...
        let block = match block_receiver.await {
            Ok(block) => block,
            Err(_) => return self.process_proposal(context, &init, None).await,
        };
        ...
        if block.id() != fin {
            return self.process_proposal(context, &init, None).await;
        }
        self.process_proposal(context, &init, Some(block)).await
    }

    async fn process_proposal<ContextT: ConsensusContext<Block = BlockT>>(
        &mut self,
        context: &mut ContextT,
        init: &ProposalInit,
        block: Option<BlockT>,
        ...
        let sm_proposal = match block {
            Some(block) => {
                ...
                // TODO(Asmaa): handle re-proposing
                self.proposals.insert(init.round, Some(block));
                StateMachineEvent::Proposal(Some(block_id), init.round)
            }
            None => {
                // TODO(Asmaa): handle re-proposing
                self.proposals.insert(init.round, None);
    ...
    async fn handle_vote<ContextT: ConsensusContext<Block = BlockT>>(
        ...
        if let Some(old) = votes.get(&(vote.round, vote.voter)) {
        ...
        votes.insert((vote.round, vote.voter), vote);
        let leader_fn =
            |_round: Round| -> ValidatorId { context.proposer(&self.validators, self.height) };
