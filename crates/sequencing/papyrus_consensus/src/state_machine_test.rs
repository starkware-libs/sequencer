...
struct TestWrapper<LeaderFn: Fn(Round) -> ValidatorId> {
    ...
    leader_fn: LeaderFn,
    pub events: VecDeque<StateMachineEvent>,
}
...
fn events_arrive_in_ideal_order(is_proposer: bool) {
    ...
    if is_proposer {
        assert_eq!(
            wrapper.events.pop_front().unwrap(),
            StateMachineEvent::GetProposal(None, ROUND)
        );
    ...
    assert_eq!(wrapper.events.pop_front().unwrap(), StateMachineEvent::Prevote(BLOCK_HASH, ROUND));
    assert!(wrapper.events.is_empty());
...
fn buffer_events_during_get_proposal(vote: Option<BlockHash>) {
    ...
    wrapper.start();
    assert_eq!(wrapper.events.pop_front().unwrap(), StateMachineEvent::GetProposal(None, 0));
    assert!(wrapper.events.is_empty());
    ...
    wrapper.send_get_proposal(BLOCK_HASH, ROUND);
    assert_eq!(wrapper.events.pop_front().unwrap(), StateMachineEvent::Proposal(BLOCK_HASH, ROUND));
    assert_eq!(wrapper.events.pop_front().unwrap(), StateMachineEvent::Prevote(BLOCK_HASH, ROUND));
    assert_eq!(wrapper.events.pop_front().unwrap(), StateMachineEvent::Precommit(vote, ROUND));
    assert!(wrapper.events.is_empty());
}
...
fn advance_to_the_next_round() {
    ...
    assert_eq!(
        wrapper.events.pop_front().unwrap(),
        StateMachineEvent::Prevote(BLOCK_HASH, ROUND + 1)
    );
}
