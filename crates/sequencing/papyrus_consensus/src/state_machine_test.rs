...
fn dont_handle_enqueued_while_awaiting_get_proposal() {
    ...
    wrapper.send_prevote(None, 0);
    wrapper.send_prevote(None, 0);
    wrapper.send_prevote(None, 0);
    wrapper.send_precommit(None, 0);
    wrapper.send_precommit(None, 0);
    wrapper.send_precommit(None, 0);
    wrapper.send_prevote(None, 1);
    wrapper.send_prevote(None, 1);
    wrapper.send_prevote(None, 1);
    wrapper.send_precommit(None, 1);
    wrapper.send_precommit(None, 1);
    wrapper.send_precommit(None, 1);

    // It now receives the proposal.
    wrapper.send_get_proposal(BLOCK_HASH, 0);
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::Proposal(BLOCK_HASH, 0));
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::TimeoutPrecommit(0));
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::GetProposal(None, 1));
    assert!(wrapper.next_event().is_none());

    // The other votes are only handled after the next GetProposal is received.
    wrapper.send_get_proposal(BLOCK_HASH, 1);
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::Proposal(BLOCK_HASH, 1));
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::TimeoutPrecommit(1));
    assert_eq!(wrapper.next_event().unwrap(), StateMachineEvent::GetProposal(None, 2));
    assert!(wrapper.next_event().is_none());
}
