...
> {
    ...
    inbound_stream_data: HashMap<PeerId, LruCache<StreamId, StreamData<T, StreamId>>>,
...
> StreamHandler<T, StreamId>
{
    ...
    fn handle_message(
        ...
        let data = match self.inbound_stream_data.entry(peer_id.clone()) {
            ...
            Occupied(mut entry) => {
                let data = match entry.get_mut().pop(&stream_id) {
                    Some(data) => data,
                    None => {
        ...
        if let Some(data) = self.handle_message_inner(message, metadata, data) {
            self.inbound_stream_data
                .entry(peer_id)
                .or_insert_with(|| LruCache::new(NonZeroUsize::new(MAX_STREAMS_PER_PEER).unwrap()))
                .put(stream_id, data);
        }
    }
