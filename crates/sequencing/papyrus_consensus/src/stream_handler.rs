...
pub trait StreamContent:
    Clone + Into<Vec<u8>> + TryFrom<Vec<u8>, Error = ProtobufConversionError> + Send
{
}
impl<T: Clone + Into<Vec<u8>> + TryFrom<Vec<u8>, Error = ProtobufConversionError> + Send>
    StreamContent for T
{
}
...
#[derive(Debug)]
struct StreamData<T: StreamContent, StreamId: StreamIdTrait> {
    ...
    message_buffer: HashMap<MessageId, StreamMessage<T, StreamId>>,
}

impl<T: StreamContent, StreamId: StreamIdTrait> StreamData<T, StreamId> {
    fn new() -> Self {
...
pub struct StreamHandler<T: StreamContent, StreamId: StreamIdTrait> {
...
impl<T: StreamContent, StreamId: StreamIdTrait> StreamHandler<T, StreamId> {
    ...
    pub fn get_channels(
        inbound_network_receiver: BroadcastTopicServer<StreamMessage<T, StreamId>>,
        outbound_network_sender: BroadcastTopicClient<StreamMessage<T, StreamId>>,
    ) -> (
        ...
        StreamId: 'static,
    {
        ...
        let mut stream_handler = StreamHandler::<T, StreamId>::new(
    ...
    fn handle_message_inner(
        ...
        mut data: StreamData<T, StreamId>,
    ) -> Option<StreamData<T, StreamId>> {
    ...
    fn process_buffer(&mut self, data: &mut StreamData<T, StreamId>) -> bool {
