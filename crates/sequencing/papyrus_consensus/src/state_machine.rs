...
impl StateMachine {
    ...
    fn upon_reproposal(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        let mut output = if block_hash.is_some_and(|v| {
            self.locked_value_round.is_none()
                || self.locked_value_round.is_some_and(|(locked_value, locked_round)| {
    ...
    fn upon_prevote_quorum(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        let Some(prevotes) = self.prevotes.get(&self.round) else {
            return VecDeque::new();
        };
        let Some(count) = prevotes.get(proposal_id) else { return VecDeque::new() };
        if count < &self.quorum {
            return VecDeque::new();
        }
    ...
    fn upon_nil_prevote_quorum(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        let Some(count) = prevotes.get(&None) else { return VecDeque::new() };
        if count < &self.quorum {
            return VecDeque::new();
        }
