...
impl StateMachine {
    ...
    fn upon_new_proposal(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        if self.locked_value.as_ref().map_or(true, |(locked_proposal, _)| {
    ...
    fn upon_prevote_quorum(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        output.append(&mut VecDeque::from([StateMachineEvent::Precommit(
            Some(*block_hash),
            self.round,
        )]));
        self.step = Step::Precommit;
    ...
    fn upon_nil_prevote_quorum(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        output.append(&mut VecDeque::from([StateMachineEvent::Precommit(None, self.round)]));
        self.step = Step::Precommit;
        output
    }
    ...
    fn maybe_timeout_prevote(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        output.append(&mut VecDeque::from([StateMachineEvent::TimeoutPrevote(self.round)]));
        output
    }
    ...
    fn maybe_timeout_precommit(&mut self) -> VecDeque<StateMachineEvent> {
        ...
        output.append(&mut VecDeque::from([StateMachineEvent::TimeoutPrecommit(self.round)]));
        output
    }
