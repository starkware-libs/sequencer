...
mod tests {
    ...
    // stream_handler.listen()).await;         stream_handler
    //     });
    ...
    //     let stream_id3 = 1; // Send in two batches of 5 messages, without the first one, don't
    // send fin.
    ...
    //     assert!(stream_handler.stream_data.clone().into_keys().all(|item|
