...
mod tests {
    ...
    fn setup_test() -> (
        ...
        // The network_sender_to_listen is the sender of the mock network, that is used by the test
        // to send messages into the StreamHandler (from the mock network).
        ...
        let listen_metadata = BroadcastedMessageMetadata::get_test_instance(&mut get_rng());
    ...
    async fn stream_handler_listen_in_reverse() {
        ...
        let peer_id = listen_metadata.originator_id.clone();
        let stream_id = 127;
    ...
    async fn stream_handler_broadcast() {
        ...
        assert_eq!(broadcasted_message.message_id, 0);
        let mut vec1 = stream_handler.broadcast_stream_receivers.keys().collect::<Vec<&u64>>();
        vec1.sort();
