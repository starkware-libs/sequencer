...
async fn validator(repeat_proposal: bool) {
    ...
    context
        .expect_proposer()
        .times(if repeat_proposal { 2 } else { 1 })
        .returning(move |_, _| *PROPOSER_ID);
    context.expect_validate_proposal().times(if repeat_proposal { 2 } else { 1 }).returning(
        move |_, _| {
            let (block_sender, block_receiver) = oneshot::channel();
            block_sender.send(BLOCK.clone()).unwrap();
            block_receiver
...
async fn vote_twice(same_vote: bool) {
    ...
    let res =
        shc.handle_message(&mut context, precommit(Some(*BLOCK_ID), 0, 0, *PROPOSER_ID)).await;
    assert_eq!(res, Ok(None));

    let first_vote = precommit(Some(*BLOCK_ID), 0, 0, *VALIDATOR_ID_2);
    let decision = shc.handle_message(&mut context, first_vote.clone()).await.unwrap().unwrap();
    let second_vote = if same_vote {
        first_vote.clone()
    } else {
        precommit(Some(BlockHash(Felt::TWO)), 0, 0, *VALIDATOR_ID_2)
    };
    ...
    // The decision made by the first vote.
    assert_eq!(decision.block, *BLOCK);
}
