...
use crate::test_utils::{precommit, prevote, proposal, proposal_fin, proposal_init};
...
async fn run_consensus_sync() {
    ...
    // TODO(guyn): refactor this test to pass proposals through the correct channels.
    let (_proposal_sender, proposal_receiver) = mpsc::channel(CHANNEL_SIZE);
...
async fn test_timeouts() {
    ...
    // TODO(guyn): refactor this test to pass proposals through the correct channels.
    let (_proposal_sender, mut proposal_receiver) = mpsc::channel(CHANNEL_SIZE);
