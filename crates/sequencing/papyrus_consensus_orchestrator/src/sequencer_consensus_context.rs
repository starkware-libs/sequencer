...
async fn stream_build_proposal(
    ...
    loop {
        let response = match batcher
            .get_proposal_content(GetProposalContentInput { proposal_id, abort: false })
            .await
        {
        ...
        match response.content {
            GetProposalContent::Txs(txs) => {
                ...
                if let Err(e) = content_sender.send(txs).await {
                    ...
                    let _ = batcher
                        .get_proposal_content(GetProposalContentInput { proposal_id, abort: true })
                        .await;
                    break;
                }
            }
