...
async fn build_proposal(
    ...
    let (proposal_content_id, content) =
        get_proposal_content(proposal_init.height, proposal_id, batcher.as_ref(), proposal_sender)
            .await;
...
async fn get_proposal_content(
    ...
    batcher: &dyn BatcherClient,
    mut proposal_sender: mpsc::Sender<ProposalPart>,
) -> (ProposalContentId, Vec<ExecutableTransaction>) {
    ...
    loop {
        ...
        match response.content {
            ...
            GetProposalContent::Finished(id) => {
                ...
                return (proposal_content_id, content);
            }
        }
    }
}
