...
pub struct SequencerConsensusContext {
    ...
    // height/round until building is done, also context only works on proposals for the
    // current round.
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn set_height_and_round(&mut self, height: BlockNumber, round: Round) {
        if self.current_height.is_none_or(|h| height > h) {
            ...
            // that consensus works on a given height until it is done (either a
            // decision is reached or sync causes us to move on) and then moves on to a
            // different height, never to return to the old height.
            self.batcher
