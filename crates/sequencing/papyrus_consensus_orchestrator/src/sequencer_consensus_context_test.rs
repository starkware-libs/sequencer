...
use crate::sequencer_consensus_context::{ProposalPartWithTxHash, SequencerConsensusContext};
...
async fn build_proposal() {
    ...
    let mut context =
        SequencerConsensusContext::new(Arc::new(batcher), outbound_internal_sender, NUM_VALIDATORS);
...
async fn repropose() {
    ...
    let txs = vec![generate_invoke_tx(Felt::TWO)]
        ...
        .collect();
    let prop_part = ProposalPartWithTxHash {
        part: ProposalPart::Transactions(TransactionBatch { transactions: txs }),
