...
const BLOBS_BUCKET_NAME: &str = "apollo_central_systest_blobs";
...
fn current_generation() -> usize {
    fs::read_to_string(BLOBS_GENERATION_FILE).unwrap().trim().parse().unwrap()
}
...
async fn bump_generation_and_store_blob_file(blobs: &[AerospikeBlob], client: &Client) {
    ...
    client
        .upload_object(
            ...
            &UploadType::Simple(Media::new(format!("{next_generation}/{BLOBS_FILE_NAME}"))),
        )
        .await
        .unwrap();
...
async fn fetch_blob_file(client: &Client) -> Vec<AerospikeBlob> {
    let blobs_json = client
        .download_object(
            &GetObjectRequest {
                bucket: BLOBS_BUCKET_NAME.to_string(),
                object: BLOBS_FILE_NAME.to_string(),
                ..Default::default()
            },
            &Range::default(),
        )
...
async fn test_make_data() {
    ...
    let fix = env::var("UPDATE_EXPECT").unwrap_or_else(|_| "0".to_string());
    if fix != "0" {
