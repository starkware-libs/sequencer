...
impl From<BlockData> for BlobParameters {
    ...
    /// recent block hashes only with the last block hash (of the previous block).
...
impl BlobFactory {
    ...
    async fn close_block(&mut self) {
        ...
        for TxData { executable, internal, should_revert } in
            std::mem::take(&mut self.next_txs).into_iter()
        {
    ...
    fn make_preconfirmed_block_from_remaining_txs(
        ...
        for (tx_index, TxData { executable, internal, .. }) in txs.into_iter().enumerate() {
