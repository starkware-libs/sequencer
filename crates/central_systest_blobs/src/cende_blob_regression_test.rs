...
impl BlobFactory {
    ...
    async fn finalize(
        self,
    ) -> (Vec<AerospikeBlob>, serde_json::Value, CendeWritePreconfirmedBlock) {
        ...
        let mut decoded_state_commitment_infos = serde_json::Map::new();
        for block in blocks.into_iter() {
            let block_number = block.block_context.block_info().block_number;
            decoded_state_commitment_infos.insert(
                block_number.0.to_string(),
                serde_json::to_value(&block.state_commitment_infos).unwrap(),
            );
        ...
        (blobs, serde_json::Value::Object(decoded_state_commitment_infos), preconfirmed_block)
    }
...
async fn bump_generation_and_store_blob_files(
    ...
    decoded_state_commitment_infos: &serde_json::Value,
    client: &Client,
) {
    ...
    upload_new_object(
        ...
        format!("{}\n", serde_json::to_string_pretty(decoded_state_commitment_infos).unwrap()),
    )
    .await;
...
async fn test_make_data() {
    ...
    if env::var("UPDATE_EXPECT").is_ok() {
        ...
        assert_eq!(
            decoded_state_commitment_infos, fetched_decoded_state_commitment_infos,
