...
impl BaseLayerContract for EthereumBaseLayerContract {
    ...
    async fn get_proved_block_at(
        ...
        let call_state_block_number = self.contract.stateBlockNumber().block(l1_block.into());
        let call_state_block_hash = self.contract.stateBlockHash().block(l1_block.into());
