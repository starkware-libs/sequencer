...
async fn latest_proved_block_ethereum() {
    ...
    let (_node_handle, contract) = EthereumBaseLayerContractBuilder.build_from_ganache();
...
async fn get_proved_block_at_unknown_block_number() {
    ...
    let (_node_handle, contract) = EthereumBaseLayerContractBuilder.build_from_anvil(None);
