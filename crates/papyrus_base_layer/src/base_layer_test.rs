...
// Offline unit test (never issues a request): verifies that cycling the provider URL re-points the
// *live* provider, not merely an internal index. A mock can't cover this because the behavior under
// test lives inside `EthereumBaseLayerContract::cycle_provider_url` itself, so we use the real
// contract with non-routable fake URLs and only inspect URL-routing state.
#[tokio::test]
async fn test_cycle_provider_url_repoints_live_provider() {
    ...
    let config = EthereumBaseLayerConfig {
        ordered_l1_endpoint_urls: vec![primary_url.clone().into(), secondary_url.clone().into()],
        ..Default::default()
    };
    ...
    // Cycling must move the live provider to the next endpoint.
    base_layer.cycle_provider_url().await.unwrap();
    assert_eq!(base_layer.get_url().await.unwrap().expose_secret(), secondary_url);

    // Cycling past the end wraps back to the primary.
    base_layer.cycle_provider_url().await.unwrap();
    assert_eq!(base_layer.get_url().await.unwrap().expose_secret(), primary_url);
}

// Offline unit test (never issues a request): verifies that natural cycling wraps back to the
// primary through a list longer than two, regardless of elapsed time. Uses the real contract with
// non-routable fake URLs and only inspects URL-routing state.
#[tokio::test]
...
// Offline unit test (never issues a request): verifies that reset_provider_url_to_primary
// re-points the live provider to the primary endpoint, and that calling it when already on the
// primary is a no-op.
#[tokio::test]
