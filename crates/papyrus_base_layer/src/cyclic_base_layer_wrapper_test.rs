...
async fn cycle_get_proved_block_at(#[case] num_failing_calls: usize) {
    ...
    let mut wrapper = CyclicBaseLayerWrapper::new(base_layer, Duration::from_secs(3600));
...
async fn pass_through_get_block_header_immutable(#[case] success: bool) {
    ...
    let wrapper = CyclicBaseLayerWrapper::new(base_layer, Duration::from_secs(3600));
...
async fn test_exhaust_from_non_primary_index_returns_last_error() {
    ...
    let url_sequence = [
        ...
        secondary_url.clone(),
        secondary_url,
        Sensitive::new(Url::parse("http://tertiary_endpoint").unwrap())
            .with_redactor(to_safe_string),
    ];
    let num_url_calls_made = Arc::new(AtomicUsize::new(0));
    let num_url_calls_made_clone = num_url_calls_made.clone();
    ...
    let mut base_layer = MockBaseLayerContract::new();
    base_layer.expect_get_url().times(url_sequence.len()).returning(move || {
        let url_index = num_url_calls_made_clone.fetch_add(1, Ordering::Relaxed);
        Ok(url_sequence[url_index].clone())
    });
    // Every attempt fails: one per endpoint in the 3-URL list.
    base_layer.expect_get_proved_block_at().times(3).returning(move |_| Err(MockError::MockError));
    // Cycling succeeds once per failed attempt.
    base_layer.expect_cycle_provider_url().times(3).returning(move || Ok(()));
    // Use a large failback interval so failback never interferes with the cycling path.
    let mut wrapper = CyclicBaseLayerWrapper::new(base_layer, Duration::from_secs(3600));
...
// Verifies that when the failback interval has elapsed, the wrapper calls
// reset_provider_url_to_primary before executing the cycling operation.
#[tokio::test]
async fn test_failback_to_primary_when_interval_elapsed() {
...
// Verifies that when the failback interval has elapsed but reset_provider_url_to_primary returns
// an error, the error propagates and the underlying operation is never called.
#[tokio::test]
async fn test_failback_error_propagates_and_skips_operation() {
    // Setup: use Duration::ZERO so the interval is always elapsed.
    let mut base_layer = MockBaseLayerContract::new();
    base_layer
    ...
    // The underlying operation must never be reached when failback errors out.
...
// Verifies that when the failback interval has not elapsed, the wrapper does not call
// reset_provider_url_to_primary.
#[tokio::test]
async fn test_no_failback_when_interval_not_elapsed() {
