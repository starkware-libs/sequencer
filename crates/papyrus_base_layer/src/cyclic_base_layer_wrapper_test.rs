...
async fn test_exhaust_from_non_primary_index_returns_last_error() {
    ...
    let num_url_calls_made = Arc::new(AtomicUsize::new(0));
    let num_url_calls_made_clone = num_url_calls_made.clone();
    let num_attempt_calls_made = Arc::new(AtomicUsize::new(0));
    let num_attempt_calls_made_clone = num_attempt_calls_made.clone();
    ...
    base_layer.expect_get_proved_block_at().times(3).returning(move |_| {
        let attempt_index = num_attempt_calls_made_clone.fetch_add(1, Ordering::Relaxed);
        Err(MockError::Numbered(attempt_index))
    });
    ...
    // Test: all three attempts fail; the wrapper must return the last attempt's error.
    let result = wrapper.get_proved_block_at(1).await;
    assert_eq!(result, Err(MockError::Numbered(2)));
}
