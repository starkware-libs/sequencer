...
pub async fn spawn_anvil_and_deploy_starknet_l1_contract(
    ...
    let anvil = anvil_instance_from_url(url.clone());
...
pub async fn deploy_starknet_l1_contract(
    ...
    let ethereum_base_layer_contract = EthereumBaseLayerContract::new(config, url);
...
pub async fn make_block_history_on_anvil(
    ...
    let base_layer = EthereumBaseLayerContract::new(base_layer_config.clone(), url);
