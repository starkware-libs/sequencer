...
pub fn encode_binary_file_to_u32(path: &Path) -> Result<Vec<u32>, ProofEncodingError> {
...
pub fn decode_u32_to_binary_file(path: &Path, data: &[u32]) -> Result<(), ProofEncodingError> {
...
pub fn encode_bytes_to_u32(data: &[u8]) -> Vec<u32> {
    let padding = if data.is_empty() { 0 } else { (4 - (data.len() % 4)) % 4 };
    let padded_len = data.len() + padding;
    let mut encoded = Vec::with_capacity(1 + padded_len / 4);
    encoded.push(padding as u32);

    // Create padded data with zeros at the beginning.
    let mut padded_data = vec![0u8; padding];
...
mod tests {
    ...
    #[case::aligned_8_bytes(vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08])]
