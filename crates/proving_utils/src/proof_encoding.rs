...
pub fn encode_bytes_to_u32(data: &[u8]) -> Vec<u32> {
    ...
    if remainder > 0 {
        // Create padded word with zeros at the beginning.
        let mut padded_word = vec![0u8; padding];
        padded_word.extend_from_slice(&data[..remainder]);
        encoded.push(u32::from_be_bytes(
            padded_word.try_into().expect("Padded word must be 4 bytes"),
        ));
    }
