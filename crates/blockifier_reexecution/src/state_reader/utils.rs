...
pub fn assert_eq_state_diff(
    expected_state_diff: CommitmentStateDiff,
    actual_state_diff: CommitmentStateDiff,
) {
    let comparable_expected_state_diff = ComparableStateDiff::from(expected_state_diff);
    let comparable_actual_state_diff = ComparableStateDiff::from(actual_state_diff);
    assert_eq!(comparable_expected_state_diff, comparable_actual_state_diff);
}
