...
/// Test that the contract class hash is different before and after the class is replaced
#[rstest]
pub fn test_contract_change_after_replace_class() {
    let state_reader_before_replace_class = TestStateReader::new_for_testing(BlockNumber(620978));
    let state_reader_after_replace_class = TestStateReader::new_for_testing(BlockNumber(630000));

    // The stark gate contract address. Replace class between the two blocks.
    let stark_gate_address =
        contract_address!("0x49d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7");

    assert_ne!(
        state_reader_before_replace_class.get_class_hash_at(stark_gate_address).unwrap(),
        state_reader_after_replace_class.get_class_hash_at(stark_gate_address).unwrap()
    );
}
