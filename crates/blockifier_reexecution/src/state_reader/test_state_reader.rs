...
impl TestStateReader {
    ...
    pub fn get_transaction_executor(
        ...
        let old_block_hash_and_number = self.get_old_block_hash_and_number(old_block_number)?;
        Ok(TransactionExecutor::<TestStateReader>::pre_process_and_create(
            self,
            block_context_next_block,
            Some(old_block_hash_and_number),
...
impl ReexecutionStateReader for TestStateReader {
    ...
    fn get_old_block_hash_and_number(
        ...
        Ok(BlockHashAndNumber {
            hash: BlockHash(Felt::from_hex(&block_hash_raw).unwrap()),
