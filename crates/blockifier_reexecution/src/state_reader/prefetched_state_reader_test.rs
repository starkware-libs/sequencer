...
async fn test_simulate_and_get_initial_reads() {
    ...
    let state_maps = tokio::task::spawn_blocking(move || {
        simulate_and_get_initial_reads(&rpc_state_reader, BlockId::Latest, &[tx], false, false)
    })
    .await
    .unwrap()
    .unwrap();
...
/// Verifies that `initial_reads` JSON is correctly deserialized into `StateMaps`,
/// including edge cases (empty object, partial fields).
#[rstest]
#[case::all_fields(
    json!({
        "storage": [
