...
use starknet_api::transaction::{DeployAccountTransaction, InvokeTransaction, Transaction};
...
pub fn simulate_and_get_initial_reads(
    ...
    let rpc_txs: Vec<RpcTransaction> = txs
        .iter()
        .enumerate()
        .filter_map(|(tx_index, tx)| match tx {
            Transaction::Invoke(InvokeTransaction::V3(v3)) => {
                RpcInvokeTransactionV3::try_from(v3.clone())
                    .inspect_err(|e| {
                        tracing::warn!(
                            tx_index,
                            "V3 Invoke → RPC conversion failed, skipping prefetch: {e}"
                        )
                    })
            ...
            Transaction::DeployAccount(DeployAccountTransaction::V3(v3)) => {
                RpcDeployAccountTransactionV3::try_from(v3.clone())
                    .inspect_err(|e| {
                        tracing::warn!(
                            tx_index,
                            "V3 DeployAccount → RPC conversion failed, skipping prefetch: {e}"
                        )
                    })
