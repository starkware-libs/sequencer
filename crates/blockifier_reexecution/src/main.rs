...
async fn main() {
    ...
    let fn_block_dir = |block_number| format!("/block_{block_number}");
    let fn_block_data_file = |block_number| fn_block_dir(block_number) + FILE_NAME;
    let fn_block_full_directory =
        |directory_path: String, block_number| directory_path + &fn_block_dir(block_number);
    let fn_block_full_file_path = |directory_path, block_number| {
        fn_block_full_directory(directory_path, block_number) + FILE_NAME
    };
    ...
    match args.command {
        ...
        Command::WriteToFile { block_numbers, directory_path, rpc_args } => {
            ...
            for block_number in block_numbers {
                let full_file_path = fn_block_full_file_path(directory_path.clone(), block_number);
        ...
        Command::Reexecute { block_numbers, directory_path } => {
            ...
            for block in block_numbers {
                let full_file_path = fn_block_full_file_path(directory_path.clone(), block);
                    ...
                    client
                        .get_object(&GetObjectRequest {
                            ...
                            object: files_prefix.clone()
                                + &fn_block_data_file(block_number),
                            ..Default::default()
                        })
                        .await
            ...
            for block_number in block_numbers {
                client
                    .upload_object(
                        ...
                        fs::read(fn_block_full_file_path(directory_path.clone(), block_number))
                            .unwrap(),
                        &UploadType::Simple(Media::new(
                            files_prefix.clone() + &fn_block_data_file(block_number),
                        )),
                    )
                    .await
                    .unwrap();
            ...
            for block_number in block_numbers {
                let res = client
                    .download_object(
                        &GetObjectRequest {
                            bucket: BUCKET.to_string(),
                            object: files_prefix.clone() + &fn_block_data_file(block_number),
                            ..Default::default()
                        },
                        &Range::default(),
                    )
                    .await
                    .unwrap();
                fs::create_dir_all(fn_block_full_directory(directory_path.clone(), block_number))
                    .unwrap();
                fs::write(fn_block_full_file_path(directory_path.clone(), block_number), res)
                    .unwrap();
            }
