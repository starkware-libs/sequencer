    ...
    match args.command {
        Command::RpcTest { block_number, rpc_args } => {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                assert!(
                    readers.reexecute_and_verify_correctness(BlockNumber(block_number)),
        ...
        Command::Reexecute { block_numbers, directory_path } => {
            ...
            for block in block_numbers {
                ...
                task_set.spawn_blocking(move || {
                    let readers = OfflineBlockReexecutor::new_from_file(
                        &full_file_path,
                        contract_class_manager,
                    )
                    .unwrap();
                    let block_number = readers.block_context_next_block.block_info().block_number;
                    let matched = readers.reexecute_and_verify_correctness(block_number);
                    if matched {
