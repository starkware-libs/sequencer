...
1- From a JSON file (-t <tx_path>)
2- From the transaction hash (-y <tx_hash>, fetched via RPC)
...
```
cargo run --bin blockifier_reexecution reexecute-single-tx -n <node_url> -b <block_number> from-file -t <tx_path>
```
Using a transaction hash:
```
cargo run --bin blockifier_reexecution reexecute-single-tx -n <node_url> -b <block_number> from-hash -y <tx_hash>
```
