...
// TODO(Tsabary: these are placeholders for the macro to compile, remove them, along the
// starknet_api and strum dependencies in the toml file)
...
define_metrics!(
    Mempool => {
        ...
        LabeledMetricCounter { MEMPOOL_TRANSACTIONS_RECEIVED, "mempool_transactions_received", "Counter of transactions received by the mempool", init = 0 , labels = RPCTRANSACTIONLABELVALUE_LABELS},
        LabeledMetricCounter { MEMPOOL_TRANSACTIONS_DROPPED, "mempool_transactions_dropped", "Counter of transactions dropped from the mempool", init = 0 , labels = RPCTRANSACTIONLABELVALUE_LABELS },
    },
);

define_metrics!(
>>>>>>> 7412416b7 (chore(starknet_sequencer_metrics): add label permutations to labeled metrics)
    Network => {
        // Gauges
