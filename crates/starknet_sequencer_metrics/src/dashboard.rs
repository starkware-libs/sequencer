...
impl Panel {
    pub const fn new(
        ...
        Self { name, description, expr, panel_type, position_priority: 1_u8 }
    }
}
...
impl Serialize for Panel {
    ...
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("Panel", 6)?; // 6 fields (including extra dict)
        ...
        state.serialize_field("position_priority", &self.position_priority)?;
