...
pub(crate) fn get_tx_data(execution_status: &TransactionExecutionStatus) -> TransactionHashingData {
    TransactionHashingData {
        transaction_signature: TransactionSignature(
            vec![Felt::from_bytes_be_slice(&[1_u8]), Felt::from_bytes_be_slice(&[2_u8])].into(),
        ),
...
fn create_execution_resources(
    ...
    ExtendedExecutionResources {
        ...
        opcode_instance_counter: BTreeMap::from([(OpcodeName::blake, blake_opcode_count)]),
    }
}
...
fn create_call_execution(
    ...
    let ordered_events: Vec<OrderedEvent> = events
        .into_iter()
        .enumerate()
        .map(|(i, event)| OrderedEvent { order: *events_counter + i, event: event.content })
        .collect();
    *events_counter += ordered_events.len();

    let ordered_messages: Vec<OrderedL2ToL1Message> = messages
        .into_iter()
        .enumerate()
        .map(|(i, message)| OrderedL2ToL1Message {
            order: *messages_counter + i,
        ...
        .collect();
    *messages_counter += ordered_messages.len();
...
fn create_transaction_resources() -> TransactionResources {
    TransactionResources {
        ...
        computation: ComputationResources {
            tx_extended_vm_resources: create_execution_resources(2000, 5, 20, 10, 7),
            os_vm_resources: create_execution_resources(500, 1, 5, 2, 0).vm_resources,
            n_reverted_steps: 0,
