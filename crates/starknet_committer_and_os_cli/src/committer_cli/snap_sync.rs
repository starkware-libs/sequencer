...
trait TreeKey: Copy + Default + Into<Felt> + Send + Sync + 'static {
    ...
    fn initial_request(context: Self::Context) -> TreeRequest<Self> {
        TreeRequest { context, start: PatriciaKey::default(), end: MAX_PATRICIA_KEY }
    }
...
/// Converts a `Felt` to a `PatriciaKey`, panicking if the `Felt` is not a valid `PatriciaKey`.
fn patricia_key_from_felt(felt: Felt) -> PatriciaKey {
    PatriciaKey::try_from(felt).expect("felt expected to be a valid PatriciaKey")
}
...
fn process_request<K: TreeKey, S: Storage + Send + 'static>(
    ...
    Box::pin(async move {
        let (state_diff, actual_end) = K::scan(&reader, &request, block_target, size_limit)
            .unwrap_or_else(|e| {
                panic!("scan failed for {} (context: {:?}): {e}", type_name::<K>(), request.context)
            });
        ...
        if covered <= shr_one(shr_one(request_range_size)) {
            ...
            let left = TreeRequest::<K> {
                ...
                start: patricia_key_from_felt(remaining_start),
                end: patricia_key_from_felt(mid),
            };
            let right = TreeRequest::<K> {
                context: request.context,
                start: patricia_key_from_felt(mid + Felt::ONE),
                end: request.end,
            };
            tokio::join!(
                process_request(
...
pub async fn run_snap_sync(args: SnapSyncArgs) {
    let storage_config = StorageConfig {
        db_config: DbConfig {
            path_prefix: args.batcher_path_prefix,
    ...
    let block_target = BlockNumber(args.block_target);
    let size_limit = args.size_limit;

    let rocks_db = RocksDbStorage::new(&args.committer_db_path, RocksDbStorageConfig::default())
        .expect("Failed to open committer RocksDB storage");
    let commit_state = Arc::new(Mutex::new(CommitState {
        committer_db: IndexDb::new(CachedStorage::new(rocks_db, Default::default())),
        state_roots: StateRoots::default(),
        num_commits: 0,
    }));

    info!("Starting snap sync at block {}", block_target.0);

    // Storage trees: one per contract that has storage at block_target.
    let mut next_addr = contract_address!(0_u64);
    loop {
        ...
        process_request(
            Arc::clone(&reader),
            StorageKey::initial_request(addr),
            block_target,
            size_limit,
            Arc::clone(&commit_state),
        )
    ...
    process_request(
        Arc::clone(&reader),
        ContractAddress::initial_request(()),
        block_target,
        size_limit,
        Arc::clone(&commit_state),
    )
