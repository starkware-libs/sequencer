...
use starknet_patricia_storage::storage_trait::{DbStorageKey, DbStorageValue, Storage};
...
fn hash_storage(storage: &HashMap<DbStorageKey, DbStorageValue>) -> (Vec<u8>, Vec<u8>) {
    let mut keys_hash = vec![0; 32];
    let mut values_hash = vec![0; 32];
...
/// This function storage_serialize_test generates a MapStorage containing DbStorageKey and
/// DbStorageValue pairs for u128 values in the range 0..=1000,
...
pub(crate) fn storage_serialize_test() -> Result<String, CommitterPythonTestError> {
    ...
    for i in 0..=99_u128 {
        let key = DbStorageKey(Felt::from(i).to_bytes_be().to_vec());
        let value = DbStorageValue(Felt::from(i).to_bytes_be().to_vec());
