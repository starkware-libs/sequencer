...
struct AggregatorSerializedInput {
...
struct AggregatorCliInput {
    layout: LayoutName,
    aggregator_serialized_input: AggregatorSerializedInput,
...
pub(crate) fn parse_and_run_aggregator(input_path: String, output_path: String) {
    let AggregatorCliInput { layout, aggregator_serialized_input, cairo_pie_zip_path } =
        load_input(input_path);
    // TODO(Aner): Validate the aggregator input.

    let aggregator_input = AggregatorInput {
        bootloader_output: aggregator_serialized_input.bootloader_output,
        full_output: aggregator_serialized_input.full_output,
        da: match aggregator_serialized_input.da_path {
            Some(path) => {
                assert!(aggregator_serialized_input.use_kzg_da);
                DataAvailability::Blob(PathBuf::from(path))
            }
            None => {
                assert!(!aggregator_serialized_input.use_kzg_da);
