...
impl TryFrom<String> for OsPythonTestRunner {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        match value.as_str() {
            ...
            "run_compress_function" => Ok(Self::RunCompressFunction),
...
impl PythonTestRunner for OsPythonTestRunner {
    ...
    async fn run(&self, input: Option<&str>) -> OsPythonTestResult {
        match self {
            ...
            Self::RunCompressFunction => {
                run_cairo_function(Self::non_optional_input(input)?, "compress", &[], &[])
            }
        }
    }
}
...
fn run_cairo_function(
    ...
    let hint_processor = BuiltinHintProcessor::new_empty(); //FIXME: Use starknet_os hint processor
    run_cairo_0_entry_point(program_bytes, function_name, args, expected_retdata, hint_processor)
        .map_err(|error| {
