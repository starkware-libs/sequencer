...
fn extract_error_from_stderr(stderr: &str) -> String {
    let meaningful_lines: Vec<&str> = stderr
        ...
        .filter(|line| {
            ...
            if trimmed.eq_ignore_ascii_case("stack backtrace:") || trimmed.starts_with("Signal info:")
            {
                return false;
            }
...
fn format_compiler_error(stderr: &str, status: &std::process::ExitStatus) -> String {
