...
fn compare_compilation_data(
    new_compiled_data: Vec<u8>,
    existing_compiled_data: String,
    contract_path: String,
    existing_path: String,
) {
    if String::from_utf8(new_compiled_data).unwrap() != existing_compiled_data {
        panic!(
            "{} does not compile to {existing_path}.\nRun `{FIX_COMMAND}` to fix the existing \
...
fn verify_feature_contracts_cairo1_logic(cairo_version: CairoVersion) -> Vec<String> {
    let (compiled_paths_on_filesystem, mut sierra_paths_on_filesystem): (Vec<String>, Vec<String>) =
...
fn verify_feature_contracts_match_enum(
    ...
    match cairo_version {
        ...
        CairoVersion::Cairo1 => {
            compiled_paths_on_filesystem = verify_feature_contracts_cairo1_logic(cairo_version);
        }
        #[cfg(feature = "cairo_native")]
        CairoVersion::Native => {
            compiled_paths_on_filesystem = verify_feature_contracts_cairo1_logic(cairo_version);
