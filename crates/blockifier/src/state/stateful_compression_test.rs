...
use super::{
    state_diff_with_allocating_aliases,
    AliasUpdater,
...
fn test_iterate_aliases() {
    ...
    let storage_diff = state_diff_with_allocating_aliases(&mut state).unwrap().storage;
    assert_eq!(
        storage_diff,
        vec![
            (
                (ALIAS_CONTRACT_ADDRESS, ALIAS_COUNTER_STORAGE_KEY),
                MIN_VALUE_FOR_ALIAS_ALLOC + Felt::from(6_u8)
            ),
            ((ALIAS_CONTRACT_ADDRESS, StorageKey::from(0x200_u16)), MIN_VALUE_FOR_ALIAS_ALLOC),
            (
            ...
            (
                ...
                MIN_VALUE_FOR_ALIAS_ALLOC + Felt::from(5_u8)
            ),
