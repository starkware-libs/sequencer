...
#[case::run_without_native(false, false)]
fn test_set_and_compile(#[case] run_cairo_native: bool, #[case] wait_on_native_compilation: bool) {
    let native_config =
        CairoNativeRunConfig { run_cairo_native, wait_on_native_compilation, ..Default::default() };
    let manager = NativeClassManager::create_for_testing(native_config);
    let request = create_test_request();
    let class_hash = request.0;
    let cached_class = CachedClass::V1(request.2.clone(), request.1.clone());

    manager.set_and_compile(class_hash, cached_class);
    if wait_on_native_compilation {
        assert_matches!(
            manager.cache.get(&class_hash).unwrap(),
...
fn test_contract_class_manager_flow(
    ...
    for _ in 0..max_n_retries {
        ...
        if manager.get_runnable(&request.0).is_some() {
            break;
        }
    }
    if should_pass {
