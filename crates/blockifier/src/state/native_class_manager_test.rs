...
use crate::blockifier::config::{CairoNativeRunConfig, CairoNativeRunMode, NativeClassesWhitelist};
...
#[rstest]
#[case::run_native_while_waiting(CairoNativeRunMode::SynchronousCompilation)]
#[case::run_native_without_waiting(CairoNativeRunMode::AsynchronousCompilation)]
#[case::run_without_native(CairoNativeRunMode::Off)]
fn test_start(#[case] cairo_native_run_mode: CairoNativeRunMode) {
    ...
    match cairo_native_run_mode {
        ...
        CairoNativeRunMode::AsynchronousCompilation => {
            assert!(
...
fn test_set_and_compile(
    #[case] cairo_native_run_mode: CairoNativeRunMode,
    ...
    if cairo_native_run_mode == CairoNativeRunMode::Off {
        assert_matches!(manager.class_cache.get(&class_hash).unwrap(), CompiledClasses::V1(_, _));
        return;
    }

    if cairo_native_run_mode == CairoNativeRunMode::AsynchronousCompilation {
        assert_matches!(manager.class_cache.get(&class_hash).unwrap(), CompiledClasses::V1(_, _));
...
fn test_send_compilation_request_channel_disconnected() {
    ...
    let native_config = CairoNativeRunConfig {
        cairo_native_run_mode: CairoNativeRunMode::AsynchronousCompilation,
