...
impl NativeClassManager {
    ...
    /// 2. `config.run_cairo_native_options` is `Off`.
    /// 3. `config.run_cairo_native_options` is `WaitOnCompilation`.
    pub fn start(config: ContractClassManagerConfig) -> NativeClassManager {
        ...
        if cairo_native_run_config.run_cairo_native_options == RunCairoNativeOptions::Off {
        ...
        if cairo_native_run_config.run_cairo_native_options
    ...
    pub fn get_runnable(
        ...
        if let CompiledClasses::V1(..) = cached_class {
            // When native mode is WaitOnCompilation, all V1 classes should have been compiled to
            // native synchronously. A V1 cache entry indicates a pipeline bug.
            assert_ne!(
                self.run_cairo_native_options(),
                RunCairoNativeOptions::WaitOnCompilation,
    ...
    /// For Cairo 1 classes:
    /// * if Native mode is DontWaitOnCompilation, triggers async compilation to Native.
    ...
    pub fn set_and_compile(&self, class_hash: ClassHash, compiled_class: CompiledClasses) {
        match compiled_class {
            ...
            CompiledClasses::V1(compiled_class_v1, sierra_contract_class) => {
                if let RunCairoNativeOptions::WaitOnCompilation = self.run_cairo_native_options() {
                ...
                if let RunCairoNativeOptions::DontWaitOnCompilation =
                    self.run_cairo_native_options()
                {
    ...
    fn run_cairo_native_options(&self) -> RunCairoNativeOptions {
        self.cairo_native_run_config.run_cairo_native_options
    }
