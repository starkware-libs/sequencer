...
impl NativeClassManager {
    ...
    /// 3. `config.cairo_native_run_mode` is `SynchronousCompilation`.
    pub fn start(config: ContractClassManagerConfig) -> NativeClassManager {
        ...
        if cairo_native_run_config.cairo_native_run_mode == CairoNativeRunMode::Off {
        ...
        if cairo_native_run_config.cairo_native_run_mode
    ...
    pub fn get_runnable(
        ...
        if let CompiledClasses::V1(..) = cached_class {
            // When native mode is SynchronousCompilation, all V1 classes should have been
    ...
    /// For Cairo 1 classes:
    /// * if Native mode is AsynchronousCompilation, triggers async compilation to Native.
    /// * If Native mode is SynchronousCompilation, caches the Native variant immediately.
    ...
    fn cairo_native_run_mode(&self) -> CairoNativeRunMode {
