...
mod storage_map_serializer {
    ...
    where
        D: Deserializer<'de>,
    {
        let nested_map: StorageMap = serde::Deserialize::deserialize(deserializer)?;
        Ok(nested_map
            .into_iter()
...
pub type StorageDiff = IndexMap<ContractAddress, IndexMap<StorageKey, Felt>>;
...
#[cfg(any(test, feature = "testing"))]
impl StateMaps {
    pub fn new(commitment_state_diff: &CommitmentStateDiff) -> Self {
        Self {
            class_hashes: commitment_state_diff
            ...
            storage: commitment_state_diff
                ...
                .flat_map(|(address, storage)| {
                    storage.iter().map(move |(key, value)| ((*address, *key), *value))
                })
                .collect(),
