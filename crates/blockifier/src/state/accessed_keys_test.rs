...
use super::compute_accessed_keys;
...
fn test_alias_contract_address() -> ContractAddress {
    ContractAddress::from(0x2_u8)
}
...
fn no_inputs_no_alias_prediction_yields_empty_storage_keys() {
    let accessed = compute_accessed_keys(
        std::iter::empty::<&TransactionExecutionInfo>(),
        std::iter::empty::<&BlockNumber>(),
        &StateMaps::default(),
        test_alias_contract_address(),
        false,
    );
    assert!(accessed.storage_keys.is_empty());
    assert!(accessed.modified_contracts.is_empty());
    assert!(accessed.compiled_class_hash_keys.is_empty());
}

#[test]
fn empty_inputs_with_alias_prediction_contains_counter_only() {
    let alias_contract_address = test_alias_contract_address();
    let accessed = compute_accessed_keys(
        std::iter::empty::<&TransactionExecutionInfo>(),
        std::iter::empty::<&BlockNumber>(),
        &StateMaps::default(),
    ...
    assert_eq!(
        ...
        HashSet::from([(alias_contract_address, ALIAS_COUNTER_STORAGE_KEY)]),
    );
    assert_eq!(accessed.modified_contracts, HashSet::from([alias_contract_address]));
}
...
fn state_diff_storage_keys_are_included() {
    ...
    assert!(accessed.modified_contracts.contains(&address));
}
...
fn proof_facts_block_numbers_map_to_block_hash_table_entries() {
    ...
    assert!(accessed.modified_contracts.contains(&BLOCK_HASH_TABLE_ADDRESS));
}
...
fn alias_predictions_toggle_controls_alias_contract_entries() {
    ...
    let with_predictions = compute_accessed_keys(
        std::iter::empty::<&TransactionExecutionInfo>(),
        std::iter::empty::<&BlockNumber>(),
        &state_diff,
        alias_contract_address,
    ...
    let without_predictions = compute_accessed_keys(
        std::iter::empty::<&TransactionExecutionInfo>(),
        std::iter::empty::<&BlockNumber>(),
        &state_diff,
        alias_contract_address,
...
fn visited_storage_entries_from_execution_info_are_included() {
    ...
    assert!(accessed.modified_contracts.contains(&storage_address));
}
