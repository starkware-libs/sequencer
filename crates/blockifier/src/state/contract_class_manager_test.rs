...
fn check_compilation_result(
    expected_native: NativeCompiledClassV1,
    cache_native: CachedCairoNative,
) {
    if let CachedCairoNative::Compiled(compiled_native) = cache_native {
        assert_eq!(compiled_native, expected_native);
    } else {
        panic!(
            "Cached Native class should match the expected result. The run logs: \n{:?}",
            TEST_LOGGER.get_logs()
        );
    }
}
...
fn test_send_compilation_request_positive_flow(
    ...
    manager.send_compilation_request(request);
    if wait_on_native_compilation {
        check_compilation_result(native, manager.get_native(&class_hash).unwrap());
    } else {
        let expected_log =
...
fn test_process_compilation_request(
    ...
    match expected_cached_native {
        CachedCairoNative::Compiled(compiled_native) => {
            check_compilation_result(compiled_native, manager.get_native(&request.0).unwrap());
        }
        CachedCairoNative::CompilationFailed => {
            assert!(matches!(
...
fn test_run_compilation_worker() {
    ...
    check_compilation_result(expected_native, cache_native);
    TEST_LOGGER.clear_logs();
}
