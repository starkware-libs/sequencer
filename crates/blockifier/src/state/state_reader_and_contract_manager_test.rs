...
#[rstest]
#[case::disabled(CairoNativeMode::Disabled)]
#[cfg_attr(feature = "cairo_native", case::async_mode(CairoNativeMode::Async))]
#[cfg_attr(feature = "cairo_native", case::sync(CairoNativeMode::Sync))]
fn test_get_compiled_class_without_native_in_cache(
    ...
    #[case] native_mode: CairoNativeMode,
) {
    // Sanity check: If the cairo_native feature is off, native must be disabled.
    #[cfg(not(feature = "cairo_native"))]
    assert!(matches!(native_mode, CairoNativeMode::Disabled));

    let test_contract = FeatureContract::TestContract(cairo_version);
    let test_class_hash = test_contract.get_class_hash();
    let contract_manager_config =
        ContractClassManagerConfig::create_for_testing(native_mode.clone());
    ...
    match cairo_version {
        CairoVersion::Cairo1(_) => {
            ...
            if matches!(native_mode, CairoNativeMode::Sync) {
                #[cfg(feature = "cairo_native")]
                assert_matches!(
                    compiled_class,
...
fn test_get_compiled_class_when_native_is_cached() {
    ...
    let test_class_hash = test_contract.get_class_hash();
    let contract_manager_config =
        ContractClassManagerConfig::create_for_testing(CairoNativeMode::Sync);
