...
#[rstest]
#[case::no_cairo_native(CairoNativeRunMode::Off)]
#[cfg_attr(
    feature = "cairo_native",
    case::cairo_native_no_wait(CairoNativeRunMode::AsynchronousCompilation)
)]
#[cfg_attr(
    feature = "cairo_native",
    case::cairo_native_and_wait(CairoNativeRunMode::SynchronousCompilation)
)]
fn test_get_compiled_class_without_native_in_cache(
    ...
    // allowed.
    #[cfg(not(feature = "cairo_native"))]
    assert_eq!(cairo_native_run_mode, CairoNativeRunMode::Off);
    ...
    match cairo_version {
        CairoVersion::Cairo1(_) => {
            ...
            if cairo_native_run_mode == CairoNativeRunMode::SynchronousCompilation {
                #[cfg(feature = "cairo_native")]
                assert_matches!(
                    compiled_class,
...
fn test_get_compiled_class_when_native_is_cached() {
    ...
    let test_class_hash = test_contract.get_class_hash();
    let contract_manager_config =
        ContractClassManagerConfig::create_for_testing(CairoNativeRunMode::SynchronousCompilation);
...
fn test_get_compiled_class_caching_scenarios(
    ...
    let contract_class_manager = ContractClassManager::start(ContractClassManagerConfig {
        cairo_native_run_config: CairoNativeRunConfig {
            cairo_native_run_mode: CairoNativeRunMode::Off,
            ..Default::default()
        },
        ..Default::default()
    });
