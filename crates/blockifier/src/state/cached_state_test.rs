...
fn test_state_cache_merge(
    ...
    assert_eq!(
        StateCache::squashed_state_diff(vec![state_cache1.clone(), state_cache2.clone()], true),
        state_cache1.to_state_diff()
    );
    assert_eq!(
        StateCache::squashed_state_diff(vec![state_cache2.clone(), state_cache1.clone()], true),
        state_cache1.to_state_diff()
    );
    ...
    assert_eq!(
        StateCache::squashed_state_diff(
            vec![state_cache1.clone(), state_cache2.clone(), state_cache3.clone()],
            true,
        ),
        state_changes_final
    );
    assert_ne!(
        StateCache::squashed_state_diff(vec![state_cache3, state_cache1, state_cache2], true),
        state_changes_final
    );
}
...
fn test_state_cache_commit_and_merge(
    ...
    let merged_changes = StateCache::squashed_state_diff(state_caches, comprehensive_state_diff);
...
fn test_allocated_keys_two_transactions(
    ...
    let merged_changes =
        StateCache::squashed_state_diff(vec![first_state_changes, second_state_changes], true);
    assert_ne!(merged_changes.allocated_keys.is_empty(), charged);
}
