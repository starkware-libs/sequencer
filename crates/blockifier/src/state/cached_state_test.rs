...
fn test_state_cache_merge(
    ...
    assert_eq!(
        StateCache::squashed_state_diff(vec![&state_cache1, &state_cache2]),
        state_cache1.to_state_diff()
    );
    assert_eq!(
        StateCache::squashed_state_diff(vec![&state_cache2, &state_cache1]),
        state_cache1.to_state_diff()
    );
    ...
    assert_eq!(
        StateCache::squashed_state_diff(vec![&state_cache1, &state_cache2, &state_cache3]),
        state_changes_final
    );
    assert_ne!(
        StateCache::squashed_state_diff(vec![&state_cache3, &state_cache1, &state_cache2]),
        state_changes_final
    );
}
...
fn test_state_cache_commit_and_merge(
    ...
    let merged_changes = StateCache::squashed_state_diff_backward_compatible(
        state_caches.iter().collect(),
...
fn test_allocated_keys_two_transactions(
    ...
    let merged_changes =
        StateCache::squashed_state_diff(vec![&first_state_changes, &second_state_changes]);
    assert_ne!(merged_changes.allocated_keys.is_empty(), charged);
}
