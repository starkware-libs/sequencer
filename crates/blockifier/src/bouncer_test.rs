...
fn class_hash_migration_data_from_state(
    ...
    let blake_opcode_gas = usize_from_u64(block_context.bouncer_config.builtin_weights.gas_costs.blake2s).unwrap();
...
fn get_tx_weights_applies_migration_gas_delta(
    ...
    let blake_opcode_gas = usize_from_u64(bc_migration_enabled.bouncer_config.builtin_weights.gas_costs.blake2s).unwrap();
