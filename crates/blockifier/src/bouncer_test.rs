...
#[fixture]
fn transactional_state(
    block_context: BlockContext,
) -> CachedState<crate::state::cached_state::MutRefState<'static, CachedState<DictStateReader>>> {
    let state: &'static mut CachedState<DictStateReader> =
        Box::leak(Box::new(test_state(&block_context.chain_info, Fee(0), &[])));
...
#[fixture]
fn execution_summary() -> ExecutionSummary {
...
fn test_block_weights_has_room_n_txs(
    ...
    assert_eq!(block_max_capacity.has_room(test_weights), has_room);
}
...
fn test_bouncer_try_update_sierra_gas(
    ...
    mut transactional_state: CachedState<
        crate::state::cached_state::MutRefState<'static, CachedState<DictStateReader>>,
    >,
) {
...
fn test_bouncer_try_update_n_txs(
    ...
    // Prepare the resources to be added to the bouncer.
    let tx_resources = TransactionResources::default();
    let tx_state_changes_keys = transactional_state.to_state_diff().unwrap().state_maps.keys();
