...
fn test_syscall_gas_cost_calculation() {
    ...
    assert_eq!(
        versioned_constants.get_syscall_gas_cost(&SyscallSelector::CallContract).base_cost,
        EXPECTED_CALL_CONTRACT_GAS_COST
    );
    assert_eq!(
        versioned_constants.get_syscall_gas_cost(&SyscallSelector::Secp256k1Mul).base_cost,
        EXPECTED_SECP256K1MUL_GAS_COST
    );
    assert_eq!(
        versioned_constants.get_syscall_gas_cost(&SyscallSelector::Sha256ProcessBlock).base_cost,
