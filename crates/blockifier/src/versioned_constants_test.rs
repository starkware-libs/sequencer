...
fn get_raw_os_constants_dict(version: StarknetVersion) -> IndexMap<String, Value> {
    let path_to_json = VersionedConstants::path_to_json(&version).unwrap();
    let mut raw_vc_string = String::new();
    std::fs::File::open(&path_to_json).unwrap().read_to_string(&mut raw_vc_string).unwrap();
