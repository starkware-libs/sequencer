...
fn test_string_inside_composed_field() {
    ...
    check_constants_serde_error(json_data, "invalid type: string \"meow\", expected u64");
}

#[cached]
fn get_raw_os_constants_dict() -> IndexMap<String, Value> {
    let raw_vc: RawVersionedConstants =
        serde_json::from_str(VERSIONED_CONSTANTS_V0_13_3_JSON).unwrap();
...
fn check_constants_serde_error(json_data: &str, expected_error_message: &str) {
    let mut os_constants_raw = get_raw_os_constants_dict();
    ...
    println!("DORI: {}", os_constants_raw.get("entry_point_initial_budget").unwrap());
...
fn test_missing_key() {
    ...
    check_constants_serde_error(json_data, "unknown field `TEN LI GAZ!");
}
...
fn test_unhandled_value_type() {
    ...
    check_constants_serde_error(json_data, "invalid type: sequence, expected u64");
}
...
fn test_invalid_number() {
    ...
    check_constants_serde_error(json_data, "invalid type: floating point `42.5`, expected u64");
}
