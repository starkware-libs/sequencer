...
impl TryFrom<&OsConstantsRawJson> for GasCosts {
    ...
    fn try_from(raw_json_data: &OsConstantsRawJson) -> Result<Self, Self::Error> {
        ...
        let syscalls_gas_costs_value : Value = serde_json::to_value(&raw_json_data.parse_syscalls_gas_costs(&base_gas_costs)?)?;
        let syscalls_gas_costs : SyscallsGasCosts = serde_json::from_value(syscalls_gas_costs_value)?;
        let gas_costs = GasCosts {
            base_gas_costs : base_gas_costs,
            syscalls_gas_costs : syscalls_gas_costs,
        };
        Ok(gas_costs)
    }
}
...
impl OsConstantsRawJson {
    ...
    fn parse_syscalls_gas_costs(&self, base_gas_costs: &BaseGasCosts) -> Result<IndexMap<String, u64>, OsConstantsSerdeError> {
        let mut gas_costs = IndexMap::new();
        let syscalls_gas_costs : IndexMap<String, Value> = serde_json::from_value((self.raw_json_file_as_dict.get("syscalls_gas_costs").unwrap()).clone())?;
        for (key, value) in syscalls_gas_costs {
            self.add_to_syscalls_gas_costs(&key, &value, &mut gas_costs, &base_gas_costs)?;
        }
        Ok(gas_costs)
    }
    ...
    fn add_to_syscalls_gas_costs (&self, key: &str,
        value: &Value,
        syscalls_gas_costs: &mut IndexMap<String, u64>, base_gas_costs: &BaseGasCosts) -> Result<(), OsConstantsSerdeError> {
            let mut cost = 0;
            match value {
                Value::Object(obj) => {
                    for (inner_key , factor) in obj {
                    ...
                    syscalls_gas_costs.insert(key.to_string(), cost);
                }
                Value::Number(n) => {
