...
impl OsConstantsRawJson {
    ...
    fn parse_syscalls(&self, base: &BaseGasCosts) -> Result<IndexMap<String, u64>, OsConstantsSerdeError> {
        let mut gas_costs = IndexMap::new();
        let syscalls : IndexMap<String, Value> = serde_json::from_value((self.raw_json_file_as_dict.get("syscalls_gas_costs").unwrap()).clone())?;
    ...
    fn add_to_syscalls (&self, key: &str,
        value: &Value,
        syscalls: &mut IndexMap<String, u64>, base: &BaseGasCosts) -> Result<(), OsConstantsSerdeError> {
            let mut cost = 0;
            match value {
                Value::Object(obj) => {
                    for (inner_key , factor) in obj {
