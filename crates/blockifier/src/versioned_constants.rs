...
macro_rules! define_versioned_constants {
    ($(($variant:ident, $path_to_json:expr)),*, $latest_variant:ident) => {
        ...
        impl StarknetVersion {
            pub(crate) fn path_to_versioned_constants_json(&self) -> &'static str {
                match self {
        ...
        pub(crate) static VERSIONED_CONSTANTS_LATEST_JSON: LazyLock<String> = LazyLock::new(|| {
            let latest_variant = StarknetVersion::$latest_variant;
            let path_to_json = latest_variant.path_to_versioned_constants_json();
            fs::read_to_string(path_to_json).expect(&format!("Failed to read file {path_to_json}."))
        });
    };
}
