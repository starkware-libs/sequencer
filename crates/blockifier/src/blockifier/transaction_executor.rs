...
pub(crate) fn finalize_block<S: StateReader>(
    ...
    insert_class_hash_migration_to_state(&lock_bouncer(bouncer), block_state);
...
// Gather the class hashes that need to be migrated and insert them into the class
// hash to compiled class hash mapping in the write set of the block state.
fn insert_class_hash_migration_to_state<S: StateReader>(
    bouncer: &Bouncer,
    block_state: &mut CachedState<S>,
) {
    let class_hash_to_casm_hash: HashMap<_, _> = bouncer
        ...
        .collect();
    block_state.cache.get_mut().writes.compiled_class_hashes.extend(class_hash_to_casm_hash);
}
