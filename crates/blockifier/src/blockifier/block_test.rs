...
fn compute_and_check_price_range(
    ...
    let price_change_ratio = new_price_f64 / prev_price_f64;
    // The max change is bounded by 1.0 / GAS_PRICE_MAX_CHANGE_DENOMINATOR.
    let max_change = 1.0 / f64::from(u32::try_from(GAS_PRICE_MAX_CHANGE_DENOMINATOR).unwrap());
    if should_increase {
        assert!((1.0..=1.0 + max_change).contains(&price_change_ratio));
    } else {
        assert!((1.0 - max_change..=1.0).contains(&price_change_ratio));
    }
    new_price
}
...
fn test_congestion_increases_price() {
    ...
    // Test a scenario where network congestion decreases, causing the gas used is below the target
    // gas.
...
fn test_reduced_gas_usage_decreases_price() {
    let mut prev_price = 1000000;
    let current_gas_target = MAX_BLOCK_SIZE / 2;
    // Test a scenario where network congestion increases, causing the gas used to exceed the target
    // gas.
    let gas_usages = [MAX_BLOCK_SIZE * 3 / 8, MAX_BLOCK_SIZE / 3];
...
fn test_stable_gas_usage() {
    let price = 1000000;
    let current_gas_target = MAX_BLOCK_SIZE / 2;
    let gas_used = MAX_BLOCK_SIZE / 2;

    assert_eq!(
        BlockInfo::calculate_next_base_gas_price(price, gas_used, current_gas_target),
        price
    );
}
...
fn test_gas_price_with_extreme_values() {
    ...
    assert!(BlockInfo::calculate_next_base_gas_price(price, gas_used, gas_target) > MIN_GAS_PRICE);
    ...
    let gas_target = MAX_BLOCK_SIZE / 2;
    let gas_used = MAX_BLOCK_SIZE;
    BlockInfo::calculate_next_base_gas_price(
        u64::try_from(price_u128).unwrap(),
        gas_used,
        gas_target,
    );
}
