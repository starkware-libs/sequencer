...
pub fn cairo1_compile(
    ...
    if let Some(cargo_arg) = additional_cargo_arg {
        base_compile_args.push(cargo_arg);
    }
    base_compile_args.push("run".into());
    base_compile_args
        .push(format!("--manifest-path={}/Cargo.toml", cairo1_compiler_path.to_string_lossy()));
    base_compile_args.push("--bin".into());
    ...
    sierra_compile_command.args(base_compile_args.clone());
