...
impl FeatureContract {
    ...
    fn all_contract_versions(&self) -> Vec<FeatureContract> {
        match self {
            ...
            | Self::TestContract(_)
            | Self::ERC20(_) => {
                // This would only work if the default version is Cairo0.
                assert!(self.cairo_version() == CairoVersion::Cairo0);
                let mut cairo1_contract = *self;
                cairo1_contract.set_cairo_version(CairoVersion::Cairo1(RunnableCairo1::Casm));
                let mut ret_val = vec![*self, cairo1_contract];
                #[cfg(feature = "cairo_native")]
                {
            ...
            Self::LegacyTestContract | Self::CairoStepsTestContract | Self::SecurityTests => vec![*self],
        }
    }
