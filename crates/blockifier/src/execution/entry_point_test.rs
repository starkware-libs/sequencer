...
/// Builds an execution context whose step limit is the block's `invoke_tx_max_n_steps`, the same
/// starting point `call_view_entry_point` caps.
...
/// The cap must never hand out more steps than the context already had. Replacing it with a plain
/// subtraction would do exactly that, and would silently widen the bound on a step-tracked view
/// call.
#[test]
...
/// The view call bounds are documented against the versioned constants: the step bound equals
/// `validate_max_n_steps`, the Sierra gas bound equals the current `validate_max_sierra_gas`, and
/// the two are equivalent budgets at the versioned `step_gas_cost`. Pin all three so that a future
/// version bumping any of them cannot silently invalidate the documentation.
#[test]
