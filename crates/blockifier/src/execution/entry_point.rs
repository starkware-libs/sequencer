    ...
    pub fn non_reverting_execute(
        ...
        if let Ok(call_info) = &execution_result {
            ...
            if call_info.execution.failed {
                context.update_revert_gas_with_next_remaining_gas(GasAmount(*remaining_gas));
...
impl SierraGasRevertTracker {
    ...
    /// remaining gas.
    pub fn update_with_next_remaining_gas(&mut self, next_remaining_gas: GasAmount) {
        // Only track sierra gas for reverts if tracking gas.
        ...
        self.sierra_gas_consumed_so_far =
            self.sierra_gas_consumed_so_far.checked_add(next_gas_consumed).unwrap_or_else(|| {
                panic!(
...
impl EntryPointExecutionContext {
    ...
    pub fn update_revert_gas_with_next_remaining_gas(&mut self, next_remaining_gas: GasAmount) {
        if self.tracked_resource_stack.last() == Some(&TrackedResource::SierraGas) {
            self.sierra_gas_revert_tracker.update_with_next_remaining_gas(next_remaining_gas);
        }
    }
