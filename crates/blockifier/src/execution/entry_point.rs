...
impl SierraGasRevertTracker {
    pub fn new(initial_remaining_gas: GasAmount) -> Self {
        Self { last_seen_remaining_gas: initial_remaining_gas }
    }
    ...
    pub fn get_gas_consumed(&self, initial_gas: GasAmount) -> GasAmount {
        initial_gas.checked_sub(self.last_seen_remaining_gas).unwrap_or_else(|| {
            panic!(
                "The consumed gas must be non-negative. Initial gas: {initial_gas}, last seen \
                 gas: {}.",
