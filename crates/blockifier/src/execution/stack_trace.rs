...
pub fn extract_trailing_cairo1_revert_trace(root_call: &CallInfo) -> String {
    ...
    while call.execution.failed {
        ...
        let expected_inner_retdata = retdata[..retdata.len() - 1].to_vec();
        let potential_inner_failures: Vec<&CallInfo> = call
            .inner_calls
            .iter()
            .filter(|inner_call| {
            ...
            .collect();
        call = match potential_inner_failures[..] {
            [unique_inner_failure] => unique_inner_failure,
