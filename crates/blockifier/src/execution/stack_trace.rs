...
pub fn extract_trailing_cairo1_revert_trace(root_call: &CallInfo) -> String {
    ...
    while call.execution.failed {
        ...
        // Note: at this point, we know `expected_retdata` is equal to the current call's retdata.
        // Popping the last element gives us the expected inner calldata (provided the last element
        // was the failure felt).
        if expected_retdata.pop() != Some(entrypoint_failed_felt) {
            break;
        }
        ...
        let mut potential_inner_failures_iter = call.inner_calls.iter().filter(|inner_call| {
            inner_call.execution.failed && *inner_call.execution.retdata.0 == expected_retdata
        });
