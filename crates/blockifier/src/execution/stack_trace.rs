...
impl Display for Cairo1RevertStack {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        if failure_reason.len()
            >= TRACE_LENGTH_CAP - *MIN_CAIRO1_FRAMES_STACK_LENGTH - header.len() - 1
        {
            let header_and_failure = format!("{}\n{}", header, failure_reason);
            let output = if header_and_failure.len() <= TRACE_LENGTH_CAP {
                header_and_failure
            } else {
                header_and_failure
                    .chars()
