...
fn test_range_check(runnable_version: RunnableCairo1) {
    ...
    pretty_assertions::assert_eq!(
        ...
        CallExecution { gas_consumed: 100, ..CallExecution::from_retdata(retdata![]) }
    );
}
...
fn test_pedersen(runnable_version: RunnableCairo1) {
    ...
    pretty_assertions::assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 7190, ..CallExecution::from_retdata(retdata![]) }
    );
}
...
fn test_bitwise(runnable_version: RunnableCairo1) {
    ...
    pretty_assertions::assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 3723, ..CallExecution::from_retdata(retdata![]) }
    );
}
...
fn test_ecop(runnable_version: RunnableCairo1) {
    ...
    pretty_assertions::assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 9425, ..CallExecution::from_retdata(retdata![]) }
    );
}
...
fn test_poseidon(runnable_version: RunnableCairo1) {
    ...
    pretty_assertions::assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 5431, ..CallExecution::from_retdata(retdata![]) }
    );
}

#[cfg_attr(feature = "cairo_native", test_case(RunnableCairo1::Native; "Native"))]
#[test_case(RunnableCairo1::Casm; "VM")]
fn test_add_and_mul_mod(runnable_version: RunnableCairo1) {
    let test_contract = FeatureContract::TestContract(CairoVersion::Cairo1(runnable_version));
    let chain_info = &ChainInfo::create_for_testing();
    let mut state = test_state(chain_info, BALANCE, &[(test_contract, 1)]);
    ...
    let calldata = Calldata(vec![].into());
    let entry_point_call = CallEntryPoint {
        entry_point_selector: selector_from_name("test_add_and_mul_mod"),
        calldata,
        ..trivial_external_entry_point_new(test_contract)
    };
    ...
    pretty_assertions::assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 21724, ..CallExecution::from_retdata(retdata![]) }
    );
}
