...
fn expected_block_info_as_felts(execution_mode: ExecutionMode) -> Vec<Felt> {
    let (block_number, block_timestamp, sequencer_address) = block_info_by_mode(execution_mode);
...
/// Test the get_execution syscall using contracts that calls the get_execution_info_syscall entry
/// point which uses the ExecutionInfoV1 cairo struct. This test is a legacy test meaning to check
