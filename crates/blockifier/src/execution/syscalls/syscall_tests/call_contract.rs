...
fn test_nested_call_storage_revert(runnable_version: RunnableCairo1) {
    ...
    assert!(
        call_info.get_sorted_events().is_empty(),
        "All events should be reverted across the call hierarchy"
    );
    assert!(
        call_info.get_sorted_l2_to_l1_messages().is_empty(),
        "All L1 messages should be reverted across the call hierarchy"
    );
}
