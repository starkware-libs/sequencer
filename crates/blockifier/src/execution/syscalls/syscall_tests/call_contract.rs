...
fn test_track_resources(
    ...
    assert_eq!(execution.tracked_resource, outer_version.into());

    let expected_inner_version =
        if outer_version == CairoVersion::Cairo0 && inner_version == CairoVersion::Cairo1 {
            CairoVersion::Cairo0
        } else {
            inner_version
        };
...
fn test_track_resources_nested() {
    ...
    let chain_info = &ChainInfo::create_for_testing();
    let mut state =
        test_state(chain_info, BALANCE, &[(cairo_1_contract, 3), (cairo_0_contract, 1)]);
    ...
    let second_calldata = create_calldata(
        cairo_1_contract.get_instance_address(1),
        "test_storage_read_write",
        &[
    ...
    assert_eq!(execution.tracked_resource, CairoVersion::Cairo1.into());
    let first_call_info = execution.inner_calls.first().unwrap();
    assert_eq!(first_call_info.tracked_resource, CairoVersion::Cairo0.into());
    assert_eq!(
        first_call_info.inner_calls.first().unwrap().tracked_resource,
        CairoVersion::Cairo0.into()
    );

    let second_inner_call_info = execution.inner_calls.get(1).unwrap();
    assert_eq!(second_inner_call_info.tracked_resource, CairoVersion::Cairo1.into());
}
