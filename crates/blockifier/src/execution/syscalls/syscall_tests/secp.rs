...
/// `secp_mul` producing the zero-x point. There is no feature-contract entry point that multiplies
/// a caller-provided point (only the generator), and reaching the zero-x point by multiplying the
/// generator would require a discrete log, so this drives the syscall handler directly: `p` is the
/// zero-x point halved (a valid point with x != 0), so `2 * p` lands on the zero-x point and must
/// be rejected by `allocate_point`.
#[test]
fn test_secp256r1_mul_zero_x_point_rejected() {
    ...
    let request = SecpMulRequest { ec_point_ptr: point_ptr, multiplier: BigUint::from(2_u8) };
    let error =
