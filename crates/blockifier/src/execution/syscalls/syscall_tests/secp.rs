...
/// `secp_mul` whose final result has x != 0 but whose OS computation hits the zero-x point as an
/// intermediate. `p` is the zero-x point halved (a valid point with x != 0), so `3 * p` has x != 0
/// (`allocate_point` would not reject it), yet the OS precompute table materializes `table[2] =
/// 2 * p =` the zero-x point. This must be rejected. There is no feature-contract entry point that
/// multiplies a caller-provided point (only the generator), and reaching this by multiplying the
/// generator would require a discrete log, so this drives the syscall handler directly.
#[test]
