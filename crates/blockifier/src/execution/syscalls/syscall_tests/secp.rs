...
use crate::execution::syscalls::secp::{SecpHintProcessor, SecpMulRequest};
...
/// Runs `entry_point` (with `calldata`) and asserts the secp256r1 point with x-coordinate 0 was
/// rejected with a hard error (not a catchable revert).
...
// The following tests verify that a secp256r1 point with x-coordinate 0 is rejected with a hard
// error across the secp256r1 syscalls. u256 arguments are passed as (low, high) 128-bit limbs.

/// The y-coordinate of a secp256r1 point with x-coordinate 0.
...
/// `secp256r1_add` that produces a point with x-coordinate 0.
#[cfg_attr(feature = "cairo_native", test_case(RunnableCairo1::Native; "Native"))]
#[test_case(RunnableCairo1::Casm; "VM")]
...
/// `secp_mul` of a point `p` (with x-coordinate != 0) that involves a point with x-coordinate 0
/// during the computation, even though the final result has x-coordinate != 0. Driven directly,
/// since no feature-contract entry point multiplies a caller-provided point.
#[test]
fn test_secp256r1_mul_zero_x_intermediate_rejected() {
    let parse = |hex: &str| BigUint::parse_bytes(hex.as_bytes(), 16).unwrap();
    let p = Affine::<ark_secp256r1::Config>::new_unchecked(
        parse("81bfb55b010b1bdf08b8d9d8590087aa278e28febff3b05632eeff09011c5579").into(),
        parse("8cd2f199d9815d7585073034eb76c93d50799b354b0fb1e77eb75eba8bff3d58").into(),
    );
    let mut secp_processor = SecpHintProcessor::<ark_secp256r1::Config>::new();
    let point_ptr = Relocatable::from((1, 0));
    secp_processor.points.insert(point_ptr, p);

    let mut vm = VirtualMachine::new(false, false);
    let request = SecpMulRequest { ec_point_ptr: point_ptr, multiplier: BigUint::from(3_u8) };
    let error =
        secp_processor.secp_mul(request, &mut vm, Relocatable::from((2, 0)), 0).unwrap_err();
    assert!(
        error.to_string().contains("secp256r1 points with x-coordinate 0 are not allowed"),
        "Unexpected error: {error}"
    );
