...
fn disable_deploy_in_validate_mode_flag_behavior(
    ...
    // Confirm deploy is disabled by default before override.
    assert_deploy_disabled_in_validation_mode();
...
/// Asserts that deploy is disabled by default in validate execution mode.
fn assert_deploy_disabled_in_validation_mode() {
    let latest = VersionedConstants::latest_constants();
    assert!(
        latest.disable_deploy_in_validation_mode,
        "Deploy should be disabled by default in validate execution mode."
    );
}
