    ...
    pub fn execute_next_syscall(
        ...
        match selector {
            SyscallSelector::CallContract => {
                self.execute_syscall(vm, call_contract, self.gas_costs().syscall_gas_costs.call_contract)
            }
            SyscallSelector::Deploy => {
                self.execute_syscall(vm, deploy, self.gas_costs().syscall_gas_costs.deploy)
            }
            SyscallSelector::EmitEvent => {
                self.execute_syscall(vm, emit_event, self.gas_costs().syscall_gas_costs.emit_event)
            }
            SyscallSelector::GetBlockHash => {
                self.execute_syscall(vm, get_block_hash, self.gas_costs().syscall_gas_costs.get_block_hash)
            }
            SyscallSelector::GetClassHashAt => self.execute_syscall(
                vm,
                get_class_hash_at,
                self.gas_costs().syscall_gas_costs.get_class_hash_at,
            ),
            SyscallSelector::GetExecutionInfo => self.execute_syscall(
                vm,
                get_execution_info,
                self.gas_costs().syscall_gas_costs.get_execution_info,
            ),
            SyscallSelector::Keccak => {
                self.execute_syscall(vm, keccak, self.gas_costs().syscall_gas_costs.keccak)
            }
            SyscallSelector::Sha256ProcessBlock => self.execute_syscall(
                vm,
                sha_256_process_block,
                self.gas_costs().syscall_gas_costs.sha256_process_block,
            ),
            SyscallSelector::LibraryCall => {
                self.execute_syscall(vm, library_call, self.gas_costs().syscall_gas_costs.library_call)
            }
            SyscallSelector::ReplaceClass => {
                self.execute_syscall(vm, replace_class, self.gas_costs().syscall_gas_costs.replace_class)
            }
            SyscallSelector::Secp256k1Add => {
                self.execute_syscall(vm, secp256k1_add, self.gas_costs().syscall_gas_costs.secp256k1_add)
            }
            SyscallSelector::Secp256k1GetPointFromX => self.execute_syscall(
                vm,
                secp256k1_get_point_from_x,
                self.gas_costs().syscall_gas_costs.secp256k1_get_point_from_x,
            ),
            SyscallSelector::Secp256k1GetXy => self.execute_syscall(
                vm,
                secp256k1_get_xy,
                self.gas_costs().syscall_gas_costs.secp256k1_get_xy,
            ),
            SyscallSelector::Secp256k1Mul => {
                self.execute_syscall(vm, secp256k1_mul, self.gas_costs().syscall_gas_costs.secp256k1_mul)
            }
            SyscallSelector::Secp256k1New => {
                self.execute_syscall(vm, secp256k1_new, self.gas_costs().syscall_gas_costs.secp256k1_new)
            }
            SyscallSelector::Secp256r1Add => {
                self.execute_syscall(vm, secp256r1_add, self.gas_costs().syscall_gas_costs.secp256r1_add)
            }
            SyscallSelector::Secp256r1GetPointFromX => self.execute_syscall(
                vm,
                secp256r1_get_point_from_x,
                self.gas_costs().syscall_gas_costs.secp256r1_get_point_from_x,
            ),
            SyscallSelector::Secp256r1GetXy => self.execute_syscall(
                vm,
                secp256r1_get_xy,
                self.gas_costs().syscall_gas_costs.secp256r1_get_xy,
            ),
            SyscallSelector::Secp256r1Mul => {
                self.execute_syscall(vm, secp256r1_mul, self.gas_costs().syscall_gas_costs.secp256r1_mul)
            }
            SyscallSelector::Secp256r1New => {
                self.execute_syscall(vm, secp256r1_new, self.gas_costs().syscall_gas_costs.secp256r1_new)
            }
            SyscallSelector::SendMessageToL1 => self.execute_syscall(
                vm,
                send_message_to_l1,
                self.gas_costs().syscall_gas_costs.send_message_to_l1,
            ),
            SyscallSelector::StorageRead => {
                self.execute_syscall(vm, storage_read, self.gas_costs().syscall_gas_costs.storage_read)
            }
            SyscallSelector::StorageWrite => {
                self.execute_syscall(vm, storage_write, self.gas_costs().syscall_gas_costs.storage_write)
            }
