...
impl<'a> SyscallHintProcessor<'a> {
    ...
    pub fn get_or_allocate_execution_info_segment(
        ...
        let exclude_l1_data_gas = self.base.should_exclude_l1_data_gas();

        // If the transaction version was overridden, or we should exclude l1_data_gas,
        // `self.execution_info_ptr` cannot be used.
        if returned_version != original_version || exclude_l1_data_gas {
