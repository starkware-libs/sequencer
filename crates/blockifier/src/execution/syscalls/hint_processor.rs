...
pub fn execute_inner_call(
    ...
    let mut revert_info = std::mem::take(&mut syscall_handler.context.revert_info);

    let call_info =
        call.execute(syscall_handler.state, syscall_handler.resources, syscall_handler.context)?;

    if call_info.execution.failed {
        syscall_handler.context.revert_info.revert(syscall_handler.state)?;
        syscall_handler.context.revert_info = revert_info;
    } else {
        revert_info
            .contract_revert_info
        ...
        for (key, value) in revert_info.contract_revert_info.into_iter() {
            match syscall_handler.context.revert_info.contract_revert_info.entry(key) {
                std::collections::hash_map::Entry::Occupied(mut entry) => {
                    entry.get_mut().original_class_hash = value.original_class_hash;
                    entry.get_mut().orig_values.extend(value.orig_values);
                }
