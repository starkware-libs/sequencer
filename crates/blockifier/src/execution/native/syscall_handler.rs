...
fn u256_to_big4int(u256: U256) -> BigInt<4> {
    fn to_u64s(bytes: [u8; 16]) -> (u64, u64) {
        let hi_bytes: [u8; 8] = bytes[0..8].try_into().expect("Take high bytes");
        let hi: u64 = u64::from_le_bytes(hi_bytes);
        let lo_bytes: [u8; 8] = bytes[8..16].try_into().expect("Take low bytes");
        let lo: u64 = u64::from_le_bytes(lo_bytes);
        (hi, lo)
    }
    let (hi_hi, hi_lo) = to_u64s(u256.hi.to_le_bytes());
    let (lo_hi, lo_lo) = to_u64s(u256.lo.to_le_bytes());
