    ...
    fn pre_execute_syscall(
        ...
        let actual_syscall_gas_cost = match syscall_gas_cost {
            SyscallGasCost::Constant(base_cost) => {
                assert!(
                    linear_coefficient_cost_param.is_none(),
                    "Unexpected linear coefficient cost param for constant gas cost"
                );
                base_cost
            }
            SyscallGasCost::Linear { base_cost, linear_factor } => {
                assert!(
                    linear_coefficient_cost_param.is_some(),
                    "Missing linear coefficient cost param for linear gas cost"
                );
...
impl StarknetSyscallHandler for &mut NativeSyscallHandler<'_> {
    ...
    fn deploy(
        ...
        self.pre_execute_syscall(
            remaining_gas,
            self.gas_costs().syscalls.deploy,
            Some(u64_from_usize(calldata.len())),
        )?;
