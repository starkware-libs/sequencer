...
impl ContractClass {
    ...
    /// Returns whether this contract should run using VM resources or Sierra gas.
...
impl ContractClassV1 {
    ...
    pub fn run_resource(&self, min_sierra_version: &Version) -> RunResource {
        ...
        if res.is_ok_and(|version| &version <= min_sierra_version) {
            ...
            // If version is not a valid semver, fall back to VM execution mode.
            RunResource::CairoSteps
        }
    }
