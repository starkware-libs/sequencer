//! A "shadow" of the OS secp256r1 EC routines, used only to reject transactions the OS cannot
//! prove. It is intentionally a close mirror of `starkware/cairo/common/secp256r1/ec.cairo` so the
//! two can be reviewed side by side.
//!
//! Background: the OS secp helpers treat any point with `x == 0` as the point at infinity (see
//! `ec_double` / `fast_ec_add` in the cairo file). secp256r1 has a *valid* affine point with
//! `x == 0` and `y != 0`, so for inputs that make the OS encounter such a point it diverges from
//! blockifier's real group arithmetic and the block becomes unprovable. The point-allocation check
//! (`secp::reject_zero_x_point`) catches such a point only when it is a syscall *result*; it cannot
//! see points internal to the OS's `ec_add` / `ec_mul` (e.g. an entry of the `ec_mul` precompute
//! table when multiplying `(zero-x point) / 2`).
//!
//! The functions below re-walk the exact control flow of the cairo routines — computing the real
//! points with `ark` purely to follow the same sequence of operations — and reject if the OS would
//! feed a non-infinity `x == 0` point into `ec_double` / `fast_ec_add`. They never feed back into
//! the syscall's result: blockifier's arithmetic is left untouched, and the only effect is a (rare)
//! rejection. Each `fn` here mirrors the cairo `func` of the same name; keep them in sync.

use ark_ec::short_weierstrass::{Affine, Projective, SWCurveConfig};
use ark_ff::{PrimeField, Zero};
use num_bigint::BigUint;

use crate::execution::secp::{reject_zero_x_point, SecpZeroXPolicy};
...
/// Whether `point` is a non-infinity point with affine `x == 0` — the point the OS mistakes for the
/// point at infinity. Tested on the projective `X` coordinate to avoid an inversion: the affine `x`
/// is `X` times a nonzero factor, so it is zero iff `X == 0` and the point is not at infinity.
fn is_zero_x_point<Curve: SWCurveConfig>(point: &Projective<Curve>) -> bool {
    !point.is_zero() && point.x.is_zero()
}

/// Rejects `point` if the OS would feed it into `ec_double` / `fast_ec_add` as a non-infinity
/// `x == 0` operand.
    ...
    Curve::BaseField: PrimeField,
{
    if is_zero_x_point(point) {
        // Re-derive the affine point only on the (rare) rejection path, reusing the shared error.
        return reject_zero_x_point(&Affine::from(*point));
    }
    Ok(())
}

/// Mirror of `ec_double`: the cairo routine mistreats an `x == 0` operand as the point at infinity.
...
/// Mirror of `fast_ec_add`: the cairo routine checks both operands for the point at infinity (which
/// it identifies by `x == 0`), so reject a non-infinity `x == 0` operand of either.
fn fast_ec_add<Curve: SWCurveConfig + SecpZeroXPolicy>(
...
/// Mirror of `ec_add`. Only its operands need checking: each branch (`fast_ec_add`, `ec_double`, or
/// the point at infinity) operates on `point0` / `point1`, and the point at infinity is naturally
/// distinguished from the valid `x == 0` point in projective form (`Z == 0` vs `X == 0`), so it is
/// not rejected.
    ...
    Curve::BaseField: PrimeField,
{
    ...
    for i in 3..16 {
        table[i] = fast_ec_add(table[i - 1], point)?;
    }
    Ok(table)
}
    ...
    Curve::BaseField: PrimeField,
{
    for nibble in nibbles {
        ...
        point = fast_ec_add(point, table[nibble])?;
    }
    Ok(point)
}
...
/// Rejects a `secp256r1_add(point0, point1)` syscall that would make the OS encounter a
/// non-infinity `x == 0` point. No-op for curves without [`SecpZeroXPolicy::REJECT_ZERO_X_POINT`].
...
/// Rejects a `secp256r1_mul(point, scalar)` syscall that would make the OS encounter a non-infinity
/// `x == 0` point in an intermediate (which the point-allocation check never sees). No-op for
/// curves without [`SecpZeroXPolicy::REJECT_ZERO_X_POINT`].
...
mod tests {
    ...
    /// The generator (no small multiple of which is the zero-x point).
    ...
    /// mul rejects when the OS precompute table would materialize the zero-x point, even though the
    /// final result has x != 0.
    #[test]
    fn mul_rejects_zero_x_in_precompute_table() {
        // P/2: table[2] = 2 * (P/2) = (0, z).
        ...
        // P/4: table[4] = (0, z) (table[1..=3] have x != 0).
    ...
    /// mul rejects when the zero-x point appears only in the windowed accumulator (no table entry
    /// is the zero-x point).
    #[test]
    fn mul_rejects_zero_x_in_accumulator() {
        // P/16: no table entry is the zero-x point, but 16 * (P/16) = (0, z) appears in the
        // accumulator. With nibble 63 == 1 the accumulator's first window reaches it.
    ...
    /// mul accepts a multiplication whose intermediates never hit the zero-x point.
    #[test]
    fn mul_accepts_points_without_zero_x_intermediate() {
        assert!(reject_zero_x_in_mul(&generator(), &BigUint::from(5_u32)).is_ok());
    }

    /// add rejects a zero-x operand but not regular points.
    #[test]
    fn add_rejects_only_zero_x_operand() {
