...
// A wrapper for execute_entry_point_call that handles the context.tracked_resource field.
pub fn execute_entry_point_call_wrapper(
    ...
    if context.tracked_resource.last().is_some_and(|x| *x == TrackedResource::CairoSteps) {
        context.tracked_resource.push(TrackedResource::CairoSteps);
    } else {
        context.tracked_resource.push(tracked_resource);
    }

    let res = execute_entry_point_call(call, contract_class, state, resources, context);
    context.tracked_resource.pop();
    res
}
