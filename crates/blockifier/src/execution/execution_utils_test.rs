...
fn test_zero_inputs() {
    ...
    let (resources, blake_opcode_count) = cost_of_encode_felt252_data_and_calc_blake_hash(0, 0);
    let expected_resources =
        ExecutionResources { n_steps: STEPS_EMPTY_INPUT, ..Default::default() };
    assert_eq!(resources, expected_resources, "Unexpected gas value for zero-input hash");
