...
impl EntryPointExecutionError {
    ...
    pub fn annotated_no_vm_frame(self) -> AnnotatedEntryPointExecutionError {
        debug_assert!(
            !matches!(self, Self::CairoRunError(_)),
            "annotated_no_vm_frame called on CairoRunError; use .annotated(tr, strip)"
        );
...
/// `EntryPointExecutionError::annotated[_no_vm_frame]` (private fields enforce this), so
/// stack-trace-feeding fields typed `AnnotatedEntryPointExecutionError` statically reject
/// unannotated errors.
#[derive(Debug, Error)]
#[error("{inner}")]
