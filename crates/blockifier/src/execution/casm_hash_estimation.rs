...
impl EstimateCasmHashResources for CasmV2HashResourceEstimate {
    ...
    // V2 (Blake)-specific per-entry-point correction, fit empirically across the feature-contract
    // suite so the generic estimate (which is near-exact for V1/Poseidon) tracks the Blake
    // `compiled_class_hash` step count. The Blake hashing path carries a per-entry-point overhead
    // that the structure-driven estimate alone undercounts; without it the under-estimate grows
    // with the number of entry points. A small constant per-call offset remains and is absorbed by
    // the test margin.
