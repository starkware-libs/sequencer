...
pub trait EstimateCasmHashResources {
    // Estimated fixed Cairo steps executed in `bytecode_hash_internal_node` leaf case.
    ...
    fn estimated_resources_of_bytecode_hash_node(
        ...
        // Additional estimated fixed Cairo steps executed in `bytecode_hash_node` node case:
