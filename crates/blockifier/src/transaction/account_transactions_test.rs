...
#[rstest]
fn test_rc96_holes(block_context: BlockContext, max_l1_resource_bounds: ValidResourceBounds) {
    let test_contract = FeatureContract::TestContract(CairoVersion::Cairo1);
    let account = FeatureContract::AccountWithoutValidations(CairoVersion::Cairo1);
    let chain_info = &block_context.chain_info;
    let state = &mut test_state(chain_info, BALANCE, &[(test_contract, 1), (account, 1)]);
...
fn test_fee_enforcement(
    ...
    assert_eq!(result.is_err(), charge_fee);
}
...
fn test_max_fee_to_max_steps_conversion(
    ...
    let tx_execution_info1 = account_tx1.execute(&mut state, &block_context, charge_fee1, true).unwrap();
    ...
    let tx_execution_info2 = account_tx2.execute(&mut state, &block_context, charge_fee2, true).unwrap();
