...
fn invalid_program_hash_proof_facts() -> ProofFacts {
    let mut snos_proof_facts = SnosProofFacts::try_from(ProofFacts::snos_proof_facts_for_testing())
        .expect("Invalid proof facts");
...
/// current block too small, zero block hash, and invalid program hash.
#[rstest]
...
#[case::invalid_program_hash(invalid_program_hash_proof_facts(), CURRENT_BLOCK_NUMBER)]
