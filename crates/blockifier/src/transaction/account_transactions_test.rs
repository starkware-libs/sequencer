...
fn test_reverted_reach_computation_limit(
    ...
    let TestInitData { mut state, account_address, contract_address, mut nonce_manager } =
        create_test_init_data(&block_context.chain_info, cairo_version);
    let tracked_resource = match cairo_version {
        CairoVersion::Cairo0 => TrackedResource::CairoSteps,
        CairoVersion::Cairo1(_) => TrackedResource::SierraGas,
    };

    // Limit the max amount of computation (so we quickly hit the limit).
    ...
    assert_eq!(
        ...
        validate_tracked_resource == TrackedResource::CairoSteps
    );
    ...
    // If this fail, try to increase the max computation limit on the block context above.
...
fn test_n_reverted_steps(
    ...
    match tracked_resource {
        TrackedResource::CairoSteps => {
            ...
            actual_resources_0.vm_resources.n_steps = n_units_0;
        }
        TrackedResource::SierraGas => {
            actual_resources_0.reverted_sierra_gas.0 +=
                u64::try_from(single_call_units_delta).unwrap();
            assert_eq!(actual_resources_0, actual_resources_1);
            actual_resources_0.sierra_gas.0 = u64::try_from(n_units_0).unwrap();
        }
    };
...
fn test_insufficient_max_fee_reverts(
    ...
    match (cairo_version, resource_used_depth1) {
        ...
        (CairoVersion::Cairo1(RunnableCairo1::Casm), ValidResourceBounds::L1Gas(l1_bound)) => {
            // If a user supplied L1 gas bounds only, sierra gas is not limitted in proportion to
            // the user bound; so we expect to revert in post-execution only.
            assert!(
                tx_execution_info3
                    .revert_error
                    .unwrap()
