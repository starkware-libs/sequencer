...
fn test_assert_actual_fee_in_bounds(
    ...
    if deprecated_tx {
        ...
        let result = AccountTransaction::ensure_actual_fee_in_bounds(&context, max_fee + actual_fee_offset);
        assert_eq!(result.is_ok(), positive_flow);
    } else {
        // All resources.
            ...
            [(all_resource_bounds, all_resource_fee), (l1_resource_bounds, l1_resource_fee)]
        {
            ...
            let context = Arc::new(block_context.to_tx_context(&tx));
            let result = AccountTransaction::ensure_actual_fee_in_bounds(&context, actual_fee);
            assert_eq!(result.is_ok(), positive_flow);
        }
    }
}
