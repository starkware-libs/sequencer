...
impl StarknetResources {
    ...
    pub fn get_l2_archival_data_cost(
        ...
        let l2_resources_in_l2_gas = self.get_calldata_and_signature_cost(versioned_constants)
            + self.get_code_cost(versioned_constants)
            + self.get_events_cost(versioned_constants);
        match mode {
            GasVectorComputationMode::All => GasVector::from_l2_gas(l2_resources_in_l2_gas),
            GasVectorComputationMode::NoL2Gas => GasVector::from_l1_gas(
                versioned_constants.convert_l2_to_l1_gas_price(l2_resources_in_l2_gas),
            ),
        }
    }
    ...
    pub fn get_events_cost(&self, versioned_constants: &VersionedConstants) -> u128 {
        let l2_resource_gas_costs = &versioned_constants.archival_data_gas_costs;
