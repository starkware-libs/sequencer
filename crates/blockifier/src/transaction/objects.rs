...
impl StarknetResources {
    ...
    pub fn get_l2_archival_data_cost(
        ...
        // Cost in L2 gas.
        let calldata_and_signature_cost = self.get_calldata_and_signature_cost(versioned_constants);
        let code_cost = self.get_code_cost(versioned_constants);
        let events_cost = self.get_events_cost(versioned_constants);
        match mode {
            GasVectorComputationMode::All => {
                GasVector::from_l2_gas(calldata_and_signature_cost + code_cost + events_cost)
            }
            GasVectorComputationMode::NoL2Gas => GasVector::from_l1_gas(
                versioned_constants.convert_l2_to_l1_gas(calldata_and_signature_cost)
                    + versioned_constants.convert_l2_to_l1_gas(code_cost)
                    + versioned_constants.convert_l2_to_l1_gas(events_cost),
            ),
        }
    }
