...
use starknet_api::transaction::fields::Resource::L1Gas;
...
use crate::utils::get_steps_from_sierra_gas;
...
impl<U: UpdatableState> ExecutableTransaction<U> for L1HandlerTransaction {
    fn execute_raw(
        ...
        let mut initial_gas = l1_handler_bounds.l2_gas.0;
        ...
        // Manually set the steps limit to match the l2 gas limit.
        let vm_steps = get_steps_from_sierra_gas(initial_gas, step_gas_cost);

        context.vm_run_resources = RunResources::new(vm_steps);

        // TODO(AvivG): Refactor OS to match update: initial_gas=l1_handler_bounds.l2_gas.0
        let execute_call_info = self.run_execute(state, &mut context, &mut initial_gas)?;
        ...
        // TODO(AvivG): Enforce for L2_gas & L1_data_gas as well.
        // Enforce L1 resource bounds.
        let l1_bounds = l1_handler_bounds.l1_gas;
        if receipt.gas.l1_gas > l1_bounds {
            let err = FeeCheckError::MaxGasAmountExceeded {
                resource: L1Gas,
                max_amount: l1_bounds,
                actual_amount: receipt.gas.l1_gas,
            };
            return Err(err.into());
        };
