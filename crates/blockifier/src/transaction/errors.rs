...
pub enum TransactionFeeError {
    ...
    #[error(
        "Gas bounds (max amount: {max_amount}, max price: {max_price}) exceed balance ({balance})."
    )]
    ...
    #[error("GasBounds: Max fee ({}) exceeds balance ({balance}).", max_fee.0, )]
    ...
    #[error(
        "Max gas price ({max_gas_price}) is lower than the actual gas price: {actual_gas_price}."
    )]
