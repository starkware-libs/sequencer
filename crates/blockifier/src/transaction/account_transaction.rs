...
use crate::execution::alias_keys::n_charged_storage_keys;
...
impl AccountTransaction {
    ...
    fn run_non_revertible<S: StateReader>(
        ...
        let n_allocated_aliases =
            self.n_charged_aliases(tx_context.clone(), state, &state_changes.0.storage)?;
        ...
        match execution_result {
            Ok(execute_call_info) => {
                ...
                let n_allocated_aliases = self.n_charged_aliases(
                    tx_context.clone(),
                    &execution_state,
                    &execution_state_changes.0.storage,
                )?;
    ...
    fn n_charged_aliases<S: StateReader>(
        ...
        if tx_context.as_ref().block_context.versioned_constants.enable_stateful_compression {
            Ok(match self {
                ...
                Self::Invoke(_) => n_charged_storage_keys(state, storage_changes)?,
            })
        } else {
            Ok(0)
        }
