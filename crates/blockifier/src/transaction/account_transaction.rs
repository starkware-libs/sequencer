...
pub struct AccountTransaction {
    ...
    pub tx: starknet_api::executable_transaction::Transaction,
    _only_query: bool,
}
...
impl From<starknet_api::executable_transaction::Transaction> for AccountTransaction {
    fn from(executable_transaction: starknet_api::executable_transaction::Transaction) -> Self {
        AccountTransaction::new( executable_transaction)
        }
    }
...
impl HasRelatedFeeType for AccountTransaction {
    fn version(&self) -> TransactionVersion {
        match &self.tx {
            Transaction::Declare(executable_tx) => executable_tx.tx.version(),
            Transaction::DeployAccount(executable_tx) => executable_tx.tx.version(),
            Transaction::Invoke(executable_tx) => executable_tx.tx.version(),
        }
    }
...
impl AccountTransaction {
    ...
    pub fn new(tx: starknet_api::executable_transaction::Transaction) -> Self {
        AccountTransaction { tx, _only_query: false }
    }

    pub fn new_for_query(tx: starknet_api::executable_transaction::Transaction) -> Self {
        AccountTransaction { tx, _only_query: true }
    }
    ...
    pub fn calldata(&self) -> Calldata {
        match &self.tx {
            Transaction::Declare(_tx) => calldata![],
    ...
    pub fn tx_hash(&self) -> TransactionHash {
        match &self.tx {
            Transaction::Declare(tx) => tx.tx_hash(),
            Transaction::DeployAccount(tx) => tx.tx_hash(),
            Transaction::Invoke(tx) => tx.tx_hash(),
        }
    }
    ...
    pub(crate) fn declare_code_size(&self) -> usize {
        if let Transaction::Declare(tx) = &self.tx { tx.class_info.code_size() } else { 0 } //aviv: TODO related to Noa's enum solution
    }
...
impl TransactionInfoCreator for AccountTransaction {
    fn create_tx_info(&self) -> TransactionInfo {
        match &self.tx {
            Transaction::Declare(tx) => tx.create_tx_info(),
            Transaction::DeployAccount(tx) => tx.create_tx_info(),
            Transaction::Invoke(tx) => tx.create_tx_info(),
        }
    }
}
