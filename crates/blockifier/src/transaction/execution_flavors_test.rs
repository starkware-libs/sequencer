...
fn get_pre_validate_test_args(
    ...
    let resource_bounds =
        l1_resource_bounds(u128::from(MAX_L1_GAS_AMOUNT).into(), MAX_L1_GAS_PRICE);
...
fn test_simulate_validate_pre_validate_with_charge_fee(
    ...
    if !is_deprecated {
        let err = account_invoke_tx(invoke_tx_args! {
            resource_bounds: l1_resource_bounds(u128::from(MAX_L1_GAS_AMOUNT).into(), u128::from(gas_price) - 1),
            nonce: nonce_manager.next(account_address),
...
fn test_simulate_validate_pre_validate_not_charge_fee(
    ...
    if !is_deprecated {
        execute_and_check_gas_and_fee!(
            ...
            l1_resource_bounds(u128::from(MAX_L1_GAS_AMOUNT).into(), u128::from(gas_price) - 1)
        );
    }
}
