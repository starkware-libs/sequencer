...
fn gas_and_fee(base_gas: u64, validation_overhead: bool, fee_type: &FeeType) -> (u64, Fee) {
    ...
    let gas = base_gas + if validation_overhead { VALIDATE_GAS_OVERHEAD } else { 0 };
    (
        gas,
...
fn calculate_actual_gas(
    ...
    tx_execution_info
        ...
        .unwrap()
        .l1_gas
        - if validation_overhead { VALIDATE_GAS_OVERHEAD.into() } else { 0 }
}
...
fn check_gas_and_fee(
    ...
    assert_eq!(calculate_actual_gas(tx_execution_info, block_context, false), expected_actual_gas.into());
...
fn test_simulate_validate_pre_validate_not_charge_fee(
    ...
    .unwrap();
    let base_gas = calculate_actual_gas(&tx_execution_info, &block_context, false).try_into().unwrap();
    assert!(
        base_gas
            > u64_from_usize(
...
fn test_simulate_charge_fee_no_validation_fail_validate(
    ...
    let block_context = BlockContext::create_for_account_testing();
    let base_gas =
        calculate_actual_gas(&transaction_execution_info, &block_context, validate).try_into().unwrap();
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    let (gas_bound, fee_bound) = gas_and_fee(6107, validate, &fee_type);
    ...
    let (limited_gas_used, limited_fee) = gas_and_fee(7759, validate, &fee_type);
