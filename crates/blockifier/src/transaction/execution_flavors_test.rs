...
fn test_simulate_validate_pre_validate_with_charge_fee(
    ...
    let err = account_invoke_tx(invoke_tx_args! {
        max_fee: Fee(10),
        resource_bounds: l1_resource_bounds(GasAmount(10), GasPrice(10)),
    ...
    let result = account_invoke_tx(invoke_tx_args! {
        ...
        resource_bounds: l1_resource_bounds(
            GasAmount(balance_over_gas_price.0 + 10),
            gas_price.into()
        ),
        nonce: nonce_manager.next(account_address),
        ..pre_validation_base_args.clone()
...
fn test_simulate_validate_pre_validate_not_charge_fee(
    ...
    // First scenario: minimal fee not covered. Actual fee is precomputed.
    execute_and_check_gas_and_fee!(Fee(10), l1_resource_bounds(GasAmount(10), GasPrice(10)));
    ...
    execute_and_check_gas_and_fee!(
        Fee(BALANCE.0 + 1),
        l1_resource_bounds(GasAmount(balance_over_gas_price.0 + 10), gas_price.into())
    );
    ...
    if !is_deprecated {
        execute_and_check_gas_and_fee!(
            ...
            l1_resource_bounds(MAX_L1_GAS_AMOUNT, GasPrice(gas_price.get().0 - 1))
        );
    }
}
...
fn test_simulate_charge_fee_no_validation_fail_validate(
    ...
    assert!(
        base_gas
            > GasAmount(u64_from_usize(get_tx_resources(TransactionType::InvokeFunction).n_steps))
    );
    let (actual_gas_used, actual_fee) = gas_and_fee(base_gas, validate, &fee_type);
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    let block_limit_gas = GasAmount(invoke_tx_max_n_steps_as_u64 + 1652);
...
fn test_simulate_validate_charge_fee_post_execution(
    ...
    let (revert_gas_usage, revert_fee) = gas_and_fee(
        GasAmount(u64_from_usize(get_tx_resources(TransactionType::InvokeFunction).n_steps) + 5730),
        validate,
        &fee_type,
    );
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(
    ...
    let (fail_actual_gas, fail_actual_fee) = gas_and_fee(
        GasAmount(u64_from_usize(get_tx_resources(TransactionType::InvokeFunction).n_steps + 2252)),
        validate,
        &fee_type,
    );
