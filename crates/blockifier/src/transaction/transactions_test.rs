...
use crate::execution::stack_trace::{EntryPointErrorFrame, ErrorStack, ErrorStackSegment};
...
fn test_l1_handler(#[values(false, true)] use_kzg_da: bool) {
    ...
    let ErrorStack { stack: mut error_stack, .. } = assert_matches!(
        execution_info,
        TransactionExecutionInfo {
            validate_call_info: None,
            execute_call_info: None,
            fee_transfer_call_info : None,
            revert_error: Some(RevertError::Execution(error_stack)),
            receipt: TransactionReceipt { fee, .. },
        }
        if fee == Fee(0)
        => error_stack
    );
    assert_eq!(error_stack.len(), 2);
    let cairo_1_revert_summery =
        error_stack.pop().expect("Expected at least two elements in the error stack");
