...
fn test_l1_handler(#[values(false, true)] use_kzg_da: bool) {
    ...
    let expected_call_info = CallInfo {
        call: CallEntryPoint {
            ...
            caller_address: ContractAddress::default(),
            call_type: CallType::Call,
            initial_gas: versioned_constants.os_constants.l1handler_resource_bounds.l2_gas.0,
        },
        execution: CallExecution {
...
fn set_resource_bound(block_context: &mut BlockContext, resource: Resource, new_bound: GasAmount) {
    let os_constants = Arc::make_mut(&mut block_context.versioned_constants.os_constants);
    match resource {
        L1Gas => os_constants.l1handler_resource_bounds.l1_gas = new_bound,
        L2Gas => os_constants.l1handler_resource_bounds.l2_gas = new_bound,
        L1DataGas => os_constants.l1handler_resource_bounds.l1_data_gas = new_bound,
    }
}

/// Executes an L1 handler transaction using the current environment,
/// after modifying the given resource bound. Returns the resulting error.
fn execute_l1_handler_with_bound(
    resource: Resource,
    new_bound: GasAmount,
) -> TransactionExecutionError {
    let test_contract = FeatureContract::TestContract(CairoVersion::Cairo1(RunnableCairo1::Casm));
    let mut block_context = BlockContext::create_for_account_testing_with_kzg(false);
    let chain_info = block_context.chain_info.clone();
    let mut state = test_state(&chain_info, BALANCE, &[(test_contract, 1)]);
    let contract_address = test_contract.get_instance_address(0);
    set_resource_bound(&mut block_context, resource, new_bound);
    let tx = l1handler_tx(Fee(1), contract_address);
    tx.execute(&mut state, &block_context).unwrap_err()
}

#[test]
fn test_l1_handler_resource_bounds_l1gas() {
    // Lower L1Gas bound to force error.
    let new_bound = GasAmount(1);
    let err = execute_l1_handler_with_bound(L1Gas, new_bound);
    ...
    assert_matches!(
        ...
        TransactionExecutionError::FeeCheckError(FeeCheckError::MaxGasAmountExceeded {
            ...
            actual_amount,
            ..
        }) if resource == L1Gas && max_amount == new_bound && actual_amount > new_bound
    );
}

#[test]
fn test_l1_handler_resource_bounds_l2gas() {
    // Lower L2Gas bound to force execution failure.
    let new_bound = GasAmount(1);
    let err = execute_l1_handler_with_bound(L2Gas, new_bound);

    // Expected retdata for L2 gas errors (e.g. "Out of gas").
    let expected_retdata = retdata![felt!["0x4f7574206f6620676173"]];

    // Expect an ExecutionError with a Cairo1 revert summary whose last_retdata matches.
    assert_matches!(
        err,
