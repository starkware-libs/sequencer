...
fn default_initial_gas_amount(block_context: Option<&BlockContext>) -> GasAmount {
    match block_context {
...
fn user_initial_gas_from_bounds(
    ...
    match bounds {
        ValidResourceBounds::L1Gas(_) => default_initial_gas_amount(block_context),
    ...
    let initial_gas = match cairo_version {
        ...
        CairoVersion::Cairo1 => match tracked_resource {
            TrackedResource::CairoSteps => {
                default_initial_gas_amount(None)
                    .min(VERSIONED_CONSTANTS.max_validation_sierra_gas())
                    .0
            }
            TrackedResource::SierraGas => {
                user_initial_gas
                    .unwrap_or(default_initial_gas_amount(None))
                    .min(VERSIONED_CONSTANTS.max_validation_sierra_gas())
                    .0
            }
        },
...
fn test_invoke_tx(
    ...
    let expected_validate_call_info = expected_validate_call_info(
        ...
        tracked_resource,
        Some(initial_gas.min(VERSIONED_CONSTANTS.max_validation_sierra_gas())),
    );
...
fn test_l1_handler(#[values(false, true)] use_kzg_da: bool) {
    ...
    let expected_call_info = CallInfo {
        call: CallEntryPoint {
            ...
            caller_address: ContractAddress::default(),
            call_type: CallType::Call,
            initial_gas: default_initial_gas_amount(Some(block_context)).0,
        },
        execution: CallExecution {
