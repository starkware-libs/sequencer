...
fn test_max_fee_exceeds_balance(
    ...
    let max_l1_gas_amount = BALANCE / MAX_L1_GAS_PRICE;
    ...
    let min_l2_amount = 445900;
    let min_l1_data_amount = 128;
    let max_l1_gas_amount: u64 = max_l1_gas_amount.0.try_into().unwrap();
    let max_l2_gas_amount: u64 =
        (BALANCE / MAX_L2_GAS_PRICE).0.try_into().expect("Failed to convert u128 to u64.");
    let max_l1_data_gas_amount: u64 =
        (BALANCE / MAX_L1_DATA_GAS_PRICE).0.try_into().expect("Failed to convert u128 to u64.");
    // Ensure that in the following, the test won't fall on min amount too low.
    assert!(max_l1_gas_amount / 3 + 1 >= min_l1_amount);
    assert!(max_l2_gas_amount / 3 + 1 >= min_l2_amount);
    assert!(max_l1_data_gas_amount / 3 + 1 >= min_l1_data_amount);
    ...
    for (l1_max_amount, l2_max_amount, l1_data_max_amount) in [
        ...
        let invalid_resource_bounds = ValidResourceBounds::AllResources(AllResourceBounds {
            l1_gas: ResourceBounds {
                ...
                max_price_per_unit: MAX_L1_GAS_PRICE.get().0,
            },
            l2_gas: ResourceBounds {
                max_amount: l2_max_amount,
                max_price_per_unit: MAX_L2_GAS_PRICE.get().0,
            },
            l1_data_gas: ResourceBounds {
                max_amount: l1_data_max_amount,
                max_price_per_unit: MAX_L1_DATA_GAS_PRICE.get().0,
            },
        });
        let invalid_tx = account_invoke_tx(invoke_tx_args! {
