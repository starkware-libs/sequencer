...
impl SerializeConfig for BuiltinWeights {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        let mut dump = BTreeMap::from([ser_param(
            "pedersen",
            &self.pedersen,
            "stwo pedersen gas weight.",
            ParamPrivacyInput::Public,
        )]);
        dump.append(&mut BTreeMap::from([ser_param(
            "range_check",
            &self.range_check,
            "stwo range_check gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "range_check96",
        ...
        dump.append(&mut BTreeMap::from([ser_param(
            "poseidon",
            &self.poseidon,
            "stwo poseidon gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "ecdsa",
            &self.ecdsa,
            "stwo ecdsa gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "ec_op",
            &self.ec_op,
            "stwo ec_op gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "add_mod",
            &self.add_mod,
            "stwo add_mod gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "mul_mod",
            &self.mul_mod,
            "stwo mul_mod gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "keccak",
            &self.keccak,
            "stwo keccak gas weight.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "bitwise",
            &self.bitwise,
            "stwo bitwise gas weight.",
            ParamPrivacyInput::Public,
        )]));
