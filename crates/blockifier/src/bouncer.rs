...
/// Converts ExecutionResources to proving gas using builtin weights.
fn vm_resources_to_proving_gas(
    resources: &ExecutionResources,
    builtin_weights: &BuiltinWeights,
    versioned_constants: &VersionedConstants,
) -> GasAmount {
    let builtin_counters = resources.prover_builtins();
    let sierra_gas = vm_resources_to_sierra_gas(resources, versioned_constants);

    proving_gas_from_builtins_and_sierra_gas(
        &builtin_counters,
        sierra_gas,
        builtin_weights,
        versioned_constants,
