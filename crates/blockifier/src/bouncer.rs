...
/// Mirror struct used solely for (de)serialization. The on-disk representation holds the
/// per-block builtin instance limits; the derived gas costs in `BouncerConfig` are recomputed
/// from them at construction time.
#[derive(Clone, Debug, Deserialize, Serialize)]
struct BouncerConfigSerde {
    block_max_capacity: BouncerWeights,
    builtin_instance_limits: BuiltinInstanceLimits,
}

#[derive(Clone, Debug, PartialEq, Deserialize, Serialize)]
#[serde(from = "BouncerConfigSerde", into = "BouncerConfigSerde")]
pub struct BouncerConfig {
    pub block_max_capacity: BouncerWeights,
    pub builtin_instance_limits: BuiltinInstanceLimits,
    /// Derived from `builtin_instance_limits` and `block_max_capacity.proving_gas` at
    /// construction. Not serialized; reconstruct via `BouncerConfig::new` if either input
    /// changes.
...
impl BouncerConfig {
    pub fn new(
        ...
        let builtin_gas_costs =
            builtin_instance_limits.induced_gas_costs(block_max_capacity.proving_gas);
        Self { block_max_capacity, builtin_instance_limits, builtin_gas_costs }
    }

    pub fn empty() -> Self {
        Self::new(BouncerWeights::empty(), BuiltinInstanceLimits::default())
    }
    ...
    pub fn max() -> Self {
        ...
        Self::new(block_max_capacity, BuiltinInstanceLimits::default())
    }

    pub fn builtin_gas_costs(&self) -> &BuiltinGasCosts {
        &self.builtin_gas_costs
    }
...
impl Default for BouncerConfig {
    fn default() -> Self {
        Self::new(BouncerWeights::default(), BuiltinInstanceLimits::default())
    }
}

impl From<BouncerConfigSerde> for BouncerConfig {
    fn from(serde_form: BouncerConfigSerde) -> Self {
        Self::new(serde_form.block_max_capacity, serde_form.builtin_instance_limits)
    }
}

impl From<BouncerConfig> for BouncerConfigSerde {
    fn from(config: BouncerConfig) -> Self {
        Self {
            block_max_capacity: config.block_max_capacity,
            builtin_instance_limits: config.builtin_instance_limits,
        }
    }
}
...
/// block-wide proving-gas budget via `induced_gas_costs`.
#[derive(Clone, Copy, Debug, Deserialize, PartialEq, Serialize)]
...
impl BuiltinInstanceLimits {
    ...
    pub fn induced_gas_costs(&self, proving_gas: GasAmount) -> BuiltinGasCosts {
        let derive = |limit: u64, name: &str| -> u64 {
            assert!(limit > 0, "BuiltinInstanceLimits.{name} must be > 0");
            proving_gas.0 / limit
        };
        BuiltinGasCosts {
            pedersen: derive(self.pedersen, "pedersen"),
            range_check: derive(self.range_check, "range_check"),
            range_check96: derive(self.range_check96, "range_check96"),
            poseidon: derive(self.poseidon, "poseidon"),
            ecdsa: derive(self.ecdsa, "ecdsa"),
            ecop: derive(self.ecop, "ecop"),
            bitwise: derive(self.bitwise, "bitwise"),
            keccak: derive(self.keccak, "keccak"),
            add_mod: derive(self.add_mod, "add_mod"),
            mul_mod: derive(self.mul_mod, "mul_mod"),
            blake: derive(self.blake, "blake"),
        }
    }
}

impl Default for BuiltinInstanceLimits {
    // SN-bound per-block instance limits, with overrides on bitwise (10.5M) and blake (1.8M).
    fn default() -> Self {
        Self {
            pedersen: 2_000_000,
            range_check: 66_666_666,
            range_check96: 33_519_553,
            poseidon: 600_000,
            ecdsa: 3_000,
            ecop: 130_000,
            bitwise: 10_500_000,
...
impl SerializeConfig for BuiltinInstanceLimits {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        let mut dump = BTreeMap::from([ser_param(
            "pedersen",
            &self.pedersen,
            "Per-block instance limit for the pedersen builtin.",
            ParamPrivacyInput::Public,
        )]);
        dump.append(&mut BTreeMap::from([ser_param(
            "range_check",
            &self.range_check,
            "Per-block instance limit for the range_check builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "range_check96",
            &self.range_check96,
            "Per-block instance limit for the range_check96 builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "poseidon",
            &self.poseidon,
            "Per-block instance limit for the poseidon builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "ecdsa",
            &self.ecdsa,
            "Per-block instance limit for the ecdsa builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "ecop",
            &self.ecop,
            "Per-block instance limit for the ec_op builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "add_mod",
            &self.add_mod,
            "Per-block instance limit for the add_mod builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "mul_mod",
            &self.mul_mod,
            "Per-block instance limit for the mul_mod builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "keccak",
            &self.keccak,
            "Per-block instance limit for the keccak builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "bitwise",
            &self.bitwise,
            "Per-block instance limit for the bitwise builtin.",
            ParamPrivacyInput::Public,
        )]));
        dump.append(&mut BTreeMap::from([ser_param(
            "blake",
            &self.blake,
            "Per-block instance limit for the blake opcode.",
            ParamPrivacyInput::Public,
        )]));
