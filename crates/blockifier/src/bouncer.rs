...
#[derive(Clone, Copy, Debug, Deserialize, PartialEq, Serialize)]
pub struct BuiltinWeights(pub BuiltinGasCosts);

impl BuiltinWeights {
    pub fn empty() -> Self {
        BuiltinWeights(BuiltinGasCosts {
            pedersen: 0,
            range_check: 0,
            ecdsa: 0,
            bitwise: 0,
...
fn proving_gas_from_builtins_and_sierra_gas(
    ...
    let builtins_proving_gas = builtins_to_gas(builtin_counters, &builtin_weights.0);
...
fn vm_resources_to_proving_gas(
    ...
    vm_resources_to_gas(resources, versioned_constants, |builtin_counters| {
        builtins_to_gas(builtin_counters, &builtin_weights.0)
    })
}
