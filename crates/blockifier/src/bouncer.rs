...
impl From<BuiltinCount> for GasAmount {
    fn from(builtins: BuiltinCount) -> Self {
        ...
        let gas_amount = [
            (builtins.add_mod, builtins_constants.add_mod),
            (builtins.bitwise, builtins_constants.bitwise),
            (builtins.ecdsa, builtins_constants.ecdsa),
            (builtins.ec_op, builtins_constants.ecop),
            (builtins.keccak, builtins_constants.keccak),
            (builtins.mul_mod, builtins_constants.mul_mod),
            (builtins.pedersen, builtins_constants.pedersen),
            (builtins.poseidon, builtins_constants.poseidon),
            (builtins.range_check, builtins_constants.range_check),
            (builtins.range_check96, builtins_constants.range_check96),
        ]
        .iter()
        .try_fold(0u64, |total, &(builtin_count, gas_cost)| {
            u64_from_usize(builtin_count)
                .checked_mul(gas_cost)
                .and_then(|val| total.checked_add(val))
        })
        .unwrap_or_else(|| {
            panic!(
                "Overflow occurred while converting builtins resources to gas. Builtins: {:?}",
                builtins
            )
        });
