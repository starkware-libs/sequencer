...
impl<S: StateReader + Send + 'static> WorkerPool<S> {
    ...
    pub fn start(stack_size: usize, concurrency_config: ConcurrencyConfig) -> Self {
        ...
        WorkerPool { senders, handlers, joined: false }
    }
...
impl<S: StateReader> Drop for WorkerPool<S> {
    fn drop(&mut self) {
        assert!(self.joined, "join must be called on WorkerPool before dropping it.");
    }
}
