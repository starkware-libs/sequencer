...
impl<S: StateReader> WorkerExecutor<S> {
    pub fn new(
        ...
        WorkerExecutor {
            scheduler: Scheduler::new(n_transactions),
            state,
            transactions: transactions
                ...
                .collect(),
            n_transactions: Mutex::new(n_transactions),
...
impl<U: UpdatableState> WorkerExecutor<U> {
    pub fn commit_chunk_and_recover_block_state(&self, n_committed_txs: usize) -> U {
        ...
        let n_transactions = self.get_n_transactions();
        log::debug!(
            "Concurrent execution done. Number of transactions: {n_transactions}; Committed chunk \
             size: {n_committed_txs}; Execute counter: {execute_counter}; Validate counter: \
