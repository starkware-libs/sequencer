...
impl<S: StateReader> WorkerExecutor<S> {
    ...
    fn commit_tx(&self, tx_index: TxIndex) -> Result<CommitResult, VersionedStateError> {
        ...
        if let Ok(tx_execution_info) = execution_output.result.as_mut() {
            ...
            let number_of_calls = execution_summary.number_of_calls.clone();
            ...
            TOTAL_CALLS.increment(number_of_calls.n_calls);
            CALLS_RUNNING_NATIVE.increment(number_of_calls.n_calls_run_native);
