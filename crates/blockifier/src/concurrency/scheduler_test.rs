...
#[case::commit_phase(TransactionStatus::Committed, true)]
fn test_try_validation_abort(#[case] tx_status: TransactionStatus, #[case] commit_phase: bool) {
    let tx_index = 0;
    let scheduler = Scheduler::new(DEFAULT_CHUNK_SIZE);
    scheduler.set_tx_status(tx_index, tx_status);
    let result = scheduler.try_validation_abort(tx_index, commit_phase);
    let (expected_result, expected_status) = match (tx_status, commit_phase) {
        (TransactionStatus::Executed, false) | (TransactionStatus::Committed, true) => {
            (true, TransactionStatus::Aborting)
        }
        _ => (false, tx_status),
    };
