...
pub fn should_migrate(
    ...
    declare_tx: Option<&DeclareTransaction>,
) -> StateResult<Option<(ClassHash, CompiledClassHashV2ToV1)>> {
    let (state_compiled_class_hash, compiled_class) = match declare_tx {
        Some(declare_tx) => {
            (declare_tx.compiled_class_hash(), declare_tx.contract_class().try_into()?)
        }
        None => (
            state_reader.get_compiled_class_hash(class_hash)?,
