...
impl StarknetResources {
    /// Creates a new `StarknetResources`.
    /// `calldata_length` should include proof_facts_length if present (they share the same
    /// gas_per_data_felt cost).
    pub fn new(
...
pub struct ArchivalDataResources {
    ...
    /// Transaction calldata length, including proof_facts if present.
    pub calldata_length: usize,
    pub signature_length: usize,
    pub code_size: usize,
    /// Whether the transaction includes client-side proof (for fixed gas_per_proof cost).
