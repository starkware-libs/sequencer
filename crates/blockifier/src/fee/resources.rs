...
impl ArchivalDataResources {
    ...
    fn get_calldata_and_signature_gas_cost(
        ...
        GasAmount(u128::from((archival_gas_costs.gas_per_data_felt * total_data_size).to_integer()))
    }
    ...
    fn get_code_gas_cost(&self, archival_gas_costs: &ArchivalDataGasCosts) -> GasAmount {
        GasAmount(u128::from(
            (archival_gas_costs.gas_per_code_byte * u64_from_usize(self.code_size)).to_integer(),
        ))
    }
...
impl MessageResources {
    ...
    pub fn to_gas_vector(&self) -> GasVector {
        ...
        let sharp_gas_usage = GasVector::from_l1_gas(GasAmount(u128_from_usize(
