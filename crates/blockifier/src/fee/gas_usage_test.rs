...
fn test_gas_computation_regression_test(
    ...
    let (l1, l1_data, mut l2) = match (&gas_vector_computation_mode, use_kzg_da) {
    ...
    let proof_facts_overhead = versioned_constants
        .get_archival_data_gas_costs(&gas_vector_computation_mode)
        .gas_per_data_felt
        .to_integer()
        * u64_from_usize(PROOF_FACTS_COUNT);

    if has_client_side_proof {
        l2 += proof_facts_overhead;
    }

    let expected_starknet_resources_gas_vector =
        GasVector { l1_gas: GasAmount(l1), l1_data_gas: GasAmount(l1_data), l2_gas: GasAmount(l2) };
    ...
    let (total_l1, total_l1_data, mut total_l2) = match (&gas_vector_computation_mode, use_kzg_da) {
        (GasVectorComputationMode::NoL2Gas, true) => (21575, 2720, 0),
        (GasVectorComputationMode::NoL2Gas, false) => (62866, 0, 0),
        (GasVectorComputationMode::All, true) => (21543, 2720, 1120394),
        (GasVectorComputationMode::All, false) => (62834, 0, 1120394),
    };

    if has_client_side_proof {
        total_l2 += proof_facts_overhead;
    }

    let expected_gas_vector = GasVector {
        l1_gas: GasAmount(total_l1),
        l1_data_gas: GasAmount(total_l1_data),
        l2_gas: GasAmount(total_l2),
    };
