...
pub fn get_vm_resources_cost(
    ...
    let vm_resource_usage_for_fee = vm_resource_usage.prover_builtins();

    // Validate used Cairo resources.
    assert!(
        HashSet::<&BuiltinName>::from_iter(vm_resource_usage_for_fee.keys())
            .is_subset(&HashSet::from_iter(vm_resource_fee_costs.builtins.keys())),
        "{:#?} should contain {:#?}",
        vm_resource_fee_costs.builtins.keys(),
        vm_resource_usage_for_fee.keys(),
    );

    // Convert Cairo builtin usage to L1 gas usage.
    let mut vm_l1_gas_usage = vm_resource_fee_costs
        .builtins
        .iter()
    ...
    // Add steps.
    vm_l1_gas_usage = u128::max(
        vm_l1_gas_usage,
        (vm_resource_fee_costs.n_steps
            * u128_from_usize(vm_resource_usage.total_n_steps() + n_reverted_steps))
        .ceil()
        .to_integer(),
    );
