    ...
    fn check_resources_within_bounds(
        ...
        match valid_resource_bounds {
            ValidResourceBounds::AllResources(all_resource_bounds) => {
                ...
                // Testing patch: mirror the runtime execution multiplier to avoid post-execution
                // "insufficient max L2Gas" reverts when we intentionally bump execution budget.
                if !cfg!(test) {
                    max_amounts.l2_gas = max_amounts
                        .l2_gas
                        .checked_factor_mul(10_000)
                        .unwrap_or_else(|| {
                            panic!(
                                "Overflow while multiplying L2 gas bound. current gas: {}, factor: 10000.",
                                max_amounts.l2_gas
                            )
                        });
                }
