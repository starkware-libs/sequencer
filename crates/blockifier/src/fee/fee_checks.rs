...
impl FeeCheckReport {
    ...
    pub fn check_all_resources_within_bounds(
        resource_bounds: &GasVector,
        gas_vector: &GasVector,
    ) -> FeeCheckResult<()> {
        for (resource, max_amount, actual_amount) in [
            (L1Gas, resource_bounds.l1_gas, gas_vector.l1_gas),
            (L2Gas, resource_bounds.l2_gas, gas_vector.l2_gas),
            (L1DataGas, resource_bounds.l1_data_gas, gas_vector.l1_data_gas),
        ] {
