...
fn test_calculate_tx_gas_usage_basic<'a>(
    ...
    let manual_starknet_l1_gas_usage_vector =
        GasVector::from_l1_gas(GasAmount(u128_from_usize(manual_starknet_l1_gas_usage)));
    let manual_sharp_gas_usage =
        message_segment_length * eth_gas_constants::SHARP_GAS_PER_MEMORY_WORD;
    let manual_gas_computation =
        GasVector::from_l1_gas(GasAmount(u128_from_usize(manual_sharp_gas_usage)))
    ...
    let manual_gas_computation = GasVector {
        l1_gas: GasAmount(u128_from_usize(manual_starknet_gas_usage + manual_sharp_gas_usage)),
    ...
    let expected_gas_vector = GasVector {
        ...
        + GasAmount(u128_from_usize(fee_balance_discount)),
