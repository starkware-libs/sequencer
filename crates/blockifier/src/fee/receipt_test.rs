...
fn test_calculate_tx_gas_usage_basic<'a>(
    ...
    for cairo_version in [CairoVersion::Cairo0, CairoVersion::Cairo1] {
        ...
        let manual_gas_vector = match gas_vector_computation_mode {
            GasVectorComputationMode::NoL2Gas => GasVector::from_l1_gas(
                (versioned_constants.get_l1_to_l2_gas_price_ratio() * code_gas_cost).to_integer(),
            ),
    ...
    let manual_starknet_gas_usage_vector = match gas_vector_computation_mode {
        GasVectorComputationMode::NoL2Gas => GasVector::from_l1_gas(
            (versioned_constants.get_l1_to_l2_gas_price_ratio() * calldata_and_signature_gas_cost)
                .to_integer(),
        ),
    ...
    let manual_starknet_l1_gas_usage_vector =
        GasVector::from_l1_gas(u128_from_usize(manual_starknet_l1_gas_usage));
...
fn test_calculate_tx_gas_usage(
    ...
    let max_resource_bounds = get_resource_bounds(&gas_vector_computation_mode);
    ...
    let execution_call_info_iter = vec![execution_call_info].into_iter();
