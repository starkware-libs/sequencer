...
#[rstest]
#[case::l1_bound_overdraft(21, 19, 19, true, Some(Resource::L1Gas))]
#[case::l2_bound_overdraft(19, 21, 19, true, Some(Resource::L2Gas))]
#[case::l1_data_bound_overdraft(5, 7, 1000, true, Some(Resource::L1DataGas))]
#[case::no_overdraft(18, 18, 18, false, None)]
fn test_post_execution_gas_overdraft_all_resource_bounds(
    ...
    #[case] expect_failure: bool,
    #[case] resource_overdraft: Option<Resource>,
) {
    ...
    if expect_failure {
        let error = report.error().unwrap();
        assert_matches!(error, FeeCheckError::MaxGasAmountExceeded {resource, .. } if resource==resource_overdraft.expect("One of the resource overdraft"))
    } else {
        assert_matches!(report.error(), None);
    }
}
