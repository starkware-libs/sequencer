...
/// Test that an L1 handler transaction fails post-execution validation
/// when the gas usage exceeds the allowed resource bounds.
#[rstest]
#[case::l1_overdraft(
    GasAmount(3_300_000 * 2),      // artificially low L1 gas limit
    GasAmount(100_000_000),        // default L2 gas limit
    GasAmount(2_000_000),          // default L1 data gas limit
    Some(Resource::L1Gas)
)]
#[case::l2_overdraft(
    GasAmount(3_300_000),          // default L1 gas limit
    GasAmount(100_000_000 * 2),      // artificially low L2 gas limit
    GasAmount(2_000_000),          // default L1 data gas limit
    Some(Resource::L2Gas)
)]
#[case::l1_data_overdraft(
    GasAmount(3_300_000),          // default L1 gas limit
    GasAmount(100_000_000),        // default L2 gas limit
    GasAmount(2_000_000 * 2),      // artificially low L1 data gas limit
    Some(Resource::L1DataGas)
)]
#[case::no_overdraft(
    GasAmount(3_300_000),          // default L1 gas limit
...
fn test_l1_handler_resource_overdraft_all_resource_bounds(
    #[case] l1_gas_used: GasAmount,
    #[case] l2_gas_used: GasAmount,
    #[case] l1_data_gas_used: GasAmount,
    #[case] resource_out_of_bounds: Option<Resource>,
) {
    // Create a standard block context and state.
    let block_context = BlockContext::create_for_account_testing();
    let account = FeatureContract::AccountWithoutValidations(CairoVersion::Cairo0);
    let mut state = test_state(&block_context.chain_info, BALANCE, &[(account, 1)]);
    let contract_address = account.get_instance_address(0);

    // Create and execute an L1 handler transaction.
    ...
    // Run the post-execution report with fee charging enabled.
    let charge_fee = true;
    let report = PostExecutionReport::new(
        &mut state,
        &block_context.to_tx_context(&tx),
