...
fn test_discounted_gas_overdraft(
    ...
    let (l1_gas_used, l1_data_gas_used, gas_bound) =
        (GasAmount(gas_bound), GasAmount(l1_gas_used), GasAmount(l1_data_gas_used));
    let (gas_price, data_gas_price) = (
        NonzeroGasPrice::new(GasPrice(gas_price)).unwrap(),
        NonzeroGasPrice::new(GasPrice(data_gas_price)).unwrap(),
    );
    ...
    let tx = account_invoke_tx(invoke_tx_args! {
        ...
        resource_bounds: l1_resource_bounds(gas_bound, GasPrice(gas_price.get().0 * 10)),
    });
...
#[case::l1_bound_overdraft(
    GasAmount(2 * MAX_L1_GAS_AMOUNT.0),
    DEFAULT_L2_GAS_MAX_AMOUNT,
    DEFAULT_L1_DATA_GAS_MAX_AMOUNT,
    Some(Resource::L1Gas)
)]
#[case::l2_bound_overdraft(
    MAX_L1_GAS_AMOUNT,
    GasAmount(2 * DEFAULT_L2_GAS_MAX_AMOUNT.0),
    DEFAULT_L1_DATA_GAS_MAX_AMOUNT,
    Some(Resource::L2Gas)
)]
#[case::l1_data_bound_overdraft(
    MAX_L1_GAS_AMOUNT,
    DEFAULT_L2_GAS_MAX_AMOUNT,
    GasAmount(2 * DEFAULT_L1_DATA_GAS_MAX_AMOUNT.0),
    Some(Resource::L1DataGas)
)]
#[case::no_overdraft(
    GasAmount(MAX_L1_GAS_AMOUNT.0 / 2),
    GasAmount(DEFAULT_L2_GAS_MAX_AMOUNT.0 / 2),
    GasAmount(DEFAULT_L1_DATA_GAS_MAX_AMOUNT.0 / 2),
    None
)]
