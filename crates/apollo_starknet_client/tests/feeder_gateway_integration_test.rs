...
struct BlocksToTestGetStateUpdate {
    // First block, the original definitions.
    first_block: u32,
...
struct ClassHashesToTestClassForHash {
...
struct TestEnvData {
    url: String,
    get_blocks: BlocksToTestGetBlock,
    get_state_updates: BlocksToTestGetStateUpdate,
    class_hashes: ClassHashesToTestClassForHash,
}
...
async fn run_get_block_test(
    apollo_starknet_client: &StarknetFeederGatewayClient,
    block_numbers: BlocksToTestGetBlock,
) {
...
async fn run_get_state_update_test(
    apollo_starknet_client: &StarknetFeederGatewayClient,
    block_numbers: BlocksToTestGetStateUpdate,
) {
    for block_number in into_block_number_vec(block_numbers) {
...
async fn run_class_by_hash_test(
    apollo_starknet_client: &StarknetFeederGatewayClient,
    class_hashes: ClassHashesToTestClassForHash,
) {
