...
fn block_post_0_14_3_carries_fee_proposal_fri() {
    ...
    assert_eq!(block.fee_proposal_fri(), Some(GasPrice(0x1dcd66000)));
}
...
#[test]
fn fee_proposal_fri_remains_after_conversion_to_starknet_api_block() {
    let block: Block =
        serde_json::from_str(&read_resource_file("reader/block_post_0_14_3.json")).unwrap();
    let api_block = block.to_starknet_api_block_and_version().unwrap();
    assert_eq!(
        api_block.header.block_header_without_hash.fee_proposal_fri,
        Some(GasPrice(0x1dcd66000)),
    );
}
