...
impl ProofManager {
    ...
    fn make_storage_key(facts_hash: Felt, nonce: Nonce) -> Felt {
        // Simple combination: XOR the facts_hash with the nonce
        // This ensures different nonces produce different keys
        let nonce_felt = nonce.0;
        Felt::from_bytes_be(&(facts_hash + nonce_felt).to_bytes_be())
    }
