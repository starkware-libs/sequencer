...
pub enum StorageReaderRequest {
    ...
    /// Request the current state marker.
    StateMarker,
}
...
pub enum StorageReaderResponse {
    ...
    /// Response containing the state marker block number.
    ...
    for GenericStorageReaderServerHandler
{
    async fn handle_request(
        ...
        match request {
            ...
            StorageReaderRequest::StateMarker => {
                let block_number = txn.get_state_marker()?;
                Ok(StorageReaderResponse::StateMarker(block_number))
            }
        }
    }
}
