...
fn raw_mdbx_cursor_set_range_past_end_prev() {
    ...
    byteorder::WriteBytesExt::write_u32::<BigEndian>(&mut past_key_simple, KEY_PAST_ALL.0).unwrap();
    byteorder::WriteBytesExt::write_u32::<BigEndian>(&mut past_key_simple, KEY_PAST_ALL.1).unwrap();
    let mut past_key_dupsort = Vec::new();
    byteorder::WriteBytesExt::write_u32::<BigEndian>(&mut past_key_dupsort, KEY_PAST_ALL.0)
        .unwrap();
