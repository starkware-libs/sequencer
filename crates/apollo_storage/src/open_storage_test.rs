...
/// Test that set_batch_size dynamically changes the batch size at runtime.
///
/// This test verifies that calling set_batch_size flushes pending writes and
/// changes the effective batch size. This is the core mechanism for adaptive
/// batching near the chain tip.
///
/// # Test Flow
/// 1. Creates storage with batch_size = 5
/// 2. Writes 3 headers (incomplete batch, not visible to reader)
/// 3. Calls set_batch_size(1) which flushes pending writes
/// 4. Verifies all 3 headers are now visible (flush happened)
/// 5. Writes one more header with batch_size=1
/// 6. Verifies it's immediately visible (new batch_size in effect)
#[test]
fn test_set_batch_size_flushes_and_changes() {
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(5).unwrap(),
    });

    // Write 3 headers (incomplete batch).
    for i in 0..3u64 {
        writer
            .begin_rw_txn()
            .expect("Failed to begin transaction")
    ...
    // Not visible yet (batch_size=5, only 3 commits).
    let header = reader
        .begin_ro_txn()
        .expect("Failed to begin read transaction")
        .get_block_header(BlockNumber(0))
        .expect("Failed to get header");
    assert!(header.is_none(), "Headers should not be visible before set_batch_size flush");

    // Change batch size to 1 -- this should flush pending writes.
    writer.set_batch_size(1).expect("Failed to set batch size");

    // All 3 headers should now be visible (flushed).
    for i in 0..3u64 {
        let header = reader
            .begin_ro_txn()
            .expect("Failed to begin read transaction")
            .get_block_header(BlockNumber(i))
            .expect("Failed to get header");
        assert!(header.is_some(), "Header {} should be visible after set_batch_size flush", i);
    }

    // Write one more header with batch_size=1 -- should commit immediately.
    writer
        .begin_rw_txn()
        .expect("Failed to begin transaction")
        .append_header(BlockNumber(3), &create_test_header_for_batching(BlockNumber(3)))
        .expect("Failed to append header")
        .commit()
        .expect("Failed to commit");
    ...
    let header = reader
        .begin_ro_txn()
        .expect("Failed to begin read transaction")
        .get_block_header(BlockNumber(3))
        .expect("Failed to get header");
    assert!(header.is_some(), "Header should be immediately visible with batch_size=1");
}

/// Test that restore_configured_batch_size restores the original batch size.
///
/// # Test Flow
/// 1. Creates storage with batch_size = 5
/// 2. Changes batch size to 1
/// 3. Restores the configured batch size
/// 4. Writes 4 headers (incomplete batch under original batch_size=5)
/// 5. Verifies they are NOT visible (batching restored)
/// 6. Writes one more to complete the batch
/// 7. Verifies all are visible
#[test]
fn test_restore_configured_batch_size() {
    let batch_size = 5;
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(batch_size).unwrap(),
    ...
    // Switch to immediate mode and back.
    writer.set_batch_size(1).expect("Failed to set batch size");
    writer.restore_configured_batch_size().expect("Failed to restore batch size");

    // Write batch_size - 1 headers (should not be visible, batching restored).
    for i in 0..(batch_size - 1) {
        writer
            .begin_rw_txn()
            .expect("Failed to begin transaction")
    ...
    let header = reader
        ...
        .get_block_header(BlockNumber(0))
        .expect("Failed to get header");
    assert!(header.is_none(), "Headers should not be visible -- batching was restored");
...
/// Test that set_batch_size is a no-op when the size doesn't change.
///
/// # Test Flow
/// 1. Creates storage with batch_size = 5
/// 2. Writes 3 headers (incomplete batch)
/// 3. Calls set_batch_size(5) (same as current)
/// 4. Verifies headers are NOT visible (no flush happened)
#[test]
fn test_set_batch_size_noop_when_unchanged() {
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(5).unwrap(),
    });
    ...
    // set_batch_size with same value should be a no-op (no flush).
    writer.set_batch_size(5).expect("Failed to set batch size");
    ...
    let header = reader
        ...
        .get_block_header(BlockNumber(0))
        .expect("Failed to get header");
    assert!(header.is_none(), "No-op set_batch_size should not flush pending writes");
}
