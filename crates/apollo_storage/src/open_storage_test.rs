...
fn get_storage_from_two_processes_with_fork() {
    ...
    match unsafe { fork() } {
        Ok(ForkResult::Parent { child }) => {
            ...
            match waitpid(child, Some(WaitPidFlag::WUNTRACED)) {
                Ok(WaitStatus::Exited(_, exit_code)) => {
                    assert!(exit_code == 0, "Child process exited with non-zero code: {exit_code}")
                }
                _ => {
        ...
        Ok(ForkResult::Child) => {
            // Child process - wrap everything in panic catching
            ...
            // Exit with appropriate code based on whether panic occurred
            match result {
                Ok(_) => {
                    std::process::exit(0);
                }
                Err(_panic_info) => {
                    // Panic occurred, exit with error code
                    std::process::exit(1);
                }
            }
        }
