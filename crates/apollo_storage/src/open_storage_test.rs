...
fn create_storage_with_batch_config(
    ...
    config.batch_config =
        BatchConfig { enabled: false, batch_size: NonZeroUsize::new(100).unwrap() };
    let (_reader, _writer) =
...
fn test_batching_persistent_transaction_reuse() {
    let batch_size = 5;
    let ((_reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(batch_size).unwrap(),
    });
...
/// Test that batch_size=1 behaves like disabled batching (commits immediately).
///
/// batch_size=0 is rejected at deserialization time (see deserialize_batch_size),
/// so batch_size=1 is the minimum valid value and should commit on every transaction.
#[test]
fn test_batching_batch_size_one_commits_immediately() {
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(1).unwrap(),
    });
    ...
    assert!(header.is_some(), "With batch_size=1, commits should happen immediately");
}
...
fn test_batching_data_persists_after_storage_restart() {
    ...
    config.batch_config =
        BatchConfig { enabled: true, batch_size: NonZeroUsize::new(batch_size).unwrap() };
    {
...
/// Test that set_batch_size dynamically changes the effective batch size.
///
/// The new batch size takes effect on the next commit cycle. This is the core
/// mechanism for adaptive batching near the chain tip.
///
/// # Test Flow
/// 1. Creates storage with batch_size = 5
/// 2. Calls set_batch_size(2)
/// 3. Writes 2 headers -- they should become visible (batch of 2 is complete)
/// 4. Verifies the new batch size is in effect
#[test]
fn test_set_batch_size_changes_effective_batch() {
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(5).unwrap(),
    });

    // Change batch size to 2 (takes effect on next commit cycle).
    writer.set_batch_size(2).expect("Failed to set batch size");

    // Write 2 headers -- should complete a batch of 2.
    for i in 0..2u64 {
        writer
            .begin_rw_txn()
            .expect("Failed to begin transaction")
    ...
    // Both headers should be visible (batch of 2 completed).
    for i in 0..2u64 {
        let header = reader
            .begin_ro_txn()
            .expect("Failed to begin read transaction")
            .get_block_header(BlockNumber(i))
            .expect("Failed to get header");
        assert!(header.is_some(), "Header {} should be visible after batch completion", i);
    }
}

/// Test that set_batch_size to 1 gives immediate commits (one-way transition).
///
/// # Test Flow
/// 1. Creates storage with batch_size = 5
/// 2. Calls set_batch_size(1)
/// 3. Writes a single header
/// 4. Verifies it's immediately visible
#[test]
fn test_set_batch_size_to_one_gives_immediate_commits() {
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(5).unwrap(),
    });

    writer.set_batch_size(1).expect("Failed to set batch size");
    ...
    let header = reader
        ...
        .get_block_header(BlockNumber(0))
        .expect("Failed to get header");
    assert!(header.is_some(), "Header should be immediately visible with batch_size=1");
}
...
/// Test that with batching disabled (batch_size=1), a write that is never committed
/// (the transaction is dropped without calling `commit()`) is discarded — exactly as in
/// non-batching mode before transaction batching existed.
///
/// Without this guarantee, an aborted write would stay in the reused persistent transaction
/// and silently fold into the next commit.
///
/// # Test Flow
/// 1. Creates storage with batching disabled (batch_size=1).
/// 2. Writes a header but drops the transaction without committing.
/// 3. Verifies a subsequent transaction does NOT observe the aborted write.
#[test]
fn test_batching_disabled_aborted_write_is_discarded() {
    let ((_reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: false,
        batch_size: NonZeroUsize::new(100).unwrap(),
    });

    // Write a header, then drop the transaction WITHOUT committing it.
    {
        let _aborted_txn = writer
            .begin_rw_txn()
            .expect("Failed to begin transaction")
            .append_header(BlockNumber(0), &create_test_header_for_batching(BlockNumber(0)))
            .expect("Failed to append header");
        // `_aborted_txn` is dropped here without commit() -> the write must be discarded.
    }

    // A subsequent transaction must not observe the aborted write.
    let header = writer
        .begin_rw_txn()
        .expect("Failed to begin transaction")
        .get_block_header(BlockNumber(0))
        .expect("Failed to get header");
    assert!(header.is_none(), "An uncommitted (aborted) write must be discarded when batch_size=1");
}

/// Test that lowering the batch size to 1 flushes a pending batch immediately.
///
/// This models the adaptive-batching tip transition: while batching is enabled a partial
/// batch accumulates in the persistent transaction, then `set_batch_size(1)` disables
/// batching near the chain tip. A read-only RW transaction (e.g. parent-hash verification)
/// runs before the next write commits. Because a dropped RW transaction discards the
/// persistent transaction when batch_size=1, the pending batch must already have been
/// flushed by `set_batch_size`, otherwise those committed blocks would be lost.
///
/// # Test Flow
/// 1. Creates storage with batching enabled (batch_size=5).
/// 2. Commits a partial batch of 3 headers (below the threshold, not yet flushed).
/// 3. Calls `set_batch_size(1)` to disable batching.
/// 4. Runs and drops a read-only RW transaction without committing.
/// 5. Verifies all 3 headers are persisted and visible to an independent reader.
#[test]
fn test_set_batch_size_to_one_flushes_pending_batch() {
    let pending_headers = 3u64;
    let ((reader, mut writer), _temp_dir) = create_storage_with_batch_config(BatchConfig {
        enabled: true,
        batch_size: NonZeroUsize::new(5).unwrap(),
    });

    // Accumulate a partial batch (below batch_size, so not yet flushed to disk).
    for block_number in 0..pending_headers {
        writer
            .begin_rw_txn()
            .expect("Failed to begin transaction")
            .append_header(
                BlockNumber(block_number),
                &create_test_header_for_batching(BlockNumber(block_number)),
            )
            .expect("Failed to append header")
            .commit()
            .expect("Failed to commit");
    ...
    // Disable batching, as the adaptive logic does near the chain tip.
    writer.set_batch_size(1).expect("Failed to set batch size");

    // A read-only RW transaction that runs and is dropped without committing must not
    // discard the pending batch (it was flushed by set_batch_size above).
    {
        let read_only_txn = writer.begin_rw_txn().expect("Failed to begin transaction");
        let _ = read_only_txn.get_block_header(BlockNumber(0)).expect("Failed to get header");
        // dropped here without commit()
    }

    // All pending headers must be persisted and visible to an independent reader.
    for block_number in 0..pending_headers {
        let header = reader
            .begin_ro_txn()
            .expect("Failed to begin read transaction")
