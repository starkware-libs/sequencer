//! Low-level storage reader API providing direct access to database tables and memory-mapped files.
//!
//! This module defines the `StorageReaderApi` trait which provides 26 low-level functions for
//! reading data from the Starknet storage. Each function either reads directly from a database
//! table or from a memory-mapped file.
...
use crate::{IndexedDeprecatedContractClass, MarkerKind, StorageResult, StorageTxn};
...
pub trait StorageReaderApi<Mode: TransactionKind> {
    ...
    fn get_state_diff_from_file(&self, location: LocationInFile) -> StorageResult<ThinStateDiff>;

    /// Returns the location of a contract class for a given class hash.
    fn get_class_location(&self, class_hash: &ClassHash) -> StorageResult<Option<LocationInFile>>;

    /// Returns the contract class from a file given its location.
    fn get_class_from_file(&self, location: LocationInFile) -> StorageResult<SierraContractClass>;
    ...
    fn get_transaction_from_file(&self, location: LocationInFile) -> StorageResult<Transaction>;
