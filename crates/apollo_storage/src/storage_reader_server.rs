    ...
    RequestHandler: StorageReaderServerHandler<Request, Response>,
{
    fn clone(&self) -> Self {
        Self {
            ...
            dynamic_config_provider: Arc::clone(&self.dynamic_config_provider),
...
where
    ...
    Response: Send + Sync + 'static,
{
    // Fetch dynamic config to check if server is enabled
    let dynamic_config =
    ...
    fn into_response(self) -> Response {
        match self {
            ...
            StorageServerError::DynamicConfig(err) => {
                let error_message = format!("Dynamic config error: {}", err);
                error!("{}", error_message);
                (StatusCode::INTERNAL_SERVER_ERROR, error_message).into_response()
            }
        }
