...
/// Static configuration for the storage reader server (binding address).
...
/// Dynamic configuration for the storage reader server (runtime toggle).
...
pub struct ServerConfig {
    /// Static part: binding address (ip, port).
    #[serde(flatten)]
    #[validate(nested)]
    pub static_config: StorageReaderServerStaticConfig,
    /// Dynamic part: runtime toggle (enable).
    #[serde(flatten)]
    #[validate(nested)]
...
impl SerializeConfig for ServerConfig {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        let mut dump = self.static_config.dump();
        dump.append(&mut self.dynamic_config.dump());
        dump
    }
}
