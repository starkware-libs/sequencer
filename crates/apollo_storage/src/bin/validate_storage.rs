...
#![allow(clippy::as_conversions, clippy::manual_is_multiple_of)]
...
use clap::Parser;
use libmdbx::{DatabaseOptions, Mode, WriteMap};
...
fn compare_table_subset(
            ...
            (Some((k_ref, v_ref)), Some((k_other, v_other))) => {
                match k_ref.as_ref().cmp(k_other.as_ref()) {
                    Ordering::Equal => {
                        ...
                        matched += 1;
                        ref_position += 1;
                        // `progress_interval == 0` disables progress logging (avoids modulo by
                        // zero when `--progress-interval 0` is passed).
                        if progress_interval != 0 && matched % progress_interval == 0 {
                            print!(
    ...
    if progress_interval != 0 && matched > progress_interval {
...
fn compare_table_probe(
    ...
    for probe_key in &probes {
        ...
        // A probe key is always within [first_key, last_key], so set_range finds a key >= it; a
        // `None` here is unexpected. Skip this probe rather than break, so an anomaly cannot
        // silently truncate the probe set and still report success.
        ...
        // The reference DB is the smaller one; every reference key must exist identically in the
        // other DB (subset semantics). A missing or divergent key is a validation failure, not a
        // skip — otherwise probe mode could report VALIDATION PASSED while data has diverged.
        match other_entry {
            None => {
                return Err(format!(
                    "Reference DB has entry missing in other DB (probe): no key >= ref key ({} \
                     bytes): 0x{}",
                    k_ref.len(),
                    hex_prefix(k_ref.as_ref(), 32),
                ));
            ...
            Some((k_other, v_other)) => {
                if k_ref.as_ref() != k_other.as_ref() {
                    return Err(format!(
                        "Reference DB has entry missing in other DB (probe). Ref key ({} bytes): \
                         0x{}  Nearest other key ({} bytes): 0x{}",
                        k_ref.len(),
                        hex_prefix(k_ref.as_ref(), 32),
                        k_other.len(),
                        hex_prefix(k_other.as_ref(), 32),
    ...
    // Probe mode never legitimately skips a reference key (any miss returns Err above).
    Ok(CompareResult { matched, skipped: 0 })
}
...
fn compare_mmap_file_prefix(path1: &PathBuf, path2: &PathBuf) -> Result<(u64, u64, u64), String> {
    ...
    // If one file is empty while the other has data, there is no common prefix to verify and the
    // populated side's data would go unchecked — flag it instead of silently reporting success.
    if compare_len == 0 && size1 != size2 {
        return Err(format!(
            "mmap size mismatch: one file is empty while the other has data ({path1:?}: {size1} \
             bytes, {path2:?}: {size2} bytes); nothing could be verified",
        ));
    }
