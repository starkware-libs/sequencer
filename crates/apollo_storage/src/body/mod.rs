...
impl BodyStorageWriter for StorageTxn<'_, RW> {
    ...
    fn append_events(
        ...
        if self.scope != StorageScope::StateOnly {
            ...
            for (offset, events) in events_per_transaction.iter().enumerate() {
                ...
                transaction_events_table.insert(&self.txn, &transaction_index, &events.clone())?;
            }
        }
    ...
    fn revert_body(
        ...
        if block_number
            .next()
            .filter(|next_block_number| current_body_marker == *next_block_number)
            .is_none()
        {
            debug!(
    ...
    fn revert_events(self, block_number: BlockNumber) -> StorageResult<Self> {
        ...
        if self.scope != StorageScope::StateOnly {
            ...
            // Scan the events table to find all entries for this block and collect them
            // for deletion. The events table is keyed by (ContractAddress, TransactionIndex)
            // with ContractAddress as the main key, so we must scan the full table.
            ...
            let mut entry = cursor.lower_bound(&min_key)?;
            while let Some(((contract_address, tx_index), _)) = entry {
                if tx_index.0 == block_number {
                    keys_to_delete.push((contract_address, tx_index));
                }
                entry = cursor.next()?;
            }
            // Drop the cursor before mutating the table.
            drop(cursor);
            for key in &keys_to_delete {
                events_table.delete(&self.txn, key)?;
            }

            // Delete event content from the transaction_events table.
            let transaction_events_table = self.open_table(&self.tables.transaction_events)?;
            let num_transactions = self.get_block_transactions_count(block_number)?.unwrap_or(0);
            for offset in 0..num_transactions {
                let tx_index = TransactionIndex(block_number, TransactionOffsetInBlock(offset));
                transaction_events_table.delete(&self.txn, &tx_index)?;
            }
        }
