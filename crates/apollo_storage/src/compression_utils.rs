...
fn with_compressor<T>(
    ...
    COMPRESSOR.with(|cell| {
        let mut borrow = cell.try_borrow_mut().map_err(|_| {
            std::io::Error::other("zstd compressor: reentrant borrow on thread-local")
        })?;
        let compressor = match borrow.as_mut() {
            Some(compressor) => compressor,
            None => borrow.insert(zstd::bulk::Compressor::new(COMPRESSION_LEVEL)?),
        };
        f(compressor)
    })
}
...
fn with_decompressor<T>(
    ...
    DECOMPRESSOR.with(|cell| {
        let mut borrow = cell.try_borrow_mut().map_err(|_| {
            std::io::Error::other("zstd decompressor: reentrant borrow on thread-local")
        })?;
        let decompressor = match borrow.as_mut() {
            Some(decompressor) => decompressor,
            None => borrow.insert(zstd::bulk::Decompressor::new()?),
        };
        f(decompressor)
    })
}
