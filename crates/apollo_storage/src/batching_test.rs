...
fn test_batching_disabled() {
    ...
    // Queue operations (they still go to queue, but won't auto-flush).
    let block_number = BlockNumber(0);
    writer.queue_header(block_number, BlockHeader::default()).unwrap();

    // With batching disabled, auto-flush should NOT happen even if batch is "full"
    // (queue grows indefinitely until manual flush).
    assert_eq!(writer.batch_queue_len(), 1);
}
