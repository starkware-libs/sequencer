...
impl<T: StorageTransaction> StateStorageReader<T::Mode> for T {
    ...
    fn get_state_reader(&self) -> StorageResult<StateReader<'_, T::Mode>> {
        ...
        let compiled_class_hash_table = inner_txn.open_table(&tables.compiled_class_hash)?;
        let declared_classes_table = inner_txn.open_table(&tables.declared_classes)?;
        let declared_classes_block_table = inner_txn.open_table(&tables.declared_classes_block)?;
        let deprecated_declared_classes_table =
            inner_txn.open_table(&tables.deprecated_declared_classes)?;
        let deprecated_declared_classes_block_table =
            inner_txn.open_table(&tables.deprecated_declared_classes_block)?;
        let deployed_contracts_table = inner_txn.open_table(&tables.deployed_contracts)?;
        let nonces_table = inner_txn.open_table(&tables.nonces)?;
        let storage_table = inner_txn.open_table(&tables.contract_storage)?;
        let markers_table = inner_txn.open_table(&tables.markers)?;
        Ok(StateReader {
            txn: inner_txn,
...
impl<'env, Mode: TransactionKind> StateReader<'env, Mode> {
    ...
    /// Returns all contract addresses in [start, end] at `block_target`, paired with their class
    /// hashes.
