...
fn test_scan_synced_block() {
    ...
    let ca2 = contract_address!("0x2"); // skipped.
    ...
    assert_eq!(
        state_reader.scan_contract_class_hashes_in_range(&ca1, &ca5, BlockNumber(0), 2).unwrap(),
        vec![(ca1, ch1), (ca3, ch3)],
    );
    ...
    assert_eq!(
        state_reader
            .scan_contract_class_hashes_in_range(&ca2, &ca2, BlockNumber(0), usize::MAX,)
            .unwrap(),
        vec![],
    );
}
