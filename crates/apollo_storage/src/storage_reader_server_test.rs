...
async fn endpoint_successful_query() {
    ...
    let dynamic_config = Arc::new(RwLock::new(config.dynamic_config.clone()));
...
async fn dynamic_config_starts_disabled_then_enables() {
    ...
    let dynamic_config = Arc::new(RwLock::new(StorageReaderServerDynamicConfig { enable: false }));
    let server = StorageReaderServer::<TestHandler, TestRequest, TestResponse>::new(
        reader.clone(),
        config,
    ...
    // Enable the server
    *dynamic_config.write().unwrap() = StorageReaderServerDynamicConfig { enable: true };
...
async fn dynamic_config_starts_enabled_then_disables() {
    ...
    let dynamic_config = Arc::new(RwLock::new(StorageReaderServerDynamicConfig { enable: true }));
    let server = StorageReaderServer::<TestHandler, TestRequest, TestResponse>::new(
        reader.clone(),
        config,
        dynamic_config.clone(),
    ...
    // Disable the server
    *dynamic_config.write().unwrap() = StorageReaderServerDynamicConfig { enable: false };
