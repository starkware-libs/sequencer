...
fn open_storage_internal(
    ...
    let batch_size = if storage_config.batch_config.enabled {
        storage_config.batch_config.batch_size
    } else {
        1
    };
...
/// This allows reader and writer traits to be implemented once generically, eliminating duplication.
...
impl Drop for StorageTxnRW<'_> {
    ...
    ///
    /// Two scenarios:
    /// 1. Uncommitted writes with pending batched data (commit_counter > 0) - panic.
    ///    This can happen due to a forgotten commit or transaction abortion while batching.
    ...
    fn drop(&mut self) {
        ...
        if self.guard.commit_counter > 0 {
            // Scenario 1: Pending batched writes exist.
            panic!(
                "StorageTxnRW dropped without commit() while {} pending writes exist! This would \
                 cause data loss. Always call commit() or handle errors properly when batching is enabled.",
