...
impl StorageTxn<'_, RW> {
    ...
    pub fn commit(self) -> StorageResult<()> {
        // Flush file handlers for both owned and borrowed transactions.
        // File handlers write to mmap files (not the database), so they must be flushed
        // regardless of whether the DB transaction commits immediately or is batched.
        self.file_handlers.flush();

        match self.txn_holder {
            TxnHolder::Owned(txn) => {
                // Non-batching mode: commit the transaction immediately.
                Ok(txn.commit()?)
            }
            TxnHolder::Borrowed(_) => {
                // Batching mode: don't commit the database transaction.
                // The persistent transaction remains active. A callback to StorageWriter
                // will check the commit_counter and perform the actual commit when the
