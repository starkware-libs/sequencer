...
/// Returns the storage, server, and a sender for config updates.
    ...
    Response: Serialize + DeserializeOwned + Send + 'static,
{
    ...
    let (server, tx) = create_storage_reader_server(reader.clone(), storage_reader_server_config);
    Ok((reader, writer, server, tx))
}
...
/// with dynamic configuration support and a sender for config updates.
pub type StorageWithServer<RequestHandler, Request, Response> = (
    StorageReader,
    StorageWriter,
    StorageReaderServer<RequestHandler, Request, Response>,
    tokio::sync::watch::Sender<StorageReaderServerDynamicConfig>,
);
