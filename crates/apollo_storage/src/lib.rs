...
fn open_storage_files(
    ...
    macro_rules! open_storage_file {
        ($name:literal, $kind:ident) => {{
            ...
            open_file(mmap_file_config.clone(), db_config.path().join(concat!($name, ".dat")), offset)
        }};
    }

    let (thin_state_diff_writer, thin_state_diff_reader) = open_storage_file!("thin_state_diff", ThinStateDiff)?;
    let (contract_class_writer, contract_class_reader) = open_storage_file!("contract_class", ContractClass)?;
