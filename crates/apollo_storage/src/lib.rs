    ...
    fn drop(&mut self) {
        ...
        if std::any::TypeId::of::<Mode>() == std::any::TypeId::of::<RW>() {
            ...
            if state.commit_counter > 0 {
                ...
                panic!(
                    "StorageTxn dropped without commit() while {} pending writes exist! This \
                     would cause data loss. Always call commit() or handle errors properly.",
                    state.commit_counter
                );
            } else {
                // Scenario 2: No pending writes, but there might be partial writes from a
                // failed operation. Abort the transaction to discard them.
