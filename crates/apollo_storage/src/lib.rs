...
pub struct StorageWriter {
    ...
    batching_state: Arc<std::sync::Mutex<BatchingState>>,
}
...
pub struct StorageTxn<'env, Mode: TransactionKind> {
    ...
    batching_state: Option<Arc<std::sync::Mutex<BatchingState>>>,
}
            ...
            TxnHolder::Borrowed(_) => {
                ...
                if let Some(state_arc) = self.batching_state {
                    ...
                    if state.commit_counter >= state.batch_size {
                        // Batch size reached - flush and commit the persistent transaction.
                        self.file_handlers.flush();
                        if let Some(txn) = state.active_txn.take() {
                            txn.commit()?;
...
impl<'env> StorageTxn<'env, RW> {
    ...
    fn with_batching_state(mut self, batching_state: Arc<std::sync::Mutex<BatchingState>>) -> Self {
