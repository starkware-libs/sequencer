...
impl StorageReader {
    ...
    ///
    /// RO transactions acquire the mutex on the shared state, allowing them to see
    /// uncommitted writes from the persistent RW transaction.
...
pub struct StorageTxn<'env> {
    guard: MutexGuard<'env, SharedState>,
    is_write: bool,
    file_handlers: FileHandlers<RW>,
    scope: StorageScope,
...
impl Drop for StorageTxn<'_> {
    ...
    fn drop(&mut self) {
        // Only apply write transaction logic if this is a write transaction
        if !self.is_write {
            return;
        }
