...
use mmap_file::{
    FileHandler, LocationInFile, MMapFileError, MMapFileStats, MmapFileConfig, Reader, Writer,
    open_file,
};
use serde::de::DeserializeOwned;
use serde::{Deserialize, Serialize};
use starknet_api::block::{
    BlockBody, BlockHash, BlockHeader, BlockNumber, BlockSignature, StarknetVersion,
};
...
impl StorageWriter {
    ...
    pub(crate) fn new(
        ...
        let (batch_queue, pending_writes_buffer) = if batch_config.enable_batching {
            let cap = batch_config.batch_size;
    ...
    pub fn flush_batch(&mut self) -> StorageResult<()> {
        ...
        // Move out to process without holding a mutable borrow of self.
...
pub enum StorageError {
    ...
    #[error(
        "Cannot use begin_rw_txn() while batching is enabled and the queue has {queue_len} \
         pending writes. Use queue_*() methods instead, or call flush_batch() first to ensure \
         consistency."
    )]
    BatchingApiMixingError { queue_len: usize },
}
