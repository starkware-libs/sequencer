...
impl Drop for StorageTxnRW<'_> {
    ...
    ///
    /// Two scenarios:
    /// 1. Uncommitted writes with pending batched data (commit_counter > 0) - panic. This can
    ///    happen due to a forgotten commit or transaction abortion while batching.
        ...
        if self.guard.commit_counter > 0 {
            ...
            panic!(
                "StorageTxnRW dropped without commit() while {} pending writes exist! This would \
                 cause data loss. Always call commit() or handle errors properly when batching is \
                 enabled.",
