...
pub fn load_and_process_config<T: for<'a> Deserialize<'a>>(
    ...
    if load_result.is_err() {
        ...
        keys_only_in_schema.retain(|item| {
            ...
            let has_suffix = item.ends_with(format!("{FIELD_SEPARATOR}{IS_NONE_MARK}").as_str());
            !has_prefix && !has_suffix
        });

        //
        if !(keys_only_in_loaded_config.is_empty() && keys_only_in_schema.is_empty()) {
            error!(
                "Detected loaded-schema config difference: 
                keys missing in schema: {:?}
                Keys missing in loaded config: {:?}",
                keys_only_in_loaded_config, keys_only_in_schema
            );
        }
    }
