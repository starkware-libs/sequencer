...
fn load_native<T: for<'a> Deserialize<'a>>(
    ...
    let (base_config_path, secret_config_paths) =
        custom_config_paths.split_first().ok_or(ConfigError::NativeModeRequiresConfigFile)?;
    ...
    for secret_config_path in secret_config_paths {
...
pub fn load_and_process_config<T: for<'a> Deserialize<'a>>(
    ...
    // In native format the config files are deserialized directly with serde, bypassing the preset
    // pipeline (pointers, env/CLI overrides, #is_none marks).
    let config_format =
