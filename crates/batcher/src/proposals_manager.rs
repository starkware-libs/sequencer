...
impl ProposalsManager {
    ...
    ) -> ProposalsManagerResult<impl Stream<Item = ProposalsManagerResult<ThinTransaction>> + '_>
    {
        info!("Starting generation of new proposal.");
        self.set_proposal_in_generation(proposal_id).await?;

        let (sender, mut receiver) = tokio::sync::mpsc::channel::<ThinTransaction>(
            self.config.outstream_tx_stream_buffer_size,
        );
        ...
        let streaming_span = info_span!("streaming_proposal", proposal_id = proposal_id);
        Ok(try_stream! {
            debug!(parent: &streaming_span, "Starting to stream transactions from the block builder.");
            while let Some(tx) = receiver.recv().await {
                debug!(parent: &streaming_span, "Received tx {} from the block builder, adding to the stream.", tx.tx_hash);
                yield tx;
            }
            debug!(parent: &streaming_span, "Finished streaming the transactions, closing the stream.");
        })
    }
...
mod block_builder {
    ...
    impl BlockBuilder {
        ...
        pub fn add_txs(&self, _txs: &[ThinTransaction]) -> bool {
            false
        }
...
struct ProposalGenerationTask {
    ...
    pub max_txs_per_mempool_request: usize,
    pub sender: tokio::sync::mpsc::Sender<ThinTransaction>,
