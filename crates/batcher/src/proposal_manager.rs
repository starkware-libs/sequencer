...
pub trait ProposalManagerTrait: Send + Sync {
    ...
    async fn decision_reached(
        &mut self,
        proposal_id: ProposalId,
    ) -> Result<ThinStateDiff, DecisionReachedError>;
}
...
type DoneProposal = Result<ThinStateDiff, BuildProposalError>;
...
impl ProposalManagerTrait for ProposalManager {
    ...
    async fn decision_reached(
        ...
        self.done_proposals
            ...
            .await
            .remove(&proposal_id)
            .ok_or(DecisionReachedError::DoneProposalNotFound { proposal_id })?
            .map_err(DecisionReachedError::BuildProposalError)
    }
}
...
impl BuildProposalTask {
    ...
    async fn active_proposal_finished(
        ...
        // TODO: send ProposalCommitment in the channel.
