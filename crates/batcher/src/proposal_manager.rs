...
pub trait ProposalManagerTrait: Send + Sync {
    ...
    async fn get_done_proposal(
        &mut self,
        proposal_id: ProposalId,
    ) -> Result<ProposalOutput, GetDoneProposalError>;
}
...
impl ProposalManagerTrait for ProposalManager {
    ...
    async fn get_done_proposal(
        ...
        self.done_proposals
            ...
            .await
            .remove(&proposal_id)
            .ok_or(GetDoneProposalError::ProposalDoesNotExist { proposal_id })
    }
}
...
impl BuildProposalTask {
    async fn run(mut self) {
        ...
        self.active_proposal_finished(result).await;
    }
    ...
    async fn active_proposal_finished(self, result: Result<ThinStateDiff, BuildProposalError>) {
        let proposal_id =
