...
use crate::transaction_provider::{MockTransactionProvider, TxStream};
...
fn mock_tx_provider_limited_calls(
    ...
    mock_tx_provider
        ...
        .returning(move |_n_txs| Ok(TxStream::Txs(input_chunks.remove(0))));
    mock_tx_provider
}
...
fn mock_tx_provider_stream_done(input_chunk: Vec<Transaction>) -> MockTransactionProvider {
    ...
    mock_tx_provider
        ...
        .in_sequence(&mut seq)
        .with(eq(TX_CHUNK_SIZE))
        .returning(move |_n_txs| Ok(TxStream::Txs(input_chunk.clone())));
    mock_tx_provider
        .expect_get_txs()
        .times(1)
        .in_sequence(&mut seq)
        .returning(|_n_txs| Ok(TxStream::TxStreamDone));
    mock_tx_provider
}
...
fn add_limitless_empty_calls(mock_tx_provider: &mut MockTransactionProvider) {
    mock_tx_provider
        .expect_get_txs()
        .with(eq(TX_CHUNK_SIZE))
        .returning(|_n_txs| Ok(TxStream::Txs(Vec::new())));
}
