...
fn one_chunk_test_expectations(
    ...
    let mock_mempool_client = mock_mempool_client(1, vec![input_txs.to_vec()], true);
...
fn two_chunks_test_expectations(
    ...
    let mock_mempool_client = mock_mempool_client(2, vec![first_chunk, second_chunk], true);
...
-> (MockTransactionExecutorTrait, MockMempoolClient, BlockExecutionArtifacts) {
    ...
    let mock_mempool_client = mock_mempool_client(1, vec![vec![]], true);
...
fn block_full_test_expectations(
    ...
    let mock_mempool_client = mock_mempool_client(1, vec![input_txs.to_vec()], false);
...
fn test_expectations_with_delay(
    ...
    let mock_mempool_client = mock_mempool_client(1, vec![first_chunk_copy], false);
