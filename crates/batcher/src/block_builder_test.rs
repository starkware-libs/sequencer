...
use blockifier::blockifier::transaction_executor::{
    MockTransactionExecutorTrait as MockBlockifierTransactionExecutorTrait,
...
use super::{BlockBuilder, BlockExecutionArtifacts};
...
async fn test_build_block() {
    ...
    let mut mock_transaction_executor = MockBlockifierTransactionExecutorTrait::new();
    ...
    let mut block_builder = BlockBuilder {
        executor: Box::new(mock_transaction_executor),
        txs_chunk_size: INPUT_TXS_LEN,
    };
...
fn executor_test_outputs(
    ...
    let mut execution_infos_mapping = HashMap::new();
