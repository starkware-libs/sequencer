...
impl BlockBuilderTrait for BlockBuilder {
    async fn build_block(
        ...
        let chunk_stream =
            input_tx_stream.chunks_timeout(self.tx_chunk_size, Duration::from_secs(0));
        pin!(chunk_stream);
        ...
        while !should_close_block && tokio::time::Instant::now() < deadline {
            let next_tx_chunk = chunk_stream.next().await;
            if let Some(next_tx_chunk) = next_tx_chunk {
...
async fn collect_execution_results_and_stream_txs(
    ...
    for (mempool_tx, result) in tx_chunk.into_iter().zip(results.into_iter()) {
        match result {
