...
// TODO remove this line in the next PR
#[allow(dead_code)]
...
impl<S: StateReader> BlockBuilder<S> {
    pub fn new(transaction_executor: TransactionExecutor<S>) -> BlockBuilderResult<Self> {
        Ok(BlockBuilder { executor: transaction_executor })
    }
    ...
    pub fn create_transaction_executor(
    ...
    ) -> BlockBuilderResult<TransactionExecutor<S>> {
        ...
        Ok(TransactionExecutor::new(state, block_context, execution_config.execute_config))
    }
}
