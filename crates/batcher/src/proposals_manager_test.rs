...
async fn proposal_generation_success(
    ...
    assert_matches!(proposals_manager.await_active_proposal().await, Some(Ok(())));
...
trait BlockBuilderTraitWrapper: Send + Sync {
    ...
    fn build_block(&self, deadline: tokio::time::Instant) -> BoxFuture<'_, ()>;
}

#[async_trait]
impl<T: BlockBuilderTraitWrapper> BlockBuilderTrait for T {
    async fn build_block(&self, deadline: tokio::time::Instant) {
