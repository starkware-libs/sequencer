...
async fn decision_reached(
    ...
    let mut proposal_manager = MockProposalManagerTraitWrapper::new();
    proposal_manager
        .expect_wrap_decision_reached()
        .with(eq(PROPOSAL_ID))
        .return_once(|_| async { Ok(state_diff_clone) }.boxed());
...
trait ProposalManagerTraitWrapper: Send + Sync {
    ...
    fn wrap_decision_reached(
        &mut self,
        proposal_id: ProposalId,
    ) -> BoxFuture<'_, Result<ThinStateDiff, DecisionReachedError>>;
}
...
impl<T: ProposalManagerTraitWrapper> ProposalManagerTrait for T {
    ...
    async fn decision_reached(
        ...
        self.wrap_decision_reached(proposal_id).await
    }
}
