...
async fn get_stream_content_success() {
    ...
    proposals_manager
        ...
        .returning(move |_, _, _| Ok(futures::stream::iter(expected_txs.clone()).boxed()));
    let mut batcher = Batcher::new(config, Arc::new(mempool_client), proposals_manager);
    ...
    for expected_tx in expected_txs_clone {
        ...
        assert_matches! {
            next_tx,
            StreamContent::Tx(tx) if tx == expected_tx
        }
    }
    let closing_value = batcher.get_stream_content(&get_stream_content_input).await.unwrap();
    assert_matches!(closing_value,
            StreamContent::StreamEnd(content_id) if content_id.tx_commitment == TransactionCommitment::default()
    );
    let stream_deleted = batcher.get_stream_content(&get_stream_content_input).await;
    assert_matches!(stream_deleted, Err(BatcherError::StreamIdDoesNotExist { stream_id }) if stream_id == 0)
}
