...
async fn proposal_generation_fails_without_start_height(
    ...
    let err = proposal_manager
        .build_block_proposal(ProposalId(0), arbitrary_deadline(), output_streaming.0)
        .await;
...
async fn consecutive_proposal_generations_success(
    ...
    proposal_manager
        .build_block_proposal(ProposalId(0), arbitrary_deadline(), output_sender_0)
        .await
        .unwrap();
    ...
    proposal_manager
        .build_block_proposal(ProposalId(1), arbitrary_deadline(), output_sender_1)
        .await
        .unwrap();
