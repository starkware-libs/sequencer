...
impl HttpServer {
    ...
    // TODO(Yael) : consider supporting both formats in the same endpoints if possible.
...
async fn add_tx(
    ...
    let rpc_tx = tx.try_into().inspect_err(|e| {
        debug!(
            "Error while converting deprecated gateway transaction into current gateway \
             transaction: {}",
            e
        );
    })?;
