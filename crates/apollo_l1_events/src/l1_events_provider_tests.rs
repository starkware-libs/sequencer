...
use crate::test_utils::{
    ...
    make_catchupper,
    ConsumedTransaction,
    FakeL1EventsProviderClient,
    L1EventsProviderContentBuilder,
};
use crate::transaction_record::TransactionState;
use crate::L1EventsProviderConfig;

fn commit_block_no_rejected(
    l1_events_provider: &mut L1EventsProvider,
    txs: &[TransactionHash],
    block_number: BlockNumber,
) {
    l1_events_provider
        .commit_block(txs.iter().copied().collect(), [].into(), block_number)
        .unwrap();
}

fn call_commit_block_to_start_catching_up_and_expect_error(
    l1_events_provider: &mut L1EventsProvider,
    block_number: BlockNumber,
) {
    let result = l1_events_provider.commit_block([].into(), [].into(), block_number);
    assert!(result.is_err());
}

fn setup_rejected_transactions() -> L1EventsProvider {
    let tx_id1 = 1;
    let tx_id2 = 2;
    let tx_id3 = 3;
    ...
    l1_events_provider.state = ProviderState::Validate;
    l1_events_provider
}
...
fn validate_happy_flow() {
    ...
    // Test.
    assert_eq!(
        l1_events_provider.validate(tx_hash!(1), BlockNumber(0)).unwrap(),
        ValidationStatus::Validated
    );
    assert_eq!(
        l1_events_provider.validate(tx_hash!(2), BlockNumber(0)).unwrap(),
        ValidationStatus::Invalid(InvalidValidationStatus::AlreadyIncludedOnL2)
    );
    // Transaction was consumed on L1.
    assert_eq!(
        l1_events_provider.validate(tx_hash!(3), BlockNumber(0)).unwrap(),
...
fn process_events_happy_flow() {
    ...
    for state in [ProviderState::Propose, ProviderState::Validate, ProviderState::Pending] {
        ...
        l1_events_provider
            ...
            .unwrap();
        l1_events_provider.add_events(vec![l1_handler_event(tx_hash!(6))]).unwrap();
...
fn process_events_committed_txs() {
    ...
    let expected_l1_events_provider = l1_events_provider.clone();

    // Test.
    // Uncommitted transaction, should fail silently.
    l1_events_provider
        .add_events(vec![timed_l1_handler_event(tx_hash!(1), timestamp.into())])
        .unwrap();
    assert_eq!(l1_events_provider, expected_l1_events_provider);

    // Committed transaction, should fail silently.
    l1_events_provider
        .add_events(vec![timed_l1_handler_event(tx_hash!(2), timestamp.into())])
        .unwrap();
    assert_eq!(l1_events_provider, expected_l1_events_provider);
}
...
fn add_tx_double_scraped_doesnt_update_scrape_timestamp() {
    ...
    l1_events_provider.add_events(vec![timed_l1_handler_event(tx_hash!(1), 2.into())]).unwrap();
    assert_eq!(l1_events_provider, expected_l1_events_provider);
}
...
fn get_txs_panics_if_transaction_on_proposable_index_is_not_pending() {
    ...
    l1_events_provider.start_block(BlockNumber(0), ProposeSession).unwrap();

    // Test.
    l1_events_provider.get_txs(1, BlockNumber(0)).unwrap();
}
...
fn pending_state_returns_error() {
    ...
    // Test.
    assert_eq!(
        l1_events_provider.get_txs(1, BlockNumber(0)),
        Err(L1EventsProviderError::UnexpectedProviderState {
            expected: ProviderState::Propose,
            found: ProviderState::Pending
        })
    );
    assert_eq!(
        l1_events_provider.validate(tx_hash!(1), BlockNumber(0)),
        Err(L1EventsProviderError::UnexpectedProviderState {
...
fn proposal_start_multiple_proposals_same_height() {
    ...
    for (session_1, session_2) in SESSION_TYPES.into_iter().cartesian_product(SESSION_TYPES) {
        l1_events_provider.start_block(BlockNumber(0), session_1).unwrap();
        l1_events_provider.start_block(BlockNumber(0), session_2).unwrap();
    }
}
...
fn commit_block_empty_block() {
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[], BlockNumber(10));
...
fn commit_block_during_proposal() {
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(1)], BlockNumber(5));
...
fn commit_block_during_pending() {
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(2)], BlockNumber(5));
...
fn commit_block_during_validation() {
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(3)], BlockNumber(5));
    let expected_l1_events_provider = L1EventsProviderContentBuilder::new()
...
async fn commit_block_backlog() {
    ...
    l1_events_provider
        .initialize(STARTUP_HEIGHT, vec![])
        .await
        .expect("l1 provider initialize failed");
    l1_events_provider.state = ProviderState::CatchingUp;
    l1_events_provider.catchupper.start_l2_sync(STARTUP_HEIGHT, TARGET_HEIGHT);

    // Test.
    // Commit height is below target height. Doesn't trigger backlog.
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(1)], STARTUP_HEIGHT);
    let expected_l1_events_provider = L1EventsProviderContentBuilder::new()
        .with_catchupper(catchupper.clone())
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[], TARGET_HEIGHT);
...
fn commit_block_before_add_tx_stores_tx_in_committed() {
    ...
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(2), tx_hash!(3)], BlockNumber(0));
    let expected_l1_events_provider = L1EventsProviderContentBuilder::new()
        .with_txs([])
        .with_committed([l1_handler(1)])
...
async fn catching_up_commit_block_received_twice_no_error() {
    ...
    l1_events_provider
        .initialize(BlockNumber(0), vec![])
        .await
        .expect("l1 provider initialize failed");
    call_commit_block_to_start_catching_up_and_expect_error(
        &mut l1_events_provider,
        BlockNumber(2),
    );
    ...
    // Test.
    commit_block_no_rejected(&mut l1_events_provider, &[tx_hash!(1)], BlockNumber(0));
...
async fn catching_up_commit_block_received_twice_error_if_new_uncommitted_txs() {
    ...
    assert_matches!(
        l1_events_provider
            ...
            .unwrap_err(),
        L1EventsProviderError::UnexpectedHeight {
...
async fn restart_service_if_initialized_in_steady_state() {
    ...
    // Test.
    l1_events_provider.initialize(BlockNumber(0), vec![]).await.unwrap();
}

#[test]
fn commit_block_rejected_transactions() {
    let l1_events_provider = setup_rejected_transactions();
...
fn validate_rejected_transactions(
    ...
    assert_eq!(l1_events_provider.validate(tx, BlockNumber(1)).unwrap(), expected_status);
}
...
fn commit_block_twice_panics() {
    ...
    // Test.
    l1_events_provider.commit_block([tx_hash!(1)].into(), [].into(), BlockNumber(0)).unwrap();
}
...
fn add_tx_identical_timestamp_both_stored() {
    ...
    let expected = L1EventsProviderContentBuilder::new()
        .with_timed_txs([
    ...
    // Test.
    let mut l1_events_provider = L1EventsProviderContentBuilder::new().build_into_l1_provider();
    l1_events_provider
        .add_events(vec![
    ...
    expected.assert_eq(&l1_events_provider);
}
...
fn get_txs_identical_timestamps() {
    ...
    let l1_events_provider_builder = L1EventsProviderContentBuilder::new()
        .with_timed_txs([
            (tx_1.clone(), timestamp_1),
            (tx_2.clone(), timestamp_2),
            (tx_3.clone(), timestamp_3),
    ...
    assert_eq!(
        l1_events_provider_builder
            .clone()
            .build_into_l1_provider()
            .get_txs(1, BlockNumber(0))
            .unwrap(),
        std::slice::from_ref(&tx_1)
    );

    assert_eq!(
        l1_events_provider_builder.build_into_l1_provider().get_txs(3, BlockNumber(0)).unwrap(),
        [tx_1, tx_2, tx_3]
    );
}
...
fn get_txs_timestamp_cutoff_some_eligible() {
    ...
    // Test.
    let result = l1_events_provider.get_txs(10, BlockNumber(0)).unwrap();
...
fn get_txs_excludes_cancellation_requested_and_returns_non_cancellation_requested() {
    ...
    // Test.
    assert_eq!(
        l1_events_provider.get_txs(4, l1_events_provider.current_height).unwrap(),
        vec![tx_2.clone()]
    );
}
...
fn get_txs_excludes_transaction_after_cancellation_expiry() {
    ...
    // Test.
    assert_eq!(l1_events_provider.get_txs(3, l1_events_provider.current_height).unwrap(), vec![]);
}
...
fn commit_block_commits_cancellation_requested_tx_not_expired() {
    ...
    let mut l1_events_provider = L1EventsProviderContentBuilder::new()
        ...
        .build_into_l1_provider();
    let expected =
        L1EventsProviderContentBuilder::new().with_txs([]).with_committed([tx.clone()]).build();

    // Test.
    l1_events_provider
        .commit_block([tx.tx_hash].into(), [].into(), l1_events_provider.current_height)
        .unwrap();
...
fn commit_block_commits_cancellation_requested_expired_and_fully_cancelled() {
    ...
    l1_events_provider.validate(tx_2.tx_hash, l1_events_provider.current_height).unwrap();
    ...
    l1_events_provider
        .commit_block(
            [tx_1.tx_hash, tx_2.tx_hash].into(),
            [].into(),
...
fn commit_block_commits_mixed_normal_and_cancellation_requested() {
    ...
    l1_events_provider
        .commit_block(txs.into(), [].into(), l1_events_provider.current_height)
        .unwrap();
...
fn add_events_tx_and_cancel_same_call_not_expired() {
    ...
    l1_events_provider.add_events(events.into()).unwrap();
...
fn add_events_tx_then_cancel_separate_calls_not_expired() {
    ...
    // Test.
    l1_events_provider.add_events(vec![l1_handler_event(tx_hash)]).unwrap();
...
fn add_events_tx_and_cancel_same_call_expired() {
    ...
    l1_events_provider.validate(tx.tx_hash, l1_events_provider.current_height).unwrap();
...
fn add_events_only_cancel_event_unknown_tx() {
    ...
    let mut l1_events_provider = L1EventsProviderContentBuilder::new().build_into_l1_provider();
    let expected_empty = L1EventsProviderContentBuilder::new()
        .with_txs([])
        .with_timed_cancel_requested_txs([])
        .build();

    // Test.
    l1_events_provider.add_events(vec![cancellation_event(unknown_tx_hash, 0.into())]).unwrap();

    expected_empty.assert_eq(&l1_events_provider);
}
...
fn add_events_double_cancellation_only_first_counted() {
    ...
    let mut l1_events_provider =
        L1EventsProviderContentBuilder::new().with_txs([tx.clone()]).build_into_l1_provider();
...
fn validate_tx_unknown_returns_invalid_not_found() {
    ...
    let status =
        l1_events_provider.validate(tx_hash!(1), l1_events_provider.current_height).unwrap();
...
fn commit_block_historical_height_short_circuits_non_catching_up() {
    ...
    let l1_events_provider_builder_clone = l1_events_provider_builder.clone();
    let expected_unchanged = l1_events_provider_builder.build();

    // Test.
    let mut l1_events_provider = l1_events_provider_builder_clone.build_into_l1_provider();
    let old_height = BlockNumber(4);
    l1_events_provider.commit_block([tx_hash!(1)].into(), [].into(), old_height).unwrap();

    expected_unchanged.assert_eq(&l1_events_provider);
}
...
async fn commit_block_historical_height_short_circuits_catching_up() {
    ...
    let l1_events_provider_builder = L1EventsProviderContentBuilder::new()
        ...
        .with_txs([l1_handler(1)]);
    let l1_events_provider_builder_clone = l1_events_provider_builder.clone();
    let mut l1_events_provider = l1_events_provider_builder.clone().build_into_l1_provider();
    l1_events_provider
        .initialize(STARTUP_HEIGHT, vec![])
        .await
        .expect("l1 provider initialize failed");
    call_commit_block_to_start_catching_up_and_expect_error(&mut l1_events_provider, TARGET_HEIGHT);

    let expected_unchanged = l1_events_provider_builder_clone
...
fn consuming_committed_tx() {
    ...
    let tx = l1_handler(1);
    let mut l1_events_provider =
        L1EventsProviderContentBuilder::new().with_committed([tx.clone()]).build_into_l1_provider();
...
fn consuming_rejected_tx() {
    ...
    let tx = l1_handler(1);
    let mut l1_events_provider =
        L1EventsProviderContentBuilder::new().with_rejected([tx.clone()]).build_into_l1_provider();
...
fn consuming_consumed_tx_panics() {
    ...
    assert!(l1_events_provider.tx_manager.records.contains_key(&tx.tx_hash));
...
fn consuming_tx_deletes_after_timelock() {
    ...
    let l1_events_provider_with_consumed = L1EventsProviderContentBuilder::new()
        .with_consumed_txs([ConsumedTransaction { tx: tx.clone(), timestamp: BlockTimestamp(0) }])
        .build();

    // The expected provider with the tx deleted.
    let l1_events_provider_with_consumed_deleted = L1EventsProviderContentBuilder::new().build();
    ...
    l1_events_provider_with_consumed.assert_eq(&l1_events_provider);

    // Advance the clock and assert the tx is deleted.
    clock.advance(l1_events_provider.config.l1_handler_consumption_timelock_seconds);
    ...
    l1_events_provider_with_consumed_deleted.assert_eq(&l1_events_provider);
}
...
fn consuming_multiple_txs_selective_deletion_after_timelock() {
    ...
    let expected_with_tx1_deleted = L1EventsProviderContentBuilder::new()
        .with_consumed_txs([
    ...
    expected_with_tx1_deleted.assert_eq(&l1_events_provider);
}
...
fn finalizing_cancellation_deletes_tx_from_records_and_proposable_index() {
    ...
    let mut l1_events_provider =
        L1EventsProviderContentBuilder::new().with_txs([tx.clone()]).build_into_l1_provider();
    let snapshot = l1_events_provider.tx_manager.snapshot();
    ...
    // Test.
    l1_events_provider
        .add_events(vec![Event::TransactionCanceled { tx_hash: tx.tx_hash }])
        .unwrap();
...
fn catching_up_commit_block_received_while_uninitialized() {
    ...
    // Test.
    let result = l1_events_provider.commit_block([].into(), [].into(), BlockNumber(1));
    assert!(result.is_err());
    assert_eq!(l1_events_provider.state, ProviderState::Uninitialized);
}
...
async fn send_commit_block(
    l1_events_provider_client: &FakeL1EventsProviderClient,
...
async fn catching_up_e2e() {
    ...
    let l1_events_provider_client = Arc::new(FakeL1EventsProviderClient::default());
    ...
    let mut l1_events_provider = L1EventsProvider::new(
        config,
        l1_events_provider_client.clone(),
        Arc::new(sync_client),
        None,
    );
    ...
    l1_events_provider.initialize(STARTUP_HEIGHT, scraped_l1_handler_txs).await.unwrap();
    call_commit_block_to_start_catching_up_and_expect_error(
        &mut l1_events_provider,
        CATCH_UP_HEIGHT,
    );
    ...
    // structure.
    let mut commit_blocks = l1_events_provider_client.commit_blocks_received.lock().unwrap();
    ...
    receive_commit_block(&mut l1_events_provider, &next_block.committed_txs, next_block.height);
    assert_eq!(l1_events_provider.current_height, BlockNumber(3));
    ...
    let next_block = commit_blocks.next().unwrap();
    receive_commit_block(&mut l1_events_provider, &next_block.committed_txs, next_block.height);
    assert_eq!(l1_events_provider.current_height, BlockNumber(4));

    // Apply height 4 ==> this triggers committing the backlogged heights 5 and 6.
    let next_block = commit_blocks.next().unwrap();
    receive_commit_block(&mut l1_events_provider, &next_block.committed_txs, next_block.height);
    assert_eq!(l1_events_provider.current_height, BlockNumber(7));

    // Assert that the catching up has been completed.
    assert!(!l1_events_provider.state.is_catching_up());
}
...
async fn catching_up_delayed_batcher_and_sync_state_with_trivial_catch_up() {
    ...
    l1_events_provider.initialize(STARTUP_HEIGHT, scraped_l1_handler_txs.into()).await.unwrap();
    call_commit_block_to_start_catching_up_and_expect_error(
        &mut l1_events_provider,
        CATCH_UP_HEIGHT,
    );
    ...
    send_commit_block(&l1_events_provider_client, &no_txs_committed, STARTUP_HEIGHT).await;
    send_commit_block(&l1_events_provider_client, &no_txs_committed, height_add(STARTUP_HEIGHT, 1))
        .await;

    // Forward all messages buffered in the client to the provider.
    l1_events_provider_client.flush_messages(&mut l1_events_provider).await;

    // Commit blocks should have been applied.
    let start_height_plus_2 = height_add(STARTUP_HEIGHT, 2);
    assert_eq!(l1_events_provider.current_height, start_height_plus_2);
    ...
    // state.
    receive_commit_block(&mut l1_events_provider, &no_txs_committed.into(), start_height_plus_2);
    assert_eq!(l1_events_provider.current_height, height_add(start_height_plus_2, 1));
...
async fn catching_up_delayed_sync_state_with_sync_behind_batcher() {
    ...
    let no_txs_committed = []; // Not testing txs in this test.
    send_commit_block(&l1_events_provider_client, &no_txs_committed, batcher_height).await;
    tokio::time::sleep(config.startup_sync_sleep_retry_interval_seconds).await;
    send_commit_block(
        &l1_events_provider_client,
        &no_txs_committed,
        batcher_height.unchecked_next(),
    )
    .await;
    ...
    assert_eq!(l1_events_provider.current_height, startup_height);
    ...
    assert_eq!(l1_events_provider.current_height, batcher_height.unchecked_next().unchecked_next());
...
async fn test_stuck_sync() {
    ...
    for i in 0..=(Catchupper::MAX_HEALTH_CHECK_FAILURES + 1) {
        receive_commit_block(
            &mut l1_events_provider,
            &[].into(),
            height_add(STARTUP_HEIGHT, i.into()),
        );
        tokio::time::sleep(config.startup_sync_sleep_retry_interval_seconds).await;
    }
}
...
async fn provider_initialized_in_pending_is_same_as_uninitialized_after_getting_initialize() {
    // Setup.
    const STARTUP_HEIGHT: BlockNumber = BlockNumber(2);
    let mut provider_starts_uninitialized = L1EventsProviderContentBuilder::new()
        .with_state(ProviderState::Uninitialized)
        .build_into_l1_provider();
    assert!(provider_starts_uninitialized.state.is_uninitialized());

    let provider_starts_pending =
        L1EventsProviderContentBuilder::new().with_height(STARTUP_HEIGHT).build_into_l1_provider();
...
async fn tx_cancelled_during_initialization_never_reaches_proposable_index() {
    ...
    let tx = l1_handler_event(tx_hash);
    let cancel_event =
        cancellation_event(tx_hash, BlockTimestamp(clock.now().timestamp().try_into().unwrap()));
    let mut l1_events_provider = L1EventsProviderContentBuilder::new()
        .with_state(ProviderState::Uninitialized)
        .with_clock(clock.clone())
        .build_into_l1_provider();
    l1_events_provider.initialize(STARTUP_HEIGHT, vec![tx, cancel_event]).await.unwrap();
