...
async fn new_l1_handler_tx_propose_validate_cooldown() {
    ...
    l1_events_provider_client
        .commit_block([l2_hash].into(), [].into(), next_block_height)
        .await
        .unwrap();
    let new_height = next_block_height.unchecked_next();

    // Make sure the message is no longer available for validation/proposal.
    l1_events_provider_client.start_block(SessionState::Validate, new_height).await.unwrap();
    assert_eq!(
        l1_events_provider_client.validate(l2_hash, new_height).await.unwrap(),
        ValidationStatus::Invalid(InvalidValidationStatus::AlreadyIncludedOnL2)
    );
    l1_events_provider_client.start_block(SessionState::Propose, new_height).await.unwrap();
    let txs = l1_events_provider_client.get_txs(n_txs, new_height).await.unwrap();
    assert!(txs.is_empty());
    ...
    l1_events_provider_client.start_block(SessionState::Validate, new_height).await.unwrap();
    assert_eq!(
        l1_events_provider_client.validate(l2_hash_2, new_height).await.unwrap(),
        ValidationStatus::Validated
    );
