...
impl L1ProviderContentBuilder {
    ...
    pub fn with_committed_txs(
        mut self,
        tx_hashes: impl IntoIterator<Item = TransactionHash>,
    ) -> Self {
        self.tx_manager_content_builder =
            self.tx_manager_content_builder.with_committed_txs(tx_hashes);
        self
    }
...
struct TransactionManagerContent {
    ...
    committed_txs: Option<IndexSet<TransactionHash>>,
}
...
impl TransactionManagerContent {
    fn complete_to_tx_manager(mut self) -> TransactionManager {
        ...
        TransactionManager {
            txs: StagedRemovalIndexMap::new(txs),
            committed: self
                .committed_txs
                .unwrap_or_default()
                .into_iter()
...
impl TransactionManagerContentBuilder {
    ...
    fn with_committed_txs(mut self, tx_hashes: impl IntoIterator<Item = TransactionHash>) -> Self {
        self.committed_txs = Some(tx_hashes.into_iter().collect());
        self
    }
    ...
    fn build(self) -> Option<TransactionManagerContent> {
        ...
        Some(TransactionManagerContent { txs: self.txs, committed_txs: self.committed_txs })
    }

    fn is_default(&self) -> bool {
        self.txs.is_none() && self.committed_txs.is_none()
    }
}
