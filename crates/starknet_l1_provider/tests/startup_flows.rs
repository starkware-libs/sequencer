...
async fn bootstrap_delayed_sync_state_with_trivial_catch_up() {
    ...
    flush_l1_provider_client(&mut l1_provider, l1_provider_client.clone()).await;
...
// Test-util boilerplate: apply the changes from the fake l1 provider client.
// Once infra supports creating up fake clients on the fly this won't be necessary anymore.
// Consider making this a method of the fake client that takes an L1Provider otherwise, would
// require supporting all types though.
async fn flush_l1_provider_client(
    l1_provider: &mut L1Provider,
    l1_provider_client: Arc<FakeL1ProviderClient>,
) {
    let commit_blocks =
