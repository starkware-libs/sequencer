...
impl BenchmarkFlavor {
    ...
    fn n_iterations(&self, n_iterations: usize) -> usize {
        match self {
            ...
            Self::Mainnet => min(5_000_000, n_iterations),
        }
    }
}
...
pub async fn run_storage_benchmark<S: Storage>(
    ...
    for block_number in curr_block_number..n_iterations {
        ...
        if (block_number + 1) % checkpoint_interval == 0 {
            let storage_stats = index_db.storage.get_stats();
            index_db.storage.reset_stats().unwrap();
        ...
        if let Some(async_storage) = index_db.storage.get_async_self() {
    ...
    if !n_iterations.is_multiple_of(checkpoint_interval) {
        time_measurement.to_csv(
            ...
            output_dir,
            index_db.storage.get_stats().map(|s| Some(s.column_values())).unwrap_or(None),
        );
    }
