...
fn apply_interference<S: AsyncStorage>(
    ...
    match interference_type {
        ...
        InterferenceType::Read1KEveryBlock => {
            ...
            if [BenchmarkFlavor::Mainnet, BenchmarkFlavor::MainnetWithSleeps]
                .contains(&benchmark_flavor)
            {
                return;
            }
...
pub async fn run_storage_benchmark<S: Storage>(
    ...
    let batcher_storage_reader: Option<StorageReader> =
        if [BenchmarkFlavor::Mainnet, BenchmarkFlavor::MainnetWithSleeps].contains(&flavor) {
    ...
    for block_number in curr_block_number..n_iterations {
        ...
        let filled_forest =
            CommitBlockImpl::commit_block(input, &mut index_db, Some(&mut time_measurement))
                .await
        ...
        if (block_number + 1) % checkpoint_interval == 0 {
            let storage_stats = index_db.get_storage().get_stats();
            index_db.get_mut_storage().reset_stats().unwrap();
        ...
        if flavor == BenchmarkFlavor::MainnetWithSleeps {
            let milliseconds_to_sleep = sleep_between_iterations(
                block_number,
        ...
        if let Some(async_storage) = index_db.get_storage().get_async_self() {
    ...
    if !n_iterations.is_multiple_of(checkpoint_interval) {
        time_measurement.to_csv(
            ...
            output_dir,
            index_db.get_storage().get_stats().map(|s| Some(s.column_values())).unwrap_or(None),
        );
    }
...
fn sleep_between_iterations(
    ...
    let next_block_timestamp = state_sync_storage_reader
        ...
        .timestamp
        .0;
    let timestamp_diff_ms = next_block_timestamp.checked_sub(current_block_timestamp)? * 1000;
