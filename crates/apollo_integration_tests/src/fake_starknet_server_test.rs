...
async fn feeder_json_hash_patched_when_hash_confirmed_before_blob() {
    ...
    let blob = json!({
        ...
        "fee_market_info": {"l2_gas_consumed": "0x0", "next_l2_gas_price": "0x1"}
    });
    post_json(&client, &url(&server, "/cende_recorder/write_blob"), &blob).await;
...
async fn test_server() -> FakeStarknetServer {
    use std::net::SocketAddr;
    FakeStarknetServer::new(SocketAddr::from(([127, 0, 0, 1], 0))).await
}
