...
#[tokio::test]
async fn get_block_header_only_returns_subset() {
    ...
    let (status, body) = get(
        ...
        &url(
            &server,
            &format!(
                "/feeder_gateway/get_block?blockNumber={BLOCK_NUMBER}&headerOnly=true&\
    ...
    assert_eq!(returned["block_hash"], format!("0x{}", BLOCK_NUMBER));
    // No extra fields beyond block_number and block_hash.
    assert_eq!(returned.as_object().unwrap().len(), 2);
}

#[tokio::test]
async fn get_state_update_returns_stored_block() {
...
async fn get_state_update_pending_returns_pending_data() {
    ...
    server.state.lock().unwrap().pending_data_json = Some(pending.to_string());
...
#[tokio::test]
async fn get_signature_returns_stored_block() {
...
async fn get_class_by_hash_returns_class() {
    ...
    server.state.lock().unwrap().classes_json.insert(CLASS_HASH.to_string(), class.to_string());
...
async fn get_compiled_class_returns_compiled_class() {
    ...
    server
        ...
        .insert(CLASS_HASH.to_string(), compiled.to_string());

    let client = reqwest::Client::new();
    let (status, body) = get(
        &client,
        &url(&server, "/feeder_gateway/get_compiled_class_by_class_hash?classHash=0xdef"),
    )
    .await;
...
async fn get_public_key() {
    ...
    server.state.lock().unwrap().sequencer_pub_key_json = Some(pub_key.to_string());
...
#[tokio::test]
async fn write_blob_stores_block_in_map() {
    ...
    assert_eq!(status, StatusCode::OK);
    let stored = server.state.lock().unwrap().blocks.get(&BLOCK_NUMBER).cloned();
...
// Combination of Cender and feeder.

#[tokio::test]
async fn write_via_cende_then_read_via_feeder() {
    const BLOCK_NUMBER: u64 = 8;
    let server = FakeStarknetServer::new().await;
    let client = reqwest::Client::new();
    let blob = make_blob(BLOCK_NUMBER);

    // Write block via Cende recorder endpoint.
    let write_status = post_json(&client, &url(&server, "/cende_recorder/write_blob"), &blob).await;
    assert_eq!(write_status, StatusCode::OK);

    // Read back via feeder gateway by number.
    let (read_status, body) = get(
        &client,
    ...
    // Visible as the latest block.
    let (latest_status, latest_body) =
        get(&client, &url(&server, "/feeder_gateway/get_block?blockNumber=latest")).await;
    assert_eq!(latest_status, StatusCode::OK);
    assert_eq!(serde_json::from_str::<Value>(&latest_body).unwrap()["block_number"], BLOCK_NUMBER);

    // get_latest_received_block reflects the write.
    let (cende_status, cende_body) =
        get(&client, &url(&server, "/cende_recorder/get_latest_received_block")).await;
    assert_eq!(cende_status, StatusCode::OK);
    assert_eq!(serde_json::from_str::<Value>(&cende_body).unwrap()["block_number"], BLOCK_NUMBER);
}
