...
impl NodeSetup {
    ...
    pub fn get_batcher(&self) -> &ExecutableSetup {
        self.executables
            .iter()
            .find(|(components, _)| components.contains(&ComponentConfigInService::Batcher))
            .map(|(_, executable)| executable)
            .expect("No executable with a batcher component.")
    }

    pub fn get_http_server(&self) -> &ExecutableSetup {
        self.executables
            .iter()
            .find(|(components, _)| components.contains(&ComponentConfigInService::HttpServer))
            .map(|(_, executable)| executable)
            .expect("No executable with an HTTP server component.")
    }

    pub fn get_state_sync(&self) -> &ExecutableSetup {
        self.executables
            .iter()
            .find(|(components, _)| components.contains(&ComponentConfigInService::StateSync))
            .map(|(_, executable)| executable)
            .expect("No executable with a state sync component.")
    }

    pub fn get_consensus_manager(&self) -> &ExecutableSetup {
        self.executables
            .iter()
            .find(|(components, _)| components.contains(&ComponentConfigInService::Consensus))
            .map(|(_, executable)| executable)
            .expect("No executable with a consensus manager component.")
    }
