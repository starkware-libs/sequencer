...
impl IntegrationTestManager {
    pub async fn new(
        ...
        fn get_base_layer_config(sequencers_setup: &NodeSetup) -> (EthereumBaseLayerConfig, Url) {
            ...
            let mut base_layer_config = None;
            for executable_setup in &sequencers_setup.executables {
                if let Some(config) = &executable_setup.get_config().base_layer_config {
                    base_layer_config = Some(config.clone());
                }
            }
            let base_layer_config =
                base_layer_config.expect("No executable with a set base_layer_config.");
            let mut base_layer_url = None;
            for executable_setup in &sequencers_setup.executables {
                if let Some(l1_monitor_config) =
                    &executable_setup.get_config().l1_endpoint_monitor_config
                {
                    base_layer_url = Some(l1_monitor_config.ordered_l1_endpoint_urls[0].clone());
                }
            }
            let base_layer_url =
                base_layer_url.expect("No executable with a set l1_endpoint_monitor_config.");
            (base_layer_config, base_layer_url)
        }
        ...
        let (anvil, starknet_l1_contract) =
            spawn_anvil_and_deploy_starknet_l1_contract(&base_layer_config, base_layer_url.clone())
                .await;
