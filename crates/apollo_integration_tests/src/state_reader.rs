...
impl PresetTestContracts {
    pub fn new() -> Self {
        ...
        // Cairo1 ERC20 so fee debits during execution use Sierra gas as required by the virtual OS.
    ...
    fn new_for_proof_flow() -> Self {
        let into_contract = |contract: FeatureContract| Contract {
            contract,
            sender_address: contract.get_instance_address(0),
        };
        let default_test_contracts = vec![into_contract(FeatureContract::TestContract(
...
/// Serialized class blobs for the proof-flow integration genesis.
pub struct ProofFlowGenesisClasses {
    pub cairo1_contract_classes: Vec<(ClassHash, (SierraContractClass, CasmContractClass))>,
}

pub fn proof_flow_integration_chain_info() -> ChainInfo {
    // Override STRK fee token to match the virtual OS test for the proof flow.
...
pub fn proof_flow_integration_genesis_data(
    ...
    let preset_test_contracts = PresetTestContracts::new_for_proof_flow();
    let test_defined_accounts = create_proof_flow_tx_generator().accounts().to_vec();
    let classes = TestClasses::new(&test_defined_accounts, preset_test_contracts.clone());
    let thin = prepare_state_diff(chain_info, &test_defined_accounts, &preset_test_contracts);
    (thin, ProofFlowGenesisClasses { cairo1_contract_classes: classes.cairo1_contract_classes })
}
