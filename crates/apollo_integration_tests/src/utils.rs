...
pub const N_TXS_IN_NON_GENERIC_INVOKE_TXS: usize = 7;
...
pub fn create_deploy_account_tx_and_invoke_tx(
    ...
    let invoke_tx = undeployed_account_tx_generator.generate_trivial_rpc_invoke_tx_with_tip(1);
...
pub fn create_invoke_txs(
    ...
    (0..n_txs)
        .map(|_| {
            tx_generator.account_with_id_mut(account_id).generate_trivial_rpc_invoke_tx_with_tip(1)
        })
        .collect()
}
