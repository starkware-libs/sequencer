...
pub fn create_node_config(
    ...
    gateway_config.static_config.recorder_url = recorder_url.clone();
    mempool_config.static_config.recorder_url = recorder_url;
    ...
    let l1_gas_price_scraper_config =
        L1GasPriceScraperConfig { chain_id: chain_info.chain_id.clone(), ..Default::default() };
    let sierra_compiler_config = SierraCompilationConfig {
        max_cpu_time: batcher_config
            .static_config
...
pub(crate) fn create_consensus_manager_configs_from_network_configs(
    ...
    let network_configs: Vec<NetworkConfig> = network_configs
        .into_iter()
...
pub fn create_state_sync_configs(
    ...
    create_connected_network_configs(ports)
        ...
        .map(|mut network_config| {
            ...
            rpc_config.execution_config.eth_fee_contract_address =
                chain_info.fee_token_addresses.eth_fee_token_address;
            rpc_config.execution_config.strk_fee_contract_address =
