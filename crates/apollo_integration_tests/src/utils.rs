...
pub async fn spawn_fake_recorder(port: u16) -> FakeStarknetServer {
    let server = FakeStarknetServer::new(port).await;
    ...
    server.state.lock().unwrap().blocks.insert(0, json!({"block_number": 0}));
    server
}
