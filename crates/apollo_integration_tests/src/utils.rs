...
pub fn create_node_config(
    ...
    let l1_events_provider_config =
        wrap_if_component_config_expected!(l1_events_provider, l1_events_provider_config);
    let l1_events_scraper_config =
        wrap_if_component_config_expected!(l1_events_scraper, l1_events_scraper_config);
...
/// The commitment infos heights a dummy recorder stored.
#[derive(Debug, Default)]
struct StoredCommitmentInfosHeights {
    ...
    offset: Option<u64>,
    /// The heights of the last stored blob, used to recognize its retried write.
    last_stored_range: Option<RangeInclusive<u64>>,
}
...
pub struct RecorderStats {
    ...
    /// Blobs whose commitment infos skipped heights, repeated stored ones or arrived out of
    /// order, breaking the exactly-once delta the served height offset defines.
    pub num_blobs_with_unexpected_delta: AtomicUsize,
    stored_commitment_infos_heights: Mutex<StoredCommitmentInfosHeights>,
}

impl RecorderStats {
    /// Stores the heights of a blob's commitment infos, counting the blob as unexpected unless
    /// they are the contiguous ascending continuation of the heights already stored.
    fn store_commitment_infos_heights(&self, heights: &[u64]) {
        let mut stored = self.stored_commitment_infos_heights.lock().unwrap();
        let (first_height, last_height) = (heights[0], heights[heights.len() - 1]);
        let is_contiguous_ascending =
            heights.windows(2).all(|heights| heights[1] == heights[0] + 1);
        let continues_stored_heights = match stored.offset {
            // The sender falls back to its whole recent-blocks window while the recorder is
            // empty, so the first blob may start at any height.
            None => true,
            Some(offset) => first_height == offset,
        };
        // The cende client retries a write it considers transiently failed, resending a blob the
        // recorder already stored.
        let is_retried_write =
            stored.last_stored_range.as_ref() == Some(&(first_height..=last_height));
        if !is_contiguous_ascending || !(continues_stored_heights || is_retried_write) {
            warn!(
                "Blob carries commitment infos for heights {heights:?}, which do not continue the \
                 heights stored up to offset {:?}.",
                stored.offset,
            );
            self.num_blobs_with_unexpected_delta.fetch_add(1, AtomicOrdering::Relaxed);
        }
        stored.offset = stored.offset.max(Some(last_height + 1));
        stored.last_stored_range = Some(first_height..=last_height);
    }
}
...
/// Creates a local recorder server that stores the commitment infos of every received blob, serves
/// their height offset, and exposes counters of what it received. A blob whose commitment infos do
/// not continue the stored heights is counted but still accepted, so the flow reaches the
            ...
            .route(
                ...
                post(move |Json(blob): Json<serde_json::Value>| {
                    ...
                    async move {
                        ...
                        let state_commitment_infos = blob["recent_state_commitment_infos"]
                            .as_array()
                            .cloned()
                            .unwrap_or_default();
                        if state_commitment_infos.is_empty() {
                            // A blob is prepared before its own height's commitment completes, so
                            // early blobs legitimately carry no commitment infos.
                            return (StatusCode::OK, StatusCode::OK.to_string());
                        }
                        recorder_stats
                            .num_blobs_with_state_commitment_infos
                            .fetch_add(1, AtomicOrdering::Relaxed);
                        let Some(heights) = state_commitment_infos
                            .iter()
                            .map(|entry| entry.get("block_number").and_then(|value| value.as_u64()))
                            .collect::<Option<Vec<u64>>>()
                        else {
                            recorder_stats.num_invalid_blobs.fetch_add(1, AtomicOrdering::Relaxed);
                            warn!("Blob carries state commitment infos without a block number.");
                            return (StatusCode::OK, StatusCode::OK.to_string());
                        };
            ...
            .route(
                ...
                get(move || {
                    ...
                    async move {
                        ...
                        let height_offset =
                            recorder_stats.stored_commitment_infos_heights.lock().unwrap().offset;
...
pub async fn end_to_end_flow(args: EndToEndFlowArgs) {
    ...
    let n_proposing_sequencers =
        sequencers.iter().filter(|s| s.add_tx_http_client.is_some()).count();
    let chain_id = mock_running_system.chain_id().clone();
    let mut send_rpc_tx_fn = |tx| sequencer_to_add_txs.assert_add_tx_success(tx);

    // Only proposing sequencers reliably increment BATCHED_TRANSACTIONS via decision_reached.
    // Validation-only nodes may take the state sync path instead of the batcher path when
    // committing a block, in which case their batcher never calls decision_reached and the counter
    // is not incremented. This tracks the cumulative count across all scenarios.
...
/// and that the recorders found no blob malformed.
async fn verify_recorder_blobs_flow(
    ...
    // carry the previously committed heights' infos. An invalid blob never carries them, so stop
    // waiting once one is seen and let the assertions below report it.
    timeout(scenario_timeout, async {
        while total_across_sequencers(|stats| &stats.num_blobs_with_state_commitment_infos) == 0
            && total_across_sequencers(|stats| &stats.num_invalid_blobs) == 0
        {
    ...
    .unwrap_or_else(|_| {
        panic!(
            "No cende blob carried state commitment infos: the recorders received {} blobs, of \
             which {} were invalid.",
            total_across_sequencers(|stats| &stats.num_blobs_received),
            total_across_sequencers(|stats| &stats.num_invalid_blobs),
        )
    });
    ...
    for sequencer in sequencers {
        ...
        assert_eq!(
            sequencer.recorder_stats.num_invalid_blobs.load(AtomicOrdering::Relaxed),
            0,
            "The recorder of sequencer {node_index} received malformed cende blobs.",
        );
        assert_eq!(
            sequencer.recorder_stats.num_blobs_with_unexpected_delta.load(AtomicOrdering::Relaxed),
            0,
            "The recorder of sequencer {node_index} received cende blobs whose commitment infos \
             did not continue the heights it had stored.",
        );
    }
    assert!(
        total_across_sequencers(|stats| &stats.num_blobs_with_state_commitment_infos) > 0,
        "No cende blob carried state commitment infos.",
    );
}
    ...
    for sequencer in sequencers {
        ...
        for block_number in (0..global_root_height.0).map(BlockNumber) {
            ...
            match state_commitment_infos.as_ref() {
                ...
                None => assert!(
                    !prior_block_witnesses_found,
                    "Block {block_number} on sequencer {node_index} is missing witnesses although \
        ...
        let num_heights_above_genesis = global_root_height.0 - 1;
        assert_eq!(
            num_heights_with_witnesses, num_heights_above_genesis,
            "Sequencer {node_index} persisted witnesses for {num_heights_with_witnesses} of the \
             {num_heights_above_genesis} committed heights above genesis.",
        );
    }
}
