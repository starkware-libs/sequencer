...
async fn main() {
    ...
    info!("Awaiting for all running nodes to revert back to block {BLOCK_TO_WAIT_FOR_DEPLOY_AND_INVOKE}.");
    ...
    info!("All nodes reverted to block {BLOCK_TO_WAIT_FOR_DEPLOY_AND_INVOKE}. Shutting down nodes.");
    integration_test_manager.shutdown_nodes(node_indices.clone());
