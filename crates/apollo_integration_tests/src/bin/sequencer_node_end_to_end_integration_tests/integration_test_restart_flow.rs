...
async fn main() {
    ...
    const N_DISTRIBUTED_SEQUENCERS: usize = 3;
    ...
    // The test restarts a distributed node and shuts down another distributed node.
    const RESTART_NODE: usize = N_CONSOLIDATED_SEQUENCERS + N_HYBRID_SEQUENCERS;
    ...
    // Assert that RESTART_NODE is a distributed node.
    assert_eq!(integration_test_manager.get_node_type(RESTART_NODE), NodeType::Distributed);
    // Assert that SHUTDOWN_NODE is also a distributed node.
    assert_eq!(integration_test_manager.get_node_type(SHUTDOWN_NODE), NodeType::Distributed);

    let mut node_indices = integration_test_manager.get_node_indices();

    info!(
        "Running all nodes: {N_CONSOLIDATED_SEQUENCERS} consolidated, {N_HYBRID_SEQUENCERS} \
         hybrid, and {N_DISTRIBUTED_SEQUENCERS} distributed sequencers"
    );
