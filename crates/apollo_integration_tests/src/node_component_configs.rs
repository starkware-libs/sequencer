...
use apollo_deployments::deployments::validator::VALIDATOR_NODE_REQUIRED_PORTS_NUM;
...
pub fn create_validator_component_configs(
    available_ports_generator: &mut AvailablePortsGenerator,
) -> NodeComponentConfigs {
    let mut available_ports = available_ports_generator
        .next()
        .expect("Failed to get an AvailablePorts instance for validator node configs");

    let ports = available_ports.get_next_ports(VALIDATOR_NODE_REQUIRED_PORTS_NUM);
    let mut services_component_config = NodeType::Validator.get_component_configs(Some(ports));
