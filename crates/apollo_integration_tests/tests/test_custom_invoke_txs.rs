...
pub fn create_cairo_1_syscall_test_txs(
    ...
    txs.extend(generate_test_deploy_txs(account_tx_generator, &test_contract, DEFAULT_TIP));
...
fn generate_custom_not_nested_invoke_txs(
    ...
    .map(|(fn_name, fn_args)| {
        account_tx_generator.generate_rpc_invoke_tx(
            DEFAULT_TIP,
            create_calldata(test_contract.get_instance_address(0), fn_name, fn_args),
        )
    })
    .collect()
}
...
fn generate_test_deploy_txs(
    ...
    txs.push(account_tx_generator.generate_rpc_invoke_tx(
        DEFAULT_TIP,
...
fn generate_test_get_execution_info_without_block_info_invoke_tx(
    ...
    let resources_bounds_values = match test_valid_resource_bounds() {
        ValidResourceBounds::AllResources(resources_bounds_values) => resources_bounds_values,
    ...
    account_tx_generator.generate_rpc_invoke_tx(
        ...
        create_calldata(test_contract.get_instance_address(0), fn_name, &calldata),
    )
}
