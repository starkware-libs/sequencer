...
fn generate_direct_test_contract_invoke_txs_cairo_0_syscall(
    ...
    [
        ("advance_counter", vec![felt!(2021_u64), felt!(7_u64), felt!(7_u64)]),
        ("xor_counters", vec![felt!(2021_u64), felt!(31_u64), felt!(21_u64)]),
        ("test_ec_op", vec![]),
    ]
    .iter()
    .map(|(fn_name, fn_args)| {
...
fn generate_invoke_txs_tests_for_deploy_contract(
    ...
    txs.push(
        account_tx_generator
            .invoke_tx_builder()
            .tip(DEFAULT_TIP)
            .calldata(test_re_entrance_calldata)
            .build_rpc_invoke_tx(),
    );
    ...
    txs.push(
        account_tx_generator
            .invoke_tx_builder()
            .tip(DEFAULT_TIP)
            .calldata(deploy_and_call_calldata)
            .build_rpc_invoke_tx(),
    );
