...
fn generate_direct_test_contract_invoke_txs_cairo_0_syscall(
    ...
    .map(|(fn_name, fn_args)| {
        ...
        rpc_invoke_tx(account_tx_generator.invoke_tx_builder().calldata(calldata))
    })
    .collect()
}
...
fn generate_invoke_txs_with_signature_cairo_0_syscall(
    ...
    vec![rpc_invoke_tx(
        account_tx_generator.invoke_tx_builder().calldata(calldata).signature(signature),
    )]
}
...
fn generate_invoke_txs_tests_for_deploy_contract(
    ...
    txs.push(rpc_invoke_tx(account_tx_generator.invoke_tx_builder().calldata(calldata)));
    ...
    let test_call_storage_consistency_args =
        vec![*newly_deployed_contract_address.0.key(), felt!(5_u64)];
    ...
    txs.push(rpc_invoke_tx(
        account_tx_generator.invoke_tx_builder().calldata(test_call_storage_consistency_calldata),
    ));

    // 2. test_re_entrance - check calculations based on the storage of the newly deployed contract.
    let test_re_entrance_args = vec![*newly_deployed_contract_address.0.key(), felt!(2_u64)];
    ...
    txs.push(rpc_invoke_tx(
        account_tx_generator.invoke_tx_builder().calldata(test_re_entrance_calldata),
    ));

    // 3. test_deploy_and_call - This function deploys test_contract.cairo and calls it.
    ...
    txs.push(rpc_invoke_tx(
        account_tx_generator.invoke_tx_builder().calldata(deploy_and_call_calldata),
    ));
