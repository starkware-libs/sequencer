...
fn generate_nested_library_call_invoke_txs(
    ...
    [("test_sha256", test_sha256_args), ("test_circuit", test_circuit_args)]
        .iter()
        .map(|(fn_name, fn_args)| {
            account_tx_generator.generate_nested_call_invoke_tx(
                DEFAULT_TIP,
                &test_contract,
...
/// Creates a few transaction to test the deploy functionality.
/// 1. test_deploy_contract - deploy a contract with constructor args and salt.
/// 2. test_call_contract on test_storage_write - write to storage via call_contract of the deployed contract.
/// 3. test_library_call on test_storage_read - read value from storage via test_library_call of the deployed contract.
/// 4. test_replace_class - replace the class of the deployed contract with an empty contract.
fn generate_test_deploy_txs(
    ...
    let newly_deployed_contract_address = calculate_contract_address(
        ...
        // Constructor calldata of the above test_deploy.
    ...
    txs.push(account_tx_generator.generate_nested_call_invoke_tx(
        tip,
        &test_contract,
