...
fn compare_compilation_data(contract: &FeatureContract) {
    ...
    match expected_compiled_raw_output {
        ...
        CompilationArtifacts::Cairo1 { casm, sierra } => {
            ...
            handel_casm_runnable_hash_test_case(contract, Some(&existing_compiled_contents));
        }
    }
}
...
fn handel_casm_runnable_hash_test_case(contract: &FeatureContract, casm_json: Option<&str>) {
...
// async fn verify_contracts_casm_hash(
//     #[values(HashVersion::V1, HashVersion::V2)] hash_version: HashVersion,
// ) {
//     for contract in FeatureContract::all_feature_contracts() {
//         match contract.cairo_version() {
//             CairoVersion::Cairo0 => assert_eq!(
//                 contract.get_compiled_class_hash(&hash_version),
//                 CompiledClassHash::default()
//             ),
//             CairoVersion::Cairo1(RunnableCairo1::Casm) => {
//                 handel_casm_runnable_hash_test_case(&contract, &hash_version)
//             }
//             #[cfg(feature = "cairo_native")]
//             // Native have the same casm as runnable casm so there is no need to test both.
//             CairoVersion::Cairo1(RunnableCairo1::Native) => {
//                 handel_casm_runnable_hash_test_case(&contract, &hash_version)
//             }
//         }
//     }
//     assert_eq!(
//         FeatureContract::ERC20(CairoVersion::Cairo0).get_compiled_class_hash(&hash_version),
//         CompiledClassHash::default()
//     );
//     handel_casm_runnable_hash_test_case(
