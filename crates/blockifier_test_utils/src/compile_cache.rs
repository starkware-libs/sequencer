...
pub fn ensure_cairo1_compiled(contract: &FeatureContract) {
    ...
    // Ensure the compiler package is available before taking the per-contract compilation lock.
    // `verify_cairo1_package` has its own per-version lock so concurrent downloads are serialized
    // without holding up compilation of other contracts.
