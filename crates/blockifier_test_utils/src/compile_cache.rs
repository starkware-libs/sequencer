...
use crate::cairo_compile::{
    ...
    cairo1_compile,
    verify_cairo1_package_blocking,
...
fn libfunc_file_for_contract(contract: &FeatureContract) -> String {
    match contract {
        FeatureContract::Experimental => {
            resolve_crate_relative("resources/experimental_libfuncs.json")
        }
        FeatureContract::LegacyTestContract | FeatureContract::CairoStepsTestContract => {
            allowed_libfuncs_legacy_json_path()
        }
        _ => allowed_libfuncs_json_path(),
    }
}

fn libfunc_arg_for_contract(contract: &FeatureContract) -> LibfuncArg {
    match contract {
        FeatureContract::Experimental => {
            LibfuncArg::ListFile(resolve_crate_relative("resources/experimental_libfuncs.json"))
        }
        FeatureContract::LegacyTestContract | FeatureContract::CairoStepsTestContract => {
...
pub fn ensure_cairo1_compiled(contract: &FeatureContract) {
    ...
    let libfunc_file = libfunc_file_for_contract(contract);
    ...
    verify_cairo1_package_blocking(&version);

    let libfunc_arg = libfunc_arg_for_contract(contract);
