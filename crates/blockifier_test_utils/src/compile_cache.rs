...
pub fn ensure_cairo1_compiled(contract: &FeatureContract) {
    ...
    // Fast path: no lock needed when the cache is already fresh.
    if is_cache_fresh(&casm_path, &cache_key) && is_cache_fresh(&sierra_path, &cache_key) {
        return;
    }

    // Acquire a per-contract file lock to prevent redundant compilations across threads and
    // processes. The lock file lives next to the artifacts so it shares the same filesystem.
    let lock_path = lock_file_path(contract);
    fs::create_dir_all(lock_path.parent().unwrap()).unwrap_or_else(|e| {
    ...
    // Re-check after acquiring the lock: another caller may have finished compilation.
    if is_cache_fresh(&casm_path, &cache_key) && is_cache_fresh(&sierra_path, &cache_key) {
        return;
    }
