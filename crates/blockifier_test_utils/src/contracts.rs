...
const CHAINLINK_AGGREGATOR_MOCK_BASE: u32 = 25 * CLASS_HASH_BASE;
...
const CHAINLINK_AGGREGATOR_MOCK_NAME: &str = "chainlink_aggregator_mock";
...
pub enum FeatureContract {
    ...
    ChainlinkAggregatorMock(RunnableCairo1),
}
...
impl FeatureContract {
    pub fn cairo_version(&self) -> CairoVersion {
        match self {
            ...
            | Self::ChainlinkAggregatorMock(runnable_version) => {
    ...
    pub fn set_cairo_version(&mut self, version: CairoVersion) {
        match self {
            ...
            | Self::CendeTest(rv)
            | Self::ChainlinkAggregatorMock(rv) => match version {
    ...
    pub fn get_integer_base(self) -> u32 {
        self.get_cairo_version_bit()
            + match self {
                ...
                Self::ChainlinkAggregatorMock(_) => CHAINLINK_AGGREGATOR_MOCK_BASE,
            }
    }
    ...
    pub fn get_non_erc20_base_name(&self) -> &str {
        match self {
            ...
            Self::ChainlinkAggregatorMock(_) => CHAINLINK_AGGREGATOR_MOCK_NAME,
        ...
        match self.cairo_version() {
            CairoVersion::Cairo0 => {
                let extra_arg: Option<String> = match self {
                    ...
                    | FeatureContract::ChainlinkAggregatorMock(_) => None,
    ...
    fn all_contract_versions(&self) -> Vec<FeatureContract> {
        match self {
            ...
            | Self::CendeTest(_)
            | Self::ChainlinkAggregatorMock(_) => {
                #[cfg(not(feature = "cairo_native"))]
                {
                    vec![*self]
                }
