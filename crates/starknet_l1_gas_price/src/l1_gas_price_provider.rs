...
impl L1GasPriceProvider {
    ...
    pub fn get_price_info(
        ...
        // Go over all elements between last_index-num_blocks+1 to last_index (inclusive).
        for (_height, sample) in self.data.iter().skip(last_index + 1 - num_blocks).take(num_blocks)
        {
