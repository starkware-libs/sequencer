...
use serde_json;
use starknet_l1_gas_price_types::errors::EthToFriOracleClientError;
use starknet_l1_gas_price_types::EthToFriOracleClientTrait;
use url::Url;

#[cfg(test)]
#[path = "eth_to_fri_oracle_test.rs"]
pub mod eth_to_fri_oracle_test;
...
pub struct EthToFriOracleConfig {
    pub base_url: Url,
    #[serde(deserialize_with = "deserialize_optional_map")]
    pub headers: Option<HashMap<String, String>>,
}

impl SerializeConfig for EthToFriOracleConfig {
    fn dump(&self) -> BTreeMap<ParamPath, SerializedParam> {
        BTreeMap::from_iter([
            ...
            ser_param(
                ...
                "Headers for the Eth to Fri oracle client, formatted as 'k1:v1 k2:v2 ...'.",
                ParamPrivacyInput::Private,
            ),
        ])
    }
...
impl Default for EthToFriOracleConfig {
    fn default() -> Self {
        Self { base_url: Url::parse("https://example.com/api?timestamp=").unwrap(), headers: None }
    }
}

/// Client for interacting with the Eth to Fri Oracle API.
pub struct EthToFriOracleClient {
    /// The base URL of the Eth to Fri Oracle API.
...
#[async_trait]
impl EthToFriOracleClientTrait for EthToFriOracleClient {
    ...
    async fn eth_to_fri_rate(&self, timestamp: u64) -> Result<u128, EthToFriOracleClientError> {
        ...
        let price = json
            .get("price")
            .and_then(|v| v.as_str())
            .ok_or(EthToFriOracleClientError::MissingFieldError("price"))?;
        // Convert hex to u128
        ...
        let decimals = json
            .get("decimals")
            .and_then(|v| v.as_u64())
            .ok_or(EthToFriOracleClientError::MissingFieldError("decimals"))?;
        if decimals != DECIMALS {
            return Err(EthToFriOracleClientError::InvalidDecimalsError(DECIMALS, decimals));
        }
        Ok(rate)
    }
}
