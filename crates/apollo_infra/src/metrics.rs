...
impl LocalServerMetrics {
    ...
    pub fn record_processing_time(&self, duration_secs: f64, label: &'static str) {
        self.processing_times.record(duration_secs, &[(LABEL_NAME_REQUEST_VARIANT, label)]);
    }

    pub fn record_queueing_time(&self, duration_secs: f64, label: &'static str) {
        self.queueing_times.record(duration_secs, &[(LABEL_NAME_REQUEST_VARIANT, label)]);
    }
}
