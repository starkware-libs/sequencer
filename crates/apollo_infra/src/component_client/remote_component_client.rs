    ...
    Response: Send + Serialize + DeserializeOwned + Debug,
{
    async fn send(&self, component_request: Request) -> ClientResult<Response> {
        ...
        // Convert the serialized request into `Bytes`, a zero-copy low-level type used by Hyper.
        // To create a Hyper request one typically forgoes ownership of the request data, converts
        // it to a `Bytes` object, which is then used to create the request.
        // Ergo, to support retries, we need to maintain copies/clones of the original request data.
        // By using `Bytes`, and cloning as needed, achieve this goal while avoiding unnecessary
        // memory allocations.
