...
pub enum ExchangeRateOracleClientError {
    ...
    #[error(
        "Stale {pair_name} price feed: last updated at {updated_at}, queried for \
         {query_timestamp}, maximum accepted staleness is {max_staleness_seconds} seconds"
    )]
    StaleFeedError {
        pair_name: String,
        updated_at: u64,
    ...
    #[error(
        "The {pair_name} price feed is dated {updated_at}, more than \
         {max_future_updated_at_seconds} seconds ahead of the queried timestamp {query_timestamp}"
    )]
    FutureFeedError {
        pair_name: String,
        updated_at: u64,
