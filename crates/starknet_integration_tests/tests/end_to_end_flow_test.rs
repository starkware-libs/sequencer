...
async fn end_to_end_flow(
    ...
    let mut send_consensus_tx_fn = |tx| async {
        match tx {
            ConsensusTransaction::RpcTransaction(rpc_tx) => {
                sequencer_to_add_txs.assert_add_tx_success(rpc_tx).await
            }
            ConsensusTransaction::L1Handler(l1_handler_tx) => {
                mock_running_system
                    .starknet_l1_contract
                    .send_message_to_l2_and_calc_tx_hash(l1_handler_tx, chain_id.clone())
                    .await
            }
        }
    };
...
fn create_test_blocks() -> Vec<TestBlockScenario> {
    ...
    let test_scenarios: Vec<(
        ...
        // does not work smoothly in the setup of this test.
        ...
        (
            ...
            test_multiple_account_txs,
            ExpectedContentId::from_hex_unchecked(
                "0x34019ceb90d454b3862334a1d62bda7d192f75ada9899cea7227ba3ebd34bcb",
            ),
        ),
        (
            create_funding_txs,
            ...
            test_single_tx,
            ExpectedContentId::from_hex_unchecked(
                "0x4cd35022018786b9c3b4153496173895e885d57008430dbc79aaa9203bcacb9",
            ),
        ),
        (
            ...
            test_two_txs,
            ExpectedContentId::from_hex_unchecked(
                "0x7ae83f980808ab12b5606078f2e9bde59fbc3a5f20159903d94748572dbe15f",
            ),
        ),
        (
            create_declare_tx,
            ...
            test_single_tx,
            ExpectedContentId::from_hex_unchecked(
                "0x1560c8b5bb62264bc15041a487bc1bff142e1804282cf282f9d8ca361dbd2e5",
            ),
        ),
    ];
