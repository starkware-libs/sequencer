...
async fn test_mempool_receives_tx_from_other_peer(tx_generator: MultiAccountTransactionGenerator) {
    ...
    let mut received_transactions: Vec<AccountTransaction> = vec![];
    for _ in 0..20 {
        if received_transactions.len() == expected_txs.len() {
            break;
        }
        received_transactions
            .append(&mut mempool_client.get_txs(expected_txs.len()).await.unwrap());
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    }
    assert_eq!(received_transactions.len(), expected_txs.len());
