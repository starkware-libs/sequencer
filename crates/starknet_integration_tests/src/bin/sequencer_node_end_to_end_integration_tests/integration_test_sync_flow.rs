...
async fn main() {
    ...
    let update_config_basic_fn = |config: &mut SequencerNodeConfig| {
    ...
    let update_config_and_use_central_sync_fn = |config: &mut SequencerNodeConfig| {
        update_config_basic_fn(config);
    ...
    integration_test_manager
        .modify_config_idle_nodes([P2P_SYNC_NODE].into(), update_config_basic_fn);
    integration_test_manager.modify_config_idle_nodes(
        [CENTRAL_SYNC_NODE].into(),
        update_config_and_use_central_sync_fn,
    );

    integration_test_manager.run_nodes(integration_test_manager.get_node_indices()).await;

    integration_test_manager.await_sync_block(BlockNumber(20)).await;
}
