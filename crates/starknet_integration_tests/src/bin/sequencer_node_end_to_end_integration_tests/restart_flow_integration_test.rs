...
async fn main() {
    ...
    let mut integration_test_manager = IntegrationTestManager::new(
        ...
        None,
        None,
        TestIdentifier::EndToEndRestartFlowIntegrationTest,
    )
    .await;

    let mut node_indices = integration_test_manager.get_node_indices();

    // node 0 is used for sending txs and metrics
    ...
    info!("Network reached block {BLOCK_TO_SHUTDOWN_AT}. Shutting down node {first_node_index}");
    integration_test_manager.shutdown_nodes(first_node_hashset.clone());

    info! {"Sending transactions while node {first_node_index} is down"}
    ...
    info!("Network reached block {BLOCK_TO_RESTART_FROM}. Shutting down node {second_node_index}");
    ...
    info!(
        "Sending transactions while node {second_node_index} is down and after {first_node_index} \
         was restarted"
    );
