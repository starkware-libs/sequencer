...
impl SequencerSimulator {
    pub fn new(config_file: String, http_server_url: String) -> Self {
        ...
        let monitoring_client =
            MonitoringClient::new(get_socket_addr(&http_server_url, port).unwrap());

        let HttpServerConfig { ip: _, port } = config.http_server_config;
        let http_client = HttpTestClient::new(get_socket_addr(&http_server_url, port).unwrap());
