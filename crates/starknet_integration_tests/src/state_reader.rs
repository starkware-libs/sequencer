...
impl StorageTestSetup {
    pub fn new(
        ...
        let classes =
            TestClasses::new(test_defined_accounts.clone(), preset_test_contracts.clone());
...
impl TestClasses {
    pub fn new(
        ...
        let contract_classes_to_retrieve = test_defined_accounts
            .into_iter()
            .map(|acc| acc.account)
            .chain(preset_test_contracts.default_test_contracts)
            .chain([preset_test_contracts.erc20_contract]);
        ...
        Self { sierra_vec, cairo0_contract_classes, cairo1_contract_classes }
    }
}
...
fn prepare_state_diff(
    ...
    state_diff_builder
        .set_contracts(&preset_test_contracts.default_test_contracts)
        .declare()
        .deploy();

    // Declare and deploy and the ERC20 contract, so that transfers from it can be made.
    state_diff_builder
        .set_contracts(std::slice::from_ref(&preset_test_contracts.erc20_contract))
        .declare()
        .deploy();
...
fn write_state_to_papyrus_storage(
    ...
    let TestClasses { sierra_vec: cairo1_sierra, cairo0_contract_classes, cairo1_contract_classes } =
        classes;
