...
// Arbitrary value. Ascii for "paid".
static PAID_FEE_ON_L1: LazyLock<U256> = LazyLock::new(|| "0x70616964".to_string().parse().unwrap());
...
pub fn create_node_config(
    ...
    let base_layer_config = EthereumBaseLayerConfig {
        node_url: base_layer_endpoint_url.clone(),
...
/// Creates an L1 handler transaction. The message is to the L2 contract
/// [TestContract](FeatureContract::TestContract), to the entry point `l1_handler_set_value`. Used
/// for flow test.
...
/// Sends the given transactions to the L1 handle at at the given URL. This function builds the
/// interface from which to send the messages to L2, converts the
/// [L1 handler transactions](L1HandlerTransaction) to match the expected interface, and sends the
/// messages.
pub async fn send_messages_to_l1(node_url: Url, l1_handler_txs: &[L1HandlerTransaction]) {
    let ethereum_base_layer_contract = EthereumBaseLayerContract::new(EthereumBaseLayerConfig {
        node_url,
        starknet_contract_address: DEFAULT_ANVIL_L1_DEPLOYED_ADDRESS.parse().unwrap(),
    });
    ...
    for l1_handler in l1_handler_txs {
        let _tx_receipt = send_message_to_l1(l1_handler, &starknet_l1_contract).await;
    }
}

/// Converts a given [L1 handler transaction](L1HandlerTransaction) to match the interface of the
/// given [starknet l1 contract](StarknetL1Contract), and sends the messages to L2.
async fn send_message_to_l1(
    ...
    let calldata = l1_handler
        ...
        .collect();
    let msg = starknet_l1_contract.sendMessageToL2(l2_contract_address, l2_entry_point, calldata);
