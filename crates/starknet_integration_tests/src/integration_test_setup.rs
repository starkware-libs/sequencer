...
impl ExecutableSetup {
    ...
    pub fn change_config<F>(&mut self, change_config_fn: F)
    where
        F: Fn(&mut SequencerNodeConfig),
    {
        change_config_fn(&mut self.config);
        self.dump_config_file_changes();
    }

    pub fn change_config_pointers<F>(&mut self, change_config_pointers_fn: F)
    where
        F: Fn(&mut ConfigPointersMap),
    {
        change_config_pointers_fn(&mut self.config_pointers_map);
        self.dump_config_file_changes();
    }
