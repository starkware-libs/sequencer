...
pub const HTTP_PORT_VAR: &str = "http-port";
pub const MONITORING_PORT_VAR: &str = "monitoring-port";
...
impl NodeSetup {
    ...
    pub fn generate_simulator_ports_json(&self, path: &str) {
        let json_data = serde_json::json!({
            HTTP_PORT_VAR: self.executables[self.http_server_index].config.http_server_config.port,
            MONITORING_PORT_VAR: self.executables[self.batcher_index].config.monitoring_endpoint_config.port
        });
