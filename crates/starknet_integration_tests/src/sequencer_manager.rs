...
impl IntegrationTestManager {
    ...
    pub async fn run(nodes: Vec<NodeSetup>) -> Self {
        ...
        let running_nodes = nodes
            .into_iter()
            .filter_map(|node| {
                let node_index = node.get_node_index()?;
                Some((node_index, node.run()))
            })
            .collect::<HashMap<usize, RunningNode>>();

        let integration_test_manager = Self { idle_nodes: HashMap::new(), running_nodes };
