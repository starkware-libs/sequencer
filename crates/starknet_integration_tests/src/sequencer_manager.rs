...
impl IntegrationTestManager {
    ...
    pub async fn await_execution(&self, expected_block_number: BlockNumber) {
        ...
        await_block(5000, expected_block_number, 50, self.batcher_monitoring_client())
            .await
...
/// Sample the metrics until sufficiently many blocks have been reported. Returns an error if
/// after the given number of attempts the target block number has not been reached.
async fn await_block(
    ...
    let get_latest_block_number_closure = || get_latest_block_number(batcher_monitoring_client);
