...
impl IntegrationTestManager {
    pub async fn run(nodes: Vec<NodeManager>) -> Self {
        info!("Running sequencers.");
        let executable_run_handles = nodes
            .iter()
            .flat_map(|node| {
        ...
        let sequencer_manager = Self { nodes, executable_run_handles };
