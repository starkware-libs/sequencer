...
pub struct IntegrationTestManager {
    pub shutdown_nodes: Vec<NodeSetup>,
...
impl IntegrationTestManager {
    pub async fn run(nodes: Vec<NodeSetup>) -> Self {
        ...
        Self::await_alive(&running_nodes, 5000, 50).await;

        Self { shutdown_nodes: Vec::new(), running_nodes }
    }

    async fn await_alive(running_nodes: &[RunningNode], interval: u64, max_attempts: usize) {
        let await_alive_tasks =
            running_nodes.iter().map(|node| node.await_alive(interval, max_attempts));
    ...
    pub fn find_shotdown_node_by_index(&self, node_index: usize) -> Option<&NodeSetup> {
        self.shutdown_nodes.iter().find(|node_setup| {
            node_setup
                .executables
                .iter()
                .any(|executable| executable.node_execution_id.get_node_index() == node_index)
    ...
    pub fn find_node_setup_by_index(&self, node_index: usize) -> Option<&NodeSetup> {
        self.find_shotdown_node_by_index(node_index)
    ...
    pub fn shutdown_nodes(mut self) -> Self {
        self.running_nodes.drain(..).for_each(|running_node| {
            ...
            self.shutdown_nodes.push(running_node.node_setup);
        });
        self
    }
