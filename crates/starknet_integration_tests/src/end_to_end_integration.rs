...
use crate::sequencer_manager::{get_sequencer_configs, verify_results, SequencerSetupManager};
...
pub async fn end_to_end_integration(tx_generator: MultiAccountTransactionGenerator) {
    ...
    // Run the sequencers.
    let sequencer_manager = SequencerSetupManager::run(sequencers).await;
    ...
    sequencer_manager.await_execution(EXPECTED_BLOCK_NUMBER, &batcher_storage_reader).await;
