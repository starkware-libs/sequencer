...
const N_LOCAL_SEQUENCERS: usize = 3;
/// The number of "full" remote sequencers that participate in the test.
const N_REMOTE_SEQUENCERS: usize = 2;
...
pub async fn end_to_end_integration(tx_generator: MultiAccountTransactionGenerator) {
    ...
    let component_configs: Vec<ComponentConfig> =
        vec![ComponentConfig::default(); N_LOCAL_SEQUENCERS]
            .into_iter()
            .chain(create_remote_node_configs(&mut available_ports, N_REMOTE_SEQUENCERS))
            .collect();
...
fn create_remote_node_configs(
    ...
    (1..n)
        ...
        .flat_map(|(http_config, non_http_config)| vec![http_config, non_http_config])
        .collect()
}
