...
use crate::sequencer_manager::{get_sequencer_configs, SequencerManager};
...
pub async fn end_to_end_integration(tx_generator: MultiAccountTransactionGenerator) {
    ...
    let simulator_config = sequencer_manager
        .run_integration_test_simulator(tx_generator, EXPECTED_BLOCK_NUMBER, N_TXS)
        .await;
