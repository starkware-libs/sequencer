...
///
/// The gateway otherwise validates against the committed block's prices. Under replay that block
/// trails the transaction's own, so in a falling market a transaction the source chain accepted is
/// rejected for having less headroom than the price fell in between — and it can never clear the
/// bound, because the prices only drop far enough once its own block is already built.
pub struct ReplayGasPricesClient {
    recorder_url: Url,
    client: ClientWithMiddleware,
    // A block's prices are immutable and consecutive replayed transactions share a block, so one
    // slot removes nearly all of the block lookups.
...
impl ReplayGasPricesClient {
    ...
    /// `None` leaves the caller on the committed block's prices.
