...
async fn test_get_nonce_fail_on_extract_state_nonce_and_run_validations() {
    ...
    let result = tokio::task::spawn_blocking(move || {
        stateful_validator
            .extract_state_nonce_and_run_validations(&executable_tx, nonce!(0), is_account_tx_in_mempool)
    })
    .await
    .unwrap()
    .unwrap_err();
...
async fn test_extract_state_nonce_and_run_validations(
    ...
    let is_account_tx_in_mempool = Ok(false);
    let result: Result<Nonce, StarknetError> = tokio::task::spawn_blocking(move || {
