...
/// Mainnet block 11926844: tx 0x1549342b caps L2 at 32000000000, its own block's price is
/// 31507218957, and the committed block 11926843's price is 32093567243. Validating against the
/// committed block rejects it forever, since the price only falls far enough once its own block is
/// built; validating against its source block admits it.
#[rstest]
...
/// An implausible block price must reject rather than wrap the threshold multiplication, which
/// release builds would otherwise do silently.
#[rstest]
#[tokio::test]
...
/// The whole STRK vector is replaced, so blockifier's own fee bounds -- which check all three
/// resources off the same block info -- see the source block's prices too.
#[rstest]
#[tokio::test]
