...
impl FetchCompiledClasses for SyncStateReader {
    ...
    fn is_declared(&self, class_hash: ClassHash) -> StateResult<bool> {
        // First check if the class is declared at all
        let is_class_declared = self
            .runtime
            .block_on(self.state_sync_client.is_class_declared_at(self.block_number, class_hash))
            .map_err(|e| StateError::StateReadError(e.to_string()))?;
        ...
        // If declared, check if it's a V1 (Cairo 1) class
        let compiled_class = match self.read_executable(class_hash)? {
            Some(compiled_class) => compiled_class,
            None => return Ok(false),
        };

        match compiled_class {
            ContractClass::V1(_) => Ok(true),
            ContractClass::V0(_) => Ok(false),
        }
    }
}
