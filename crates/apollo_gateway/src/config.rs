...
where
    D: Deserializer<'de>,
{
    let maybe_raw: Option<String> = Option::deserialize(de)?;
    ...
    for s in raw.split(',') {
        let felt = Felt::from_str(s.trim())
            .map_err(|e| de::Error::custom(format!("Failed to parse Felt from '{}': {}", s, e)))?;

        let addr = ContractAddress::try_from(felt)
            .map_err(|e| de::Error::custom(format!("Invalid contract address '{}': {}", s, e)))?;
