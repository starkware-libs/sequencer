...
async fn test_returns_latest_block_fixed_reader_when_blocks_exist() {
    ...
    let block_info = fixed_block_reader.get_block_info().await.unwrap();
    assert_eq!(block_info.block_number, header.block_number);
    assert_eq!(block_info.block_timestamp, header.timestamp);
    assert_eq!(block_info.sequencer_address, header.sequencer.0);
    assert!(block_info.use_kzg_da);

    let expected_wei_l1: NonzeroGasPrice = header.l1_gas_price.price_in_wei.try_into().unwrap();
    let expected_wei_l1_data: NonzeroGasPrice =
        header.l1_data_gas_price.price_in_wei.try_into().unwrap();
    let expected_wei_l2: NonzeroGasPrice = header.l2_gas_price.price_in_wei.try_into().unwrap();
    let expected_fri_l1: NonzeroGasPrice = header.l1_gas_price.price_in_fri.try_into().unwrap();
    let expected_fri_l1_data: NonzeroGasPrice =
        header.l1_data_gas_price.price_in_fri.try_into().unwrap();
    let expected_fri_l2: NonzeroGasPrice = header.l2_gas_price.price_in_fri.try_into().unwrap();
