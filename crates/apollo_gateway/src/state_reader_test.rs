...
async fn test_get_storage_at() {
    ...
    let wrapped_state_reader = state_reader_and_contract_manager(
        Arc::new(mock_state_sync_client),
        Arc::new(mock_class_manager_client),
        contract_class_manager.clone(),
        block_number,
...
async fn test_get_compiled_class(
    #[case] class_manager_client_get_executable_result: Option<
    ...
    if let Some(class_manager_client_get_executable_result) =
        class_manager_client_get_executable_result
    {
        mock_class_manager_client
            .expect_get_executable()
            .times(1)
            .with(predicate::eq(class_hash))
            .return_once(move |_| class_manager_client_get_executable_result);
    }
