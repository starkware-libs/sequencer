// TODO(Arni): Consider renaming this file to state_reader_and_contract_class_wrapper.
use std::sync::Arc;
...
async fn test_get_block_info() {
    ...
    let boxed_state_sync_reader = Box::new(state_sync_reader);

    let wrapped_state_reader = StateReaderAndContractManagerWrapper::<
        Box<dyn GatewayStateReaderWithCompiledClasses>,
    >(StateReaderAndContractManager {
        state_reader: boxed_state_sync_reader,
        contract_class_manager: contract_class_manager.clone(),
    });
    let result = wrapped_state_reader.get_block_info().unwrap();
...
async fn test_get_storage_at() {
    ...
    let result = tokio::task::spawn_blocking(move || {
        wrapped_state_reader.get_storage_at(contract_address, storage_key)
    })
    .await
    .unwrap()
    .unwrap();
...
async fn test_get_nonce_at() {
    ...
    let result =
        tokio::task::spawn_blocking(move || wrapped_state_reader.get_nonce_at(contract_address))
            .await
            .unwrap()
            .unwrap();
...
async fn test_get_class_hash_at() {
    ...
    let result = tokio::task::spawn_blocking(move || {
        wrapped_state_reader.get_class_hash_at(contract_address)
    })
    .await
    .unwrap()
    .unwrap();
...
#[case::class_declared(
    ...
    Some(Ok(Some(SierraContractClass::default()))),
    Ok(true),
...
async fn test_get_compiled_class(
    #[case] class_manager_client_get_executable_result: ClassManagerClientResult<
        Option<ExecutableClass>,
    >,
    #[case] class_manager_client_get_sierra_result: Option<
    ...
    mock_class_manager_client
        ...
        .returning(move |_| class_manager_client_get_executable_result.clone());

    // Add expectation for get_sierra when the contract class is V1
    if let Some(result) = class_manager_client_get_sierra_result {
        mock_class_manager_client
            .expect_get_sierra()
            .with(predicate::eq(class_hash))
    ...
    let result =
        tokio::task::spawn_blocking(move || wrapped_state_reader.get_compiled_class(class_hash))
            .await
            .unwrap();
