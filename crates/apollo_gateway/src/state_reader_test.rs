...
fn cached_but_verification_failed_scenario() -> GetCompiledClassTestScenario {
    GetCompiledClassTestScenario {
        expectations: GetCompiledClassTestExpectation {
            class_hash: *DUMMY_CLASS_HASH,
            get_executable_result: Ok(None), // Not called due to caching
...
#[case::cairo_1_declared_then_verification_failed(
    cairo_1_declared_scenario(),
    cached_but_verification_failed_scenario()
)]
...
async fn test_get_compiled_class_caching_scenarios(
    ...
    let first_result = tokio::task::spawn_blocking({
        ...
        move || state_reader.get_compiled_class(*DUMMY_CLASS_HASH)
    })
    .await
    .unwrap();
