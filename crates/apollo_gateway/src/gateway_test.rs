...
fn mock_dependencies() -> MockDependencies {
    ...
    MockDependencies {
        ...
        mock_config_manager_client: default_config_manager_client(),
    }
}
...
fn default_config_manager_client() -> MockConfigManagerClient {
    let mut mock = MockConfigManagerClient::new();
    mock.expect_get_gateway_dynamic_config().returning(|| Ok(GatewayDynamicConfig::default()));
    mock
}
...
async fn add_tx_returns_error_when_extract_state_nonce_and_run_validations_fails(
    ...
    > {
        ...
        config_manager_client: Arc::new(default_config_manager_client()),
    };
