...
impl MockDependencies {
    ...
    fn setup_class_manager_client_mock(
        ...
        let contract_class_hash = executable.compiled_class_hash();
...
async fn convert_rpc_tx_to_internal(
    ...
    if matches!(&rpc_tx, RpcTransaction::Declare(_)) {
        MockDependencies::setup_class_manager_client_mock(&mut class_manager_client);
    }
...
async fn test_add_tx(
    ...
    let mut rpc_tx = create_tx(&tx_type);

    if let RpcTransaction::Declare(RpcDeclareTransaction::V3(tx)) = &mut rpc_tx {
        let compiled_class_hash = MockDependencies::setup_class_manager_client_mock(
            &mut mock_dependencies.mock_class_manager_client,
        );
        tx.compiled_class_hash = compiled_class_hash;
    }
