...
async fn test_unauthorized_declare_config(
    ...
    let RpcTransaction::Declare(RpcDeclareTransaction::V3(ref declare_tx_inner)) = &rpc_declare_tx
    else {
        panic!("Expected Declare V3 transaction");
    };
    let sender_address = declare_tx_inner.sender_address;
    assert_ne!(sender_address, authorized_address, "Sender address should not be authorized");
...
#[case::two_addresses(
    "0xabcd,0x1234",
    Some(vec![
        ContractAddress(patricia_key!("0xabcd")),
...
fn test_deserialize_authorized_declarer_accounts(
    #[case] input_addrs: &str,
    #[case] expected_accounts: Option<Vec<ContractAddress>>,
) {
    // Json represent a GatewayConfig with default values.
    let json = format!(
        r#"{{
            ...
            "chain_info": {{
                "chain_id": "0x0",
                "fee_token_addresses": {{
                    "eth_fee_token_address": "0x0",
                    "strk_fee_token_address": "0x0"
                }}
            }},
            "block_declare": false,
    ...
    assert_eq!(config.authorized_declarer_accounts, expected_accounts);

    let expected =
        GatewayConfig { authorized_declarer_accounts: expected_accounts, ..Default::default() };
