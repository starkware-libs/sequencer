...
impl ComponentRequestHandler<GatewayRequest, GatewayResponse> for Gateway {
    async fn handle_request(&mut self, request: GatewayRequest) -> GatewayResponse {
        match request {
            ...
            GatewayRequest::UpdateTimestamps(mappings) => GatewayResponse::UpdateTimestamps(
                self.update_timestamps(mappings).await.map_err(|source| {
                    GatewayError::DeprecatedGatewayError { source, p2p_message_metadata: None }
                }),
            ),
        }
    }
