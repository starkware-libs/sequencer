...
// FixedBlockSyncStateReader is disconnected from the state sync client. RPC state reader acts as a
// fixed block sync state reader because it can be used to get the block info for a given block
// number. But it does not use the state sync client to get the block info.
#[async_trait]
impl FixedBlockSyncStateReader for RpcStateReader {
