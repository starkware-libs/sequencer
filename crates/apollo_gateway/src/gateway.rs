...
> GenericGateway<TStatelessValidator, TTransactionConverter>
{
    ...
    async fn convert_rpc_tx_to_internal_and_executable_txs(
        ...
        StarknetError,
    > {
        let (internal_tx_for_exec, verification_handle) =
        ...
        if let Some(handle) = verification_handle {
            ...
            if let Some(task) = task {
                let verification_result = task.await.map_err(|e| {
                    let err_msg = format!("Proof verification task join error: {}", e);
                    warn!("{}", err_msg);
                ...
                verification_result.map_err(|e| {
                    ...
                    StarknetError::internal_with_logging("Proof verification failed", &e)
                })?;
            }
        }
        ...
        let executable_tx = self
            .transaction_converter
            .convert_internal_rpc_tx_to_executable_tx(internal_tx_for_exec.clone())
            .await
            .map_err(|e| {
        ...
        Ok((internal_tx_for_exec, executable_tx, proof_data))
    }
}
