...
impl ProcessTxBlockingTask {
    ...
    fn process_tx(self) -> GatewayResult<AddTransactionArgs> {
        ...
        if self.stateless_tx_validator.config.validate_non_zero_resource_bounds {
            // Skip this validation for the bootstrap transactions.
            let l2_gas_price =
                validator.block_context().block_info().gas_prices.strk_gas_prices.l2_gas_price;
            validate_l2_gas_price_within_threshold(executable_tx.resource_bounds(), l2_gas_price)?;
        }
...
fn validate_l2_gas_price_within_threshold(
    resource_bounds: ValidResourceBounds,
    l2_gas_price: NonzeroGasPrice,
) -> GatewayResult<()> {
    // This validation is relevant for version onwards 0.14.0. AllResources is the only valid
    // resource bounds variant. If this is not the case, skip this validation.is `AllResources`
    if let ValidResourceBounds::AllResources(resource_bounds) = resource_bounds {
        let txs_l2_gas_price = resource_bounds.l2_gas.max_price_per_unit;
        let gas_price_threshold_moltiplier = Ratio::new(MIN_GAS_PRICE_PRECENTAGE.into(), 100_u128);
        let threshold = (gas_price_threshold_moltiplier * l2_gas_price.get().0).to_integer();
        if txs_l2_gas_price.0 < threshold {
            return Err(StarknetError {
                code: StarknetErrorCode::KnownErrorCode(KnownStarknetErrorCode::GasPriceTooLow),
                message: format!(
                    "Transaction gas price {} is below the threshold {}.",
                    txs_l2_gas_price, threshold
                ),
            });
        }
    };
