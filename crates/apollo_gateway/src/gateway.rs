...
impl Gateway {
    ...
    async fn add_tx_inner(
        ...
        // let account_nonce =
        //     stateful_transaction_validator.get_nonce(executable_tx.contract_address()).await?;

        // Run the blocking validations on the current runtime worker.
        let curr_span = Span::current();
        let executable_tx_cloned = executable_tx.clone();
        let mempool_client = self.mempool_client.clone();
        let runtime = tokio::runtime::Handle::current();
        let nonce = tokio::task::spawn_blocking(move || {
            curr_span.in_scope(|| {
                stateful_transaction_validator.run_transaction_validations(
                    &executable_tx_cloned,
                    mempool_client,
                    runtime,
                )
            })
