...
> GenericGateway<TStatelessValidator, TTransactionConverter, TStatefulValidatorFactory>
{
    ...
    async fn add_tx_inner(
        ...
        let proof_data = match tx {
            RpcTransaction::Invoke(RpcInvokeTransaction::V3(ref tx)) => {
                if !tx.proof_facts.is_empty() {
                    Some((tx.proof_facts.clone(), tx.nonce, tx.proof.clone()))
                } else {
                    None
                }
            }
        ...
        if let Some((proof_facts, tx_nonce, proof)) = proof_data {
            ...
            if let Err(e) =
                proof_manager_client.set_proof(proof_facts.clone(), tx_nonce, proof.clone()).await
            {
