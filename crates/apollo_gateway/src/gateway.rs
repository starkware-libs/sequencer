...
> GenericGateway<TStatelessValidator, TTransactionConverter, TStatefulValidatorFactory>
{
    ...
    async fn add_tx_inner(
        ...
        // Converting a declare transaction triggers Sierra-to-CASM compilation, which is CPU- and
        // memory-intensive and runs before the transaction's signature and balance are verified.
        // Bound the number of concurrent compilations so a flood of declares carrying arbitrary
        // contract classes cannot exhaust the node's resources; reject excess declares immediately
        // rather than queueing them. The permit is held only across the conversion (compilation)
        // and released before the cheaper stateful validation.
        let compilation_permit = if matches!(tx, RpcTransaction::Declare(_)) {
            Some(self.declare_compilation_semaphore.try_acquire().map_err(|_| {
                let error = StarknetError {
                    ...
                    message: "Too many declare transactions are being compiled concurrently. \
                              Please retry later."
                        .to_string(),
                };
