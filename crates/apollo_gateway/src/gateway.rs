...
> GenericGateway<TStatelessValidator, TTransactionConverter, TStatefulValidatorFactory>
{
    ...
    async fn add_tx_inner(
        ...
        // Declare conversion triggers CPU- and memory-intensive Sierra-to-CASM compilation before
        // the tx's signature and balance are verified. Bound concurrent compilations so a declare
        // flood cannot exhaust resources; reject excess declares immediately rather than queueing.
        // The permit is held only across compilation and released before stateful validation.
