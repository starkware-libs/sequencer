...
> GenericGateway<TStatelessValidator, TTransactionConverter>
{
    ...
    async fn store_proof_and_spawn_archiving(
        &self,
        proof_data: Option<(ProofFacts, Nonce, ContractAddress, Proof)>,
        tx_hash: TransactionHash,
    ) -> ProofArchiveHandle {
        let (proof_facts, nonce, sender_address, proof) = proof_data?;
        ...
        let store_result = self
            .transaction_converter
            .store_proof_in_proof_manager(proof_facts.clone(), proof.clone(), nonce, sender_address)
            .await;
        match store_result {
