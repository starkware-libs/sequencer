...
/// `base_path` exercises a recorder URL configured with a path prefix.
...
/// The transaction's own block prices are returned, not the committed block's.
#[rstest]
...
/// Consecutive transactions share a source block, so the block lookup happens once.
#[rstest]
#[tokio::test]
...
/// Whatever the recorder does, the caller must fall back rather than fail the transaction.
#[rstest]
