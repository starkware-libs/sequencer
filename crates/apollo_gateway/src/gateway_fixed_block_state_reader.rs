...
impl GatewayFixedBlockSyncStateClient {
    ...
    async fn get_block_info_from_sync_client(&self) -> Result<BlockInfo, StarknetError> {
        ...
        let block_info = BlockInfo {
            ...
            gas_prices: GasPrices {
                eth_gas_prices: GasPriceVector {
                    l1_gas_price: block_header.l1_gas_price.price_in_wei.try_into().map_err(
                        |e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L1 gas price (ETH, wei) to non-zero",
                                e,
                            )
                        },
                    )?,
                    l1_data_gas_price: block_header
                        ...
                        .map_err(|e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L1 data gas price (ETH, wei) to non-zero",
                                e,
                            )
                        })?,
                    l2_gas_price: block_header.l2_gas_price.price_in_wei.try_into().map_err(
                        |e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L2 gas price (ETH, wei) to non-zero",
                                e,
                            )
                        },
                    )?,
                ...
                strk_gas_prices: GasPriceVector {
                    l1_gas_price: block_header.l1_gas_price.price_in_fri.try_into().map_err(
                        |e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L1 gas price (STRK, fri) to non-zero",
                                e,
                            )
                        },
                    )?,
                    l1_data_gas_price: block_header
                        ...
                        .map_err(|e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L1 data gas price (STRK, fri) to non-zero",
                                e,
                            )
                        })?,
                    l2_gas_price: block_header.l2_gas_price.price_in_fri.try_into().map_err(
                        |e| {
                            StarknetError::internal_with_logging(
                                "Failed converting L2 gas price (STRK, fri) to non-zero",
                                e,
                            )
                        },
                    )?,
