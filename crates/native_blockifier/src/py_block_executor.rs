...
pub(crate) type ClassHashesToMigrate = Vec<PyFelt>;
...
impl PyBlockExecutor {
    ...
    pub fn finalize(&mut self) -> NativeBlockifierResult<FinalizeResult> {
        ...
        let py_class_hashes_to_migrate: Vec<PyFelt> =
            class_hashes_to_migrate.into_iter().map(|class_hash| PyFelt(class_hash.0)).collect();
