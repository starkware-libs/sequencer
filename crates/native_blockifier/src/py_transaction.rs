    ...
    pub fn try_from(
        ...
        let class_info = match tx {
            ...
            | starknet_api::transaction::DeclareTransaction::V3(_) => {
                ...
                let sierra_program_length = NonZeroUsize::new(py_class_info.sierra_program_length)
                    .ok_or(ContractClassError::ContractClassVersionSierraProgramLengthMismatch {
                        contract_class_version: 1,
                        sierra_program_length: 0,
                    })?;
                ClassInfo::new_v1(contract_class, sierra_program_length, py_class_info.abi_length)
            }
        };
        Ok(class_info)
    }
