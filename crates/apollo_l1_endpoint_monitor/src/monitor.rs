...
impl L1EndpointMonitor {
    ...
    pub async fn get_active_l1_endpoint(&mut self) -> L1EndpointMonitorResult<Sensitive<Url>> {
        ...
        for offset in 1..n_urls {
            ...
            if self.is_operational(idx).await {
                ...
                warn!(
                    ...
                    to_safe_string(self.get_node_url(current_l1_endpoint_index).as_ref()), /* Turns sensitive URL into safe string */
                    to_safe_string(self.get_node_url(idx).as_ref()) /* Turns sensitive URL into
                                                                     * safe string */
                );
        ...
        error!(
            ...
            self.config
                ...
                .as_ref()
                .iter()
                .map(to_safe_string) // Turns sensitive URL into safe string
                .collect::<Vec<_>>()
        );
    ...
    fn get_node_url(&self, index: usize) -> Sensitive<Url> {
        self.config.ordered_l1_endpoint_urls.as_ref()[index].clone().into()
    }
    ...
    async fn is_operational(&self, l1_endpoint_index: usize) -> bool {
        ...
        let l1_endpoint_url = to_safe_string(l1_endpoint_url.as_ref()); // Turns sensitive URL into safe string
