...
pub trait BatcherClient: Send + Sync {
    ...
    /// Gets the stored block hash for a given block number (even if it is not part of the chain).
    async fn get_stored_block_hash(
        &self,
        block_number: BlockNumber,
    ) -> BatcherClientResult<BlockHash>;
...
pub enum BatcherResponse {
    ...
    GetStoredBlockHash(BatcherResult<BlockHash>),
    ...
    ComponentClientType: Send + Sync + ComponentClient<BatcherRequest, BatcherResponse>,
{
    ...
    async fn get_stored_block_hash(
        ...
        let request = BatcherRequest::GetStoredBlockHash(block_number);
        handle_all_response_variants!(
            BatcherResponse,
            GetStoredBlockHash,
            BatcherClientError,
