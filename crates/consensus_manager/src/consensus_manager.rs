...
fn create_fake_network_channels() -> BroadcastConsensusMessageChannel {
    ...
    let reported_messages_sender_fn: fn(
        ...
        |_| todo!("messages_to_broadcast_sender should not be used");
    let broadcast_topic_client = BroadcastTopicClient {
        messages_to_broadcast_sender: mpsc::channel(0).0.with(messages_to_broadcast_fn),
        reported_messages_sender: mpsc::channel(0).0.with(reported_messages_sender_fn),
        continue_propagation_sender: mpsc::channel(0).0,
    };
