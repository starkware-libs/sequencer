...
impl FsClassStorageBuilderForTesting {
    ...
    pub fn build(self) -> (FsClassStorage, Option<(TempDir, TempDir)>) {
        let class_hash_storage =
            ClassHashStorage::new(self.config.class_hash_storage_config).unwrap();
        let fs_class_storage =
            FsClassStorage { persistent_root: self.config.persistent_root, class_hash_storage };
        (fs_class_storage, self.handles)
    }
}
