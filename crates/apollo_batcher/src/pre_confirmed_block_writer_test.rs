...
/// Closing both channels immediately triggers a single write with an empty transaction list
...
async fn write_candidate_transactions() {
    ...
    tokio::time::sleep(Duration::from_millis(WRITE_INTERVAL_MS * 5)).await;
...
async fn candidate_does_not_overwrite_pre_confirmed() {
    ...
    tokio::time::sleep(Duration::from_millis(WRITE_INTERVAL_MS * 3)).await;
