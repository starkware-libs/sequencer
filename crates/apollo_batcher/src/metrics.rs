...
define_metrics!(
    Batcher => {
        ...
        MetricHistogram { STATE_DIFF_COMMITMENT_PER_STATE_DIFF_LENGTH, "batcher_state_diff_commitment_per_state_diff_length", "Duration of state diff commitment computation per state diff length in seconds" },
    },
);
...
pub fn register_metrics(storage_height: BlockNumber, global_root_height: BlockNumber) {
    ...
    STATE_DIFF_COMMITMENT_PER_STATE_DIFF_LENGTH.register();
