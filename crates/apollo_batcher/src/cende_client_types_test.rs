...
const TRANSACTION_RECEIPT_FILE_NAME: &str = "transaction_receipt.json";
...
#[test]
fn serialize_starknet_client_transaction_receipt() {
    let receipt = StarknetClientTransactionReceipt {
    ...
    let dumped_receipt = serde_json::to_value(&receipt)
        .expect("Failed to serialize StarknetClientTransactionReceipt");

    let expected_receipt = read_json_file(TRANSACTION_RECEIPT_FILE_NAME);

    assert_json_eq(
        &dumped_receipt,
        &expected_receipt,
        "The serialized StarknetClientTransactionReceipt does not match the expected JSON."
            .to_string(),
    );
}
