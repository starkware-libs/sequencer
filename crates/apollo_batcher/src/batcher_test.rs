...
async fn create_batcher_impl<R: BatcherStorageReader + 'static>(
    ...
    let mempool_client = if config.static_config.validation_only {
        None
    } else {
        Some(Arc::new(clients.mempool_client) as SharedMempoolClient)
    };
...
async fn validation_only_flag_true_with_mempool_panics() {
    let mempool = Some(Arc::new(MockMempoolClient::new()) as SharedMempoolClient);
