...
impl Default for MockDependencies {
    fn default() -> Self {
        ...
        storage_reader.expect_committer_height().returning(|| Ok(INITIAL_HEIGHT));
...
async fn create_batcher_impl(
    ...
    let block_hash_height =
        storage_reader.committer_height().expect("Failed to get block hash height from storage.");
...
async fn add_sync_block_missing_block_header_commitments_for_new_block() {
    ...
    storage_reader.expect_committer_height().returning(move || Ok(block_number));
    let mock_dependencies = MockDependencies { storage_reader, ..Default::default() };
...
async fn revert_block_empty_storage() {
    ...
    storage_reader.expect_committer_height().returning(|| Ok(BlockNumber(0)));
    let mut mock_dependencies = MockDependencies { storage_reader, ..Default::default() };
    mock_dependencies.batcher_config.revert_config.should_revert = true;
    let mut batcher = create_batcher(mock_dependencies).await;
