...
/// steps, so between them they exercise both of the view call resource bounds. A step-tracked
/// contract is not a hypothetical: a proxy whose implementation class an external key can `upgrade`
/// can be pointed at a Cairo 0 class.
...
/// Recursion depths that exhaust a view call resource bound, while still fitting in the budget a
/// view call had before those bounds were introduced (`HIGH_GAS_AMOUNT` = 10^10 Sierra gas and the
/// block's `invoke_tx_max_n_steps` = 10^7 steps). A revert of the bounds therefore fails the tests
/// using them, and each depth proves the new bound rather than the pre-existing one.
///
/// Calibrated on 2026-08-11: one level of `recurse` costs about 973 Sierra gas in the Cairo 1
/// contract and about 4 Cairo steps in the Cairo 0 one. To recalibrate, keep `depth *
/// cost_per_level` inside (10^8, 10^10) for the Sierra gas depth and inside (10^6, 10^7) for the
/// Cairo steps depth.
...
/// The rejection reason is the only thing the caller of a view call sees, so it has to name both
/// the length that was returned and the limit that rejected it.
#[rstest]
