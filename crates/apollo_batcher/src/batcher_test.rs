...
async fn create_batcher_with_real_storage(
    ...
    let mut batcher = Batcher::new(
        ...
        Arc::new(mock_dependencies.clients.l1_provider_client),
        Arc::new(mock_dependencies.clients.mempool_client),
        TransactionConverter::new(
            mock_dependencies.clients.class_manager_client,
            CHAIN_ID_FOR_TESTS.clone(),
        ),
        Box::new(mock_dependencies.clients.block_builder_factory),
        Box::new(mock_dependencies.clients.pre_confirmed_block_writer_factory),
        None,
    );
    // Call post-creation functionality (e.g., metrics registration).
    batcher.start().await;
