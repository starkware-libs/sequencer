...
// ) -> DecisionReachedResponse { let mut batcher = create_batcher(mock_dependencies).await;
//   batcher.start_height(StartHeightInput { height: INITIAL_HEIGHT }).await.unwrap();
//   batcher.propose_block(propose_block_input(PROPOSAL_ID)).await.unwrap();
//   batcher.await_active_proposal().await; batcher.decision_reached(DecisionReachedInput {
//   proposal_id: PROPOSAL_ID }).await.unwrap()
// }

async fn batcher_propose_and_get_execution_objects(
    mock_dependencies: MockDependencies,
) -> GetProposalExecutionObjectsResponse {
...
async fn decision_reached_flow() {
    ...
    let decision_reached_response =
        batcher_propose_and_get_execution_objects(mock_dependencies).await;
...
async fn test_execution_info_order_is_kept() {
    ...
    let response = batcher_propose_and_get_execution_objects(mock_dependencies).await;
