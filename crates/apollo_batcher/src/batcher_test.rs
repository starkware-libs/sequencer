    ...
    fn commit_proposal(
        ...
        match storage_commitment_block_hash {
            StorageCommitmentBlockHash::ParentHash(parent_block_hash) => {
                if let Some(parent_height) = height.prev() {
                    data.block_hashes.insert(parent_height, parent_block_hash);
                }
            }
            StorageCommitmentBlockHash::Partial(components) => {
...
/// Non-storage mock dependencies for use with `create_batcher_impl`.
...
async fn create_batcher_with_mock_storage(
    ...
    create_batcher_impl(mock_storage, Box::new(storage_writer), mock_dependencies).await
}
