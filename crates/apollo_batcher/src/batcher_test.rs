...
#[case::abort(AfterEndProposalAction::Abort)]
#[tokio::test]
async fn action_on_invalid_proposal(#[case] action: AfterEndProposalAction) {
    let mut batcher =
        create_batcher_with_active_validate_block(Err(BUILD_BLOCK_FAIL_ON_ERROR)).await;
    batcher.await_active_proposal(DUMMY_FINAL_N_EXECUTED_TXS).await.unwrap();

    match action {
        AfterEndProposalAction::Abort => {
...
// This case is tested in `proposal_not_found_after_terminal_action::send_txs_after_abort`.
#[case::send_txs_after_abort(SendProposalContent::Abort, SendProposalContent::Txs(test_txs(0..1)))]
// TODO(Itamar): Remove this case once we migrate to `abort_proposal`.
// This case is tested in `proposal_not_found_after_terminal_action::send_finish_after_abort`.
#[case::send_finish_after_abort(
    SendProposalContent::Abort,
    SendProposalContent::Finish(DUMMY_FINAL_N_EXECUTED_TXS)
)]
// TODO(Itamar): Remove this case once we migrate to `abort_proposal`.
// This case is tested in `proposal_not_found_after_terminal_action::abort_after_abort`.
