...
impl ProofValidator {
    ...
    pub async fn validate_proof(&self, tx: &InternalConsensusTransaction) -> Result<(), String> {
        if let InternalConsensusTransaction::RpcTransaction(internal_rpc_tx) = tx {
            if let InternalRpcTransactionWithoutTxHash::Invoke(RpcInvokeTransaction::V3(v3_tx)) =
                &internal_rpc_tx.tx
            {
                ...
                if !v3_tx.proof_facts.is_empty() {
                    ...
                    if !contains_proof {
                        // TODO(Einat): Try verifying the proof before returning an error.
                        return Err(format!(
                            "Proof not found in proof manager for facts hash: {proof_facts_hash:?}"
                        ));
                    }
                }
            }
