...
impl ProposeTransactionProvider {
    ...
    pub fn new_with_bootstrap(
        ...
        let start_phase = if bootstrap_txs.is_empty() {
            TxProviderPhase::L1
        } else {
    ...
    fn get_bootstrap_txs(&mut self, n_txs: usize) -> Vec<InternalConsensusTransaction> {
        ...
        let result: Vec<_> = self.bootstrap_txs[self.bootstrap_tx_index..self.bootstrap_tx_index + to_take]
            .to_vec();
    ...
    async fn get_txs(&mut self, n_txs: usize) -> TransactionProviderResult<NextTxs> {
        ...
        if self.phase == TxProviderPhase::L1 {
            let n_l1handler_txs_to_get =
                min(self.max_l1_handler_txs_per_block - self.n_l1handler_txs_so_far, n_txs - txs.len());
