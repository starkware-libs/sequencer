...
impl BlockExecutionArtifacts {
    pub async fn new(
        ...
        let (header_commitments, timings) = calculate_block_commitments(
            &transactions_data,
        ...
        .await;
        record_block_commitment_timings(timings);
...
fn record_block_commitment_timings(timings: BlockCommitmentsMeasurements) {
    BLOCK_COMMITMENT_TRANSACTION_LATENCY
        .record_lossy(timings.transaction_commitment_duration.as_secs_f64());
    BLOCK_COMMITMENT_EVENT_LATENCY.record_lossy(timings.event_commitment_duration.as_secs_f64());
    if timings.n_events > 0 {
        BLOCK_COMMITMENT_PER_EVENT_LATENCY.record_lossy(
            timings.event_commitment_duration.as_secs_f64() / timings.n_events as f64,
        );
    }
    BLOCK_COMMITMENT_RECEIPT_LATENCY
        .record_lossy(timings.receipt_commitment_duration.as_secs_f64());
    BLOCK_COMMITMENT_STATE_DIFF_LATENCY
        .record_lossy(timings.state_diff_commitment_duration.as_secs_f64());
}
