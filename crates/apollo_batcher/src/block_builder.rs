...
impl BlockBuilder {
    async fn build_block_inner(&mut self) -> BlockBuilderResult<BlockExecutionArtifacts> {
        ...
        while !(finished_adding_txs && self.n_processed_txs == self.block_txs.len()) {
            ...
            if finished_adding_txs {
                ...
                // transactions.
                self.sleep().await;
            } else if self.start_executing_txs().await? {
                finished_adding_txs = true;
            }
        }

        info!(
            "Finshed building block with {} out of {} transactions.",
            self.n_processed_txs,
            self.block_txs.len()
        );
    ...
    async fn start_executing_txs(&mut self) -> BlockBuilderResult<bool> {
        let n_txs_in_progress = self.block_txs.len() - self.n_processed_txs;
        let n_txs_to_fetch = self.tx_chunk_size - min(n_txs_in_progress, self.tx_chunk_size);
    ...
    async fn handle_processed_txs(&mut self) -> BlockBuilderResult<()> {
        ...
        let old_processed_txs = self.n_processed_txs;
        self.n_processed_txs += results.len();

        self.send_pre_confirmed_txs(old_processed_txs, self.n_processed_txs).await;

        collect_execution_results_and_stream_txs(
            &self.block_txs[old_processed_txs..self.n_processed_txs],
            results,
