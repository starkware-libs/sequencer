...
struct MockDependencies {
    ...
    final_n_executed_txs_sender: tokio::sync::mpsc::Sender<usize>,
    final_n_executed_txs_receiver: tokio::sync::mpsc::Receiver<usize>,
}
...
impl MockDependencies {
    ...
    fn get_final_n_executed_txs_sender(&self) -> tokio::sync::mpsc::Sender<usize> {
        self.final_n_executed_txs_sender.clone()
    }
}
...
-> (tokio::sync::mpsc::Sender<usize>, tokio::sync::mpsc::Receiver<usize>) {
...
async fn get_final_n_executed_txs(mock_dependencies: MockDependencies) {
    let final_n_executed_txs_sender = mock_dependencies.get_final_n_executed_txs_sender();
    let mut validate_tx_provider = mock_dependencies.validate_tx_provider();
    ...
    final_n_executed_txs_sender.send(10).await.unwrap();
