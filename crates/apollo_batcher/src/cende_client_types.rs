...
trait OrderedStruct {
    type UnorderedStruct;
    /// converts to a tuple of (order, non-ordered struct)
    fn to_ordered_tuple(&self, from_address: ContractAddress) -> (usize, Self::UnorderedStruct);

    fn order(&self) -> usize;

    fn getter_from_call_execution<'a>(
        execution: &'a CallExecution,
    ) -> impl Iterator<Item = &'a Self>
    where
        Self: 'a;

    fn accumulated_sorted_structs(
        execution_info: &TransactionExecutionInfo,
    ) -> Vec<Self::UnorderedStruct>
    where
        Self: std::marker::Sized,
    {
        // TODO(Arni): Fix this call. The iterator returns all the call infos in the order:
        // `validate`, `execute`, `fee_transfer`. For `deploy_account` transactions, the
        // order is `execute`, `validate`, `fee_transfer`.
        ...
        for main_call_info in main_call_info_iterator {
            for call_info in main_call_info.iter() {
                let sortable_structs =
                    Self::getter_from_call_execution(&call_info.execution).map(|ordered_struct| {
                        ordered_struct.to_ordered_tuple(main_call_info.call.storage_address)
                    });
                accumulated_sortable_structs.extend(sortable_structs);
            }
        }
        // Sort the structs by their order.
        accumulated_sortable_structs.sort_by_key(|(order, _)| *order);

        // Convert the sorted structs into the UnorderedStruct type.
...
impl OrderedStruct for OrderedL2ToL1Message {
    type UnorderedStruct = L2ToL1Message;
...
impl OrderedStruct for OrderedEvent {
    type UnorderedStruct = Event;
...
fn get_l2_to_l1_messages(execution_info: &TransactionExecutionInfo) -> Vec<L2ToL1Message> {
    OrderedL2ToL1Message::accumulated_sorted_structs(execution_info)
}

fn get_events_from_execution_info(execution_info: &TransactionExecutionInfo) -> Vec<Event> {
    OrderedEvent::accumulated_sorted_structs(execution_info)
}
