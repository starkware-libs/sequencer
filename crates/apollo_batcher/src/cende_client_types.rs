...
fn get_execution_resources(execution_info: &TransactionExecutionInfo) -> ExecutionResources {
    ...
    let resources = &receipt.resources.computation.total_extended_vm_resources().vm_resources;
        ...
        match internal_rpc_transaction.tx {
            ...
            starknet_api::rpc_transaction::InternalRpcTransactionWithoutTxHash::DeployAccount(
                ...
                let InternalRpcDeployAccountTransaction { tx, contract_address } =
                    deploy_account_transaction;
                // V3 and V4 share the same field set; only the version (and the address
                // derivation, already reflected in `contract_address`) differ.
