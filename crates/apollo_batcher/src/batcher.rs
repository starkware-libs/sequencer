...
impl Batcher {
    ...
    pub(crate) fn new(
        ...
        let validation_only = config.static_config.validation_only;
        assert!(
            validation_only == mempool_client.is_none(),
    ...
    pub async fn propose_block(
        ...
        // Guaranteed to be Some because validation_only=false was checked above and the
        // consistency assert in Batcher::new enforces mempool_client is present when not
        // validation-only.
    ...
    pub async fn get_batch_timestamp(&self) -> BatcherResult<UnixTimestamp> {
        let mempool_client =
            self.mempool_client.as_ref().ok_or(BatcherError::ProposingNotSupported)?;
