...
impl Batcher {
    ...
    /// Returns the block hash stored at the given block number (even if it is not part of the
    /// chain).
    pub fn get_stored_block_hash(&self, block_number: BlockNumber) -> BatcherResult<BlockHash> {
        self.storage_reader
