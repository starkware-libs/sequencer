...
use apollo_storage::block_hash::PartialBlockHashComponentsStorageWriter;
...
impl Batcher {
    ...
    pub async fn add_sync_block(&mut self, sync_block: SyncBlock) -> BatcherResult<()> {
        ...
        // TODO(Nimrod): Modify `SyncBlock` to include the partial block hash, it should be already
        // computed.
    ...
    pub async fn decision_reached(
        ...
        let execution_infos = block_execution_artifacts
            ...
            .execution_infos_and_signatures
            .into_iter()
            .map(|(tx_hash, (info, _signature))| (tx_hash, info))
            .collect();
