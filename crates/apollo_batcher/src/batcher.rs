...
impl Batcher {
    ...
    pub async fn add_sync_block(&mut self, sync_block: SyncBlock) -> BatcherResult<()> {
        ...
        let optional_state_diff_commitment =
            if let StorageCommitmentBlockHash::Partial(partial_block_hash_components) =
                &storage_commitment_block_hash
            {
                Some(partial_block_hash_components.header_commitments.state_diff_commitment)
            } else {
                None
            };
        ...
        // Add a commitment task for the block.
        self.commitment_manager
            .as_mut()
            .expect("Commitment manager should be set in non-revert mode.")
            .add_commitment_task(height, state_diff, optional_state_diff_commitment)
            .await
            .map_err(|err| {
                error!("Failed to add commitment task to the commitment manager: {}", err);
                BatcherError::InternalError
            })?;
