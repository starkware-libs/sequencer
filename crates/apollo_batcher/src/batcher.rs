...
pub trait BatcherStorageReaderTrait: Send + Sync {
    ...
    /// For deployed contracts, set class hash to zero.
...
impl BatcherStorageReaderTrait for apollo_storage::StorageReader {
    ...
    fn reversed_state_diff(
        ...
        let Some(ThinStateDiff {
            deployed_contracts,
            storage_diffs,
            class_hash_to_compiled_class_hash,
            nonces,
            ..
        }) = txn.get_state_diff(height)?
        else {
            panic!("Missing state diff for block {height}.");
        };

        // Contracts that were deployed at the given height and are not deployed at the previous
        // height.
        let undeployed_contracts = deployed_contracts
            .keys()
            .map(|contract_address| (*contract_address, ClassHash::default()))
            .collect();

        let state_reader = txn.get_state_reader()?;

        // In the next three maps, set empty values to zero.
        ...
        Ok(ThinStateDiff {
            deployed_contracts: undeployed_contracts,
