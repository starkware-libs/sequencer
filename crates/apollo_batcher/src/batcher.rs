...
impl Batcher {
    ...
    async fn spawn_proposal(
        ...
        let writer_join_handle =
            pre_confirmed_block_writer.map(|mut pre_confirmed_block_writer| {
                tokio::spawn(async move {
                    ...
                    pre_confirmed_block_writer.write_block().await.ok();
                })
            });
        ...
        self.active_proposal_task =
            Some(ProposalTask { abort_signal_sender, join_handle, writer_join_handle });
        Ok(())
    }
    ...
    pub async fn await_active_proposal(&mut self) {
        if let Some(proposal_task) = self.active_proposal_task.take() {
            ...
            if let Some(writer_join_handle) = proposal_task.writer_join_handle {
                writer_join_handle.await.ok();
            }
        }
    }
