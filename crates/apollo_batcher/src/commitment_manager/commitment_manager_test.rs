...
use apollo_batcher_config::config::{BatcherConfig, CommitmentManagerConfig, FirstBlockWithPartialBlockHash};
...
async fn wait_for_empty_sender<T>(sender: &mut Sender<T>) {
    let max_n_retries = 3;
    let mut n_retries = 0;
    while get_number_of_tasks_in_sender(sender) != 0 {
        sleep(Duration::from_millis(500)).await;
        n_retries += 1;
        if n_retries >= max_n_retries {
            panic!(
                "Timed out waiting for sender to be empty after {} retries.",
                max_n_retries
            );
        }
    }
...
/// Fills the task channel by adding three tasks:
/// One task will remain in task channel, one channel will be waiting to be sent to results channel,
/// and third task will be in results channel. Returns the next block height.
async fn fill_task_channel(commitment_manager: &mut ApolloCommitmentManager, first_block_with_partial_block_hash: &Option<FirstBlockWithPartialBlockHash>,storage_reader: Arc<MockBatcherStorageReader>, storage_writer: &mut Box<MockBatcherStorageWriter>) -> BlockNumber {
    ...
    commitment_manager
        .add_commitment_task(first_batch, state_diff.clone(), state_diff_commitment, first_block_with_partial_block_hash, storage_reader.clone(), storage_writer)
        .await
        .unwrap_or_else(|_| panic!("Failed to add commitment task with correct height."));
    wait_for_n_results(&mut commitment_manager.results_receiver, 1).await;
    assert_eq!(get_number_of_tasks_in_receiver(&commitment_manager.results_receiver), 1,);
    assert_eq!(get_number_of_tasks_in_sender(&commitment_manager.tasks_sender), 0,);

    // Adding one additional task is not enough to fill the channel, because the state committer 
    // will read a result from the results channel. Adding two additional tasks to fill both 
    // channels and make task stuck trying to send the third task.
    commitment_manager
        .add_commitment_task(second_batch, state_diff.clone(), state_diff_commitment, first_block_with_partial_block_hash, storage_reader.clone(), storage_writer)
        .await
        .unwrap_or_else(|_| panic!("Failed to add commitment task with correct height."));
    wait_for_empty_sender(&mut commitment_manager.tasks_sender).await;
    commitment_manager
        .add_commitment_task(third_batch, state_diff.clone(), state_diff_commitment, first_block_with_partial_block_hash, storage_reader.clone(), storage_writer)
        .await
        .unwrap_or_else(|_| panic!("Failed to add commitment task with correct height."));
    assert_eq!(get_number_of_tasks_in_receiver(&commitment_manager.results_receiver), 1,);
...
async fn test_create_commitment_manager(mut mock_dependencies: MockDependencies) {
    ...
    assert_eq!(get_number_of_tasks_in_receiver(&commitment_manager.results_receiver), 0,);
}
...
async fn test_add_task_wait_for_full_channel(mut mock_dependencies: MockDependencies) {
    ...
    for height in vec![INITIAL_HEIGHT, INITIAL_HEIGHT.next().unwrap()] {
    mock_dependencies
        .storage_reader
        .expect_get_parent_hash_and_partial_block_hash_components()
        .with(eq(height))
    ...
    let next_height = fill_task_channel(&mut commitment_manager, &None, storage_reader.clone(), &mut storage_writer).await;
...
async fn test_add_task_panic_on_full_channel(mut mock_dependencies: MockDependencies) {
    ...
    let (mut commitment_manager, storage_reader, mut storage_writer) =
        create_commitment_manager(mock_dependencies).await;
    assert!(commitment_manager.config.panic_if_task_channel_full, 
        "Wait for tasks channel should be false for this test.");
    ...
    commitment_manager
        ...
        &None, storage_reader.clone(), &mut storage_writer)
        .await
