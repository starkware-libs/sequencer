...
async fn test_create_commitment_manager_with_missing_tasks(
    ...
    let result = (results.first().unwrap()).clone().expect_commitment().unwrap();
...
async fn test_get_commitment_results(mut mock_dependencies: MockDependencies) {
    ...
    let second_result = results.get(1).unwrap().clone().expect_commitment().unwrap();
    let first_result = results.first().unwrap().clone().expect_commitment().unwrap();
