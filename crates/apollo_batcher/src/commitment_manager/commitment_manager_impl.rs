...
const N_RECENT_STATE_COMMITMENT_INFOS: NonZeroUsize =
    NonZeroUsize::new(2 * STORED_BLOCK_HASH_BUFFER as usize).unwrap();
...
impl<S: StateCommitterTrait> CommitmentManager<S> {
    ...
    >(
        ...
        accessed_keys: Option<StorageAccessedKeys>,
    ...
    >(
        ...
        for commitment_task_output in commitment_results.into_iter() {
            ...
            let FinalBlockCommitment { height, block_hash, global_root, state_commitment_infos } =
    ...
    fn finalize_commitment_output<R: BatcherStorageReader + ?Sized>(
        ...
        Ok(FinalBlockCommitment { height, block_hash, global_root, state_commitment_infos })
    }
    ...
    fn update_task_duration_metric(
        ...
        if let Some(task_duration) = self.task_timer.stop_timer(task_type, height) {
            match task_type {
                // Both commit endpoints share the commit metric; per-endpoint latency is
                // observable via the `request_variant`-labeled infra metrics.
