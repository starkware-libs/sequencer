...
impl<S: StateCommitterTrait> CommitmentManager<S> {
    ...
    /// Verifies that the new global root computer by the committer is consistent with the
    /// global root of blocks received from the state sync component.
    fn validate_commitment_result<R: BatcherStorageReader>(
        &mut self,
        commitment_result: &FinalBlockCommitment,
        storage_reader: Arc<R>,
    ) {
        let global_root = commitment_result.global_root;

        // If the block hash was computed, we can try to compare it to the received block hash from
        // state sync, which is present if we haven't restarted since syncing on block n+1 (n is the
        // currently committed block).
        ...
        // When the block hash was not computed, we settle for verifying the global root,  which is
        // present if we haven't restarted since syncing on block n.
