...
async fn log_error_and_maybe_panic(error: CommitterClientError) {
    // TODO(Amos): If the task performer panics, the Batcher will panic when it attempts to add
    // tasks to the commitment manager. In this case, print an informative message.
    match error {
