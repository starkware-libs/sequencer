...
fn transaction_failed_test_expectations() -> TestExpectations {
    ...
    let signature = TransactionSignature(vec![Felt::ONE].into());
...
fn block_builder_expected_output(
    ...
    let execution_infos_mapping = (0..execution_info_len_u8)
        .map(|i| (tx_hash!(i), (execution_info(), TransactionSignature(vec![i.into()].into()))))
        .collect();
...
async fn partial_chunk_execution_proposer() {
    ...
    let expected_execution_infos_and_signatures: IndexMap<_, _> = executed_txs
        .iter()
        .map(|tx| {
            (tx.tx_hash(), (execution_info(), TransactionSignature(vec![tx.tx_hash().0].into())))
        })
        .collect();
