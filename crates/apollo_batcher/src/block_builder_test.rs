...
impl ExpectationHelper {
    ...
    fn expect_successful_get_processed_txs(&mut self, n_txs: usize) {
    ...
    /// For such a block, `get_processed_txs` and `is_done` will be called repeatedly until the
    /// deadline is reached.
    fn deadline_expectations(&mut self) {
        self.mock_transaction_executor.expect_get_processed_txs().returning(Vec::new);
...
fn one_chunk_mock_executor(
    ...
    helper.expect_successful_get_processed_txs(0);
    helper.expect_is_done(false);
    helper.expect_add_txs_to_block(input_txs);
    helper.expect_successful_get_processed_txs(block_size);
    helper.expect_is_done(false);
...
fn two_chunks_test_expectations() -> TestExpectations {
    ...
    helper.expect_successful_get_processed_txs(first_chunk.len());
    helper.expect_is_done(false);
    helper.expect_add_txs_to_block(&second_chunk);
    helper.expect_successful_get_processed_txs(second_chunk.len());
    helper.expect_is_done(false);
    helper.deadline_expectations();
...
fn mock_transaction_executor_block_full(
    ...
    helper.expect_successful_get_processed_txs(1);
...
fn mock_partial_transaction_execution(
    ...
    if n_completed_txs > 0 {
        helper.expect_successful_get_processed_txs(n_completed_txs);
        helper.expect_is_done(false);
        helper.expect_add_txs_to_block(second_chunk);
    }
...
async fn test_build_block_abort() {
    ...
    helper.expect_successful_get_processed_txs(3);
    helper.expect_is_done(false);
    helper.deadline_expectations();
...
async fn partial_chunk_execution_without_fail_on_error_flag() {
    ...
    helper.expect_successful_get_processed_txs(executed_txs.len());
    helper.expect_is_done(true);
...
async fn partial_chunk_execution_with_fail_on_error_flag() {
    ...
    helper.expect_add_txs_to_block(&input_txs);
    // Return only 2 txs, simulating a partial chunk execution.
    helper.expect_successful_get_processed_txs(2);
    helper.expect_is_done(true);
