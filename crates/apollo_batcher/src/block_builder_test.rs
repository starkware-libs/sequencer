...
fn one_chunk_mock_executor(
    ...
    helper.expect_add_txs_to_block(input_txs, block_size);
...
fn two_chunks_test_expectations() -> TestExpectations {
    ...
    helper.expect_add_txs_to_block(&first_chunk, first_chunk.len());
    helper.expect_add_txs_to_block(&second_chunk, second_chunk.len());
...
fn mock_transaction_executor_block_full(
    ...
    helper.expect_add_txs_to_block(input_txs, n_execution_results);
...
async fn test_build_block_abort() {
    ...
    let mut helper = ExpectationHelper::new();
    helper.expect_add_txs_to_block(&test_txs(0..3), 3);
...
async fn partial_chunk_execution_without_fail_on_error_flag() {
    ...
    helper.expect_add_txs_to_block(&input_txs, executed_txs.len());
...
async fn partial_chunk_execution_with_fail_on_error_flag() {
    ...
    let mut helper = ExpectationHelper::new();
    // Return only 2 transactions, simulating a partial chunk execution.
    helper.expect_add_txs_to_block(&input_txs, 2);
