...
#[rstest]
#[tokio::test]
async fn test_failed_l1_handler_transaction_consumed() {
    let l1_handler_tx = test_l1_handler_txs(0..1);
    ...
    mock_transaction_executor.expect_add_txs_to_block().times(1).return_once(move |_| {
        vec![Err(TransactionExecutorError::StateError(StateError::OutOfRangeContractAddress))]
    });
    ...
    // Verify that the L1 handler transaction is in the consumed l1 transactions
    assert!(
        result_block_artifacts
            .execution_data
            .consumed_l1_handler_tx_hashes
            .contains(&l1_handler_tx[0].tx_hash())
    );
}
