...
impl BatcherStorageReaderHandler {
    pub fn new(
        ...
        Self { storage_reader, bootstrap_state_machine }
    }
            ...
            BatcherStorageReaderRequest::Bootstrap(req) => {
                let resp = match req {
                    BootstrapRequest::GetBootstrapState => BootstrapResponse::BootstrapState(
                        self.bootstrap_state_machine.current_state(&self.storage_reader),
                    ),
                    BootstrapRequest::GetBootstrapTransactions => {
                        let state =
                            self.bootstrap_state_machine.current_state(&self.storage_reader);
                        BootstrapResponse::BootstrapTransactions(
                            self.bootstrap_state_machine.transactions_for_state(state),
                        )
                    }
                };
...
impl BatcherStorageReaderServer {
    pub fn new(
        ...
        let handler = BatcherStorageReaderHandler::new(storage_reader, bootstrap_state_machine);
        Self { handler, config }
    }
...
pub fn create_bootstrap_storage_reader_server(
    ...
    if config.is_enabled() {
        Some(BatcherStorageReaderServer::new(storage_reader, config, bootstrap_state_machine))
    } else {
        None
    }
}
...
mod tests {
    ...
    fn make_handler(bootstrap_enabled: bool) -> (BatcherStorageReaderHandler, StorageReader) {
        ...
        let sm = Arc::new(BootstrapStateMachine::new(bootstrap_enabled));
        let handler = BatcherStorageReaderHandler::new(reader.clone(), sm);
        (handler, reader)
    }
