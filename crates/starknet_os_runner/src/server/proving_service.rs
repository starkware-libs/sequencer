//! Business logic for the proving service.
//!
...
/// Service for proving transactions.
///
/// Encapsulates all proving logic, including OS execution and proof generation.
/// This service is independent of the HTTP layer and can be used directly for testing.
#[derive(Clone)]
...
impl ProvingService {
    /// Creates a new ProvingService from configuration.
    ...
    /// Returns the chain ID used by this service.
    ...
    pub async fn prove_transaction(
    ...
    ) -> Result<ProvingOutput, ProvingServiceError> {
        ...
        let runner_output = runner
            .run_virtual_os(txs)
            .await
            .map_err(|err| ProvingServiceError::RunnerError(Box::new(err)))?;
...
pub fn extract_invoke_tx(tx: RpcTransaction) -> Result<InvokeTransaction, ProvingServiceError> {
    match tx {
        RpcTransaction::Invoke(RpcInvokeTransaction::V3(invoke_v3)) => {
            Ok(InvokeTransaction::V3(invoke_v3.into()))
        }
        RpcTransaction::Declare(_) => Err(ProvingServiceError::InvalidTransactionType(
            "Declare transactions are not supported; only Invoke transactions are allowed"
                .to_string(),
        )),
        RpcTransaction::DeployAccount(_) => Err(ProvingServiceError::InvalidTransactionType(
...
pub fn calculate_tx_hash(
    ...
    chain_id: &ChainId,
) -> Result<TransactionHash, ProvingServiceError> {
    let version = invoke_tx.version();
    invoke_tx.calculate_transaction_hash(chain_id, &version).map_err(|e| {
        ProvingServiceError::ValidationError(format!("Failed to calculate transaction hash: {e}"))
    })
}
