...
fn compile_schema_for_ref(spec: &Value, ref_path: &str) -> JSONSchema {
    ...
    let ref_schema: Value =
        serde_json::from_str(&format!(r#"{{"$ref": "{ref_uri}"}}"#)).unwrap();
...
fn compile_error_schemas(spec: &Value, method_name: &str) -> JSONSchema {
    ...
    let any_of: Vec<Value> =
        refs.iter().map(|r| serde_json::json!({"$ref": r})).collect();
...
fn build_test_rpc_module() -> RpcModule<ProvingRpcServerImpl> {
    ...
    let rpc_impl =
        ProvingRpcServerImpl::new(RpcVirtualSnosProver::new(&config), 2);
    rpc_impl.into_rpc()
}
...
fn prove_transaction_result_matches_schema() {
    ...
    let result = ProveTransactionResult {
        ...
        l2_to_l1_messages: vec![MessageToL1 {
            ...
            to_address: EthAddress::try_from(felt!("0xdead"))
                .unwrap(),
    ...
    let schema =
        compile_schema_for_ref(&spec, "components/schemas/PROVE_TRANSACTION_RESULT");
    assert!(
...
#[test]
fn proof_schema_validates_base64_encoding() {
    let spec = load_spec();
    let proof_schema = compile_schema_for_ref(&spec, "components/schemas/PROOF");

    let valid_proof = serde_json::to_value(Proof::from(vec![1u32, 2, 3])).unwrap();
    assert!(
        proof_schema.is_valid(&valid_proof),
        "Expected serialized proof to satisfy PROOF schema.\nProof: {}",
        serde_json::to_string_pretty(&valid_proof).unwrap(),
    );

    let invalid_proof = serde_json::json!("not_base64!");
    assert!(
        !proof_schema.is_valid(&invalid_proof),
        "Expected invalid base64 proof to be rejected by PROOF schema.\nProof: {}",
        serde_json::to_string_pretty(&invalid_proof).unwrap(),
    );
}

#[test]
fn rpc_transaction_schema_matches_invoke_v3_shape() {
    let spec = load_spec();
    ...
    assert!(
        tx_schema.is_valid(&valid_tx),
        "Valid invoke transaction does not match RPC_TRANSACTION schema.\nTx: {}\nErrors: {:?}",
        serde_json::to_string_pretty(&valid_tx).unwrap(),
        tx_schema
            .validate(&valid_tx)
            .err()
            .map(|e| e.collect::<Vec<_>>()),
    ...
    let invalid_tx_type = serde_json::json!({
        "type": "DECLARE",
        "sender_address": "0x1",
        "calldata": ["0x2"],
        "version": "0x3",
    ...
    assert!(
        !tx_schema.is_valid(&invalid_tx_type),
        "Expected non-invoke transaction to be rejected by RPC_TRANSACTION schema.\nTx: {}",
        serde_json::to_string_pretty(&invalid_tx_type).unwrap(),
    );

    let invalid_tx_proof = serde_json::json!({
        "type": "INVOKE",
        "sender_address": "0x1",
        "calldata": ["0x2"],
        "version": "0x3",
    ...
    assert!(
        !tx_schema.is_valid(&invalid_tx_proof),
        "Expected invalid proof encoding to be rejected by RPC_TRANSACTION schema.\nTx: {}",
        serde_json::to_string_pretty(&invalid_tx_proof).unwrap(),
    );
}
...
#[tokio::test]
async fn spec_version_call_succeeds_with_zero_params() {
    let module = build_test_rpc_module();
    let response = raw_call(&module, "starknet_specVersion").await;
    assert!(
        response.get("result").is_some(),
        "starknet_specVersion should succeed with zero params, got: {}",
