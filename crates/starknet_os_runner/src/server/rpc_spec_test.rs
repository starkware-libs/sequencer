...
fn build_method_test_cases(
    ...
    sample_values
        ...
        .map(|(name, values)| {
            ...
            MethodTestCase { method_name: name, method_index, sample_values: values }
        })
        .collect()
}
...
async fn spec_request_params_match_rpc_module(rpc_module: RpcModule<ProvingRpcServerImpl>) {
    ...
    for MethodTestCase { method_name, method_index, sample_values } in &test_cases {
...
/// Builds response test cases for each RPC method with sample result values.
/// Also asserts that the test cases cover every method in the spec and module (completeness guard).
///
/// For specVersion: calls the real RPC module to obtain the actual response.
/// For proveTransaction: uses a sample result value.
async fn build_response_test_cases(
    module: &RpcModule<ProvingRpcServerImpl>,
) -> Vec<ResponseTestCase> {
    ...
    let sample_results: Vec<(&str, Value)> = vec![
    ...
    // Completeness guard: test cases must cover all spec and module methods.
    let methods = SPEC["methods"].as_array().unwrap();
    let tested: HashSet<&str> = sample_results.iter().map(|(name, _)| *name).collect();
    let spec_methods: HashSet<&str> =
        methods.iter().map(|m| m["name"].as_str().unwrap()).collect();
    let module_methods: HashSet<&str> = module.method_names().collect();
    assert_eq!(tested, spec_methods, "Response test cases don't cover all spec methods");
    assert_eq!(tested, module_methods, "Response test cases don't cover all module methods");
    ...
    sample_results
        ...
        .map(|(name, result)| {
            let method_index =
                methods.iter().position(|m| m["name"].as_str().unwrap() == name).unwrap();
            ResponseTestCase { method_name: name, method_index, sample_result: result }
        })
        .collect()
}
...
async fn spec_response_schemas_match_rpc_module(rpc_module: RpcModule<ProvingRpcServerImpl>) {
    let test_cases = build_response_test_cases(&rpc_module).await;

    for ResponseTestCase { method_name, method_index, sample_result } in &test_cases {
        let schema =
