...
/// Extracts and deserializes a single field from a JSON object.
fn json_field<T: serde::de::DeserializeOwned>(
    obj: &serde_json::Value,
    name: &str,
) -> Result<T, String> {
    serde_json::from_value(obj.get(name).cloned().ok_or_else(|| format!("missing {name}"))?)
        .map_err(|e| format!("{name}: {e}"))
}

/// Deserializes pathfinder's v0.10 `initial_reads` JSON into blockifier `StateMaps`.
///
/// Pathfinder serializes each map as an array of objects (e.g. `[{contract_address, key, value}]`).
pub(crate) fn deserialize_initial_reads(value: serde_json::Value) -> Result<StateMaps, String> {
    let mut state_maps = StateMaps::default();
    let empty = vec![];

    for item in value.get("storage").and_then(|v| v.as_array()).unwrap_or(&empty) {
        state_maps.storage.insert(
            (json_field(item, "contract_address")?, json_field(item, "key")?),
            json_field(item, "value")?,
        );
    }
    for item in value.get("nonces").and_then(|v| v.as_array()).unwrap_or(&empty) {
        state_maps
            .nonces
            .insert(json_field(item, "contract_address")?, json_field(item, "nonce")?);
    }
    for item in value.get("class_hashes").and_then(|v| v.as_array()).unwrap_or(&empty) {
        state_maps
            .class_hashes
            .insert(json_field(item, "contract_address")?, json_field(item, "class_hash")?);
    }
    for item in value.get("declared_contracts").and_then(|v| v.as_array()).unwrap_or(&empty) {
        let is_declared = item.get("is_declared").and_then(|v| v.as_bool()).unwrap_or(false);
        state_maps
            .declared_contracts
            .insert(json_field(item, "class_hash")?, is_declared);
    }
...
///     "http://localhost:9547/rpc/v0_10".to_string(),
///     chain_info,
///     block_id,
///     RpcVirtualBlockExecutorConfig { use_simulate: true },
/// );
...
pub(crate) trait VirtualBlockExecutor: Send + 'static {
    /// Configuration type for this executor. Each impl has its own concrete config.
    ...
    /// When `txs` is provided, implementations may use them to prefetch state (e.g. via simulate).
    fn state_reader(
        &self,
        block_id: BlockId,
...
/// State reader that uses prefetched StateMaps for state reads and delegates compiled class
/// lookups to RpcStateReader (since initial_reads from simulate do not include class bytecode).
struct SimulatedStateReader {
...
impl StateReader for SimulatedStateReader {
    fn get_storage_at(
        ...
        Ok(self.state_maps.storage.get(&(contract_address, key)).copied().unwrap_or_default())
    }

    fn get_nonce_at(&self, contract_address: ContractAddress) -> StateResult<Nonce> {
        Ok(self.state_maps.nonces.get(&contract_address).copied().unwrap_or_default())
    }

    fn get_class_hash_at(&self, contract_address: ContractAddress) -> StateResult<ClassHash> {
        Ok(self.state_maps.class_hashes.get(&contract_address).copied().unwrap_or_default())
    }
...
/// Configuration for the RPC-backed virtual block executor.
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
pub(crate) struct RpcVirtualBlockExecutorConfig {
    /// When true, call starknet_simulateTransactions with RETURN_INITIAL_READS to prefetch state.
    pub(crate) use_simulate: bool,
}
...
pub(crate) struct RpcVirtualBlockExecutor {
    ...
    /// Executor-specific configuration (e.g. use_simulate for prefetch).
...
impl RpcVirtualBlockExecutor {
    ...
    /// Calls starknet_simulateTransactions with RETURN_INITIAL_READS and parses initial_reads into StateMaps.
    fn simulate_and_create_reader(
        &self,
        block_id: BlockId,
        txs: &[(InvokeTransaction, TransactionHash)],
        ...
        let result = self.rpc_state_reader.send_rpc_request("starknet_simulateTransactions", params).map_err(
            |e| VirtualBlockExecutorError::ReexecutionError(Box::new(e.into())),
        )?;

        let initial_reads_value = result.get("initial_reads").cloned().ok_or_else(|| {
            VirtualBlockExecutorError::TransactionExecutionError(
                "simulateTransactions response missing initial_reads (ensure RETURN_INITIAL_READS and v0.10 endpoint)".to_string(),
            )
        })?;

        deserialize_initial_reads(initial_reads_value).map_err(|e| {
            VirtualBlockExecutorError::TransactionExecutionError(format!(
...
impl VirtualBlockExecutor for RpcVirtualBlockExecutor {
    type Config = RpcVirtualBlockExecutorConfig;
    ...
    fn state_reader(
        ...
        if self.config.use_simulate {
            let state_maps = self.simulate_and_create_reader(block_id, txs)?;
            Ok(Box::new(SimulatedStateReader {
                state_maps,
                rpc_state_reader: self.rpc_state_reader.clone(),
            }))
        } else {
            Ok(Box::new(self.rpc_state_reader.clone()))
        }
    }
