...
pub enum StwoRunAndProveError {
    ...
    #[error(
        "stwo_run_and_prove binary not found. \
         Either set {STWO_RUN_AND_PROVE_PATH_ENV} environment variable, \
         add it to PATH, or run: scripts/install_stwo_run_and_prove.sh"
    )]
    BinaryNotFound,
    ...
    #[error(
        "stwo_run_and_prove failed with exit code {exit_code}.\n\
         Stderr (last {stderr_lines} lines):\n{stderr}"
    )]
    ...
    #[error(
        "stwo_run_and_prove timed out after {duration:?}. \
         Consider increasing the timeout or checking resource constraints."
    )]
...
pub struct StwoRunAndProveConfig {
    ...
    /// Whether to save debug data on error.
...
/// Builder for constructing `StwoRunAndProveInput`.
#[derive(Debug, Default)]
pub struct StwoRunAndProveInputBuilder {
    program_path: Option<PathBuf>,
    program_input_path: Option<PathBuf>,
    prover_params_path: Option<PathBuf>,
    proof_output_path: Option<PathBuf>,
    program_output_path: Option<PathBuf>,
}

impl StwoRunAndProveInputBuilder {
    ...
    pub fn program_path(mut self, path: impl AsRef<Path>) -> Self {
        self.program_path = Some(path.as_ref().to_path_buf());
        self
    }

    /// Sets the program input path.
    pub fn program_input_path(mut self, path: impl AsRef<Path>) -> Self {
        self.program_input_path = Some(path.as_ref().to_path_buf());
        self
    }

    /// Sets the prover parameters path.
    pub fn prover_params_path(mut self, path: impl AsRef<Path>) -> Self {
        self.prover_params_path = Some(path.as_ref().to_path_buf());
        self
    }

    /// Sets the proof output path.
    pub fn proof_output_path(mut self, path: impl AsRef<Path>) -> Self {
        self.proof_output_path = Some(path.as_ref().to_path_buf());
        self
    }

    /// Sets the program output path.
    pub fn program_output_path(mut self, path: impl AsRef<Path>) -> Self {
        self.program_output_path = Some(path.as_ref().to_path_buf());
        self
    }
    ...
    /// # Errors
    ///
    /// Returns an error if required fields (program_path, proof_output_path) are not set.
    pub fn build(self) -> Result<StwoRunAndProveInput, StwoRunAndProveError> {
        let program_path = self.program_path.ok_or_else(|| {
            StwoRunAndProveError::InvalidInputPath { path: PathBuf::from("<program_path not set>") }
        })?;

        let proof_output_path =
            self.proof_output_path.ok_or_else(|| StwoRunAndProveError::InvalidInputPath {
                path: PathBuf::from("<proof_output_path not set>"),
            })?;

        Ok(StwoRunAndProveInput {
            program_path,
            program_input_path: self.program_input_path,
            prover_params_path: self.prover_params_path,
            proof_output_path,
            program_output_path: self.program_output_path,
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_proof_format_as_str() {
        assert_eq!(ProofFormat::Json.as_str(), "json");
        assert_eq!(ProofFormat::CairoSerde.as_str(), "cairo-serde");
        assert_eq!(ProofFormat::Binary.as_str(), "binary");
    }

    #[test]
    fn test_input_builder() {
        let input = StwoRunAndProveInputBuilder::new()
            .program_path("/path/to/program.json")
            .proof_output_path("/path/to/proof.json")
            .program_input_path("/path/to/input.json")
            .build()
            .unwrap();

        assert_eq!(input.program_path, PathBuf::from("/path/to/program.json"));
        assert_eq!(input.proof_output_path, PathBuf::from("/path/to/proof.json"));
        assert_eq!(input.program_input_path, Some(PathBuf::from("/path/to/input.json")));
        assert!(input.prover_params_path.is_none());
        assert!(input.program_output_path.is_none());
    }

    #[test]
    fn test_input_builder_missing_required_fields() {
        // Missing program_path.
        let result =
            StwoRunAndProveInputBuilder::new().proof_output_path("/path/to/proof.json").build();
        assert!(result.is_err());

        // Missing proof_output_path.
        let result =
            StwoRunAndProveInputBuilder::new().program_path("/path/to/program.json").build();
        assert!(result.is_err());
    }
}
