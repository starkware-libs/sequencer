    ...
    V: VirtualBlockExecutor,
{
    ...
    /// Consumes the runner.
    pub(crate) async fn execute_virtual_block(
        self,
        txs: Vec<(InvokeTransaction, TransactionHash)>,
    ) -> Result<VirtualBlockExecutionData, RunnerError> {
        // Destructure self to move executor into spawn_blocking while keeping providers.
        let Self { virtual_block_executor, contract_class_manager, block_id, .. } = self;
