...
use starknet_api::transaction::{InvokeTransaction, InvokeTransactionV3, Transaction};
...
fn construct_balance_of_invoke() -> BlockifierTransaction {
    ...
    let invoke_tx = InvokeTransactionV3 {
        ...
        calldata,
        signature: TransactionSignature::default(), // Empty - validation skipped
    ...
    let tx = Transaction::Invoke(InvokeTransaction::V3(invoke_tx));
    let tx_hash = tx.calculate_transaction_hash(&ChainId::Mainnet).unwrap();

    // Skip fee charging and nonce check, but enable validation for this test.
