...
fn verify_storage_proof_completeness(
    ...
    while index < FELT_BITS {
        ...
        // Parse the preimage.
        let preimage = Preimage::try_from(raw_preimage)
            .unwrap_or_else(|e| panic!("Failed to parse preimage: {e:?}"));
        ...
        match preimage {
            ...
            Preimage::Edge(edge_data) => {
                ...
                if index + path_length > FELT_BITS {
                    panic!(
                        "Edge path extends beyond tree height: index={index}, \
                         path_length={path_length}, total={FELT_BITS}",
                    );
                }
                ...
                if relevant_key_path != relevant_edge_path {
                    ...
                    // If the key doesn't exist and we're not expecting it to, we're done.
                    return;
                }
...
#[case::non_existing_key(StorageKey::from(999999u32), false, false)]
...
#[should_panic(expected = "Missing node in commitment_facts")]
#[case::non_existing_key_incomplete(StorageKey::from(999999u32), false, true)]
#[ignore]
