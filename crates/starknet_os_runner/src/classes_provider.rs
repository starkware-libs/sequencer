...
pub trait ClassesProvider {
            ...
            if let Some(runnable) = contract_class_manager.get_runnable(&class_hash) {
                match runnable {
                    ...
                    RunnableCompiledClass::V1(v1) => {
                        ...
                        // TODO(Aviv): Verify that we actually need the compiled class hash.
                        let compiled_hash = contract_class_manager
    ...
    /// - `prime`: Set to the Stark prime constant
    ...
    fn compiled_class_v1_to_casm(class: &CompiledClassV1) -> CasmContractClass {
        ...
        let bytecode_segment_lengths: NestedIntList = class.bytecode_segment_felt_sizes().into();

        CasmContractClass {
            prime,
            compiler_version: String::new(),
            bytecode,
            bytecode_segment_lengths: Some(bytecode_segment_lengths),
            hints: Vec::new(),
            pythonic_hints: None,
