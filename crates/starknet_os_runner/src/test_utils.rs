...
/// Fixture: RpcRunnerFactory configured for Sepolia.
#[fixture]
pub fn sepolia_runner_factory() -> RpcRunnerFactory {
    ...
    RpcRunnerFactory::new(rpc_url, ChainId::Sepolia, contract_class_manager)
}

/// Fetches the latest block number from Sepolia (async).
pub async fn fetch_sepolia_block_number() -> BlockNumber {
    let rpc_url = Url::parse(&get_sepolia_rpc_url()).expect("Invalid Sepolia RPC URL");
    let provider = RpcStorageProofsProvider::new(rpc_url);
    BlockNumber(provider.0.block_number().await.expect("Failed to fetch block number"))
}
