...
use apollo_class_manager_types::proof_verification::run_stwo_verify_on_proof;
...
async fn test_prove_cairo_pie_10_transfers() {
    ...
    // Verify the proof.
    let verify_output = run_stwo_verify_on_proof(output.proof.clone())
        .await
...
fn test_simple_bootloader_program_hash_matches_expected() {
    ...
    let program = Program::from_bytes(&program_bytes, Some("main"))
        .expect("Failed to load bootloader");
