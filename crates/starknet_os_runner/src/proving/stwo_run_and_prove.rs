...
impl From<CairoPie> for SimpleBootloaderInput {
    fn from(cairo_pie: CairoPie) -> Self {
        let task = Task::Pie(cairo_pie);
        let task_spec = TaskSpec { task: Rc::new(task), program_hash_function: HashFunc::Blake };
        Self { fact_topologies_path: None, single_page: true, tasks: vec![task_spec] }
    }
}
...
pub(crate) fn prove_pie_in_memory(
    ...
    let bootloader_input = SimpleBootloaderInput::from(cairo_pie);
