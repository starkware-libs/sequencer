...
///
/// Grouped by feed, so the page names the rejected reading. The derived ETH/STRK rate reads both
/// USD feeds, and one alert instance per feed is what separates a dead ETH/USD feed from a dead
/// STRK/USD one.
///
...
/// fallback carries no `feed` label, so once the counter exists it joins the per-feed instances as
/// an unlabeled one that holds at 0 and never fires.
///
