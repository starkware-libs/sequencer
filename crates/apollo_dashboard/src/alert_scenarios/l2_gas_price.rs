...
pub(crate) fn get_l2_gas_price_above_maximum_alert() -> Alert {
    ...
    let above_maximum_query = CONSENSUS_L2_GAS_PRICE_CLAMPED
        .get_name_with_filer_and_additional_fields(&format!(
            "{LABEL_L2_GAS_PRICE_CLAMP_BOUND}=\"{maximum_bound}\""
        ));
    Alert::new(
        "consensus_l2_gas_price_above_maximum",
        "L2 gas price computation above the configured maximum",
        EvaluationRate::Default,
        format!("sum(increase({above_maximum_query}[{COUNTER_SAMPLING_WINDOW}])) or vector(0)"),
        vec![AlertCondition::new(AlertComparisonOp::GreaterThan, 0.0, AlertLogicalOp::And)],
        "10m",
