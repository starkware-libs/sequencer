...
fn get_consensus_p2p_no_peers_subscribed_errors(
    alert_env_filtering: AlertEnvFiltering,
    alert_severity: AlertSeverity,
) -> Alert {
    Alert::new(
        "consensus_p2p_no_peers_subscribed_errors",
        "Consensus p2p no peers subscribed errors",
        AlertGroup::Consensus,
        format!(
            "increase({}[2m])",
            CONSENSUS_NUM_NO_PEERS_SUBSCRIBED_ERRORS.get_name_with_filter()
        ),
        vec![AlertCondition {
            comparison_op: AlertComparisonOp::GreaterThan,
            comparison_value: 5.0,
...
pub(crate) fn get_consensus_p2p_no_peers_subscribed_errors_vec() -> Vec<Alert> {
    vec![
        get_consensus_p2p_no_peers_subscribed_errors(
            AlertEnvFiltering::MainnetStyleAlerts,
            AlertSeverity::Sos,
        ),
        get_consensus_p2p_no_peers_subscribed_errors(
