...
use super::{
    get_chainlink_oracle_future_feed_alert,
...
/// Every alert built by `guard_trip_alert`, so a new guard alert is covered by each test below
/// without being listed in it.
fn guard_alerts() -> [Alert; 3] {
    [
        get_chainlink_oracle_stale_feed_alert(),
        get_chainlink_oracle_future_feed_alert(),
        get_chainlink_oracle_rate_out_of_bounds_alert(),
...
/// The guard counters are labeled by currency pair, so the page must name the rejected reading
/// rather than aggregate the two USD feeds and the derived rate into one number.
#[test]
fn guard_alerts_group_by_currency_pair() {
    for alert in guard_alerts() {
        let alert_json = alert_as_json(&alert);
        let expression = alert_json["expr"].as_str().expect("Expression should be a string.");
        assert!(
            expression.contains(&format!("sum by ({LABEL_NAME_CURRENCY_PAIR})")),
            "{} must group by the currency pair label, got: {expression}.",
            alert_json["name"]
        );
    }
}
...
#[test]
fn the_bounds_guard_outranks_the_freshness_guards() {
    ...
    assert_eq!(alert_as_json(&get_chainlink_oracle_future_feed_alert())["severity"], "p4");
