...
use apollo_l1_gas_price::metrics::LABEL_NAME_CHAINLINK_FEED;
...
/// The guard counters are labeled by feed, so the page must name the rejected reading rather than
/// aggregate the two USD feeds and the derived rate into one number.
#[test]
fn guard_alerts_group_by_feed() {
    for alert in
        [get_chainlink_oracle_stale_feed_alert(), get_chainlink_oracle_rate_out_of_bounds_alert()]
    {
        let alert_json = alert_as_json(&alert);
        let expression = alert_json["expr"].as_str().expect("Expression should be a string.");
        assert!(
            expression.contains(&format!("sum by ({LABEL_NAME_CHAINLINK_FEED})")),
            "{} must group by the feed label, got: {expression}.",
            alert_json["name"]
        );
    }
}
