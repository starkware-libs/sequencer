...
use super::{get_chainlink_oracle_rate_out_of_bounds_alert, get_chainlink_oracle_stale_feed_alert};
...
fn guard_alerts_evaluate_to_zero_before_the_first_trip() {
    // `sum` over an empty vector returns empty, which would put the rule in the no-data state that
    // the generated rules page on. The fallback keeps it a real zero until a guard first trips.
    for alert in
        [get_chainlink_oracle_stale_feed_alert(), get_chainlink_oracle_rate_out_of_bounds_alert()]
    {
        let alert_json = alert_as_json(&alert);
...
fn the_bounds_guard_outranks_the_freshness_guard() {
    // A feed publishing an impossible price is the case consensus cannot detect on its own, so it
    // must not be filed behind a feed that merely stopped publishing.
