...
const PANEL_CENDE_PREPARED_BLOB_BLOCK_NUMBER: Panel =
    Panel::from_gauge(CENDE_PREPARED_BLOB_BLOCK_NUMBER, PanelType::Stat);
const PANEL_CENDE_PREPARE_BLOB_FOR_NEXT_HEIGHT_LATENCY: Panel = Panel::new(
    ...
    formatcp!("avg_over_time({}[5m])", CENDE_PREPARE_BLOB_FOR_NEXT_HEIGHT_LATENCY.get_name()),
    PanelType::Stat,
);
const PANEL_CENDE_WRITE_PREV_HEIGHT_BLOB_LATENCY: Panel = Panel::new(
    ...
    formatcp!("avg_over_time({}[5m])", CENDE_WRITE_PREV_HEIGHT_BLOB_LATENCY.get_name()),
    PanelType::Stat,
);
...
const CONSENSUS_ROW: Row = Row::new(
    ...
    &[
        ...
        PANEL_CENDE_PREPARED_BLOB_BLOCK_NUMBER,
