...
pub fn get_apollo_dashboard() -> Dashboard {
    // Rows are grouped into logical sections, in the order an operator would investigate from
    // overall health → block production → transaction lifecycle → execution → networking →
    // operational concerns → infrastructure / per-component plumbing.
    Dashboard::new(
