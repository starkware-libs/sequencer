...
fn _get_local_server_panels(local_server_metrics: &LocalServerMetrics) -> Vec<Panel> {
    let received_msgs_panel =
        Panel::from_counter(local_server_metrics.received_msgs, PanelType::TimeSeries);
    let processed_msgs_panel =
        Panel::from_counter(local_server_metrics.processed_msgs, PanelType::TimeSeries);
    let queue_depth_panel =
        Panel::from_gauge(local_server_metrics.queue_depth, PanelType::TimeSeries);
    let processing_times_panels = create_request_type_labeled_hist_panels(
        local_server_metrics.processing_times,
        PanelType::TimeSeries,
    );
    let queueing_times_panels = create_request_type_labeled_hist_panels(
        local_server_metrics.queueing_times,
        PanelType::TimeSeries,
    );
...
fn _get_remote_server_panels(remote_server_metrics: &RemoteServerMetrics) -> Vec<Panel> {
    let total_received_msgs_panel =
        Panel::from_counter(remote_server_metrics.total_received_msgs, PanelType::TimeSeries);
    let valid_received_msgs_panel =
        Panel::from_counter(remote_server_metrics.valid_received_msgs, PanelType::TimeSeries);
    let processed_msgs_panel =
        Panel::from_counter(remote_server_metrics.processed_msgs, PanelType::TimeSeries);
    let number_of_connections_panel =
        Panel::from_gauge(remote_server_metrics.number_of_connections, PanelType::TimeSeries);
    vec![
