...
fn merge_panel_map<'a>(
    local: HashMap<&'a str, Vec<Panel>>,
    remote: HashMap<&'a str, Vec<Panel>>,
) -> HashMap<&'a str, Vec<Panel>> {
    let mut merged: HashMap<&'a str, Vec<Panel>> = HashMap::new();
    for (k, v) in local {
        merged.entry(k).or_default().extend(v.into_iter().map(|p| {
            let mut p = p;
            p.name = format!("local {}", p.name);
            p
        }));
    }
    for (k, v) in remote {
...
pub(crate) fn get_component_infra_row(row_name: &'static str, metrics: &InfraMetrics) -> Row {
    ...
    let mut labeled_client_panels = HashMap::new();
    for (k, v) in grouped_client_panels {
        let mut labeled_client_panel = merge_panels(k, v);
        labeled_client_panel.name = format!("{} (client)", labeled_client_panel.name);
        labeled_client_panels.insert(k, labeled_client_panel);
    }

    let mut labeled_server_panels = HashMap::new();
    for (k, v) in grouped_server_panels {
        let mut labeled_server_panel = merge_panels(k, v);
        labeled_server_panel.name = format!("{} (server)", labeled_server_panel.name);
        labeled_server_panels.insert(k, labeled_server_panel);
    }
