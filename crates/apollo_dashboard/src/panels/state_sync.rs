...
fn get_panel_central_sync_central_block_marker() -> Panel {
    Panel::new(
        ...
        "The first block number that is not yet known to the centralized Starknet",
...
fn get_panel_state_sync_new_header_available() -> Panel {
    Panel::new(
        "New Block Header Latency",
        "The time it takes for a block header to become available after the block's timestamp",
...
pub(crate) fn get_state_sync_row() -> Row {
    Row::new(
        ...
        vec![
            ...
            get_panel_state_sync_new_header_available(),
