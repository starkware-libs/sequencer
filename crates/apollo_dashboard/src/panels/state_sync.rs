...
fn get_panel_state_sync_processed_transactions() -> Panel {
    Panel::new(
        ...
        "The number of transactions processed by the state sync (10m window)",
        vec![format!(
            "increase({}[10m])",
...
fn get_panel_state_sync_reverted_transactions() -> Panel {
    Panel::new(
        ...
        "The number of transactions reverted by the state sync (10m window)",
...
fn get_panel_state_sync_partially_synced_blocks() -> Panel {
    Panel::new(
        "Partially Synced Blocks",
        "The number of blocks with known headers but not yet fully synced",
        vec![format!(
            "{} - {}",
            STATE_SYNC_HEADER_MARKER.get_name_with_filter(),
            STATE_SYNC_CLASS_MANAGER_MARKER.get_name_with_filter()
        )],
        PanelType::TimeSeries,
    )
...
pub(crate) fn get_state_sync_row() -> Row {
    Row::new(
        ...
        vec![
            ...
            get_panel_state_sync_partially_synced_blocks(),
