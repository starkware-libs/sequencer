...
fn get_panel_mempool_pending_queue_size() -> Panel {
    Panel::new(
        ...
        "Number of transactions ready for batching but not prioritized",
...
fn get_panel_mempool_total_size_in_bytes() -> Panel {
    Panel::new(
        "Mempool Total Size in Bytes",
        "Average size in bytes of the the transactions in the mempool (2m window)",
...
fn get_panel_mempool_transaction_time_spent_until_committed() -> Panel {
    Panel::new(
        "Time Spent in Mempool Until Committed",
