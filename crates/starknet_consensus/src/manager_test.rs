...
async fn timely_message_handling() {
    ...
    // Try sending another proposal, to check that that, even though sync was known at the beginning
    // of the height and so consensus was not actually run, the inbound channels are cleared.
