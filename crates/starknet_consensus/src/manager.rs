...
impl<ContextT: ConsensusContext> MultiHeightManager<ContextT> {
    ...
    fn report_max_cached_height_metric(&self, height: BlockNumber) {
        // If nothing is cached use current height as "max", so the metric is 0.
        let max_cached_height = self.cached_proposals.keys().max().unwrap_or(&height.0);
        let cached_heights_above_current = max_cached_height - height.0;

        // TODO(guyn): use int metrics so `as f64` may be removed.
        #[allow(clippy::as_conversions)]
        CONSENSUS_MAX_CACHED_HEIGHT.set(cached_heights_above_current as f64);
    }
}
