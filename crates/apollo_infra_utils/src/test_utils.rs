...
// The allocated range (currently 11000..63000) overlaps the kernel's ephemeral port range
// (`/proc/sys/net/ipv4/ip_local_port_range`, 32768..60999 by default), so an outbound connection
// can take a port between `is_port_in_use` returning false and the child process binding it. The
// whole range does not fit below the ephemeral floor at the current
// MAX_NUMBER_OF_TESTS * MAX_NUMBER_OF_INSTANCES_PER_TEST * PORTS_PER_INSTANCE budget.
