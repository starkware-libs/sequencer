use std::env;
use std::fs::{create_dir_all, set_permissions, File, OpenOptions, Permissions, TryLockError};
...
fn lease_port_slot(label: &str) -> u16 {
    ...
    // 1777, like /tmp itself: every user needs to add slot files, and the sticky bit stops one
    // user removing another's. Set after creation because DirBuilder::mode is masked by umask.
    // Fails when another user owns the directory, which means they already set it.
    let _ = set_permissions(&lease_dir, Permissions::from_mode(0o1777));
...
fn open_lease_file(lease_path: &Path) -> File {
    if let Ok(lease_file) = File::open(lease_path) {
        return lease_file;
    }
    // Another process can create the file between the open above and this one, and a file created
    // by a different user is 0644, so asking for write access then fails. Fall back to a
    // read-only open, which is all flock needs.
    match OpenOptions::new().create(true).write(true).truncate(false).open(lease_path) {
        Ok(lease_file) => lease_file,
        Err(create_error) => File::open(lease_path).unwrap_or_else(|open_error| {
            panic!("Failed to open {lease_path:?}: {create_error}, then {open_error}")
        }),
    }
}
