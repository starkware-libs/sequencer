    ...
    async fn start(&mut self) -> Result<(), ComponentError> {
        tokio::select! {
            ...
            _never = &mut self.new_block_receiver_future => {
                unreachable!("New block receiver future should never finish")
            }
...
impl StateSyncRunner {
    ...
    fn new_central_state_sync_client(
        ...
        let base_layer_source = base_layer_config.map(EthereumBaseLayerSource::new);
