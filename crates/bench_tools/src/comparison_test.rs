...
/// A measurement too noisy to conclude anything from: the point estimate is far above the limit
/// but the interval straddles it, so the run does not support calling this a regression. The
/// observed +31.59% on `transfers_sequential_benchmark_vm` had a tighter interval than this and is
