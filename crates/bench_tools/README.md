...
Supports both relative (percentage change) and absolute (time in nanoseconds) limits.

**Limits:**
- `--regression-limit`: Required. Maximum acceptable percentage change vs
baseline (e.g., 5.0 = 5%).
- `--set-absolute-time-ns-limit`: Optional. Can be specified multiple
times to set absolute time thresholds for specific benchmarks.
...
- absolute limit:
```
❌ tree_computation_flow: 28765432.10ns (EXCEEDS 28765400ns limit)
```
