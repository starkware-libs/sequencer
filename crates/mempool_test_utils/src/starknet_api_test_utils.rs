...
impl AccountTransactionGenerator {
    ...
    pub fn generate_call_contract_invoke_tx(
        &mut self,
        contract_address: ContractAddress, // self.sender_address().get_instance_address(0)
        tip: u64,
    ) -> RpcTransaction {
        assert!(
            self.is_deployed(),
        ...
        let invoke_args = invoke_tx_args!(
            ...
            sender_address: self.sender_address(),
            resource_bounds: test_valid_resource_bounds(),
            calldata: create_calldata(self.test_contract_address(), "test_call_contract", &inner_calldata.0)
        );
    ...
    pub fn generate_all_library_call_invoke_txs(&mut self) -> Vec<RpcTransaction> {
        ...
        .iter()
        .map(|(fn_name, fn_args)| self.generate_invoke_tx_library_call(fn_name, fn_args))
        .collect()
    }

    pub fn generate_all_direct_call_invoke_txs(&mut self, tip: u64) -> Vec<RpcTransaction> {
        let test_deploy_args = vec![
            self.test_contract.get_class_hash().0, // class hash
            felt!(7_u64),                          // salt
