...
impl AccountTransactionGenerator {
    ...
    fn genreate_invoke_tx_library_call(
        &mut self,
        tip: u64,
        inner_fn_name: &str,
        inner_fn_args: &[Felt],
        ...
        let invoke_args = invoke_tx_args!(
            ...
            sender_address: self.sender_address(),
            resource_bounds: test_valid_resource_bounds(),
            calldata: create_calldata(self.test_contract_address(), "test_library_call", &calldata.0),
        );
    ...
    pub fn generate_all_library_call_invoke_txs(&mut self, tip: u64) -> Vec<RpcTransaction> {
        ...
        .iter()
        .map(|(fn_name, args)| self.genreate_invoke_tx_library_call(tip, fn_name, args))
        .collect()
    }
