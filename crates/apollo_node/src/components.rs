...
pub async fn create_node_components(
    ...
    // Create the config manager runner first, as the config_manager_client is used by
    // other components below.
    let (config_manager_runner, config_manager_client) = match config
        .components
        .config_manager
        .execution_mode
    {
        ReactiveComponentExecutionMode::LocalExecutionWithRemoteDisabled => {
            ...
            let config_manager_client = Arc::new(channel_client);
            let config_manager_runner = ConfigManagerRunner::new(
                config_manager_config.clone(),
                dynamic_config_tx,
                cli_args,
            );
            (Some(config_manager_runner), Some(config_manager_client))
        }
    ...
    let batcher = match config.components.batcher.execution_mode {
        ...
        | ReactiveComponentExecutionMode::LocalExecutionWithRemoteEnabled => {
            ...
            let config_manager_client =
                config_manager_client.clone().expect("Config Manager client should be available");
            Some(
                create_batcher(
    ...
    let gateway = match config.components.gateway.execution_mode {
        ...
        | ReactiveComponentExecutionMode::LocalExecutionWithRemoteEnabled => {
            ...
            let proof_manager_client = clients
                .get_proof_manager_shared_client()
                .expect("Proof Manager client should be available");
            let config_manager_client = config_manager_client
                .clone()
