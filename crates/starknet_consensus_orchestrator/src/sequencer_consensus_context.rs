...
use starknet_l1_gas_price_types::errors::{EthToStrkOracleClientError, L1GasPriceClientError};
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn build_proposal(
        ...
        let l1_gas_price_provider_client = Arc::clone(&self.l1_gas_price_provider);
...
fn convert_to_sn_api_block_info(block_info: ConsensusBlockInfo) -> starknet_api::block::BlockInfo {
    ...
    let l1_gas_price_fri = l1_gas_price_wei * block_info.eth_to_fri_rate;
    let l1_data_gas_price_wei =
        NonzeroGasPrice::new(GasPrice(block_info.l1_data_gas_price_wei)).unwrap();
    let l1_data_gas_price_fri = l1_data_gas_price_wei * block_info.eth_to_fri_rate;
    let l2_gas_price_fri = NonzeroGasPrice::new(GasPrice(block_info.l2_gas_price_fri)).unwrap();
    let l2_gas_price_wei = l2_gas_price_fri / block_info.eth_to_fri_rate;
