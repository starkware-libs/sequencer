...
enum BuildProposalError {
    ...
    #[error("Eth to Fri Oracle error: {0}")]
    EthToFriOracle(#[from] EthToFriOracleClientError),
}
...
pub struct SequencerConsensusContext {
    ...
    eth_to_fri_oracle_client: Option<Arc<dyn EthToFriOracleClientTrait>>,
...
impl ConsensusContext for SequencerConsensusContext {
    ...
    async fn build_proposal(
        ...
        let eth_to_fri_oracle_client = self.eth_to_fri_oracle_client.clone();
...
async fn initiate_build(
    ...
    let eth_to_fri_rate = match eth_to_fri_oracle_client {
        Some(eth_to_fri_oracle_client) => {
            eth_to_fri_oracle_client.eth_to_fri_rate(timestamp).await?
        }
        None => 1,
    };
