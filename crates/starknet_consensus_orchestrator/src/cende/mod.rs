...
// TODO(dvir): use the metrics infra when it will be completed.
// TODO(dvir): add cende metrics to the dashboard when it will be completed infra-wise.
const CENDE_PREPARED_BLOB_BLOCK_NUMBER: &str = "CENDE_PREPARED_BLOB_BLOCK_NUMBER";
...
impl CendeContext for CendeAmbassador {
    ...
    #[latency_histogram("cende_prepare_blob_for_next_height_latency_seconds", false)]
    async fn prepare_blob_for_next_height(
        &self,
        blob_parameters: BlobParameters,
    ) -> CendeAmbassadorResult<()> {
        ...
        *self.prev_height_blob.lock().await = Some(aerospike_blob);
        ...
        counter!(CENDE_PREPARED_BLOB_BLOCK_NUMBER).absolute(block_number.0);
        Ok(())
    }
}

#[latency_histogram("cende_send_write_blob_latency_seconds", false)]
