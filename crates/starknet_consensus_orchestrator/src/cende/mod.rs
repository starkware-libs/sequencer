...
pub(crate) struct AerospikeBlob {
    ...
    contract_classes: Vec<(ClassHash, CentralSierraContractClass)>,
...
impl CendeAmbassador {
    ...
    async fn get_sierras_and_casms(
        &self,
        transactions: &[InternalConsensusTransaction],
    ) -> CendeAmbassadorResult<(
        Vec<(ClassHash, CentralSierraContractClass)>,
        Vec<(CompiledClassHash, CentralCasmContractClass)>,
    )> {
        let mut sierras = Vec::new();
        let mut casms = Vec::new();
        for tx in transactions {
            let InternalConsensusTransaction::RpcTransaction(rpc_transaction) = tx else {
                continue;
            };
            let InternalRpcTransactionWithoutTxHash::Declare(declare_tx) = &rpc_transaction.tx
            else {
                continue;
            };

            let class_hash = declare_tx.class_hash;
            let ContractClass::V1(casm) = self.class_manager.get_executable(class_hash).await?
            else {
                panic!("Only V1 contract classes are supported");
            };
            casms
                .push((declare_tx.compiled_class_hash, casm_contract_class_central_format(casm.0)));
            sierras.push((class_hash, self.class_manager.get_sierra(class_hash).await?)); // TODO handle error
        }
        Ok((sierras, casms))
    }
}
...
impl CendeContext for CendeAmbassador {
    ...
    async fn prepare_blob_for_next_height(
        ...
        let (contract_classes, compiled_classes) =
            self.get_sierras_and_casms(&blob_parameters.rpc_transactions).await?;
...
pub struct BlobParameters {
    ...
    pub(crate) rpc_transactions: Vec<InternalConsensusTransaction>,
