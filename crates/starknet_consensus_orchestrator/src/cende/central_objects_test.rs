...
fn central_deploy_account_tx_json() -> Value {
    let deploy_account_tx = InternalRpcDeployAccountTransaction {
...
#[tokio::test]
async fn test_transactions_conversion_to_sierra_and_casm_mappings() {
    ...
    // Prepare the input blob paramaters with a list of transaction including declare transactions.
    let declare: Vec<InternalConsensusTransaction> = (0u8..2)
        .map(|i| {
    ...
    let invokes: Vec<InternalConsensusTransaction> = (2u8..4)
        .map(|i| {
            InternalConsensusTransaction::RpcTransaction(InternalRpcTransaction {
    ...
    let transactions = [&declare[..], &invokes[..]].concat();
    let blob_parameters = BlobParameters { transactions, ..Default::default() };
