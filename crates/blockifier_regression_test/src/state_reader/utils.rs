    ...
    match (tx_type.as_str(), tx_version.as_str()) {
        ("INVOKE", version) => match version {
            ...
            "0x3" => {
                ...
                if let Some(l2_gas_value) = resource_bounds.remove("l2_gas") {
        ...
        ("DEPLOY_ACCOUNT", version) => match version {
        ...
        (tx_type, _) => Err(serde::de::Error::custom(format!("unimplemented tx type: {tx_type}"))),
    }
}
