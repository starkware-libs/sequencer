...
impl TestStateReader {
    ...
    pub fn get_block_info(&self) -> StateResult<BlockInfo> {
        ...
        if let Some(block_header_map) = json.as_object_mut() {
            ...
            let block_header: BlockHeader =
                serde_json::from_value(json).map_err(serde_err_to_state_err)?;
            let block_info: BlockInfo = block_header.try_into()?;
            return Ok(block_info);
        }

        // The json value should be an object.
