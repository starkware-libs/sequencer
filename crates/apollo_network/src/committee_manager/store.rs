...
/// Shared committee state, protected by `RwLock`.
///
/// - **Readers** (Propeller, Authenticator): acquire a read lock via `Arc<RwLock<CommitteeStore>>`.
/// - **Writer** (CommitteeManagerBehaviour): acquires a write lock to mutate state.
///
...
// ---------------------------------------------------------------------------
// Read API (used by Propeller and Authenticator via Arc<RwLock<CommitteeStore>>)
// ---------------------------------------------------------------------------
...
impl CommitteeStore {
    ...
    /// Returns `true` if the staker is a member of at least one active committee **and** does not
    /// already have a peer mapped.
    ///
    /// Used by the Authenticator during the connection handshake to decide whether to accept a
    /// new peer claiming to be this staker.
    pub fn is_valid_staker(&self, staker_id: &StakerId) -> bool {
        self.staker_ref_counts.contains_key(staker_id)
            && !self.staker_to_peer.contains_key(staker_id)
    }
}
