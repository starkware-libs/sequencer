...
fn add_peer_for_staker_succeeds() {
    let mut store = CommitteeStore::new(2);
    store.add_committee(10, committee_id(1), vec![member(1)]).unwrap();
    store.add_peer_for_staker(staker(1), get_peer_id(3)).unwrap();
    assert_eq!(store.staker_to_peer.get(&staker(1)).unwrap(), &get_peer_id(3));
    assert_eq!(store.peer_to_staker.get(&get_peer_id(3)).unwrap(), &staker(1));
}
