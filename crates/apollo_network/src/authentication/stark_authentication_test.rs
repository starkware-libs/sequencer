...
use crate::authentication::stark_authentication::{StarkAuthInitiator, StarkAuthResponder};
use crate::authentication::{AuthNegotiator, Bytes, ConnectionEnd};
...
async fn stark_authentication_protocol(
    ...
    let mut initiator_connection =
        ConnectionEnd { sender: initiator_sender, receiver: responder_receiver };
    let mut responder_connection =
        ConnectionEnd { sender: responder_sender, receiver: initiator_receiver };
    ...
    let initiator = StarkAuthInitiator {
    ...
    let responder = StarkAuthResponder {
    ...
    // Test.
    let initiator_future =
        initiator.negotiate(initiator_peer_id, responder_peer_id, &mut initiator_connection);
    let responder_future =
        responder.negotiate(responder_peer_id, initiator_peer_id, &mut responder_connection);
