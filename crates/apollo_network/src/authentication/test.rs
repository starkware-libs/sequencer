...
use super::*;
use crate::authentication::composed_noise::{ComposedNoiseConfig, NegotiatorError};
...
fn stark_auth_with_challenge(n: u8) -> StarkAuthentication {
    let challenge_and_identity = ChallengeAndIdentity {
        staker_address: ContractAddress::default(),
        public_key: PublicKey::default(),
        challenge: Challenge(Felt::from_hex_unchecked(&format!("0x{n:x}"))),
    };
...
async fn test_composed_noise_config_upgrade_with_none_negotiator() {
    ...
    let (reported_client_id, reported_server_id) =
        perform_composed_upgrade_with_negotiator_with_keys::<DummyNegotiatorType>(
...
async fn test_composed_noise_config_returns_failure_when_initiator_fails() {
    ...
    perform_composed_upgrade_with_negotiator(Some(server_negotiator), Some(client_negotiator))
        .await
...
async fn test_composed_noise_config_transfers_messages_between_peers() {
    ...
    perform_composed_upgrade_with_negotiator(Some(EvenOddNegotiator), Some(EvenOddNegotiator))
        .await
        .expect("Negotiation failed.");
}
