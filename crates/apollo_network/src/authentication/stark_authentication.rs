...
use apollo_protobuf::authentication::{ChallengeAndIdentity, SignedChallengeAndIdentity};
...
impl StarkAuthNegotiator {
    async fn negotiate_connection(
        ...
        let my_challenge_and_identity = ChallengeAndIdentity {
            ...
            challenge: Challenge(self.challenge_generator.generate().await),
        };
        let (_, other_challenge_and_identity) = tokio::try_join!(
        ...
        } = ChallengeAndIdentity::try_from(other_challenge_and_identity)?;
