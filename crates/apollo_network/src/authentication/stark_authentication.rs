...
use apollo_protobuf::authentication::{
    Challenge,
    PublicKey as AuthenticationPublicKey,
...
impl StarkAuthNegotiator {
    async fn negotiate_connection(
        ...
        // 1. Send my public key and receive other's public key.
        let my_pk = AuthenticationPublicKey { public_key: self.my_stark_public_key.0 };
        let (_, other_pk) =
            tokio::try_join!(connection_sender.send(my_pk.into()), connection_receiver.receive())?;

        let other_pk = PublicKey(AuthenticationPublicKey::try_from(other_pk)?.public_key);
        ...
        match verify_identity(other_peer_id, other_challenge, other_signature, other_pk)? {
