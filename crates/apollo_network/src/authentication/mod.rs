...
use negotiator::{Negotiator, NegotiatorOutput};
use noise::Error as NoiseError;
use tracing::error;

use crate::authentication::negotiator::NegotiationSide;
    ...
    S::Error: std::fmt::Debug,
{
        ...
        Socket: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        async move {
            ...
            if let NegotiatorOutput::DuplicatePeer(dup_peer_id) = negotiator
    ...
    S::Error: std::fmt::Debug,
{
    ...
    fn protocol_info(&self) -> Self::InfoIter {
        std::iter::once(match &self.negotiator {
            // TODO(noam.s): Check this value with leo@.
...
// TODO(noam.s): Remove the allow(dead_code) once we use the ComposedNoiseConfig in the network
// manager.
// TODO(noam.s): Move this to where it's used (network manager)
#[allow(dead_code)]
#[derive(Clone)]
...
impl Negotiator for DummyNegotiatorType {
    ...
    async fn negotiate_connection(
        ...
        _connection_sender: &mut negotiator::ConnectionSender,
        _connection_receiver: &mut negotiator::ConnectionReceiver,
