...
struct ClosureNegotiator {
    incoming_closure:
        std::sync::Arc<dyn Fn(PeerId, PeerId) -> Result<NegotiatorResult, Error> + Send + Sync>,
    outgoing_closure:
        std::sync::Arc<dyn Fn(PeerId, PeerId) -> Result<NegotiatorResult, Error> + Send + Sync>,
...
impl ClosureNegotiator {
    ...
    where
        F1: Fn(PeerId, PeerId) -> Result<NegotiatorResult, Error> + Send + Sync + 'static,
        F2: Fn(PeerId, PeerId) -> Result<NegotiatorResult, Error> + Send + Sync + 'static,
...
fn test_composed_noise_config_generates_protocol_info() {
    ...
    let negotiator = ClosureNegotiator::new(
        |_, _| Ok(NegotiatorResult::SuccessfulNegotiation),
        |_, _| Ok(NegotiatorResult::SuccessfulNegotiation),
        || PROTOCOL_NAME,
    );
...
async fn test_composed_noise_config_upgrade_passes_correct_peer_ids_to_negotiator() {
    ...
    // let server_peer_id_clone = server_peer_id.clone();
    let server_negotiator = ClosureNegotiator::new(
        move |my_peer_id, other_peer_id| {
            assert_eq!(my_peer_id, server_peer_id.clone());
            assert_eq!(other_peer_id, client_peer_id.clone());
            Ok(NegotiatorResult::SuccessfulNegotiation)
        },
    ...
    let client_negotiator = ClosureNegotiator::new(
        ...
        move |my_peer_id, other_peer_id| {
            assert_eq!(my_peer_id, client_peer_id.clone());
            assert_eq!(other_peer_id, server_peer_id.clone());
            Ok(NegotiatorResult::SuccessfulNegotiation)
        },
        || unreachable!(""),
    );
...
async fn test_composed_noise_config_returns_failure_when_initiator_fails() {
    ...
    let client_negotiator = ClosureNegotiator::new(
        |_, _| Ok(NegotiatorResult::SuccessfulNegotiation),
        |_, _| Err(Error::AuthenticationFailed),
        || unreachable!(""),
    );
