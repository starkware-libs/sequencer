...
fn create_peer_private_key(peer_index: usize) -> [u8; 32] {
    let peer_index: u64 = peer_index
        .try_into()
        .expect("usize cannot be converted to u64, what magical CPU are you using?");
    let array = peer_index.to_le_bytes();
    assert_eq!(array.len(), 8);

    let mut private_key = [0u8; 32];
    for (i, j) in [(0, 8), (8, 16), (16, 24), (24, 32)].iter().cloned() {
        private_key[i..j].copy_from_slice(&array);
    }
    private_key
}
