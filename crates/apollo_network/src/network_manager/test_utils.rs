...
pub fn create_connected_network_configs(ports: Vec<u16>) -> Vec<NetworkConfig> {
    ...
    let bootstrap_peer_multiaddr = MultiaddrVectorConfig {
        domain: vec!["127.0.0.1".to_string(); number_of_nodes],
        port: ports.iter().copied().collect(),
        peer_id: public_keys.iter().map(|pk| PeerId::from_public_key(pk)).collect(),
    };
    ports
        .into_iter()
        .zip(private_keys)
        .map(|(port, private_key)| NetworkConfig {
            port,
            bootstrap_peer_multiaddr: bootstrap_peer_multiaddr.clone(),
