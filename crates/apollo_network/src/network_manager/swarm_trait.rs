...
pub trait SwarmTrait: Stream<Item = Event> + Unpin {
    ...
    fn propeller_broadcast(&mut self, message: Bytes);
}
...
impl SwarmTrait for Swarm<mixed_behaviour::MixedBehaviour> {
    ...
    fn set_propeller_peers(
        ...
        self.behaviour_mut().propeller.register_channel_peers(PROPELLER_CHANNEL, peers)
    }

    fn propeller_broadcast(&mut self, message: Bytes) {
        self.behaviour_mut().propeller.broadcast(PROPELLER_CHANNEL, message).unwrap();
    }
}
