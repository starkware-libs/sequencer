...
pub enum NetworkError {
    ...
    #[error(transparent)]
    PropellerError(#[from] libp2p::propeller::ShredPublishError),
    #[error("Propeller peer set error: {0}")]
    PropellerPeerSetError(String),
    #[error(transparent)]
    PropellerReconstructionError(#[from] libp2p::propeller::ReconstructionError),
}
pub struct GenericNetworkManager<SwarmT: SwarmTrait> {
    ...
    propeller_messages_sender: Option<Sender<(libp2p::propeller::MessageId, Bytes)>>,
    messages_to_propeller_receiver: Receiver<(libp2p::propeller::MessageId, Bytes)>,
...
impl<SwarmT: SwarmTrait> GenericNetworkManager<SwarmT> {
    ...
    ) -> Result<PropellerChannels<T>, libp2p::propeller::PeerSetError>
    where
        T: TryFrom<Bytes> + 'static,
        Bytes: From<T>,
    {
...
pub type PropellerSender<T> = With<
    Sender<(libp2p::propeller::MessageId, Bytes)>,
    (libp2p::propeller::MessageId, Bytes),
    (T, libp2p::propeller::MessageId),
    Ready<Result<(libp2p::propeller::MessageId, Bytes), SendError>>,
    fn(
        (T, libp2p::propeller::MessageId),
    ) -> Ready<Result<(libp2p::propeller::MessageId, Bytes), SendError>>,
>;
...
pub type PropellerMessageServer<T> =
    Map<Receiver<(libp2p::propeller::MessageId, Bytes)>, PropellerReceivedMessagesConverterFn<T>>;

pub type ReceivedPropellerMessage<Message> =
    (libp2p::propeller::MessageId, Result<Message, <Message as TryFrom<Bytes>>::Error>);

type PropellerReceivedMessagesConverterFn<T> =
    fn((libp2p::propeller::MessageId, Bytes)) -> ReceivedPropellerMessage<T>;
