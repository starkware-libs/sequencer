...
#[rstest]
fn add_existing_committee_returns_empty_new_committee(mut active_committees: ActiveCommittees) {
...
fn eviction(
    ...
    #[case] removed_committee_from_epoch: Option<usize>,
    ...
    match removed_committee_from_epoch {
        Some(epoch_idx) => {
...
#[rstest]
fn duplicate_epoch_id_is_ignored(mut active_committees: ActiveCommittees) {
    let output1 = active_committees.add_epoch(1, vec![member(1)]).unwrap();
    assert!(output1.new_committee.is_some());

    // Adding the same epoch_id again should be a no-op.
    let output2 = active_committees.add_epoch(1, vec![member(2)]).unwrap();
    assert!(output2.new_committee.is_none());
    assert!(output2.removed_committee.is_none());
    assert_eq!(output2.allowed_peers, peer_ids(&[1]));
}
...
/// Regression test: verifies that the committee ID hash is stable across code changes.
#[rstest]
