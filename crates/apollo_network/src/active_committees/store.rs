...
fn sort_and_dedup_members(members: &mut [CommitteeMember]) -> Result<(), DuplicatePeerIdError> {
    members.sort_by_key(|m| m.peer_id);

    for window in members.windows(2) {
        if window[0].peer_id == window[1].peer_id {
            return Err(DuplicatePeerIdError(window[0].peer_id));
        }
    }
    Ok(())
}
...
pub struct DuplicatePeerIdError(PeerId);
...
impl ActiveCommittees {
    ...
    ) -> Result<AddEpochOutput, DuplicatePeerIdError> {
        // If the epoch already exists, ignore it to avoid corrupting ref counts.
        if self.epochs.contains_key(&epoch_id) {
            let allowed_peers = self.compute_allowed_peers();
            return Ok(AddEpochOutput {
                allowed_peers,
                new_committee: None,
                removed_committee: None,
            });
        }
