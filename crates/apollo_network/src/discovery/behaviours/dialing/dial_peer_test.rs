...
async fn external_connection_established_enters_cooldown_and_terminates(
    ...
    // Another entity established the connection.
    send_connection_established(&mut stream, peer_id);

    // Stream should be in CooldownBeforeDeletion.
    assert_stream_pending(&mut stream);
...
/// When a connection was established and the cooldown timer expires, the stream terminates.
#[rstest]
#[tokio::test(start_paused = true)]
async fn stream_terminates_after_cooldown_expires(
    retry_config: RetryConfig,
    peer_id: PeerId,
    addresses: Vec<Multiaddr>,
) {
    ...
    // Before cooldown expires, stream is pending.
    tokio::time::advance(TEST_COOLDOWN - Duration::from_millis(1)).await;
    assert_stream_pending(&mut stream);

    // After cooldown, stream terminates.
...
fn retry_config() -> RetryConfig {
    RetryConfig {
        ...
        factor: FACTOR,
        new_connection_stabilization_seconds: TEST_COOLDOWN,
    }
}
...
fn send_connection_established(stream: &mut DialPeerStream, peer_id: PeerId) {
