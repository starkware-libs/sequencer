...
impl DialPeerStream {
    ...
    fn emit_dial<T, W>(&mut self) -> ToSwarm<T, W> {
        let opts = DialOpts::peer_id(self.peer_id)
            ...
            // The `connection_limits` behaviour caps established connections at one per peer, so
            // concurrent dials cannot produce a second connection regardless of this condition.
