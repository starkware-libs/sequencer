...
impl Behaviour {
    ...
    pub fn set_peers_to_request(&mut self, peers: std::collections::HashSet<PeerId>) {
        self.kad_requesting.set_peers_to_request(peers);
    }
}
...
impl BridgedBehaviour for Behaviour {
    fn on_other_behaviour_event(&mut self, event: &mixed_behaviour::ToOtherBehaviourEvent) {
        ...
        {
            self.kad_requesting.enqueue_dials_for_matching_peers(peers);
        }
    }
}
