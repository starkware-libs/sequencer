...
class ExperimentRunner:
    ...
    def __init__(self, use_docker, num_nodes: int):
        """
        ...
        self.bootstrap_peer_id = "12D3KooWDpJ7As7BWAwRMfu1VU2WCqNjvq387JEYKDBj4kx6nXTN"
    ...
    def run_with_args(
        ...
        if self.use_docker:
            cont = self.client.containers.run(
                ...
                ports={f"{metric_port}/tcp": metric_port, f"{p2p_port}/tcp": p2p_port},
                command=args,
                extra_hosts={"host.docker.internal": "host-gateway"},
            )
            self.docker_containers.append(cont)
            ...
            exe = os.path.abspath(
                f"{self.project_root}/target/release/network_stress_test"
            )
    ...
    def run_network_stress_test_node(self, i: int):
        pr(f"Running node {i}...")
        assert i >= 0
        metric_port = 2000 + i
        p2p_port = 10000 + i
        ...
        if i == 0:
            self.bootstrap_multi_address = (
                f"/ip4/127.0.0.1/tcp/{p2p_port}/p2p/{self.bootstrap_peer_id}"
            )

    def run_network_stress_test_nodes(self):
        pr("Running network_stress_test nodes without Docker...")
        for i in range(self.num_nodes):
            self.run_network_stress_test_node(i)
    ...
    def run_experiment(self):
        ...
        self.run_network_stress_test_nodes()
        self.run_prometheus()
...
def main():
    ...
    parser.add_argument("-n", help="Number of nodes to run", type=int, default=3)
    args = parser.parse_args()

    pr("Starting network stress test experiment...")
    pr(f"This will run {args.n} nodes in a local environment without Docker.")
    with ExperimentRunner(use_docker=False, num_nodes=args.n) as runner:
