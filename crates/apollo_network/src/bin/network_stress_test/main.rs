...
fn receive_stress_test_message(
    stress_test_message: Result<StressTestMessage, Infallible>,
    _metadata: BroadcastedMessageMetadata,
) {
    let end_time = SystemTime::now();

    let received_message = stress_test_message.unwrap();
...
async fn receive_stress_test_messages(
    ...
    broadcasted_messages_receiver
        .for_each(|result| async {
            let (stress_test_message, metadata) = result;
            tokio::task::spawn_blocking(|| {
                receive_stress_test_message(stress_test_message, metadata)
            });
        })
        .await;
