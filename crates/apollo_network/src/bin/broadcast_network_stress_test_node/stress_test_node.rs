...
impl BroadcastNetworkStressTestNode {
    ...
    async fn receive_stress_test_messages_impl(message_receiver: MessageReceiver) {
        ...
        let message_index_tracker = Arc::new(Mutex::new(Default::default()));
        let tracker_ref = &message_index_tracker;
        message_receiver
            .for_each(move |message, peer_id| {
                receive_stress_test_message(message, peer_id, tracker_ref.clone())
            })
            .await;
