...
use crate::metrics::{
    monitor_process_metrics, receive_stress_test_message, seconds_since_epoch, update_broadcast_metrics,
    BROADCAST_MESSAGE_BYTES, BROADCAST_MESSAGE_BYTES_SUM, BROADCAST_MESSAGE_COUNT,
    BROADCAST_MESSAGE_SEND_DELAY_SECONDS, BROADCAST_MESSAGE_THROUGHPUT, NETWORK_RESET_TOTAL,
};
...
impl BroadcastNetworkStressTestNode {
    ...
    fn setup_explore_config(args: &Args) -> Option<ExploreConfiguration> {
        if let Mode::Explore = args.mode {
            let (cool_down, run_duration, min_throughput, min_message_size) = extract_explore_params(args);
            let explore_config = ExploreConfiguration::new(cool_down, run_duration, min_throughput, min_message_size);
            Some(explore_config)
        } else {
            None
        }
    ...
    pub async fn start_message_sender(&mut self) -> Option<BoxFuture<'static, ()>> {
        ...
        Some(
            async move {
                Self::send_stress_test_messages_impl(message_sender, &args_clone, &explore_config)
                    .await;
            }
            .boxed(),
        )
