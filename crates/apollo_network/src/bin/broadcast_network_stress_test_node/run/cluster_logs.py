...
def cluster_log():
    ...
    for i in range(num_nodes):
        # For Indexed Jobs, pods are named: job-name-index-randomsuffix
        # Use label selector to find the pod for this index
        run_cmd(
            f"timeout 5 kubectl logs -n {namespace_name} -l app=broadcast-network-stress-test,batch.kubernetes.io/job-completion-index={i} > /tmp/broadcast-network-stress-test-{i}.logs.txt",
