...
import json
from cluster_port_forward_grafana import cluster_port_forward_grafana
from utils import (
    make_timestamp,
    run_cmd,
    pr,
...
from yaml_maker import (
    ...
    get_grafana_configmap_yaml_file,
    get_grafana_deployment_yaml_file,
    get_grafana_service_yaml_file,
    get_grafana_headless_service_yaml_file,
...
def write_yaml_files(
    ...
    files = {
        ...
        "grafana-config.yaml": get_grafana_configmap_yaml_file(),
        "grafana-statefulset.yaml": get_grafana_deployment_yaml_file(),
        "grafana-service.yaml": get_grafana_service_yaml_file(),
        "grafana-headless-service.yaml": get_grafana_headless_service_yaml_file(),
        # "namespace-deleter-rbac.yaml": get_namespace_deleter_rbac_yaml_file(
        # namespace_name
        # ),
        # "namespace-deletion-cronjob.yaml": get_namespace_deletion_job_yaml_file(
...
class ExperimentRunner:
    ...
    def deploy_yaml_files(self, name_space_name: str):
        for file_name in self.deployment_file["yaml_files"]:
            ...
            # RBAC resources and namespace deletion cronjob go to default namespace
            if file_name in [
                "namespace-deleter-rbac.yaml",
                "namespace-deletion-cronjob.yaml",
            ]:
                raise NotImplementedError(
                    "RBAC resources and namespace deletion cronjob are not supported anymore"
                )
                run_cmd(
    ...
    def run_experiment(self, args: argparse.Namespace):
        ...
        pr(
            "To access Grafana, run: kubectl port-forward service/grafana-service 3000:3000 (no login required)"
        )
        pr(
...
def main():
    ...
    pr("Running cluster_port_forward_grafana.py")

    cluster_port_forward_grafana()
