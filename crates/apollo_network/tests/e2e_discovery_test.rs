...
async fn run_discovery_test(num_peers: usize, timeout: Duration) {
    ...
    // Channels for reporting connection/disconnection events from wrapper swarms.
    let (connection_sender, mut connection_receiver) =
        tokio::sync::mpsc::unbounded_channel::<(PeerId, PeerId)>();
    let (disconnection_sender, mut disconnection_receiver) =
        tokio::sync::mpsc::unbounded_channel::<(PeerId, PeerId)>();
    ...
    let result = tokio::time::timeout(timeout, async {
        loop {
            tokio::select! {
                ...
                msg = disconnection_receiver.recv() => {
                    if let Some((from, to)) = msg {
                        tracker.remove_connection(from, to);
                    }
                }
...
impl ConnectionTracker {
    fn new(all_peer_ids: &HashSet<PeerId>) -> Self {
        let num_peers = all_peer_ids.len();
        ...
        Self { connections, bootstrap_connections: HashSet::new(), ordered_peer_ids, num_peers }
    }
    ...
    fn print_progress(&self, start: tokio::time::Instant, total_expected: usize) {
        ...
        println!(
            "{elapsed:.1}s total_connections: {total}/{total_expected} bootstrap: {boot_count}/{} \
             per_peer: {per_peer}",
            self.num_peers
        );
    }
