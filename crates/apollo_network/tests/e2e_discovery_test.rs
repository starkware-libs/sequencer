...
use apollo_network::{mixed_behaviour, Bytes};
...
async fn all_peers_discover_each_other_when_given_peer_ids() {
    ...
    // Only non-bootstrap peer IDs — the bootstrap node is excluded from the full-mesh
    // connectivity check since it serves only as an initial discovery relay.
    ...
    for swarm in &mut swarms {
        let excluding_self: HashSet<PeerId> =
            all_peer_ids.iter().copied().filter(|id| id != swarm.local_peer_id()).collect();
        swarm.behaviour_mut().discovery.as_mut().unwrap().set_target_peers(excluding_self);
    }

    // Channels for reporting connection events from wrapper swarms.
    ...
    let mut tracker = ConnectionTracker::new(&all_peer_ids);
        ...
        loop {
            tokio::select! {
                msg = connection_receiver.recv() => {
                    match msg {
                        Some((from, to)) => {
                            tracker.add_connection(from, to);
                            if tracker.total_connections() >= total_expected {
                                return;
                            }
                        }
                        None => return,
                ...
                msg = bootstrap_receiver.recv() => {
                    if let Some(peer_id) = msg {
                        tracker.add_bootstrap_connection(peer_id);
                    }
                }
                _ = tick.tick() => {
                    tracker.print_progress(start, total_expected);
                }
            }
        }
    })
    ...
    // Assert full mesh connectivity was achieved within the timeout.
    if result.is_err() {
        let mut connections_per_peer: Vec<_> =
            tracker.connections.iter().map(|(id, peers)| (peers.len(), id)).collect();
        connections_per_peer.sort();
        eprintln!("\nPer-peer connection counts (sorted):");
        for (num_connections, peer_id) in &connections_per_peer {
            eprintln!("  {num_connections:>3}/{}  {peer_id}", NUM_PEERS - 1);
        }
        panic!(
            "Timed out waiting for full connectivity: {}/{total_expected} connections established",
            tracker.total_connections(),
        );
    }
}

/// Tracks peer-to-peer and bootstrap connections for progress reporting.
...
impl ConnectionTracker {
    fn new(all_peer_ids: &HashSet<PeerId>) -> Self {
        let connections = all_peer_ids.iter().map(|id| (*id, HashSet::new())).collect();
        let mut ordered_peer_ids: Vec<PeerId> = all_peer_ids.iter().copied().collect();
        ordered_peer_ids.sort_by_key(|id| id.to_string());
        Self { connections, bootstrap_connections: HashSet::new(), ordered_peer_ids }
    }

    /// Safe to unwrap: `from` is always `local_peer_id` of a peer swarm, which is guaranteed
    /// to be in `all_peer_ids` (and therefore in `self.connections`) by construction.
    ...
    fn print_progress(&self, start: tokio::time::Instant, total_expected: usize) {
        let total = self.total_connections();
        let boot_count = self.bootstrap_connections.len();
...
/// A wrapper around `Swarm<MixedBehaviour>` that reports `ConnectionEstablished` events
/// via channels, allowing tests to observe connections without modifying NetworkManager.
...
impl futures::Stream for ConnectionReportingSwarm {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if let Poll::Ready(Some(SwarmEvent::ConnectionEstablished { peer_id, .. })) = &poll {
...
fn create_swarm(bootstrap_peer_multiaddr: Option<Multiaddr>) -> Swarm<MixedBehaviour> {
    let peer_access_control = None;
    let latency_histogram = None;
