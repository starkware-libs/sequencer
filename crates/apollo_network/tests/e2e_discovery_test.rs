...
async fn run_discovery_test(num_peers: usize, timeout: Duration) {
    ...
    let (disconnection_sender, disconnection_receiver) =
        tokio::sync::mpsc::unbounded_channel::<(PeerId, PeerId)>();
    let (bootstrap_sender, bootstrap_receiver) = tokio::sync::mpsc::unbounded_channel::<PeerId>();
    ...
    tracker.run_until_full_mesh(timeout).await;
}
...
struct ConnectionsTracker {
    ...
    num_peers: usize,
    connection_receiver: tokio::sync::mpsc::UnboundedReceiver<(PeerId, PeerId)>,
    disconnection_receiver: tokio::sync::mpsc::UnboundedReceiver<(PeerId, PeerId)>,
    bootstrap_receiver: tokio::sync::mpsc::UnboundedReceiver<PeerId>,
}
...
impl ConnectionsTracker {
    ...
    /// Collects connections until full mesh or timeout, printing progress every second.
    async fn run_until_full_mesh(&mut self, timeout: Duration) {
        ...
        if result.is_err() {
            ...
            panic!(
                "Timed out waiting for full connectivity: {}/{total_expected} connections \
                 established",
