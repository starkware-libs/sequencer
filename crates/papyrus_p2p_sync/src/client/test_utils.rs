...
pub async fn run_test(max_query_lengths: HashMap<DataType, u64>, actions: Vec<Action>) {
    ...
    // create fut pending poll
    let (run_p2p_sync_sender, run_p2p_sync_receiver) = oneshot::channel();
    let mut run_p2p_sync_sender = Some(run_p2p_sync_sender);
        ...
        _ = async {
            for action in actions {
                match action {
                    ...
                    Action::RunP2pSync => {
                        run_p2p_sync_sender.take().expect("Called RunP2pSync twice").send(()).expect("Failed to send message to run P2P sync");
                    }
                }
            }
        } => {},
        res = run_p2p_sync_receiver.then(|res| async { res.expect("Failed to run P2P sync");
        p2p_sync.run().await}) => {
            res.unwrap();
            panic!("P2P sync client finished running before all actions were executed");
        }
