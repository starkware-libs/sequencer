...
async fn not_enough_classes() {
    let (state_diff_chunk, class) = create_random_state_diff_chunk_with_class(&mut get_rng());

    validate_class_sync_fails(
        vec![2],
        vec![
            Some(state_diff_chunk.clone()),
            Some(create_random_state_diff_chunk_with_class(&mut get_rng()).0),
        ],
...
async fn class_not_in_state_diff() {
    ...
    validate_class_sync_fails(
        vec![1],
        vec![Some(create_random_state_diff_chunk_with_class(&mut get_rng()).0)],
