...
use papyrus_network::network_manager::{ClientResponseManager, SqmrClientSender};
    ...
    DataOrFin<InputFromNetwork>: TryFrom<Vec<u8>, Error = ProtobufConversionError>,
{
    ...
    fn parse_data_for_block<'a>(
        client_response_manager: &'a mut ClientResponseManager<DataOrFin<InputFromNetwork>>,
    ...
    fn create_stream<TQuery: Send + From<Query> + 'static>(
        mut payload_sender: SqmrClientSender<TQuery, DataOrFin<InputFromNetwork>>,
        ...
        stream! {
            ...
            'send_query_and_parse_responses: loop {
                ...
                let mut client_response_manager = payload_sender
                    .send(
                        TQuery::from(Query {
                ...
                match client_response_manager.next_response().await {
