...
fn register_query<Data, TQuery>(
    ...
    TQuery: TryFrom<Vec<u8>, Error = ProtobufConversionError> + Send + Clone + 'static,
    Query: From<TQuery>,
{
    let query = server_query_manager.query();
    match query {
        Ok(query) => {
...
where
    ...
    Query: From<TQuery>,
{
    let query = server_query_manager.query().expect("Failed to parse inbound query.");
    let query = Query::from(query);
