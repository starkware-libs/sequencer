...
fn validation_only_with_tx_ingestion_disabled_succeeds() {
    ...
    normalize_pointer_groups(&mut config);
    // `normalize_pointer_groups` resets `validation_only`; restore the mode under test.
    config.validation_only = true;
    config.batcher_config.as_mut().expect("default has a batcher").static_config.validation_only =
        true;
...
/// Negative: the gateway's `behavior_mode` diverging from consensus and mempool fails.
#[test]
fn pointer_group_gateway_behavior_mode_mismatch_fails() {
    let mut config = SequencerNodeConfig::default();
    normalize_pointer_groups(&mut config);
    config.gateway_config.as_mut().unwrap().static_config.behavior_mode = BehaviorMode::Echonet;
    let err = config.validate_node_config().unwrap_err();
    assert!(format!("{err:?}").contains("behavior_mode"), "Unexpected error: {err:?}");
}
...
/// A stale top-level key (the flat `revert_config` the pre-jsonnet ops scripts wrote) fails to load
/// instead of being dropped.
#[test]
fn unknown_top_level_key_is_rejected() {
    let err =
        serde_json::from_value::<SequencerNodeConfig>(serde_json::json!({ "revert_config": null }))
            .unwrap_err();
    assert!(err.to_string().contains("unknown field `revert_config`"), "{err}");
}
