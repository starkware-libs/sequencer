...
pub static CONFIG_POINTERS: LazyLock<ConfigPointers> = LazyLock::new(|| {
    ...
    let revert_config_description = "The component will revert blocks up to and including this \
                                     block number. Use carefully to prevent significant revert \
                                     operations and data loss.";
    let mut revert_config_pointers: ConfigPointers = vec![
        (
            ser_pointer_target_param("revert_config", &0_u64, revert_config_description),
            set_pointing_param_paths(&[
        ...
        (
            ser_is_param_none("revert_config", true),
            set_pointing_param_paths(&[
