...
impl SequencerNodeConfig {
    ...
    fn validate_validation_only_config(&self) -> Result<(), ConfigError> {
        ...
        if self.components.gateway.execution_mode != ReactiveComponentExecutionMode::Disabled {
            return Err(ConfigError::ComponentConfigMismatch {
                component_config_mismatch: "gateway must be disabled when validation_only is true"
                    .to_string(),
            });
        }
        if self.components.http_server.is_running_locally() {
            return Err(ConfigError::ComponentConfigMismatch {
        ...
        if self.components.mempool.execution_mode != ReactiveComponentExecutionMode::Disabled {
            return Err(ConfigError::ComponentConfigMismatch {
