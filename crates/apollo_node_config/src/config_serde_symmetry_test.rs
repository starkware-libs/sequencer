...
fn mempool_p2p_config_round_trips() {
    ...
    let mut config = MempoolP2pConfig {
        transaction_batch_rate_millis: Duration::from_millis(2468),
        ..Default::default()
    };
    set_network_config_non_default(&mut config.network_config);
    assert_round_trips_ignoring_sensitive(config, &["/network_config/secret_key"]);
...
fn l1_events_scraper_config_round_trips() {
    ...
    let config = L1EventsScraperConfig {
        startup_rewind_time_seconds: Duration::from_secs_f64(5.5),
        polling_interval_seconds: Duration::from_secs_f64(6.5),
        l1_block_time_seconds: Duration::from_secs_f64(12.0),
        ..Default::default()
    };
    assert_round_trips(config);
}
...
fn l1_gas_price_provider_config_round_trips() {
    ...
    let mut config = L1GasPriceProviderConfig {
        lag_margin_seconds: Duration::from_secs_f64(33.5),
        ..Default::default()
    };
...
fn l1_gas_price_scraper_config_round_trips() {
    ...
    let config = L1GasPriceScraperConfig {
        polling_interval: Duration::from_secs_f64(7.5),
        ..Default::default()
    };
    assert_round_trips(config);
}
