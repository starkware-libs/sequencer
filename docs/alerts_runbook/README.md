...
Some alerts use a per-environment severity placeholder (`(configurable per env)`
in the index) — the actual severity is set per-environment in the deployment
config and may differ between mainnet, testnet, and integration.
...
guide exists, then `[done]` with a link. Severity `(configurable per env)` means it is set per environment via
a placeholder.
...
- `[TODO]` `batched_transactions_stuck` (configurable per env)
- `[TODO]` `batched_transactions_stuck_long_time` (configurable per env)
- `[TODO]` `consensus_block_number_stuck` (configurable per env)
- `[TODO]` `consensus_block_number_stuck_long_time` (p2)
- `[TODO]` `consensus_p2p_not_enough_peers_for_quorum` (configurable per env)
- `[TODO]` `consensus_p2p_not_enough_peers_for_quorum_long_time` (p2)
- `[TODO]` `consensus_round_high` (configurable per env)
...
- `[TODO]` `consensus_round_above_zero_multiple_times` (configurable per env)
...
- `[TODO]` `state_sync_lag` (configurable per env)
...
- `[TODO]` `gateway_low_successful_transaction_rate` (configurable per env)
...
- `[TODO]` `high_empty_blocks_ratio` (configurable per env)
- `[TODO]` `http_server_avg_add_tx_latency` (configurable per env)
- `[TODO]` `http_server_min_add_tx_latency` (configurable per env)
- `[TODO]` `http_server_p95_add_tx_latency` (p5)
- `[TODO]` `mempool_p2p_disconnections` (p4) — defined in `alert_definitions.rs`
- `[TODO]` `mempool_p2p_peer_down` (configurable per env)
...
- `[TODO]` `http_server_internal_error_ratio` (configurable per env)
- `[TODO]` `mempool_transaction_drop_ratio` (configurable per env)

### Mempool size

Source: `alert_scenarios/mempool_size.rs`

- `[TODO]` `mempool_evictions_count` (configurable per env)
- `[TODO]` `mempool_pool_size_increase` (configurable per env)

### Preconfirmed block

Source: `alert_scenarios/preconfirmed.rs`

- `[TODO]` `preconfirmed_block_not_written` (configurable per env)
...
- `[TODO]` `l1_message_no_successes` (configurable per env) — `alert_scenarios/l1_handlers.rs`
...
- `[TODO]` `eth_to_strk_success_count` (configurable per env) — `alert_scenarios/l1_gas_prices.rs`
- `[TODO]` `l1_gas_price_provider_insufficient_history` (configurable per env) — `alert_scenarios/l1_gas_prices.rs`
...
- `[TODO]` `l1_gas_price_scraper_success_count` (configurable per env) — `alert_scenarios/l1_gas_prices.rs`
