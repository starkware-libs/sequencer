...
. No sync - can be used by other nodes for discovery but not for syncing.
. Syncing from central gateway, and connected to bootstrap peer.
. Syncing using p2p and connected to bootstrap peer.

==== No sync (only discovery)
[source, bash]
----
...
* rpc.server_address
* storage.db_config.path_prefix (if there are multiple syncing nodes they require different storage directories)
====
